...
char WHAT_XCSYNTARP[] = "@(#)SYNTAX - $Id: ARP.c 3179 2023-05-06 08:32:16Z garavel $" WHAT_DEBUG;

#include "SS.h"
#include "RLR.h"

extern SXVOID	ARP_conflict_messages (struct ARC_struct *XARC, 
...
static SXVOID	oflw_ARP_items (SXINT old_size, SXINT new_size)
{
    sxuse(old_size); /* pour faire taire gcc -Wunused */
    ARP_items_set = sxba_resize (ARP_items_set, new_size + 1);
}

static SXVOID	oflw_ARPxTRANS (SXINT old_size, SXINT new_size)
{
    sxuse(old_size); /* pour faire taire gcc -Wunused */
...
static SXBOOLEAN	ARP_is_final_state (SXINT ARP_state)
{
...
static SXVOID	ARP_trans (SXINT item_init)
{
...
static	SXVOID ARP_next (SXINT ARP_state, SXINT trans)
{
...
static	SXVOID ARP_sort (SXINT ARP_state)
{
...
static SXVOID	ARP_init (struct ARC_struct *XARC, SXINT XARC_state)
{
...
static SXVOID	PARTIAL_FSA_ARP_init (SXINT bfsa1, SXINT bfsa2)
{
    SXINT	bot, is, bfsa;
...
static SXBOOLEAN	ARP_is_ambiguous (struct ARC_struct *XARC, 
				  SXINT XARC_state, 
				  SXINT ARP_state)
{
...
SXVOID call_ARP_conflict_messages (struct ARC_struct *XARC,
				 SXINT prev_XARC_state, 
				 SXINT t, 
				 SXINT next_XARC_state)
{
...
static SXVOID	ARP_construction (void)
{
...
SXBOOLEAN PARTIAL_FSA_ARP_construction (struct ARC_struct *XARC, SXINT bfsa1, SXINT bfsa2)
{
  SXINT	ARP_state;
