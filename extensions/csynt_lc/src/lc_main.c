...
#include "bstr.h"
#include "put_edit.h"
char WHAT_LCMAIN[] = "@(#)SYNTAX - $Id: lc_main.c 3234 2023-05-15 16:52:27Z garavel $" WHAT_DEBUG;
...
/* valeurs par defaut */
    ...
    sxverbosep = SXFALSE;
    is_LC = print_time = SXTRUE;
    options_set |= OPTION (LC);
    options_set |= OPTION (TIME);

    is_ELC = is_PLR = is_automaton = SXFALSE;
...
/* Decodage des options */
	...
	case AUTOMATON:
	    is_automaton = SXTRUE, options_set |= OPTION (AUTOMATON);
	    break;

	case -AUTOMATON:
	    is_automaton = SXFALSE, options_set &= noOPTION (AUTOMATON);
	    break;

	case LC:
	    is_LC = SXTRUE, is_PLR = SXFALSE, is_ELC = SXFALSE, options_set &= noOPTION (PLR), options_set &= noOPTION (ELC), options_set |= OPTION (LC);
	    break;

	case ELC:
	    is_LC = SXFALSE, is_PLR = SXFALSE, is_ELC = SXTRUE, options_set &= noOPTION (PLR), options_set &= OPTION (ELC), options_set |= noOPTION (LC);
	    break;

	case PLR:
	    is_LC = SXFALSE, is_PLR = SXTRUE, is_ELC = SXFALSE, options_set &= OPTION (PLR), options_set &= noOPTION (ELC), options_set |= noOPTION (LC);
	    break;

	case TIME:
	    print_time = SXTRUE, options_set |= OPTION (TIME);
	    break;

	case -TIME:
	    print_time = SXFALSE, options_set &= noOPTION (TIME);
	    break;
