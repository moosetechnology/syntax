...
char WHAT_DRCGSMP[] = "@(#)SYNTAX - $Id: drcg_smp.c 4199 2024-08-29 12:04:27Z garavel $" WHAT_DEBUG;
...
drcg_write (void)
{
    ...
    out_define ("SXDRCGhas_integer_op", (SXINT) (has_integer_op ? 1 : 0));
	...
	fprintf (F_drcg, ", /* %ld */ %ld", (SXINT) bot, (SXINT) seq [bot]);
    }
...
/* 0 */ (SXINT*)NULL,\n", (SXINT) rule_nb+1);
    ...
    for (prod = 1; prod <= rule_nb; prod++) {
	fprintf (F_drcg, "/* %ld */ param_nb_seq+%ld,\n", (SXINT) prod, (SXINT) clause2xseq [prod]);
    }
...
/* 0 */ 0,\n", (SXINT) rule_nb+1);
    ...
    for (prod = 1; prod <= rule_nb; prod++) {
	fprintf (F_drcg, "/* %ld */ %ld,\n", (SXINT) prod, (SXINT) prod2var_nb [prod]);
    }
...
/* 0 */ 0,\n", (SXINT) XH_list_top (SXDRCGvar_names));

    for (x = 1; x < XH_top (SXDRCGvar_names); x++) {
	fprintf (F_drcg, "/* %ld */ ", (SXINT) x);

	bot = XH_X (SXDRCGvar_names, x);
	top = XH_X (SXDRCGvar_names, x+1);

	do {
	    fprintf (F_drcg, "%ld, ", (SXINT) XH_list_elem (SXDRCGvar_names, bot));
	} while (++bot < top);
...
/* 0 */ 0,\n", (SXINT) top);
	...
	for (term = 1; term <= term_top; term++) {
		...
		kind = REF2KIND (ref);
		val = REF2VAL (ref);
		fprintf (F_drcg, "%ld /* (%s, %ld) */, ", (SXINT) ref, kind2str [kind], (SXINT) val);
	    }
...
/* 0 */ 0,\n", (SXINT) 2*SXDRCGmax_list+1);
	for (list = 1; list <= SXDRCGmax_list; list++) {
	    ref = XxY_X (list_hd, list);
	    kind = REF2KIND (ref);
	    val = REF2VAL (ref);
	    fprintf (F_drcg, "/* %ld */ %ld /* (%s, %ld) */, ", (SXINT) list, (SXINT) ref, kind2str [kind], (SXINT) val);

	    ref = XxY_Y (list_hd, list);
	    kind = REF2KIND (ref);
	    val = REF2VAL (ref);
	    fprintf (F_drcg, "%ld /* (%s, %ld) */,\n", (SXINT) ref, kind2str [kind], (SXINT) val);
	}
	...
	kind = REF2KIND (ref);
	val = REF2VAL (ref);
	fprintf (F_drcg, ", /* %ld */ %ld /* (%s, %ld) */", (SXINT) bot, (SXINT) ref, kind2str [kind], (SXINT) val);
    }
...
/* 0 */ \"\",\n", F_drcg);
	...
	fprintf (F_drcg, "/* %ld */ \"%s\",\n", (SXINT) atom, sxstrget (atom2ste [atom]));
    }
	...
	fprintf (F_drcg, "/* %ld */ \"%s\",\n", (SXINT) gvar, sxstrget (gvar2ste [gvar]));
    }
...
/* 0 */ \"\",\n", F_drcg);
    ...
    for (t = 1; t <= max_t; t++) {
	fprintf (F_drcg, "/* %ld */ \"%s\",\n", (SXINT) t, sxstrget (t2ste [t]));
    }

    fputs ("};\n", F_drcg);

    out_define ("SXDRCGis_dynam_atom", (SXINT) (da_top > 0 ? 1 : 0));
...
/* 0 */ 0,\n", F_drcg);
	...
	for (prod = 1; prod <= rule_nb+1; prod++)
	    fprintf (F_drcg, "/* %ld */ %ld,\n", (SXINT) prod, (SXINT) clause2da_disp [prod]/2);
	...
	for (prod = 1; prod <= rule_nb; prod++) {
	    ...
	    if (nb) {
		fprintf (F_drcg, "/* %ld */ ", (SXINT) prod);

		for (xda = clause2da_disp [prod]; xda < clause2da_disp [prod+1]; xda += 2) {
		    SXINT X = clause2da [xda+1];
		    fprintf (F_drcg, "%ld /* (%ld, %ld) */, ", (SXINT) (clause2da [xda]<<logvar)+X, (SXINT) clause2da [xda], (SXINT) X);
		}
...
/* 0 */ 0,\n", F_drcg);
    ...
    out_define ("SXDRCGhas_prolog", (SXINT) (has_prolog ? 1 : 0));
	...
	for (prod = 0; prod <= maxpppprod+1; prod++) {
	    fprintf (F_drcg, "/* %ld */ %ld,\n", (SXINT) prod, (SXINT) PPPprolon [prod]);
	}
	...
	for (prod = 1; prod <= maxpppprod; prod++) {
		...
		do {
		    ppp = PPPlispro [bot];
		    fprintf (F_drcg, "%ld /* %s */, ", (SXINT) ppp, ppp2str [ppp]);
		} while (++bot < top);
	...
	for (prod = 1; prod <= maxpppprod; prod++) {
		...
		do {
		    fprintf (F_drcg, "%ld, ", (SXINT) ppp_term_top);
...
void
drcg_smp (SXINT what, SXTABLES *drcg_smp_drcg_tables)
{
