...
char WHAT_TABLESOUTDPTBL[] = "@(#)SYNTAX - $Id: out_DP_tbl.c 3295 2023-05-23 12:46:26Z garavel $" WHAT_DEBUG;
...
static void out_tabint2 (FILE *file, char *name, SXINT *tab, SXINT size, SXINT top, char *elt_m1)
{
    ...
    fprintf (file, "\n\nstatic SXINT %s[%ld]={%s", name, (SXINT) size + 1, elt_m1);
...
{
    fprintf (sxstdout, "{%ld, %ld}, ", (SXINT) SXDG.r_priorities [rule].assoc,
	     (SXINT) SXDG.r_priorities [rule].value);
}

static void print_t_priorities (SXINT rule)
{
    fprintf (sxstdout, "{%ld, %ld}, ", (SXINT) SXDG.t_priorities [rule].assoc,
	     (SXINT) SXDG.t_priorities [rule].value);
}
