...
static void
Aij_m_n_hd_oflw (SXINT old_size, SXINT new_size)
{
  (void) old_size;
...
static void
Pij_m_n_hd_oflw (SXINT old_size, SXINT new_size)
{
  (void) old_size;
...
{
  SXINT prod, lhs, rhs, bot, top, rhs_Xpq, new_rhs, new_prod, Aij_rhs;
...
{
  SXINT Aij_rhs, rhs, Aij, prod, Aij_id, id, prod2, rhs2, rhs_set_id, prev_Aij_id, bot, top, cur, i, new_id, new_Aij_id, new_prod, couple;
  SXBA  nbest_set;
...
#if 0
static void
fill_kth_best_tree (SXINT Pij, double weight)
{
  SXINT Aij;
...
static void
fill_unfolded_forest (SXINT Pij, double weight)
{
...
{
  if(Aij > 0)
    printf("%s[%ld..%ld]", spf.inputG.ntstring[Aij2A(Aij)], Aij2i(Aij), Aij2j(Aij));
  else
/*     printf("Tij"); */
    printf("%s[%ld..%ld]", spf.inputG.tstring[Tij2T(-Aij)], Tij2i(-Aij), Tij2j(-Aij)); 
  return 1;
}
...
void Aij_get_m_n_best(SXINT Aij, SXINT m, SXINT p); 
...
#if EBUG_FCT
  printf("decompose_matrice_x(%ld, %ld, mat, %ld, %ld)\n", k_min, k_max, XMAX, YMAX);
#endif
...
#if EBUG_FCT
      fprintf(stderr, "on a rien trouve dans la colonne\n");
#endif
      if(rectangle_en_cours){
#if EBUG_FCT
	fprintf(stderr, "on ferme le rectangle en cours\n");
#endif
	current->x_max = x-1;
...
#if EBUG_FCT
	fprintf(stderr, "il n'y a pas de rectangle en cours -> on en cree un nouveau\n");
#endif
...
RECTANGLE *decompose_matrice_y(SXINT k_min, SXINT k_max, SXINT **mat, SXINT XMAX, SXINT YMAX)
{
  ...
  for(y=1; y <= YMAX; y++){
    ...
    if((x > XMAX) || (mat[x][y] == INVALIDE)){ 
      /* on n'a pas trouve une case x y telle que k_min <= mat[x][y] <= k_max */ 
#if EBUG_FCT
      fprintf(stderr, "on a rien trouve dans la ligne\n");
#endif
      if(rectangle_en_cours){
#if EBUG_FCT
	fprintf(stderr, "on ferme le rectangle en cours\n");
...
{
  if(l){
    printf("(X[%ld, %ld] x  Y[%ld, %ld]), ", l->x_min, l->x_max, l->y_min, l->y_max);
...
void Pij_get_m_n_best(SXINT Pij, SXINT m, SXINT p)
{
  RECTANGLE *l1, *l2, *l;
  SXINT indice;
  ...
  if(Aij_nb == 1){ /* on est sur une regle unaire */
    Aij_get_m_n_best(Bik, m,p);
    XxYxZ_set(&Pij_m_n_hd, Pij, m, p, &indice);
    Pij_m_n[indice] = NULL;
  }
  else{
    ...
    l1 = decompose_matrice_x(m, p, Pij_mat[Pij], Pij_i_max[Pij], Pij_j_max[Pij]);
    l2 = decompose_matrice_y(m, p, Pij_mat[Pij], Pij_i_max[Pij], Pij_j_max[Pij]);
...
void Aij_get_m_n_best(SXINT Aij, SXINT m, SXINT p)
{
  ...
  /* la sequence lispro se termine par 0 */
  for(hook = GET_HOOK(Aij); (Pij = spf.outputG.rhs [hook].lispro) != 0; hook++)
    if(Pij > 0)
      Pij_nb++;
  ...
  /* <A,i,j,m,p> existe deja, on ne fait rien */
  XxYxZ_set(&Aij_m_n_hd, Aij, m, p, &indice);
  if(Aij_m_n[indice]){
#if EBUG
    printf("l'entree existe deja\n");
#endif
    return;
  }
  ...
  for(i=m; (dedication_ptr = get_dedication_ptr(Aij, i)) && (i <= p); i++){
    Pij = -dedication_ptr->PijK;
    signature = dedication_ptr->signature;
    bot = XH_X(signatures_hd, signature);
...
/*     printf("\n-------------------------------------------\n");  */
    ...
    /* la sequence lispro se termine par 0 */
    for(hook = GET_HOOK(Aij), pos=0; (Pij = spf.outputG.rhs [hook].lispro) != 0; hook++){
      if(Pij > 0){
...
/*  	  B_str = sxstrget (idag.p2ff_ste [Tij2i (-Bik)]);  */
	...
	if(rhs_length == 1){
 	  printf("%s[%ld..%ld][%ld..%ld] ->\tt[%ld..%ld][%ld..%ld]\n", A_str, i, j, m,p , i,j,m,p);

/*  	  printf("%s[%d..%d][%d..%d] ->\t%s[%d..%d][%d..%d]\n", A_str, i, j, m,p , B_str, i,j,m,p);   */
	}
	...
	for(rectangle = liste_rectangles; rectangle; rectangle = rectangle->suiv){
	  printf("%s[%ld..%ld][%ld..%ld] ->", A_str, i, j, m, p);
	  printf("\t%s[%ld..%ld][%ld..%ld]", B_str, i, k, rectangle->x_min, rectangle->x_max);
...
void affiche_measures(void){
  ...
  printf("MEASURES: rectangles = (|N|=%ld, |T|=%ld, |P|=%ld, |G|=%ld)\n",N, T, P, G); 
}
