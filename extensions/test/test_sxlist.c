...
static void sxSL_move (sxSL_header *header, SXINT x, SXINT y, SXINT lgth)
{
...
{
    int 	listnb, erasenb, listsize, elem_val, i, j, l, top, lim, elem,
    scrmbl, *p;
    SXINT	k, result, *stack;
    int		subset_id;
    SXBA	subset;
    long	t1;
    ...
    printf ("list generation time = %ld\n", t1);
    ...
    printf ("sxsubset_time (delete) = %ld\n", t1);
    sxsubset_free (&sxsubset_work);
    
{
    unsigned long set_id, elem2;
	...
	for (k = 0; k < lim; k++) {
	    elem2 = (unsigned long) *p;
	    sxset_add_member (&sxset_work, /* set_id, */ elem2);
	    p++;
	}
    }
