...
char WHAT_TABLESOUTPTBL[] = "@(#)SYNTAX - $Id: out_P_tbl.c 3147 2023-05-02 13:42:48Z garavel $" WHAT_DEBUG;
...
static SXVOID	out_P_local_mess (void)
{
	...
	for (j = 1; j <= PL [i].param_no; j++) {
	    printf ("%ld,", PL [i].string_ref [j].param);
	}
...
static SXVOID	out_P_global_mess (void)
{
    ...
    copy (m, R.P_string_mess + PG [1].index, lg);
