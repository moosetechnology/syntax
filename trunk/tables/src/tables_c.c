...
char WHAT_TABLESC[] = "@(#)SYNTAX - $Id: tables_c.c 3574 2023-09-09 19:56:34Z garavel $" WHAT_DEBUG;

SXBOOLEAN		is_lig;

static SXBOOLEAN is_dynamic_parser,
               is_dynamic_scanner;
...
extern SXBOOLEAN	parser_read (struct parser_ag_item *parser_ag_ptr, char *langname);
extern SXVOID	parser_free (struct parser_ag_item *parser_ag_ptr);
...
/* extern SXVOID	paradis_free () already declared in incl/util/PP_tables.h (struct PP_ag_item *paradis_ag_ptr) ; */ 
extern SXBOOLEAN	semat_read (struct T_ag_item *T_ag, char *langname);
extern SXVOID	semat_free (struct T_ag_item *T_ag);
extern SXBOOLEAN	init_tables (char *name);
extern SXINT   	max_P_tables (void);
extern SXINT    max_S_tables (void);
extern SXVOID     sxdp_free (void);
extern SXBOOLEAN	out_lig_tables (char *);
extern SXVOID     out_T_tables (void);
extern SXVOID     out_PP_tables (void);
extern SXVOID     out_P_tables (void);
extern SXBOOLEAN  out_S_tables (void);
extern SXBOOLEAN  ds_read (char *ds_read_language_name);
extern SXBOOLEAN  dp_read (char *dp_read_language_name);
extern SXVOID     sxre_free (void);
...
static SXVOID	out_name_date (char *nom, char *lib)
{
    printf ("/* %s of sxtables for %s */\n", lib, nom);
}

static SXVOID   out_licence (void)
{
    printf ("%s\n", SXCOPYRIGHT);
}
...
static SXVOID	out_sxtables (void)
{
...
static SXBOOLEAN	sem_read (char *name)
{
    switch (PC.sem_kind) {
    case no_sem:
    case sem_by_action:
...
static SXVOID	out_sem_tables (void)
{
    switch (PC.sem_kind) {
    case no_sem:
    case sem_by_action:
...
int main (int argc, char *argv[])
{
  ...
  is_dynamic_parser = SXFALSE;
  is_dynamic_scanner = SXFALSE;
  is_lig = SXFALSE;
