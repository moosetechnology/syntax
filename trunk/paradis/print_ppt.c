...
int main (int argc, char *argv[])
{
  ...
  printf ("PP_indx_lgth = %ld\n", (SXINT) PP_ag.PP_constants.PP_indx_lgth);
  printf ("PP_schema_lgth = %ld\n", (SXINT) PP_ag.PP_constants.PP_schema_lgth);
  printf ("\n\ti\tPP_indx [i]\n");

  for (i = 1; i <= PP_ag.PP_constants.PP_indx_lgth; i++)
    printf ("\t%ld\t%ld\n", (SXINT) i, (SXINT) PP_ag.PP_indx [i]);

  printf ("\n\ti\tPP_schema [i].PP_codop\tPP_schema [i].PP_act\n");

  for (i = 1; i <= PP_ag.PP_constants.PP_schema_lgth; i++)
    printf ("\t%ld\t%ld\t\t\t%ld\n", (SXINT) i, (SXINT) PP_ag.SXPP_schema [i].PP_codop, (SXINT) PP_ag.SXPP_schema [i].PP_act);
