...
c  File name: pitcon66/pitcon66_prb4.f
c
      program main

c*********************************************************************72
c
cc MAIN is the main program for PITCON66_PRB4.
c
c  The Freudenstein-Roth function.
c
c  This version of the Freudenstein-Roth test problem is used to 
c  demonstrate the use of the fixed parameterization option.
c
c  Six runs are made:
c
c  IWORK(2)  IWORK(3)  LIM
c
c     2         0       0  Vary index, no limit points.
c     2         1       0  Index=2, no limit points.
c     2         0       1  Vary index, find limit points in index 1.
c     2         1       1  Index=2, find limit points in index 1.
c     2         0       3  Vary index, find limit points in index 3.
c     2         1       3  Index=2, find limit points in index 3.
c
...
c
      ...
      iwork(4)=0
      iwork(5)=3
      if(itry.eq.1.or.itry.eq.2)then
        iwork(6)=0
        write(*,*)'No limit points are sought.'
      elseif(itry.eq.3.or.itry.eq.4)then
        iwork(6)=1
        write(*,*)'Seek limit points in index ',iwork(6)
      elseif(itry.eq.5.or.itry.eq.6)then
        iwork(6)=3
        write(*,*)'Seek limit points in index ',iwork(6)
      endif
