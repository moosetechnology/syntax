...
c  File name: fft_openmp/fft_openmp.f
c
      program main
...
c  Purpose:
c
c    MAIN is the main program for FFT_OPENMP.
c
c  Discussion:
c
c    The complex data in an N vector is stored as pairs of values in a
c    real vector of length 2*N.
c
c  Modified:
c
c    17 April 2009
...
#include "omp_lib.h"
     ...
     &  '  Demonstrate an implementation of the Fast Fourier Transform'
      ...
      write ( *, '(a)' ) '  using OpenMP for parallel execution.'
...
c
     ...
     &        * ( 5.0D+00 * dble ( n ) * dble ( ln2 ) )
            ...
            write ( *, 
     &        '(2x,i12,2x,i8,2x,g14.6,2x,g12.4,2x,g12.4,2x,g12.4)' ) 
     &        n, nits, error, wtime, wtime / dble ( 2 * nits ), mflops
...
c
c    Input, double precision SGN, is +1 for a "forward" FFT and -1 for a "backward" FFT.
c
      implicit none
...
c
      ...
      t = mod ( 16807.0D+00 * dble ( seed ), d2 )
      seed = dble ( t )
...
c$omp parallel
c$omp&  shared ( a, b, c, d, lj, mj, mj2, sgn, w ) 
c$omp&  private ( ambr, ambu, j, ja, jb, jc, jd, jw, k, wjw )
