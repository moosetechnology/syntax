...
c  File name: nas/nas.f
c
      program main

c*********************************************************************72
c
cc MAIN is the main program for NAS.
c
c  Discussion:
c
c    This is a version of the NAS kernel benchmark program,
c    whose original version was created by David Bailey,
c    dated 17 December 1984.
c
c    Each of the tests begins by filling arrays with pseudorandom values
c    generated by the recursion:
...
c  Author:
c
c    Original FORTRAN77 version by David Bailey.
...
c*********************************************************************72
c
cc CPTIME returns a reading of the CPU or elapsed time clock.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c*********************************************************************72
c
cc COPY copies a double precision vector.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c*********************************************************************72
c
cc MXMTST is the test program for MXM.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c*********************************************************************72
c
cc MXM computes the matrix product C = A * B.
c
c  Discussion:
c
c    The function uses 4-way unrolled loops to carry out matrix multiplication.
c
c    M must be a multiple of 4.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c*********************************************************************72
c
cc FFTTST is the test program for CFFT2D1 and CFFTD2.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
      ...
      do j = 1, n
        do i = 1, m
          t1 = dmod ( f7 * t2, 1.0D+00 )
          t2 = dmod ( f7 * t1, 1.0D+00 )
          x(i,j) = dcmplx ( t1, t2 )
        end do
      end do
...
c  Results.
c
      er = dabs ( ( dble ( x(19,19) ) - ans ) / ans )
      fp = dble ( it * m * n ) * ( 2.0D+00  
     &  + 10.0D+00 * dlog ( dble ( m * n ) ) / dlog ( 2.0D+00 ) )
...
c*********************************************************************72
c
cc CFFT2D1 performs complex radix 2 FFT's on the first dimension.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
      ...
      if ( is .eq. 0 ) then
        do i = 1, m2
          ...
          w(i) = dcmplx ( dcos ( t ), dsin ( t ) )
        end do
        return
      end if
c
c  Perform forward or backward FFT's according to IS = 1 or -1.
c
      do i = 1, m
        ip(1,i) = i
      end do
...
c*********************************************************************72
c
cc CFFT2D2 performs complex radix 2 FFT's on the second dimension.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c*********************************************************************72
c
cc CHOTST is the test program for CHOLSKY.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c*********************************************************************72
c
cc CHOLSKY carries out Cholesky decomposition and back substitution.
c
c  Discussion:
c
c    The Cholesky decomposition is performed on a set of input matrices
c    which are provided as a single three-dimensional array.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
        ...
        do l = 0, nmat
          a(l,0,j) = 1.0D+00 / dsqrt ( dabs ( epss(l) + a(l,0,j) ) )
        end do
...
c*********************************************************************72
c
cc BTRTST is the test program for BTRIX.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
      ...
      do ii = 1, it
        ...
        do k = 1, kd
          call copy ( nb, bx, b )
          call btrix ( js, je, ls, le, k, jd, kd, ld, md,
     &      a, b, c, s )
        end do
      end do
...
c  Results.
c
      er = dabs ( ( s(19,19,19,1) - ans ) / ans )
      fp = dble ( it * md * ( le - 1 ) * 19165 )

      return
      end
      subroutine btrix ( js, je, ls, le, k, jd, kd, ld, md,
     &  a, b, c, s )

c*********************************************************************72
c
cc BTRIX is a block tridiagonal solver in one direction.
c
c  Discussion:
c
c    The array has four dimensions.  The routine solves along the
c    "J" index.
c
c  Licensing:
c
...
c
        do l = ls, le
          ...
          l33(l) = 1.0D+00 / 
     &      ( b(3,3,j,l) - l31(l) * u13(l) - l32(l) * u23(l) )
          u34(l)  = ( b(3,4,j,l) - l31(l) * u14(l) 
     &      - l32(l) * u24(l) ) * l33(l)
          u35(l)  = ( b(3,5,j,l) - l31(l) * u15(l) 
     &      - l32(l) * u25(l) ) * l33(l)
        end do
     ...
     &      - l53(l) * u34(l)
          l55(l) = 1.0D+00 / 
     &      ( b(5,5,j,l) - l51(l) * u15(l) - l52(l) * u25(l)
     &      - l53(l) * u35(l) - l54(l) * u45(l) )
        end do
c
c  Step 3.  Solve for intermediate vector.
c
c  A.  Construct the right hand side.
c
        if ( j .ne. js ) then
          do m = 1, 5
            do l = ls, le
...
c
        do l = ls, le
          ...
          d4 = ( s(j,k,l,4) - l41(l) * d1 - l42(l) * d2 
     &      - l43(l) * d3 ) * l44(l)
...
c
c  Step 4.  Construct U(I) = inverse(L(I))*C(I+1) by columns and store in B.
c
        if ( j .ne. je ) then
...
c
              ...
              c4 = ( c(4,n,j,l) - l41(l) * c1 - l42(l) * c2 
     &          - l43(l) * c3 ) * l44(l)
...
c
              ...
              b(2,n,j,l) = c2 - u23(l) * b(3,n,j,l) 
     &          - u24(l) * b(4,n,j,l) - u25(l) * c5
              b(1,n,j,l) = c1 - u12(l) * b(2,n,j,l) 
     &          - u13(l) * b(3,n,j,l) - u14(l) * b(4,n,j,l) 
     &          - u15(l) * c5
            end do
          end do
...
c*********************************************************************72
c
cc GMTTST is the test program for GMTRY.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c*********************************************************************72
c
cc GMTRY computes solid-related arrays.
c
c  Discussion:
c
c    This function was extracted from a vortex method program.
c    It sets up arrays needed for the computation, and performs
c    Gauss elimination on the matrix of wall-influence coefficients.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
      ...
      do l = 1, nb
        ...
        do k = 1, nwall(l)
          arcl = arcl + cdabs ( wall(k,l) - wall(1+mod(k,nwall(l)),l) )
        end do
      end do
c
...
c
        ...
        do k = 1, nwall(l)
          ...
          if ( 0.0D+00 .lt. dreal ( ( zcr(kp,l) - zcr(k,l) ) *
      ...
      do l1 = 1, nb
        ...
        do l2 = 1, nb
          ...
          do j = 1, nwall(l2)
            ...
            z1 = cdexp ( ( wall(1,l1) - zcr(j,l2) ) * pidp )
            z1 = z1 - 1.0D+00 / z1
            dum = sig2 + dreal ( z1 )**2 + dimag ( z1 )**2
            do i = 2, nwall(l1)
              zi = cdexp ( ( wall(i,l1) - zcr(j,l2) ) * pidp )
              zz = zi - 1.0D+00 / zi
              rmatrx(i0+i,j0+j) = -0.25D+00 / pi * dlog ( dum /
...
c*********************************************************************72
c
cc EMITST is the test program for EMIT.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
     ...
     &    gamma, nwall, ps, psi, refpt, rhs, rmatrx, rmom, wall, 
     &    z, zcr )
      end do
...
c  Results.
c
      er = dabs ( ( rhs(19) - ans ) / ans )
      fp = dble ( it * ( 56 * nv + nb * nw 
     &  * ( 97 + 44 * nv + 2 * nb * nw ) ) )
...
c*********************************************************************72
c
cc EMIT creates new vortices according to certain boundary conditions.
c
c  Discussion:
c
c    This function was extracted from a vortex method program.
c    It emits new vortices to satisfy the boundary condition.
c    It also finishes computing pressure, forces, and other quantities.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
c  Store exp(z(i)) and exp(-z(i)) to reduce work in inner loop.
c
c  Note that the NV used here is a variable, whereas the NV in the
c  calling program is a constant.  They are separate quantities.
c
      nv = 1000
      pidp = pi / period

      do i = 1, nv
        expz(i) = cdexp ( z(i) * pidp )
      ...
      do l = 1, nb
        ...
        do k = 1, nwall(l)
          ...
          expwkl = cdexp ( wall(k,l) * pidp )
          expmwk = 1.0D+00 / expwkl

          sps = 0.0D+00
          do i = 1, nv
            dum3 = expz(i) * expmwk - expwkl * expmz(i)
            ps(i) = gamma(i) * dlog ( dreal ( dum3 ) ** 2 +
     &        dimag ( dum3 ) ** 2 + sig2 )
            sps = sps + ps(i)
          end do
...
c
        force(l) = force(l) 
     &    * dcmplx ( 0.0D+00, 2.0D+00 / ( delt * chord * cupst ) )
...
c*********************************************************************72
c
cc VPETST is the test program for VPENTA.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
      ...
      do i = 1, it
        call copy ( lf, fx, f )
        call vpenta ( jl, ju, kl, ku, nja, njb, a, b, c, d, e, f,
     &    x, y )
      end do
...
c  Results.
c
      er = dabs ( ( f(19,19,1) - ans ) / ans )
      fp = dble ( it * ku * ( 40 * ku - 53 ) )

      return
      end
      subroutine vpenta ( jl, ju, kl, ku, nja, njb, a, b, c, d, e, f,
     &  x, y )

c*********************************************************************72
c
cc VPENTA inverts 3 pentadiagonal systems simultaneously.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
      ...
      do j = jl + 2, ju - 2
        do k = kl, ku
          ...
          rldi = 1.0D+00 / rld
          f(j,k,1) = ( f(j,k,1) - rld2 * f(j-2,k,1) 
     &      - rld1 * f(j-1,k,1) ) * rldi
          f(j,k,2) = ( f(j,k,2) - rld2 * f(j-2,k,2) 
     &      - rld1 * f(j-1,k,2) ) * rldi
          f(j,k,3) = ( f(j,k,3) - rld2 * f(j-2,k,3) 
     &      - rld1 * f(j-1,k,3) ) * rldi
          x(j,k) = ( d(j,k) - rld1 * y(j-1,k) ) * rldi
          y(j,k) = e(j,k) * rldi
        end do
      ...
      do k = kl, ku
        ...
        rld = c(j,k) - ( rld2 * y(j-2,k) + rld1 * x(j-1,k) )
        rldi = 1.0D+00 / rld
        f(j,k,1) = ( f(j,k,1) - rld2 * f(j-2,k,1) - rld1 * f(j-1,k,1) ) 
     &    * rldi
        f(j,k,2) = ( f(j,k,2) - rld2 * f(j-2,k,2) - rld1 * f(j-1,k,2) ) 
     &    * rldi
        f(j,k,3) = ( f(j,k,3) - rld2 * f(j-2,k,3) - rld1 * f(j-1,k,3) ) 
     &    * rldi
        x(j,k) = ( d(j,k) - rld1 * y(j-1,k) ) * rldi
      end do
