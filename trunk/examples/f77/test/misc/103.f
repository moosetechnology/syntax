...
C                      L < K.
C             IER = -3 if an error flag is returned by SWAP
...
C
      LPL2 = LEND(N2)
      IF ( (BDRY  .OR.  LEFT(XR,YR,ZR,XL,YL,ZL,X1,Y1,Z1))
     .     .AND.  (LIST(LPL2) .LT. 0  .OR.
     .      LEFT(XL,YL,ZL,XR,YR,ZR,X2,Y2,Z2)) ) GO TO 2
C
...
C     X, Y, and Z.
C
    2 CALL SWAP (NL,NR,N1,N2, LIST,LPTR,LEND, LP21)
      IWL = IWL + 1
      IF (NL .LE. N1) THEN
        IWK(1,IWL) = NL
      ELSE
