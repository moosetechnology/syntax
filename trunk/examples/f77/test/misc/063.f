...
c  File name: paranoia/paranoia.f
c
      program paranoia

cc PARANOIA investigates floating point arithmetic.
c
        COMMON /STDIO/ IN, OUT
        INTEGER IN, OUT
        COMMON /PGNUM/PGNUMB
...
c
cc COMMUTE tests for commutative multiplication.
c
        COMMON /STDIO/ IN, OUT
        INTEGER IN, OUT
        COMMON /GLOBAL/ FAILS, SDEFCT, DEFECT, FLAWS, RADIX, ULPPLS,
...
c
cc RANDOM computes a "somewhat" randomized number.
c
...
2950    X=X9+R9
        ...
        SUBROUTINE EXTRA
c
cc EXTRA tests for extra precision in subexpressions.
c
        COMMON /STDIO/ IN, OUT
        INTEGER IN, OUT
        COMMON /GLOBAL/ FAILS, SDEFCT, DEFECT, FLAWS, RADIX, ULPPLS,
...
1851    FORMAT(' That feature is not tested further by this program.')
        ...
        SUBROUTINE GUARD
c
cc GUARD checks for guard digits and normalization in subtraction.
c
        COMMON /STDIO/ IN, OUT
        INTEGER IN, OUT
        COMMON /GLOBAL/ FAILS, SDEFCT, DEFECT, FLAWS, RADIX, ULPPLS,
...
c
cc INTRO prints the introduction.
c
        INTEGER MILES
        COMMON /STDIO/ IN, OUT
        INTEGER IN, OUT
...
c
cc LOGIT writes information to a checkpoint file.
c
        INTEGER MILE
...
c
cc CMPXY compares X and Y=Z**Q for equality.
c
        COMMON /STDIO/ IN, OUT
        INTEGER IN, OUT
        COMMON /GLOBAL/ FAILS, SDEFCT, DEFECT, FLAWS, RADIX, ULPPLS,
...
c
cc PRT2 prints N, and pauses if N > 0.
c
        INTEGER N, MILES
        COMMON /STDIO/ IN, OUT
        INTEGER IN, OUT
...
c
cc PWRCMP compares Z**I with Z*Z*...*Z.
c
        COMMON /GLOBAL/ FAILS, SDEFCT, DEFECT, FLAWS, RADIX, ULPPLS,
     ~        ULPMIN, PRECIS, W, MULGRD, DIVGRD, SUBGRD, A1, ONEMIN
        INTEGER         FAILS, SDEFCT, DEFECT, FLAWS
...
4020    RETURN
        ...
        SUBROUTINE PRTCNT (N)
c
cc PRTCNT prints the number of discrepancies.
c
        INTEGER N
        COMMON /STDIO/ IN, OUT
        INTEGER IN, OUT
...
c
cc BADSQR reports on errors involving square of the square root.
c
        INTEGER SFLAG
...
c
cc OVERF tests overflow threshholds.
c
        COMMON /STDIO/ IN, OUT
        INTEGER IN, OUT
        COMMON /GLOBAL/ FAILS, SDEFCT, DEFECT, FLAWS, RADIX, ULPPLS,
...
c
cc PAGE waits for the user to press RETURN.
c
        COMMON /STDIO/ IN, OUT
        INTEGER IN, OUT
        COMMON /GLOBAL/ FAILS, SDEFCT, DEFECT, FLAWS, RADIX, ULPPLS,
...
c
cc PARTUF tests for partial underflow.
c
        LOGICAL RESTRT
C
        COMMON /STDIO/ IN, OUT
...
c
cc POWER tests the calculation of Y**X.
c
        COMMON /STDIO/ IN, OUT
        INTEGER IN, OUT
        COMMON /GLOBAL/ FAILS, SDEFCT, DEFECT, FLAWS, RADIX, ULPPLS,
...
c
cc RADX finds the radix and precision.
c
        COMMON /STDIO/ IN, OUT
        INTEGER IN, OUT
        COMMON /GLOBAL/ FAILS, SDEFCT, DEFECT, FLAWS, RADIX, ULPPLS,
...
c
cc ROUND tests rounding in multiplication, division, addition and subtraction.
c
        INTEGER MILES
C
C
...
c
cc SMLINT performs tests on small integers.
c
        COMMON /STDIO/ IN, OUT
        INTEGER IN, OUT
        COMMON /GLOBAL/ FAILS, SDEFCT, DEFECT, FLAWS, RADIX, ULPPLS,
...
c
cc SQUARE tests the square root function.
c
        COMMON /STDIO/ IN, OUT
        INTEGER IN, OUT
        COMMON /GLOBAL/ FAILS, SDEFCT, DEFECT, FLAWS, RADIX, ULPPLS,
...
c
cc SQERR asseses error in SQRT ( X * X ) - X.
c
        COMMON /STDIO/ IN, OUT
        INTEGER IN, OUT
        COMMON /GLOBAL/ FAILS, SDEFCT, DEFECT, FLAWS, RADIX, ULPPLS,
...
c
cc SQRTDX tests the SQRT function.
c
        COMMON /GLOBAL/ FAILS, SDEFCT, DEFECT, FLAWS, RADIX, ULPPLS,
     ~        ULPMIN, PRECIS, W, MULGRD, DIVGRD, SUBGRD, A1, ONEMIN
        INTEGER         FAILS, SDEFCT, DEFECT, FLAWS
...
c
cc UNDERF tests underflow threshholds.
c
        INTEGER           MILES, NUMTRY, FROM
C        MILES  ... NUMBER OF MILESTONE REACHED SO FAR IN TESTING
C        NUMTRY ... NUMBER OF TIMES TO TRY RANDOM TRIALS
...
c
cc ZEROS investigates division by zero.
c
        INTEGER MILES
C
        COMMON /STDIO/ IN, OUT
...
6130    MILES = 220
      ...
      SUBROUTINE BADMIL
c
cc BADMIL reports on unrecognizable milestones.
c
      COMMON /STDIO/ IN, OUT
      INTEGER IN, OUT
      WRITE(OUT,11110)
...
c
cc IDINT converts a double precision value to an integer.
c
        DOUBLE PRECISION X
        REAL Y
        Y=X
