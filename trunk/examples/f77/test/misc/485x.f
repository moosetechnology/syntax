...
c  File name: fd1d_burgers_lax/fd1d_burgers_lax.f
c
      program main

c*********************************************************************72
c
cc FD1D_BURGERS_LAX: nonviscous Burgers equation, Lax-Wendroff method.
c
c  Licensing:
c
...
c
      ...
      write ( *, '(a)' ) 
     &  '  Solve the non-viscous time-dependent Burgers'
      write ( *, '(a)' ) '  equation using the Lax-Wendroff method.'
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) '  Equation to be solved:'
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) '    du/dt + u * du/dx = 0'
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) '  for x in [ a, b ] and t in [t_init, t_last]'
      write ( *, '(a)' ) ' '
...
c
      ...
      call r8vec_amax ( n, un, u_max )
      stability = ( dt / dx ) * u_max
...
c
     ...
     &      / dble ( step_num        )
        ...
        do i = 2, n - 1
          un(i) = uo(i) 
     &      - ( dt    / dx  ) * ( uo(i+1)**2 - uo(i-1)**2 ) 
     &      + 0.5D+00 * ( dt**2 / dx**2 ) * 0.5D+00 *
     &      (   ( uo(i+1) + uo(i) ) * ( uo(i+1)**2 - uo(i)**2 ) 
     &        - ( uo(i) + uo(i-1) ) * ( uo(i)**2 - uo(i-1)**2 ) )
        end do
...
c
c    Output, double precision AMAX, the value of the entry
...
c
c    Input, double precision STABILITY, the stability factor, which should be 
c    no greater than 1.
c
      implicit none
