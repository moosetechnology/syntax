...
c
c  CPU_TIME is actually a FORTRAN90 routine, which is not guaranteed to
c  be available to a FORTRAN77 program.
c
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) 
...
c
      ...
      double precision x
c
c  Statement function for the pair potential and its derivative.
c
c  This potential is a harmonic well which smoothly saturates to a
c  maximum value at PI/2.
c
      v(x) = ( sin ( min ( x, PI2 ) ) )**2
...
c
      do j = 1, np
        do i = 1, nd
          pos(i,j) = box(i) * r8_uniform_01 ( seed )
          vel(i,j) = 0.0D+00
          acc(i,j) = 0.0D+00
        end do
      end do
