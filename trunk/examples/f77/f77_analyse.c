...
char WHAT_F77ANALYSE[] = "@(#)SYNTAX - $Id: f77_analyse.c 4558 2025-01-06 14:17:37Z garavel $";
...
/* Attention : on ne store/restore pas buflength car si le buffer de sxsrcmngr 
   a changé (longueur, emplacement, ...) on conserve cette nouvelle def */
...
static SXINT seek_kw (char *string)
{
	...
	if (strlen (string) > 6) {
	    c = string [6];
	    i = c == 'R' ? INTEGER_p : (c == 'I' ? IMPLICIT_p : (c == 'S' ? INTRINSIC_p : UNKNOWN_p));
	}
	...
	if (strlen (string) > 4) {
	    c = string [1];
	    i = c == 'R' ? (string [2] == 'I' ? PRINT_p : PROGRAM_p) : (c == 'A' ? PAUSE_p : UNKNOWN_p);
	}
...
static bool check_for_a_lhs (SXINT tok_no)
{
    ...
    */
    ...
    for (;;) /* Pour avoir un point de sortie unique de la fonction */
    {
...
This one, and all others in the sequel, are converted into the corresponding upper case version.",
    ...
    case 8:
      /* @8 : On est sur "(", si is_implicit_statement => @Set(1) */
#if EBUG
      print_srcmngr_curchar ();
#endif
