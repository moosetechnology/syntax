...
\date{
		mis à jour par Benoît Sagot en juillet 2008 \\
		mis à jour par Hubert Garavel en 2022--2023 \\
		édition du \today.
}
...
Ce document permet l'utilisation des modules de production de traducteurs déterministes du système
{\SYNTAX}, implanté sous {\UNIX}; il décrit donc une sous-partie de la version 6.0 de SYNTAX.

Le système SYNTAX dans son ensemble est un logiciel libre, distribué sous licences CeCILL et
CeCILL-C\footnote{Le code source de SYNTAX est distribué avec deux licences différentes selon les
  fichiers:\begin{itemize}
  \item le contenu récursif de src/ et de incl/ et le contenu non-récursif du dossier racine de
    SYNTAX sont distribué sous licence CeCILL-C
  \item le contenu récursif des autres répertoires est distribué sous licence CeCILL.
  \end{itemize}
  Tout le code produit par SYNTAX doit être considéré comme étant sous licence CeCILL-C. Les
  licences de la famille CeCILL sont décrites sur leur page internet: \url{www.cecill.info}.}. Les
sources de SYNTAX sont hébergées sur la GForge de
Renater\footnote{\url{http://sourcesup.renater.fr/projects/syntax}}. 

Tous commentaires, remarques, questions et suggestions seront bienvenus, qu'ils concernent le
système lui-même ou le présent manuel. 

\par\vfil\small

Copyright {\copyright} 1988-2023 INRIA.
...
Les modules principaux de {\SYNTAX} permettant la construction de traducteurs déterministes sont les
suivants\,:
\begin{itemize}
...
{\BNF} est le processeur de base pour la mise sous forme interne des définitions syntaxiques\,; il
lit une grammaire non contextuelle, effectue quelques vérifications simples de cohérence, et produit
une forme interne de cette grammaire (des ``tables'') qui sera utilisée par les autres processeurs.

La grammaire d'entrée est écrite dans un langage proche de la ``Backus-Naur Form'' (BNF) bien
connue.  Les non-terminaux et les terminaux sont distingués lexicalement.  Chaque alternative donne
lieu à une production différente.

{\BNF} accepte des grammaires ambiguës, à condition que ces
ambiguïtés puissent être levées par la donnée de niveaux de
...
unité lexicale en avance ({\em lookahead}) peuvent l'être de plusieurs autres
manières\,:
\begin{itemize}
...
employées par {\SYNTAX} ont la propriété du préfixe
valide, ce qui garantit qu'une erreur est détectée dès que la
...
valeurs est décrit en~C (processeur {\SEMC}).

Dans les années 1980, il existait aussi un processeur {\TABACT}
permettant décrire ce calcul en {\PASCAL}. Plusieurs
...
{\SYNTAX} comprend un certain nombre d'utilitaires dont le code est
fourni en exemple\,:
\begin{itemize}
...
d'ambiguïté que posent les terminaux commençant par un des
caractères
\begin{quotation}
 \verb|; ~ " < % @ & #|
\end{quotation}
...
$\alpha\beta'$ soit un préfixe valide d'une sentence du langage.  Or
...
On trouvera en annexe~\ref{annexe:ambig.bnf} une grammaire ambiguë
...
certain cas d'ambiguïté (le cas général est indécidable). Si
{\CSYNT} détecte une ambiguïté, deux dérivations droites
...
Pour un conflit donné, si aucune ambiguïté a été détectée,
...
<EXPR>	\>= \%IDENT		\>;
 \end{tabbing}
\end{figure}
Cette grammaire est de toute évidence ambiguë\,; il est cependant
possible de résoudre aisément les conflits, comme le montre la
figure~\ref{fig:expr.prio}.
\begin{figure}[hbtp] \centering
 \caption{Désambiguïsation de la grammaire d'expressions.}
	\label{fig:expr.prio}
 \tt
 \begin{tabbing}
<EXPR>\ \ \==\ <EXPR> + <EXPR>\ \ \=;	\kill
...
Les règles de {\sl désambiguïsation\/} sont utilisées par {\CSYNT}
...
\verb|sxcheck_keyword| qui permet de vérifier si une chaîne est ou non
...
l'analyseur lexical appelle \verb|sxcheck_keyword| qui retourne\,:
\begin{itemize}
...
par une expression régulière puis utilisation de la fonction \verb|sxcheck_keyword|
...
actions système (et de générer la fonction \verb|sxcheck_keyword|).  Or
...
ambiguïtés.  Soit un langage où ``\verb"*"'' et ``\verb"**"'' sont
...
{\SYNTAX} permet trois types distincts de traitement sémantique\,:
\begin{itemize}
...
d'évaluation d'{\sl attributs sémantiques}.
 ...
 \item soit en attachant des {\sl attributs synthétisés\/} aux 
non-terminaux de la grammaire et en calculant ces attributs synthétisés 
au moyen d'actions sémantiques attachées aux règles de la grammaire 
et exécutées pendant l'analyse syntaxique. Ces attributs calculent des
fragments de l'arbre abstrait complet, lequel est généralement obtenu
par un attribut calculé sur le non-terminal axiome de la grammaire.
\end{itemize}

Ces trois possibilités sont obtenues respectivement par utilisation
des constructeurs {\SEMACT}, {\SEMAT} et {\SEMC}.
...
La macro ``\verb|SXVISITED|'' s'étend en un pointeur vers le n{\oe}ud
...
\begin{center} \tt
 \begin{tabular}{rcl}
${\tt B}_1$ &	= & {\tt sxbrother (SXVISITED, 1)}	\\
SXVISITED &	= & {\tt sxbrother (SXVISITED, SXVISITED->position)} \\
${\tt B}_n$ &	= & {\tt sxbrother (SXVISITED, $n$)}	\\
& & {\rm avec } $n$ = {\tt SXVISITED->father->degree}	\\[5pt]
${\tt S}_1$ &	= & {\tt sxson (SXVISITED, 1)}		\\
${\tt S}_i$ &	= & {\tt sxson (SXVISITED, $i$)}		\\
${\tt S}_m$ &	= & {\tt sxson (SXVISITED, $m$)}		\\
& & {\rm avec } $m$ = {\tt SXVISITED->degree}
 \end{tabular}
\end{center}
...
\verb|SXVISITED->brother| plutôt que par \verb|sxbrother (SXVISITED, SXVISITED->position + 1)|.

 \item Si l'on est en visite héritée, le frère de gauche (s'il
existe) peut être accédé par \verb|SXLEFT| plutôt que par
\verb|sxbrother (SXVISITED, SXVISITED->position - 1)|.
 \item En visite synthétisée, le fils le plus à droite peut être
accédé par \verb|SXLAST_ELEM| plutôt que par
\verb|sxson (SXVISITED, SXVISITED->degree)|.
\end{itemize}
...
(\verb|SXINHERITED| ou \verb|SXDERIVED|), en utilisant la macro
...
\section[SEMC~: Attributs synthétisés calculés par actions sémantiques]
	{\SEMC~: Attributs synthétisés calculés par actions sémantiques}

Une présentation synthétique du processeur {\SEMC} est fournie par le
rapport Bouchon-88.pdf qui fait partie de la documentation de {\SYNTAX}.
...
--  scan_act (SXINIT)  : erases the source text from beginning until the
...
void  bnf_found_bad_beginning_of_rule ()
{
...
bool lecl_scan_act (SXINT entry, SXINT act_no)
{
    switch (entry) {
    case SXOPEN:
    case SXCLOSE:
    ...
    case SXACTION:
        ...
        case 1:
            ...
            if (sxsrcmngr.current_char
                 !=
                 sxsvar.sxlv_s.token_string [sxsvar.sxlv.ts_lgth - 1])
                sxerror
...
{
    switch (entry) {
    case SXOPEN:  /* appele avant l'analyse des textes source. On prepare
                   les donnees ne dependant que du langage source et
                   non des textes source */
        ...
        break;

    case SXINIT:  /* appele avant l'analyse d'un texte */
        ...
        break;

    case SXACTION:/* appele a chaque reduction syntaxique */
        action (arg); /* arg == action_no !!!! */
        break;

    case SXERROR: /* appele en cas d'erreur de syntaxe non corrigible */
        ...
        break;

    case SXFINAL: /* appele apres l'analyse syntaxique d'un texte */
        ...
        break;

    case SXSEMPASS: /* appele apres l'analyse syntaxique d'un texte*/
        ...
        break;

    case SXCLOSE: /* appele apres l'analyse de tous les textes source */
        ...
        break;
...
case VOCABULARY_S_n :/* SXVISITED->name = {ACTION_n, GENERIC_TERMINAL_n,
...
\section{Une grammaire ambiguë}
	\label{annexe:ambig.bnf}

On trouvera ci-dessous une grammaire ambiguë illustrant le problème
...
bool XNT_parsact (entry, action_no)
    int         entry, action_no;
{
    switch (entry) {
    case SXOPEN:
...
bool NDL_parsact (SXINT entry, SXINT action_no)
{
    switch (entry) {
    case SXOPEN:
    case SXCLOSE:
    ...
    int         action_no;
{
    ...
    case SXCLOSE:
        llev_svar = sxsvar;
        sxsvar = hlev_svar;
        (*(llev_svar.sxtables->SX_scanner)) (entry, &hlev_tables);
        hlev_svar = sxsvar;
        sxsvar = llev_svar;
        return;
    ...
    case SXACTION:
        ...
        case 0 /* Call From Level 0 (llev scanner) */ :
            ...
            (*(sxplocals.SXP_tables.P_scan_it)) (); /* Quick Scanner Call */
            hlev_svar = sxsvar;
            sxsvar = llev_svar;

            if (sxsrcmngr.current_char != '}')
                sxerror (source_coord, sxsvar.sxtables->err_titles [2][0], "%sComment not closed.",
                         sxsvar.sxtables->err_titles [2]+1);
        ...
        case 1 /* Call From Level Greater Than 0 (hlev scanner) */ :
            ...
            if (sxsrcmngr.current_char != '}')
                sxerror (source_coord, sxsvar.sxtables->err_titles [2][0], "%sSub-Comment not closed.",
                         sxsvar.sxtables->err_titles [2]+1);
...
bool your_scan_act (SXINT entry, SXINT act_no)
{
    switch (entry) {
    case SXOPEN:
    case SXCLOSE:
