...
char WHAT_LALR1[] = "@(#)SYNTAX - $Id: LALR1.c 3502 2023-08-20 19:31:52Z garavel $" WHAT_DEBUG;
...
extern SXBOOLEAN	check_lr1 (SXINT xac1, SXBA t_set, SXINT nt_trans1, SXINT nt_trans2, SXBOOLEAN is_duplicate, SXBOOLEAN is_sample_path);
extern SXBOOLEAN	check_ambig (SXINT t, SXINT item, SXINT nt_trans);
extern SXBOOLEAN	open_listing (SXINT kind, char *ln, char *ME_open_listing, char *component_name);
...
static char	star_71 [] = "***********************************************************************";
static SXBOOLEAN	is_lalr1, is_lr1, is_ambiguous;
...
static SXBOOLEAN	is_old_StNt (SXINT state, SXINT nt, SXINT *StNt)
{
    /* Met dans StNt l'index dans le tableau xor_StNts du couple (state, nt).
       Si ce couple n'existait pas, il est insere et la fonction retourne SXFALSE */
...
static SXVOID	put_newr_in_includes (SXINT X1, SXINT X2)
{
...
static SXVOID	put_r_in_includes (SXINT X1, SXINT X2)
{
...
static SXVOID	lookback (SXINT state, SXINT lookback_item, SXINT StNt)
{
...
SXVOID	compute_includes (SXINT xac1)
{
    SXINT	y, lim;
	...
	for (; y < lim; y++, orig++, xplxi++) {
	    ...
	    is_old_StNt (xac1, ++ntbidon, orig) /* SXFALSE */ ;
...
SXVOID	filprd (SXINT pred, SXINT suc)
{
    maj (&xlstprd, (SXINT)6);
...
SXBOOLEAN		t_in_deriv (SXINT t, SXINT t_in_deriv_item)
{
...
static SXVOID	erase (SXINT StNt, SXINT StNt1)
{
    SXINT	x, y;
...
static SXVOID	cutback (SXINT St1, SXINT cutback_item, SXINT StNt, SXINT q, SXINT q1, SXBOOLEAN to_be_cut)
{
    SXINT	x, St;
    SXINT		StNt1;
	...
	for (x = S1 [St1].pprd; x != 0; x = lstprd [x].f2) {
	 ...
	 if (to_be_cut) {
	is_old_StNt (St1, bnf_ag.WS_NBPRO [bnf_ag.WS_INDPRO [cutback_item].prolis].reduc /* A */ , &StNt1) /* SXTRUE */ ;
	erase (StNt, StNt1);
    }
}
...
SXVOID	cut (SXINT St, SXINT q, SXINT q1)
{
	...
	if (xor_StNts [StNt].X1_head != 0) {
		...
		while (xtant < lim) {
			...
			cutback (St, cut_item, StNt, q, q1, SXFALSE);
		}
	    }
	    else {
		cutback (St, -cut_xptant, StNt, q, q1, SXFALSE);
	    }
	}
    }
}
...
SXVOID	fill_items_set (SXINT *fill_items_set_nt_trans_set, SXINT *fill_items_set_nt_trans_stack, SXINT nt_trans)
{
...
/* Calcul de la relation includes */
	...
	if (is_lr1_constructor) {
	    ...
	    SXBA	la_set_r1;
	    SXINT		r1;
	    SXBOOLEAN	LALR1_is_lalr1, LALR1_is_lr1;
...
/* On regarde s'il y a un conflit Shift/Reduce, si oui => non LR(1) */
			...
			t = S1 [xac1].shftt;
			lim = S1 [t + 1].ptt;
			LALR1_is_lr1 = LALR1_is_lalr1 = SXTRUE;
			...
			for (xt1 = S1 [t].ptt; LALR1_is_lr1 && xt1 < lim; xt1++) {
				...
				if (sxba_bit_is_set (look_ahead_sets [x], -t)) {
				    ...
				    LALR1_is_lr1 = LALR1_is_lalr1 = SXFALSE;
				    break;
				}
			    }
			}
			...
			for (xtr1 = base_xtr1; LALR1_is_lr1 && xtr1 < (lim_xtr1-1); xtr1++) {
				...
				if (!sxba_is_empty (sxba_and (t_set, la_set_r1))) {
				    ...
				    LALR1_is_lalr1 = SXFALSE;

				    if (!check_lr1 (xac1, t_set, plxi [r1], plxi [x], SXFALSE, SXFALSE)) {
					LALR1_is_lr1 = SXFALSE;
					break;
				    }
				}
			    }
			...
			if (LALR1_is_lr1) {
				...
				for (xtr1 = base_xtr1; LALR1_is_lalr1 && xtr1 < (lim_xtr1-1); xtr1++) {
					...
					if (!sxba_is_empty (sxba_and (t_set, la_set_r1))) {
					    /* Conflit Reduce/Reduce => non LALR(1) */
					    LALR1_is_lalr1 = SXFALSE;
					    check_lr1 (xac1, t_set, plxi [r1], plxi [x], SXTRUE, SXFALSE);
...
/* Floyd_Evans des transitions terminales */
	...
	prio_read (-limt, limpro, &t_priorities, &r_priorities, ln);
	is_fsa_built = SXFALSE;
	xlas = 1;
...
/* Recapitulatif au terminal des conflits */
	...
	if (is_automaton && open_listing (1, ln, ME, ".la.l")) {
	    VARSTR	varstring;
	    SXINT	xac1, x, z;
	    SXINT		lim;
		...
		if (xac1 != 1) {
			...
			put_edit_apnb ((SXINT) lstprd [S1 [xac1].pprd].f1);
			put_edit_nl (1);
		    }
		    else {
			SXINT	j;
...
/* On verifie que toutes les reductions sont generees.
   Le traitement des conflits ayant pu en supprimer. */
    ...
    if (is_long_listing && open_listing (1, ln, ME, ".la.l")) {
	struct state	*astate;
	...
	put_edit_apnb ((SXINT) init_state);
