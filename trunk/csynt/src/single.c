...
char WHAT_SINGLE[] = "@(#)SYNTAX - $Id: single.c 3179 2023-05-06 08:32:16Z garavel $" WHAT_DEBUG;
...
static SXINT	set_an_xnt (SXINT nt, SXINT prdct)
{
    ...
    if (is_new_xnt)
	sprintf (s, ">%ld<", (SXINT) nt);
    else
	sprintf (s, " &%ld", (SXINT) prdct);
...
/* 
 * Pierre Boullier et Hubert Garavel (mars 2023) :
...
static SXINT	next (SXINT s, SXINT item)
{
...
static void create_new_trans (struct nt_etat *aetat, SXINT A, SXINT prdct, SXINT action)
{
