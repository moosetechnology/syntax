...
{  
  ...
  state1 = (intptr_t) * (intptr_t * const *) p1;
  state2 = (intptr_t) * (intptr_t * const *) p2;
	       ...
	       )
{
...
#ifdef ESSAI_INVERSE_MAPPING
  if (new_state2old_state)
    to_be_sorted = new_state2old_state;
  else
    to_be_sorted = (SXINT*) sxalloc (final_state+1, sizeof (SXINT));
#else /* ESSAI_INVERSE_MAPPING */
  to_be_sorted = (SXINT*) sxalloc (final_state+1, sizeof (SXINT));
#endif /* ESSAI_INVERSE_MAPPING */
  ...
  */
  ...
  prev_from_new_state = 0;

  for (new_state = init_state; new_state < final_state; new_state++) {
    (*normalized_fsa_forward_trans)(to_be_sorted [new_state], output_new_trans);
  }
...
epsilon_closure (SXBA cur_state_set)
{
  ...
  while (!IS_EMPTY (nfa_ec_stack)) {
    ...
    if ((*nfa_empty_trans)(state, nfa_wstate_set)) {
      ...
      next_state = 0; /* et non nfa_init_state-1, car dans certains cas (DAG) on n'a pas la garantie qu'une transition i -t-> j verifie i<j */
...
print_configuration (void)
{
  ...
  printf ("\nConfiguration = %ld\n", (SXINT)  ++configuration_nb);

  for (part = 1; part <= last_partition; part++) {
    state = part2attr [part].state;
    printf ("Partition %ld = {%ld", (SXINT) part, (SXINT) state);
	      ...
	      )
{
  ...
  SXINT                 size, pred_state, cur_state, next_part, cur_part_card, nb_max, id, nb, id_max, last_part_set_id, old_last_partition, last_new_part;
  ...
  XH_alloc (&XH_t_trans, "XH_t_trans", cur_dfa_final_state+1, 1, eof_ste, (sxoflw0_t) NULL, NULL);

  XH_alloc (&XH_trans_list_id, "XH_trans_list_id", cur_dfa_final_state+1, 1, eof_ste, (sxoflw0_t) NULL, NULL);
  ...
  if (dfa_state2pred_nb) {
    SXINT              pred_part, cur_part_sub_card;
    ...
    sxba_fill (w_state_set);
    SXBA_0_bit (w_state_set, 0);
	...
	if (cur_part_card > 1) {
	  ...
	  if (nb_max < cur_part_card) {
		...
		sxtrap (ME, "dfa_minimize");
#else
              (void) old_last_partition; /* pour faire taire gcc -Wall */
#endif /* EBUG */
  ...
  last_new_part = new_part-1;

  if (!to_be_normalized) {
    for (new_part = 1; new_part <= last_new_part; new_part++) {
    ...
    */
...
#ifdef ESSAI_INVERSE_MAPPING
    if (inverse_mapping) {
      /* L'appelant a demande' que l'on remplisse le mapping new_state -> {old_states}, on le dit a` fsa_normalize ... */
      SXINT new_norm_part, old_part, dfa_state;
      SXINT *new_state2old_state;
      SXBA  dfa_state_set;

      new_state2old_state = (SXINT*) sxalloc (last_new_part+1, sizeof (SXINT)), new_state2old_state [0] = 0;
      fsa_normalize (1, last_new_part, eof_ste, forward, mindfa_fill_trans, new_state2old_state);
      
      /* ... et on le fait */
      /* On alloue les structures c'est l'appelant qui les liberera */
      inverse_mapping->new_state2old_state_set_id = (SXINT*) sxalloc (last_new_part+1, sizeof (SXINT)), inverse_mapping->new_state2old_state_set_id [0] = 0;
      XH_alloc (&(inverse_mapping->XH_state_set), "inverse_mapping->XH_state_set", last_new_part+1, 1, (cur_dfa_final_state/last_new_part)+1, NULL, NULL);

      dfa_state_set = sxba_calloc (cur_dfa_final_state+1);

      for (new_norm_part = 1; new_norm_part <= last_new_part; new_norm_part++) {
	new_part = new_state2old_state [new_norm_part];
	old_part = new_part2part [new_part];

	dfa_state = part2attr [old_part].state;

	/* On a new_norm_part -> state */
	SXBA_1_bit (dfa_state_set, dfa_state);

	while ((dfa_state = state2attr [dfa_state].next_state)) {
	  /* On a new_norm_part -> state */
	  SXBA_1_bit (dfa_state_set, dfa_state);
	}
	...
	while ((dfa_state = sxba_scan_reset (dfa_state_set, dfa_state)) > 0)
	  XH_push (inverse_mapping->XH_state_set, dfa_state);

	XH_set (&(inverse_mapping->XH_state_set), inverse_mapping->new_state2old_state_set_id+new_norm_part);
      }

      inverse_mapping->init_state = 1;
      inverse_mapping->final_state = last_new_part;

      sxfree (new_state2old_state);
    }
    else
      fsa_normalize (1, last_new_part, eof_ste, forward, mindfa_fill_trans, NULL);
#else /* ESSAI_INVERSE_MAPPING */
    fsa_normalize (1, last_new_part, eof_ste, forward, mindfa_fill_trans);
#endif /* ESSAI_INVERSE_MAPPING */
...
nfa2dfa (SXINT init_state, 
	 SXINT final_state, 
	 SXINT max_state,  /* en general, == 0 ; si > 0, on est dans le cas DAG (pas de trans
	 ...
	 )
{
  ...
  if (empty_trans) {
    ...
    nfa_ec_stack = (SXINT*) sxalloc (nfa_max_state+1, sizeof (SXINT)), nfa_ec_stack [0] = 0;
    nfa_wstate_set = sxba_calloc (nfa_max_state+1);
  }

  nfa_t_stack = (SXINT*) sxalloc (eof_ste+1, sizeof (SXINT)), nfa_t_stack [0] = 0;
  DALLOC_STACK (final_dfa_state_stack, final_state);
  DALLOC_STACK (nfa_state_stack, nfa_max_state);
  t_set = sxba_calloc (eof_ste+1);
  t2nfa_state_set = sxbm_calloc (eof_ste+1, nfa_max_state+1);
  XH_alloc (&(XH_dfa_states), "XH_dfa_states", nfa_max_state+1, 1, 10, (sxoflw0_t) NULL, NULL);
  ...
  SXBA_1_bit (cur_state_set, init_state);
  ...
  while (!IS_EMPTY (nfa_state_stack)) {
    ...
    (void) dfa_state_is_final; /* pour faire taire gcc -Wall */
	...
	DPUSH (final_dfa_state_stack, dfa_state);
      ...
      else { /* transition vers un etat non final */
  ...
  while (!IS_EMPTY (final_dfa_state_stack)) {
    ...
    if (mindfa_fill_trans) {
      ...
      XxY_out_trans2next_state [id] = dfa_final_state;
    }
  }
  ...
  if (mindfa_fill_trans) {
#ifdef ESSAI_INVERSE_MAPPING
    if (inverse_mapping) {
      /* L'appelant a demande' que l'on remplisse le mapping new_state -> {old_states}, on le dit a` dfa_minimize ... */
      SXINT                  new_state, bot2, top2;
      SXBA                   nfa_state_set;
      struct inverse_mapping dfa_inverse_mapping;

      dfa_minimize (dfa_init_state, dfa_final_state, eof_ste, dfa_extract_trans, mindfa_fill_trans, to_be_normalized, &dfa_inverse_mapping);

      /* ... et on le fait */
      nfa_state_set = sxba_calloc (final_state+1);

      inverse_mapping->new_state2old_state_set_id = (SXINT*) sxalloc (dfa_inverse_mapping.final_state+1, sizeof (SXINT)), inverse_mapping->new_state2old_state_set_id [0] = 0;
      XH_alloc (&(inverse_mapping->XH_state_set), "inverse_mapping->XH_state_set", dfa_inverse_mapping.final_state+1, 1, (final_state/dfa_inverse_mapping.final_state)+1, NULL, NULL);

      for (new_state = dfa_inverse_mapping.init_state; new_state <= dfa_inverse_mapping.final_state; new_state++) {
	id = dfa_inverse_mapping.new_state2old_state_set_id [new_state];
	bot = XH_X (dfa_inverse_mapping.XH_state_set, id);
	top = XH_X (dfa_inverse_mapping.XH_state_set, id+1);

	while (bot < top) {
	  dfa_state = XH_list_elem (dfa_inverse_mapping.XH_state_set, bot);

	  bot2 = XH_X (XH_dfa_states, dfa_state);
	  top2 = XH_X (XH_dfa_states, dfa_state+1);

	  while (bot2 < top2) {
	    nfa_state = XH_list_elem (XH_dfa_states, bot2);
	    SXBA_1_bit (nfa_state_set, nfa_state);
	...
	while ((nfa_state = sxba_scan_reset (nfa_state_set, nfa_state)) > 0) 
	  XH_push (inverse_mapping->XH_state_set, nfa_state);

	XH_set (&(inverse_mapping->XH_state_set), inverse_mapping->new_state2old_state_set_id + new_state);
      }

      inverse_mapping->init_state = dfa_inverse_mapping.init_state;
      inverse_mapping->final_state = dfa_inverse_mapping.final_state;

      sxfree (dfa_inverse_mapping.new_state2old_state_set_id);
      XH_free (&dfa_inverse_mapping.XH_state_set);

      sxfree (nfa_state_set);
    }
    else
      dfa_minimize (dfa_init_state, dfa_final_state, eof_ste, dfa_extract_trans, mindfa_fill_trans, to_be_normalized, NULL);
#else /* ESSAI_INVERSE_MAPPING */
...
#ifdef ESSAI_INVERSE_MAPPING
    if (inverse_mapping) {
      /* L'appelant a demande' que l'on remplisse le mapping new_state -> {old_states}, sans appel a minimize ... */
      SXINT                  bot2, top2;
      SXBA                   nfa_state_set;
      ...
      inverse_mapping->new_state2old_state_set_id = (SXINT*) sxalloc (dfa_final_state+1, sizeof (SXINT)), inverse_mapping->new_state2old_state_set_id [0] = 0;
      XH_alloc (&(inverse_mapping->XH_state_set), "inverse_mapping->XH_state_set", dfa_final_state+1, 1, (final_state/dfa_final_state)+1, NULL, NULL);

      for (dfa_state = 1; dfa_state <= dfa_final_state; dfa_state++) {
	bot2 = XH_X (XH_dfa_states, dfa_state);
	top2 = XH_X (XH_dfa_states, dfa_state+1);
	...
	XH_set (&(inverse_mapping->XH_state_set), inverse_mapping->new_state2old_state_set_id + dfa_state);
      }

      inverse_mapping->init_state = 1;
      inverse_mapping->final_state = dfa_final_state;
...
static struct node **tete_open (SXINT size)
{
    ...
    if (tete == NULL) {
        sxprepare_for_possible_reset (tete);
...
static SXINT *ints_open (SXINT size)
{
    ...
    if (ints == NULL) {
        sxprepare_for_possible_reset (ints);
...
static SXBA sets_open (SXINT size)
{
    ...
    if (sets == NULL) {
        sxprepare_for_possible_reset (sets);
...
static struct node	*create_leaf (SXBA_INDEX i, SXBA_INDEX j, SXINT (*get_name_refs) (SXBA_INDEX, SXBA_INDEX, char ** ))
{
    ...
    for (leaf_node_ptr = *(orig = leafs_hd + (((intptr_t) p) % head_s_no)); leaf_node_ptr != NULL; leaf_node_ptr = leaf_node_ptr->link) {
...
static struct node	*create_op (SXINT op, struct node *l, struct node *r)
{
    ...
    for (p = *(orig = head_s + ((op + (((intptr_t) (l) ^ (intptr_t) (r)) >> 1)) % head_s_no)); p != NULL; p = p->link) {
...
static struct node	*create_or_list (struct node **p_s, SXINT m, SXINT create_or_list_M)
{
    ...
    if (to_be_sorted == NULL) {
        sxprepare_for_possible_reset (to_be_sorted);
...
static SXVOID	gen_splits (SXBA_INDEX pred, SXBA_INDEX next)
{
    ...
    if (succ_set == NULL) {
        sxprepare_for_possible_reset (succ_set);
...
static struct node	*cut (struct node *node_ptr)
{
    ...
    case leaf:
	return (*(node_ptr->body.string_ptr) == SXNUL) ? (NULL) : (node_ptr);
...
static VARSTR
fsa2re_body (VARSTR varstr_ptr, SXBA *R, SXBA *R_plus, SXBA_INDEX size, SXBA_INDEX initial_s, SXBA_INDEX final_s, SXINT (*get_name_refs) (SXBA_INDEX, SXBA_INDEX, char **))
{
    SXBA_INDEX i, j;
    SXBA	and_set;
...
/* L'ancienne definition de ID_CODE etait :
      ~( ( (SXUINT)15) << SHIFT )
   de type SXUINT. La nouvelle definition a le type SXINT */
#define ID_CODE       (SXINT_MAX >> 3)

#define code2id(c)    ((c) & ID_CODE)
...
print_code (VARSTR vstr, SXUINT code)
{
  ...
  if (trans_kind == ATOMIC_TRANS) {
    trans_name = (*dag_get_trans_name) (id);
    vstr = varstr_lcatenate_with_escape_except_for_comments (vstr, trans_name, strlen (trans_name), "?*+");
  }
  else {
    bot = XH_X (code_hd, id);
    ...
    switch (trans_kind) {
    case OPTION_TRANS :
      code1 = (SXUINT) XH_list_elem (code_hd, bot);
	...
	code1 = (SXUINT) XH_list_elem (code_hd, cur);
	...
	if (trans1_kind == OR_TRANS || trans1_kind == FOR_TRANS)
	  sxtrap (ME, "print_code (no OR or FOR transition should be found as operand of an OR or FOR)");
#endif /* EBUG && 0 */
    ...
    default :
      sxtrap (ME, "print_code (unknown transition kind)");
      break;
    }
  }
  ...
  if (tree) {
	...
	if (++nb == 1) {
	  ...
	  if (nb == 2) {
		...
		code = (SXUINT) DPOP (code_stack);
		XH_push (code_hd, (SXINT) code);
	      }
	    ...
	    if (prev_lgth > 1) {
	      while (prev_lgth-- > 0 ) {
		code2 = (SXUINT) DPOP (code_stack);
		XH_push (code_hd, (SXINT) code2);
	      }

	      set_code (&code2);
	      code2 |= CAT_CODE;
	      DPUSH (code_stack, (SXINT) code2);
	    }
	      
	    DPUSH (code_stack, (SXINT) code);
	  }
	  else {
	    if (lgth > 1) {
	      /* Vraie cat */
		...
		XH_push (code_hd, (SXINT) code);
	      ...
	      set_code (&code);
	      code |= CAT_CODE;
	      DPUSH (code_stack, (SXINT) code); /* stockage temporaire */
	    }
	  }
	}
      }
...
gen_branch (SXINT branch, SXINT path, SXBA set, SXBA final_set)
{
  ...
  if (tree) {
    ...
    (void) tree_pos; /* pour faire taire gcc -Wall */
  }
...
extract_factor (void)
{
  ...
  SXINT code_size, path_branch, next_path, bot_tree, bot_branch;
  SXUINT code;
  ...
  */
  ...
  for (branch = top; branch > 0; branch--) {
    if ((nb = branch2occur_nb [branch]) > 1) {
      ...
      code = (SXUINT) XxY_X (branches, branch);
      tree = XxY_Y (branches, branch);
	...
	next_path = XxY_is_set (&paths, path, (SXINT) code);
...
factorize_OR (SXINT or_code)
{
  ...
  for (cur = bot; cur < top; cur++) {
	...
	code1 = (SXUINT) XH_list_elem (code_hd, cur1);
	lgth++;
  ...
  (void) has_common; /* pour faire taire gcc -Wall */
...
static SXUINT
make_code (SXUINT code1, SXUINT code2, SXUINT what_to_do)
{
  SXINT trans1_kind, trans2_kind, id1, id2, bot1, cur1, top1, bot2, cur2, top2;
  SXUINT code;
  ...
  case FOR_TRANS:
    /* make_code ne fait jamais directement de FOR_TRANS */
    sxtrap (ME, "make_code (FOR_TRANS)");
    set_code (&code); /* pour faire taire clang -Wsometimes-uninitialized */
    break;
...
fill_dag_hd (SXINT p, SXINT t, SXINT q)
{
  ...
  if (first_time) {
    ...
    /* On fait un or */
    attr_ptr->trans = (SXINT) make_code ((SXUINT) (attr_ptr->trans), ATOMIC_CODE | (SXUINT) t, OR_TRANS);
  }
}
...
make_a_new_trans (SXINT dag1_id, SXINT dag2_id)
{
  ...
  if (first_time) {
    ...
    if ((SXUINT)attr_ptr->trans == (eof_trans | ATOMIC_CODE)) {
      attr_ptr->trans = (SXINT) make_code (code, (SXUINT) 0, OPTION_TRANS);
    }
    else
      attr_ptr->trans = (SXINT) make_code ((SXUINT) (attr_ptr->trans), code, OR_TRANS);
  }
...
#if LOG
	if (dag_id)
	  printf ("Merged %ld -> %ld -> %ld into %ld -> %ld\n",
		  XxY_X (dag_hd, trans_id), q, XxY_Y (dag_hd, trans_id2),
		  XxY_X (dag_hd, trans_id), XxY_Y (dag_hd, trans_id2));
	else 
	  printf ("Could not merge %ld -> %ld -> %ld\n",
		  XxY_X (dag_hd, trans_id), q, XxY_Y (dag_hd, trans_id2));
#endif /* LOG */      
      }
    }
	...
	char *(*get_trans_name) (SXINT))
{
  ...
  XH_alloc (&code_hd, "code_hd", dag_final_state+1, 1, 64, code_hd_oflw, NULL);
  ...
  XxY_alloc (&pathXbranch, "pathXbranch", 30, 1, 0, 1 /* Yforeach */, (sxoflw0_t) NULL, NULL);
  XxY_alloc (&pathXtree, "pathXtree", 30, 1, 0, 1 /* Yforeach */, (sxoflw0_t) NULL, NULL);
  XH_alloc (&trees, "trees", 30, 1, 4, trees_oflw, NULL);
  tree2size = (SXINT *) sxalloc (XH_size (trees)+1, sizeof (SXINT));
  tree2occur_nb = (SXINT *) sxcalloc (XH_size (trees)+1, sizeof (SXINT));
...
#if EBUG
  {
    SXINT prev_dag_hd_trans_nb = dag_hd_trans_nb;
#endif /* EBUG */
    
    while (dag_hd_trans_nb > 1) {
      single_trans = dag_reduce ();
#if EBUG
      if (dag_hd_trans_nb == prev_dag_hd_trans_nb)
	sxtrap (ME, "dag2re (could not extract unique initial and final states)");
      prev_dag_hd_trans_nb = dag_hd_trans_nb;
    }
#endif /* EBUG */
  }
...
word_tree_free (struct word_tree_struct *word_tree_ptr)
{
  if (word_tree_ptr->stats) {
    fprintf (word_tree_ptr->stats, "// Statistics for \"%s\": total size (in bytes) = %ld\n", word_tree_ptr->name,
	     ...
	     + ((word_tree_ptr->final_path_set) ? (sizeof (SXBA_ELT) * (SXNBLONGS (SXBASIZE (word_tree_ptr->final_path_set))+1)) : (sizeof (SXINT) * (word_tree_ptr->max_final_state_nb + 1)))
	     );
  }
...
word_tree2sxdfa (struct word_tree_struct *word_tree_ptr, struct sxdfa_struct *sxdfa_ptr, char *name, FILE *stats, SXBOOLEAN to_be_minimized)
{
  ...
  XH_alloc (&trans_list_hd, "trans_list_hd", last_new_path+1, 1, max_trans_nb, (sxoflw0_t) NULL, NULL);
...
sxdfa_tree2min_dag (struct sxdfa_struct *sxdfa_ptr)
{
  ...
  XH_alloc (&dico2_trees, "dico2_trees", sxdfa_ptr->last_state+1, 1, sxdfa_ptr->number_of_out_trans/sxdfa_ptr->last_state + 1, (sxoflw0_t) NULL, NULL);
  ...
  for (tree = dico2_max_tree-1; tree > 0; tree--) {
    ...
    (void) state; /* pour faire taire gcc -Wall */
	   ...
	   SXBOOLEAN to_be_normalized)
{
  ...
  nfa_max_state = cur_max_state == 0 ? cur_final_state : cur_max_state;
  nfa_empty_trans = empty_trans; /* Appels multiples */
  ...
  /* a serait bien d'avoir en arg le nb de transitions de l'automate non-deterministe pour calculer
     une taille moyenne */
  XH_alloc (&trans_list_hd, "trans_list_hd", cur_final_state+1, 1, trans_nb/cur_final_state + 1, (sxoflw0_t) NULL, NULL);
...
sxdfa_minimize (struct sxdfa_struct *sxdfa_ptr, SXBOOLEAN to_be_normalized)
{
  ...
  XH_alloc (&XH_trans_list_id, "XH_trans_list_id", sxdfa_ptr->last_state+1, 1, sxdfa_ptr->max_arity, (sxoflw0_t) NULL, NULL);
...
sxdfa_fill_dag_hd (struct sxdfa_struct *sxdfa_ptr, SXINT p, SXINT t, SXINT q)
{
  ...
  if (first_time) {
    dag_hd_trans_nb++;
    attr_ptr->trans = ATOMIC_CODE | (SXUINT) t;
  }
  else {
    /* On fait un or */
    attr_ptr->trans = (SXINT) make_code ((SXUINT) (attr_ptr->trans), ATOMIC_CODE | (SXUINT) t, OR_TRANS);
  ...
  if (sxdfa_ptr->states [q] & 1) {
    ...
    if (first_time) {
      ...
      /* On fait un or */
      attr_ptr->trans = (SXINT) make_code ((SXUINT) (attr_ptr->trans), ATOMIC_CODE, OR_TRANS);
    }
  }
...
sxdfadag2re (VARSTR varstr_ptr, struct sxdfa_struct *sxdfa_ptr, char *(*get_trans_name) (SXINT))
{
  ...
  XH_alloc (&code_hd, "code_hd", final_state+1, 1, 
	    sxdfa_ptr->number_of_out_trans/final_state + 1 /* si un jour ca explose ici, on peut tenter de remplacer ce truc malin par, genre, 64 */, 
	    code_hd_oflw, NULL);
  code2size = (SXINT*) sxalloc (XH_size (code_hd)+1, sizeof (SXINT));
...
sxdfa_seek (struct sxdfa_struct *sxdfa_ptr, SXUINT (*get_next_symb) (struct sxdfa_private *), SXINT *input_lgth)
{
  ...
  while (lgth > 0 && state > 0) {
    ...
    trans = (SXINT) ((*get_next_symb) (&(sxdfa_ptr->private)));
...
static SXINT                 *base2stack, char_stack_list_size, char_stack_list_top;
static unsigned char         *char_stack_list;
...
install_a_comb (SXBA *comb_vector_ptr, SXBA comb, SXBA *base_pos_set_ptr)
{
  ...
  comb_size = SXBASIZE (comb);
  last_comb_word = comb + SXNBLONGS (comb_size);
  comb_vector_size = SXBASIZE (comb_vector);
  ...
  if (base_comb_vector)
    free_comb_vector_elem = (SXUINT) base_comb_vector;
  else
    free_comb_vector_elem = first_comb_elem-1;

  for (;;) {
    free_comb_vector_elem = (SXUINT) sxba_0_lrscan (comb_vector, free_comb_vector_elem);
    if (free_comb_vector_elem == (SXUINT)(-1)) {
      /* possibilite' franchement improbable de debordement */
      comb_vector_size *= 2;
      *comb_vector_ptr = comb_vector = sxba_resize (comb_vector, (SXBA_INDEX) comb_vector_size);
      *base_pos_set_ptr = base_pos_set = sxba_resize (base_pos_set, (SXBA_INDEX) comb_vector_size);
      free_comb_vector_elem = sxba_0_lrscan (comb_vector, (SXBA_INDEX_OR_ERROR) free_comb_vector_elem);
    }

    base_value = (SXINT) (free_comb_vector_elem - first_comb_elem);
...
{
  SXINT         state, base_pos, size, max_base_pos, t, class, tooth, next_state, is_final_state, bot, bot_trans, stop_mask;
  SXINT         *state_ptr, *next_states_ptr, *top_next_states_ptr, *trans_list_ptr;
  SXINT         *state2base_pos, char_comb_cardinal = 0;
  ...
  /* On remplit sxdfa_comb_ptr */
  {
    ...
    base_shift = sxdfa_comb_ptr->base_shift = (SXUINT) k + 1 /* stop_mask */;
    stop_mask = sxdfa_comb_ptr->stop_mask = ((SXUINT) 1) << k;
  ...
  sxdfa_comb_ptr->base2stack = NULL;
  sxdfa_comb_ptr->char_stack_list = NULL;
  sxdfa_comb_ptr->char_stack_list_top = 0;

  comb = sxba_calloc (max_class_value+1);
  char_comb = sxba_calloc (256);
  ...
  if (is_generate_char_stack_list) {
    base2stack = (SXINT*) sxcalloc (size, sizeof (SXINT));
    char_stack_list_size = size * 6;
    char_stack_list = (unsigned char *) sxalloc (char_stack_list_size, sizeof (unsigned char));
    char_stack_list_top = 1;
  }
  ...
  if (optim_kind > 2)
    sxtrap (ME, "sxdfa2comb_vector (unknown optim_kind ; note that generation of char_stack_list for optim_kind 1 is not yet implemented)");
  ...
  case 2 :
	...
	base_comb_vector = (SXBA_INDEX_OR_ERROR) (SXBASIZE (comb_vector));
	...
	if (is_generate_char_stack_list) {
	  if (SXBA_bit_is_reset_set (char_comb, t))
	    char_comb_cardinal++;
	}
      }
      ...
      if (is_generate_char_stack_list && char_comb_cardinal) {
	if (char_stack_list_size < char_stack_list_top + char_comb_cardinal + 1)
	  char_stack_list = (unsigned char*) sxrealloc (char_stack_list, char_stack_list_size *= 2, sizeof (unsigned char));
	
	t = -1;

	base2stack [base_pos] = char_stack_list_top;
	char_stack_list [char_stack_list_top++] = (unsigned char) char_comb_cardinal;

	while ((t = sxba_scan_reset (char_comb, t)) >= 0) {
	  char_stack_list [char_stack_list_top++] = (unsigned char) t;
	}
      }
    ...
    if (is_generate_char_stack_list) {
      sxdfa_comb_ptr->base2stack = base2stack;
      sxdfa_comb_ptr->char_stack_list = char_stack_list;
      sxdfa_comb_ptr->char_stack_list_top = char_stack_list_top;
    }
  ...
  case 1:
    ...
    {
      SXINT             top, trans_id_nb, trans_id, pos, min_class_value, nb, x, common_base_pos, class_set_card;
	...
	comb_vector_ptr = sxdfa_comb_ptr->comb_vector + (SXUINT) pos;
	tooth = *comb_vector_ptr;

	if ((tooth & stop_mask) == 0 && (class = (SXINT) (tooth & sxdfa_comb_ptr->class_mask)) >= min_class_value /* vraie classe */) {
	  base_pos = state2base_pos [tooth >>= base_shift];
	  *comb_vector_ptr = (base_pos << base_shift) + (SXUINT) class;
	}
      }
...
sxdfa_comb2c (struct sxdfa_comb *sxdfa_comb_ptr, FILE *file, char *dico_name, SXBOOLEAN is_static)
{
  ...
  fprintf (file, "\nstatic SXUINT %s_comb_vector [%ld] = {", dico_name == NULL ? "" : dico_name, (SXINT) sxdfa_comb_ptr->max+1);

  for (i = 0; i <= sxdfa_comb_ptr->max; i++) {
    if ((i % 10) == 0)
      fprintf (file, "\n/* %ld */ ", i);

    fprintf (file, "%ld, ", (SXINT) sxdfa_comb_ptr->comb_vector [i]);
  }
  ...
  if (sxdfa_comb_ptr->base2stack) {
    fprintf (file, "\nstatic SXINT %s_base2stack [%ld] = {", dico_name == NULL ? "" : dico_name, (SXINT) sxdfa_comb_ptr->max+1);
    
    for (i = 0; i <= sxdfa_comb_ptr->max; i++) {
      if ((i % 10) == 0)
	fprintf (file, "\n/* %ld */ ", i);
      
      fprintf (file, "%ld, ", sxdfa_comb_ptr->base2stack [i]);
    }
    
    fprintf (file, "\n};\nstatic unsigned char %s_char_stack_list [%ld] = \"\\000\\\n", dico_name == NULL ? "" : dico_name, sxdfa_comb_ptr->char_stack_list_top + 1);
    
    {
      SXINT sub_stack_remaining_height = 0;
      char c;
      
      for (i = 1; i < (SXUINT) sxdfa_comb_ptr->char_stack_list_top; i++) {
	if (sub_stack_remaining_height) {
	  sub_stack_remaining_height--;
	  c = (char) (sxdfa_comb_ptr->char_stack_list [i]);
	  fprintf (file, "%s", SXCHAR_TO_STRING(c));
	} else {     
	  //	  if (i > 1)
	  //fputs ("\"", file);
	  
	  sub_stack_remaining_height = (SXINT) (sxdfa_comb_ptr->char_stack_list [i]);
	  //	  fprintf (file, "\n/* %ld, %ld */ ", i, sub_stack_remaining_height);
	  fputs ("\\\n", file);

	  c = (char) sub_stack_remaining_height;
	  //fprintf (file, "\"%s", SXCHAR_TO_STRING(c));
	  fprintf (file, "%s", SXCHAR_TO_STRING(c));
	}
      }
...
sxdfa_comb_seek (struct sxdfa_comb *sxdfa_comb_ptr, SXUINT (*get_next_symb) (struct sxdfa_private *), SXINT *input_lgth)
{
  ...
  for (;;) {
    ...
    if (tooth /* Base non vide... */ 
	&& (tooth & class_mask) == 0 /* ... et qui n'est pas une dent appartenant a une autre base :
					 elle contient donc des infos (id deja certain par
					 anticipation et/ou le fait que c'est un etat final) */) {
      if (tooth & stop_mask) { /* Si on a atteint un etat final (mais puisque c'est une base, il y a
				  des transitions qui en partent). Le prefixe courant est un mot du
				  dico ... */
	tooth >>= base_shift;
	/* ... dont l'id est ce tooth shifte. En effet, on est sur une base (des transitions en
	   partent) qui est aussi un etat final: on n'a donc pas pu connaitre l'id par anticipation,
	   et ce tooth shifte est bien l'id */
	if (lgth == 0) /* c'est donc bien ce mot qu'on avait en entree */
	  return tooth;

	/* Ici, le mot a reconnaitre n'a le mot d'id tooth que comme prefixe strict */
	/* On note ce prefixe au cas ou */
	final_id = tooth;
	final_lgth = lgth;

	/* Si d'ici il ne sort qu'une seule transition et qu'elle ne mene (directement ou non) qu'a
	   un seul mot du dico alors on a l'id de ce mot. Ces infos sont dans... */
	tooth = comb_vector_ptr [1];
	/* ... de la facon suivante: si : */
	if ((tooth & class_mask) == 1) {
	  /* alors on est bien dans ce cas ou il n'y a plus qu'un mot possible selon le dico, et il
	     a alors pour id: */
	  last_final_id = tooth >> base_shift;
	}
      }
      else { /* On n'est pas sur un etat final */
	if (lgth == 0) { /* si on a deja fini le mot d'entree, c'est donc un prefixe strict d'au
			  moins un mot du dico, et en particulier du mot d'id last_final_id s'il est
			  positionne */
	  break;
	}
	/* tooth >> base_shift est l'id du seul mot que l'on peut atteindre en poursuivant d'ici par
	   des transitions du dico. Or on est sur qu'il est non nul, puisque tooth est non nul et
	   que tooth & class_mask est nul */
	last_final_id = tooth >> base_shift;
      }
    }
    else {
      /* Base vide: la base tooth ne contient aucune info sur la transition courante car elle n'a
	 rien de particulier (attention, tooth a du coup de bonnes chances d'etre une dent
	 appartenant a une autre base). On poursuit sagement le parcours... */
      if (lgth == 0) {
	/* ...sauf si le mot est fini: le mot en entree est alors un prefixe strict de mots du dico
	 (ou d'un mot unique si last_final_id > 0, qui est alors son id) */
	break;
      }
    }

    /* on va donc avancer le long de la prochaine transition, qui se fait sur le prochain caractere
       de l'entree. Ce caractere donne la classe a suivre dans le dico, d'ou la transition si elle
       existe */

    if ((class = char2class [(*get_next_symb) (&(sxdfa_comb_ptr->private))]) == 0)
       /* Le caractere courant est invalide */ 
      break;

    tooth = comb_vector_ptr [class];

    if ((tooth & class_mask) != class) { /* on est tombe sur un endroit qui n'appartient pas a notre
      base : la transition recherchee n'existe pas. On fait break (sauf dans le cas -comb 1, ou on
      fait des trucs) */
    ...
    /* La transition est valide: depuis base, le caractere trouve sur l'entree a bien une
       transition, qui mene au nouveau tooth */

    lgth = --*input_lgth; /* transition OK */

    if (tooth & stop_mask) {
      /* Si on atterrit sur un etat final qui n'est pas une base: c'est fini */
      if ((tooth >>= base_shift) == 0)
	tooth = (last_final_id) ? last_final_id /* id connu */ : 1 /* id inconnu, mais succes (il
								      n'y a pas d'id dans le dico,
								      c'est tout ;) ) */;

      /* On rend tooth dans tous les cas. Si *input_lgth est nul, c'est qu'on a gagne. S'il n'est
	 pas nul, c'est qu'on a atteint un etat final du dico sans epuiser le mot en entree Le mot
	 tooth du dico est donc un prefixe strict du mot en entree. C'est donc a l'utilisateur de
	 faire la difference. */
      return tooth;
    }

    /* On doit continuer le parcours. tooth est forcement une base, puisque (1) on l'a atteint et
       (2) stop_mask est faux */
    base = tooth >> base_shift;
  }
...
sxdfa_comb_free (struct sxdfa_comb *sxdfa_comb_ptr)
{
  if (!sxdfa_comb_ptr->is_static) {
    ...
    if (sxdfa_comb_ptr->base2stack) sxfree (sxdfa_comb_ptr->base2stack), sxdfa_comb_ptr->base2stack = NULL;
    if (sxdfa_comb_ptr->char_stack_list) sxfree (sxdfa_comb_ptr->char_stack_list), sxdfa_comb_ptr->char_stack_list = NULL;
  }
}
...
sxdfa2sxdfa_packed (struct sxdfa_struct *sxdfa_ptr, struct sxdfa_packed_struct *sxdfa_packed_ptr, char *name, FILE *stats)
{
  ...
  sxdfa_packed_ptr->stop_mask = stop_mask = ((SXUINT) 1) << k;
  sxdfa_packed_ptr->id_mask = id_mask = ((SXUINT) 1) << (k+1);
...
{
  sxdfa_packed_ptr->private.ptr.SXINT_ptr = (sxdfa_packed_ptr->private.from_left_to_right) ? input_stack + 1 : input_stack + *input_stack;
...
static struct nfa_intersection_struct {
  SXINT   init_state, final_state, max_state, eof_ste;
  SXBA    t_set, p_set;
  SXINT   *t2q_hd;
  SXINT   top, size;
  struct t2q_list {
...
} nfa_intersection_struct_nfa1, nfa_intersection_struct_nfa2, *nfa_intersection_struct_ptr;
...
static void
nfa_intersection_process_a_non_eps_trans (SXINT p, SXINT t, SXINT q)
{
  SXINT hd, delta, top;

  sxuse(p); /* pour faire taire gcc -Wunused */
  /* On a une transition p ->t q non vide sur l'operande pointe' par nfa_intersection_struct_ptr */
  if (SXBA_bit_is_reset_set (nfa_intersection_struct_ptr->t_set, t))
    nfa_intersection_struct_ptr->t2q_hd [t] = q;
  else {
    hd = nfa_intersection_struct_ptr->t2q_hd [t];

    delta = (hd > 0) ? 2 : 1;
      
    if ((top = nfa_intersection_struct_ptr->top += delta) >= nfa_intersection_struct_ptr->size) {
      nfa_intersection_struct_ptr->size *= 2;
      nfa_intersection_struct_ptr->q_list = (struct t2q_list*) sxrealloc (nfa_intersection_struct_ptr->q_list, nfa_intersection_struct_ptr->size+1, sizeof (struct t2q_list));
    }
      
    nfa_intersection_struct_ptr->t2q_hd [t] = -top;

    if (hd > 0) {
      /* 1ere fois */
      nfa_intersection_struct_ptr->q_list [top].p = q;
      nfa_intersection_struct_ptr->q_list [top].next = top-1;
      nfa_intersection_struct_ptr->q_list [top-1].p = hd;
      nfa_intersection_struct_ptr->q_list [top-1].next = 0;
    }
    else {
      nfa_intersection_struct_ptr->q_list [top].p = q;
      nfa_intersection_struct_ptr->q_list [top].next = -hd;
    }
  }
}

static void
nfa_intersection_fill_non_eps_trans (SXINT nfa_list,
				     void (*nfa1_extract_non_eps_trans)(SXINT, void (*)(SXINT, SXINT, SXINT)))
{
  SXINT bot, top, nfa_state;

  bot = XH_X (XH_intersection_state_list_hd, nfa_list);
  top = XH_X (XH_intersection_state_list_hd, nfa_list+1);

  while (bot < top) {
    nfa_state = XH_list_elem (XH_intersection_state_list_hd, bot);

    nfa1_extract_non_eps_trans (nfa_state, nfa_intersection_process_a_non_eps_trans);
    
    bot++;
  }
}

/* Associe' a t, ds t2q_hd, on a un ensemble d'etats p.  Pour chaque p, on fait une eps closure.
   l'ensemble de tous les etats q atteints depuis tous les p sont tries et stockes ds un XH (XH_intersection_state_list_hd)
				   ...
				   SXBOOLEAN  *contains_the_final_state)
{
  ...
  p = nfa_intersection_struct_ptr->t2q_hd [t];

  if (p > 0) {
    /* etat unique */
    if (SXBA_bit_is_reset_set (nfa_intersection_struct_ptr->p_set, p) &&
	nfa_extract_empty_trans &&
	((nfa_intersection_struct_ptr->max_state == 0) /* On doit regarder les trans depuis l'etat final */ || p != nfa_intersection_struct_ptr->final_state)) {
	PUSH (nfa_intersection_ec_stack, p);
    }
  }
  else {
    next = -p;
    ...
    do {
      p = nfa_intersection_struct_ptr->q_list [next].p;
    ...
    } while ((next = nfa_intersection_struct_ptr->q_list [next].next) > 0);
  }

  /* nfa_intersection_ec_stack contient des etats dont il faut examiner les eps trans */
  while (!IS_EMPTY (nfa_intersection_ec_stack)) {
    p = POP (nfa_intersection_ec_stack);

    if ((*nfa_extract_empty_trans)(p, nfa_intersection_ec_set)) {
      /* wstate_set est non vide */
      q = 0; /* et non nfa_init_state-1, car dans certains cas (DAG) on n'a pas la garantie qu'une transition i -t-> j verifie i<j */

      while ((q = sxba_scan_reset (nfa_intersection_ec_set, q)) >= 0) {
	if (SXBA_bit_is_reset_set (nfa_intersection_struct_ptr->p_set, q) &&
	    ((nfa_intersection_struct_ptr->max_state == 0) /* On doit regarder les trans depuis l'etat final */ || q != nfa_intersection_struct_ptr->final_state)) {
	  PUSH (nfa_intersection_ec_stack, q);
	}
      }
    }
  }
  ...
  *contains_the_final_state = is_the_final_state;

  XH_set (&XH_intersection_state_list_hd, &q_list);
...
#if EBUG
static void
print_nfa_intersection_state (SXINT nfa_list)
{
  SXINT cur, bot, top, p;
  ...
  for (cur = bot; cur < top; cur++) {
    p = XH_list_elem (XH_intersection_state_list_hd, cur);
...
static void
print_nfa_intersection_trans (XxY_header *hd_ptr, SXINT nfa_intersection_state, SXINT t, SXINT next_nfa_intersection_state)
{
  SXINT nfa1_list, nfa2_list, next_nfa1_list, next_nfa2_list;

  nfa1_list = XxY_X (*hd_ptr, nfa_intersection_state);
  fputs ("(", stdout);
  print_nfa_intersection_state (nfa1_list);
  fputs (", ", stdout);
  nfa2_list = XxY_Y (*hd_ptr, nfa_intersection_state);
  print_nfa_intersection_state (nfa2_list);
  printf (")\t%s\t(", sxstrget (t));
  next_nfa1_list = XxY_X (*hd_ptr, next_nfa_intersection_state);
  print_nfa_intersection_state (next_nfa1_list);
  fputs (", ", stdout);
  next_nfa2_list = XxY_Y (*hd_ptr, next_nfa_intersection_state);
  print_nfa_intersection_state (next_nfa2_list);
  fputs (")\n", stdout);
}
#endif /* EBUG */
...
static void
XxY_nfa_intersection_states_oflw (SXINT old_size, SXINT new_size)
{
  sxuse (old_size); /* pour faire taire gcc -Wunused */
  nfa_intersection_tbp_stack = (SXINT *) sxrealloc (nfa_intersection_tbp_stack, new_size+1, sizeof (SXINT));
  nfa_intersection_final_state_set = sxba_resize (nfa_intersection_final_state_set, new_size+1);
}
...
{
  SXINT                 nfa_intersection_state, nfa1_list, nfa2_list, t, nfa_intersection_init_state, nfa_intersection_final_state = 0, triple, next_nfa_intersection_state, p, q;
  SXINT                 size1, size2;
  SXBA                  nfa_intersection_working_t_set;
  SXBA                  nfa_intersection_tbp_set = NULL;
  ...
  /* chaque etat du intersection_nfa est un couple (liste1, liste2) stocke' ds
     ...
     liste1 et liste2 sont des ensembles d'etats de nfa1 et de nfa2 ranges ds
     XH_intersection_state_list_hd
  */
  size1 = nfa1_max_state == 0 ? nfa1_final_state : nfa1_max_state;
  size2 = nfa2_max_state == 0 ? nfa2_final_state : nfa2_max_state;


  /* allocation des structures locales */
  nfa_intersection_struct_nfa1.init_state = nfa1_init_state;
  nfa_intersection_struct_nfa1.final_state = nfa1_final_state;
  nfa_intersection_struct_nfa1.max_state = nfa1_max_state;
  nfa_intersection_struct_nfa1.eof_ste = nfa1_eof_ste;
  nfa_intersection_struct_nfa1.t_set = sxba_calloc (nfa1_eof_ste+1);
  nfa_intersection_struct_nfa1.p_set = sxba_calloc (size1+1);
  nfa_intersection_struct_nfa1.t2q_hd = (SXINT *) sxalloc (nfa1_eof_ste+1, sizeof (SXINT));
  nfa_intersection_struct_nfa1.q_list = (struct t2q_list*) sxalloc ((nfa_intersection_struct_nfa1.size = size1) + 1, sizeof (struct t2q_list));
  nfa_intersection_struct_nfa1.top = 0;

  nfa_intersection_struct_nfa2.init_state = nfa2_init_state;
  nfa_intersection_struct_nfa2.final_state = nfa2_final_state;
  nfa_intersection_struct_nfa2.max_state = nfa2_max_state;
  nfa_intersection_struct_nfa2.eof_ste = nfa2_eof_ste;
  nfa_intersection_struct_nfa2.t_set = sxba_calloc (nfa2_eof_ste+1);
  nfa_intersection_struct_nfa2.p_set = sxba_calloc (size2+1);
  nfa_intersection_struct_nfa2.t2q_hd = (SXINT *) sxalloc (nfa2_eof_ste+1, sizeof (SXINT));
  nfa_intersection_struct_nfa2.q_list = (struct t2q_list*) sxalloc ((nfa_intersection_struct_nfa2.size = size2) + 1, sizeof (struct t2q_list));
  nfa_intersection_struct_nfa2.top = 0;

  nfa_intersection_working_t_set = sxba_calloc (nfa1_eof_ste+nfa2_eof_ste+1);

  XxY_alloc (&XxY_nfa_intersection_states_hd, "XxY_nfa_intersection_states_hd", size1 + size2 + 1, 1, 0, 0, XxY_nfa_intersection_states_oflw, NULL);
  nfa_intersection_tbp_stack = (SXINT *) sxalloc (XxY_size (XxY_nfa_intersection_states_hd)+1, sizeof (SXINT)), RAZ (nfa_intersection_tbp_stack);
  nfa_intersection_final_state_set = sxba_calloc (XxY_size (XxY_nfa_intersection_states_hd)+1);

  XxYxZ_alloc (&XxYxZ_work_hd, "XxYxZ_work_hd", size1 + size2 + 1, 1, XxYxZ_work_foreach, (sxoflw0_t) NULL, NULL);

  XH_alloc (&XH_intersection_state_list_hd, "XH_intersection_state_list_hd", size1 + size2 + 1,1, 2, (sxoflw0_t) NULL, NULL);

  nfa_intersection_ec_stack = (SXINT *) sxalloc (size1 + size2 + 1, sizeof (SXINT)), RAZ (nfa_intersection_ec_stack);
  nfa_intersection_ec_set = sxba_calloc (size1 + size2 + 1);


  nfa_intersection_struct_ptr = &(nfa_intersection_struct_nfa1);
  nfa_intersection_process_a_non_eps_trans (0 /* bidon */, 0 /* bidon */, nfa1_init_state);
  /* On fabrique la liste des etats de nfa1 que l'on peut atteindre (apres eps closure)
     sur la transition bidon 0 */
  nfa1_list = nfa_intersection_make_eps_closure (0, nfa1_empty_trans, &nfa1_list_contains_the_final_state);
  SXBA_0_bit (nfa_intersection_struct_nfa1.t_set, 0); /* RAZ de nfa_intersection_struct_nfa1 */

  nfa_intersection_struct_ptr = &(nfa_intersection_struct_nfa2);
  nfa_intersection_process_a_non_eps_trans (0 /* bidon */, 0 /* bidon */, nfa2_init_state);
  /* On fabrique la liste des etats de nfa1 que l'on peut atteindre (apres eps closure)
     sur la transition bidon 0 */
  nfa2_list = nfa_intersection_make_eps_closure (0, nfa2_empty_trans, &nfa2_list_contains_the_final_state);
  SXBA_0_bit (nfa_intersection_struct_nfa2.t_set, 0); /* RAZ de nfa_intersection_struct_nfa1 */

  XxY_set (&XxY_nfa_intersection_states_hd, nfa1_list, nfa2_list, &nfa_intersection_init_state);
  ...
  while (!IS_EMPTY (nfa_intersection_tbp_stack)) {
    ...
    nfa1_list = XxY_X (XxY_nfa_intersection_states_hd, nfa_intersection_state);
    nfa2_list = XxY_Y (XxY_nfa_intersection_states_hd, nfa_intersection_state);

    nfa_intersection_struct_ptr = &(nfa_intersection_struct_nfa1);
    nfa_intersection_fill_non_eps_trans (nfa1_list, nfa1_extract_non_eps_trans);
    nfa_intersection_struct_ptr = &(nfa_intersection_struct_nfa2);
    nfa_intersection_fill_non_eps_trans (nfa2_list, nfa2_extract_non_eps_trans);

    if (sxba_2op (nfa_intersection_working_t_set, SXBA_OP_COPY, nfa_intersection_struct_nfa1.t_set, SXBA_OP_AND, nfa_intersection_struct_nfa2.t_set)) {
      /* Il y a des transitions communes ... */
      t = 0;

      while ((t = sxba_scan (nfa_intersection_working_t_set, t)) > 0) {
	nfa_intersection_struct_ptr = &(nfa_intersection_struct_nfa1);
	nfa1_list = nfa_intersection_make_eps_closure (t, nfa1_empty_trans, &nfa1_list_contains_the_final_state);
	nfa_intersection_struct_ptr = &(nfa_intersection_struct_nfa2);
	nfa2_list = nfa_intersection_make_eps_closure (t, nfa2_empty_trans, &nfa2_list_contains_the_final_state);

	if (!XxY_set (&XxY_nfa_intersection_states_hd, nfa1_list, nfa2_list, &next_nfa_intersection_state)) {
	  /* Nouvel etat */
	  if (nfa1_list_contains_the_final_state && nfa2_list_contains_the_final_state) {
	    ...
	    if (nfa1_max_state == 0)
	      /* Il peut y avoir des transitions sortantes de l'etat final */
	      PUSH (nfa_intersection_tbp_stack, next_nfa_intersection_state);
	  }
	  else
	    PUSH (nfa_intersection_tbp_stack, next_nfa_intersection_state);
	}

	/* On a donc une transition nfa_intersection_state ->t next_nfa_intersection_state */
	/* On differe la sortie car elle peut ne pas conduire a un etat final */
#if EBUG
	print_nfa_intersection_trans (&XxY_nfa_intersection_states_hd, nfa_intersection_state, t, next_nfa_intersection_state);
#endif /* EBUG */

	XxYxZ_set (&XxYxZ_work_hd, nfa_intersection_state, t, next_nfa_intersection_state, &triple);
      }
    }
    else {
      /* Impasse les chemins qui arrivent la seront supprimes */
    }

    sxba_empty (nfa_intersection_struct_nfa1.t_set);
    sxba_empty (nfa_intersection_struct_nfa2.t_set);
  }

  if (!result_is_empty) {
    /* On fait un etat final unique d'id max ... */
    nfa_intersection_final_state = XxY_top (XxY_nfa_intersection_states_hd);

    if (!SXBA_bit_is_set (nfa_intersection_final_state_set, nfa_intersection_final_state)) {
      /* l'etat max n'est pas final */
      nfa_intersection_final_state++;
    }

    /* ... et on met les intersections vers cet etat */
    p = nfa_intersection_final_state;

    while ((p = sxba_1_rlscan (nfa_intersection_final_state_set, p)) > 0)
      XxYxZ_set (&XxYxZ_work_hd, p, nfa2_eof_ste /* !! */, nfa_intersection_final_state, &triple);
  
    /* Elimination des branches mortes ... */
    nfa_intersection_tbp_set = sxba_calloc (nfa_intersection_final_state+1);

    /* On marque les etats accessibles depuis l'etat final */
    PUSH (nfa_intersection_tbp_stack, nfa_intersection_final_state);
    SXBA_1_bit (nfa_intersection_tbp_set, nfa_intersection_final_state);

    while (!IS_EMPTY (nfa_intersection_tbp_stack)) {
      q = POP (nfa_intersection_tbp_stack);

      XxYxZ_Zforeach (XxYxZ_work_hd, q, triple) {
	p = XxYxZ_X (XxYxZ_work_hd, triple);

	if (SXBA_bit_is_reset_set (nfa_intersection_tbp_set, p))
	  PUSH (nfa_intersection_tbp_stack, p);
      }
    }

    /* ... et sortie de l'intersection */
    for (p = nfa_intersection_init_state; p <= nfa_intersection_final_state; p++) {
      if (SXBA_bit_is_set (nfa_intersection_tbp_set, p)) {
	XxYxZ_Xforeach (XxYxZ_work_hd, p, triple) {
	  q = XxYxZ_Z (XxYxZ_work_hd, triple);

	  if (SXBA_bit_is_set (nfa_intersection_tbp_set, q))
	    (*nfa_intersection_fill_trans)(p, XxYxZ_Y (XxYxZ_work_hd, triple), q, (q == nfa_intersection_final_state));
	}
      }
    }
  }
  ...
  sxfree (nfa_intersection_struct_nfa1.t_set), nfa_intersection_struct_nfa1.t_set = NULL;
  sxfree (nfa_intersection_struct_nfa1.p_set), nfa_intersection_struct_nfa1.t_set = NULL;
  sxfree (nfa_intersection_struct_nfa1.t2q_hd), nfa_intersection_struct_nfa1.t2q_hd = NULL;
  sxfree (nfa_intersection_struct_nfa1.q_list), nfa_intersection_struct_nfa1.q_list = NULL;

  sxfree (nfa_intersection_struct_nfa2.t_set), nfa_intersection_struct_nfa2.t_set = NULL;
  sxfree (nfa_intersection_struct_nfa2.p_set), nfa_intersection_struct_nfa1.t_set = NULL;
  sxfree (nfa_intersection_struct_nfa2.t2q_hd), nfa_intersection_struct_nfa2.t2q_hd = NULL;
  sxfree (nfa_intersection_struct_nfa2.q_list), nfa_intersection_struct_nfa2.q_list = NULL;

  sxfree (nfa_intersection_working_t_set);

  XxYxZ_free (&XxYxZ_work_hd);

  sxfree (nfa_intersection_tbp_stack), nfa_intersection_tbp_stack = NULL;
  sxfree (nfa_intersection_final_state_set), nfa_intersection_final_state_set = NULL;

  sxfree (nfa_intersection_ec_stack), nfa_intersection_ec_stack = NULL;
  sxfree (nfa_intersection_ec_set), nfa_intersection_ec_set = NULL;

#ifdef ESSAI_INVERSE_MAPPING
  if (!result_is_empty && inverse_mapping) {
    SXINT nfa1, bot1, top1, state1, nfa2, bot2, top2, state2;

    /* On alloue les structures c'est l'appelant qui les liberera */
    inverse_mapping->new_state2old_state_set_id = (SXINT*) sxalloc (nfa_intersection_final_state+1, sizeof (SXINT)), inverse_mapping->new_state2old_state_set_id [0] = 0;
    XH_alloc (&(inverse_mapping->XH_state_set), "inverse_mapping->XH_state_set", nfa_intersection_final_state+1, 1, ((nfa1_final_state+nfa2_final_state)/nfa_intersection_final_state)+1, NULL, NULL);

    for (p = 1; p <= nfa_intersection_final_state; p++) {
      if (SXBA_bit_is_set (nfa_intersection_tbp_set, p)) {
	nfa1 = XxY_X (XxY_nfa_intersection_states_hd, p);
	bot1 = XH_X (XH_intersection_state_list_hd, nfa1);
	top1 = XH_X (XH_intersection_state_list_hd, nfa1+1);

	nfa2 = XxY_Y (XxY_nfa_intersection_states_hd, p);
	bot2 = XH_X (XH_intersection_state_list_hd, nfa2);
	top2 = XH_X (XH_intersection_state_list_hd, nfa2+1);

	while (bot1 < top1 || bot2 < top2) {
	  if (bot1 < top1)
	    state1 = XH_list_elem (XH_intersection_state_list_hd, bot1);
	  else
	    state1 = SXINT_MAX;

	  if (bot2 < top2)
	    state2 = XH_list_elem (XH_intersection_state_list_hd, bot2);
	  else
	    state2 = SXINT_MAX;

	  if (state1 == state2) {
	    XH_push (inverse_mapping->XH_state_set, state1);
	    bot1++;
	    bot2++;
	  }
	  else {
	    if (state1 < state2) {
	      ...
	      XH_push (inverse_mapping->XH_state_set, state2);
	      bot2++;
	    }
	  }
	}

	XH_set (&(inverse_mapping->XH_state_set), inverse_mapping->new_state2old_state_set_id + p);
      }
      else
	inverse_mapping->new_state2old_state_set_id [p] = 0;
    }

    inverse_mapping->init_state = nfa_intersection_init_state;
    inverse_mapping->final_state = nfa_intersection_final_state;
  }
#endif /* ESSAI_INVERSE_MAPPING */

  if (!result_is_empty)
    sxfree (nfa_intersection_tbp_set);

  XxY_free (&XxY_nfa_intersection_states_hd);
  XH_free (&XH_intersection_state_list_hd);
...
/* WRAPPERS */
/* Petit wrappeur autour de sxdico_get */
SXBA_INDEX
dico_seek (struct dico *dico, unsigned char *word, SXINT lgth)
{
  return sxdico_get (dico, &word, &lgth);
}

/* Petits wrappeurs autour de sxdfa_comb_seek_a_string */
SXINT
sxdfa_comb_get (struct sxdfa_comb *dico, char *word, SXINT lgth)
{
  SXINT id = sxdfa_comb_seek_a_string (dico, /* pas & */word, &lgth);
  return (lgth == 0 && id > 0) ? id : 0;
}

SXINT
sxdfa_comb_get_bounded (struct sxdfa_comb *dico, char **word_ptr, SXINT *lgth_ptr)
{
  SXINT prev_lgth = *lgth_ptr;
  SXINT id = sxdfa_comb_seek_a_string (dico, /* pas & */*word_ptr, lgth_ptr);
  if (dico->private.from_left_to_right)
    *word_ptr += prev_lgth - *lgth_ptr;
  /* else
    *word_ptr inchange -- semble jamais utilise
    */
  return (id > 0 && prev_lgth - *lgth_ptr > 0) ? id : 0;
}
