...
put_in_comb (SXINT state)
{
    ...
    SXBA_INDEX  xi, yi, x, start; /* start >= 0 */
    SXBA	set;
    SXINT		*line;
	...
	if (!SXBA_bit_is_set (comb.disp_set, start)) {
		...
		if (start+last_class > (SXBA_INDEX) dico.max)
		...
		if (mot2.print_on_sxtty) fprintf (sxtty, "\010\010\010\010\010\010%-6ld", (SXINT) ++ps_nb);
		return start;
	    }
	}
    }
...
gen (SXINT bi, SXINT bf)
{
	...
	if ((mot2.process_kind == PARTIEL) || (min > max)) {
	    ++pw_nb;
#if !EBUG
	    if (mot2.print_on_sxtty) fprintf (sxtty, "\010\010\010\010\010\010%-6ld", (SXINT) pw_nb);
#endif
	    return kw;
	}
    }
...
make_dico (void)
{
	...
	dico.partiel = mot2.process_kind == (SXUINT) (PARTIEL ? 1 : 0);
...
sxdico_process (struct mot2 input, struct dico *output)
{
    ...
    if (mot2.print_on_sxtty)
      fprintf (sxtty, "\nProcessing of %ld words (%ld characters, %ld classes)...       ",
	       (SXINT) mot2.nb, (SXINT) mot2.total_lgth, (SXINT) class_nb);
    ...
    if (mot2.print_on_sxtty) fprintf (sxtty, " done\n(state_nb = %ld, last_class = %i, teeth_nb = %ld)\n",
				      (SXINT) state_top, last_class, (SXINT) teeth_nb);
    ...
    if (mot2.print_on_sxtty) fprintf (sxtty, "Automaton packing of %ld states (%ld cells)...       ",
				      (SXINT) state_top, (SXINT) state_top*last_class);
	...
	fprintf (sxtty, "Dictionary size = %lu bytes for %ld words of %ld characters\n",
		 (SXUINT)(256+(dico.max+1)*(sizeof (SXINT))), (SXINT) mot2.nb, (SXINT) mot2.total_lgth);
    }
...
sxdico2c (struct dico *sxdico2c_dico, FILE *file, char *dico_name, SXBOOLEAN is_static)
{
  ...
  for (i = 0; i <= sxdico2c_dico->max; i++) {
    ...
    fprintf (file, "%ld, ", (SXINT) sxdico2c_dico->comb_vector [i]);
  }
  ...
  fprintf (file, "%ld /* max */, %ld /* init_base */, %ld /* class_mask */, %ld /* stop_mask */, %ld /* is_suffix_mask */, %ld /* base_shift */, %ld /* partiel */,\n",
	  (SXINT) sxdico2c_dico->max, (SXINT) sxdico2c_dico->init_base, (SXINT) sxdico2c_dico->class_mask, (SXINT) sxdico2c_dico->stop_mask, (SXINT) sxdico2c_dico->is_suffix_mask, (SXINT) sxdico2c_dico->base_shift, (SXINT) sxdico2c_dico->partiel);
