...
char WHAT_L_ACT[] = "@(#)L_act.c\t- SYNTAX [unix] -  Mar 15 Mar 1994 11:11:03";
...
static SXINT	*rule_stack, *rule_attr_stack, *block_stack, *reactivate_stack, *deactivate_stack,
    *xtnt_stack;
static SXINT	*symbols;
static int symbols_size, symbols_top;
static SXINT	symbs [7];
static int	block_level, block_nb, current_block_nb, basic_type_name, current_listof_level,
                listof_level, prev_listof, prev_recordtype_lhs, sem_stack_size;
static SXINT    record_name;
...
static void symbol_table_oflw (SXINT old_size, SXINT new_size)
{
    (void) old_size;
    st_to_attr = (struct st_attr*) sxrealloc (st_to_attr, new_size + 1, sizeof (struct st_attr));
}

static void type_table_oflw (SXINT old_size, SXINT new_size)
{
    (void) old_size;
    tt_to_attr = (struct tt_attr*) sxrealloc (tt_to_attr, new_size + 1, sizeof (struct tt_attr));
}

static void type_steXblock_nb_oflw (SXINT old_size, SXINT new_size)
{
    (void) old_size;
...
static void symbols_oflw (int size)
{
    do {
	symbols_size *= 2;
    } while (size > symbols_size);

    symbols = sxrealloc (symbols, symbols_size + 1, sizeof (int));
}

static SXINT desambig ()
{
...
static void desactivate_var_rules (int name)
{
...
static void desactivate_type_rules (int name)
{
...
static void push_rule_attr (int rule, int action, int priority, int associativity)
{
    int x;
    bool oflw;
...
static int add_rule (SXINT *arg_symbols, int rhs_lgth, bool clean, int act, int prio, int assoc)
{
    ...
    SXINT rule, action, priority, associativity;

    if (sxdp_rule_set (rhs_lgth, arg_symbols, &rule, 0, prio, assoc))
    {
...
static int build_listof (int arg_current_listof_level, int arg_basic_type_name);

static int get_LHS_or_FUN (int arg_current_listof_level, int arg_basic_type_name, int kind)
{
    SXINT  couple;
    int    nt;
    ...
    if (arg_basic_type_name > 0)
	pattr = &(tt_to_attr [arg_basic_type_name]);

    if (arg_current_listof_level == 0)
    {
	switch (kind)
	{
	case 1:			/* LHS */
	    nt = (arg_basic_type_name < 0)
		? (arg_basic_type_name == -KW_INT
		      ? INT_LHS
		      : BOOL_LHS)
		: pattr->lhs;
	    break;
    ...
    {
	couple = build_listof (arg_current_listof_level, arg_basic_type_name);
...
{
    SXINT	couple;
    int		LISTOFnm1_TYPE, LISTOFn_TYPE, LISTOFn_TYPE_LHS,
                LISTOFn_TYPE_FUN, LISTOFn_TYPE_VAR, rule;
    bool	is_new;
    struct listof_attr  *pl;

    is_new = !XxY_set (&listof_hd, arg_current_listof_level, arg_basic_type_name, &couple);
    pl = &(listof_to_attr [couple]);

    if (is_new || !pl->is_activated)
    {
	LISTOFnm1_TYPE = (arg_current_listof_level > 1)
	    ? listof_to_attr [build_listof (arg_current_listof_level - 1, arg_basic_type_name)].expr
		: get_LHS_or_FUN (0, arg_basic_type_name, 3 /* EXPR */);
...
static void rules_at_block_exit (bool delete)
{
    int rule, x;
...
static void ACTION_2 (bool delete)
{
    int tnt, name, x;
...
static SXSEMACT_FUNCTION semact;

int L_semact (SXINT what, SXINT act_no, SXTABLES *arg)
{
    /* Filtre les vrais appels a "semact". */
    switch (what) {

    case SXACTION:
	/* On memorise le numero de reduction */
	/* Utilise' ds les "with" pour recuperer, a partir du non-terminal en pg
	   les regles generees lors de la declaration du corps d'un record (voir
...
{
    SXINT		name, x;
    int			rule, xt, lhs_case, prev_xname, prev_lhs_case,
                        prev_block, param_type_name;
    bool		is_new_name;
    struct tt_attr	*ptt_attr;

    switch (what) {
    case SXOPEN:
        /* On filtre les appels a "semact" pour utiliser "sem_stack". */
        semact = sxplocals.SXP_tables.P_semact;
	sxplocals.SXP_tables.P_semact = L_semact;
...
*/
	    ...
	    */
		...
		else if (record_name != 0)
		{
		    ...
		    /* On change les prefixes par :
		       ...
		       La 2eme est necessaire car une fonction "f" peut rendre un record
	...
	case 13: /* <SUBTYPE_NAME+> = <SUBTYPE_NAME+> , %id @13 ;
	     ...
	     *
	     *<BOOL_EXPR>	= <SHORT_EXPR> comp_op <SHORT_EXPR> ;
	     *
	...
	case 19: /* <TYPE_DECL> = record @19 %id 
		...
		int recordtype_var, recordtype_lhs, recordtype_fun, recordtype_name;
		SXINT couple;
		bool	already_found;
		bool	already_found;
		struct type_steXblock_nb_to_attr	*ptb_to_attr;
...
/* @20 et @21 doivent-ils etre mis dans SXDO/SXUNDO ?? */
	case 20: /* <STMT> = with <RECORDTYPE_LHS> do @20 <STMT> ";" @21 ; */
	    /* @20 : Pour toute regle "lhs -> <RECORDTYPE_VAR> . rhs" generee par le traitement
