...
FILE	*sxtty, *sxstdout, *sxstderr;

#define TIME_INIT	0
#define TIME_FINAL	1

void sxtime (SXINT what, char *str)
{
...
struct for_parsact {
    ...
    void 	(*sem_final) (int);
    bool	(*prdct) (void);
    bool	(*action) (int, int, ushort *, int*);
    bool	(*constraint) (int, int, int, int);
};
...
static struct for_parsact for_parsact;
...
static bool RL_mreduction_item (int item_j, SXBA I, int j)
{
    ...
    ap_set = ap_sets [Y];
    is_new = SXBA_bit_is_reset_set (RL_nt_set, Y);
    if (is_new)
    {
...
static void RL_mreduction (void)
{
    int				j, A, order, tnb;
    SXBA			ap_set;
    struct parse_item		*PTj;
...
static void print_prod (int i, int k, ushort *prod_core)
{
...
static bool ksem_incr (int nbnt, int *arg_rhs_stack)
{
    int x, i, shift;
	...
	if (i < max_tree_nb && ARN_disp_ksem [arg_rhs_stack [x+1]][i].val != -1)
	{
	    ksem_vector |= (i << shift);
	    return true;
	}
...
static int ARN_keval (int Aij, int i, int j, int *arg_rhs_stack)
{
	...
	else
	{
		...
		for (x = 0; x < nbnt; x++, shift += log_max_tree_nb)
		{
		    ...
		    val *= ARN_disp_ksem [arg_rhs_stack [x+1]][vectx].val;
		}
	    ...
	    } while (ksem_incr (nbnt, arg_rhs_stack));
	}
    }
...
static bool ARN_kparsact (int i, int j, ushort *prod_core, int arg_rhs_stack [])
{
    ...
    if ((top = ARN_keval (prod, i, j, arg_rhs_stack)) > 0)
    {
...
static void ksem_bu_extract (void)
{
...
static bool ARN_eval (int prod, int i, int j, int *arg_rhs_stack, float *val)
{
    int		Aij, nbnt, Bhk;
    float	proba;
...
static bool ARN_parsact (int i, int j, ushort *prod_core, int arg_rhs_stack[])
{
    int 	Aij, prod;
    int		nbnt;
    float	val;
    ...
    if (ARN_eval (prod, i, j, arg_rhs_stack, &val))
    {
	/* val est positionne */
...
static void sem_bu_extract (void)
{
...
static bool spf_count_parsact (int i,  int j, ushort *prod_core, int arg_rhs_stack [])
{
    int		Aij;
    double	val;
    int 	prod, nbnt;
	...
	if (nbnt > 0)
	{
	    val = spf_count [arg_rhs_stack [nbnt]];
...
\t%s Lexical error at char #%ld\n", ME, (TOK - tok) + 1);
    ...
    *++tok = tmax;
    ...
    if (print_time)
	sxtime (TIME_INIT, "" /* chaine vide ajoutee par Hubert */);
