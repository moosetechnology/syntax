...
put_prdct (SXNODE *visited)
{
  ...
  if (visited->position == 2) {
	...
	top += tnb+1;
      ...
      printf ("&%ld ", (SXINT) prdct);
      fprintf (outfile, "&%ld ", (SXINT) prdct);
    }
  }
}
...
tree_walk (SXNODE *visited)
{
  ...
  for (;;) {
    ...
    case X_LHS_n:
      ...
      printf ("%ld : %s ", (SXINT) ++prod, sxstrget (ste));
	...
	*p = '"';
	...
	printf ("/* %ld */ %ld,\n", (SXINT) prdct, (SXINT) prdct_disp [prdct]);
      }
	...
	for (x = XH_X (rhs_tlist, prdct); x < y; x++) {
	  printf ("%ld, ", (SXINT) XH_list_elem (rhs_tlist, x));
	}
...
/*-----------------------------------------------*/
  ...
  if (is_predicate /* Le scanner a trouve des predicats */ ) {
    sxtmsg (prgentname, "%sOnly predicate-free grammars can be transformed.", sxtab_ptr->err_titles [2]+1 /* error */);
    return;
  }
