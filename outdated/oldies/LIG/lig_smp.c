...
char WHAT_LIG_SMP[] = "@(#)lig_smp.c	- SYNTAX [unix] - Ven 15 Nov 1996 10:14:21";
...
static void print_comment (char *comment)
{
  if (comment != NULL)
    {
      printf ("%s", comment);
...
static void lig_check_nt (struct lig_node *visited)
{
...
static void lig_check_productions (struct lig_node *visited)
{
	...
	if (lhs_son->name != VOID_n) {
		...
		if (is_normal_form)
		    sxerror (lhs_son->son->token.source_index,
				 "%sNot in normal form.",
	...
	for (rhs = lhs->brother->brother->son /* On saute = et on prend le fils de VOCABULARY_S */;
		...
		if (rhs_son->name != VOID_n) {
			...
			if (++P_object_nb > 2)
			    sxerror (rhs->son->brother->token.source_index,
			...
			if (is_normal_form)
			    sxerror (rhs_son->son->token.source_index,
					 "%sNot in normal form.",
					 sxtab_ptr->err_titles [1]);
...
static void lig_pi ()
{
	...
	case 4 :		/* SXVISITED->name = VOID_n */
		...
		sxerror (SXVISITED->father->son->token.source_index,
...
static bool lig_write ()
{
    static char		ME2 [] = "lig_write";
    long		lt;
	...
	fprintf (sxstderr, "%s: cannot open (create) ", ME2);
	sxperror (file_name);
	return false;
    }

    tmp_vstr = varstr_alloc (64);
...
/* Allows the access to SXLIGprdct when the parser is sxndparser */\n", F_lig);
    ...
    if (!is_reduced)
	sxba2c (RDGt_set, F_lig, "SXLIGt_set", "", true /* is_static */, tmp_vstr);
	...
	if (SXLIGEQUALn_set == NULL)
	    ...
	    sxbm2c (F_lig, SXLIGEQUALn_set, inputG.maxnt+1, "SXLIGEQUALn_set", "", true /* is_static */, tmp_vstr);

	if (SXLIGPUSHPOPn_set == NULL)
	    fputs ("\n\nstatic SXBA *SXLIGPUSHPOPn_set;\n", F_lig);
	else
	    sxbm2c (F_lig, SXLIGPUSHPOPn_set, inputG.maxnt+1, "SXLIGPUSHPOPn_set", "", true /* is_static */, tmp_vstr);
	...
	if (SXLIGPOPn_set == NULL) {
	    ...
	    sxbm2c (F_lig, SXLIGPOPn_set, inputG.maxnt+1, "SXLIGPOPn_set", "", true /* is_static */, tmp_vstr);
	}
...
static void lig_reduction ()
{
    int ssymb, prod, item, i, X, nb;
...
void lig_smp (int what, SXTABLES *sxtables_ptr)
{
    switch (what) {
    case SXOPEN:
