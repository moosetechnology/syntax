...
static SXINT		*block_level_stack, *type_specifier_stack, *struct_level_stack, *name_id_stack;
...
static int t_to_type (int type, int t, struct sxsource_coord *si)
{
...
static int process_tag_name (int string_table_entry, int kind /* STRUCT_ or UNION_ */, int struct_level, struct sxsource_coord *source_index)
{
    bool	is_ok;
    SXINT	x;
...
static int get_indirection_level (int n, SXNODE *node)
{
...
static int get_closed_il (struct attribute **ppattr)
{
    int 		tmp_type_specifier, n;
    struct attribute	*tmp_pattr = *ppattr;
    
    tmp_type_specifier = tmp_pattr->type_specifier;
    n = get_indirection_level (0, tmp_pattr->declarator);
    
    while (tmp_pattr->name_id != 0 &&
	   (tmp_type_specifier & BASIC_TYPE_RANGE_) != STRUCT_ &&
	    (tmp_type_specifier & BASIC_TYPE_RANGE_) != UNION_) {
	/* tmp_pattr->name_id designe un typedef */
	tmp_pattr = ids_attr + tmp_pattr->name_id;
	tmp_type_specifier = tmp_pattr->type_specifier;
	n += get_indirection_level (0, tmp_pattr->declarator);
    }
...
static void check_unary_operator (SXNODE *visited)
{
...
static void dc_pd ()
{
	...
	if (operand_type.name_or_type != 0) {
	    ...
	    if (operand_type.name_or_type == 0)
		sxerror (SXLAST_ELEM->token.source_index,
			     sxplocals.sxtables->err_titles [2][0],
			     "%sIllegal indexing.",
			     sxplocals.sxtables->err_titles [2]+1);
	}
...
void dc_smp (int what, SXTABLES *sxtables_ptr)
{
    switch (what) {
    case SXOPEN:
