...
char WHAT_ARN_NO_PARSACT[] = "@(#)ARN_parsact.c	- SYNTAX [unix] - Mer 31 Mai 1995 10:50:06";
...
static SXINT ARN_symbols_oflw (SXINT old_size, SXINT new_size)
{
    (void) old_size;
    (void) new_size;
#if 0
...
static SXINT ARN_parsers_oflw (SXINT old_size, SXINT new_size)
{
    (void) old_size;
    (void) new_size;
...
static SXINT ARN_action_new_top (SXINT bot, SXINT new_top, SXINT symbol)
{
...
static void ARN_build_a_rule (int parser, int rhs)
{
    ...
    (void) new_symbol (parse_stack.ared->reduce,
		    0,
...
static SXINT ARN_action_final (SXINT arg)
{
