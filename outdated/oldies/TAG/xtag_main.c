...
char WHAT_XTAG_MAIN[] = "@(#)xtag_main.c\t- SYNTAX [unix] - Mer 4 Jan 1995 10:14:22";
...
extern void xtag_second_pass (void);

long             options_set;
bool             is_normal_form, is_test_empty_stack;
char             prgentname [32];
SXTABLES *sxtab_ptr;

extern SXTABLES	sxtables;

extern void sxatc (SXINT what, SXINT action_no, SXTABLES *arg);
...
static void	xtag_run (char *pathname)
{
...
/* Decodage des options */
	...
	case TEST_EMPTY_STACK:
	    is_test_empty_stack = true, options_set |= OPTION (TEST_EMPTY_STACK);
	    break;

	case -TEST_EMPTY_STACK:
	    is_test_empty_stack = false, options_set &= noOPTION (TEST_EMPTY_STACK);
	    break;
...
run:
	...
	do {
	    ...
	    if (argnum < argc)
	      ...
	      sxatc (SXCLOSE, SXANY_INT, &sxtables);
