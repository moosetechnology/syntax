...
char WHAT_XTAG_SMP[] = "@(#)xtag_smp.c	- SYNTAX [unix] - Mer 18 Jan 1995 10:30:38";
...
static SXINT         current_node_address;
...
static SXINT          *axiomes;

static VARSTR       local_address;

  
static void node_address_oflw (SXINT old_size, SXINT new_size)
{
  (void) old_size;
...
static void xtag_first_pass (SXNODE *visited)
{
  SXNODE         *son = visited->son;
  unsigned int *pkind;
  int          father_node_address, son_node_address;
  ...
  case ATTRIBUT_S_n :
	...
	if ((node_address2attr [current_root_address].kind & NA_c) == 0)
	  {
	    sxerror (node_address2attr [current_root_address].source_index,
			 sxtab_ptr->err_titles [2][0],
  ...
  case FOOTP_n :
	...
	if (visited->father->brother != NULL) 
	  {
	    sxerror (visited->son->token.source_index,
			 sxtab_ptr->err_titles [2][0],
			 "%sShould be a leaf.",
			 sxtab_ptr->err_titles [2]+1);
	  }
	...
	if (current_node_name != current_root_name)
	...
	else
	  {
	    if (tree2attr [current_tree_index].is_aux) 
	      {
		sxerror (visited->father->token.source_index,
			 sxtab_ptr->err_titles [2][0],
			 "%sMultiple foot node.",
			 sxtab_ptr->err_titles [2]+1);
	      }
...
static void process_adjunction (int arg_tree_nb, int lhs_address, char *string_address)
{
  /* On est sur le noeud d'adresse lhs_address dans l'arbre arg_tree_nb,
  ...
  /* Racine,  initial             : <A.n.0.t> = <A.t> ( ) ;           
     ...
     interne, aux, !is_on_spine   : <A.address.t> = <A.t> ( ) ;   
                                  : <A.b> ( ) = <A.address.b> ;
     interne, aux, is_on_spine    : <A.address.t> ( .. arg_tree_nb ) = <A.t> ( .. arg_tree_nb ) ;
                                  : <A.b> ( .. ) = <A.address.b> ( .. ) ;
  */
  ...
  if (tree2attr [arg_tree_nb].is_aux)
    {
      /* adjonction dans un arbre auxiliaire */
      if (*string_address != SXNUL)
	{
	  /* Noeud interne */
	  printf ("<%s.%i%s.t>", A, arg_tree_nb, string_address);

	  if (pattr->is_on_spine)
	    printf (" ( .. %i )\t= <%s.t> ( .. %i ) ;\n", arg_tree_nb, A, arg_tree_nb);
	  else
	    printf ("\t= <%s.t> ( ) ;\n", A);
	}
  ...
  if (tree2attr [arg_tree_nb].is_aux)
      ...
      if (*string_address != SXNUL)
	{
	  /* Noeud interne */
	  if (pattr->is_on_spine)
	    printf ("<%s.b> ( .. )\t= <%s.%i%s.b> ( .. ) ;\n", A, A, arg_tree_nb, string_address);
	  else
	    printf ("<%s.b> ( )\t= <%s.%i%s.b> ( ) ;\n", A, A, arg_tree_nb, string_address);
	}
	/* else Racine (erreur) */
    ...
    {
      /* adjonction dans un arbre initial */
      printf ("<%s.b> ( )\t= <%s.%i", A, A, arg_tree_nb);
...
static void process_lhs (int arg_tree_nb, int lhs_address, char *string_address)
{
  char	*A;
  struct node_address2attr *pattr = node_address2attr + lhs_address;
  ...
  /* Si OA et !AUX => Erreur */
	 ...
	 Aux,     Interne,  is_on_spine, !OA,  KNEE !AUX	: "<A.address.b> ( .. arg_tree_nb ) = "
	 Aux,     Interne,  is_on_spine, !OA   KNEE  AUX 	: "<A.address.t> ( .. ) = 
	                                                           <A.address.b> ( .. ) ;"
	 							: "<A.address.b> ( .. arg_tree_nb ) = "
	 Aux,     Interne,  is_on_spine,  OA   KNEE  AUX 	: "<A.address.b> ( .. arg_tree_nb ) = "
      */
  ...
  if (tree2attr [arg_tree_nb].is_aux)
      ...
      else
	{
	  ...
	  if (pattr->is_on_spine)
		...
		{
		  /* Pas d'adjonction */
		  printf (".%i%s> ( .. ", arg_tree_nb, string_address);

		  if (pattr->kind & KNEE_c)
		    /* Pere du pied */
		    printf ("%i ", arg_tree_nb);
		  ...
		  if ((pattr->kind & OA_c) == 0)
		    {
			  ...
			  string_address, A, arg_tree_nb, string_address, A);
		    }
		  
		  printf (".%i%s.b> ( .. ", arg_tree_nb, string_address);
	      ...
	      else
		{
		  ...
		  if ((pattr->kind & OA_c) == 0)
		    {
		      /* non obligatoire */
		      printf (".%i%s.t>\t= <%s.%i%s.b> ( ) ;\n<%s", arg_tree_nb, string_address, A,
		      arg_tree_nb, string_address, A);
		    }
      ...
      if (*string_address == SXNUL)
	{
	  ...
	  if ((pattr->kind & NA_c) || !SXBA_bit_is_set (aux_set, pattr->node_name))
		...
		{
		  /* non obligatoire */
		  printf (".%i.0.t>\t= <%s.%i.0.b> ;\n<%s", arg_tree_nb, A, arg_tree_nb, A);
		}

	      printf (".%i.0.b>", arg_tree_nb);
	    }
	}
      else 
	{
...
static void process_rhs (int arg_tree_nb, int rhs_address, char *string_address)
  {
    ...
    /* On genere 
       ...
       Si aux, interne,  is_on_spine, !AUX, father == ROOT      <A.address> ( .. arg_tree_nb )
       Si aux, interne,  is_on_spine, !AUX, father != ROOT      <A.address> ( .. )
       Si aux, interne,  is_on_spine,  AUX, father == ROOT      <A.address.t> ( .. arg_tree_nb )
	...
	if (tree2attr [arg_tree_nb].is_aux)
	  {
		...
		if (pattr->is_on_spine)
	      ...
	      {
		/* Noeud interne */
		printf (".%i%s.%i", arg_tree_nb, string_address, position);
...
static void process_rule (int arg_tree_nb, int lhs_address, char *lhs_string_address)
{
  int son, nadd;
  struct node_address2attr *pattr = node_address2attr + lhs_address;
  
  process_lhs (arg_tree_nb, lhs_address, lhs_string_address);

  for (son = 1; son <= pattr->sons_nb; son++) 
    {
      nadd = XxY_is_set (&node_address, lhs_address, son);
      process_rhs (arg_tree_nb, nadd, lhs_string_address);
    }

  printf (";\n");

  process_adjunction (arg_tree_nb, lhs_address, lhs_string_address);
}



static void process_tree (int arg_tree_nb, int root_address, VARSTR arg_local_address)
{
  ...
  length = varstr_length (arg_local_address);
      
  if (component != 0)
    {
	  /* inside node */
      sprintf (string, ".%i", component);
      varstr_catenate (arg_local_address, string);
    }

  if (sons_nb > 0)
    {
  
      process_rule (arg_tree_nb, root_address, varstr_tostr (arg_local_address));
  
      for (son = 1; son <= sons_nb; son++) 
	{
	  nadd = XxY_is_set (&node_address, root_address, son);
	  process_tree (arg_tree_nb, nadd, arg_local_address);
	}
    }
  else
    process_adjunction (arg_tree_nb, root_address, varstr_tostr (arg_local_address));   
  
  if (component != 0)
    *(arg_local_address->current = arg_local_address->first + length) = SXNUL;
}


void xtag_second_pass ()
{
      /* Sortie de la LIG */
  ...
  if (!IS_ERROR) {
    ...
    if (sxverbosep) {
	    fprintf (sxtty, "\tLIG generation from a TAG with %ld nodes in %i elementary trees\n",
...
void xtag_smp (int what, SXTABLES *sxtables_ptr)
{
  int local_tree_nb;
  ...
  case SXACTION:
	...
	if (SXSTRtop () > (SXINT) SXBASIZE (aux_set))
