;;; Modified Wed Aug 13 18:34:01 1986		;;;
...
;(bind-to-key "buffer-edit" "\^X\^B"); PhD: removed
...
(progn loop
	...
	(local-bind-to-key "ebuf-revert" "R")
;(local-bind-to-key "ebuf-beep" "\^X\^B"); PhD: removed
...
;(local-bind-to-key "redraw-display" "\^L"); PhD: removed
	(local-bind-to-key "previous-page" "\ev")
	(local-bind-to-key "previous-page" "\eV")
	(local-bind-to-key "scroll-one-line-up" "\^Z\^V"); PhD
	(local-bind-to-key "scroll-one-line-down" "\ez"); PhD
	(local-bind-to-key "beginning-of-file" "\e<")
	(local-bind-to-key "end-of-file" "\e>")
	(local-bind-to-key "return-to-monitor" "\")
;(local-bind-to-key "exit-emacs" "\^X\^C"); PhD:removed
	(local-bind-to-key "delete-window" "q"); PhD
	(local-bind-to-key "delete-window" "Q"); PhD
    )
)
