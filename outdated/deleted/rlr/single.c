...
char WHAT_SINGLE[] = "@(#)single.c	- SYNTAX [unix] - 13 octobre 1992";
    ...
    int		nt, prdct;
{
    ...
    if (xntstr + strlen (s) + 1 + (is_new_xnt ? 0 : get_nt_length (nt)) > bnf_ag.WS_TBL_SIZE.nt_string_length) {
...
partial_spe (single_prod, restriction_set)
    SXBA	single_prod, restriction_set;
{
    /*  ELIMINATION PARTIELLE DES PRODUCTIONS SIMPLES */

    int		*pd_set, *pd_to_pg, *pg_to_act;
	...
	if ((astatei = nt_states + i)->lgth > 0) {
		...
		if (aitem->check > bnf_ag.WS_TBL_SIZE.ntmax) {
		    ...
		    pg_to_act [pd_to_pg [aitem->check] = bnf_ag.WS_NBPRO [afe->reduce].reduc] = -1
