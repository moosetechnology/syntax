...
#include "f77_td.h"
#include <ctype.h>
char WHAT_F77ANALYSE[] = "@(#)SYNTAX - $Id: f77_analyse.c 1558 2008-09-04 11:55:31Z rlacroix $";

/* Il faudrait INTERDIRE l'insertion d'un EOL par le rattrapage d'erreur
   du scanner. Utiliser sxs_srcvr? */


extern SXBOOLEAN		is_ansi, is_pretty_printer; /* Options d'appel */
...
static SXSHORT		buf [74], prev_source_char;
static SXINT		comment_line_no, continuation_line_no;
struct source_item {
    SXSHORT			c;
    struct sxsource_coord	source_index;
};
...
static SXINT	comment_lgth, cur_comment_lgth;
...
static SXBOOLEAN	is_function_stmt, is_iokw_allowed;
...
static struct sxparse_mode	do_parse_mode_nomess = {SXPARSER, SXWITHOUT_ERROR, 0, 0, 0, SXTRUE, SXTRUE, SXFALSE, SXFALSE, SXFALSE, SXFALSE};
static struct sxparse_mode	format_parse_mode_nomess = {SXPARSER, SXWITH_CORRECTION, 0, 0, 0, SXTRUE, SXTRUE, SXFALSE, SXFALSE, SXFALSE, SXFALSE};
/*
...
#ifdef EBUG
static SXVOID print_toks_buf (void)
{
    SXINT i;
    struct sxtoken	*pt;
	...
	fprintf (sxstdout,
		 ...
		 "EMPTY_STE" : "ERROR_STE"),
		 (long unsigned int)pt->source_index.line,
		 (long unsigned int)pt->source_index.column,
		 pt->comment);
    }
}
#endif
...
static SXVOID	comment_put (char **comment_local, 
			     SXINT *comment_lgth_local, 
			     char *str, 
			     SXINT lgth)
{
    if (*comment_local == NULL) {
	strcpy (*comment_local = (char*) sxalloc (lgth + 1, sizeof (char)), str);
	*comment_lgth_local = lgth;
    }
    else {
	strcpy ((*comment_local = (char*) sxrealloc (*comment_local, *comment_lgth_local +
	     lgth + 1, sizeof (char))) + *comment_lgth_local, str);
	*comment_lgth_local += lgth;
    }
}
...
static SXBOOLEAN fill_buf (void)
{
    SXSHORT c;
    SXUINT x;
    SXBOOLEAN	ligne_vide = SXFALSE, is_an_initial_line, is_header_empty;
    struct sxsource_coord source_index;
...
static SXVOID get_next_source_item (void)
{
...
processing the enclosing do labeled %i and terminated at line %i.",
	...
	case 1: {
	    /* Post action de %id */
	    SXINT			Mtok_no, lahead, cur_lahead, tok_no, tail_tok_no;
	    char	*str;
	    SXINT	strlgth;
	    SXBOOLEAN		after_logical_if;
		...
		while (after_logical_if) {
			...
			case END_t:
			    /* c'est une instruction SXEND, le "D" doit etre
...
this expression is erased.",
		...
		if (ETAT == ETAT_0) {
			...
			if (++main_program_nb > 1) {
			    sxerror (SXGET_TOKEN (tail_tok_no).source_index,
				     sxsvar.sxtables->err_titles [2][0] /* Error */,
...
This one, and all others in the sequel, are converted into a legal form.",
	...
	case 4: {
		...
		if (val > 0) {
		    ...
		    sprintf (string, "%.2ld", (long)(val_init - val));
		    sxsvar.sxlv.ts_lgth = 0;
