...
#include "varstr.h"
#include "sxunix.h"
char WHAT_LFGLEXSMP[] = "@(#)SYNTAX - $Id: lfg_lex_smp.c 2227 2018-09-26 16:22:59Z garavel $" WHAT_DEBUG;
  ...
  case ATTRIBUTE_n :
    ...
    if (max_local_atom_id > 31) {
      sxhmsg (SXVISITED->token.source_index.file_name, 
	      "Implementation restriction : for some types, the number of their atomic values exceeds 31.",
...
check_referenced_field_set (void)
{
  ...
  if (field_id>=0) {
    ...
    sxtmsg (sxatcvar.atc_lv.abstract_tree_root->token.source_index.file_name, 
	    "The following field name%s never used:\n\
\"%s\".",
...
smppass (void)
{
  ...
  /* Pour avoir une idee des tailles!! */
 {
  ...
  if (t_id2t == NULL) {
    sxtmsg (sxatcvar.atc_lv.abstract_tree_root->token.source_index.file_name, 
	    "The \"TERMINAL\" chapter is missing.\n",
	    sxtab_ptr->err_titles [2]);
  }
  else {
	...
	if (t_id > 0)
      ...
      sxtmsg (sxatcvar.atc_lv.abstract_tree_root->token.source_index.file_name, 
	      "The following terminal name%s not defined:\n\
\"%s\".",
	      sxtab_ptr->err_titles [1],
	      nb == 1 ? " is" : "s are",
	      varstr_tostr (vstr));
...
#define PUSH(s,x)	(s)[++*(s)]=(x)
	...
	if (((((SXUINT) (~0))>>dico.base_shift) < dico.max)) {
	  sxtmsg (SXGET_TOKEN (0).source_index.file_name,
		  "Too many amalgam definitions, dictionary not produced\n",
		  sxtab_ptr->err_titles [2]);
	  dico_OK = SXFALSE;
	}
      }
...
SXINT hd, next;\n\
	...
	if ((((SXUINT) (~0))>>dico.base_shift) < dico.max) {
	  sxtmsg (SXGET_TOKEN (0).source_index.file_name,
		  "Too many terminal definitions, dictionary not produced\n",
		  sxtab_ptr->err_titles [2]);
	  dico_OK = SXFALSE;
	}
      }
	...
	if ((((SXUINT) (~0))>>dico.base_shift) < dico.max) {
	  sxtmsg (SXGET_TOKEN (0).source_index.file_name,
		  "Too many lexical definitions, dictionary not produced\n",
		  sxtab_ptr->err_titles [2]);
	  dico_OK = SXFALSE;
	}
      }
...
", (long)max_field_id+1);
	...
	sxtmsg (SXGET_TOKEN (sxplocals.Mtok_no).source_index.file_name,
		"Too many lexeme definitions, dictionary not produced\n",
		sxtab_ptr->err_titles [2]);
      else {
	fputs ("\n\n#ifdef LEXICON\n", stdout);
