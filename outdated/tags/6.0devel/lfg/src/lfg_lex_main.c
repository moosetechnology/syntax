...
#include "sxversion.h"
#include "sxunix.h"
char WHAT_LFGLEXMAIN[] = "@(#)SYNTAX - $Id: lfg_lex_main.c 2227 2018-09-26 16:22:59Z garavel $" WHAT_DEBUG;
...
static SXVOID	lfg_run (char *pathname)
{
	...
	rewind (infile);
	syntax (SXBEGIN, &lfg_lex_tables, infile, "");
	...
	syntax (SXBEGIN, &lfg_lex_tables, infile, pathname);
	stat (sxsrcmngr.source_coord.file_name, &buf);
	lfg_lex_time = buf.st_mtime;
    }

    syntax (SXACTION, &lfg_lex_tables);
    syntax (SXEND, &lfg_lex_tables);
    fclose (infile);
