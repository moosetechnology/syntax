...
char WHAT_XXYXZ_MNGR[] = "@(#)SYNTAX - $Id: XxYxZ_mngr.c 2279 2020-03-10 11:21:28Z garavel $" WHAT_DEBUG;
...
void XxYxZ_array_to_c (XxYxZ_header *header, FILE *F_XxYxZ /* named output stream */, char *name)
{
	...
	if ((i_counter & 3) == 0)
	    fprintf (F_XxYxZ, "\n/* %ld */ ", (unsigned long)i_counter);

	fprintf (F_XxYxZ, "{%ld, %ld, %ld}, ",
		 (long)header->display [i_counter].X, (long)header->display [i_counter].Y, (long)header->display [i_counter].Z);
    } 
	...
	if (size != 0) {
		...
		fprintf (F_XxYxZ, "%ld, ", (long)header->lnk_hd [kind] [i_counter]);
	    } 
		...
		fprintf (F_XxYxZ, "%ld, ", (long)header->lnk [kind] [i_counter]);
	    } 
