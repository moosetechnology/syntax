...
char WHAT_SXDAG_MNGR[] = "@(#)SYNTAX - $Id: fsa_mngr.c 2289 2020-03-10 14:17:38Z garavel $" WHAT_DEBUG;
	       ...
	       )
{
  ...
  */
  ...
  for (new_state = init_state; new_state < final_state; new_state++) {
    (*normalized_fsa_forward_trans)(to_be_sorted [new_state], (void (*)(SXINT, SXINT, SXINT))output_new_trans);
...
print_configuration (void)
{
  ...
  for (part = 1; part <= last_partition; part++) {
    ...
    fprintf (sxstderr, "Partition %ld = {%ld", (long)part, (long)state);

    while ((state = state2attr [state].next_state)) {
      fprintf (sxstderr, ", %ld", (long)state);
    }
...
char WHAT_FSA_TO_RE[] = "@(#)SYNTAX - $Id: fsa_mngr.c 2289 2020-03-10 14:17:38Z garavel $" WHAT_DEBUG;
...
sxdfa_comb2c (struct sxdfa_comb *sxdfa_comb_ptr, FILE *file, char *dico_name, SXBOOLEAN is_static)
{
  ...
  if (sxdfa_comb_ptr->base2stack) {
	...
	fprintf (file, "\n/* %ld */ ", i);
      ...
      fprintf (file, "%ld, ", (long)sxdfa_comb_ptr->base2stack [i]);
    }
...
nfa_intersection (SXINT nfa1_init_state, 
		  ...
		  SXINT nfa1_max_state,  /* en général, == 0 ; si > 0, on est dans le cas DAG (pas de trans
					    ...
					    final_state != max_state, et on a le droit d'avoir des transitions
					    vers final_state qui ne sont pas sur eof_ste */
		  SXINT nfa1_eof_ste, /* il s'agit vraiment d'un ste seulement si le NFA1 a pour étiquettes de transitions des ste */
		  ...
		  )
{
...
#if EBUG
  if (nfa1_eof_ste != nfa2_eof_ste)
    sxtrap (ME, "nfa_intersection (Cannot intersect two NFAs with distinct EOF codes)");
#endif /* EBUG */
  ...
  */
  ...
  nfa_intersection_working_t_set = sxba_calloc (nfa1_eof_ste+1);
  ...
  if (!result_is_empty) {
	...
	XxYxZ_Xforeach (XxYxZ_work_hd, p, triple) {
	  ...
	  if (SXBA_bit_is_set (nfa_intersection_tbp_set, q))
	    (*nfa_intersection_fill_trans)(p, XxYxZ_Y (XxYxZ_work_hd, triple), q, (SXBOOLEAN) (q == nfa_intersection_final_state));
	}
      }
    }
  }
...
{
  return (SXINT) sxdico_get (dico, &word, &lgth);
}
