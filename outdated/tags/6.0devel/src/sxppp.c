...
char WHAT_SXPPP[] = "@(#)SYNTAX - $Id: sxppp.c 2240 2018-10-25 09:47:19Z garavel $" WHAT_DEBUG;
...
static SXVOID	string_manager (SXINT ste, SXINT lahead)
{
    ...
    {
      printf ("string_manager (%ld, %ld):\t", (long)ste, (long)lahead);
    }
...
static SXVOID	sxppopen (struct sxtables *sxtables)
{
	...
	sxhmsg (sxsrcmngr.source_coord.file_name, "%s: incorrect \"max_margin\". Value taken: %d.", sxtables->err_titles [0], ME, sxppvariables.max_margin = (sxppvariables.line_length * 2) / 3);
	...
	sxhmsg (sxsrcmngr.source_coord.file_name, "%s: incorrect tabulation interval. Value taken: %d.", sxtables->err_titles [0], ME, sxppvariables.tabs_interval = (DFLT_TABS_INTERVAL > sxppvariables.max_margin ? sxppvariables.max_margin : DFLT_TABS_INTERVAL));
