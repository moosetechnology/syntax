...
char WHAT_READARE[] = "@(#)SYNTAX - $Id: read_a_re.c 2227 2018-09-26 16:22:59Z garavel $" WHAT_DEBUG;
...
re_smp (SXINT what, struct sxtables *sxtables_ptr)
{
  ...
  case SXACTION:
	...
	sxtmsg (sxsvar.sxlv.terminal_token.source_index.file_name,
		"Sentence too long w.r.t. the \"-mis\" option, abandoned\n",
	   ...
	   SXINT what_to_do)
{
  ...
  (*(re_tables.analyzers.parser)) (SXBEGIN, &re_tables) /* Allocation des
  ...
  (*(re_tables.analyzers.parser)) (SXEND, &re_tables);
...
sxparser_re_tcut  (SXINT what_to_do, struct sxtables *arg)
{
  ...
  case SXACTION:
	...
	if (sxplocals.Mtok_no > 1) {
	  (*(re_tables.SXP_tables.semact)) (SXCLOSE, arg); /* on libère l'arbre abstrait de la phrase=ligne qu'on vient de passer */
	  sxcomment_clear (&(sxplocals.sxcomment));
	  {
	    // et on le prépare à parser un nouveau RE
	    /* (équiv. à (*(re_tables.analyzers.scanner)) (SXINIT, arg) sans le sxnext_char final, car déjà fait en lisant le eol */
	    SXINT i;
	...
	cur_input_vstr = NULL;
	(*(re_tables.SXP_tables.semact)) (SXSEMPASS, arg); /* ... On exploite l'arbre abstrait et on appelle prelude_re, store_re et postlude_re  */
    ...
    (*(re_tables.SXP_tables.semact)) (SXFINAL, arg);
