...
char WHAT_OUTPUT_SEMACT[] = "@(#)SYNTAX - $Id: output_semact.c 2227 2018-09-26 16:22:59Z garavel $" WHAT_DEBUG;
...
FILE    *xml_file = NULL;     /* denotes also the file for PennTreeBank output and for dotty ouput, needs to be changed to be consistent */
...
/* parcours top-down y compris les terminaux: boucle récursive pour le format Penn TreeBank */
/* tabulat gives the string to insert before the line according to the recursion level */
/* sep is the string to insert to separate recursion level (usually \n or " ") */
static void
spf_td_walk_incl_terminals_berk (SXINT Aij, char *tabulat, char *sep)
{
  SXINT            output_prod, output_item, Xpq, hook, i, tok_no;
  SXINT            Aij_val;
  char             *ntstring;
  SXBOOLEAN        rec_level_up;
  struct sxtoken   *tok_ptr = NULL;
  static SXINT     last_f;

  rec_level_up = SXFALSE;
  if (Aij == spf.outputG.start_symbol)
    last_f = 0;
  ...
  if ((Aij_val = spf_count [Aij]) == -1) {
    ...
    if (ntstring != SXNUL && Aij2i (Aij) < Aij2j (Aij)) {
      for (i = 0; i < rec_level; i++) fprintf(xml_file, "%s", tabulat);
      if (ntstring[0] != '@') { /* Tree nodes starting with @ have been inserted to make a binary tree, simply forget them here */
	fprintf(xml_file,"%s(%s", sep, ntstring);
	rec_level++;
	rec_level_up = SXTRUE;
      }
    }
	...
	while ((Xpq = spf.outputG.rhs [output_item++].lispro) != 0) {
	  ...
	  if (Xpq <= spf.outputG.maxnt) {
	    if (Xpq > 0) {
	      /* non terminal */
	      spf_td_walk_incl_terminals_berk (Xpq, tabulat, sep);
	    }
	    else {
	      /* terminal*/
	      tok_no = spf_get_Tij2tok_no_stack (-Xpq)[1]; /* le premier token de Tpq  (ils ont tous le meme commentaire) */
	      tok_ptr = &(SXGET_TOKEN (tok_no));
	      for (i = 0; i < rec_level; i++) 
		fprintf(xml_file, "%s", tabulat);
	      fprintf(xml_file, "%s(%s %s)", sep, spf.inputG.tstring [-Tij2T(-Xpq)], sxstrget (tok_ptr->string_table_entry));
	    }
	  }
	}
      }
  ...
  for (i = 0; i < rec_level; i++) fprintf(xml_file, "%s", tabulat);
  if (rec_level_up) fprintf(xml_file, ")"); /* Only when the node is not fogotten */
}

/* parcours top-down y compris les terminaux: boucle récursive pour dotty */
static void
spf_td_walk_incl_terminals_dotty (SXINT Aij, char *father, char *df)
{
  SXINT            output_prod, output_item, Xpq, hook, tok_no;
  SXINT            Aij_val;
  char             *ntstring;
  struct sxtoken   *tok_ptr = NULL;
  static SXINT     last_f;
  ...
  newfather = (char *) sxalloc(256, sizeof(char)); /* get the current node and keep it for all the children */
  ...
  if ((Aij_val = spf_count [Aij]) == -1) {
    ...
    if (ntstring != SXNUL && Aij2i (Aij) < Aij2j (Aij)) {
      if (ntstring[0] != '@') {
	sprintf(newfather, "%s_%ld_%ld", ntstring, Aij2i(Aij), Aij2j(Aij));
	if (father[0] != '\0') {
	  fprintf(xml_file,"\"%s\" -> \"%s\" ;\n", father, newfather);
	}
	fprintf(xml_file,"\"%s\" [label=\"%s\"];\n", newfather, ntstring);	
	rec_level++;
      }
      else {
	sprintf(newfather, "%s", father);
      }
      nl_done=SXFALSE;
    }
	...
	while ((Xpq = spf.outputG.rhs [output_item++].lispro) != 0) {
	  ...
	  if (Xpq <= spf.outputG.maxnt) {
	    if (Xpq > 0) {
	      /* non terminal */
	      spf_td_walk_incl_terminals_dotty (Xpq, newfather, df);
	    }
	    else {
	      /* terminal*/
	      nl_done = SXTRUE;
	      ...
	      tok_no = spf_get_Tij2tok_no_stack (-Xpq)[1]; /* le premier token de Tpq  (ils ont tous le meme commentaire) */
	      tok_ptr = &(SXGET_TOKEN (tok_no));
	      fprintf(xml_file, "\"%s\" -> \"x%sx_%ld_%ld\" ;\n", newfather, spf.inputG.tstring [-Tij2T(-Xpq)], Aij2i(Aij), Aij2j(Aij));
	      tmpdotty = (char *) sxalloc(128, sizeof(char));
	      sprintf(tmpdotty, "\"x%sx_%ld_%ld\" [label=\"%s\"];\n", spf.inputG.tstring [-Tij2T(-Xpq)], Aij2i(Aij), Aij2j(Aij), sxstrget (tok_ptr->string_table_entry));
	      strcat(df, tmpdotty);
	    }
	  }
	}
...
xml_print_forest (void)
{
  ...
  fprintf (xml_file,
	   ...
	   xml_count_leaves(),
	   (long) sxba_cardinal (Aij_set),
	   (long) sxba_cardinal (prod_set)
	   );
...
berkeley_output (void) {
  ...
  if (spf.outputG.start_symbol) {
    ...
    for (i=0;i<=spf.outputG.maxxnt;i++)
      spf_count[i]=-1;
    fputs("(", xml_file);
    spf_td_walk_incl_terminals_berk(spf.outputG.start_symbol, "", " ");
    fputs(")\n", xml_file); /* synchronize la sortie */
  } else {
    fputs("# No parse found\n", xml_file);
  }
}

static void
dotty_output (void) {
  char* dotty_final; /* will contain the list of terminals to put them in same rank */
  SXINT i;
  if (spf.outputG.start_symbol) {
    spf_count = (SXINT*) sxalloc (spf.outputG.maxxnt+1, sizeof (SXINT));
    for (i=0;i<=spf.outputG.maxxnt;i++)
    ...
    fputs("digraph G {", xml_file);
    dotty_final = (char *) sxalloc(128*spf.outputG.maxxnt, sizeof(char));
    sprintf(dotty_final, "{rank = same\n");
    spf_td_walk_incl_terminals_dotty(spf.outputG.start_symbol, "", dotty_final);
    fprintf(xml_file, "%s}}\n", dotty_final); /* liste des terminaux */
  } else {
    fputs("/* No parse found */\n", xml_file);
  }
}
