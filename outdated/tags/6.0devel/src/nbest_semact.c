...
char WHAT_NBEST_SEMACT[] = "@(#)SYNTAX - $Id: nbest_semact.c 2227 2018-09-26 16:22:59Z garavel $" WHAT_DEBUG;
...
static SXLOGPROB    min_weight_arg;
static SXFLOAT    fudge_factor;
...
static void nbest_sem_open (void);
...
nbest_semact (void)
{
  ...
  */
  ...
  for_semact.sem_open = nbest_sem_open; /* avant chaque phrase */
...
static long              cur_PijKs_stack_top, cur_PijKs_stack_size;
...
static SXLOGPROB            *Tpq_item2weight;
...
static SXLOGPROB            best_Aij_weight = SXLOGPROB_MIN;
#endif /* 0 */
...
PijKs_stack_list_hd_free (void)
{
  ...
  if (total_cur_PijKs_stack_size)
    printf ("total_cur_PijKs_stack_size = " SXILD " in " SXILD " bags\n", total_cur_PijKs_stack_size, nb);
#endif /* LLOG */
}
...
#if LLOG
  printf ("\nReallocation of dedication_list [" SXILD "]/[" SXILD "]\n", old_size+1, new_size+1);
#endif /* LLOG */
}
...
#if LLOG
  printf ("\nReallocation of PijK2ctxt_elem_list [" SXILD "]/[" SXILD "] and Pij2attr [" SXILD "]/[" SXILD "]\n", old_size+1, new_size+1, old_size+1, new_size+1);
#endif /* LLOG */
}
...
#if LLOG
  printf ("\nReallocation of Pijxsignpost2signet [" SXILD "]/[" SXILD "]\n", old_size+1, new_size+1);
#endif /* LLOG */
}
...
{  /* On "reactive" les XxY statiques */
  ...
  if (Tpq_item2weight == NULL) {
    ...
    Tpq_item2weight = (SXLOGPROB*) sxcalloc ((SXUINT)spf.outputG.lhs [spf.outputG.maxxprod+1].prolon+1, sizeof (SXLOGPROB));
  }
  /* sinon deja alloue' ... */
}
...
nbest_allocate (SXINT nbest_allocate_nbest, SXINT cur_contextual_proba_kind)
{
  ...
  if (!XxY_is_allocated (dedication_hd) ) {
    ...
    Pij2one_best_local_weight = (SXLOGPROB*) sxalloc (spf.outputG.maxxprod+1, sizeof (SXLOGPROB));
...
get_dedication_ptr (SXINT item_or_Aij, SXINT k)
{
...
#if LLLOG
  printf ("********** Entering get_dedication_ptr (item_or_Aij=" SXILD ", k=" SXILD ") **********\n", item_or_Aij, k);
#endif /* LOG */
...
#if LLLOG
  printf ("********** Leaving get_dedication_ptr (item_or_Aij=" SXILD ", k=" SXILD ") ", item_or_Aij, k);
  ...
  if (dedication == 0)
    ...
    printf ("dedication = " SXILD ": Pij=" SXILD ", signature=" SXILD, dedication, Pij, signature);
    print_signature (signature);
    printf (", k=" SXILD ", weight=" SXFDLD " **********\n", dedication_ptr->k, dedication_ptr->total_weight);
  }
#endif /* LOG */
...
print_ctxt (SXINT item)
{
  ...
  case EP_POS_EPCFG:
    ...
    printf ("[val=" SXILD ", pos=" SXILD "]", (long)val, (long)pos);
    break;
  case EP_EPCFG:
    print_init_prod (init_prod);
    printf ("[val=" SXILD "]", (long)val);
    break;
  case P_EPCFG:
    print_init_prod (init_prod);
    break;
...
set_ctxt_item_weight (SXINT item, SXINT *best_PijKs_ptr, SXINT k)
{
  ...
  SXLOGPROB             best_total_weight, total_weight;
  struct rhs         *prhs;
  struct dedication  *dedication_ptr;
  SXINT  *PijKs_ptr;
  struct signet_elem *signet_elem_ptr;
#if LOG
  SXLOGPROB ctxt_weight, local_weight;
#endif
...
#if LOG
  printf ("********** Entering set_ctxt_item_weight (item=" SXILD ", k=" SXILD ")", item, k);
  print_ctxt (item);
  fputs (" **********\n", stdout);
#endif /* LOG */
  ...
  if (best_PijKs_ptr) {
	...
	if (PijK && (signet_elem_ptr = PijK2ctxt_elem_list [PijK].signet)) {
	  ...
	  print_ctxt (item);
	  printf (", ctxt weight=" SXFDLD ", local weight=" SXFDLD ", total weight=" SXFDLD ", signature=" SXILD, ctxt_weight, local_weight, total_weight, signature);
	  print_signature (signature);
	  fputs ("\n", stdout);
	}
...
#if LOG
  printf ("********** Leaving set_ctxt_item_weight (item=" SXILD ", k=" SXILD ")", item, k);

  if (best_PijKs_ptr) {
    print_ctxt (item);
    Pij = XxY_X (PijxItem_hd, PijK);
    printf (": dedication=" SXILD ": total_weight=" SXFDLD ", Pij=" SXILD ", signature=" SXILD,
...
set_item_weight_list (SXINT item)
{
  ...
  SXLOGPROB                   min_ctxt_weight, prev_min_ctxt_weight, best_total_weight, ctxt_weight, local_weight, total_weight;
  ...
  while ((Ppq = spf.outputG.rhs [hook++].lispro) != 0) {
    if (Ppq > 0) {
      ...
      printf ("\tctxt weight=" SXFDLD " for ", PijK2ctxt_elem_list [PpqK].ctxt_weight);
...
get_Tpq_logprob (SXINT item, SXINT cur_contextual_proba_kind)
{
  ...
  dynweight = p_q_t2logprob (Tij2i (Tpq), Tij2j (Tpq), t);
...
{
  SXINT               item, Xpq;
  SXLOGPROB            weight, Xpq_weight;

#if LLLOG
  printf ("********** Entering compute_Pij_ctxt_weight (Pij=" SXILD ") **********\n", Pij);
#endif /* LOG */
...
#if LLLOG
  printf ("********** Leaving compute_Pij_ctxt_weight (Pij=" SXILD ") son's total ctxt weight=" SXFDLD " **********\n", Pij, weight);
#endif /* LOG */
...
static SXLOGPROB
compute_Pij_local_weight (SXINT Pij, SXINT signature, SXLOGPROB *ctxt_weight_ptr)
{
  SXINT               item, Xpq, pos;
...
#if LLLOG
  printf ("********** Entering compute_Pij_local_weight (Pij=" SXILD ", vector=" SXILD, Pij, signature);
  print_signature (signature);
  fputs (") **********\n", stdout);
#endif /* LOG */
...
#if LLLOG
  printf ("********** Leaving compute_Pij_local_weight (Pij=" SXILD ", vector=" SXILD, Pij, signature);
  print_signature (signature);
  printf (") weight=" SXFDLD " **********\n", weight);
#endif /* LOG */
...
get_Aij_or_item_best_weight (SXINT Aij_or_item, SXINT k)
{
  ...
  if (contextual_proba_kind > EPCFG) {
	...
	if (signet_elem_ptr) {
	  ...
	  printf (" total weight=" SXFDLD ", signature=" SXILD, signet_elem_ptr->total_weight, signet_elem_ptr->signature);
...
#if LOG
      spf_print_Xpq (stdout, Aij);
      printf (", couple(Aij=" SXILD ", k=" SXILD ")=" SXILD ": k=" SXILD ", cur_best_weight=" SXFDLD ", Pij=" SXILD ", signature=" SXILD,
  ...
  printf (", couple=" SXILD "(Pij, signature): k=(" SXILD "/%f), weight=" SXFDLD ", Pij=" SXILD ", signature=" SXILD, couple, dedication_ptr->k, spf.tree_count.nt2nb [Aij],
...
{
  SXLOGPROB w = p1->weight - p2->weight;
...
reorg_dicho (SXINT PijK)
{
  ...
  SXINT                      old_signpost;
#endif
  SXLOGPROB                   prev_weight, cur_weight;
...
static struct signet_elem*
signpost_seek (SXINT PijK, SXINT *lower_signpost_ptr, SXINT upper_signpost, SXLOGPROB weight)
{
...
next_signet (struct dedication *dedication_ptr)
{
  ...
  SXLOGPROB                   ctxt_weight, *ctxt_weight_ptr;
...
#if LOG
    printf ("********** Entering next_signet (Pij=" SXILD ") **********\n", Pij);
    spf_print_iprod (stdout, Pij, "", "");
    printf (" signature=" SXILD, prev_signature);
...
#if LOG
    printf ("********** Leaving next_signet (Pij=" SXILD ")", Pij);

    if (signet_elem_ptr == NULL)
      fputs (" (erased)", stdout);
    else {
      printf (" signature=" SXILD, signet_elem_ptr->signature);
      print_signature (signet_elem_ptr->signature);
      printf (", total weight=" SXFDLD "", signet_elem_ptr->total_weight);
    }
...
one_best_post_bu_walk (SXINT Aij, SXBOOLEAN status)
{
...
#if LOG
  spf_print_Xpq (stdout, Aij);
  printf (" : 1st_best_weight=" SXFDLD " for Pij=" SXILD " (signature=" SXILD,
  ...
  best_Aij_local_weight = SXLOGPROB_MIN; /* pour le coup d'apres */
...
one_best_bu_walk (SXINT Pij)
{
  ...
  if (contextual_proba_kind > EPCFG) {
    ...
    */
    ...
    local_weight = compute_Pij_local_weight (Pij, unit_vector_signature, (SXLOGPROB*)NULL);
  ...
  spf_print_iprod (stdout, Pij, "", "");
  printf (" total local weight=" SXFDLD, local_weight);
  printf (" signature=" SXILD, unit_vector_signature);
  print_signature (unit_vector_signature);
  putchar ('\n');
#endif /* LOG */
...
void
nbest_seek_kth_best_tree (SXINT Aij_or_item, SXINT k, void (*f) (SXINT, SXLOGPROB))
{
...
output_kth_best_tree (SXINT Aij_or_item, SXINT k)
{
  ...
  if (dedication_ptr) {
    ...
    spf_print_iprod (stdout, Pij, "", "");
    printf (" total weight=" SXLPD "\n", SXLOGPROB_display (weight));
...
print_decorated_Aij (SXINT Aij_or_item)
{
  ...
  while (next) {
	...
	if (Xpq > 0) {
	  printf ("[" SXILD "] ", (long)XH_list_elem (signatures_hd, bot));
	...
	bot++;
      ...
      printf ("; weight = " SXLPD "\n", SXLOGPROB_display (dedication_ptr->total_weight));
    }
...
print_decorated_item (SXINT item)
{
  ...
  if (Xpq < 0) {
    ...
    spf_print_Xpq (stdout, Xpq);
    printf ("weight=" SXLPD "\n\n", SXLOGPROB_display (Tpq_item2weight [item]));
  }
  else {
    print_ctxt (item);
    fputs ("\n", stdout);
...
static void
output_best_weights (SXINT k, SXLOGPROB weight)
{
  ...
  fprintf (best_weights_file, "#" SXILD " " SXILD " " SXILD " " SXLPD "\n", (long)sentence_id, (long)k, (long)contextual_proba_kind, SXLOGPROB_display (weight));
}
...
nbest_sem_pass_with_cpk (SXINT root_Aij, SXINT kind)
{
  ...
  SXLOGPROB   threshold, best_forest_weight, prev_weight, weight;
  char     str [128];
  SXBOOLEAN ret_val;
  ...
  best_Aij_local_weight = SXLOGPROB_MIN;
  ...
  if (min_weight_arg != 1 && prev_weight < min_weight_arg)
    ...
    if (beam != 0)
      threshold = best_forest_weight - proba2SXLOGPROB ((SXFLOAT)beam);

    for (k = 2; k <= nbest; k++) {
#if LOG
      printf ("\n************** " SXILD "%s_best_weight computation **************\n", k, (k == 2) ? "nd" : ((k == 3) ? "rd" : "th"));
#endif /* LOG */
...
#if LOG
	printf ("\n\t\tThere are only " SXILD "_best_weight in this forest\n\n", k-1);
#endif /* LOG */
  ...
  if (is_print_nbest_trees) {
    ...
    for (k = 1; k <= real_nbest; k++) {
      printf ("\n************** " SXILD "%s best tree **************\n", (long)k, (k == 1) ? "st": ((k == 2) ? "nd" : ((k == 3) ? "rd" : "th")));
  ...
  if (is_output_nbest_value) {
    printf ("best_weight = " SXLPD ", exit at step " SXILD " with weight = " SXLPD "\n", SXLOGPROB_display (best_forest_weight), (long)real_nbest, SXLOGPROB_display (prev_weight));
  }
  ...
  if (is_print_time) {
    ...
    sprintf (str, "\tN-Best (n = " SXILD ", contextuality level " SXILD ", best_weight = " SXLPD ") (TRUE)", (long)real_nbest, (long)contextual_proba_kind, SXLOGPROB_display (best_forest_weight));
    sxtime (SXACTION, str);
	...
	fputs (", ", sxtty);
      ...
      fprintf (sxtty, SXFDLD, dedication_ptr->total_weight);
    }
...
nbest_sem_init (void)
{
  if (spf.outputG.start_symbol > 0) {
	...
	nbest_allocate (nbest, cpk);
      ...
      if (is_print_time) {
	sprintf (str, "\t" SXILD " best (SKIPPED)", (long)nbest);
	sxtime (SXACTION, str);
      }
    }
  }
