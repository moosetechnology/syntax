...
char WHAT_XXY_MNGR[] = "@(#)SYNTAX - $Id: XxY_mngr.c 2280 2020-03-10 11:22:52Z garavel $" WHAT_DEBUG;
...
void XxY_array_to_c (XxY_header *header, FILE *F_XxY /* named output stream */, char *name)
{
	...
	fprintf (F_XxY, "{%ld, %ld}, ", (long)header->display [i_counter].X, (long)header->display [i_counter].Y);
    } 
	...
	if ((X_hd = header->X_hd [kind]) != NULL) {
		...
		fprintf (F_XxY, "%ld, ", (long)header->lnk_hd [kind] [i_counter]);
	    } 
		...
		fprintf (F_XxY, "%ld, ", (long)header->lnk [kind] [i_counter]);
	    } 
