...
char WHAT_SXMEM_MNGR[] = "@(#)SYNTAX - $Id: sxmem_mngr.c 2227 2018-09-26 16:22:59Z garavel $" WHAT_DEBUG;
...
# if BUG
    fprintf (stdout, "mallocating %lu:", (unsigned long)size);
# endif
...
# if BUG
    fprintf (stdout, "allocating (%lu == %lu*%lu):", (unsigned long)n*size, (unsigned long)n, (unsigned long)size);
# endif
...
# endif
  ...
  if (mem_signature_mode) {
	...
	if (X_set (&X_hd, (SXINT) (mem_signatures->free_signature.ptr_stack [i]), &addr))
...
#if BUG
      fprintf (stdout, "RAZing 0x%lX (%lu)\n", (SXUINT) ptr, (unsigned long)(mem_signatures->tobereset_sizes [i])); 
#endif /* BUG */
    }
...
void * sx_alloc_and_copy (size_t nbemb, size_t size, void *original_content, size_t copy_size)
{
  ...
  if ( (nbemb * size) < copy_size ) {
    fprintf (sxstderr, "allocated size is too small (nbelem %lu/ size_elem %lu/ copy_size %lu)",
	     (unsigned long)nbemb, (unsigned long)size, (unsigned long)copy_size);
