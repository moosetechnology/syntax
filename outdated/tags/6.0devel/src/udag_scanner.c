...
char WHAT_UDAG_SCANNER[] = "@(#)SYNTAX - $Id: udag_scanner.c 2227 2018-09-26 16:22:59Z garavel $" WHAT_DEBUG;
...
static SXBOOLEAN  input_is_a_dag,input_is_a_sdag;
static VARSTR     vstr;

static SXINT      *temporary_tok_no_stack;
static SXINT      *temporary_tok_no_stack_hd;
...
SXBOOLEAN
input_dag_transition_names_are_ffs ()
{
#ifdef def_sxdfa_comb_inflected_form
  return SXTRUE;
#else /* def_sxdfa_comb_inflected_form */
  return SXFALSE;
#endif /* def_sxdfa_comb_inflected_form */
}
...
{
  SXFLOAT logprob = 0; /* valeur impossible */

  if (idag.pqt2logprob)
    logprob = idag.pqt2logprob [p_q_t2pqt (from_state, to_state, t)];

#if dico_contains_terminals &&  defined(def_t2string)
  printf (SXILD "\t%s %s/%s(" SXILD ")%s%s\t" SXILD "\t(logprob=%g)\n", from_state, comment ? comment : "", ff, t ? t2string [t] : "((unk))", t,  semlex ? " " : "", semlex ? semlex : "", to_state, logprob);
#else /* def_t2string */
  printf (SXILD "\t%s %s/(" SXILD ")%s%s\t" SXILD "\t(logprob=%g)\n", from_state, comment ? comment : "", ff, t,  semlex ? " " : "", semlex ? semlex : "", to_state, logprob);
#endif /* def_t2string */
}
#endif /* LOG */
...
/* Utilisé par la lexicalisation ou les parseurs */
/* ça ne peut pas etre fait avec idag_infos car :
...
fill_idag_q2pq_stack (void)
{
  ...
  if (idag.q2pq_stack_hd == NULL) {
    ...
    q2p_nb [1] = 1; // transition O-EOF->1

    idag.q2pq_stack_hd = (SXINT *) sxalloc (idag.final_state+1, sizeof (SXINT)), idag.q2pq_stack_hd [0] = 0;
    idag.q2pq_stack = (SXINT *) sxalloc (nb+idag.final_state+1/*pour le EOF entre 0 et 1*/+1, sizeof (SXINT)), idag.q2pq_stack [0] = 0;

    top = 0;

    for (q = idag.init_state; q <= idag.final_state; q++) {
    ...
    pq_stack = idag.q2pq_stack + idag.q2pq_stack_hd [1];
    PUSH (pq_stack, p_q2range (0,1)); // transition O-EOF->1
 ...
 */
void
fill_more_idag_infos (SXBOOLEAN is_qstack_needed, SXBOOLEAN is_pstack_needed, SXBOOLEAN is_inside_t_needed, SXBOOLEAN is_range2dag_kind_needed)
{
  ...
  if (is_qstack_needed) {
    idag.pt2q_disp = (SXINT ***) sxalloc (idag.final_state+1/*pour le pseudo-état idag.final_state+1*/+1, sizeof (SXINT**)),
      idag.pt2q_disp [idag.final_state+1] = NULL;

    idag.pt2q_disp [0] = (SXINT **) sxcalloc ((idag.final_state+1)*(idag.eof_code+1)+1,
						 sizeof (SXINT*));
    *(idag.pt2q_disp [0]) =
      pt2q_stack = (SXINT *) sxalloc (2*(idag.ptq_nb+2 /* pour eof_code->init_state et final_state->eof_code */) +1 /* Normalement ne deborde pas !! */, sizeof (SXINT)),
      TOP (pt2q_stack) = 0;

    t2q_set = sxbm_calloc (idag.eof_code+1, idag.final_state+1);
    pt_set = sxba_calloc (idag.eof_code+1);
    
    cur_pt2q_area_ptr = idag.pt2q_disp [0];
    PUSH (pt2q_stack, 1); /* Le compte, qu'on connaît déjà dans ce cas */
    cur_pt2q_area_ptr[idag.eof_code] = pt2q_stack + TOP (pt2q_stack);
    PUSH (pt2q_stack, idag.init_state);
    ...
    idag.pt2q_disp [idag.final_state] = cur_pt2q_area_ptr;
    PUSH (pt2q_stack, 1); /* Le compte, qu'on connaît déjà dans ce cas */
    cur_pt2q_area_ptr[idag.eof_code] = pt2q_stack + TOP (pt2q_stack);
    PUSH (pt2q_stack, idag.final_state+1); /* idag.final_state+1 est l'état virtuel auquel on peut arriver depuis idag.final_state */
  ...
  if (is_range2dag_kind_needed) {
    SXINT bot_pq, top_pq, p, q, r, qr, range, *tstack, *qr_stack, *cur_qr_ptr;
    SXBOOLEAN found_one_p_q_r_path;
    char pq_kind, qr_kind;
    SXBA sdag_range_kinds, path;

#if EBUG
    if (idag.range2p == NULL)
      sxtrap (ME, "fill_more_idag_infos (fill_idag_infos must be called before fill_more_idag_infos in order to fill idag.range2[pq]");
#endif /* EBUG */

    idag.range2dag_kind = (char*) sxcalloc (idag.max_range_value+1, sizeof (char)); /* on fait un calloc de sorte qu'un range de dag_kind valant NO_PATH ( = 0 ) signifie qu'il n'y a pas de chemin correspondant à ce range range */

    /* hack pour éviter d'avoir à tester plus tard si on est sur un
       range de longueur nulle */
    for (p = 1; p <= idag.final_state; p++) 
      idag.range2dag_kind [p_q2range (p,p)] = STRING_KIND + IS_CONSECUTIVE_SEQUENCE; 

    if (idag.dag_kind == STRING_KIND) {
      for (range = 1; range <= idag.max_range_value; range++) {
	p = range2p (range);
	q = range2q (range);
	if (p > 0 && p < q && q <= idag.final_state)
	  idag.range2dag_kind [range] = STRING_KIND + IS_CONSECUTIVE_SEQUENCE;
      }
    }
    else {
      if (idag.dag_kind == SDAG_KIND) {
	sdag_range_kinds = sxba_calloc (idag.final_state);
      }

      /* initialisation: les ranges de longueur 1 (transitions directes) sont STRING ou SDAG */
      for (p = idag.init_state; p < idag.final_state; p++) {
	bot_pq = idag.p2pq_hd [p];
	top_pq = idag.p2pq_hd [p+1];
	...
	while (bot_pq < top_pq) {
	  ...
	  tstack = idag.t_stack + idag.pq2t_stack_hd [range];

	  idag.range2dag_kind [range] = (*tstack == 1 ? STRING_KIND : SDAG_KIND) + (q-p == 1 ? IS_CONSECUTIVE_SEQUENCE : 0);

	  if (idag.dag_kind == SDAG_KIND && *tstack > 1)
	    SXBA_1_bit (sdag_range_kinds, p);

	  bot_pq++;
	}
      }

      /* boucle principale pour remplir les dag_kind de tous les ranges */
      if (idag.dag_kind == SDAG_KIND) {
	for (p = idag.init_state; p < idag.final_state; p++) {
	  r = sxba_scan (sdag_range_kinds, p-1);
	  if (r == -1) { /* pas de transition SDAG partant de p ou plus à droite que p */
	    for (q = p+2 /* p+1 fait ci-dessus */; q <= idag.final_state; q++)
	      idag.range2dag_kind [p_q2range (p,q)] = STRING_KIND + IS_CONSECUTIVE_SEQUENCE;
	  }
	  else {
	    for (q = p+2 /* p+1 fait ci-dessus */; q < r; q++)
	      idag.range2dag_kind [p_q2range (p,q)] = STRING_KIND;
	    for (/* q=r (inutile ;-) ) */; q <= idag.final_state; q++)
	      idag.range2dag_kind [p_q2range (p,q)] = SDAG_KIND + IS_CONSECUTIVE_SEQUENCE;
	  }
	}
      }
      else {
	...
	for (p = idag.init_state; p < idag.final_state; p++) {
	  path = idag.path [p]; /* sxba des états accessibles (directement ou indirectement) depuis p */
	  r = p;
	  
	  while ((r = sxba_scan (path, r)) > 0) {
	    range = p_q2pq (p, r);
	    
	    qr_stack = idag.q2pq_stack + idag.q2pq_stack_hd [r];

	    found_one_p_q_r_path = SXFALSE;

	    for (cur_qr_ptr = qr_stack + *qr_stack; cur_qr_ptr > qr_stack; cur_qr_ptr--) {
	      qr = *cur_qr_ptr;
	      q = range2p (qr);
	      /* s'il n'y a que des transitions directes p->r,
		 range2dag_kind a été déjà positionné plus haut. On
		 n'a donc plus qu'à étudier le cas q != p (en prenant
		 en compte évidemment l'existence éventuelle de
		 transitions directes p->r */
	      if (q != p) {
		if (SXBA_bit_is_set (path, q)) {
		  if (found_one_p_q_r_path) {
		    idag.range2dag_kind [range] = DAG_KIND;
		    break;
		  }
		  /* on est donc sur le premier p->*q->r */
		  pq_kind = idag.range2dag_kind [p_q2range (p,q)];
		  if (pq_kind == DAG_KIND) {
		    idag.range2dag_kind [range] = DAG_KIND;
		    break;
		  }
		  if (p_q2pq (p,r) <= idag.last_pq) {
		    /* il existe par ailleurs au moins une transition
		       directe de p à r; or on a aussi un chemin
		       indirect p->*q->r; donc c'est un DAG */
		    idag.range2dag_kind [range] = DAG_KIND;
		    break;
		  }
		  /* on est donc pour la première fois sur un q tel que p->*q->r,
		     et on a pas de p->r directe */
		  found_one_p_q_r_path = SXTRUE;
		  qr_kind = idag.range2dag_kind [p_q2range (q,r)];
		  if ((pq_kind & SDAG_KIND) || (qr_kind & SDAG_KIND))
		    idag.range2dag_kind [range] = SDAG_KIND;
		  else
		    idag.range2dag_kind [range] = STRING_KIND;
		  if ((pq_kind & IS_CONSECUTIVE_SEQUENCE) && (qr_kind & IS_CONSECUTIVE_SEQUENCE))
		    idag.range2dag_kind [range] += IS_CONSECUTIVE_SEQUENCE;
		}
	      }
	    }
	  }
    ...
    for (range = 1; range <= idag.max_range_value; range++) {
      ...
      if (p == 0 || p == q || q > idag.final_state)
	continue;
      pq_kind = range2dag_kind (range);
      if (pq_kind == 0)
	fprintf (sxstdout, SXILD " : [" SXILD ".." SXILD "]\tNo path\n", range, p, q);
      else if (pq_kind & STRING_KIND)
	fprintf (sxstdout, "%ld : [%ld..%ld]\tString %s\n", range, p, q, (pq_kind & IS_CONSECUTIVE_SEQUENCE ? "(consec.)" : "(non-consec.)"));
      else if (pq_kind & SDAG_KIND)
	fprintf (sxstdout, "%ld : [%ld..%ld]\tSdag %s\n", range, p, q, (pq_kind & IS_CONSECUTIVE_SEQUENCE ? "(consec.)" : "(non-consec.)"));
      else if (pq_kind & DAG_KIND)
	fprintf (sxstdout, SXILD " : [" SXILD ".." SXILD "]\tDag\n", range, p, q);
      else
	sxtrap (ME, "fill_more_idag_infos (unknown (sub-)dag kind)");
    }
#endif /* EBUG */
  }
}
...
fill_idag_infos (void)
{
  ...
  idag.dag_kind = input_is_a_dag ? dag_kind : (input_is_a_sdag ? SDAG_KIND : STRING_KIND);
  ...
  for (from_state = idag.init_state; from_state < idag.final_state; from_state++) {
    ...
    while (bot_pq < top_pq) {
      ...
      current_trans_t_set = idag.pq2t_set [bot_pq];
...
#define CODE_STACK t_code_stack
		...
		if (id > MAX_ID) {
		  ...
		  /* une apparente string devient SDAG dès lors que
		     l'une des ff est associée à plus d'un terminal
		     dans le lexique */
		  if (idag.dag_kind == STRING_KIND)
		    idag.dag_kind = SDAG_KIND;
...
#define CODE_STACK t_code_stack
		  ...
		  if (id > MAX_ID) {
		    ...
		    while (nb-- > 0) {
		      ...
		      if (tok <= 0 || id == tok) { /* cas ff__terminal, on cherche a savoir si le terminal fournit est bon */
			if (SXBA_bit_is_reset_set (current_trans_t_set, id))
			  DPUSH (idag.t_stack, id), ptq_nb++;
		  ...
		  if (top == DTOP(idag.local_t_stack)) {		      
		    sxerror (ptok->source_index,
			     sxsvar.sxtables->err_titles [1][0],
#ifdef NO_SPF
			     "%sUnknown tagged word \"%s\", new tag assumed.",
#else /* NO_SPF */
			     "%sUnknown tagged word \"%s\", new tag \"%s\" assumed.",
#endif /* NO_SPF */
			     sxsvar.sxtables->err_titles [1]+1,
			     str
#ifndef NO_SPF
			     , spf.inputG.tstring[id]
#endif /* NO_SPF */
			     );
		    if (SXBA_bit_is_reset_set (current_trans_t_set, id))
		      DPUSH (idag.t_stack, id), ptq_nb++;
      ...
      /* On exploite current_trans_t_set (qui est idag.pq2t_set [bot_pq])*/
      top = TOP (idag.t_stack);
      idag.t_stack [cur_t_stack_bot] = top - cur_t_stack_bot;

      idag.p2t_set [from_state] = sxba_or (idag.p2t_set [from_state], current_trans_t_set);
      
      bot_pq++;
    }
  }

  SXBA_1_bit (idag.p2t_set [0], EOF_CODE);
  SXBA_1_bit (idag.p2t_set [idag.final_state], EOF_CODE);
  /* on remplira pq2t_set pour les EOF transitions une fois p_q2range rempli */
  ...
  {
    /* Le 16/06/2010 on a des ranges qui partent de 0 et arrivent a idag.final_state+1.
     Cela permet de traiter de fac,on uniforme les eof de debut et de fin */
    SXINT p, q, pq, range;
    SXINT *int_ptr;

    idag.p_q2range = (SXINT**) sxalloc (idag.final_state+2, sizeof (SXINT*));
    idag.p_q2range [0] /* was ...range_store, but can conveniently be stored in ...range [0] */ 
      = (SXINT*) sxcalloc ((idag.final_state+2)*(idag.final_state+3)/2+1, sizeof (SXINT));
    
    /* pointe vers le dernier elem de la ligne 0 : &(idag.p_q2range [0] [idag.final_state+1]) */
    int_ptr = idag.p_q2range [0]+idag.final_state+1;

    /* VRAIES TRANSITIONS */
    for (p = idag.init_state; p <= idag.final_state; p++) {
      idag.p_q2range [p] = int_ptr;

      for (pq = idag.p2pq_hd [p]; pq <= idag.last_pq && pq < idag.p2pq_hd [p+1]; pq++) { // valgrind dit que idag.p2pq_hd [p+1] n'est pas toujours initialisé (le tester en second semble résoudre cela)
	q = idag.pq2q [pq];
	idag.p_q2range [p] [q] = pq;
      }

      int_ptr += idag.final_state + 1 - p;
    }

    idag.p_q2range [p] = int_ptr;

    /* TRANSITIONS EOF PRÉ ET POST-DAG */
    /* on veut contrôler la valeur des ranges sur les EOF, on les renseigne à la main en premier */
    /*    on a besoin, pour eux (p.ex. dans DSA), de pq2t_stack_hd (et donc de remplir t_stack en conséquence) */
    idag.p_q2range [0][1] = idag.last_pq + 1;
    DPUSH (idag.t_stack, 1); /* un seul terminal pour 0..1 */
    idag.pq2t_stack_hd [idag.last_pq + 1] = TOP (idag.t_stack);
    DPUSH (idag.t_stack, EOF_CODE);
    idag.p_q2range [idag.final_state][idag.final_state+1] = idag.last_pq + 2;
    DPUSH (idag.t_stack, 1); /* un seul terminal pour final_state..final_state+1 */
    idag.pq2t_stack_hd [idag.last_pq + 2] = TOP (idag.t_stack);
    DPUSH (idag.t_stack, EOF_CODE);
    
    /* RANGES THAT ARE NOT TRANSITIONS */
    idag.max_range_value = idag.last_pq + 2;

    for (p = 0; p <= idag.final_state+1; p++) {
      int_ptr = idag.p_q2range [p] + p;

      for (q = p; q <= idag.final_state+1; q++) {
	if (*int_ptr == 0)
    ...
    idag.range2p = (SXINT *) sxalloc (idag.max_range_value+1, sizeof (SXINT));
    idag.range2q = (SXINT *) sxalloc (idag.max_range_value+1, sizeof (SXINT));
  ...
  /* p_q2range et pas p_q2pq car on déborde de ce que *2pq connaît: seuls les ranges connaissent les EOF-transitions
     0->1 et final_state->final_state+1 */
  SXBA_1_bit (idag.pq2t_set [p_q2range (0, idag.init_state)], EOF_CODE); 
  SXBA_1_bit (idag.pq2t_set [p_q2range (idag.final_state, idag.final_state+1)], EOF_CODE);
...
void
fill_states_in_all_paths_set (void)
{
  SXINT p, q;
  
#if EBUG
  if (!idag.p_q2range)
    sxtrap (ME, "fill_states_in_all_paths_set (idag.p_q2range is not allocated)");
#endif /* EBUG */

  idag.states_in_all_paths_set = sxba_calloc (idag.final_state+2);
  SXBA_1_bit (idag.states_in_all_paths_set, 0);
  SXBA_1_bit (idag.states_in_all_paths_set, 1);
  SXBA_1_bit (idag.states_in_all_paths_set, idag.final_state);
  SXBA_1_bit (idag.states_in_all_paths_set, idag.final_state + 1);

  for (p = 2; p < idag.final_state; p++) {
    for (q = 2; q < idag.final_state; q++) {
      if (p == q)
	continue;
      if (q < p) {
	if (p_q2range (q, p) == 0)
    ...
    if (q == idag.final_state) /* on a fini la boucle sans breaker (pas même à la dernière itération) */
      SXBA_1_bit (idag.states_in_all_paths_set, p);    
  }
}
...
{
  SXINT            pq, q, prev_tok_no, nb, *local_temporary_tok_no_stack;
  ...
  if (dag_kind == DAG_KIND) {
    ...
    if (/* CAS PARTICULIER DES TRANSITIONS FINALES SUR EOF, c'est à dire les transitions telles que: */
	tok_no <= sxplocals.Mtok_no /* 1) tok_no > sxplocals.Mtok_no veut dire que l'on a pas un token unique, et donc que l'on ne peut pas faire SXGET_TOKEN sur ce tok_no; ça veut dire aussi qu'on n'est de toute façon pas sur une (des) EOF-transition(s) finale(s) */
        && SXGET_TOKEN (tok_no).string_table_entry == cur_fsa->eof_ste /* 2) on a le ste d'EOF */) {
  ...
  if (TOP (idag.p2pq_hd) < from_state) {
    ...
    if (TOP (idag.p2pq_hd) != from_state-1)
      sxtrap (ME, "store_idag (before processing transitions starting in p, transitions starting in p-1 should have been processed)"); // p = from_state dans ce message
#endif /* EBUG */
    ...
    if (tok_no <= sxplocals.Mtok_no)
      idag.pq2tok_no [pq] = tok_no;
    else {
      local_temporary_tok_no_stack = temporary_tok_no_stack + temporary_tok_no_stack_hd [tok_no-sxplocals.Mtok_no];
      nb = DTOP(local_temporary_tok_no_stack);
      PUSH (idag.tok_no_stack, nb);
      idag.pq2tok_no [pq] = -TOP (idag.tok_no_stack);
      while (nb > 0) {
	tok_no = local_temporary_tok_no_stack [nb];
#if EBUG
	if (tok_no <= 0)
	  sxtrap (ME, "store_idag (trying to push a non-postive tok-no in idag.tok_no_stack)");
#endif /* EBUG */
	PUSH (idag.tok_no_stack, tok_no);
	nb--;
      }
	...
	local_temporary_tok_no_stack = temporary_tok_no_stack + temporary_tok_no_stack_hd [tok_no-sxplocals.Mtok_no];
	nb = DTOP(local_temporary_tok_no_stack);
	idag.tok_no_stack [-prev_tok_no] += nb - 1; /* - 1 car on a déjà incrémenté le compte de 1 (8 lignes plus haut) */
	while (nb > 0) {
	  tok_no = local_temporary_tok_no_stack [nb];
	  PUSH (idag.tok_no_stack, tok_no);
	  nb--;
...
idag_free (struct idag *idag_ptr)
{
  ...
  if (idag_ptr->p2pq_hd) {
    ...
    if (idag.states_in_all_paths_set)
      sxfree (idag.states_in_all_paths_set), idag.states_in_all_paths_set = NULL;
    ...
    if (idag_ptr->pq2t_stack_hd) {
      ...
      sxbm_free (idag_ptr->p2t_set), idag_ptr->p2t_set = NULL;
	...
	sxfree (idag_ptr->orig_source_set);
      ...
      if (idag_ptr->pqt2logprob)
	sxfree (idag_ptr->pqt2logprob), idag_ptr->pqt2logprob = NULL;
      sxbm_free (idag_ptr->pq2t_set), idag_ptr->pq2t_set = NULL;
    }
    ...
    if (idag_ptr->range2dag_kind)
      sxfree (idag_ptr->range2dag_kind), idag_ptr->range2dag_kind = NULL;
...
after_last_idag_trans ()
{
  ...
  */
  ...
  idag.pq2t_stack_hd = (SXINT *) sxalloc (last_pq+2/* pour les deux EOF-transitions */+1, sizeof (SXINT)), idag.pq2t_stack_hd [0] = 0;
  idag.p2t_set = (SXBA *) sxbm_calloc (idag.final_state+1, EOF_CODE+1);
  idag.pq2t_set = (SXBA *) sxbm_calloc (last_pq+2/* pour les deux EOF-transitions */+1, EOF_CODE+1);
...
check_udag ()
{
  ...
  if (is_print_time) {
    sentence_nb++;
    sprintf (mess, "%s\t%s Scanner[sentence #" SXILD ", final_state=" SXILD ", terminal_transition_nb=" SXILD "%s]",
	     sentence_nb == 1 ? "" : "\n",
...
{
  SXINT            triple, lgth, tmp_ste, nb;
  struct sxtoken   tok, *ptok, *semlex_ptok;
  SXINT            cur_bot;
  ...
  if ((ptok = *++ptok_ptr) != NULL) {
	...
	&& semlex_ptok == NULL /* le futur (unique) semlex token */)
      semlex_ptok = *++semlex_ptok_ptr;
  ...
  if (semlex_ptok)
    sxput_token (*semlex_ptok); /* mis en sxplocals.Mtok_no */
#endif /* 0 */
  ...
  while ((ptok = *++ptok_ptr) != NULL) {
    ...
    if (nb == 2) { /* on va devoir fabriquer une pile de tokens, on le fait et on y met déjà le premier (celui du coup précédent) */
#if EBUG
      if (temporary_tok_no_stack_hd == NULL)
	sxtrap (ME, "store_re (temporary_tok_no_stack_hd is NULL where it shouldn't)");
      if (temporary_tok_no_stack == NULL)
	sxtrap (ME, "store_re (temporary_tok_no_stack is NULL where it shouldn't)");
#endif /* EBUG */
      DPUSH (temporary_tok_no_stack, 1);
      cur_bot = DTOP(temporary_tok_no_stack);
      DPUSH (temporary_tok_no_stack, sxplocals.Mtok_no);
    }
    
    if (semlex_ptok_ptr /* la pile (pê null) de semlex tokens recue en entrée */)
      semlex_ptok = *++semlex_ptok_ptr;
    else
      semlex_ptok = NULL;

    ptok->string_table_entry = ste; /* prudence, c'est pas fait sur eof */

    sxput_token (*ptok); /* mis en sxplocals.Mtok_no */

    if (nb >= 2) { /* on rajoute dans la pile de tokens le token courant */
      DPUSH (temporary_tok_no_stack, sxplocals.Mtok_no);
      temporary_tok_no_stack [cur_bot]++;
    }
...
#if EBUG
    printf (SXILD "\t%s %s\t\t" SXILD "\n", state, ptok->comment ? ptok->comment : "", sxstrget (ste), next_state);
#endif /* EBUG */

  }

  if (nb == 1)
    XxYxZ_set (&(cur_fsa->fsa_hd), state, semlex_ptok ? sxplocals.Mtok_no - 1 : sxplocals.Mtok_no, next_state, &triple);
  else {
    DPUSH (temporary_tok_no_stack_hd, cur_bot);
    XxYxZ_set (&(cur_fsa->fsa_hd), state, -DTOP (temporary_tok_no_stack_hd) /* négatif, i.e. pile de plusieurs tokens */, next_state, &triple);    
  }
...
DFA_extract_trans (SXINT dfa_state, void (*output_trans) ())
{
  ...
  XxYxZ_Xforeach (cur_fsa->fsa_hd, dfa_state, triple) {
    tok_no = XxYxZ_Y (cur_fsa->fsa_hd, triple);
    if (tok_no > 0)
      (*output_trans) (dfa_state, tok_no, XxYxZ_Z (cur_fsa->fsa_hd, triple));
    else
      (*output_trans) (dfa_state, sxplocals.Mtok_no - tok_no, XxYxZ_Z (cur_fsa->fsa_hd, triple));
  }
}
...
postlude_re (SXINT fsa_kind)
{
...
#if LOG
  sprintf (mess, "RE2DFA: state_nb = " SXILD, cur_fsa->final_state);
  sxtime (SXACTION, mess);
#endif /* LOG */
  ...
  dfa_minimize (1, cur_fsa->final_state, sxplocals.Mtok_no + DTOP (temporary_tok_no_stack_hd), DFA_extract_trans, store_idag, SXTRUE /* to_be_normalized */
...
#if LOG
  sprintf (mess, "DFA2min_DFA: state_nb = " SXILD ", total_trans_nb = " SXILD, idag.final_state, idag.toks_buf_top-1);
  sxtime (SXACTION, mess);
#endif /* LOG */
...
#ifdef is_probabilized_lexicon
#  ifndef ift_code2proba_is_static
    SXFLOAT_TYPE_CHECK (get_ift_code2proba_type);
#  endif /* ift_code2proba_is_static */
#endif /* is_probabilized_lexicon */
  ...
  case SXACTION:
    ...
    case DAG_KIND :
    default : /* Par defaut on dit qu'on a des DAG en entrée !! */
      DALLOC_STACK (temporary_tok_no_stack, 254);
      DALLOC_STACK (temporary_tok_no_stack_hd, 126);
      ...
      DFREE_STACK (temporary_tok_no_stack);
      DFREE_STACK (temporary_tok_no_stack_hd);
      break;
...
void
fill_idag_pqt2logprob (void) {
  SXINT from_state, bot_pq, top_pq, tok_no, t, pqt, pq_tok_nb, id, tok_terminal_nb, nb;
  SXINT *tok_no_stack, *top_tok_no_stack, *t_stack, *top_t_stack, *stack, *bot_tok_no_stack;
  struct sxtoken  *ptok, *semlex_ptok;
  SXFLOAT proba_tok, proba_terminal_sachant_tok;
  SXFLOAT *pqt2proba;

  if (idag.pqt2logprob == NULL) {
    fill_idag_pqt2pq ();

    pqt2proba = (SXFLOAT *) sxcalloc (MAX_pqt () + 1, sizeof (SXFLOAT));
    idag.pqt2logprob = (SXLOGPROB *) sxcalloc (MAX_pqt () + 1, sizeof (SXLOGPROB));
	...
	while (tok_no_stack <= top_tok_no_stack) {
	  ...
	  if (semlex_ptok->lahead == get_SEMLEX_lahead ()) {
	    proba_tok = sxatof (sxstrget (semlex_ptok->string_table_entry));
	    //	    proba_trans = fudge_factor * sxlog10 (dynweight);
	  }
	  else {
	    proba_tok = (SXFLOAT)1 / pq_tok_nb ; // A FAIRE: le cas où certaines trans p->q ont un semlex et d'autres pas
	  }
...
#define CODE_STACK t_code_stack
	  ...
	  if (id > MAX_ID) {
	    ...
	    while (nb-- > 0) {
	      ...
	      id = ift_code2terminal [id]; /* on rétablit le terminal à partir du ift_code */
#else /* is_probabilized_lexicon */
	      proba_terminal_sachant_tok = (SXFLOAT)1;// / tok_terminal_nb;
#endif /* is_probabilized_lexicon */

	      pqt = pq_t2pqt (bot_pq, id);

	      pqt2proba [pqt] += proba_tok * proba_terminal_sachant_tok;
	    }
	  }
	  else {
	    ...
	    id = ift_code2terminal [id]; /* on rétablit le terminal à partir du ift_code */
#else /* is_probabilized_lexicon */
	    proba_terminal_sachant_tok = (SXFLOAT)1;
#endif /* is_probabilized_lexicon */
	...
	for (top_t_stack = t_stack + *t_stack; top_t_stack > t_stack; top_t_stack--) {
	  ...
	  idag.pqt2logprob [pqt] = (SXLOGPROB)((SXFLOAT)fudge_factor * (SXFLOAT)proba2SXLOGPROB (pqt2proba [pqt]));
	}
#if LOG
#ifndef no_dico_lecl
#ifndef is_LFG
	tok_no_stack = bot_tok_no_stack;
	...
	while (tok_no_stack <= top_tok_no_stack) {
	  ...
	  if (semlex_ptok->lahead == get_SEMLEX_lahead ()) {
	    semlex_str = sxstrget (semlex_ptok->string_table_entry);
	  }
	  else {
	    semlex_str = NULL;
	  }
	  ...
	  if (id > MAX_ID) {
	    ...
	    while (nb-- > 0) {
	      ...
	      print_idag_trans (from_state, ptok->comment, sxstrget (ptok->string_table_entry), semlex_str, id, idag.pq2q [bot_pq]);
	    }
	  }
	  else {
#ifdef is_probabilized_lexicon
...
idag_pq_t2pqt (SXINT pq, SXINT t)
{
  ...
  /* else
     pq == 0 : transition sur les EOF => pqt == 0 */
...
#include "SS.h"
/* met ds la SS_stack parametre l'ensemble des etats q
   qui se trouvent a la distance l >= 0 de p dans idag */
/* retourne la SS_stack qui a pu etre reallouee */
SXINT *
idag_p_l2q_SS_stack (SXINT *SS_stack, SXINT p, SXINT l)
{
  SXINT i, q, pq, bot, top, r;
  SXBA  q_set1, q_set2;

  q_set1 = idag.q_sets [0];
  q_set2 = idag.q_sets [1];
  ...
  while (l-- > 0) {
    ...
    while ((q = sxba_scan_reset (q_set1, q)) > 0) {
      bot = idag.p2pq_hd [q];
      top = idag.p2pq_hd [q+1];
    ...
    i++;
    i = i % 2;
    q_set2 = idag.q_sets [i]; /* resultat futur */
  }

  /* Ici q_set1 contient le resultat et q_set2 est vide */
  SS_open (SS_stack);
