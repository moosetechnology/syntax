...
char WHAT_XH_MNGR[] = "@(#)SYNTAX - $Id: XH_mngr.c 2227 2018-09-26 16:22:59Z garavel $" WHAT_DEBUG;
...
#define max(x,y)	((x)>=(y) ? (x) : (y))
	...
	fprintf (stat_file, "name = %s, hash_size = %ld (%ld%%), size = %ld (%ld%%), minl = %ld, maxl = %ld, meanl = %ld, list_size = %ld (%ld%%), minll = %ld, maxll = %ld, meanll = %ld, Total_size (byte) = %ld (%ld%%)\n",
		 header->name, (long)hash_size, (long)(100*n)/hash_size, (long)size, (long)(100*top)/size, (long)minl, (long)maxl, (long)meanl, (long)list_size, (long)(100*list_top)/list_size, (long)minll, (long)maxll, (long)list_top/top, (unsigned long)total_size, (unsigned long)(100*total_top)/total_size);
    }
    else
