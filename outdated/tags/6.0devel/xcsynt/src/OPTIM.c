...
char WHAT_XCSYNTOPTIM[] = "@(#)SYNTAX - $Id: OPTIM.c 2227 2018-09-26 16:22:59Z garavel $" WHAT_DEBUG;
...
static SXBOOLEAN	is_0_indistinguable (struct floyd_evans *afei, 
...
static SXBOOLEAN	is_k_indistinguable (struct floyd_evans *afei, 
				     struct floyd_evans *afej)
{
...
SXVOID	fe_to_equiv (struct floyd_evans *afe1,
...
static SXINT gen_prdct_seq (SXINT hd, 
			    ...
			    struct floyd_evans *fe, 
			    SXBOOLEAN is_t /* Cas des t_tables */ )
{
...
static SXVOID compute_action_set (SXINT xac1,
...
static SXVOID reset_germe_to_gr_act (void)
{
...
/*   L I N E A R I S A T I O N   D E S   M A T R I C E S   */
...

	{
	    ...
	    if (sxverbosep) {
	        fprintf (sxtty, "(density=%ld%%) ", (long)100*vector_used_nb/t_vector_size);
		sxtime (SXACTION, "done");
		sxttycol1p = SXTRUE;
	    }
	}
...
/* On prend comme defaut l'adresse la plus populaire et en cas d'egalite la plus
   elevee. */ 
	...
	if (sxverbosep) {
	    fprintf (sxtty, "(density=%ld%%) ", (long)100*vector_used_nb/(MAX - MIN + 1));
	    sxtime (SXACTION, "done");
	    sxttycol1p = SXTRUE;
	}
    }
