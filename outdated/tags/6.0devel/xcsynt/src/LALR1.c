...
#include "bstr.h"
#include "SS.h"
char WHAT_XCSYNTLALR1[] = "@(#)SYNTAX - $Id: LALR1.c 2227 2018-09-26 16:22:59Z garavel $" WHAT_DEBUG;


extern SXBOOLEAN	prio_read (SXINT t_priorities_size, 
...
extern SXBOOLEAN	ambiguity (SXBA conflict_set, SXBA ambig_root_set), 
...
extern SXBOOLEAN	ante (SXINT state, SXINT item, SXBOOLEAN (*f)(SXINT, SXINT));
extern SXBOOLEAN	ARC_construction (SXINT *failed_kind, 
                                  SXINT xac1, 
                                  SXBA conflict_t_set, 
...
static char	star_71 [] = "***********************************************************************";
static SXBOOLEAN	is_lalr1, is_ARCs, is_conflict, is_ARC_init_called, ff_needed,
...
static SXBOOLEAN print_current_conflict, print_next_conflict;
...

static SXVOID	oflw_Q0xT_to_conflict_action (SXINT old_size, SXINT new_size)
{
...
/* A METTRE DS LR(0) */
static SXBOOLEAN is_item_in_state (SXINT item, SXINT state)
{
...
static SXVOID ambig1_print (SXINT *epsilons, SXBA state_set)
{
...
static SXVOID cycle_print (SXBA *Rplus, SXBA state_set)
{
    SXINT i, nt;
...
static SXVOID	lookback (SXINT state, SXINT item, SXINT lookback_StNt)
{
...
SXVOID compute_Next_states_set (void)
{
...
static SXVOID	compute_includes (SXINT xac1)
{
...
static SXBOOLEAN erase_StNt_StNt1 (SXINT St, SXINT item)
{
    /* Appelee depuis ante */
...
SXVOID	cut_includes (SXINT q, SXINT q1)
{
...
static SXVOID	fill_items_set (SXINT *fill_items_set_nt_trans_set, 
...
static SXVOID	fill_look_ahead_set (SXINT *fill_look_ahead_set_items_set, SXBA la_set)
{
...
static SXVOID	set_conflict_action (SXINT xac1)
{
...
static SXBOOLEAN	compute_lalr1_conflicts (SXINT xac1)
{
    ...
    SXBA	la_set_r1;
    struct Q0	*aQ0;
    SXBOOLEAN	compute_lalr1_conflicts_is_conflict = SXFALSE;
	...
	if (cur_red_no > 1 || aQ0->t_trans_nb > 0) {
		...
		while (++r1 < cur_rule_no) {
			...
			if (!compute_lalr1_conflicts_is_conflict) {
			    ...
			    is_lalr1 = SXFALSE;
			    compute_lalr1_conflicts_is_conflict = SXTRUE;
			}
		    }
...
static SXBOOLEAN state_to_conflict_set (SXINT xac1, 
				      SXBOOLEAN is_u, 
				      SXBOOLEAN is_s, 
				      SXINT mask, 
...
SXVOID print_conclusions (SXINT xac1, SXBA print_conclusions_t_set)
{
...
static SXVOID print_minimal_message (SXINT xac1, SXBA print_minimal_message_t_set, char *gk)
{
...
static SXVOID print_minimal_messages (void)
{
    SXINT	xac1 = 0;
...
static SXVOID set_conflict_kind (SXINT germe_local, SXBA set_conflict_kind_t_set, SXINT failed_kind, SXBOOLEAN is_ALL)
{
...
SXVOID solved_by_ARC (SXINT germe_local, SXINT arc_no, SXBA solved_by_ARC_t_set)
{
...
SXBOOLEAN	should_print_conflict (SXINT conflict_kind)
{
...
static SXBOOLEAN	call_ARC (SXBOOLEAN is_u, SXBOOLEAN is_s, SXINT processed_kind)
{
    SXINT	xac1 = 0;
...
char *conflict_kind_to_string (VARSTR result, SXINT conflict_kind, SXINT h_val, SXINT k_val)
{
	...
	if ((ck & already_processed) != ck) {
	    ...
	    case NOT_RoxoLRk_:
		sprintf (string, "N O T   R (oxo) L R (%ld)", (long)k_val);
	    ...
	    case NOT_RhLALR0_:
		sprintf (string, "N O T   R (%ld) L A L R (0)", (long)h_val);
		varstr_catenate (result, string);
		break;
		
	    case NOT_RoxoLALRk_:
		sprintf (string, "N O T   R (oxo) L A L R (%ld)", (long)k_val);
		varstr_catenate (result, string);
		break;
		
	    case NOT_RhLALRk_:
		sprintf (string, "N O T   R (%ld) L A L R (%ld)", (long)h_val, (long)k_val);
		varstr_catenate (result, string);
		break;
	    ...
	    case NOT_LRk_:
		sprintf (string, "N O T   L R (%ld)", (long)k_val);
		varstr_catenate (result, string);
		break;
	    ...
	    case NOT_LALRk_:
		sprintf (string, "N O T   L A L R (%ld)", (long)k_val);
		varstr_catenate (result, string);
		break;
...
static SXBOOLEAN is_ambig_1 (SXBA StNt_set)
{
...
static SXVOID nd_list (SXINT kind, SXINT t, SXINT *act)
{
...

	...
	if (!(is_epsilon_free = sxba_scan (bnf_ag.BVIDE, 0) < 0)) {
		...
		while ((i = sxba_scan (ambig_states_by_identical_epsilon_derivation, i)) > 0)
		    set_conflict_kind (i, (SXBA)NULL, AMBIGUOUS_, SXTRUE);
	...
	if (ambig_root_set != NULL) {
		...
		struct Q0	*aQ0;
		SXBOOLEAN		LALR1_should_print_conflict;
		...
		while ((LALR1_StNt = sxba_scan (ambig_root_set, LALR1_StNt)) > 0) {
			...
			set_conflict_kind (xac1, (SXBA)NULL, AMBIGUOUS_, SXTRUE);
	...
	if (!is_epsilon_free) {
		...
		struct Q0	*aQ0;
		SXBA			StNt_set2;
		SXBOOLEAN			LALR1_should_print_conflict_2;
		...
		while ((xac1 = sxba_scan (conflict_set, xac1)) > 0) {
			...
			if ((StNt_set2 = is_not_LRpi_on_StNt (LALR1_StNt_2)) != NULL) {
				...
				grammar_kind |= kind;
				set_conflict_kind (xac1, (SXBA)NULL, kind, SXTRUE);
...

	...
	if (is_systems_conflict) {
		...
		if (sxverbosep)
		    fprintf (sxtty, "\t%sLR (%ld) construction%s ... ", is_lr_constructor ? "" : "LA", (long)k_value, is_automaton &&
			     is_listing_opened ? " & ARC output" : "");
		
		next_processed_kind = is_lr_constructor ? NOT_RoxoLR0_ :
					 (h_value == 0 ? NOT_RoxoLALR0_ : NOT_RLALR_);
		print_current_conflict = (SXBOOLEAN) (!is_rlr_constructor && is_list_system_conflicts);
		...
		if (sxverbosep) {
		    ...
		    fprintf (sxtty, "\t%s construction",
			     get_constructor_name (name, (SXBOOLEAN) (!is_lr_constructor), SXTRUE, h_value, k_value));
		...
		for (xe1 = lr0_automaton_state_nb + 1; xe1 < xac2; xe1++) {
		    fprintf (sxtty, "\t\tState #%ld is a clone of #%ld.\n", (long)xe1, (long)clone_to_lr0_state [xe1 - lr0_automaton_state_nb]);
		}
	    }
...
/* Generation du defaut */
	...
	if (psr + dsr + rlrsr + prr + drr + rlrrr > 0) {
	    ...
	    sprintf (results, "   %-6ld | %-6ld |  %-6ld  | %-6ld | %-6ld |  %-6ld  | %-6ld\n",
		     (long)(psr + dsr + rlrsr + prr + drr + rlrrr), (long)psr, (long)dsr, (long)rlrsr, (long)prr, (long)drr, (long)rlrrr);
	    fprintf (sxstderr,
		     header,
