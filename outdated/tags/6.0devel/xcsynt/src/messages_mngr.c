...
char WHAT_XCSYNTMESSAGESMNGR[] = "@(#)SYNTAX - $Id: messages_mngr.c 2227 2018-09-26 16:22:59Z garavel $" WHAT_DEBUG;
...
extern SXBOOLEAN	is_t_in_first (SXINT t, SXINT item);
extern SXBOOLEAN	ttrue (SXINT item, SXINT LA_ref, SXBOOLEAN is_prefixe);
extern SXBOOLEAN	earley (SXINT item, 
			SXINT q, 
			SXINT LA, 
			SXINT LA_lgth, 
			XH_header *hd, 
			SXINT BS2);
extern SXBOOLEAN	is_pss_include (SXINT *bot1, SXINT *top1, SXINT *bot2, SXINT *top2);
extern SXBA	fill_pss_set (struct ARC_struct *ARC, 
			      ...
			      SXINT type, 
			      SXBA set);
extern SXBOOLEAN	bh_ARC_one_LA (struct ARC_struct *ARC, SXINT qtq);
extern SXBOOLEAN	bs_ARC_one_LA (struct ARC_struct *ARC, SXINT qtq);
...

static struct bstr	*bstr_cat_Vs (struct bstr *bstr, SXINT Vs, SXBOOLEAN is_bot_up)
{
...
static SXVOID	xcsynt_write (SXINT red_no, SXINT item)
{
    /* item : A -> alpha . beta
       ecrit si red_no == 0
...
static SXVOID compute_items_np (SXINT cur_index, SXINT top_index, SXINT write_index, SXBOOLEAN is_shift)
{
...
static SXBOOLEAN is_conflict_prod_reached (SXINT **bot_items)
{
...
static SXBOOLEAN ecrire (SXBOOLEAN should_write)
{
    /* BOT_items et TOP_items sont des pointeurs dans stack_hd dans une liste d'items. */
    SXINT	x, item, tnt, nb;
    SXINT			red_no, *ptr;
...
static SXVOID	print_a_quad (SXBOOLEAN is_eof, SXINT Vs, SXINT items/* , Ts */ /*, delta_items */)
{
...
SXBOOLEAN erase_quads (SXINT ARC_kind, SXINT qtq)
{
...
SXBOOLEAN make_a_quad (SXINT etat_charniere, SXINT item_charniere, SXINT *items_stack)
{
...
static SXBOOLEAN call_earley (SXINT call_earley_fork)
{
...
static SXVOID build_XARC_path (/* XARC, XARC_state, */ SXINT type, SXINT build_XARC_path_fork)
{
...
static SXBOOLEAN conformity_parse_stack_pss (SXINT *parse_stack, SXINT pss)
{
...
static SXBOOLEAN conformity_LALR (SXINT *parse_stack)
{
...
SXBOOLEAN check_real_conformity (SXINT *parse_stack)
{
...
static SXVOID	print_title (char *shift_or_reduce, char *suffixe, char *conflict_name)
{
    char	str [8];
    ...
    sprintf (str, "%ld ", (long)germe);
    pre = bstr_cat_str (pre, str);
    pre = bstr_cat_str (pre, "on ");
    pre = bstr_cat_Vs (pre, LA, SXTRUE);
...
static SXBOOLEAN	ARP_knot_xpath (SXINT entree, SXINT sortie, SXINT *kbot, SXINT *ktop)
{
...
static SXVOID	ARP_call_knot_xpath (SXINT entree, SXINT sortie, SXINT knot)
{
...

SXVOID messages_mngr_alloc (void)
{
    SXINT l;
...
SXVOID messages_mngr_free (void)
{
...
static SXBOOLEAN one_ambig_LA (struct ARC_struct *XARC,
...
SXVOID mm_process_one_LA (struct ARC_struct *ARC)
{
...
SXVOID	sambig_conflict_messages (struct ARC_struct *XARC,
				  SXINT	qtq, 
...
static SXBOOLEAN is_in (SXINT path1, SXINT path2)
{
...
static SXBOOLEAN fork_couple_not_processed (SXINT fork_couple_not_processed_fork1, SXINT fork2)
{
...
SXVOID	ARP_conflict_messages (struct ARC_struct *XARC, 
			       SXINT prev_XARC_state, 
			       SXINT t, 
...
SXVOID conflict_messages_output (SXBA conflict_t_set)
{
