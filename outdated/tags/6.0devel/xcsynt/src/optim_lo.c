...
char WHAT_XCSYNTOPTIMLO[] = "@(#)SYNTAX - $Id: optim_lo.c 2227 2018-09-26 16:22:59Z garavel $" WHAT_DEBUG;
...

static SXVOID alloc_alpha_table (void)
{
...
{
    if (code >= 0) {
	sprintf (small_string, "&%ld", (long)code);
	return small_string;
    }
...
SXVOID	soritem (SXINT x, SXINT d, SXINT f, char *c1, char *s2)
{
    char	car;
    SXINT	i, c;
...
VARSTR	prod_to_str (VARSTR prod_to_str_vstr, SXINT prod)
{
    ...
    sprintf (small_string, "%-ld :\t", (long)prod);
...
SXBOOLEAN		open_listing (SXINT kind, 
		   ...
		   struct floyd_evans *afe)
{
    ...
    default:
#if EBUG
      sxtrap("xcsynt_optim_lo","unknown switch case #1");
#endif
      break;
    }
...
SXVOID	optim_lo (char *optim_lo_language_name, char *ME)
{
    static char		hyph60 [] = "-------------------------------------------------------------";
    static char		dot46 [] = ".............................................";
    SXINT	i, x, y, l;
    ...
    open_listing ((SXINT)2, optim_lo_language_name, ME, ".op.l");
    stats.nt.c.cols = 0;
    ...
    fprintf (listing, "\tT_tables:\t%ld\t%ld\t%ld\n", (long)stats.t.i.lines, (long)stats.t.i.cols, (long)(x = stats.t.i.lines * stats.t.i.cols))
	 ;
    fprintf (listing, "\tNT_tables:\t%ld\t%ld\t%ld\n", (long)stats.nt.i.lines, (long)stats.nt.i.cols, (long)(y = stats.nt.i.lines * stats.nt.i.
	 cols));
    put_edit_nl (3);
    ...
    fprintf (listing, "\tT_tables:\t%ld\t%ld\t%ld\t%ld\n", (long)stats.t.r.lines, (long)stats.t.i.cols, (long)i, (long)100 * (x - i) / x);
    i = stats.nt.r.lines * stats.nt.i.cols;
    fprintf (listing, "\tNT_tables:\t%ld\t%ld\t%ld\t%ld\n", (long)stats.nt.r.lines, (long)stats.nt.i.cols, (long)i, (long)100 * (y - i) / y);
    put_edit_nl (3);
    ...
    *nt_to_ad = 0 /* nombre de branchements directs */ ;
    ...
    fprintf (listing, "\t\t\t%ld\t%ld\t%ld\t%ld\t\t%ld over %ld\n", (long)stats.nt.c.lines, (long)stats.nt.c.cols, (long)i, (long)100 * (y - i) / y, (long)*
	 nt_to_ad, (long)bnf_ag.WS_TBL_SIZE.xnbpro);
    put_edit_nl (3);
    ...
    fprintf (listing, "\tT_tables:\t%ld\t%ld\t%ld\t%ld\n", (long)stats.t.r.lines, (long)t_vector_size, (long)y, (long)100 * (x - y) / x);
    x = stats.nt.i.lines * stats.nt.i.cols;
    y = stats.nt.c.lines * 3 + nt_vector_size * 2;
    fprintf (listing, "\tNT_tables:\t%ld\t%ld\t%ld\t%ld\n", (long)stats.nt.c.lines, (long)nt_vector_size, (long)y, (long)100 * (x - y) / x);
	...
	if (xpredicates > 0) {
	    struct P_prdct	*optim_lo_aitem, *optim_lo_lim;
	    struct state	*astate;
		...
		if (astate->lgth != 0 /* etat non vide */ ) {
		    optim_lo_aitem = prdcts + astate->start;
		    put_edit_nl (1);
		    put_edit_nnl (1, optim_lo_aitem->prdct >= 20000 ? "NonDeter " : "Prdct ");
		    put_edit_apnb (i);

		    for (optim_lo_lim = optim_lo_aitem + astate->lgth; optim_lo_aitem < optim_lo_lim; optim_lo_aitem++) {
			output_fe (optim_lo_aitem->prdct >= 20000 ? NonDeter : Predicate, optim_lo_aitem->action, optim_lo_aitem->prdct, tnt_fe + optim_lo_aitem->action);
		    }
		}
	    }
...
SXVOID	optim_ll (void)
{
    ...
    fprintf (listing, "\t%ld\t%ld\t%ld\t%ld\t%ld\t%ld\n", (long)mMrednt, (long)Mred, (long)Mprdct, (long)Mfe, (long)M0ref, (long)Mref);
    put_edit_nl (3);
...
SXVOID	optim_sizes (void)
{
    ...
    fprintf (listing, "tables_size = (%ld + %ld)*3 + %ld*2 + %ld*4 + %ld*2 + %ld + %ld*3 = %ld\n", (long)Mtstate, (long)Mntstate, (long)Mvec, (long)Mred, (long)xprdct_items, (long)Mgerme - mgerme, (long)prdct_list_top, (long)(
