...
char WHAT_XCSYNTEARLEY[] = "@(#)SYNTAX - $Id: earley.c 2227 2018-09-26 16:22:59Z garavel $" WHAT_DEBUG;
...
extern SXBOOLEAN	check_real_conformity (SXINT *parse_stack);
extern SXBOOLEAN	check_level_conformity (SXINT item, SXINT level);
extern SXBOOLEAN	make_a_quad (SXINT etat_charniere, SXINT item_charniere, SXINT *items_stack);
...
static SXVOID	oflw_EI (SXINT old_size, SXINT new_size)
{
    sxuse(old_size); /* pour faire taire gcc -Wunused */
    EI_set = sxba_resize (EI_set, new_size + 1);
}

static SXVOID EISL_stack_sature (void)
{
...
SXBOOLEAN ttrue (SXINT item, SXINT LA_ref, SXBOOLEAN is_prefixe)
{
...
static SXVOID earley_alloc (SXINT l)
{
    if (EI_set == NULL) {
...
static SXBOOLEAN check_item (SXINT item)
{
...
static SXBOOLEAN trans (SXINT t)
{
    SXINT	id, x, item;
    SXINT			z, lim;
...
static SXBOOLEAN new_EI (SXINT state, SXINT item)
{
...
static SXBOOLEAN prev_state (SXINT state, SXINT item)
{
...
static SXBOOLEAN check_bottom (SXINT d)
{
...
static SXVOID build_parse_stack (SXINT x)
{
    SXINT	item, z, q1;
    SXINT			id, i, d;
