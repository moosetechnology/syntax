...
char WHAT_XCSYNTFIRST[] = "@(#)SYNTAX - $Id: first.c 2227 2018-09-26 16:22:59Z garavel $" WHAT_DEBUG;
...
static SXBOOLEAN		(*parse_completed)(SXINT, SXINT, SXBOOLEAN);

static SXVOID	oflw_IxES (SXINT old_size, SXINT new_size)
{
    sxuse(old_size); /* pour faire taire gcc -Wunused */
    EI_set = sxba_resize (EI_set, new_size + 1);
}

static SXVOID	oflw_ESxT (SXINT old_size, SXINT new_size)
{
    sxuse(old_size); /* pour faire taire gcc -Wunused */
    ES_T_to_ES = (SXINT*) sxrealloc (ES_T_to_ES, new_size + 1, sizeof (SXINT));
}

static SXVOID first_alloc (void)
{
...
static SXVOID	ei_put (SXINT item, SXINT d)
{
    SXINT id;
...
static SXBOOLEAN prefixe (SXINT item, SXINT t_ref)
{
...
SXBOOLEAN earley_parse (SXINT LA_init, SXINT LA_final, XH_header *hd, SXINT item, SXBOOLEAN (*F)(SXINT, SXINT, SXBOOLEAN))
{
    stack_hd = hd;
    LA_top = LA_final;
    parse_completed = F;
