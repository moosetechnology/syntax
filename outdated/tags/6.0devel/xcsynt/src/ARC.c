...
char WHAT_XCSYNTARC[] = "@(#)SYNTAX - $Id: ARC.c 2227 2018-09-26 16:22:59Z garavel $" WHAT_DEBUG;

#include "SS.h"
#include "RLR.h"


extern SXBOOLEAN		erase_quads (SXINT ARC_kind, SXINT qtq);
extern SXVOID		solved_by_ARC (SXINT germe_local, SXINT arc_no, SXBA solved_by_ARC_t_set);
extern SXVOID		mm_process_one_LA (struct ARC_struct *ARC);
...
static SXVOID ARC_UNBOUNDED_Q_oflw (SXINT old_size, SXINT new_size)
{
...
static SXVOID ARC_BOUNDED_or_FSA_Q_oflw (SXINT old_size, SXINT new_size)
{
    sxuse(old_size); /* pour faire taire gcc -Wunused */
...
static SXVOID	oflw_pss_to_Next (SXINT old_size, SXINT new_size)
{
    sxuse(old_size); /* pour faire taire gcc -Wunused */
...
static SXVOID	oflw_ibfsa (SXINT old_size, SXINT new_size)
{
    sxuse(old_size); /* pour faire taire gcc -Wunused */
...
static SXVOID	oflw_fknot (SXINT old_size, SXINT new_size)
{
    sxuse(old_size); /* pour faire taire gcc -Wunused */
...
static SXVOID	oflw_PSSxT (SXINT old_size, SXINT new_size)
{
    sxuse(old_size); /* pour faire taire gcc -Wunused */
...
static SXVOID	oflw_pss (SXINT old_size, SXINT new_size)
{
    SXINT	i = new_size;
...
SXVOID	ARC_allocate (struct ARC_struct *ARC,
		      SXINT kind,
		      SXVOID (*F_oflw1)(SXINT, SXINT) )
{
...
static SXVOID	ARC_clear (struct ARC_struct *ARC)
{
    XH_clear (&(ARC->Q_hd));
...
static SXVOID	ARC_deallocate (struct ARC_struct *ARC)
{
    XH_free (&(ARC->Q_hd));
    sxfree (ARC->Q_set);
...
static SXVOID	undo (SXINT old_xac2)
{
...
SXBOOLEAN check_level_conformity (SXINT item, SXINT level)
{
...
SXBOOLEAN check_bot_state_conformity (SXINT state, SXINT level)
{
    /* Appele depuis earley.
...
static SXBOOLEAN	is_new_pss (SXINT pss, SXINT state, SXINT is_new_pss_index, SXINT next_state, SXINT *new_pss)
{
...
static SXVOID	complete_cycle_on_reads (void)
{
...
static SXVOID	Next_with_fknot (SXINT pss, SXINT fknot);

static SXVOID	Next (SXINT top_state, SXINT pss)
{
...
static SXVOID call_Next (SXINT top_state, SXINT pss)
{
...
static SXVOID	put_newr_in_back (SXINT X1, SXINT X2)
{
...
static SXVOID	put_r_in_back (SXINT X1, SXINT X2)
{
...
static SXBOOLEAN is_new_knoted_pss (SXINT pss, SXINT pred, SXINT is_new_knoted_pss_index, SXINT next_state, SXINT *pss1)
{
...
static SXVOID	compute_back (SXINT ppss, SXINT state, SXINT compute_back_index, SXINT item, SXINT pss)
{
...
static SXVOID	call_compute_back (SXINT ppss, SXINT state, SXINT call_compute_back_index, SXINT item, SXINT pss)
{
    if (ibfsa_needed) {
...
SXVOID	back_depth_first_traversal (SXINT X1)
{
...
SXBOOLEAN is_pss_include (SXINT *bot1, SXINT *top1, SXINT *bot2, SXINT *top2)
{
...
static SXVOID	pss_set_reduction (SXBA pss_set, SXINT *m, SXINT *M)
{
...
static SXVOID	put_in_work_set (SXINT xc)
{
...
static SXVOID	sim_a_red (SXINT pss, SXINT t)
{
...
static SXINT	couple_name (SXBA_INDEX x, SXBA_INDEX y, char **name)
{
    ...
    sprintf (s, "%ld", (long)X_X (Q0_to_short_hd, y));
    ate = sxstrsave (s);
    *name = sxstrget (ate);
    return 0;
}
...
static SXVOID	ARC_cg (SXINT xac1_init, SXINT new_state_nb, SXINT plulong)
{
...
static SXBOOLEAN ARC_max_la (SXINT state, SXINT lgth)
{
...
static SXBOOLEAN	BS_couple (SXINT *X, SXINT *Y, SXINT n)
{
...
static VARSTR	couples_to_re (SXINT *bot, SXINT *top, SXINT entree, SXINT sortie, VARSTR couples_to_re_vstr)
{
	...
	do {
	    couple = *bot;
	    sprintf (s, "%ld %ld", (long)XxY_X (Q0xQ0_hd, couple), (long)XxY_Y (Q0xQ0_hd, couple));
...
static SXBOOLEAN	is_same_pss (SXINT xkas, SXBA *is_same_pss_pss_sets)
{
...
SXBOOLEAN	make_inter_pss (SXINT pss1, SXINT pss2)
{
...
static SXBOOLEAN is_cycle_on_fsa (SXINT pss)
{
...
static SXINT	is_inter_pss (SXINT xkas, SXBA *is_inter_pss_pss_sets, SXBOOLEAN is_all, SXBOOLEAN is_cycle_needed)
{
...
SXBOOLEAN ARC_walk_backward (struct ARC_struct *ARC,
			   SXINT qtq,
			   SXBOOLEAN (*bh)(struct ARC_struct *, SXINT),
			   SXBOOLEAN (*bs)(struct ARC_struct *, SXINT))
{
...
static SXBOOLEAN ARC_walk_forward (struct ARC_struct *ARC,
				 SXINT qtq,
				 SXBOOLEAN (*fh)(struct ARC_struct *, SXINT),
				 SXBOOLEAN (*fs)(struct ARC_struct *, SXINT))
{
...
static SXBOOLEAN fh_propagate_cycle (struct ARC_struct *ARC, SXINT qtq)
{
...
static SXBOOLEAN fh_propagate_lgth (struct ARC_struct *ARC, SXINT qtq)
{
	...
	if (pss_kind == UNBOUNDED_ && attr->lgth != -1)
	    ARC_walk_forward (ARC, qtq, fh_propagate_cycle, (SXBOOLEAN(*)())NULL);
...
SXBOOLEAN fs_unmark (struct ARC_struct *ARC, SXINT qtq)
{
...
SXVOID	ts_to_kas (SXINT xpts, struct kas *akas)
{
    SXINT	x, lim, V;
...
SXVOID	print_bfsa (SXINT bfsa, SXINT ate)
{
    char	s [8];
...
static SXBOOLEAN	make_fknot (SXINT e, SXINT s, SXINT nb, SXINT *result)
{
...
SXVOID	print_pss (SXINT pss)
{
    SXINT	*bot, *top;
    char	s [8];

    sprintf (s, "%ld=", (long)pss);
    put_edit_apnnl (s);
    varstr_raz (vstr);
    bot = PSS_BOT (pss);
    top = PSS_TOP (pss);
...
static SXVOID	ARC_print (struct ARC_struct *ARC)
{
    ...
    get_constructor_name (c_name, SXTRUE, (SXBOOLEAN) (ARC->pss_kind != UNBOUNDED_), ARC->h_value, ARC->k_value);
...
SXBOOLEAN		X_sees_pss (SXINT X, SXINT pss)
{
...
static SXBOOLEAN	check_lc (SXINT check_lc_max_min_lgth)
{
    ...
    return check_lc_max_min_lgth > ORIG_K_VALUE ? (NEW_K_VALUE = check_lc_max_min_lgth, SXTRUE) : SXFALSE;
}
...
SXVOID	fill_kas (struct ARC_struct *ARC, 
		  SXINT ARC_state, 
		  ...
		  SXINT conflict_kind)
{
	...
	*xkas = xk;
	...
	is_inter_pss (xk, fill_kas_pss_sets, SXTRUE, SXFALSE);
    }
...
SXVOID	ARC_init (SXBOOLEAN print_conflicts, 
		  SXINT kind, 
...
SXVOID	ARC_get_sizes (SXINT *arc_state_nb, SXINT *arc_trans_nb)
{
    SXINT l, m;
...
static SXINT	install_a_pss_list (struct kas *akas, SXBOOLEAN is_init_state)
{
...
static SXBOOLEAN	install_arc_state (struct ARC_struct *ARC,
...
static SXVOID	ARC_gather_trans (struct ARC_struct *ARC, SXINT ARC_state)
{
    /* On est sur un [X]ARC.
...
static SXBOOLEAN ARC_trans (struct ARC_struct *ARC, 
			  SXINT ARC_state, 
			  SXINT t, 
...
SXBOOLEAN bh_ARC_one_LA (struct ARC_struct *ARC, SXINT qtq)
{
...
SXBOOLEAN bs_ARC_one_LA (struct ARC_struct *ARC, SXINT qtq)
{
...
static SXVOID ARC_conflict_processing (struct ARC_struct *ARC, SXINT qtq)
{
...
SXBOOLEAN bh_mark (struct ARC_struct *ARC, SXINT qtq)
{
...
SXBOOLEAN bh_unmark (struct ARC_struct *ARC, SXINT qtq)
{
    struct ARC_ATTR	*attr = ARC->attr + XxYxZ_X (ARC->QxTxQ_hd, qtq);
...
static SXVOID ARC_init_state_building (struct ARC_struct *ARC)
{
...
static struct ARC_struct *call_ARC_building (struct ARC_struct *XARC, SXINT qtq)
{
    ...
    ARC_walk_backward (XARC, qtq, bh_mark, (SXBOOLEAN(*)())NULL);

    ARC_building (ARC, ARC_trans, ARC_UNBOUNDED_get_conflict_kind);
    
    /* On enleve les marques. */
    ARC_walk_backward (XARC, qtq, bh_unmark, (SXBOOLEAN(*)())NULL);
...
static SXBOOLEAN ARC_conflicting_node_processing (struct ARC_struct *ARC, SXINT qtq)
{
...
static SXBOOLEAN fh_remote_conflict (struct ARC_struct *ARC, SXINT qtq)
{
...
static SXBOOLEAN bh_not_pure_tree (struct ARC_struct *ARC, SXINT qtq)
{
    struct ARC_ATTR	*attr = ARC->attr + XxYxZ_X (ARC->QxTxQ_hd, qtq);
...
SXINT	ARC_building (struct ARC_struct *ARC,
		      SXBOOLEAN (*F_trans)(struct ARC_struct *, SXINT, SXINT, SXINT*),
		      SXINT (*F_get_conflict_kind) (struct ARC_struct *, SXINT, SXBOOLEAN, SXINT))
{
    SXINT	ARC_state, t;
    struct ARC_ATTR	*attr;
    SXINT			next_ARC_state, qtq, ck, x_tree_orig, type_nb, new_lgth;
    SXBOOLEAN		give_up, is_new_ARC_state;
	...
	while ((t = sxba_scan_reset (ARC->t_set, t)) > 0) {
		...
		if (qtq != 0) {
			...
			if (ck != NO_CONFLICT_)
			    ARC_walk_backward (ARC, qtq, bh_not_pure_tree, (SXBOOLEAN(*)())NULL);
...
SXBOOLEAN ARC_construction (SXINT *failed_kind, 
			  SXINT xac1, 
			  SXBA conflict_t_set, 
...
SXVOID	RLR_code_generation (SXINT xac1, SXINT arc_no, SXBA t_set, SXINT plulong)
{
