...
#include "S_tables.h"
#include "put_edit.h"
char WHAT_LECLSTTOC[] = "@(#)SYNTAX - $Id: st_to_c.c 2313 2020-03-25 12:00:26Z garavel $" WHAT_DEBUG;
...
static SXBOOLEAN	is_shift [] = {SXFALSE, SXTRUE, SXTRUE, SXFALSE, SXFALSE, SXFALSE, SXFALSE, SXTRUE, SXTRUE, SXFALSE};
static SXBOOLEAN	is_hash_reduce [] = {SXFALSE, SXFALSE, SXFALSE, SXTRUE, SXFALSE, SXTRUE, SXFALSE, SXFALSE, SXFALSE, SXFALSE};
...
static SXINT	i, j;
static SXBOOLEAN	is_erase, is_include, is_mark, is_release, is_dummy_hash, is_user_action_or_prdct,
...
static SXVOID	out_S_char_to_simple_class (void)
{
...
static SXBOOLEAN	end_of_token (SXINT xprod)
{
    for (;;) {
...
static SXVOID	goto_state (SXINT state_no, SXBOOLEAN is_keep, SXBOOLEAN is_scan)
{
...
static SXVOID	reduce (SXINT tok, SXBOOLEAN is_keep, SXBOOLEAN is_scan, SXBOOLEAN is_hash, SXBOOLEAN is_empty, SXBOOLEAN reduce_is_post_action)
{
...
static SXVOID	gen_error (SXINT state_no)
{
...
static	SXVOID gen_label (SXBOOLEAN is_case, SXBA sc_set)
{
...
static SXVOID	gen_body (SXINT state_no, SXBOOLEAN is_empty, transition_matrix_s gen_body_stmt)
{
    SXBOOLEAN	is_keep, is_scan, is_ts_null_done;
    SXINT		codop, next;

    is_ts_null_done = SXFALSE;
...
goto_field:
    ...
    case Reduce:
	reduce (next, is_keep, is_scan, SXFALSE, is_empty, SXFALSE);
	break;

    case HashReduce:
	reduce (next, is_keep, is_scan, SXTRUE, is_empty, SXFALSE);
	break;
    ...
    case ReducePost:
	reduce (next, is_keep, is_scan, SXFALSE, is_empty, SXTRUE);
	break;
	
    case HashReducePost:
	reduce (next, is_keep, is_scan, SXTRUE, is_empty, SXTRUE);
	break;
    default:
#if EBUG
...
static	void gen_prdct_seq (SXINT state_no, SXINT y)
{
    SXINT		x;
    SXBOOLEAN	is_empty;
    SXBOOLEAN	is_in_la = NEXT (lecl_tables.S_transition_matrix [state_no] [1]) != 0;
...
static void	gen_state (SXINT state_no)
{
    SXBOOLEAN	is_error, is_mref, is_empty;
    SXINT	gen_state_i;
...
int main (int argc, char *argv[])
{
    ...
    SXBOOLEAN	state_status, next_state_status;
...
/* On calcule le nombre de references a un etat */
    ...
    is_dummy_hash = SXFALSE;
    is_first_col = SXFALSE;
    is_last_col = SXFALSE;
    is_user_action_or_prdct = SXFALSE;
    is_stmt_5_0 = SXFALSE;
    is_la = SXFALSE;
    is_post_action = SXFALSE;
    is_user_prdct = SXFALSE;
    is_keep_shr = SXFALSE;
    is_shr = SXFALSE;
    is_keep_sr = SXFALSE;
    is_sr = SXFALSE;
    is_ascii = SXTRUE;
    is_keep_sr_to_comments = SXFALSE;
    is_sr_to_comments = SXFALSE;
    is_erase = SXFALSE;
    is_include = SXFALSE;
    is_mark = SXFALSE;
    is_release = SXFALSE;
...
new_try:
	...
	for (j = 1; j <= lecl_tables.S_tbl_size.S_last_simple_class_no; j++) {
	    ...
	    case Reduce:
		if (next == 0) /* comments */ {
		    SXBOOLEAN	is_empty = sxba_bit_is_set (is_ts_empty, i);
			...
			if (KEEP (stmt))
			    is_keep_sr_to_comments = SXTRUE;
			else if (!is_empty)
			    is_sr_to_comments = SXTRUE;
		    }
		    else {
			if (is_empty)
			    sxba_1_bit (is_ref, 1);
...
once_more:
	...
	if (!fe.is_system) {
	    is_user_action_or_prdct = SXTRUE;

	    if (fe.kind == IsPredicate)
		is_user_prdct = SXTRUE;
	}
...
/* generation de l'epilogue */
	...
	put_edit_ap ("sxsvar.sxlv.mode.is_silent = SXFALSE;");
	put_edit_ap ("sxsvar.sxlv.mode.with_system_act = SXTRUE;");
	put_edit_ap ("sxsvar.sxlv.mode.with_user_act = SXTRUE;");
	put_edit_ap ("sxsvar.sxlv.mode.with_system_prdct = SXTRUE;");
	put_edit_ap ("sxsvar.sxlv.mode.with_user_prdct = SXTRUE;");
