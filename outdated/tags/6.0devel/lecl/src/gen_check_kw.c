...
#include "lecl_ag.h"
#include "sxdico.h"
char WHAT_LECLGENCHECKKW[] = "@(#)SYNTAX - $Id: gen_check_kw.c 2227 2018-09-26 16:22:59Z garavel $" WHAT_DEBUG;
...
SXVOID gen_check_kw (void)
{
	...
	if (nbndef > 1) {
	    ...
	    if (((~((SXUINT) 0))>>dico.base_shift) < dico.max)
	      sxtmsg (sxsrcmngr.source_coord.file_name,"Too many keywords, dictionary not produced\n", sxplocals.sxtables->err_titles [2] /* Error */);
	}
