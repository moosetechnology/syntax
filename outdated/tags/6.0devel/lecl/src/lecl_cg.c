...
#include "lecl_ag.h"
#include "sxsstmt.h"
char WHAT_LECLCG[] = "@(#)SYNTAX - $Id: lecl_cg.c 2227 2018-09-26 16:22:59Z garavel $" WHAT_DEBUG;
...
/* Calcul de max_la */
	...
	sxtmsg (sxsrcmngr.source_coord.file_name,
	    "The generated finite state automaton is nondeterministic.", err_titles [1]);
    }
...
/* On previent l'utilisateur que certains de ses predicats vont s'executer
   en look-ahead, le code doit donc en tenir compte... */
	...
	sxtmsg (sxsrcmngr.source_coord.file_name,
	    "The following user's predicates will be executed in look-ahead: %s.", err_titles [1], varstr_tostr (vstr));
	varstr_free (vstr);
    }
...
/* On verifie que tous les tokens d'interet sont reconnus */
	...
	sxtmsg (sxsrcmngr.source_coord.file_name,
		"The following regular expression%s not been generated :\n%s.",
