...
struct G {
  ...
  SXBOOLEAN	is_top_down_erasing;
  SXBOOLEAN	is_bottom_up_erasing;
...
#if 0
/* vieille version qui semble pré-p/q2range */
#define MAKE_A_RANGE(i,j)		(((i)<<idag.logn)+(j))

#define RANGE2i(ij)	((ij)>>logn)
#define RANGE2j(ij)	((ij)&ANDj)
#endif /* 0 */
#define MAKE_A_RANGE(p,q)		p_q2range(p,q)

#define RANGE2i(range)	range2p(range)
#define RANGE2j(range)	range2q(range)

SX_GLOBAL_VAR_RCG SXBOOLEAN    arity_eq_2_is_used, arity_eq_3_is_used, arity_geq_4_is_used;
...
SX_GLOBAL_VAR_RCG SXINT check_if_too_large (SXINT lognt);
#if is_generator
SX_GLOBAL_VAR_RCG SXINT generator (struct G *pG, SXINT CLAUSE, SXINT *RET_VALS, SXINT *INDEXES, SXINT *ILB, SXINT *IUB, SXINT *OLB, SXINT *OUB);
SX_GLOBAL_VAR_RCG SXINT _GStrEq (SXINT *lb, SXINT *ub);
#endif /* is_generator */
//SX_GLOBAL_VAR_RCG SXINT _StrReverse (SXINT *rho0, SXINT *lb, SXINT *ub);
//SX_GLOBAL_VAR_RCG SXINT StrConcat (SXBOOLEAN is_first_arg, SXINT *arg, SXINT *Xlb, SXINT *Xub);
SX_GLOBAL_VAR_RCG SXINT _StrEq2 (SXINT *rho0, SXINT size1, SXINT *arg1, SXINT size2, SXINT *arg2);
SX_GLOBAL_VAR_RCG SXINT _StrLen2 (SXINT *arg, SXINT *Xlb, SXINT *Xub);
SX_GLOBAL_VAR_RCG SXINT sdag_semact (SXINT code, SXINT numact);
#if is_guided && !is_coupled
SX_GLOBAL_VAR_RCG SXINT guide_semact (SXINT code, SXINT numact);
#endif /* is_guided && !is_coupled */
SX_GLOBAL_VAR_RCG SXINT _StrEq (SXINT range0, SXINT range1);
SX_GLOBAL_VAR_RCG SXINT _StrLen (SXINT range, SXINT len);
SX_GLOBAL_VAR_RCG SXINT _StrEqLen (SXINT range0, SXINT range1);
SX_GLOBAL_VAR_RCG SXBOOLEAN dynam_lex (SXINT lb, SXINT ub, SXINT nb, SXINT *t_list);
SX_GLOBAL_VAR_RCG SXINT MAKE_A_TERMINAL_RANGE (SXINT p, SXINT q);
SX_GLOBAL_VAR_RCG SXINT t_clause_processing (struct G *G, SXINT prdct_id, SXINT *rho0, SXINT p, SXINT q);
...
#ifndef SXEXIT
# define SXEXIT(sevlev)	sxexit ((int) sevlev)	/* should never return */
SX_GLOBAL_VAR_RCG SXVOID sxexit (int sevlev);
#endif


SX_GLOBAL_VAR_RCG void       memo_oflw_second_part (SXINT old_size, SXINT new_size)
