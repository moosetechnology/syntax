...
\documentclass[draft,twoside]{report}  % `draft' for the time being
...
\title{
  {\huge T}ABC   \\[10pt]
  PRÉSENTATION SUCCINCTE\footnote{Ces quelques pages ont été écrites par Pascal Bouchon en juin
    1988, alors élève de troisième année à l'ENSIMAG.}  }
...
\chapter{Présentation}
   ...
   décrite dans un fichier {\tt nom\_du\_langage.tabc}.
...
\section{Attributs synthétisés}
   ...
   \TABC{} permet la gestion entièrement transparente pour le 
   programmeur d'attributs synthétisés.

   Ces attributs sont soumis à des conditions d'utilisation
   afin de permettre à \TABC{} de vérifier la cohérence de
   leurs emplois.
...
\end{verbatim}
   Cette règle déclare localement l'attribut compteur associé au non-
...
%\begin{example}
     ...
     Le fichier {\it mots.tabc} correspondant s'écrit alors~:
\begin{verbatim}
