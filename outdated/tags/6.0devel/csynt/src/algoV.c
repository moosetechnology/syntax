...
#include "sxunix.h"
char WHAT_ALGOV[] = "@(#)SYNTAX - $Id: algoV.c 2227 2018-09-26 16:22:59Z garavel $" WHAT_DEBUG;
...
static SXVOID	overflow (SXINT *lower, SXINT *upper)
{
...
static SXBOOLEAN	is_compatible (struct item *avector, struct item *aitem, struct item *lim)
{
    SXINT	act, actpi, t;
    struct item	*pi;
...
static SXBOOLEAN	par_poids (SXINT i, SXINT j)
{
...
static SXBOOLEAN	par_gain (SXINT i, SXINT j)
{
...
static SXVOID	put_in_Vic (struct item *aitem, struct item *lim)
{
    SXINT	t, act;
...
static SXVOID	put_in_Vic_with_default (struct item *aitem, struct item *lim)
{
    SXINT	t, act, actVic;
...
static SXVOID	get_compatibility_with_Vic (struct item *aitem, struct item *lim, SXINT *size, SXINT *def)
{
    SXINT	t, act, actVic;
...
static SXVOID	Ecp_construction (struct state *states, struct item *items)
...
static SXINT	algoIV (struct item *aitem, struct item *lim, SXINT state)
{
    ...
    x = get_next_origine (state, aitem, lim);
    /* note : get_next_origine() peut modifier la variable globale vector */
    avector = vector + x;
