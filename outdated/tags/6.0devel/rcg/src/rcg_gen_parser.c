...
char WHAT_RCGGENPARSER[] = "@(#)SYNTAX - $Id: rcg_gen_parser.c 2227 2018-09-26 16:22:59Z garavel $" WHAT_DEBUG;


extern SXINT		is_in_Tstar (SXINT, SXINT);
extern SXINT            clause2lhs_nt (SXINT clause);
extern SXBOOLEAN        rhs_arg2sub_arg (SXINT lhs_prdct, SXINT arg);


#define has_semantics		SXTRUE
/* A VOIR */
...
/* Le 05/02/04 prgentname est le prefixe du nom de fichier, il peut contenir
   ...
   On cree donc la macro PRGENTNAME(b) : si b==SXTRUE => prgentname sinon "RCG" */
...
static SXBOOLEAN		Acut, Anon_range, is_source_checked, rhs_arg_nb;
...
static SXBOOLEAN	has_predef_prdct, has_predef_prdct_call, is_some_equiv, is_static_phase;
static SXBOOLEAN	has_external, has_ext_def, has_ext_decl;
static SXBOOLEAN	clause_has_cut, is_lex_clause;
...
static SXBOOLEAN		is_cyclic_grammar;
...
static SXBOOLEAN        is_GET_QSTACK, is_GET_PSTACK;
  ...
  if (nt2arity [lhs_nt]) {
	...
	if (nt == lhs_nt && XH_list_elem (rcg_predicates, bot2) == 0) {
	  ...
	  /* Vu les conditions d'appel, on a
	     !is_combinatorial && !is_lex_clause && !SXBA_bit_is_set (loop_clause_set, clause) == SXTRUE */
...
{
  SXINT       lhs_prdct, lhs_bot2_gen_nl_stmt_list, lhs_top2_gen_nl_stmt_list, lhs_cur2, lhs_nt, rhs, top, bot, cur, pos, param, bot3, top3, cur3, size, clause, nb, var_nb, rhs_pos, top_list, bot1, top1, cur1;
  ...
  SXINT       symb_id, cur_lhs_t_nb, max_lhs_pos;
  ...
  SXBOOLEAN   is_a_non_linear_clause, is_a_true_non_linear_clause; 
  ...
  DALLOC_STACK (symb_stack, max_clause_arg_nb*(max_arg_size+1)+1);
  ...
  for (clause = 1; clause <= last_clause; clause++) {
    ...
    is_a_non_linear_clause = SXFALSE;
	...
	max_lhs_pos = DTOP (symb_stack);

	/* ... et ceux de la RHS. */
	rhs = XxY_Y (rcg_clauses, clause);
	cur_pos = lhs_arity;

	is_a_true_non_linear_clause = SXFALSE;
		...
		if (param > 0) {
		  ...
		  for (cur3 = bot3; cur3 < top3; cur3++) {
			...
			if (pos_stack [next] >= lhs_arity)
			  /* ... en rhs */
			  is_a_true_non_linear_clause = SXTRUE;
	...
	for (pos = 0; pos < lhs_arity; pos++) {
	  ...
	  if ((var_nb = nb = *pos_symb_stack) > 0) {
		...
		if (rhs_pos < 0) {
		  ...
		  if (rhs_pos <= max_lhs_pos) { /* top-down erasing: la variable de LHS en cours de traitement n'est nulle part en RHS */
		    /* on ne tolère ce cas que si le symbole
		       (variable) concerné n'est dans un ou plusieurs
		       arguments de la LHS que seul symbole de son ou
		       ses argument(s) */
		    if (var_nb > 1) {
		      sxtrap (ME, "gen_nl_stmt_list (top-down erasing of variables is only allowed for variables that are single-variable arguments. E.g., A(X, a1) --> . is allowed, A (X Y) --> B(Y) . is not, neither is A(X a Y) --> .)");
		    }
		    XH_push (nl_stmt_list, pos), size++;
...
/* Le 22/04/08 (pour le cas !isallow_input_dags, désormais le seul cas possible (l'option de compilation a été virée en csq))
...
gen_dag_parser (SXINT clause)
{
  ...
  SXBOOLEAN has_changed, is_pos_call, rhs_prdct_args_are_known, is_a_counter, is_a_non_linear_clause, has_strlen; 
  char      *exit_stmt;
  ...
  for (Ak = A_k2Ak (lhs_nt, 0), pos = 0, lhs_cur2 = lhs_bot2_gen_dag_parser+2;
	...
	/* param2[lr]adr [param] seront remis à 0 si l'argument est un compteur
	 ou ne contient que des terminaux */
...
|| adr2bound_values[%ld] != adr2bound_values[%ld] %s;\n",
	    ...
	    if (symb >= -max_ste) {
	      ...
	      if (terminal_file)
		fprintf (parser_file, "\
if ((rho [%ld] = p_q_t2pqt (adr2bound_values[%ld], adr2bound_values[%ld], SXT_%s)) == 0) %s;\n",
			 (long)(cur_rho_size+local_lhs_t_nb), adr, adr+1, sxstrget (t2Cid_ste [-symb]), exit_stmt);
	      else
		fprintf (parser_file, "\
if ((rho [%ld] = p_q_t2pqt (adr2bound_values[%ld], adr2bound_values[%ld], %ld)) == 0) %s;\n",
			 (long)(cur_rho_size+local_lhs_t_nb), adr, adr+1, -symb, exit_stmt);
	    }
	    else {
	  ...
	  if (SXBA_bit_is_set (counter_Ak_set, Ak)) {
	    ...
	    /* regardons s'il y a autre chose que des terminaux dans cet arg */
	    for (cur3 = bot3; cur3 < top3; cur3++) {
	      symb = XH_list_elem (rcg_parameters, cur3);
	    ...
	    if (cur3 == top3) { /* il n'y avait que des terminaux, on
				   ne peut pas considérer l'arg comme
				   known */
	      param2ladr [param] = 0;
	      param2radr [param] = 0;
	    }
  ...
  while (nb_of_arg_tbp) {
	...
	rhs_prdct_args_are_known = SXTRUE;
	pos = cur_pos;
	  ...
	  if (param > 0) {
		...
		for (cur3 = bot3; cur3 < top3; cur3++) {
		  ...
		  if (symb > 0 && symb2madr [symb] == 0 && symb2counter_madr [symb] == 0) {
		    /* unknown */
		    rhs_prdct_args_are_known = SXFALSE;
		    break;
		  }
		}
	      }
	...
	if (rhs_prdct_args_are_known) {
	  ...
	  /*
	    Nouveau de décembre 2014: nous commençons à tout mettre à jour pour que les Aij soient construits comme suit:
	    - premier bit: 1 ssi prédicat prédéfini
	    - deuxième bit: 1 ssi prédicat prédéfini qui est strlen
	    - lognt bits suivants:
	         - si prédicat standard ou prédicat prédéfini autre que strlen, c'est l'identifiant du prédicat (toujours positif, même si prédéfini)
		 - si strlen, stocke la longueur
              > ainsi, lognt n'est plus vraiment le log du max nt, il prend en compte aussi le nombre de bits nécessaires pour
                stocker les identifiants de tous les prédicats prédéfinis ainsi que la plus grande longueur trouvée dans les strlen
                de la grammaire (ou plutôt du module)
            - les bits restants: l'identifiant du n-uplet de ranges instanciés
	   */
	  
	  if (nt > 0) {
	    fprintf (parser_file,
		     "if (_%s(&(rho[%ld]), &(olb[%ld]), &(oub[%ld]))==-1) %s;\n",
		     sxstrget (nt2ste [nt]), (long)rhs_prdct_pos+1, (long)pos, (long)pos, exit_stmt);
	  }
	  else {
	    switch (nt) {
	    case STRLEN_ic:
	      pos++; /* on saute le premier arg de StrLen, qui est la longueur */
	      fprintf (parser_file,"{\n");
	      fprintf (parser_file, "SXINT local_range = p_q2range(olb[%ld],oub[%ld]);\n", pos, pos);
	      fprintf (parser_file, "if (_StrLen (local_range, %ld)==-1) %s;\n", len, exit_stmt); /* meme si len == 0 */
	      fprintf (parser_file, "X_set (&rhoA_hd, (((local_range<<lognt) + %ld) << Aij_type_offset) + 3 /* binaire 11, soit 1 pour dire que c'est un prédicat prédifini et 2 pour dire que c'est strlen, et que la zone réservée habituellement pour l'identifiant de prédicat stocke la longueur */, &dum_Aij);\n",len);
	      fprintf (parser_file, "rho [%ld] = MAKE_Aij_pid (-dum_Aij, 0);\n", rhs_prdct_pos+1);
	      fprintf (parser_file,"}\n");
	      break;
...
adr2bound_values [%ld] = SS_pop (SS_stack); // on pop le sommet\n", madr, len, ladr+1);
		  ...
		  fprintf (parser_file, "adr2bound_values[%ld] = adr2bound_values[%ld];\n", ladr+1, madr+1);
		  fprintf (parser_file, "if (p_q2dag_kind (adr2bound_values[%ld], adr2bound_values[%ld] == NO_PATH) %s;\n", ladr+1, radr, exit_stmt);
		}
	      }
	      else {
		if ((madr = symb2madr [symb]) == 0) {
		...
		if (XH_list_elem (rcg_parameters, x) < 0)
	      ...
	      is_GET_QSTACK = SXTRUE;

	      if (terminal_file)
		fprintf (parser_file, "\
if ((qstack_%ld = GET_QSTACK (adr2bound_values[%ld], SXT_%s)) == NULL) %s;\n\
for (top_qstack_%ld = qstack_%ld + *qstack_%ld; ++qstack_%ld <= top_qstack_%ld; ) {\n\
  if ((adr2bound_values[%ld] = *qstack_%ld) > adr2bound_values[%ld]) break;\n",
			 (long)local_lhs_t_nb, (long)ladr, sxstrget (t2Cid_ste [symb]), exit_stmt,
			 (long)local_lhs_t_nb, (long)local_lhs_t_nb, (long)local_lhs_t_nb, (long)local_lhs_t_nb, (long)local_lhs_t_nb,
			 (long)ladr+1, (long)local_lhs_t_nb, (long)radr);
	      else
		fprintf (parser_file, "\
if ((qstack_%ld = GET_QSTACK (adr2bound_values[%ld], %ld)) == NULL) %s;\n\
for (top_qstack_%ld = qstack_%ld + *qstack_%ld; ++qstack_%ld <= top_qstack_%ld; ) {\n\
  if ((adr2bound_values[%ld] = *qstack_%ld) > adr2bound_values[%ld]) break;\n",
			 (long)local_lhs_t_nb, (long)ladr, (long)symb, exit_stmt,
			 (long)local_lhs_t_nb, (long)local_lhs_t_nb, (long)local_lhs_t_nb, (long)local_lhs_t_nb, (long)local_lhs_t_nb,
			 (long)ladr+1, (long)local_lhs_t_nb, (long)radr);
		...
		exit_stmt = "continue";
	      ...
	      Tpq_stack_pos = get_Tpq_stack_pos (pos, bot3, cur3);
	      
	      if (terminal_file)
		fprintf (parser_file, "\
if ((rho [%ld] = p_q_t2pqt (adr2bound_values[%ld], adr2bound_values[%ld], SXT_%s)) == 0) %s;\n",
			 (long)(cur_rho_size+Tpq_stack_pos), (long)ladr, (long)ladr+1, sxstrget (t2Cid_ste [symb]), exit_stmt);
	      else
		fprintf (parser_file, "\
if ((rho [%ld] = p_q_t2pqt (adr2bound_values[%ld], adr2bound_values[%ld], %ld)) == 0) %s;\n",
			 (long)(cur_rho_size+Tpq_stack_pos), (long)ladr, (long)ladr+1, (long)symb, exit_stmt);
	    }
	  }
	  ...
	  if (ladr == radr-1) {
		...
		if (terminal_file)
		  fprintf (parser_file, "\
if ((rho [%ld] = p_q_t2pqt (adr2bound_values[%ld], adr2bound_values[%ld], SXT_%s)) == 0) %s;\n",
			   (long)(cur_rho_size+Tpq_stack_pos), ladr, ladr+1, sxstrget (t2Cid_ste [-symb]), exit_stmt);
		else
		  fprintf (parser_file, "\
if ((rho [%ld] = p_q_t2pqt (adr2bound_values[%ld], adr2bound_values[%ld], %ld)) == 0) %s;\n",
			   (long)(cur_rho_size+Tpq_stack_pos), ladr, ladr+1, -symb, exit_stmt);
	      }
	      else {
		counter = -symb - max_ste - 1;
		/* compteur */
...
fill_param_pos (SXINT clause)
{
  ...
  SXINT		*eq_ptr, *eq_ptr2;
  SXBOOLEAN	is_useful, not_found, has_local_etc, is_pos_call, arg_is_a_range, has_strlen, is_a_counter;
  ...
  is_useful = SXTRUE;
  is_static_phase = SXTRUE;
  clause_has_cut = has_predef_prdct = has_predef_prdct_call = SXFALSE;
  lhs_prdct = XxY_X (rcg_clauses, clause) & lhs_prdct_filter;
  lhs_bot2 = XH_X (rcg_predicates, lhs_prdct);
  lhs_top2 = XH_X (rcg_predicates, lhs_prdct+1);
  lhs_nt = XH_list_elem (rcg_predicates, lhs_bot2+1);
  ...
  for (rhs_prdct_pos = 1, cur = bot; cur < top; rhs_prdct_pos++, cur++) {
...
#endif /* 0 */
	    if (nt == CUT_ic)
	      clause_has_cut = SXTRUE;

	    has_predef_prdct_call = SXTRUE;
	    rhs_prdct_nyp_nb++;
    ...
    /* modif du 17 juin 2014: on integre les predicats speciaux 'standard' dans la foret, et du coup... */
    if (nt > 0 || nt == STRLEN_ic || nt == STREQLEN_ic || nt == STREQ_ic || nt == TRUE_ic) {
      /* index ds rho */
	...
	if ((nt == STRLEN_ic && cur2 == bot2+2) /* Le 1er arg de &StrLen est un entier */ ||
	  ...
	  arg_is_a_range = SXFALSE;
	}
	else {
	  arg_is_a_range = SXTRUE;
  ...
  for (cur = bot; cur < top; cur++) {
...
#endif /* 0 */
	     nt == STREQ_ic || nt == STREQLEN_ic ||
	     ((nt == FIRST_ic || nt == LAST_ic || nt == STRLEN_ic) && cur2 > bot2+2) ||
	...
	if (nt > 0 || (nt == STRLEN_ic && cur2 > bot2+2) || nt == STREQLEN_ic || nt == STREQ_ic) {
...
static void
gen_StrEq_spcl (SXINT prdct_pos, SXINT arg_pos, SXINT bot3, SXINT top3, char *exit_string, SXBOOLEAN is_pos_call)
{
    SXINT len, cur3, symb, tooth, i;

    len = top3-bot3;
    cur3 = bot3;
    is_source_checked = SXTRUE;
    ...
    case 1:
        ...
        if (is_pos_call) {
	  if (terminal_file)
            fprintf (parser_file, "if ((Tpq = p_q_t2pqt (olb[%ld], oub[%ld], SXT_%s)) == 0) %s;\n",
                     (long)arg_pos, (long)arg_pos, sxstrget (t2Cid_ste [-symb]), exit_string);
	  else
            fprintf (parser_file, "if ((Tpq = p_q_t2pqt (olb[%ld], oub[%ld], %ld)) == 0) %s;\n",
                     (long)arg_pos, (long)arg_pos, (long)-symb, exit_string);
	  ...
	  if (terminal_file)
            fprintf (parser_file, "if (p_q_t2pqt (olb[%ld], oub[%ld], SXT_%s) != 0) %s;\n",
                     (long)arg_pos, (long)arg_pos, sxstrget (t2Cid_ste [-symb]), exit_string);
	else
            fprintf (parser_file, "if (p_q_t2pqt (olb[%ld], oub[%ld], %ld) != 0) %s;\n",
                     (long)arg_pos, (long)arg_pos, (long)-symb, exit_string);
	}
        break;
    ...
    default:
	    ...
	    if (terminal_file)
	      fprintf (parser_file, "if (p_q_t2pqt (local_val+%ld, local_val+%ld=1, SXT_%s) %s= 0 %s\n",
		       (long)i, (long)i, sxstrget (t2Cid_ste [-symb]), is_pos_call ? "=" : "!", len ? (is_pos_call ? "||" : "&&") : "");
	    else
	      fprintf (parser_file, "if (p_q_t2pqt (local_val+%ld, local_val+%ld=1, %ld) %s= 0 %s\n",
		       (long)i, (long)i, -symb, is_pos_call ? "=" : "!", len ? (is_pos_call ? "||" : "&&") : "");
...
arg_evaluated (SXINT nt, SXBOOLEAN is_first_arg, SXINT rhs_param_pos, SXINT cur2)
{
    ...
    if (param > 0) {
        ...
        if (is_in_Tstar (bot3, top3) == 1) return SXTRUE;
...
static SXBOOLEAN  A_is_loop, A_is_cyclic;
...
rhs_processing (SXINT level, SXINT clause, char *exit_string)
{
  ...
  SXBOOLEAN     has_c, range1, range2, is_pos_call, known_counter;
  ...
  if (rhs_prdct_nyp_nb) {
	...
	if (nt < 0 && nt != CUT_ic) {
	  ...
	  fprintf (sxstderr, "! rhs_prdct_nyp_nb = %ld ; max_rhs_prdct+2 = %ld ; rhs_prdct_pos = %ld\n", rhs_prdct_nyp_nb, max_rhs_prdct+2, rhs_prdct_pos);
	  if (SXBA_bit_is_set (rhs_prdct_nyp, rhs_prdct_pos)
	    ...
	    if (nt == TRUE_ic) {
	      ...
	      if (arg_evaluated (nt, SXTRUE, rhs_param_pos, bot2+2)) {
		fprintf(sxstderr, "sortie 1\n");
		...
		SXBA_0_bit (rhs_prdct_nyp, rhs_prdct_pos);
	      ...
	      if (arg_evaluated (nt, SXTRUE, rhs_param_pos, bot2+2) &&
		  arg_evaluated (nt, SXFALSE, rhs_param_pos+1, bot2+3)) {
		fprintf(sxstderr, "sortie 2\n");
		...
		if (nt == STRLEN_ic) {
		  ...
		  if (is_pos_call && (len = XH_list_elem (rcg_parameters, bot3)) != 0 /* lgth != 0 */) {
		    ...
		    fprintf (parser_file,"{\n");
		    fprintf (parser_file, "SXINT local_range = p_q2range(olb[%ld],oub[%ld]);\n", pos, pos);
		    //		    fprintf (parser_file, "if (_StrLen (local_range, %ld) < 0) %s;\n", len, exit_string); /* meme si len == 0 */
		    fprintf (parser_file, "X_set (&rhoA_hd, (((local_range<<lognt) + %ld) << Aij_type_offset) + 3 /* binaire 11, soit 1 pour dire que c'est un prédicat prédifini et 2 pour dire que c'est strlen, et que la zone réservée habituellement pour l'identifiant de prédicat stocke la longueur */, &dum_Aij);\n",len);
		    fprintf (parser_file, "rho [%ld] = MAKE_Aij_pid (-dum_Aij, 0);\n", rhs_prdct_pos+1);
		    fprintf (parser_file,"}\n");
		  }
		...
		if ((!range1 && (tstar1 == 0)) || (!range2 && (tstar2 == 0))) {
		  ...
		  case STREQ_ic:
		    ...
		    if (!(range1 || tstar1)) {
		      fputs ("parg1 = StrConcat (SXTRUE, rcg_arg1, Xlb, Xub);\n", parser_file);
		    }

		    if (!(range2 || tstar2)) {
		      fputs ("parg2 = StrConcat (SXFALSE, rcg_arg2, Xlb, Xub);\n", parser_file);
		    }
		  ...
		  if (is_pos_call) {
			...
			if ((param = XH_list_elem (rcg_predicates, bot2+2))) {
			  bot3 = XH_X (rcg_parameters, param);
			  top3 = XH_X (rcg_parameters, param+1);
			  gen_StrEq_spcl (rhs_prdct_pos, pos+1, bot3, top3, exit_string, SXTRUE);
			}
		      }
		      else {
#if 0
			...
			if ((param = XH_list_elem (rcg_predicates, bot2+2))) {
			  bot3 = XH_X (rcg_parameters, param);
			  top3 = XH_X (rcg_parameters, param+1);
			  gen_StrEq_spcl (rhs_prdct_pos, pos+1, bot3, top3, exit_string, SXFALSE);	
			}
			else
	...
	if (SXBA_bit_is_set (rhs_prdct_nyp, rhs_prdct_pos)) {
	  ...
	  for (pos = rhs_param_pos, cur2 = bot2+2; cur2 < top2; pos++, Ak++, cur2++) {
		...
		known_counter = SXTRUE;
	      }

	      if (!known_counter && SXBA_bit_is_set (pos_is_a_range, pos)) {
		lmadr = rhs_pos2lmadr [pos];
		rmadr = rhs_pos2rmadr [pos];
...
if ((rho [%ld] = X_is_set (&rhoA_hd, ((local_val<<lognt)+%ld)<<Aij_type_offset)) == 0) %s;\n\
#endif /* is_earley_guide */\n", (long)pos, (long)pos, (long)tooth+1, (long)nt, exit_string);
		  }
...
, &(tc2rho[%ld])\n\
	      ...
	      /* on est sur un prédicat prédéfini */
	      pos = rhs_param_pos-lhs_arity;
	      tooth = rhs_prdct_pos2tooth [rhs_prdct_pos];
		...
		break;
	      default:
#if EBUG
		sxtrap(ME,"The handling of this predefined predicate still needs to be developed");
#endif
		break;		
	      }
	    }
...
gen_first_test (SXINT clause, SXINT rhs_pos, char *exit_stmt)
{
  ...
  SXBOOLEAN	is_lcur_tok, is_oub, ff, fe, fv;
...
gen_last_test (SXINT clause, SXINT rhs_pos, char *exit_stmt)
{
  ...
  char	kind;
  char	cur_tok_string [32];
  SXBOOLEAN	is_rcur_tok, is_olb, lf, le, lv;
...
static SXBOOLEAN
lhs_arg_processing (SXINT clause, SXINT pos, char *exit_stmt, SXBOOLEAN first_call)
{
  SXINT		  adr, ladr, radr, madr, symb, atom, rhs_pos, cur_rhs_pos, Index, rmadr, lmadr, nt;
  VARSTR	  lvstr, rvstr;
#if 0
  SXINT           Ak;
  SXBOOLEAN       has_counter, has_range;
#endif
  char	          string [16];
  ...
  if (ladr < radr) {
    /* Normalement, si ladr == radr => first_call == SXTRUE && arg == \varepsilon */
	...
	if (symb < 0) {
	  if (symb >= -max_ste) {
	    ...
	    fprintf (parser_file, "\
if (pos2dag_kind [%ld] & SDAG_KIND) {\n\
", pos);
	    fputs ("/* on regarde si la pile des transitions sortantes sur le terminal demande' est vide ou non */\n", parser_file);
	    if (terminal_file)
	      fprintf (parser_file, "\
if ((GET_QSTACK (%s, SXT_%s)) == NULL) %s;\n",
		       varstr_tostr (lvstr), sxstrget (t2Cid_ste [-symb]), exit_stmt);
	    else
	      fprintf (parser_file, "\
if ((GET_QSTACK (%s, %ld)) == NULL) %s;\n",
		       varstr_tostr (lvstr), (long)-symb, exit_stmt);
	    fputs ("}\n", parser_file);

	    if (terminal_file)
	      fprintf (parser_file, "\
if ((rho [%ld] = p_q_t2pqt (%s, %s+1, SXT_%s)) == 0 /* test inutile dans le cas SDAG_KIND, mais tant pis */) %s;\n",
		       (long)(max_son_nb+2+1+adr2tpos[ladr]), varstr_tostr (lvstr), varstr_tostr (lvstr), sxstrget (t2Cid_ste [-symb]), exit_stmt);
	    else
	      fprintf (parser_file, "\
if ((rho [%ld]= p_q_t2pqt (%s, %s+1, %ld)) == 0 /* test inutile dans le cas SDAG_KIND, mais tant pis */) %s;\n",
		       (long)(max_son_nb+2+1+adr2tpos[ladr]), varstr_tostr (lvstr), varstr_tostr (lvstr), (long)-symb, exit_stmt);
	...
	if (symb < 0) {
	  ...
	  if (symb >= -max_ste) {
	    ...
	    if (SXBA_bit_is_reset_set (tmadr_set, madr)) {
	      ...
	      if (terminal_file)
		fprintf (parser_file, "\
if ((rho [%ld] = p_q_t2pqt (%s, iub[%ld], SXT_%s)) == 0) %s;\n",
		       (long)(max_son_nb+2+1+adr2tpos[ladr]), varstr_tostr (lvstr), pos, sxstrget (t2Cid_ste [-symb]), exit_stmt);
	      else
		fprintf (parser_file, "\
if ((rho [%ld] = p_q_t2pqt (%s, iub[%ld], %ld)) == 0) %s;\n",
		       (long)(max_son_nb+2+1+adr2tpos[ladr]), varstr_tostr (lvstr), pos, (long)-symb, exit_stmt);
	...
	if (SXBA_bit_is_reset_set (already_seen_madr_set, madr)) {
	  if (SXBA_bit_is_set (is_madr_an_ob, madr)) {
	    ...
	    if (rhs_pos > 0 && (Ak = rhs_pos2Ak [rhs_pos]) > 0 && SXBA_bit_is_set (counter_Ak_set, Ak)) {
	      ...
	      BOOLEAN local_generation;

	      local_generation = SXFALSE;
 ...
 || (rho [%ld] = p_q_t2pqt (q-1, q, SXT_%s) == 0))\n  %s;\n",
			     varstr_tostr (rvstr), pos, (long)(max_son_nb+2+1+adr2tpos[radr]), sxstrget (t2Cid_ste [-symb]), exit_stmt);
		else
		  fprintf (parser_file, "if ((q=%s)==ilb[%ld]/* empty range */\
 || (rho [%ld] = p_q_t2pqt (q-1, q, %ld) == 0))\n  %s;\n",
			     varstr_tostr (rvstr), pos, (long)(max_son_nb+2+1+adr2tpos[radr]), (long)-symb, exit_stmt);
...
process_single_atom (char *exit_stmt)
{
  ...
  SXINT		*eq_ptr;
  char	*wstr, string [32];
  SXBOOLEAN	is_assign, Asize_is_useful;
...
gen_level (SXINT level, SXINT clause, char *exit_stmt)
{
  ...
  while ((pos = sxba_scan (lhs_arg_tbp_set, pos)) >= 0) {
    lhs_arg_processing (clause, pos, exit_stmt, SXFALSE);
  }
...
gen_guided_bounds (SXINT clause)
{
    ...
    SXBOOLEAN 	gen_guided_bounds_is_left_linear, is_i, is_o, is_pos2bound, is_next_pos2bound;
  ...
  for (xeq = 0; xeq < lhs_equation; xeq++) {
    if (xeq < lhs_arity)
	 ...
	 sont inconditionnelles et sont inutiles si un test ulterieur echoue! */
      lhs_arg_processing (clause, xeq, exit_stmt, SXTRUE);
...
\t(*) SYNTAX is a trademark of INRIA.\n\
   ******************************************************************** */\n\n",
	     PRGENTNAME(SXTRUE), ctime (&date_time));
...
gen_local_variables (void)
{
  ...
  SXBOOLEAN	is_first, done, has_ext_prdct_call;
  char        *str;
...
/* 0 */ \"\",\n", parser_file);
...

  {
    ...
    sxword_to_c (&t_names, parser_file, "t_names", SXTRUE);
  ...
  if (is_a_complete_terminal_grammar) {
    ...
    word_tree_alloc (&dico_word_tree, "dico_word_tree", max_t+1, 16
		     , 1 /* Xforeach */, 0 /* Yforeach */, SXTRUE /* from_left_to_right */
		     , SXTRUE /* with_id */, dico_word_tree_oflw,
#if EBUG
		     stdout
#else
		     NULL
    ...
    */
    ...
    sxdfa_comb2c (&dico, stdout, "t_names_dico" /* dico_name */, SXTRUE /* is_static */);
  ...
  if (is_a_complete_terminal_grammar) {
    ...
    sxbm3_to_c (parser_file, prdct2t_set, max_nt+1, "prdct2t_set", "", SXTRUE /* is_static */);
	...
	if ((clause_id = t2clause [t]) < 0) {
	  ...
	  fprintf (parser_file, "/* %ld */ %ld", (long)top, (long)nb);

	  for (x = -clause_id; x != 0; x = clause_stack_lnk [x]) {
	    top++;
	    fprintf (parser_file, ", %ld", (long)clause_stack [x]);
	  }
	...
	for (cur = bot; cur < top; cur++) {
	  ...
	  if (A > 0 && SXBA_bit_is_set (ext_decl_set, A)) {
		...
		fputs (", ", parser_file);
	      ...
	      fprintf (parser_file, "is_useful_%ld = SXTRUE", (long)clause);
	    }
	  }
	}
      }
...
static void
fill_Lex_t2clause_set (SXBA **L, SXBA **t2, SXBA **c2, SXBOOLEAN with_First_Last, SXINT t_nb_lim)
{    
  ...
  SXBOOLEAN       has_first_last;
#if 0
  SXBOOLEAN       is_loop_clause_set_empty;
#endif /* 0 */
...
gen_lex (void)
{
  ...
  SXBOOLEAN is_Lex_empty = SXTRUE;

  fill_Lex_t2clause_set (&Lex, &t2clause_set, &clause2rhs_nt_set, SXTRUE, (SXINT)(lfsa_file ? 0 : 1));
  ...
  if (max_t) {
    ...
    sxbm3_to_c (parser_file, Lex, last_clause+1, "Lex", "", SXTRUE /* is_static */);
  ...
  fputs ("\n#if is_lex3\n", parser_file);
  sxbm3_to_c (parser_file, clause2rhs_nt_set, last_clause+1, "clause2rhs_nt_set", "", SXTRUE /* is_static */);
  fputs ("\n#endif /* is_lex3 */\n", parser_file);
...

  {
    ...
    for (A = 1; A <= max_nt; A++) {
      ...
      sxba_to_c (clause_id_set, parser_file, "nt2clause_id_set_", str, SXTRUE /* static */);
#endif /* 0 */
    }
...
gen_clause_id (void)
{
  ...
  /* ATTENTION, Le 29/5/2002, on fait disparaitre clause2Aclause */
    ...
    sxba_to_c (main_clause_set, parser_file, "main_clause_set", "", SXTRUE /* static */);
...
gen_first_last_pp (void)
{
	...
	for (rhs_prdct_pos = 1, cur = bot; cur < top; rhs_prdct_pos++, cur++) {
		...
		if (nt == FIRST_ic)
		    sxba_to_c (t_set, parser_file, "first_", istr, SXTRUE /* static */);
		else
		    sxba_to_c (t_set, parser_file, "last_", istr, SXTRUE /* static */);
...
#endif /* is_dynamic_lexicalization */\n", (long)(A_k2Ak(max_nt, nt2arity [max_nt]-1)+1));
}
...
gen_nl_decl (void)
{
    ...
    fprintf (parser_file, "#if !is_a_linear_module\n\
...
static SXBA_CST_ALLOC (basic_lexicalization_clause_set, %ld);\n",
	     (long)(A_k2Ak(max_nt, nt2arity [max_nt]-1)+1), (long)(last_clause+1));

    sxba_to_c (non_right_linear_clause_set, parser_file, "non_right_linear_clause_set", "", SXTRUE /* static */);


    fputs ("#endif /* !is_a_linear_module */\n\n", parser_file);

    fputs ("static SXINT A2A0 [ntmax+1] = {0,\n", parser_file);
    
    for (A = 1; A <= max_nt; A++) {
	fprintf (parser_file, "/* %s = %ld */ ", sxstrget (nt2ste [A]), (long) A);
...
{\n", parser_file);
 ...
 if (!is_a_complete_terminal_grammar)
  ...
  fputs ("  memo_oflw_second_part (old_size, new_size);\n", parser_file);
...
gen_memo_fun (void)
{
	...
	fputs ("#if is_cyclic\n", parser_file);
	fputs ("static SXBOOLEAN is_glbl_cycle;\n", parser_file);
...
SXINT l;\n\
#if EBUG\n\
SXBOOLEAN arity_check = SXTRUE;\n\
#endif /* EBUG */\n", PRGENTNAME(SXFALSE));

    fputs ("SS_stack = SS_alloc (50);\n", parser_file);
...
lexicalization (&G);\n\
	...
	while ((A = sxba_scan (ext_decl_set, A)) > 0) {
	    ...
	    /* fprintf (parser_file, "is_useful_%ld = SXFALSE;\n", (long)clause); */
	    fprintf (parser_file, "arity_check = SXFALSE;\n\
...
rhoA_hd.user_oflw = (ifun) memo_oflw /* prudence */;\n\
	...
	case 2:
	    /* XxY */
	    fputs ("arity_eq_2_is_used = SXTRUE, G.prange_2 = &arity_eq_2;\n", parser_file);
#if 0
	...
	case 3:
	    /* XxYxZ */
	    fputs ("arity_eq_3_is_used = SXTRUE, G.prange_3 = &arity_eq_3;\n", parser_file);
#if 0
	...
	case 4:
	    /* XH */
	    fputs ("arity_geq_4_is_used = SXTRUE, G.prange_n = &arity_geq_4;\n", parser_file);
#if 0
...
(*G.semact.first_pass_final) (&G, 1, 0); */\n", parser_file);
    ...
    fputs ("SS_free (SS_stack), SS_stack = NULL;\n", parser_file);
...
gen_macros (void)
{
	...
	fprintf (parser_file, "#define lognt\t%ld\n", (long)sxlast_bit (MAX(MAX(new_max_nt,max_len_in_strlen),-CUT_ic)));
	fprintf (parser_file, "#define Aij_type_offset 2\n");
	fputs ("#else\n", parser_file);
    }
    
    fprintf (parser_file, "#define lognt\t%ld\n", (long)sxlast_bit (MAX(MAX(max_nt,max_len_in_strlen),-CUT_ic)));
    fprintf (parser_file, "#define Aij_type_offset 2\n");
	...
	fprintf (parser_file, "#define glognt\t%ld\n", (long)guide_lognt); /* TODO: prendre en compte CUT_ic dans guide_lognt ??? */
...
gen_Aij (void)
{
    ...
    fputs ("static char	*nt2str[] = {\n\
\"Cut\", \"Lex\", \"True\", \"Last\", \"First\", \"False\", \"StrLen\", \"StrEqLen\", \"StrEq\", \"\"",
	   parser_file);
...
0 /* CUT */, 1 /* LEX */, 1 /* TRUE */, 2 /* LAST */, 2 /* FIRST */, 0 /* FALSE */, 1 /* STRLEN */, 2 /* STREQLEN */, 2 /* STREQ */, 0 /* ORIGIN */,",
	     parser_file);
...
gen_loops (void)
{
    ...
    if (is_loop_grammar) {
      /* Le 26/11/2002 loop_clause_set est genere a part et enleve' de t2clause_set[tmax+1] */
        sxba_to_c (loop_clause_set, parser_file, "loop_clause_set", "", SXTRUE /* static */);
...
gen_instantiated_prdct2struct (void)
{
    ...
    if (profile_nb > 1)
	fputs ("static struct Aij_struct Aij_struct = {(char*)NULL, 0, 0, 0, lower_bounds, upper_bounds};\n", parser_file);
    else
	fprintf (parser_file,
		 "static struct Aij_struct Aij_struct = {(char*)NULL, 0, %ld, 0, lower_bounds, upper_bounds};\n", (long)profile);
    ...
    fputs ("if (couple & 1) { /* prédicat prédéfini */\n", parser_file);
    fputs ("  if (couple & 2) { /* le prédicat prédéfini est strlen */\n", parser_file);
    fputs ("    Aij_struct.A = STRLEN_ic;\n", parser_file);
    fputs ("    Aij_struct.len = (couple>>Aij_type_offset)&((1<<lognt)-1);\n", parser_file);
    fputs ("  } else { /* le prédicat prédéfini n'est pas strlen */\n", parser_file);
    fputs ("    Aij_struct.A = -((couple>>Aij_type_offset)&((1<<lognt)-1));\n", parser_file);
    fputs ("  }\n", parser_file);
    fputs ("} else { /* prédicat standard */\n", parser_file);
    fputs ("#if EBUG\n", parser_file);
    fputs ("  if (couple & 2)\n", parser_file);
    fputs ("    sxtrap (ME, \"instantiated_prdct2struct (Invalid Aij: first bit is 0, i.e., standard predicate, and second bit is 1)\");\n", parser_file);
    fputs ("#endif /* EBUG */\n", parser_file);
    fputs ("  Aij_struct.A = ((couple>>Aij_type_offset)&((1<<lognt)-1));\n", parser_file);
    fputs ("}\n", parser_file);
    fprintf (parser_file, "    Aij_struct.Astr = (nt2str+%ld) [Aij_struct.A];\n", -CUT_ic);
    fputs ("rho = couple >> (lognt + Aij_type_offset);\n", parser_file);
...
gen_G (void)
{
    ...
    fprintf (parser_file, "\"%s.%s\",\n", PRGENTNAME(SXTRUE), suffixname);/* char *name; */
...
*/
    fputs ("{sxvoid /* first_pass_init */, sxvoid /* second_pass_init */, sxvoid /* first_pass_final */, sxvoid /* second_pass_final */, sxvoid /* first_pass_loop */, sxvoid /* second_pass_loop */, sxvoid /* oflw */, sxvoid /* nl_Ai_processing */, sxivoid /* first_pass */, sxivoid /* second_pass */, sxivoid /* last_pass */, sxbvoid /* lexicalizer */, NULL /* SXBOOLEAN   (*dynamic_lexicalizer)(SXINT, SXBA) */},\n", parser_file); /* semact */

    fputs ("{sxvoid /* first_pass_init */, sxvoid /* second_pass_init */, sxvoid /* first_pass_final */, sxvoid /* second_pass_final */, sxvoid /* first_pass_loop */, sxvoid /* second_pass_loop */, sxvoid /* oflw */, sxvoid /* nl_Ai_processing */, sxivoid /* first_pass */, sxivoid /* second_pass */, sxivoid /* last_pass */, sxbvoid /* lexicalizer */, NULL /* SXBOOLEAN   (*dynamic_lexicalizer)(SXINT, SXBA) */},\n", parser_file); /* parse */

    fputs ("NULL,\n", parser_file); /* pointeur vers la fonction Aij2struct */
      
    fputs ("clause2nl_stmt_stack,\n\
nl_stmt_stack,\n\
NULL /* Ai2inside_t_set */,\n", parser_file);

    fputs ("#if !is_a_linear_module\n\
non_right_linear_clause_set,\n\
nl_Ai_set,\n\
#else\n\
NULL /* non_right_linear_clause_set */,\n\
NULL /* nl_Ai_set */,\n\
#endif /* !is_a_linear_module */\n", parser_file);
...
/* Actually unused */\n", parser_file);
    ...
    fprintf (parser_file, "%s /* is_combinatorial */,\n", is_combinatorial_grammar ? "SXTRUE" : "SXFALSE"); 
    fprintf (parser_file, "%s /* is_linear */,\n", is_left_linear_grammar && is_right_linear_grammar ? "SXTRUE" : "SXFALSE"); 
    fprintf (parser_file, "%s /* is_overlapping */,\n", is_overlapping_grammar ? "SXTRUE" : "SXFALSE"); 
    fprintf (parser_file, "%s /* is_top_down_erasing */,\n", is_top_down_erasing_grammar ? "SXTRUE" : "SXFALSE"); 
    fprintf (parser_file, "%s /* is_bottom_up_erasing */,\n", is_bottom_up_erasing_grammar ? "SXTRUE" : "SXFALSE"); 
...
SXFALSE,\n\
    ...
    fputs ("SXFALSE /* is_GET_QSTACK */,\n", parser_file);
    fputs ("SXFALSE /* is_GET_PSTACK */,\n", parser_file);
...
gen_fun_decl (void)
{
  ...
  if (A > 0) {
    fputs ("extern SXBOOLEAN\t", parser_file);
  ...
  if (A > 0) {
    fputs ("static SXBOOLEAN\t", parser_file);
...
nt2cst (SXBA nt2cst_A_clause_set)
{
	...
	if (SXBA_bit_is_set (clause2non_range_arg, clause)) Anon_range = SXTRUE;
...
gen_function_local_variables (SXINT A, 
			      ...
			      SXBOOLEAN gen_function_local_variables_A_is_cyclic)
{
...
SXINT range;\n\
    ...
    fprintf (parser_file, "SXINT\t\trho[%ld];\n", (long)2+Arhs_prdct_nb+1+Alhs_t_nb);
    ...
    fprintf (parser_file, "char\t\tpos2dag_kind[%ld];\n", (long)A_arity);
	...
	fputs ("SXBOOLEAN\tis_local_cycle, is_cyclic_ip;\n", parser_file);
...
gen_sem_act (void)
{
    ...
    fputs ("\nstatic SXBOOLEAN\n", parser_file);
    fprintf (parser_file, "_%s_semact (SXINT *rho, SXINT son_nb, SXINT sons[], SXBOOLEAN is_first)\n", PRGENTNAME(SXFALSE));
    fputs ("{\n", parser_file);
    fputs ("bfun call;\n", parser_file);
    fputs ("if ((call = semact [rho[0]]) == NULL) return SXTRUE;\n", parser_file);
...
output_t2clause_set (SXBA_INDEX	*repr)
{
  ...
  sxbm3core_to_c (repr, parser_file, max_t+1, "t2clause_set", "", SXTRUE /* is_static */);
...
\n\
  ...
  Aij_struct.A = ((Aij>>Aij_type_offset)&((1<<lognt)-1));\n",parser_file);
  fprintf (parser_file, "    Aij_struct.Astr = (nt2str+%ld) [Aij_struct.A];\n", -CUT_ic);
  fputs ("\
#if EBUG\n\
  if (Aij & 3)\n\
    sxtrap (ME, \"instantiated_prdct2struct (predefined predicates are not possible in 'terminal grammars')\");\n", parser_file);
  fprintf (parser_file, "  if (/* Aij_struct.arity = */ (nt2arity+%ld) [Aij_struct.A] != 1)", -CUT_ic);
  fputs ("\
    sxtrap (ME, \"instantiated_prdct2struct (only predicates with arity 1 are possible in 'terminal grammars')\");\n\
#endif /* EBUG */\n\
  Aij_struct.arity = 1;\n\
  pq = Aij >> (lognt + Aij_type_offset);\n\
...
#if EBUG
  SXINT local_A_arity;
  SXBOOLEAN local_A_is_external, local_A_is_internal, local_A_is_loop, local_A_is_cyclic;
#endif /* EBUG */
...
#if EBUG
	local_A_is_external = SXBA_bit_is_set (is_nt_external, A);
	local_A_is_internal = SXBA_bit_is_set (is_nt_internal, A);
	local_A_arity = nt2arity [A];
	local_A_is_loop = SXBA_bit_is_set (loop_nt_set, A);
	local_A_is_cyclic = SXBA_bit_is_set (cyclic_nt_set, A);

	if (!local_A_is_external || local_A_is_internal || local_A_is_loop || local_A_is_cyclic || local_A_arity != 1 || is_loop_grammar)
...
{\n\
  return t_clause_processing (&G, %ld, rho0, ilb [0], iub [0]);\n\
		 ...
		 , /* " bidon pour le beautifier emacs */Astr, (long)A, Astr);
      }
    }
  }
}
...
}\n", /* " */PRGENTNAME(SXFALSE), PRGENTNAME(SXFALSE), (long)(A_k2Ak(max_nt, nt2arity [max_nt]-1)+1));
}
...
fputs ("check_if_too_large (lognt);\n", parser_file);
...
if (is_GET_QSTACK)
     ...
     fputs ("G.is_GET_QSTACK = SXTRUE;\n", parser_file);

     if (is_GET_PSTACK)
     /* Le parseur utilise GET_PSTACK */
     fputs ("G.is_GET_PSTACK = SXTRUE;\n", parser_file);
...
fputs ("else if (is_parse_tree_number)\n", parser_file);
fputs ("#if is_parse_forest\n", parser_file);
fputs ("  rcg_spf_tree_count_begins ();\n", parser_file);
fputs ("#else /* !is_parse_forest */\n", parser_file);
fputs ("  rcg_spf_tree_count_in_first_pass_begins ();\n", parser_file);
fputs ("#endif /* is_parse_forest */\n", parser_file);
...
if (repr) {
  if (max_t && Lex) {
	...
	while ((t = sxba_scan (line, t)) > 0) {
	  if (terminal_file)
	    ...
	    fprintf (parser_file, "SXBA_1_bit (Lex_%ld, SXT_%ld);\n", clause, (long)t);
	}
      }
    }
  ...
  if (max_t) {
	...
	if (terminal_file)
	  fprintf (parser_file, "t2clause_set [SXT_%s] = t2clause_set_%ld;\n", sxstrget (t2Cid_ste [t]), (long)(repr [t]));
	else
	  fprintf (parser_file, "t2clause_set [%ld] = t2clause_set_%ld;\n", (long)t, (long)(repr [t]));
      }
    }
...
{\n\
  sxba_or (inside_t_set, Ai2inside_t_set [%ld]);\n\
      ...
      sxstrget (nt2ste [A]),
      (long)(A)); /* Pour une grammaire terminale on a A_k2Ak (A, 0) == A */
      }
    }
  }
	...
	if (param > 0)  {
		...
		if (symb < 0) {
          ...
          SXBA_1_bit (Ai_inside_t_set, inside_t);\n", sxstrget (t2Cid_ste [-symb]));
			...
			fprintf (parser_file, "          inside_t = X_is_set (&(idag.global_t2inside_t), %ld);\n\
          SXBA_1_bit (Ai_inside_t_set, inside_t);\n", (long)(-symb));
		    }
		}
		else {
				...
				for (cur2 = bot2+2; cur2 < top2; cur2++) {
					...
					for (cur4 = bot4; cur4 < top4; cur4++) {
						...
						fprintf (parser_file, "        _%s_dynamic_lexicalization (%ld, Ai_inside_t_set);\n",
							 sxstrget (nt2ste [nt]), (long) (cur2-(bot2+2)));
					    }	
					}
				    }
				}
...
", /* " */SXBA_bit_is_set (is_nt_external, A) ? "" : "static ", sxstrget (nt2ste [A]), (long)A, sxstrget (nt2ste [A]), (long)A, (long)A);
	...
	for (cur3 = bot3; cur3 < top3; cur3++) {
	  ...
	  if (symb < 0 && symb >= -max_ste) {
	    ...
	    if (terminal_file)
	      ...
	      fprintf (parser_file, "%sSXBA_bit_is_set (idag.source_set, %ld)",
		       is_first_t ? "  if (" : "\n      && ", (long)(-symb));
	    is_first_t = SXFALSE;
	  }
	}
...
", /* " */(long)clause, (long)A, (long)clause2clause_id [clause], varstr_tostr (wvstr));
  }
...
#endif /* EBUG */\n\
  return ret_val;\n\
} /* _terminal_%s */\n", /* " */(long)A, sxstrget (nt2ste [A]), sxstrget (nt2ste [A]));
  }
}
...
gen_parser (void)
{
  ...
  SXINT		d, degree;
  SXBOOLEAN	A_is_internal, A_is_external;
  char	        *Astr;
...
{\n",
	...
	if (A_clause_nb == 0) {
	  ...
	  if (!Lex_file) {
	    ...
	    if (A_arity)
	      fprintf (parser_file, "X_set (&rhoA_hd, ((local_val<<lognt)+%ld)<<Aij_type_offset, &Aij);\n", (long)A);
	    else
	      fprintf (parser_file, "X_set (&rhoA_hd, (%ld)<<Aij_type_offset, &Aij));\n", (long)A);
...
;\n", A2max_clause_id [A]+1);
	  ...
	  if (A == 1 /* Le predicat courant est l'axiome de la rcg si c'est la principale il doit avoir pour code 1
	    ...
	    fputs ("if (G.pid == 0) MAKE_Aij_pid (Aij, G.pid) /* AXIOME! L'affectation se fera + tard, normalement */;\n", parser_file);
	  }
...
#if EBUG
		  printf ("(%s)out_guide(%ld): S1 (%ld..%ld) SXFALSE\n", G.name, call_level, ilb[0], iub[0], );
		  call_level--;
#endif /* EBUG */
		  return -1;
		}
...
iprdct_found_in_guide++;\n\
	  ...
	  if (A_is_cyclic) {
	    ...
	    fputs ("if (!((is_glbl_cycle || is_cyclic_ip) ? ((is_glbl_cycle) ? ((is_cyclic_ip) ? (SXBA_bit_is_reset_set (memoC [3], Aij)) : (SXBA_bit_is_reset_set (memoC [2], Aij))) : (SXBA_bit_is_reset_set (memoC [3], Aij))) : SXTRUE)) {\n", parser_file);
	    ...
	    fputs (", SXBA_bit_is_set (memoN1, Aij) ? \"SXTRUE\" : \"SXFALSE\");\ncall_level--;\n#endif /* EBUG */\n", parser_file);
...
return -1;\n", parser_file);
    ...
    printf (\"(%%s)out_dynamic_lexicalization(%%ld): %s (%%ld..%%ld", sxstrget (nt2ste [A]));
	  ...
	  for (i = 1; i < A_arity; i++) {
             fprintf (parser_file, ", ilb[%ld], iub[%ld]", (long)i, (long)i);
	  }
...
if (SXBA_bit_is_set (nt2clause_id_set_%ld, 1))\n\
		...
		fprintf (parser_file, "/* rho is structured as follows:\n\
  - rho [0] = clause number in the grammar\n\
  - rho [1] = Aij (pid-aware) of the LHS\n\
  - rho [2] ... rhs [i] = the Aij's in the RHS\n\
  - rho [i+1] = number of terminal symbols in the LHS\n\
  - rho [i+2] ... rho [j] = not affected ; will contain, if it exists, the pqt's of the transitions in the input DAG over terminal symbols of the LHS\n\
*/\n");

		fprintf (parser_file, "rho [0] = %ld;\n", (long)clause);
		fprintf (parser_file, "rho [%ld] = %ld /* #terminals in clause #%ld */;\n", (long) cur_rho_size, (long)lhs_t_nb, (long)clause);
...
if (is_useful_%ld)\n\
		...
		for (k = 0; k < A_arity; k++)
		  fprintf (parser_file, "SXINT range_%ld = p_q2range (ilb[%ld], iub [%ld]);\n", k, k, k);


		fputs ("SXBOOLEAN cannot_optimize_for_consecutive_strings_or_sdags =\n\
input_is_a_dag\n\
&& !(\n", parser_file);
		for (k = 0; k < A_arity; k++) {
		  if (k > 0)
		    fputs ("&&\n", parser_file);
		  /* IS_CONSECUTIVE_SEQUENCE ne peut être positionné à 1 que dans les cas STRING_KIND et SDAG_KIND */
		  fprintf (parser_file, "((pos2dag_kind [%ld] = range2dag_kind (range_%ld)) & IS_CONSECUTIVE_SEQUENCE)\n", k, k);
		}
		fputs (");\n", parser_file);

		fputs ("if (cannot_optimize_for_consecutive_strings_or_sdags) {\n", parser_file);
		fprintf (parser_file, "#if EBUG\nprintf (\"(%%s)%s [DAG mode]\\n\", G.name);\n#endif\n",
			 varstr_tostr (wvstr));

		gen_dag_parser (clause);

		fputs ("} /* cannot_optimize_for_consecutive_strings_or_sdags */\n", parser_file);
		fputs ("else {\n", parser_file);
		fprintf (parser_file, "#if EBUG\nprintf (\"(%%s)%s [string mode]\\n\", G.name);\n#endif\n",
			 varstr_tostr (wvstr));
  ...
  if (has_semantics) {
	...
	if ((main_A = nt2identical [A]) != A) {
	  ...
	  if (A_clause_nb) {
	    ...
	    fputs ("SXBOOLEAN\tret_val;\n", parser_file);
	    fputs ("struct spf_node\t**prev_icp;\n", parser_file);

	    if (A_is_cyclic) {
	      fputs ("#if is_cyclic\n", parser_file);
	      fputs ("SXBOOLEAN\tis_cyclic_ip, local_ret_val;\n", parser_file);
	    ...
	    if (A_is_cyclic)
	      fputs ("#if is_cyclic\n\
&& (SXBA_bit_is_set (memoC [1], rho0) ? (ret_val = SXBA_bit_is_set (memoN0, rho0), SXFALSE) : SXTRUE)\n\
#endif\n", parser_file);
	    ...
	    if (A_is_cyclic)
	      fputs ("\n#if is_cyclic\n\
is_cyclic_ip = SXBA_bit_is_reset_set (memoC [0], rho0) ? SXFALSE : (SXBA_1_bit (memoC [1], rho0), SXTRUE);\n\
\n\
...
static SXBOOLEAN is_glbl_cycle; /* memorise l'etat courant (vis-a-vis des cycles) du ip A\rho courant */
...
if (!((is_glbl_cycle || is_cyclic_ip) ? ((is_glbl_cycle) ? ((is_cyclic_ip) ? (SXBA_bit_is_reset_set (memoC [3], Aij)) : (SXBA_bit_is_reset_set (memoC [2], Aij))) : (SXBA_bit_is_reset_set (memoC [3], Aij))) : SXTRUE)) return SXBA_bit_is_set (memoN1, Aij) ? 1 : -1;
...
for (/* each A_clause */) {
  ...
  case A_clause:
	...
	/* SXBA_bit_is_set (memoP [Aij], %ld) == SXFALSE */
        (*G.semact.first_pass) ();
      }
      /* ... */
    } while (0);
...
if (ret_val > 0)
  ...
  if (!(ret_val = SXBA_bit_is_set (memoN1, rho0))
	...
	} while (SXFALSE); /* Pour les continue ... */
