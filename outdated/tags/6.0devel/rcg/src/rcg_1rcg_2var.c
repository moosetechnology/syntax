...
char WHAT_RCG1RCG2VAR[] = "@(#)SYNTAX - $Id: rcg_1rcg_2var.c 2227 2018-09-26 16:22:59Z garavel $" WHAT_DEBUG;

extern SXINT is_arg_in_Tstar (SXINT param);

/* commun a -lbnf et -lfsa */
static SXINT *nt2lhs_nb, *nt2rhs_nb; /* Cas is_print == SXFALSE */
...
static SXBOOLEAN	to_be_memorized;
...
out_var_text (SXINT var)
{
	...
	fprintf (out_file, "X%ld", (long)var-1);
    }
}
...
less_nt (SXINT nt1, SXINT nt2)
{
	...
	if (nt2 < 0) {
	    ...
	    return nt1 == STREQ_ic ? SXTRUE : SXFALSE;
	}

	return SXFALSE;
    }

    return (nt2 < 0) ? SXTRUE : (nt1 < nt2);
}
...
one_rcg (void)
{
  ...
  SXBOOLEAN	rhs_ok, comment_clause, pushed, is_arg1, is_arg2, has_t;
  ...
  for (clause = init_clause_nb; clause <= last_clause; clause++) {
	...
	if (rhs_ok) {
...
#endif
	  fprintf (out_file, "_%ld(", (long)guide_lhs_nt);
	  ...
	  while (xbot < xtop) {
...
#endif
		fprintf (out_file, "_%ld", (long)guide_rhs_nt);
	      }
		...
		if (rhs_nt == STRLEN_ic)
		  fprintf (out_file, "%ld", (long)rhs_param);
		else {
	  ...
	  while (xbot < xtop) {
	    ...
	    if (Ak > 0 && (len = Ak2len [Ak]) != -1 && !SXBA_bit_is_set (strlen_set, rhs_param)) {
	      ...
	      SXBA_1_bit (strlen_set, rhs_param);
	      fprintf (out_file, "&StrLen(%ld, ", (long)len);
	      out_arg (rhs_param);
	      fputs (") ", out_file);
	    }
	...
	if (SXBA_bit_is_set (loop_clause_set, clause)) {
	  ...
	  fprintf (out_file, "\n%% %ld is identical to clause #%ld\n", (long)clause, (long)clause2identical [clause]);
	}
      }
    }
  }
...
Chacune de ces productions peut etre gauche ou droite, la liste est terminee par 0. */\n"
	...
	if (right_prod_nb) {
	  ...
	  while ((one_rcg_right_prod = sxba_scan (line, one_rcg_right_prod)) > 0) {
	    fprintf (out_file, "0X%lX, ", (long)one_rcg_right_prod|IS_RIGHT_PROD);
	  }
	}
	...
	if (left_prod_nb) {
	  ...
	  while ((one_rcg_left_prod = sxba_scan (line, one_rcg_left_prod)) > 0) {
	    fprintf (out_file, "0X%lX, ", (long)one_rcg_left_prod|IS_LEFT_PROD);
	  }
	}
...
static SXBOOLEAN	is_t_set, is_local_t_set, has_known_lgth_var, block_extraction, previous_block_extraction;
...
static void
gen_symb (SXINT symb, SXBOOLEAN *is_first)
{
...
static void
gen_arg (SXINT bot, SXINT top, SXBOOLEAN *is_first)
{
    SXINT pos;
...
static SXINT
lhs_gen_or_degree (struct arg2bot_top *p, SXINT arity, SXBOOLEAN is_gen/* , is_main */)
{
...
fill_local_eq_set (struct arg2bot_top *p, SXINT arity)
{
    ...
    sxinitialise(eq_len) /* Pour un compilateur croisé pour l'architecture itanium64 qui est gcc version 3.4.5
...
clause_in_2var_form (SXINT clause)
{
    ...
    SXBOOLEAN		is_main, OK, done, is_first, before, after;
	...
	if (!is_main) {
		...
		fprintf (out_file, "clause_%ld_%ld", (long)clause, (long)slice2new_clause [cur_slice]);
	    }
	}
	...
	while ((eq = sxba_scan (eq_set, eq)) > 0) {
		...
		if (degree < min_degree) {
		    min_degree = degree;
		    sxba_copy (previous_best_eq_set, best_eq_set);
		    previous_block_extraction = SXTRUE;
		}
	...
	lhs_gen_or_degree (p, arity, SXTRUE/* , is_main */);
	...
	if (rhs > 0) {
		...
		while ((symb = sxba_scan_reset (var_set, symb)) > 0) {
			...
			fprintf (out_file, "(%ld, ", (long)var2lgth [symb]);
	...
	if (new_var_nb > max_gvar) {
		...
		if (to_be_memorized) {
		    ...
		    fprintf (out_file, "clause_%ld_%ld(", (long)clause, (long)new_clause_nb);
		}
	...
	if (clause2identical [clause] == clause && clause >= init_clause_nb) {
		...
		if (rhs_nt > 0) {
			...
			if (rhs_nt == LEX_ic) {
			    ...
			    if (rhs_nt == FALSE_ic)
				is_combinatorial = SXTRUE; /* inutile de la passer a la moulinette */
			}
		    }
		}
...
static SXINT
print_new_prdct (SXBOOLEAN is_lhs, SXINT cur)
{
    ...
    if (nt > max_nt) {
	nt -= max_nt;
	fprintf (out_file, "clause_%ld_%ld", (long)new_nts [nt].old_clause, (long)new_nts [nt].new_clause);
...
print_new_clause (SXINT bot, SXINT top)
{
    ...
    cur = print_new_prdct (SXTRUE, bot);
    fputs ("--> ", out_file);

    while (cur < top) {
	cur = print_new_prdct (SXFALSE, cur);
    }
...
do_reduction (void)
{
	...
	for (nt1 = 1; nt1 <= max_nt; nt1++) {
		...
		if (check_set) {
			...
			"Predicate names %s and %s are equivalent.",
...
/* ********************************************************************************** */
/* LRFSA */
#define is_fsa2rcg SXFALSE

/* Les essais ont montre que ca marchait mieux ... */
#define process_single_transition SXFALSE
#define is_early_cycle_erasing SXTRUE
...
static SXBOOLEAN is_bnf_in_reverse, fsa_has_reduction;
...
static SXBOOLEAN has_null_trans, is_deterministic;
...
static void rcg2bnf (SXBOOLEAN is_print);
...
static SXBOOLEAN R_is_cyclic, appel_a_fermer;
...
/* Si appel_a_fermer est SXTRUE, retourne ds F la fermeture transitive de M */
...
k_equiv (void)
{
  ...
  SXBOOLEAN has_changed = SXFALSE, is_first, p_kequiv;
...
dfsa2min_fsa (void)
{
  ...
  for (p = 1; p <= dfsa_state_nb; p++) {
	...
	SXBA_1_bit (fsa_final_state_set, p2);
      ...
      XxYxZ_Xforeach (dfsa_hd, p, triple) {
	X = XxYxZ_Y (dfsa_hd, triple);
	if (X >= 0) fsa_has_reduction = SXTRUE;
...
reduce_fsa (void)
{
  ...
  XxYxZ_header    ncfsa_hd;
  SXBOOLEAN is_multiple, must_shrink = SXFALSE, is_final;
...
static VARSTR
get_UBs (SXINT t, SXBOOLEAN *has_UBs)
{
...
{
  /* retourne SXTRUE ssi il y a une unique transition depuis q (elle est mise ds X et r) */
  SXINT foo, t, s;
  SXBOOLEAN is_first = SXTRUE;

  XxYxZ_Xforeach (fsa_hd, q, foo) {
    if (!is_first) return SXFALSE;
...
fsa2rcg (void)
{
  ...
  SXINT *state_stack;
  SXBOOLEAN is_first, has_xtrans, has_null, has_UBs, T0;
...
static SXINT final_state = %ld, prev_final_state = %ld;\n", (long)final_state, (long)prev_final_state);

#if 0
  /* inutile */
  sxba_to_c (fsa_final_state_set, out_file, "fsa_final_state_set", "", SXTRUE /* static */);
#endif

  matrix2vector (&fsa_shift, &vector, 25);
  vector2c (&vector, out_file, "fsa_shift", SXTRUE /* static */);
  ...
  if (fsa_red_top == 1) {
    ...
    fprintf (out_file, "\nstatic SXINT fsa_red_disp [%ld] = {0,", (long)fsa_state_nb+2);

    for (i = 1; i <= fsa_state_nb+1; i++) {
      if ((i%10)==1)
	fprintf (out_file, "\n/* %ld */ ", (long)i);

      fprintf (out_file, "%ld, ", (long)fsa_red_disp [i]);
    }

    fputs ("\n};\n", out_file);

    fprintf (out_file, "\nstatic struct trans fsa_red_trans [%ld] = {{0, 0},", (long)fsa_red_top);

    for (i = 1; i < fsa_red_top; i++) {
      if ((i%10)==1)
	fprintf (out_file, "\n/* %ld */ ", (long)i);

      fprintf (out_file, "{%ld, %ld}, ", (long)fsa_red_trans [i].X, (long)fsa_red_trans [i].q);
    }
  ...
  if ((top = XH_top (mtrans_hd)) > 1) {
    fprintf (out_file, "\nstatic SXINT mtrans_disp [%ld] = {0,", (long)top+1);

    for (i = 1; i <= top; i++) {
      if (i % 10 == 1)
	fprintf (out_file, "\n/* %ld */ ", (long)i);

      fprintf (out_file, "%ld, ", (long)XH_X (mtrans_hd, i));
    }

    fputs ("\n};\n", out_file);

    fprintf (out_file, "static SXINT mtrans_list [%ld] = {0,", (long)XH_list_top (mtrans_hd));
  ...
  if (is_look_ahead) {
    ...
    while ((q = sxba_scan (for_look_ahead, q)) > 0) {
      ...
      sprintf (string, "_%ld", (long)q);
      sxba_to_c (t_set, out_file, "look_ahead", string, SXTRUE /* static */);
      sxba_empty (t_set);
    }
  ...
  if (!is_tagger || is_keep_lex) {
    ...
    fputs ("#if is_lex\n", out_file);
    sxbm3_to_c (out_file, t2prod_set, max_t+1, "t2prod_set", "", SXTRUE /* is_static */);
    fputs ("#endif /* is_lex */\n", out_file);
...
lrfsa (void)
{
  ...
  if (is_tagger) {
    /* Le FSA sera utilise pour faire du POS tagging, on ne s'interesse donc pas aux reductions.
       ...
       is_keep_lex. 
       L'experience a montre que si is_keep_lex est SXFALSE, l'analyse est + rapide
    */
  ...
  if (is_shallow) {
	...
	fprintf (sxtty, "WARNING: the \"shallow\" file %s contains %ld unknown predicate names.\n",
		 shallow_file_name, (long)unknown_nt_nb);
    }
  }
...
static SXBOOLEAN has_eps_rules, has_eps_trans, eps_rules_are_null;
...
cfg2efcfg ()
{
  ...
  if (SXBA_bit_is_set (bvide, 1)) {
    /* La chaine vide est ds le langage, on genere S --> ; */
      ...
      efbnf.prolon [efbnf.nbpro+1] = efbnf.indpro+1;
      has_eps_rules = SXTRUE;
  }
  else
      has_eps_rules = SXFALSE;
...
rcg2Lex_file (void)
{
  ...
  SXBOOLEAN has_lex, has_identical_Lex, is_map_needed, has_strlen, has_streq;
  ...
  fprintf (out_file, "#define has_streq %ld\n", (long)(S1_is_empty (streq_list) ? 0 : 1));

  fprintf (out_file, "static SXINT prod2clause [%ld] = {0,", (long)nbpro);

  for (prod_no = 1; prod_no < nbpro; prod_no++) {
    if (prod_no % 10 == 1)
      fprintf (out_file, "\n/* %ld */ ", (long)prod_no);

    fprintf (out_file, "%ld, ", (long)prod2clause [prod_no]);
  }
  ...
  for (clause = 1; clause <= last_clause; clause++) {
    ...
    if ((cur = clause2idem_disp [clause])) {
      fprintf (out_file, "/* %ld */ ", (long)clause);

      do {
	fprintf (out_file, "%ld, ", (long)clause2idem [cur]);
	bot++;
      } while ((cur = idem2next [cur]) > 0);
  ...
  for (clause = 1; clause <= last_clause; clause++) {
    if ((prod_no = clause2prod [clause]) != 0) {
      ...
      has_identical_Lex = SXTRUE; /* A priori */
	...
	if (!has_identical_Lex) {
	  ...
	  SXBOOLEAN	must_check_identical_prod = SXTRUE;

	  if (!SXBA_bit_is_set (has_Lex, clause))
	    must_check_identical_prod = SXFALSE;
  ...
  fputs ("\n#if has_Lex\n", out_file);
  sxbm3_to_c (out_file, Lex2prod_item_set, max_t+1, "Lex2prod_item_set", "", SXTRUE /* is_static */);
  sxbm3_to_c (out_file, prod2multiple_Lex_set, nbpro, "prod2multiple_Lex_set", "", SXTRUE /* is_static */);
  sxba_to_c (multiple_Lex_item_set, out_file, "multiple_Lex_item_set", "", SXTRUE /* static */);
  sxba_to_c (must_check_identical_prod_set, out_file, "must_check_identical_prod_set", "", SXTRUE /* static */);

  fprintf (out_file, "#define last_clause\t%ld\n\n", (long)last_clause);
  /* Ici has_Lex[clause]==SXTRUE <==> clause est le representant non trivial de clauses equivalentes ayant des Lex
     non identiques */
  sxba_to_c (has_Lex, out_file, "clause_has_Lex", "", SXTRUE /* static */);
  fputs ("#endif /* has_Lex */\n", out_file);
  
  if (sxba_is_empty (rcg2Lex_file_loop_clause_set))
    fputs ("static SXBA rcg2Lex_file_loop_clause_set\n", out_file);
  else
    sxba_to_c (rcg2Lex_file_loop_clause_set, out_file, "rcg2Lex_file_loop_clause_set", "", SXTRUE /* static */);

  fputs ("\n#if has_Lex\n", out_file);
  sxbm3_to_c (out_file, rcg2Lex_file_clause2lex_set, last_clause+1, "rcg2Lex_file_clause2lex_set", "", SXTRUE /* is_static */);
  fputs ("#endif /* has_Lex */\n", out_file);
  ...
  for (prod_no = 1; prod_no < nbpro; prod_no++) {
    ...
    fprintf (out_file, "%ld, ", (long)prod2rhs_stack_map [prod_no]);
  }
  ...
  if (!S1_is_empty (streq_list)) {
	...
	fprintf (out_file, "\n/* %ld */ ", (long)clause);
      ...
      fprintf (out_file, "%ld, ", (long)clause2streq [clause]);
    }
...
tag2tig (void)
{
  ...
  SXBOOLEAN is_strong_left_clause, is_strong_right_clause, is_strong_left, is_strong_right, done, is_first, has_changed; 
  ...
  for (clause = 1; clause <= last_clause; clause++) {
	...
	lhs_top2 = XH_X (rcg_predicates, lhs_prdct+1);
	is_strong_left_clause = is_strong_right_clause = SXTRUE;
	...
	for (lhs_arg = 0, lhs_cur2 = lhs_bot2+2; lhs_cur2 < lhs_top2; lhs_arg++, lhs_cur2++) {
	  ...
	  if (lhs_param > 0) {
		...
		if (lhs_arg == 0) {
		  is_strong_right_clause = SXFALSE;
		}
		else {
		  is_strong_left_clause = SXFALSE;
		}
	      }
	    }
	  }
  ...
  */
  ...
  for (A = 1; A <= max_nt; A++) {
    if (nt2arity [A] == 2) {
      ...
      is_strong_left = is_strong_right = SXTRUE;
	...
	if (lhs_arity == 2 && SXBA_bit_is_set (strong_left_A_set, lhs_nt) && SXBA_bit_is_set (strong_right_A_set, lhs_nt)) {
	  ...
	  if (lhs_param > 0) {
		...
		if (SXBA_bit_is_set (lvar_set, symb)) {
		  if (is_first) is_first = SXFALSE; else fputs (" ", out_file);
...
tag2tig (void)
{
  ...
  SXBOOLEAN is_vanished, has_changed, is_first_arg, is_first_symb; 
  SXBA lvar_set;
  ...
  if (modified_clause_nb) {
	...
	for (k = 0, lhs_cur2 = lhs_bot2+2; lhs_cur2 < lhs_top2; k++, lhs_cur2++) {
	  if (Ak2len [A_k2Ak (lhs_nt, k)] != 0) {
		...
		if (symb > 0) {
		  if (SXBA_bit_is_set (lvar_set, symb)) {
		    if (is_first_symb) is_first_symb = SXFALSE; else fputs (" ", out_file);
		    fprintf (out_file, "X%ld", (long)symb-1);
		  }
		  else
		    has_changed = SXTRUE;
		}
	...
	while (rhs_cur < rhs_top) {
	  ...
	  if (rhs_nt > 0) {
		...
		if (SXBA_bit_is_set (lvar_set, left_symb)) {
		  /* on genere &StrLen(0, left_symb); */
		  fprintf (out_file, "&%s(0,X%ld) ", sxttext(sxtab_ptr, strlen_t), (long)left_symb-1);
		}
		else {
		  if (SXBA_bit_is_set (lvar_set, right_symb)) {
		    /* on genere &StrLen(0, right_symb); */
		    fprintf (out_file, "&%s(0,X%ld) ", sxttext(sxtab_ptr, strlen_t), (long)right_symb-1);
		  }
		}
	      }
...
tig_form (void)
{
  ...
  if (sxverbosep) {
    if (modified_clause_nb)
      fprintf (sxtty, "succes[%ld/%ld]\n", (long)modified_clause_nb, (long)last_clause);
    else
      fputs ("failure\n", sxtty);
