...
char WHAT_BNFHUGE[] = "@(#)SYNTAX - $Id: bnf_huge.c 1132 2008-03-05 12:17:53Z syntax $" WHAT_DEBUG;
...
bnf_huge (struct bnf_ag_item *B, char *langname)
{
  ...
  for (i = 0; i <= B->WS_TBL_SIZE.indpro; i++) {
    printf ("/* %ld */ %ld,\n", (long)i, (long)B->WS_INDPRO [i].prolis);
  }
  fputs ("};\n", stdout);
  out_endif ("prolis");

  out_ifdef ("prolon");
  printf ("static SXINT prolon [%ld] = {\n", (long)B->WS_TBL_SIZE.nbpro+2);
  rhs_lgth = 0;
...
/* 0 */ \"\",\n", (long)-B->WS_TBL_SIZE.tmax+1);
...
#endif /* 0 */
  {
    ...
    word_tree_alloc (&word_tree_struct_inflected_form, "word_tree_struct_inflected_form", -B->WS_TBL_SIZE.tmax /* word_nb */, (B->WS_TBL_SIZE.t_string_length/(-B->WS_TBL_SIZE.tmax))+1 /* word_lgth */,
		     1 /* Xforeach */, 0 /* Yforeach */, TRUE /* from_left_to_right */, TRUE /* with_path2id */, NULL /* void (*oflw) () */, NULL /* FILE *stats */);
...
/* 0 */ \"\",\n", (long)B->WS_TBL_SIZE.ntmax+1);
...

  {
    ...
    if (multiple_t_prod_set) {
      printf ("%li\n", (long)sxba_cardinal (multiple_t_prod_set));
	...
	printf ("/* %ld \"%s\" */ ", (long)i, B->T_STRING+B->ADR [-i]);

      for (cur = bot; cur < top; cur++) {
	printf ("%ld, ", (long)t2prod_list [cur]);
      }
...
/* 0 */ 0,\n", (long)B->WS_TBL_SIZE.ntmax+1);
      ...
      for (A = 1; A <= B->WS_TBL_SIZE.ntmax; A++) {
	printf ("/* %ld=%s */ %ld,\n", (long)A, B->NT_STRING+B->ADR [A], (long)nt2gen_t_list [A]);
      }
    
      fputs ("};\n", stdout);

      printf ("static SXINT gen_t_list [%ld] = {\n\
/* 0 */ 0,", (long)gen_t_list_top);

      for (A = 1; A <= B->WS_TBL_SIZE.ntmax; A++) {
	if ((i = nt2gen_t_list [A])) {
	  printf ("\n/* %ld=%s */ ", (long)A, B->NT_STRING+B->ADR [A]);

	  nb = gen_t_list [i++];
	  printf ("%ld /* nb */, ", (long)nb);
...
/* 0 */ 0,\n", (long)B->WS_TBL_SIZE.ntmax+2);
  for (i = 1; i <=  B->WS_TBL_SIZE.ntmax+1; i++) {
    printf ("/* %ld */ %ld,\n", (long)i, (long)B->WS_NTMAX [i].npg);
  }
  fputs ("};\n", stdout);
  out_endif ("npg");
  ...
  for (i = 0; i <= B->WS_TBL_SIZE.nbpro+1; i++) {
    printf ("/* %ld */ %ld,\n", (long)i, (long)B->WS_NBPRO [i].numpg);
  }
  fputs ("};\n", stdout);
  out_endif ("numpg");
  ...
  for (i = 0; i <=  B->WS_TBL_SIZE.ntmax+1; i++) {
    printf ("/* %ld */ %ld,\n", (long)i, (long)B->WS_NTMAX [i].npd + ((i>1) ? i-1 : 0));
  }
  fputs ("};\n", stdout);
  out_endif ("npd");

  out_ifdef ("numpd");
  printf ("static SXINT numpd [%ld] = {\n", (long)B->WS_TBL_SIZE.maxnpd+1+B->WS_TBL_SIZE.ntmax);
  k = 0;
  j = 1;
  for (i = 0; i <=  B->WS_TBL_SIZE.maxnpd; i++) {
    printf ("/* %ld */ %ld,\n", (long)k, (long)B->NUMPD [i]);
    k++;
