...
#include "lecl_ag.h"
#include "lecl_pcn.h"
char WHAT_LECLSTC[] = "@(#)SYNTAX - $Id: lecl_stc.c 1114 2008-02-27 15:13:51Z rlacroix $" WHAT_DEBUG;
...

BOOLEAN		lecl_check_octal_code (SXINT ate, struct sxsource_coord designator, SXINT *val)
{
    if (ate == 0)
	return (FALSE);
...
static BOOLEAN	is_a_son (struct lecl_node *node_ptr,
			  SXINT node_name)
{
...
static BOOLEAN	check_environments (struct lecl_node *node_ptr,
				    SXINT name)
{
    struct lecl_node	*son;
...
static BOOLEAN	check_component (struct lecl_node *node_ptr,
				 SXINT *val)
{
...
static BOOLEAN	st_check_slice (struct lecl_node *ptr1, 
...
static SXINT	define_class_by_string (SXUINT *alpha_table_entry, 
					struct sxsource_coord designator, 
					BOOLEAN is_not)
{
    SXINT 		car, delta;
...
static SXINT	define_class_by_octal_code (SXUINT *alpha_table_entry, 
					    struct sxsource_coord designator, 
					    BOOLEAN is_not)
{
    SXINT		codofcar;
...
static SXINT	ref_class_by_name (SXUINT *alpha_table_entry, 
				   SXINT class, 
				   BOOLEAN is_not)
{
    /* is_not always true */
...
static VOID	finalize_re (SXINT re_no, SXINT finalize_re_item_no, BOOLEAN is_abbrev, struct lecl_node *node_ptr)
{
    SXINT		x;
...
static SXINT	set_action_or_prdct (BOOLEAN is_action, SXINT alpha_table_entry)
{
...
static VOID	st_finalize_context (BOOLEAN is_unbounded, BOOLEAN kind)
{
    struct ers_disp_item	*ers_disp_ptr;
