...
#include "varstr.h"
#include "lecl_ag.h"
char WHAT_LECLDIAGNOS[] = "@(#)SYNTAX - $Id: lecl_diagnos.c 1115 2008-02-28 09:03:25Z syntax $" WHAT_DEBUG;
...
static BOOLEAN	less_origine_index (SXINT less_origine_index_i, SXINT less_origine_index_j)
{
...
static BOOLEAN	less_item (SXINT less_item_i, SXINT less_item_j)
{
    struct item	*pi, *pj;
...
BOOLEAN		has_la_trans (SXBA *M, SXINT has_la_trans_state)
{
    BOOLEAN	is_la;
...
/* On ne conserve que les transitions allant d'un etat mixte vers un etat en
   look_ahead en conservant les etats "PRDCT" intermediaires */
	...
	for (conflict_record_ptr = head_ptr;
	    ...
	    sprintf (s1, "%-ld", (long)conflict_record_ptr->seed_state);
	    sprintf (s2, "%-ld", (long)conflict_record_ptr->detection_state);
				  ...
				  BOOLEAN with_detection_point)
{
    ...
    sprintf (s, "%-ld", (long)detection_state);
