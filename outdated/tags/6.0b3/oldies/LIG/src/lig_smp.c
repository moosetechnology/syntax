...
static BOOLEAN		is_LIG_in_normal_form, is_post_act, is_post_prdct, is_useless, is_reduced, sxttycol1p;
...
lig_pi ()
{
	...
	case 4 :		/* VISITED->name = VOID_n */
		...
		sxput_error (VISITED->father->son->token.source_index,
		...
		lig_rule_to_line_no [rule_no] = VISITED->father->son->token.source_index.line;
...
lig_pd ()
{
	...
	if (VISITED->father->name == VOCABULARY_S_n ||
	    VISITED->father->name == LHS_OBJECT_n && VISITED->father->son->brother->name == VOID_n)
	{
	    XH_push (bnf_rules, VISITED->token.string_table_entry);
	    XH_push (lig_rules, VISITED->token.string_table_entry);
	}
	...
	if (VISITED->father->name == X_TERMINAL_n ||
	    VISITED->father->name == RHS_OBJECT_n && VISITED->father->son->brother->name == VOID_n)
	{
	    XH_push (bnf_rules, VISITED->token.string_table_entry);
	    XH_push (lig_rules, VISITED->token.string_table_entry);
	}
...
lig_pi2 ()
{
	...
	case 2 :		/* VISITED->name = VOID_n */
	    print_comment (VISITED->token.comment);
	    putchar ('=');
	    break;
...
lig_pd2 ()
{
	...
	if (VISITED->father->name == VOCABULARY_S_n ||
	    VISITED->father->name == LHS_OBJECT_n && VISITED->father->son->brother->name == VOID_n)
	{
	    printf ("%s", sxstrget (VISITED->token.string_table_entry));
	}
	...
	printf ("<%s>", sxstrget (VISITED->token.string_table_entry));
	BNFlhs [BNFprod] = ste2nt [VISITED->token.string_table_entry];
	...
	print_comment (VISITED->token.comment);
	printf ("<%s>", sxstrget (VISITED->token.string_table_entry));
	BNFlispro [++BNFmaxitem] = ste2nt [VISITED->token.string_table_entry];
	BNFprod2nbnt [BNFprod]++;
	break;
	...
	if (VISITED->father->name == X_TERMINAL_n)
	{
	    varstr_quote (varstr_raz (vstr), sxstrget (VISITED->token.string_table_entry));
...
lig_reduction ()
{
    ...
    int 		*rhs_stack;
    BOOLEAN		is_local;
    static BOOLEAN	SXLIG_semact ();
    static BOOLEAN 	SXLIG_sem_pass ();

    int			top, bot, AsB, sB, AB, A, B, As, A00, B00;
    int			*p, *plim;
    BOOLEAN		EQUALnfull, PUSHPOPnfull, POPnfull;
    char		kind;
