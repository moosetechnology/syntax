...
static int		max_local_temp, lhs_var_nb, _ste;
...
static int		SXDCGmax_gvar, SXDCGmax_gvarppp, gvar_nb, *SXDCG2var_disp, *ste2gvar, *gvar2ste;
...
/* Le prolog accepte */
static BOOLEAN		is_prolog, has_prolog, is_dynam_constant, is_prolog_tree;
    ...
    BOOLEAN	is_nt;
{
	...
	if (ste2nt [ste] == 0) {
	    ste2nt [ste] = ++BNFmaxnt;
	    nt2ste [BNFmaxnt] = ste;
	    BNFntstring [BNFmaxnt] = sxstrget (ste);
	}
...
dcg_pi ()
{
    ...
    case BODY_S_n :		/* VISITED->name = RHS_ITEM_S_n */
	rhs_arg_nb = 0;
    ...
    case DCG_n :		/* VISITED->name = RULE_S_n */
	break;

    case DIV_n :
	switch (VISITED->position) {
	case 1 :		/* VISITED->name = {DIV_n, MINUS_n, MOD_n, MULT_n, PLUS_n, UNARY_MINUS_n, floating_point_number_n, integer_number_n, 
				   variable_n} */
	    break;

	case 2 :		/* VISITED->name = {DIV_n, MINUS_n, MOD_n, MULT_n, PLUS_n, UNARY_MINUS_n, floating_point_number_n, integer_number_n, 
				   variable_n} */
	    break;
    ...
    case LHS_n :
	switch (VISITED->position) {
	case 1 :		/* VISITED->name = MAIN_FUNCTOR_n */
	    lhs_functor = VISITED->son->token.string_table_entry;
	    break;

	case 2 :		/* VISITED->name = {TERMINAL_S_n, VOID_n} */
	    break;
    ...
    case RHS_ITEM_S_n :		/* VISITED->name = {MAIN_FUNCTOR_n, PROLOG_S_n, TERMINAL_S_n, VOID_n} */
	break;

    case RULE_n :
	switch (VISITED->position) {
	case 1 :		/* VISITED->name = LHS_n */
	    lhs_or_rhs = 0;
	    lhs_arg_nb = 0;
	    break;

	case 2 :		/* VISITED->name = BODY_S_n */
    ...
    case TERMINAL_S_n :		/* VISITED->name = {atom_n, floating_point_number_n, integer_number_n, string_n, variable_n} */
	break;
...
dcg_pd ()
{
	...
	if (XxY_set (&dcg_rules, dcg_lhs_part, dcg_rhs_part, &rule_no)) {
	    ...
	    dcg_rule_to_line_no [rule_no] = VISITED->token.source_index.line;
	...
	if (VISITED->father->name == TERMINAL_S_n && VISITED->father->father->name == RHS_ITEM_S_n) {
...
static void
gen_ppp (ste, param_nb, node_ptr)
    int 	ste, param_nb;
    NODE	*node_ptr;
{
    int		code, bot, i, type, param, kind, x, nt;
    /* On verifie le "type" des parametres */
    code = ste2prolog_code [ste];
...
#if 0
	prolog_code2nt [code] = nt = ste2nt [ste];
...
dcg_pi2 ()
{
	...
	case 1 :		/* VISITED->name = atom_n */
	    param_nb = VISITED->brother->degree;

	    if (VISITED->father->father->name == RHS_ITEM_S_n) {
		ste = VISITED->token.string_table_entry;
		str = sxstrget (ste);
		printf (" <%s>", str);
	...
	if (VISITED->father->father->name == RHS_ITEM_S_n) {
		...
		if (VISITED->name != atom_n || isalpha (*str))
		    printf (" %s", str);
		else
		    printf (" #%s", str);
		...
		if (VISITED->name == integer_number_n)
...
dcg_pd2 ()
{
    ...
    case atom_n :
	if (VISITED->father->name != TERMINAL_S_n &&
	    VISITED->father->father->name != PROLOG_S_n &&
	    VISITED->father->name != MAIN_FUNCTOR_n) {
	    int	atom;
...
dcg_write ()
{
    ...
    int			bot, top, atom, var, j, next;
    int			SXDCGmax_term, SXDCGmax_list;
    int			gvar;
    int			term_top, list_top, val, ref, kind, nt, ppp, ppp_term_top, param;
...
/* 0 */ 0,\n", XH_list_top (SXDCGvar_names));
    ...
    for (x = 1; x < XH_top (SXDCGvar_names); x++) {
	 ...
	 do {
	     fprintf (F_dcg, "%i, ", XH_list_elem (SXDCGvar_names, bot));
	 } while (++bot < top);
...
prolog_init ()
{
	...
	ste = sxstrretrieve (prolog_name2str [name]);

	if (ste != ERROR_STE) {
	    code = prolog_name2prolog_code [name];
	    ste2prolog_code [ste] = code;
	}
    }
}
...
smppass ()
{
	...
	SXDCG2var_disp = (int*) sxalloc (rule_nb+1, sizeof (int));
