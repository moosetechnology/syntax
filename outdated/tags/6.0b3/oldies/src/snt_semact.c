...
//char WHAT_SNT_SEMACT[] = "@(#)SYNTAX - $Id: snt_semact.c 805 2007-09-28 08:03:20Z rlacroix $" WHAT_DEBUG;
     ...
     int Aij;
{
  ...
  double localmin;
  BOOLEAN found_best_hook, blessed_Aij;
  ...
  */
  ...
  while ((cur_Pij = spf.outputG.rhs [hook].lispro) != 0) {
    if (cur_Pij > 0) { /* il ne s'agit pas d'une branche elaguee */
	  ...
	  && !blessed_Aij   /* et la prod n'est pas "bénite" à cause d'un snt de rhs béni */
	  ) { 
#if EBUG
