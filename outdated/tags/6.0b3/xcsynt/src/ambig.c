...
char WHAT_XCSYNTAMBIG[] = "@(#)SYNTAX - $Id: ambig.c 1128 2008-03-04 11:49:22Z rlacroix $" WHAT_DEBUG;
...
static SXINT	*StNt_stack, *read_empty;
static SXBA	StNt_set, StNt_set2;
static SXBA	ambig_orig_set, proto_ambig_StNt_set, ambig_StNt_set;
static SXINT	*state_path1, *state_path2, *items1, *items2;
static VARSTR	post1;
static SXINT	next_state, init_t;

static XxY_header	reads_hd;
static SXBA		cycle_set;
static SXINT		cycle_pivot;

static SXBA		NT_set, S_set;
static SXINT		*S_stack, *NT_stack;
static VARSTR		cycle_vstr;
static BOOLEAN		(*includes_F) (SXINT);
...
static char	dots [] = " ... ";
static char	equals [] = " === ";
static BOOLEAN	grow_items1 (SXINT *incl, 
			     SXINT *path, 
			     ...
			     SXINT nt2, 
			     SXINT item2, 
			     SXINT grow_items1_nt, 
			     SXINT top);



static VOID	oflw_StNt_reads_cycle (SXINT old_size, SXINT new_size)
{
    sxuse(old_size); /* pour faire taire gcc -Wunused */
    StNt_to_reads_cycle_nb = (SXINT*) sxrealloc (StNt_to_reads_cycle_nb, new_size +
	 1, sizeof (SXINT));
}


static VOID	nt_trans_to_spath (SXINT nt_trans, SXINT item, SXINT *spath)
{
...
static SXINT	nt_trans_to_item_no_i (SXINT nt_trans2, SXINT nt_trans1, SXINT i)
{
    ...
    SXINT	next, n, no, item;
    SXINT			nt1, rule_no, state1, state2;
...
static BOOLEAN	grow_items2 (SXINT *incl, 
			     SXINT *path, 
...
static VOID	slice_cat (SXINT item1, SXINT item2)
{
...
static VOID	outderiv (SXINT xitem, SXINT *items, char *t_string, char **pf)
{

/* Ecrit la derivation contenue dans items, la chaine t_string est sortie a partir du niveau
   xitem */
    SXINT	x, outderiv_lim, item;
    SXINT		pre_col, pre_lgth, red_no, init_item;
    BOOLEAN	is_t_string;
...
static char	*make_t_string (VARSTR make_t_string_post1, SXINT item, SXINT make_t_string_init_t)
{
    SXINT	tnt;
...
BOOLEAN is_t_trans_function (SXINT item)
{
...
static SXBA first (SXBA t_set, SXINT item)
{
  /* item : A -> alpha . X1 --- Xn */
...
BOOLEAN is_t_in_first (SXINT t, SXINT item)
{
  /* item : A -> alpha . X1 --- Xn */
...
static BOOLEAN	SCC (SXINT origine, SXINT X1, SXINT *stack, SXBA check_set)
{
...

static VOID	print_ambiguity (SXINT *stack1, SXINT item, SXINT nt_trans)
{
    char	*t_string, *pf;
    SXINT		print_ambiguity_nt;
...
/* Sortie des derivations montrant l'ambiguite */
    ...
    *items2 = 1;
    ...
    if (grow_items2 (state_path2, state_path1, items1, (SXINT)0, items2, bnf_ag.WS_NBPRO [bnf_ag.WS_INDPRO [item].prolis].reduc,
	 item - 1, print_ambiguity_nt, (SXINT)1, *items2)) {
	inverse (items1);
	inverse (items2);
...
/* Sortie de la deuxieme derivation */
	...
	pf = equals + 5;
	outderiv (*items2, items2, t_string, &pf);
	outderiv ((SXINT)0, items1, t_string, &pf);
	put_edit_nl (1);
    }
    else {
...
static BOOLEAN	includes_traversal (SXINT X)
{
...
static VOID NT_closure (SXBA XNT_set, SXINT XNT)
{
    SXINT j, k, NT;
...
static BOOLEAN check_item (SXINT item, SXINT *check_item_S_stack, SXINT *check_item_NT_stack)
{
...
static VOID print_not_LRpi (SXINT *cycle_stack, SXINT kind)
...
static BOOLEAN	read_non_empty (SXINT read_non_empty_next_state)
{
    SXINT 	n, item, kind;

    if (read_empty == NULL)
	read_empty = (SXINT*) sxcalloc (xac2, sizeof (SXINT));
...
static BOOLEAN	ambiguity_check (SXINT X1)
{
    ...
    SXINT	x, X, X2, ambiguity_check_next_state;
...

BOOLEAN	ambiguity (SXBA conflict_set, SXBA ambig_root_set)
{
	...
	StNt_stack = (SXINT*) sxcalloc (XxY_top (Q0xV_hd) + 1, sizeof (SXINT));
	StNt_stack [0] = -1;
    }
...
BOOLEAN is_ambig_on_StNt (SXINT StNt)
{
    return SXBA_bit_is_set (ambig_StNt_set, StNt);
}

VOID	free_ambiguities (void)
{
...
static BOOLEAN ambig_orig_F (SXINT X)
{
...
VOID	print_ambiguities (/* xac1, */SXBA t_set, SXINT StNt)
{
    /* On sait que:
	...
	state_path1 = (SXINT*) sxcalloc (l, sizeof (SXINT));
	state_path2 = (SXINT*) sxcalloc (l, sizeof (SXINT));
	items1 = (SXINT*) sxcalloc (l, sizeof (SXINT));
	items2 = (SXINT*) sxcalloc (l, sizeof (SXINT));
...

{
	...
	if (is_cycle) {
		...
		StNt_stack = (SXINT*) sxcalloc (l, sizeof (SXINT));
		StNt_stack [0] = -1;
	    }

	    indexes = (SXINT*) sxcalloc (reads_index_size, sizeof (SXINT));
	    ...
	    StNt_to_reads_cycle_nb = (SXINT*) sxalloc (X_size (StNt_reads_cycle_hd) + 1, sizeof (SXINT));
...
SXBA is_not_LRpi_on_StNt (SXINT StNt)
{
    sxba_empty (StNt_set);
    includes_F = NULL;
    includes_traversal (StNt);
...
static BOOLEAN cycle_set_to_path (SXINT X)
{
...
VOID	print_not_LRpi_conflicts (SXINT kind)
{
    ...
    */
	...
	S_stack = (SXINT*) sxcalloc (xac2, sizeof (SXINT));
	S_stack [0] = -1;
