...
/************************************************************************/
/* 20030312 16:43 (phd):	(undefvarfetch) sur Alpha		*/
/************************************************************************/
...
char WHAT_XCSYNTXARC[] = "@(#)SYNTAX - $Id: XARC.c 1128 2008-03-04 11:49:22Z rlacroix $" WHAT_DEBUG;

#include "SS.h"
#include "RLR.h"


extern VOID	print_bfsa (SXINT bfsa, SXINT ate);
extern VOID	print_pss (SXINT pss);
extern BOOLEAN	PARTIAL_FSA_ARP_construction (struct ARC_struct *XARC, SXINT fsa1, SXINT bfsa2);
...
extern SXINT	UNBOUNDED_ARP_construction (struct ARC_struct *XARC,
					    SXINT	qtq,
					    BOOLEAN message_needed);
extern BOOLEAN	make_inter_pss (SXINT pss1, SXINT pss2);
extern char	*conflict_kind_to_string (VARSTR result, SXINT conflict_kind, SXINT h_val, SXINT k_val);
extern BOOLEAN	bh_mark (struct ARC_struct *ARC, SXINT qtq);
extern BOOLEAN	bh_unmark (struct ARC_struct *ARC, SXINT qtq);
...

static VOID XARC_UNBOUNDED_Q_oflw (SXINT old_size, SXINT new_size)
{
...
static VOID XARC_FSA_Q_oflw (SXINT old_size, SXINT new_size)
{
    sxuse(old_size); /* pour faire taire gcc -Wunused */
    ARCs [XARC_FSA].attr = (struct ARC_ATTR*) sxrealloc (ARCs [XARC_FSA].attr, new_size + 1, sizeof (struct ARC_ATTR));
}

static VOID	oflw_XQ0xXQ0 (SXINT old_size, SXINT new_size)
{
    sxuse(old_size); /* pour faire taire gcc -Wunused */
...
static VOID	oflw_forks (SXINT old_size, SXINT new_size)
{
    sxuse(old_size); /* pour faire taire gcc -Wunused */
...
static VOID	oflw_FORKxT (SXINT old_size, SXINT new_size)
{
    sxuse(old_size); /* pour faire taire gcc -Wunused */
...
static VOID	oflw_bfsa (SXINT old_size, SXINT new_size)
{
    SXINT i;
...
static VOID	oflw_pss_to_seq (SXINT old_size, SXINT new_size)
{
    sxuse(old_size); /* pour faire taire gcc -Wunused */
...
static VOID	oflw_back (SXINT old_size, SXINT new_size)
{
    sxuse(old_size); /* pour faire taire gcc -Wunused */
...
static VOID	XARC_allocate (void)
{
...
static VOID	fks_to_fork_set (SXINT xfks, SXBA set, SXINT *m, SXINT *M)
{
...
static VOID put_in_fork_work_set (SXINT bfsa, SXINT pss)
{
...
static VOID LRpi_Next (SXINT bfsa, SXINT pss)
{
...
static VOID couples_set_to_XH (XH_header *header)
{
    SXINT couple = 0;
...
static VOID XH_to_couples_set (SXINT *bot, SXINT *top)
{
    SXINT couple;
...
static VOID back_to_couples_set (SXINT ibfsa, SXINT X, SXINT Y)
{
...
static VOID	process_knot (SXBA ipss_set)
{
...
static BOOLEAN check_loop_on_back (SXINT X1)
{
...
static BOOLEAN call_check_loop_on_back (void)
{
    SXINT ipss = 0;
...
static VOID down_occur_nb (SXINT X, SXINT nb);
static VOID up_occur_nb (SXINT Y, SXINT nb)
{
    SXINT X, i;
...
static VOID compute_occur_nb (SXINT R, SXINT N)
{
...
static VOID fork_collecting (SXINT bfsa_init, SXINT pssxt)
{
    ...
    BOOLEAN		couples_set_is_empty, couples_set_has_been_cleared;
...
static VOID	reached_states (SXBA reached_set, SXBA reached_states_couples_set, BOOLEAN normal)
{
...
static VOID back_to_forest (void)
{
    SXINT		x, y, i;
    struct back	*aback;
    SXBA		set, loop_set;
...
BOOLEAN	XARC_state_scan (struct ARC_struct *XARC, 
			 SXINT qtq, 
			 BOOLEAN (*F)(struct ARC_struct*, SXINT, SXINT, SXINT, SXINT, SXINT))
{
...
static BOOLEAN	FSA_case (struct ARC_struct *XARC,
			  SXINT	qtq, 
...
static BOOLEAN	fork_equality (struct ARC_struct *XARC,
			       SXINT qtq, 
...
VOID	XARC_print (struct ARC_struct *XARC)
{
...
VOID	n_to_forks (struct ARC_struct *XARC,
		    SXINT XARC_state, 
		    SXINT n, 
		    SXINT type)
{
...
static VOID XARC_init_state_building (struct ARC_struct *XARC)
{
...
static BOOLEAN	XARC_trans (struct ARC_struct *XARC,
			    SXINT XARC_state, 
			    SXINT t, 
...
VOID	XARC_resize (SXINT old_xac2, SXINT new_non_lalr1_state_nb)
{
