...
char WHAT_XCSYNTARC[] = "@(#)SYNTAX - $Id: ARC.c 1128 2008-03-04 11:49:22Z rlacroix $" WHAT_DEBUG;

#include "SS.h"
#include "RLR.h"


extern BOOLEAN		erase_quads (SXINT ARC_kind, SXINT qtq);
...
BOOLEAN check_level_conformity (SXINT item, SXINT level)
{
...
BOOLEAN check_bot_state_conformity (SXINT state, SXINT level)
{
    /* Appele depuis earley.
...
static BOOLEAN	is_new_pss (SXINT pss, SXINT state, SXINT is_new_pss_index, SXINT next_state, SXINT *new_pss)
{
...
static BOOLEAN is_new_knoted_pss (SXINT pss, SXINT pred, SXINT is_new_knoted_pss_index, SXINT next_state, SXINT *pss1)
{
...
BOOLEAN is_pss_include (SXINT *bot1, SXINT *top1, SXINT *bot2, SXINT *top2)
{
...
static BOOLEAN ARC_max_la (SXINT state, SXINT lgth)
{
...
static BOOLEAN	BS_couple (SXINT *X, SXINT *Y, SXINT n)
{
...
static BOOLEAN	is_same_pss (SXINT xkas, SXBA *is_same_pss_pss_sets)
{
...
BOOLEAN	make_inter_pss (SXINT pss1, SXINT pss2)
{
...
static BOOLEAN is_cycle_on_fsa (SXINT pss)
{
...
static SXINT	is_inter_pss (SXINT xkas, SXBA *is_inter_pss_pss_sets, BOOLEAN is_all, BOOLEAN is_cycle_needed)
{
...
BOOLEAN ARC_walk_backward (struct ARC_struct *ARC,
			   SXINT qtq,
			   BOOLEAN (*bh)(struct ARC_struct *, SXINT),
			   BOOLEAN (*bs)(struct ARC_struct *, SXINT))
{
...
static BOOLEAN ARC_walk_forward (struct ARC_struct *ARC,
				 SXINT qtq,
				 BOOLEAN (*fh)(struct ARC_struct *, SXINT),
				 BOOLEAN (*fs)(struct ARC_struct *, SXINT))
{
...
static BOOLEAN fh_propagate_cycle (struct ARC_struct *ARC, SXINT qtq)
{
...
static BOOLEAN fh_propagate_lgth (struct ARC_struct *ARC, SXINT qtq)
{
	...
	if (pss_kind == UNBOUNDED_ && attr->lgth != -1)
	    ARC_walk_forward (ARC, qtq, fh_propagate_cycle, (BOOLEAN(*)())NULL);
...
BOOLEAN fs_unmark (struct ARC_struct *ARC, SXINT qtq)
{
...
static BOOLEAN	make_fknot (SXINT e, SXINT s, SXINT nb, SXINT *result)
{
...
BOOLEAN		X_sees_pss (SXINT X, SXINT pss)
{
...
static BOOLEAN	check_lc (SXINT check_lc_max_min_lgth)
{
...
VOID	ARC_init (BOOLEAN print_conflicts, 
		  SXINT kind, 
...
static SXINT	install_a_pss_list (struct kas *akas, BOOLEAN is_init_state)
{
...
static BOOLEAN	install_arc_state (struct ARC_struct *ARC,
...
static BOOLEAN ARC_trans (struct ARC_struct *ARC, 
			  SXINT ARC_state, 
			  SXINT t, 
...
BOOLEAN bh_ARC_one_LA (struct ARC_struct *ARC, SXINT qtq)
{
...
BOOLEAN bs_ARC_one_LA (struct ARC_struct *ARC, SXINT qtq)
{
...
BOOLEAN bh_mark (struct ARC_struct *ARC, SXINT qtq)
{
...
BOOLEAN bh_unmark (struct ARC_struct *ARC, SXINT qtq)
{
    struct ARC_ATTR	*attr = ARC->attr + XxYxZ_X (ARC->QxTxQ_hd, qtq);
...
static struct ARC_struct *call_ARC_building (struct ARC_struct *XARC, SXINT qtq)
{
    ...
    ARC_walk_backward (XARC, qtq, bh_mark, (BOOLEAN(*)())NULL);

    ARC_building (ARC, ARC_trans, ARC_UNBOUNDED_get_conflict_kind);
    
    /* On enleve les marques. */
    ARC_walk_backward (XARC, qtq, bh_unmark, (BOOLEAN(*)())NULL);
...
static BOOLEAN ARC_conflicting_node_processing (struct ARC_struct *ARC, SXINT qtq)
{
...
static BOOLEAN fh_remote_conflict (struct ARC_struct *ARC, SXINT qtq)
{
...
static BOOLEAN bh_not_pure_tree (struct ARC_struct *ARC, SXINT qtq)
{
    struct ARC_ATTR	*attr = ARC->attr + XxYxZ_X (ARC->QxTxQ_hd, qtq);
...
SXINT	ARC_building (struct ARC_struct *ARC,
		      BOOLEAN (*F_trans)(struct ARC_struct *, SXINT, SXINT, SXINT*),
		      SXINT (*F_get_conflict_kind) (struct ARC_struct *, SXINT, BOOLEAN, SXINT))
{
    SXINT	ARC_state, t;
    struct ARC_ATTR	*attr;
	...
	while ((t = sxba_scan_reset (ARC->t_set, t)) > 0) {
		...
		if (qtq != 0) {
			...
			if (ck != NO_CONFLICT_)
			    ARC_walk_backward (ARC, qtq, bh_not_pure_tree, (BOOLEAN(*)())NULL);
...
BOOLEAN ARC_construction (SXINT *failed_kind, 
			  SXINT xac1, 
			  SXBA conflict_t_set, 
