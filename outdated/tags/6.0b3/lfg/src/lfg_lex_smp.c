...
#include "varstr.h"
#include "sxunix.h"
char WHAT_LFGLEXSMP[] = "@(#)SYNTAX - $Id: lfg_lex_smp.c 1196 2008-03-11 15:42:58Z rlacroix $" WHAT_DEBUG;
...
gen_structure_hd (void)
{
  ...
  printf ("\nstatic SXINT lex_atom_hd [%ld] = {0, ", (long)XH_top (atom_hd)+1);
  ...
  printf ("\nstatic SXINT lex_atom_list [%ld] = {0,", (long)top+1);

  for (atom_id = 1; atom_id < XH_top (atom_hd);  atom_id++) {
    bot = XH_X (atom_hd, atom_id);
    top = XH_X (atom_hd, atom_id+1);

    printf ("\n/* %ld (%ld..%ld) */ ", (long)atom_id, (long)bot, (long)top-1);

    while (bot < top) {
      val = XH_list_elem (atom_hd, bot);
      
      if (val < 0)
	printf ("0X%lX, ", (long)val);
      else
	printf ("%ld, ", (long)val);
  ...
  printf ("\nstatic SXINT lex_adjunct_hd [%ld] = {0, ", (long)XH_top (adjunct_hd)+1);
  ...
  printf ("\nstatic SXINT lex_adjunct_list [%ld] = {0,", (long)top+1);

  for (adjunct_id = 1; adjunct_id < XH_top (adjunct_hd);  adjunct_id++) {
    bot = XH_X (adjunct_hd, adjunct_id);
    top = XH_X (adjunct_hd, adjunct_id+1);

    printf ("\n/* %ld (%ld..%ld) */ ", (long)adjunct_id, (long)bot, (long)top-1);
  ...
  out_ifdef ("attr_vals");
  printf ("\nstatic struct attr_val attr_vals [%ld] = { {0, 0, 0, 0},", (long)xattr_val+1);

  for (x = 1, pattr_val = attr_vals+1; x <= xattr_val; x++, pattr_val++) {
    if (x%5==1)
      printf ("\n/* %ld */ ", (long)x);
    if (pattr_val->val < 0)
      printf ("{%ld, %ld, 0X%lX, %ld}, ", (long)pattr_val->attr, (long)pattr_val->kind, (SXUINT)pattr_val->val, (long)pattr_val->next);
    else
      printf ("{%ld, %ld, %ld, %ld}, ", (long)pattr_val->attr, (long)pattr_val->kind, (long)pattr_val->val, (long)pattr_val->next);
  }
...
static SXINT
fill_structure_hd (SXNODE *visited, SXINT *lexeme, BOOLEAN *is_main_structure)
{
...
*/
static SXINT
process_named_structure (NODE *visited)
{
  SXINT     i, macro_id;
  NODE    *son, *brother;
  SXINT     ret_val;
...
static void
fill_glbl_id2fathers_sons (NODE *visited, SXINT father_id)
{
...
#if 0
static void
fill_named_structure2fathers_sons (NODE *visited, SXINT father_shared_id)
{
...
#if 0
static SXINT
fill_is_closed (NODE *visited, SXINT *unique)
{
...
static BOOLEAN
call_process_named_structure (NODE *visited)
{
  ...
  */
  ...
  case ATOM_S_n :/* VISITED->name = {ATOM_MINUS_n, ATOM_PLUS_n, CHAINE_n, ENTIER_n, IDENT_n, PRED_n} */
	...
	case IDENT_n:
	  ...
	  if (VISITED->token.string_table_entry == (SXUINT)ste_Aij || VISITED->token.string_table_entry == (SXUINT)ste_Pij) {
	    if (father->father->name != ATTR_DOTS_n) {
	      ...
	      if (VISITED->token.string_table_entry == (SXUINT)ste_Aij)
		atom_Aij = atom_id;
	      else
		atom_Pij = atom_id;
	    }
  ...
  case ATTR_ATOMS_n :
    ...
    case 1 :/* VISITED->name = IDENT_n */
      ...
      if (SXWORD_top (field_names) == (SXUINT)field_top) {
	/* Redeclaration */
	sxput_error (VISITED->token.source_index,
  ...
  case MACRO_STRUCTURE_n :
    ...
    case 1 :/* VISITED->name = MACRO_NAME_n */
      ...
      if (SXWORD_top (macro_names) == (SXUINT)macro_top) {
	/* Redeclaration */
	sxput_error (VISITED->token.source_index,
...
break;
  ...
  case RULE_n :
    ...
    case 3 :/* VISITED->name = {CHAINE_n, IDENT_n, PRED_n} */
      break;
  ...
  case TERMINAL_n :/* VISITED->name = ARGUMENT_S_n */
    ...
    if (tmax == 0) {
      ...
      t_id2t = (SXINT *) sxalloc (VISITED->degree+2, sizeof (SXINT));
...
\"%s\".",
  ...
  fprintf (sxtty, "There are %ld static fields (%ld+%ld atom types).\n", SXWORD_top (field_names) <= 3 ? (long)0 : (long)max_field_id-1,
	  (long)max_atom_field_nb, (long)max_undef_atom_field_nb);
...
/* 1 */ \"\",\n", (long)size);
	...
	for (id = 2; id < size; id++) {
	  printf ("/* %ld */ \"%s\",\n", (long)id, SXWORD_get (amalgam_or_compound_component_names, id));
...
SXINT hd, next;\n\
	...
	printf ("static SXINT amlgm_component_stack [%ld] = {", (long)amlgm_component_stack [0]+1);
 
	for (x = 0; x <= amlgm_component_stack [0]; x++) {
	  if (x % 10 == 0)
	    printf ("\n/* %ld */ ", (long)x);

	  printf ("%ld, ", (long)amlgm_component_stack [x]);
	}
	...
	for (x = 0; x < amalgam_size; x++) {
	  printf ("/* %ld */ {%ld, %ld},\n", (long)x, (long)amlgm_list [x].hd, (long)amlgm_list [x].next);
	}
	...
	printf ("static SXINT ac_id2amlgm_list [%ld] = {", (long)size);
 
	for (id = 0; id < size; id++) {
	  if (id % 10 == 0)
	    printf ("\n/* %ld */ ", (long)id);

	  printf ("%ld, ", (long)ac_id2amlgm_list [id]);
	}
	...
	for (x = 0; x < compound_size; x++) {
	  printf ("/* %ld */ {%ld, %ld},\n", (long)x, (long)cmpnd_list [x].hd, (long)cmpnd_list [x].next);
	}
	...
	printf ("static SXINT cc2cmpnd_list [%ld] = {", (long)XH_top (XH_cc_list));
 
	for (id = 0; id <  XH_top (XH_cc_list); id++) {
	  if (id % 10 == 0)
	    printf ("\n/* %ld */ ", (long)id);

	  printf ("%ld, ", (long)cc2cmpnd_list [id]);
	}
	...
	printf ("/* %ld */ \"%s\",\n", (long)am_id, SXWORD_get (amalgam_names, amalgam_ids [am_id]));
      }
...
/* 0 */ 0, ", (long)max_id+1);
	...
	for (smppass_if_id = 1; smppass_if_id <= max_id; smppass_if_id++) {
	  ...
	  if (smppass_if_id%10 == 0)
	    printf ("\n/* %ld */", (long)smppass_if_id);

	  printf ("%ld, ", (long)t_list_id);
	}
...
/* 0 */ 0, 1, ", (long)lim+1);
	...
	for (i = 1; i < lim; i++) {
	  ...
	  for (cur = smppass_bot; cur < smppass_top; cur++) {
	    printf ("%ld, ", (long)XH_list_elem (stack, cur));
	  }
...
#define sxtonodiacritics(c) ((SXD+128)[c])
	  ...
	  for (smppass_id_2 = pred_id; smppass_id_2 < smppass_top_2; smppass_id_2++) {
	    ...
	    printf ("#define %s_id %ld\n",  varstr_tostr (vstr), (long)smppass_id_2);
	  }
	}
...
SXINT lexeme, sous_cat1, sous_cat2;\n\
	...
	out_ifdef ("pred_vals");
	printf ("\nstatic struct pred_val pred_vals [%ld] = {{0, 0, 0},", (long)XH_top (pred_val_hd));
	...
	for (id = 1 ; id < XH_top (pred_val_hd); id++) {
	  ...
	  printf ("{%ld, %ld, %ld}, ",
		  (long)XH_list_elem (pred_val_hd, smppass_bot), (long)XH_list_elem (pred_val_hd, smppass_bot+1), (long)XH_list_elem (pred_val_hd, smppass_bot+2));
	}
	...
	out_ifdef ("sous_cat");
	printf ("\nstatic SXINT sous_cat [%ld] = {0,", (long)XH_top (sous_cat_hd)+1);

	for (id = 1 ; id <= XH_top (sous_cat_hd); id++) {
	  if (id%10==1)
	    printf ("\n/* %ld */ ", (long)id);

	  printf ("%ld, ", (long)XH_X (sous_cat_hd, id));
	}
	...
	printf ("\nstatic SXINT sous_cat_list [%ld] = {0,", (long)XH_list_top (sous_cat_hd));
	...
	for (id = 1 ; id <= XH_top (sous_cat_hd); id++) {
	  ...
	  for (smppass_bot = XH_X (sous_cat_hd, id), smppass_top = XH_X (sous_cat_hd, id+1); smppass_bot < smppass_top; smppass_bot++) {
	    printf ("%ld, ", (long)XH_list_elem (sous_cat_hd, smppass_bot));
	  }
	}
	...
	printf ("\nstatic SXINT compound_field [%ld] = {0,", (long)XH_top (compound_fields_hd)+1);

	for (id = 1 ; id <= XH_top (compound_fields_hd); id++) {
	  if (id%10==1)
	    printf ("\n/* %ld */ ", (long)id);

	  printf ("%ld, ", (long)XH_X (compound_fields_hd, id));
	}
	...
	printf ("\nstatic SXINT compound_field_list [%ld] = {0,", (long)XH_list_top (compound_fields_hd)+1);

	for (id = 1 ; id < XH_top (compound_fields_hd); id++) {
	  printf ("\n/* %ld */ ", (long)id);

	  for (smppass_bot = XH_X (compound_fields_hd, id), smppass_top = XH_X (compound_fields_hd, id+1); smppass_bot < smppass_top; smppass_bot++) {
	    printf ("%ld, ", (long)XH_list_elem (compound_fields_hd, smppass_bot));
	  }
	}
	...
	printf ("\nstatic SXINT priority_hd [%ld] = {0,", (long)XH_top (XH_priority_hd)+1);

	for (id = 1 ; id <= XH_top (XH_priority_hd); id++) {
	  if (id%10==1)
	    printf ("\n/* %ld */ ", (long)id);

	  printf ("%ld, ", (long)XH_X (XH_priority_hd, id));
	}
	...
	printf ("\nstatic SXINT priority_list [%ld] = {0,", (long)XH_list_top (XH_priority_hd)+1);

	for (id = 1 ; id < XH_top (XH_priority_hd); id++) {
	  printf ("\n/* %ld */ ", (long)id);

	  for (smppass_bot = XH_X (XH_priority_hd, id), smppass_top = XH_X (XH_priority_hd, id+1); smppass_bot < smppass_top; smppass_bot++) {
	    printf ("%ld, ", (long)XH_list_elem (XH_priority_hd, smppass_bot));
	  }
	}
	...
	printf ("\nstatic SXINT inflected_form2display [%ld] = {", (long)max_id+2);

	for (id = 0; id <= max_id+1; id++) {
	  if (id%10 == 0)
	    printf ("\n/* %ld */", (long)id);

	  printf ("%ld, ", (long)id2display [id]);
	}
...
SXINT t, list;\n\
	...
	printf ("\nstatic struct if_id2t_list if_id2t_list [%ld] = {", (long)smppass_top+1);
	...
	for (smppass_x = 0; smppass_x <= smppass_top; smppass_x++) {
	  ...
	  printf ("{%ld, %ld}, ", (long)if_id2t_list [smppass_x].t, (long)if_id2t_list [smppass_x].structure_arg);
	}
...
SXINT lexeme, struct_id, priority;\n\
	...
	printf ("\nstatic struct structure_args_list structure_args_list [%ld] = {", (long)smppass_top+1);
	...
	for (smppass_x = 0; smppass_x <= smppass_top; smppass_x++) {
	  ...
	  printf ("{%ld, %ld, %ld}, ",
		  (long)structure_args_list [smppass_x].lexeme,
		  (long)struct_id,
		  (long)structure_args_list [smppass_x].priority);
	}
...
/* The operator \"lex\" is used within the associated LFG */\n", stdout);
	  ...
	  printf ("\nstatic SXINT lexeme_id2display [%ld] = {", (long)top2+1);

	  for (lexeme_id = 0; lexeme_id <= top2; lexeme_id++) {
	    if (lexeme_id%10 == 0)
	      printf ("\n/* %ld */ ", (long)lexeme_id);

	    printf ("%ld, ", (long)lexeme_id2display [lexeme_id]);
	  }
	  ...
	  printf ("\nstatic SXINT args_ref_list [%ld] = {", (long)nb+1);

	  for (smppass_x_2 = 0; smppass_x_2 <= nb; smppass_x_2++) {
	    if (smppass_x_2%10 == 0)
	      printf ("\n/* %ld */ ", (long)smppass_x_2);

	    printf ("%ld, ", (long)args_ref_list [smppass_x_2]);
	  }
...
/* 0 */ NULL, NULL, ", max_field_id+1);
	...
	printf ("\nstatic unsigned char field_id2kind [%ld] = {", (long)smppass_top);
	...
	printf ("\nstatic char *field_id2string [%ld] = {", (long)smppass_top);
...

	{
	  ...
	  printf ("\nstatic SXINT field_id2pass_nb [%ld] = {", (long)smppass_top);
	  ...
	  printf ("\nstatic char *atom_id2string [%ld] = {", (long)max_unbounded_atom_id+1);
...
", (long)max_field_id+1);
	  for (field_id = EMPTY_STE+2; field_id <= max_field_id; field_id++) {
	    ...
	    if (field_kind & ATOM_KIND) {
	      printf ("/* %s = %ld */ &(atom_id_set_for_%s [0]),\n", SXWORD_get (field_names, field_id), (long)field_id, SXWORD_get (field_names, field_id));
	    }
	    else
	      printf ("/* %s = %ld */ NULL,\n", SXWORD_get (field_names, field_id), (long)field_id);
	  }
	  ...
	  printf ("\nstatic char *lexeme_id2string [%ld] = {", (long)smppass_top);
...
void
lfg_lex_smp (SXINT what, struct sxtables *sxtables_ptr)
{
  switch (what) {
  case OPEN:
