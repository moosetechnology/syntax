...
#include "sxunix.h"
#include <ctype.h>
char WHAT_LFGSMP[] = "@(#)SYNTAX - $Id: lfg_smp.c 1108 2008-02-26 09:40:21Z rlacroix $" WHAT_DEBUG;
...
static void
fill_leaves_field_kind (NODE *visited)
{
  SXINT field_id;
  ...
  */
  ...
  case COMPLETE_n :/* VISITED->name = PAIR_S_n */
    /* Le 20/06/05 Ca permet specifier la liste des non-terminaux dont les f_structures doivent etre complete et coherentes */
    {
      ...
      complete_name2ste = (SXINT*) sxcalloc (VISITED->degree+2, sizeof (SXINT));
...
# CATEGORY\n", vocabulary_file);  
  ...
  for (prod = 1; prod <= prodmax; prod++) {
	...
	nt2nt_names [nt] = X+1;
	fprintf (vocabulary_file, "# /* %ld */ %s;\n", (long)nt, SXWORD_get (nt_names, X+1));
      }
    }
  }
...
/* 0 */ 0, ", (long)top+1);
	...
	fprintf (output_file, "\n/* %ld */ ", (long)i);
      ...
      fprintf (output_file, "%ld, ", (long)dfield_pairs2field_id [i]);
    }
...
/* 0 */ 0, ", (long)top+1);
	...
	fprintf (output_file, "\n/* %ld */ ", (long)i);
      ...
      fprintf (output_file, "%ld, ", (long)datom_pairs2atom_id [i]);
    }
  ...
  if (dfield_pairs2field_id || datom_pairs2atom_id) {
    ...
    printf ("\nstatic char *suffix_id2string [%ld] = {", (long)top);
  ...
  printf ("\nstatic SXINT operator_atom2atom_list [%ld] = {0, ", (long)XH_top (atom_hd)+1);
  sxinitialise (top);
  ...
  out_ifdef ("atom_list");
  printf ("\nstatic SXINT atom_list [%ld] = {0,", (long)top+1);

  for (i = 1; i < XH_top (atom_hd);  i++) {
    bot = XH_X (atom_hd, i);
    top = XH_X (atom_hd, i+1);

    printf ("\n/* %ld (%ld..%ld) */ ", (long)i, (long)bot, (long)top-1);

    while (bot < top) {
      printf ("%ld, ", (long)XH_list_elem (atom_hd, bot));
      bot++;
    }
  }
...
/* 0 */ 0, ", output_file);
  ...
  for (i = 1; i < XH_top (prod_hd); i++) {
    ...
    fprintf (output_file, "%ld, ", (long)prod2rule [i]);
  }
...
/* 0 */ 0, ", output_file);
  ...
  for (i = 1; i <= rule_nb; i++) {
    ...
    fprintf (output_file, "%ld, ", (long)rule2next_rule [i]);
  }
...
/* 0 */ 0, ", (long)XH_list_top (code_stack));
...

  {
	...
	fprintf (stdout, "/* %ld: %s */ \"%s\", \n", (long)nt, SXWORD_get (nt_names, nt_id), sxstrget (ste));
	nb = 0;
      }
