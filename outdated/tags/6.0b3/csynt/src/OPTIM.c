...
/************************************************************************/
/* 18-01-93 11:20 (pb):		Bug (non raz de xt_to_act) au cours de	*/
/*				l'utilisation de plulong s'il y a des	*/
/*				predicats.				*/
/************************************************************************/
/* 15-04-92 13:30 (pb):		Non execution des predicats sur reduce	*/
...
char WHAT_OPTIM[] = "@(#)SYNTAX - $Id: OPTIM.c 1103 2008-02-25 15:22:05Z rlacroix $" WHAT_DEBUG;
...

static BOOLEAN	row_le (SXINT i, SXINT j)
{
...
static BOOLEAN	row_eq (SXINT i, SXINT j)
{
    SXINT	li, vali, valj;
    struct row		*arowi = rows + i, *arowj = rows + j;
    struct P_item	*ai, *aj;
...
static SXINT	put_in_reductions (SXINT *put_in_reductions_hashs, SXINT *put_in_reductions_lnks, struct floyd_evans *afe)
{
    SXINT	hash, x, y;
...
static BOOLEAN	is_0_indistinguable (struct floyd_evans *afei, struct floyd_evans *afej)
{
    ...
    case Prdct:
    default:
	fputs ("ERROR: UNKNOWN CODOP ", sxtty);
	sxexit(1);
    }
...
static BOOLEAN	is_k_indistinguable (struct floyd_evans *afei, struct floyd_evans *afej)
{
    /* Regarde (dans le cas [Scan]Shift) si les etats references par les productions de
       ...
       etre 0_indistinguables */

    switch (afei->codop) {
    case Shift:
    case ScanShift:
	return t_state_equiv_class [afei->next] == t_state_equiv_class [afej->next]
	  && nt_state_equiv_class [afei->next] == nt_state_equiv_class [afej->next];
...
static BOOLEAN	is_stable (SXINT *equiv_class,
			   struct state	*states,
			   struct P_item *items,
			   struct floyd_evans *fe,
			   SXINT *partitions,
			   ...
			   SXINT *partition_no)
{
    ...
    struct P_item	*aitemi, *aitemj;
    SXINT		new_part;
    BOOLEAN	is_a_new_partition = FALSE;
...
static SXVOID	insert_xt_lnk (SXINT xt, struct xt_to_act *xt_to_act)
{
    SXINT	prev, next, prdct;

    for (next = xt_to_act [prev = XT_TO_T_CODE (xt)].lnk, prdct = XT_TO_PRDCT_CODE (xt); next != 0; prev = next, next =
	 xt_to_act [next].lnk) {
	if (XT_TO_PRDCT_CODE (next) < prdct)
	    break;
    }

    xt_to_act [xt].lnk = next;
    xt_to_act [prev].lnk = xt;
}



static SXVOID	insert_xnt_lnk (SXINT xnt, struct xt_to_act *xnt_to_act)
{
    SXINT	prev, next, prdct;
...
SXVOID	fe_to_equiv (struct floyd_evans *afe1, struct floyd_evans *afe2)
{
    SXINT	k;
...

SXINT	OPTIM (char *language_name)
{
    if (sxverbosep) {
	fprintf (sxtty, "%s\n", ME);
	sxttycol1p = TRUE;
    ...
    /* Passage 3.9 => 5.0 */
    prdct_list_top = mgerme = Mgerme = 0;
    nd_degree = -1; 
...
/* Le floyd_evans des transitions terminales, non-terminales et des predicats est regenere
   dans une structure unique tnt */
	...
	tnt_fe = (struct floyd_evans*) sxalloc (i = *t_lnks + *nt_lnks + xpredicates + 1, sizeof (struct floyd_evans));
	tnt_lnks = (SXINT*) sxalloc (i, sizeof (SXINT));
	tnt_fe_to_new = (SXINT*) sxcalloc (i, sizeof (SXINT));
	reductions = (struct P_reduction*) sxalloc (i, sizeof (struct P_reduction));
	lnks = (SXINT*) sxalloc (i, sizeof (SXINT));
...

	{
		...
		if ((l = (astate = nt_states + i)->lgth) > 0) {
			...
			for (aitem = old_aitem; aitem < lim; aitem++) {
			    /* On commence par remplir nt_set et xnt_to_act */
			    if (aitem->action != 0) {
				xnt = aitem->check;
			...
			for (nt = sxba_scan (nt_set, 0); nt != -1; nt = sxba_scan (nt_set, nt)) {
			    sxba_0_bit (nt_set, nt);

			    if ((xnt = (pact = xnt_to_act + nt)->lnk) != 0) {
				/* Traitement des predicats */
				for (xnt = nt; xnt != 0; xnt = xnt_to_act [xnt].lnk) {
				    build_branch (XNT_TO_PRDCT_CODE (xnt), xnt_to_act [xnt].action, 0);
				}

				action = gen_tree (0, nt_fe);
			    }
			    else {
				/* Cas simple */
...
/* Attention le test de "EOF" doit etre conserve sur l'instruction "Halt" */
	...
	xt_to_act = (struct xt_to_act*) sxcalloc (any + 1, sizeof (struct xt_to_act));
	xt_items = 1;
	...
	for (i = 1; i < xe1; i++) {
		...
		if (xt_state_set != NULL && sxba_bit_is_set (xt_state_set, i)) {
			...
			/* On commence par remplir t_set et xt_to_act */

			if (aitem->action != 0) {
			    xt_to_act [xt = aitem->check].action = aitem->action;
			    sxba_1_bit (t_set, xt);
			}
		    }

		    /* On regarde les predicats */
		    xt = -bnf_ag.WS_TBL_SIZE.tmax;

		    while ((xt = sxba_scan (t_set, xt)) > 0) {
			if (t_fe [action = xt_to_act [xt].action].codop == Reduce) {
			    t = XT_TO_T_CODE (xt);

			    if (!sxba_bit_is_set (t_set, t) ||
				xt_to_act [t].action == action) {
				/* Candidat a la suppression */
				SXINT xt1 = xt;

				while ((xt1 = sxba_scan (t_set, xt1)) > 0 &&
				       t == XT_TO_T_CODE (xt1) &&
				       xt_to_act [xt1].action == action);

				if (xt1 < 0) {
				    if (!sxba_bit_is_set (t_set, t)) {
					/* C'est le representant */
					sxba_1_bit (t_set, t);
					xt_to_act [t].action = action;
				    }
				    ...
				    do {
					sxba_0_bit (t_set, xt1);
					xt_to_act [xt1].action = 0;
				    } while ((xt1 = sxba_scan (t_set, xt1)) > 0 &&
					   t == XT_TO_T_CODE (xt1));
				}
			    }
...
/* Calcul de plulong */
		    ...
		    if (lim->action == t_error) {
			for (t = sxba_scan (t_set, 0); t != -1; t = sxba_scan (t_set, t)) {
			    action = (pact = xt_to_act + t)->action;

			    if ((xt = pact->lnk) != 0 /* Traitement des predicats */ )
				/* Les sequences de predicats seront generees, elles ne comptent
				   donc pas dans le calcul de plulong */
				continue;

			    if ((afe = t_fe + action)->codop == Reduce && afe->reduce != 0 /* pas Halt */  && ++reduces [
				 action] > reduces [plulong])
				plulong = action;
			}
		    }
			...
			if ((xt = (pact = xt_to_act + t)->lnk) != 0) {
			    /* Traitement des predicats */
			    for (xt = t; xt != 0; xt = xt_to_act [xt].lnk) {
				build_branch (XT_TO_PRDCT_CODE (xt), xt_to_act [xt].action, 0);
			    }

			    action = gen_tree (0, t_fe);
			}
			else {
			    if (plulong == pact->action) {
				pact->lnk = pact->action = 0;
				continue;
			    }
			    else {
				/* Cas simple */
				fe_to_equiv (tnt_fe, t_fe + pact->action);
				action = put_in_fe (tnt_fe, tnt_hash, tnt_lnks, tnt_fe);
			    }
			}
		    ...
		    if (lim->action == t_error) {
			for (aitem = old_aitem; aitem < lim; aitem++) {
			    if (aitem->action != 0 && (afe = t_fe + aitem->action)->codop == Reduce && afe->reduce != 0
				 && ++reduces [aitem->action] > reduces [plulong])
				plulong = aitem->action;
			}
		    }

		    for (aitem = old_aitem; aitem < lim; aitem++) {
			if (aitem->action != 0) {
			    reduces [aitem->action] = 0;

			    if (plulong != aitem->action) {
				fe_to_equiv (tnt_fe, t_fe + aitem->action);
				t_items [xt_items].check = aitem->check;
				t_items [xt_items++].action = put_in_fe (tnt_fe, tnt_hash, tnt_lnks, tnt_fe);
				astate->lgth++;
			    }
			}
		    }
...
/* On remplit reductions et tnt_fe_to_new pour les transitions terminales
   ...
   l'interpretation dynamique (a l'analyse) du nombre de depillages est different et seule
   l'adresse permet de savoir si on a a faire a une transition terminale ou non terminale */
	...
	for (i = nt_fe_size + 1; i <= *tnt_lnks; i++) {
		...
		tnt_fe_to_new [i] = put_in_reductions (hashs
, lnks, afe);
	}
...
/*   L I N E A R I S A T I O N   D E S   M A T R I C E S   */
	...
	SXINT	*state_to_state, *witness, xw;
	SXBA	bases_set;
...

	{
	    ...
	    pour le cas ou on manipule des XT (prudence), est change par : */
	    algoV (FALSE, xe1, bnf_ag.WS_TBL_SIZE.xtmax, xt_items, t_states, (struct item*) t_items,
		   (struct base*) t_bases, (struct item**) &vector, &bases_set, &MIN, &MAX, &delta);
   ...
   elevee. */ 

	{
		...
		if ((l = nt_states [i].lgth) > 0) {
			...
			if (aitem->action != 0) {
			    if ((k = ++(occur_nb [aitem->action])) > (nt = occur_nb [occur_nb [0]])
				|| (k == nt && aitem->action > occur_nb [0]))
	    ...
	    algoV (FALSE, xe1, -bnf_ag.WS_TBL_SIZE.tmax, xt_items, t_states, t_items,
	    t_bases, &vector, &bases_set, &MIN, &MAX, &delta); 
	    pour le cas ou on manipule des XNT (prudence), est change par : */
	    algoV (TRUE, xac2, bnf_ag.WS_TBL_SIZE.xntmax, xnt_items, nt_states, (struct item*) nt_items,
		   (struct base*) nt_bases, (struct item**) &vector, &bases_set, &MIN, &MAX, &delta);
...

    ...
    /* Nouvelle version recopiee de xcsynt/OPTIM de la version 5.0 */
    /* Sans ARC ni non-determinisme */
/* IDEE:
...
/* On complete tnt_fe_to_new */
	...
	for (lim = (fe = tnt_fe + (i = 1)) + tnt_lnks [0]; fe < lim; fe++, i++) {
		...
		case ScanShift:
		case Shift:
		    j = XxY_is_set (&ref_to_ee, nt_state_equiv_class [fe->pspl], t_state_equiv_class [fe->next])
		      + ((fe->codop == LaScanShift) ? Mref : Mprdct);
...
/* Permet, a l'analyse (par dichotomie), de retrouver un predicat a partir
   du couple (red_no, position). */
	    SXINT	item, lim, v, red_no, pos, prdct;
	...
	SXINT	*old_t_state_to_new, *old_nt_state_to_new;
	SXBA	state_set;
	struct floyd_evans	*fe, *lim;
	struct P_base	*old_t_bases, *old_nt_bases;
	struct P_item	*old_vector;
...
/* On remplit new_t_state_to_old, new_nt_state_to_old, old_t_state_to_new et old_nt_state_to_new*/

	new_t_state_to_old = (SXINT*) sxcalloc (xe1 + 1, sizeof (SXINT));
	new_nt_state_to_old = (SXINT*) sxcalloc (xac2 + 1, sizeof (SXINT));
	old_t_state_to_new = (SXINT*) sxcalloc (xe1 + 1, sizeof (SXINT));
	old_nt_state_to_new = (SXINT*) sxcalloc (xac2 + 1, sizeof (SXINT));
	state_set = sxba_calloc (xe1 + 1);

/* Les etats tels que pspl == next sont traites en premier */

	Mtstate = 0;

	for (lim = (fe = tnt_fe + 1) + tnt_lnks [0]; fe < lim; fe++) {
	    if (fe->codop == Shift || fe->codop == ScanShift) {
		if ((i = nt_state_equiv_class [fe->pspl]) > 0 && i == t_state_equiv_class [fe->next] && !sxba_bit_is_set (
		     state_set, i)) {
		    sxba_1_bit (state_set, i);
		    ++Mtstate;
		    new_nt_state_to_old [Mtstate] = new_t_state_to_old [Mtstate] = i;
		    old_nt_state_to_new [i] = old_t_state_to_new [i] = Mtstate;
		}
	    }
	}

	Mref = Mntstate = Mtstate;


/* On complete new_t_state_to_old et old_t_state_to_new */

	for (i = 1; i < xe1; i++) {
	    if (t_bases [i].defaut != 0 && !sxba_bit_is_set (state_set, i)) {
		new_t_state_to_old [++Mtstate] = i;
		old_t_state_to_new [i] = Mtstate;
	    }
	}


/* On complete new_nt_state_to_old et old_nt_state_to_new */

	for (i = 1; i < xac2; i++) {
	    if (nt_bases [i].defaut != 0 && !sxba_bit_is_set (state_set, i)) {
		new_nt_state_to_old [++Mntstate] = i;
		old_nt_state_to_new [i] = Mntstate;
	    }
	}
...
/* On calcule la plus petite reference dans vector */
	    ...
	    for (i = 1, abase = t_bases + 1; i <= Mtstate; i++, abase++) {
		*abase = old_t_bases [new_t_state_to_old [i]];
	    ...
	    for (i = 1, abase = nt_bases + 1; i <= Mntstate; i++, abase++) {
		*abase = old_nt_bases [new_nt_state_to_old [i]];
...
/* On remplit gotos */
	...
	gotos = (struct P_goto*) sxalloc (tnt_lnks [0] - Mred + 1, sizeof (struct P_goto));
	Mgotos = 0;
	M0ref = 0;

	for (lim = (fe = tnt_fe + (i = 1)) + tnt_lnks [0]; fe < lim; fe++, i++) {
	    if (tnt_fe_to_new [i] == 0 && (fe->codop == Shift || fe->codop == ScanShift))
		if (nt_state_equiv_class [fe->pspl] == 0) {
		    j = old_t_state_to_new [t_state_equiv_class [fe->next]];
		    M0ref = max (M0ref, j);
		}
		else if (nt_state_equiv_class [fe->pspl] != t_state_equiv_class [fe->next]) {
		    ++Mgotos;
		    tnt_fe_to_new [i] = (fe->codop == ScanShift) ? Mprdct + Mgotos : mMprdct - Mgotos;
		    gotos [Mgotos].state = fe->pspl;
		    gotos [Mgotos].next = fe->next;
		}
	}
...
/* On complete tnt_fe_to_new */
	...
	for (lim = (fe = tnt_fe + (i = 1)) + tnt_lnks [0]; fe < lim; fe++, i++) {
	    if (tnt_fe_to_new [i] == 0 && (fe->codop == Shift || fe->codop == ScanShift)) {
		j = old_t_state_to_new [t_state_equiv_class [fe->next]] + ((nt_state_equiv_class [fe->pspl] == 0) ? Mfe :
		     M0ref);
		tnt_fe_to_new [i] = (fe->codop == Shift) ? -j : j;
	    }
	}
...
/* Modifications de gotos */

	    for (i = 1, afe = gotos + 1; i <= Mgotos; i++, afe++) {
		afe->state = ((j = nt_state_equiv_class [afe->state]) < 0) ? tnt_fe_to_new [-j] : Mref +
		     old_nt_state_to_new [j];
		afe->next = old_t_state_to_new [t_state_equiv_class [afe->next]];
	    }
	}
	...
	parser_ag.constants.Mntstate = Mntstate;
	parser_ag.constants.Mvec = Mvec;
	parser_ag.constants.Mgotos = Mgotos;
	parser_ag.constants.deltavec = deltavec;
	parser_ag.constants.init_state = old_t_state_to_new [init_state];
	parser_ag.constants.mMprdct = mMprdct;
	parser_ag.constants.mMred = mMred;
	parser_ag.constants.mMrednt = mMrednt;
	parser_ag.constants.Mprdct = Mprdct;
	...
	parser_ag.nt_bases = nt_bases;
	parser_ag.vector = vector;
	parser_ag.gotos = gotos;
	parser_ag.prdcts = prdcts;
	parser_write (&parser_ag, language_name);
	parser_free (&parser_ag);
	sxfree (tnt_fe_to_new), tnt_fe_to_new = NULL;
	sxfree (old_nt_state_to_new);
	sxfree (old_t_state_to_new);
	sxfree (new_nt_state_to_old), new_nt_state_to_old = NULL;
	sxfree (new_t_state_to_old), new_t_state_to_old = NULL;
    }
#endif /* 0 */
