...
char WHAT_NBEST_SEMACT[] = "@(#)SYNTAX - $Id: nbest_semact.c 1135 2008-03-05 15:32:38Z sagot $" WHAT_DEBUG;
...
static BOOLEAN
nbest_args_decode (SXINT *pargnum, SXINT argc, char *argv[])
{
...
static BOOLEAN Aij_or_item_kth_best (SXINT Aij_or_item, SXINT k);
...
print_ctxt (SXINT item)
{
  ...
  case EP_POS_EPCFG:
    ...
    printf ("[val=%ld, pos=%ld]", (long)val, (long)pos);
    break;
  case EP_EPCFG:
    print_init_prod (init_prod);
    printf ("[val=%ld]", (long)val);
    break;
  case P_EPCFG:
    print_init_prod (init_prod);
    break;
...
{
  SXINT        Pij, Tpq, t, ff_id, lex_id, init_prod, A, full_ctxt, couple, tok_no;
  SXINT        nb;
  SXINT        *tok_no_stack;
  double       weight;
  struct rhs   *prhs;
...
static SXINT
one_best_post_bu_walk (SXINT Aij, BOOLEAN status)
{
  SXINT               dedication;
#if LOG
  double            tree_nb;
...
{
  SXINT               Aij, next, signature, bot, item, Xpq, Pij, PijK;
  struct dedication *dedication_ptr;
  ...
  while (next) {
    ...
    while ((Xpq = spf.outputG.rhs [item++].lispro) != 0) {
      ...
      if (Xpq > 0)
	printf ("[%ld] ", (long)XH_list_elem (signatures_hd, bot));
...
nbest_print_forest (SXINT Aij_or_item, SXINT cur_contextual_proba_kind, SXINT k)
{
  ...
  if (cur_contextual_proba_kind > EPCFG) {
    ...
    spf_topological_top_down_walk (spf.outputG.rhs [Aij_or_item].lispro, print_decorated_ctxt_prod, NULL, (SXINT(*)(SXINT, BOOLEAN))NULL);
  }
  else
...
output_best_weights (SXINT k, double weight)
{
  ...
  fprintf (best_weights_file, "#%ld %ld %ld %f\n", (long)sentence_id, (long)k, (long)contextual_proba_kind, weight);
}
...
nbest_sem_pass_with_cpk (SXINT root_Aij, SXINT kind)
{
  ...
  if (is_print_time) {
    ...
    sprintf (str, "\t%ld best, -cp=%ld, best_weight = %f (TRUE)", (long)real_nbest, (long)contextual_proba_kind, best_forest_weight);
    sxtime (ACTION, str);
...
nbest_sem_pass (void)
{
  ...
  if (nbest) {
    ...
    if (is_print_time) {
      sprintf (str, "\t%ld best (SKIPPED)", (long)nbest);
      sxtime (ACTION, str);
    }
  }
...
SXINT
nbest_perform (SXINT root_Aij, SXINT h /* k */, SXINT b /* beam */ , SXINT cur_contextual_proba_kind, BOOLEAN do_filtering)
{
