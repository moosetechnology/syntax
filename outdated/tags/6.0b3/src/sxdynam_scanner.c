...
char WHAT_SXDYNAM_SCANNER[] = "@(#)SYNTAX - $Id: sxdynam_scanner.c 1015 2008-02-13 08:37:43Z rlacroix $" WHAT_DEBUG;
...
static void
sxds_lvcpy (SXINT j, SXINT i, BOOLEAN all)
{
	...
	if (plv_j->counters == NULL) {
	    plv_j->counters = (long*) sxcalloc ((SXUINT) cs + 1, sizeof (long));		
	}
...
static SXINT
sxds_clone (SXINT old, SXINT current_state, BOOLEAN all)
{
...
static void
SetConflict (struct configs *pconfigs, BOOLEAN check_ambiguity)
{
...
sxdynam_scanner (SXINT	what_to_do)
{
	...
	if (SXDS->counters_size >= 0)
		...
		(long*) sxcalloc ((SXUINT)SXDS->counters_size + 1, sizeof (long));
