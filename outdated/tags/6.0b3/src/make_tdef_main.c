...
#include "sxunix.h"
char WHAT_TDEFMAKETDEFMAIN[] = "@(#)SYNTAX - $Id: make_tdef_main.c 1188 2008-03-11 11:06:21Z sagot $" WHAT_DEBUG;
    ...
    struct sxtables	*arg;
{
  ...
  case ACTION:
	...
	if (code <= 1)
	  sxput_error (tok->source_index,
		       "%sThis unknown %sterminal symbol \"%s\" is ignored.", err_titles [1], (cas == 1) ? "" : "non-", tnt_name);
	else
