...
char WHAT_XXYXZ_MNGR[] = "@(#)SYNTAX - $Id: XxYxZ_mngr.c 1009 2008-02-12 15:18:48Z rlacroix $" WHAT_DEBUG;
...
BOOLEAN	XxYxZ_set (XxYxZ_header *header, SXINT X, SXINT Y, SXINT Z, SXINT *ref)
{
    /* Si l'element X-Y-Z de la matrice creuse header existe, cette fonction
...
BOOLEAN XxYxZ_write (XxYxZ_header *header, filedesc_t F_XxYxZ /* file descriptor */)
{
    SXINT		bytes, kind;
    SXINT		*lnk_hd, *lnk;
...
BOOLEAN XxYxZ_read (XxYxZ_header *header, 
		    filedesc_t F_XxYxZ /* file descriptor */, 
		    char *name, 
		    void (*user_oflw) (SXINT, SXINT), 
		    FILE *stat_file)
{
...
void XxYxZ_array_to_c (XxYxZ_header *header, FILE *F_XxYxZ /* named output stream */, char *name)
{
    ...
    fprintf (F_XxYxZ, "\n\nstatic struct XxYxZ_elem %s_display [%ld] = {", name, (long)header->size + 1);
	...
	fprintf (F_XxYxZ, "{%ld, %ld, %ld}, ",
		 (long)header->display [i].X, (long)header->display [i].Y, (long)header->display [i].Z);
    } 
	...
	if (kind < 3) {
		...
		sprintf (sub_name + i, "s_X_hd_%ld", (long)kind);
		...
		memcpy (sub_name, name, (size_t)i);
		sprintf (sub_name + i, "_XxY_hd_%ld", (long)kind - 3);
	...
	if (size != 0) {
		...
		fprintf (F_XxYxZ, "%ld, ", (long)header->lnk_hd [kind] [i]);
	    } 
		...
		fprintf (F_XxYxZ, "%ld, ", (long)header->lnk [kind] [i]);
	    } 
...
void XxYxZ_to_c (XxYxZ_header *header, FILE *F_XxYxZ /* named output stream */, char *name, BOOLEAN is_static)
{
