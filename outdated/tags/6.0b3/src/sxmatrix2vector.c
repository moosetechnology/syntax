...
char WHAT_SXMATRIX2VECTOR[] = "@(#)SYNTAX - $Id: sxmatrix2vector.c 1023 2008-02-13 08:39:22Z rlacroix $" WHAT_DEBUG;
...
struct %s_vector {\n\
  ...
  fprintf (cfile, "\n\nstatic SXINT %s_disp [%ld] = {", name, (long)vector->last_line + 1);

  for (i = 0; i <= vector->last_line; i++) {
    if ((i % 10) == 0)
      fprintf (cfile, "\n/* %ld */ ", (long)i);

    fprintf (cfile, "%ld, ", (long)vector->disp [i]);
  } 

  fprintf (cfile, "\n};\n");

  if (vector->list) {
    fprintf (cfile, "\n\nstatic struct %s_velem %s_list [%ld] = {", name, name, (long)vector->last_index + 1);
  ...
  if (vector->matrix) {
    fprintf (cfile, "\n\nstatic %s %s_matrix [%ld] = {", vector_type, name, (long)vector->last_mindex + 1);
...
%s%s\n\
	   ...
	   is_static ? "static " : "",
	   name, name,
	   (long)vector->last_line, (long)vector->last_column, (long)vector->last_index, (long)vector->last_mindex,
	   name,
