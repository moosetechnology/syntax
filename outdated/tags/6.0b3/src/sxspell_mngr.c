...
char WHAT_SXSPELL_MNGR[] = "@(#)SYNTAX - $Id: sxspell_mngr.c 1165 2008-03-07 15:32:14Z sagot $" WHAT_DEBUG;
...
sxspell_init_fsa (char *mot, SXINT lgth, struct FSA *FSA)
{
  ...
  if (FSA->orig_char_set == NULL) {
    ...
    XxY_alloc (&(FSA->Q_hd), (char*)mot, 10*lgth+1, 1, FSA->dico->private.from_left_to_right /* Xforeach */, !(FSA->dico->private.from_left_to_right) /* Yforeach */, area_oflw, NULL);
    ...
    if (BASIZE (FSA->final_set) <= (SXUINT)FSA->Q_nb)
...
static char *replace_strings [256];
static SXBA_fdcl (replace_set, 256);

#define unicode ((SXINT) (char) '\201')
...
sxspell_no_repeat (struct FSA *FSA, SXINT p, SXINT nr_weight)
{
  ...
  for (pair = 1; pair <= FSA->Q_init-p; pair = pair2) {
    car = (SXINT) FSA->word [pair-1];

    n = 1;

    for (pair2 = pair+1; pair2 < FSA->Q_init; pair2++) {
      if (car != (SXINT) FSA->word [pair2-1])
	break;
...
*/
void
sxspell_replace (struct FSA *FSA, char *R[], SXINT R_size, SXINT replace_weight)
{
  SXINT           i, car, pair;
  SXBA          char_set;
  char new_car, *line;
...
void
sxspell_replace_n_p (struct FSA *FSA, char *R[], SXINT R_size, SXINT replace_weight)
{
  SXINT           n, p, i, j, top_j, k, car, pair, new_j, prefix_lgth, suffix_lgth, min_n_p, true_n;
  SXBA          new_char_set, char_set;
  char true_car, true_rcar, *line, *left_line, *right_line, *replace_line, *mot, *r_line;
...
sxspell_swap (struct FSA *FSA, SXINT swp_weight)
{
  ...
  for (p = 1; p <= top_state; p++) {
    ...
    while ((car = sxba_scan (char_set, car)) >= 0) {
      weight = new_char_weight [car] = get_new_compose_weight (char_weight [car], cur_weight, (SXINT)1);
    }
...
static void
store_sub_word (SXINT id, struct weight weight, BOOLEAN is_an_old_id)
{
...
static void dico_walk (char cur_char,
...
FSA_walk (SXINT p, SXINT q)
{
  ...
  if (dico_process_base (p, q)) /* si on est arrivé au bout du FSA (au cours de ce test, on store le
  ...
  if (is_suffix) {
	...
	while ((cur_char = sxba_scan (char_set, cur_char)) >= 0) {
	  if ((char) cur_char != source_char)
	    dico_walk ((char) cur_char, char_weight [cur_char], p, r);
	}
      }
    }
	...
	while ((cur_char = sxba_scan (char_set, cur_char)) >= 0) {
	  if ((char) cur_char != source_char)
	    dico_walk ((char) cur_char, char_weight [cur_char], r, q);
	}
      }
    }
	   ...
	   SXINT q)
{
  ...
  if ((class = char2class [cur_char])) { /* class est non nul */
	...
	if (stop_mask_hit || FSA_walk_complete) {
	  if (stop_mask_hit) {
	    /* ... et le mot est là ds le dico, et n'est le préfixe d'aucun autre mot du dico */
...
static SXINT
merge_compare (SXINT *p1, SXINT *p2)
{
...
/* modèle de correcteur appelé par -DEMO */
#ifdef EMO
