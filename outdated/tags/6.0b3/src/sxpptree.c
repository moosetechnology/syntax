...
Version du Mardi 20 janvier 2004 (Philippe Deschamp):
Adaptation à Linux 64 bits.

Version du Mercredi 7 mai 2003 (Philippe Deschamp):
Adaptation à SGI (64 bits).
...
char WHAT_SXPPTREE[] = "@(#)SYNTAX - $Id: sxpptree.c 1000 2008-02-11 15:24:52Z rlacroix $" WHAT_DEBUG;

#define USER_PTR	SXINT*		/* Any pointer */
...
static VOID	shift_subtree (struct node *node, long s)
{
    struct node	*son;
    SXINT	count;
...
static VOID	place (USER_PTR user_ptr, struct line **line_ptr, SXINT level, BOOLEAN is_left_son)
{
    struct line	*line;
    struct node	*node;
    long	left_pos;
...
/* On le place au milieu de ses fils, en les recadrant si necessaire */
	...
	if (!first_node_on_this_line_p) {
	    long	shift_q = left_pos - node->pos.left;
...
static VOID	replace (struct line *line, SXINT level, BOOLEAN is_left_son, BOOLEAN first_time_on_this_level)
{
    struct node		*node;
    long	left_pos;
    SXINT		sons_nb;
...
/* On le place au milieu de ses fils, en les recadrant si necessaire */
	...
	{
	    long	val = ((node->pos.l_son_middle + node->pos.r_son_middle + 1) >> 1) - node->pos.middle;
...
static VOID	catenate_copy (struct string *string, char character, long number)
{
    char	*str;
...
static VOID	rateau (void)
{
	...
	if (node->pos.middle >= start_pos) {
	  ...
	  /* prepare_arrows: 
		(pas de goto sur ce label, ce qui provoque un warning du compilo icc d'intel) */
	  {
	    long	spaces_nb = node->pos.middle - pos;
...
{
    long	current_key_length, remaining_space;
