...
#include <ctype.h>
#include <string.h>
char WHAT_RCGGENBNF[] = "@(#)SYNTAX - $Id: rcg_gen_bnf.c 1112 2008-02-26 12:54:01Z rlacroix $" WHAT_DEBUG;
...
gen_bnf_rule (void)
{
	...
	for (x = 1; x <= top; x++) {
		...
		if (symb < predef_nt_nb) {
			...
			k = Ak_A2k (Ak, A);
			fprintf (bnf_file, "<%s[%ld]> ", sxstrget (nt2ste [A]), (long)k+1);
		    }
		    else {
...
static void
pre_substitute (SXINT lhs_bot3, SXINT lhs_top3, BOOLEAN is_spcl)
{
