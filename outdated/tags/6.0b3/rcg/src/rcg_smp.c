...
#include <string.h>
#include <stdlib.h>
char WHAT_RCGSMP[] = "@(#)SYNTAX - $Id: rcg_smp.c 1127 2008-02-29 13:47:48Z syntax $" WHAT_DEBUG;
...
static SXINT		max_ste, cur_var, lhs_var, lhs_prdct, cur_prdct;
...
rcg_pi (void)
{
	...
	if (!is_first_pass) {
		...
		SXINT lahead = VISITED->son->token.lahead;
...
rcg_pd (void)
{
	...
	if (!is_first_pass) {
		...
		sxput_error (VISITED->token.source_index,
	      ...
	      SXINT t = VISITED->father->father->father->son->token.lahead;
...
#if 0
static BOOLEAN first_last_arg_walk (SXINT clause, 
				    SXINT cur, 
				    SXINT top, 
				    SXINT bot2, 
				    SXINT top2, 
...
static SXINT
walk_memel (SXINT iarg_bot, SXINT iarg, BOOLEAN with_Ak)
{
  ...
  if (!IS_ERROR) {
	...
	if (arity >= 2) {
	  ...
	  while (bot < top) {
	    ...
	    fprintf (listing, "%s(%ld) [%ld == %ld]\n", sxstrget (nt2ste [nt]), (long)nt, (long)I+1, (long)J+1);
    ...
    for (nt = 1; nt <= max_nt; nt++) {
      if (SXBA_bit_is_set (is_lhs_nt, nt) && nt2identical [nt] == nt) {
	fprintf (listing, "%s(%ld) [", sxstrget (nt2ste [nt]), (long)nt);
	arity = nt2arity [nt];
	Ak = A_k2Ak (nt, 0);

	for (x = 1; x <= arity; x++) {
	  fprintf (listing, "(<%ld..%ld>, %ld)", (long)Ak2min [Ak], (long)Ak2max [Ak], (long)Ak2len [Ak]);
	  Ak++;
...
smppass (void)
{
	...
	if (!IS_ERROR) {
	  SXINT smppass_nt, Ak, arity, k, smppass_clause_2, main_clause;
