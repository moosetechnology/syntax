...
/*                                                           */
/*                                                           */
/*  This program has been translated from lecl_css.pl1       */
/*  on Wednesday the twelfth of March, 1986, at 14:45:25,    */
...
#include "lecl_node.h"
char WHAT_LECLCSS[] = "@(#)SYNTAX - $Id: lecl_css.c 606 2007-06-01 14:04:31Z rlacroix $" WHAT_DEBUG;

extern BOOLEAN	lecl_check_octal_code ();
static void 	prdct_processing ();
static int	buck_no, val, cur_xcld_no, cur_kw_no;




static VOID act_processing (visited, name, param)
...
static void	prdct_processing (visited, name, param)
    struct lecl_node	*visited;
    char	*name;
    int		param;
{
...

static VOID st_cs (visited)
    struct lecl_node	*visited;
{
    struct lecl_node	*brother, *son_1;
    int		old_exclude_size, old_keyword_size;;
    ...
    case ACTION_ERASE_n  :
	act_processing (visited, "@Erase", 0);
	return;
	...
	if (byte_length == 0 || byte_length > 8) {
	    sxput_error (visited->token.source_index,
	      "%s%d is outside the range one to eight allowed for a byte length specification; the value eight is taken."
		 , err_titles [1], byte_length);
	    byte_length = 8;
	}
    ...
    {
	register int l;
	register char *str = (char*) sxalloc ((l = sxstrlen (visited->token.string_table_entry) + 1) + 2, sizeof (char));
    ...
    case IS_FIRST_COL_n  :
	prdct_processing (visited, "&Is_First_Col", 0);
	return;

    case IS_LAST_COL_n  :
	prdct_processing (visited, "&Is_Last_Col", 0);
	return;
    ...
    case LOWER_CASE_n  :
	act_processing (visited, "@Lower_Case", 0);
	return;

    case LOWER_TO_UPPER_n  :
	act_processing (visited, "@Lower_To_Upper", 0);
	return;

    case MARK_n  :
	act_processing (visited, "@Mark", 0);
	return;
    ...
    case RELEASE_n  :
	act_processing (visited, "@Release", 0);
	return;
   ...
   de contextes */
    ...
    case UPPER_CASE_n  :
	act_processing (visited, "@Upper_Case", 0);
	return;

    case UPPER_TO_LOWER_n  :
	act_processing (visited, "@Upper_To_Lower", 0);
	return;
	...
	if (word_length > 32 || word_length == 0) {
	    sxput_error (visited->token.source_index,
    "%s%d is outside the range one to thirty-two allowed for a word length specification; the value thirty_two is taken."
		 , err_titles [1], word_length);
	    word_length = 32;
	}
