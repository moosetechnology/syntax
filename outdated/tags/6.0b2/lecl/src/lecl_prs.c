...
#include "lecl_ag.h"
#include "lecl_nn.h"
char WHAT_LECLPRS[] = "@(#)SYNTAX - $Id: lecl_prs.c 579 2007-05-24 11:30:48Z rlacroix $" WHAT_DEBUG;



static int	xclass, i, codofcar, class_no, xtc;
static SXBA	/* char_max */ char_set;
static struct lecl_node		*brother;



static VOID	maj (visited)
...
/* we check that each character (defined in the source language) has a
   representation in the target machine */
	...
	if (visited->token.string_table_entry != 0)
	    for (i = 1; i <= atoi (sxstrget (visited->token.string_table_entry)); i++) {
		maj (visited);
		classe [xclass] = 1;
		/* illegal char */
