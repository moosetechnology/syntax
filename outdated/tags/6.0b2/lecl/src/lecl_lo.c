...
/*                                                                */
/*                                                                */
/*  This program has been translated from lecl_listing_output.pl1 */
/*  on Tuesday the twenty-ninth of April, 1986, at 11:34:58,      */
...
#include "lecl_ag.h"
#include "put_edit.h"
char WHAT_LECLLO[] = "@(#)SYNTAX - $Id: lecl_lo.c 601 2007-05-30 10:43:19Z rlacroix $" WHAT_DEBUG;
...
/* extern long	time (); */
extern VOID	scramble_listing_output ();
extern char	*options_text ();
extern VARSTR	lecl_gen_cc ();
extern VARSTR	lecl_gen_sc ();
extern VOID	equality_sort ();
extern char	by_mess [], release_mess [];
static int	context_table_slice_size, i, j, k, l, x, max_slice_no, slice_no, tmin, tmax, min_re, origine, prev,
...
static int	*to_be_sorted /* 1:Smax */ ;
...
static char	*cat_red_name (cat_red_name_s, cat_red_name_j)
    char	*cat_red_name_s;
    int		cat_red_name_j;
{
    if (cat_red_name_j < 0)
...
static char	*get_master_name (get_master_name_s, ner)
    char	*get_master_name_s;
    int		ner;
{
    *get_master_name_s = NUL;
    return (cat_red_name (get_master_name_s, ers_disp [ers_disp [ner].master_token].reduc));
}
static char	*get_re_name (get_re_name_s, ner)
    char	*get_re_name_s;
    int		ner;
{
    int		mner;
...
static char	*cv_x (cv_x_s, cv_x_x)
    char	*cv_x_s;
    int		cv_x_x;
{
    ...
    sprintf (cv_x_s, "%-d", cv_x_x);
    return cv_x_s;
}



static char	*cv_delete (cv_delete_s, del)
    char	*cv_delete_s;
    BOOLEAN	del;
{
    if (del)
...
static char	*cv_priority (cv_priority_s, kind)
    char	*cv_priority_s;
    char	kind /* bit (3) */ ;
{
    *cv_priority_s = NUL;
...
static int	cv_to_t_code (ner)
    int		ner;
{
    int		ate, cv_to_t_code_t;
...
static VOID	put_body (put_body_stmt)
    transition_matrix_item	put_body_stmt;
{
    char	message [64];
    int		put_body_next = NEXT (put_body_stmt);
    int		codop = CODOP (put_body_stmt);
    ...
    case SameState:
    case State:
	sprintf (message, "GOTO STATE %-d", put_body_next);
	break;

    case ActPrdct:
	switch (action_or_prdct_code [put_body_next].kind) {
	case IsPredicate:
	    sprintf (message, "GOTO PRDCT %-d", put_body_next);
	    break;

	case IsAction:
	    sprintf (message, "GOTO ACTION %-d", put_body_next);
	    break;

	case IsNonDeter:
	    sprintf (message, "GOTO NONDETER %-d", put_body_next);
	    break;
	default:
#if EBUG
...
static char	*gen_action_or_prdct_name_from_fe (gen_action_or_prdct_name_from_fe_s, gen_action_or_prdct_name_from_fe_fe)
    char	*gen_action_or_prdct_name_from_fe_s;
    struct action_or_prdct_code_item	gen_action_or_prdct_name_from_fe_fe;
{
	...
	if (gen_action_or_prdct_name_from_fe_fe.is_system) {
	    ...
	    case 3:
		/* Set */
		sprintf (gen_action_or_prdct_name_from_fe_s, "@Set (%-d)", gen_action_or_prdct_name_from_fe_fe.param);
		break;

	    case 4:
		/* Reset */
		sprintf (gen_action_or_prdct_name_from_fe_s, "@Reset (%-d)", gen_action_or_prdct_name_from_fe_fe.param);
		break;
		...
		if (gen_action_or_prdct_name_from_fe_fe.param == 0)
		    ...
		    sprintf (gen_action_or_prdct_name_from_fe_s, "@Include (Post_Action : @%-d)", gen_action_or_prdct_name_from_fe_fe.param);
	    ...
	    case 9:
		/* Put */
		strcat (strcat (strcpy (gen_action_or_prdct_name_from_fe_s, "@Put (\""), CHAR_TO_STRING (gen_action_or_prdct_name_from_fe_fe.param)), "\")");
		break;
	    ...
	    case 12:
		/* Incr */
		sprintf (gen_action_or_prdct_name_from_fe_s, "@Incr (%-d)", gen_action_or_prdct_name_from_fe_fe.param);
		break;

	    case 13:
		/* Decr */
		sprintf (gen_action_or_prdct_name_from_fe_s, "@Decr (%-d)", gen_action_or_prdct_name_from_fe_fe.param);
		break;
	    default:
#if EBUG
	    ...
	    sprintf (gen_action_or_prdct_name_from_fe_s, "@%-d", gen_action_or_prdct_name_from_fe_fe.action_or_prdct_no);
	}
    }
	...
	if (gen_action_or_prdct_name_from_fe_fe.is_system) {
	    ...
	    case 2:
		/* Is_Set */
		sprintf (gen_action_or_prdct_name_from_fe_s, "&Is_Set (%-d)", gen_action_or_prdct_name_from_fe_fe.param);
		break;

	    case 3:
		/* Is_Reset */
		sprintf (gen_action_or_prdct_name_from_fe_s, "&Is_Reset (%-d)", gen_action_or_prdct_name_from_fe_fe.param);
		break;
	...
	sprintf (gen_action_or_prdct_name_from_fe_s, "ND%-d", gen_action_or_prdct_name_from_fe_fe.action_or_prdct_no);
    }
...
static BOOLEAN	less_prdct (less_prdct_i, less_prdct_j)
    int		less_prdct_i, less_prdct_j;
{
...
static BOOLEAN	compare_kw (compare_kw_i, compare_kw_j)
    int		compare_kw_i, compare_kw_j;
{
    char	*si, *sj;

    for (si = sxstrget (keywords [compare_kw_i].string_table_entry), sj = sxstrget (keywords [compare_kw_j].string_table_entry); *si != NUL &&
	 *si == *sj; si++, sj++)
	;
...
char	*strrevcpy (destination, source, size)
...
VOID	lecl_list_out ()
{
	...
	for (i = 1; i <= max_re; i++) {
		...
		for (x = origine + 1; x < ers_disp [i + 1].prolon; x++) {
			...
			if (*(lecl_list_out_s + 1) == 'P') {
			    /* @Put */
			    put_edit_nnl (51, "@Put (\"");
			    put_edit_apnnl (CHAR_TO_STRING (*(lecl_list_out_s + 6)));
			    put_edit_apnnl ("\")");
			}
			else
	...
	if (xprdct_to_ate > prdct_false_code) {
	    int		*lecl_list_out_to_be_sorted /* 1:xprdct_to_ate */ ;
	    int		prdct_no, lecl_list_out_i;
	    BOOLEAN	done;

	    lecl_list_out_to_be_sorted = (int*) sxalloc ((int) (xprdct_to_ate + 1), sizeof (int));
...
/* sortie des mot-cles */
    ...
    if (nbndef > 0) {
	int	*sort_kw /* 1:nbndef */ ;
	int	*tk_defining_kw /* 1:current_token_no+1 */ ;
	int	kwsize, tksize, lecl_list_out_x, lecl_list_out_t, xtk, ner;

	sort_kw = (int*) sxalloc ((int) (nbndef + 1), sizeof (int));
	tk_defining_kw = (int*) sxalloc ((int) (current_token_no + 2), sizeof (int));
	xtk = 0;
...
/* on trie les mot-cles par ordre alphabetique */
...

	{
	    ...
	    tk = (char**) sxalloc ((int) (xtk + 1), sizeof (char*));

	    for (i = 0; i <= xtk; i++)
		tk [i] = (char*) sxalloc ((int) (tksize + 1), sizeof (char));
	    ...
	    *s = NUL;
		...
		sprintf (line, "%3d  | %-*s | ", lecl_list_out_t, kwsize, sxstrget (keywords [l].string_table_entry));
		k = kwsize + 10;
...
/* context_table_edition: */
...

	{
	    ...
	    int		lecl_list_out_i, lecl_list_out_j, lecl_list_out_k, tn, lecl_list_out_x;

	    ren = (char**) sxalloc ((int) (tmax - tmin + 2), sizeof (char*)) + 1;

	    for (lecl_list_out_i = tmin - 1; lecl_list_out_i <= tmax; lecl_list_out_i++)
		ren [lecl_list_out_i - tmin] = (char*) sxalloc ((int) (maxsize + 1), sizeof (char));
	    ...
	    *s = NUL;
		...
		if (ers_disp [lecl_list_out_i].prolon != ers_disp [lecl_list_out_i + 1].prolon) {
		    sprintf (line, "%-*s | ", resize, get_re_name (line, lecl_list_out_i));
	...
	for (i = 1; i < xfsa2; i++) {
	    ...
	    put_edit_apnnl (",del=");
	    put_edit_apnb (fsa [i].del);
	    put_edit_apnnl (",scan=");
	    put_edit_apnb (fsa [i].scan);
	    put_edit_ap ("]");
	    put_edit_nl (1);
		...
		put_edit_apnnl (",");
		put_edit_nb (33, item [j].attributes.kind);
		put_edit_ap ("]");
	    }
