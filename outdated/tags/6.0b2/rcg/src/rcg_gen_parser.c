...
#endif
char WHAT_RCGGENPARSER[] = "@(#)SYNTAX - $Id: rcg_gen_parser.c 643 2007-06-21 09:35:02Z rlacroix $" WHAT_DEBUG;


extern BOOLEAN		is_in_Tstar (int, int);
extern int              clause2lhs_nt (int clause);
extern BOOLEAN          rhs_arg2sub_arg (int lhs_prdct, int arg);
...
static int		max_degree, max_degree_clause, *for_loops_nb;
static int		Aisize_nb, ASindex_nb, Avar_nb, Arhs_arg_nb, Arhs_prdct_nb, Aetc_nb, Alhs_t_nb;
static BOOLEAN		Acut, Anon_range, is_source_checked, rhs_arg_nb;
static int		max_tooth, max_son_nb, max_generator_pos; 
static int		lhs_bot2, lhs_top2, lhs_arity, rhs_prdct_nyp_nb, wtop, D, varstrings_top, next_ii, lhs_t_nb;
static int		max_equation, max_adr, max_atom, lhs_equation, eq_top, Sindex;
...
static int		A_clause_nb, clause_id, lhs_arg_tbp, not_substituted_nb;
static VARSTR		*varstrings, wvstr, vstr_cond, vstr_body, t_indexes_vstr;

static int		**equation, *eq_list, *equation2Xnb, *equation_stack, *equation_stack2;
static int		*equation2vstr_kind;
...
static int		*lhs_pos2adr, *lhs_pos2ladr, *lhs_pos2radr;
...
static int		*rhs_pos2Ak, *rhs_pos2nt, *rhs_pos2param, *rhs_pos2lmadr, *rhs_pos2rmadr;
static int		*rhs_pos2rhs_mpos;
...
static int		*adr2cur, *adr2madr, *madr2olb, *madr2oub, *adr2lA, *adr2uA, *adr2pos, *adr_stack, *adr2tpos; 
static SXBA		madr_set, lmadr_set, rmadr_set, tmadr_set, synchro_set, is_madr_an_ob;
static int		*synchro2adr;
...
static int		*olbs, *oubs;

static int		*atom2min, *atom2max, *gatom2nb, *atom_stack, *atom2var_nb, *atom2static_size;
static SXBA		Asize_set;
static VARSTR		*atom2rsize;
static XxY_header	VxV;

static int		*ext_decl2clause;
static SXBA		ext_decl_set, ext_decl_clause_set, clause_set, clause_has_ext_decl;

static int		*var2atom, *var2nb, *var2lhs_pos;

static XH_header	XH_prdct_args;
static int		*XH_prdct_args2mprdct_pos;

static int		*prdct_pos2mprdct_pos, *mprdct_pos2generated, *rhs_prdct_pos2tooth, *rhs_prdct_pos2generator_pos;
static SXBA		rhs_prdct_nyp;

static int		max_component_nb;
static int		*component2pos, *component2bot, *component2top, *component2lmadr, *component2rmadr;

static int		*Indexes, *clause2lhst_disp;
...
static int		*clause2clause_id, max_clause_id, *A2max_clause_id;

static SXBA             *t2clause_set; 
/* ---------------------------------------------------------------------------------------------------- */

static void
XH_prdct_args_oflw (old_size, new_size)
int old_size, new_size;
{
    XH_prdct_args2mprdct_pos = (int*) sxrealloc (XH_prdct_args2mprdct_pos, new_size+1, sizeof (int));
}

SXVOID
equation_oflw ()
{
  int 	pos, new_top = eq_top*2;
  int		*ptr, *ptr_top;

  equation = (int**) sxrealloc (equation, new_top+1, sizeof (int*));
  ptr = eq_list = (int*) sxrealloc (eq_list, (new_top+1)*(max_gvar+1)+1, sizeof (int));
  equation2Xnb = (int*) sxrealloc (equation2Xnb, new_top+1, sizeof (int));
  equation_stack2 = (int*) sxrealloc (equation_stack2, new_top+1, sizeof (int));
  equation_stack = (int*) sxrealloc (equation_stack, new_top+1, sizeof (int));
  ...
  equation2vstr_kind = (int*) sxrealloc (equation2vstr_kind, new_top+1, sizeof (int));
...
static VARSTR
prdct2varstr (vstr, prdct)
    VARSTR	vstr;
    int 	prdct;
{
  int bot, cur, top, nt, param, bot2, cur2, top2, symb;
...
static VARSTR
clause2varstr (vstr, clause)
    VARSTR	vstr;
    int 	clause;
{
  int rhs, bot, cur, top, ste;
...
static void
gen_nt_text (gen_nt_text_parser_file, A)
     FILE *gen_nt_text_parser_file; 
     int A;
{
  char	*p1;
...
static void
gen_t_text (gen_t_text_parser_file, t)
     FILE *gen_t_text_parser_file;
     int t;
{
  char	c, *p1, *p;
...
static void
gen_var_text (gen_var_text_parser_file, var)
     FILE *gen_var_text_parser_file; 
     int var;
{
  fprintf (gen_var_text_parser_file, "X%i", var-1);
}

static int *lhs_symb_disp, *lhs_symb_nb, *rhs_arg_pos_list;

static void
seek (lhs_prdct, rhs_bot3, i, rhs_arg_pos)
    int lhs_prdct, rhs_bot3, i, rhs_arg_pos;
{
  /* On note toutes les occur du sous arg rhs_bot3, i de la RHS ds la LHS */
  int seek_lhs_bot2, seek_lhs_top2, lhs_cur2, lhs_param, lhs_bot3, lhs_top3, j, lhs_cur3, lhs_symb, rhs_symb, lhs_symb_pos;
...
gen_Xoccur ()
{
  ...
  int		Xoccur_top, local_top, nt, n, l;
  int 	i, clause, lhs_prdct, gen_Xoccur_lhs_bot2, lhs_cur2, gen_Xoccur_lhs_top2, lhs_param, lhs_bot3, lhs_top3, lhs_cur3;
  int		rhs, rhs_top, rhs_bot, rhs_cur, rhs_prdct, rhs_bot2, rhs_cur2, rhs_top2, rhs_param, rhs_bot3, rhs_top3, rhs_cur3, rhs_symb, rhs_arg_pos;
  int		*Xoccur_disp;
  int		nb, symb_nb, lhs_symb_pos, bot, x;
  ...
  lhs_symb_disp = (int*) sxalloc (l+1, sizeof (int));
  lhs_symb_nb = (int*) sxalloc (l+1, sizeof (int));
  rhs_arg_pos_list = (int*) sxalloc (l*max_rhs_arg_nb+1, sizeof (int));
  ...
  Xoccur_disp = (int*) sxalloc (last_clause+2, sizeof (int));
  Xoccur_disp [0] = 0;

  Indexes = (int*) sxalloc (last_clause+1, sizeof (int));

  fputs ("static int Xoccur [] = {0,\n", parser_file);
    
  Xoccur_disp [Xoccur_top = 1] = 1;
    
  for (clause = 1; clause <= last_clause; clause++) {
    fprintf (parser_file, "/* %i */", clause);
  ...
  fputs ("static int Xoccur_disp [] = {0,\n\
/* 1 */ 1,", parser_file);
  ...
  for (clause = 1; clause <= last_clause; clause++) {
    ...
    fprintf (parser_file, "/* %i */ %i, ", clause+1, Xoccur_disp [clause+1]);
  }
...
static void
gen_ret_vals (A_clause_set)
  SXBA	A_clause_set;
{
  /* Si is_generator, permet de connaitre la composition des args des clauses */
  int		ret_vals_top, Indexes_top;
  int		clause, rhs, rhs_top, rhs_bot, local_top, rhs_cur, rhs_prdct, rhs_bot2, nt; 
  ...
  fprintf (parser_file, "int\tret_vals[%i];\n", ret_vals_top);

  fputs ("#if is_generator\n", parser_file);
  fprintf (parser_file, "int\tIndexes[%i];\n", Indexes_top);
  fputs ("#endif\n", parser_file);
}


static void
gen_prdct_text (gen_prdct_text_parser_file, prdct)
     FILE *gen_prdct_text_parser_file; 
     int prdct;
{
  int bot, cur, top, nt, param, bot2, cur2, top2, symb;
  ...
  while (cur < top) {
	...
	if (nt == STRLEN_ic && cur == bot+2) {
	  fprintf (gen_prdct_text_parser_file, "%i", symb);
	}
	else {
	  if (symb == 0) {
	    if (cur == bot+2 && (nt == FIRST_ic || nt == LAST_ic))
	      gen_t_text (gen_prdct_text_parser_file, 0); /* eof */
	    else
	      fputs ("...", gen_prdct_text_parser_file);
	  }
	  else {
...
rec_clauses ()
{
  ...
  int		clause, prdct, A, rhs, bot, cur, top, nt;
    ...
    char	*exit_stmt;
{
  ...
  if (is_static_phase) {
    ...
    fprintf (parser_file, "if (%s != %i) %s;\n",
	     varstr_tostr (equation2vstr [xeq]), tnb, exit_stmt);
  }
...
static BOOLEAN
substitute_atom_in_eq (substitute_atom_in_eq_equation_stack2, exit_stmt)
    int		*substitute_atom_in_eq_equation_stack2;
    char	*exit_stmt;
{
  /* On substitue (la taille de) atom ds les autres equations */
  /* On cherche l'equation qui a l'atom max */
  int		xeq, atom, atom2, Xnb, Xnb2, xeq2, i, v, kind1, kind2, kind; 
  int		*eq_ptr, *eq_ptr2;
  BOOLEAN	is_useful = TRUE;
  char	string [16];
...
set_Asize_set (exit_stmt)
    char	*exit_stmt;
{
  int		atom, xeq, Xnb, nb, kind, atom2;
  int		*eq_ptr;
  BOOLEAN	is_assign;
  ...
  while (!IS_EMPTY (atom_stack)) {
	...
	if (nb > 2) {
	  if (xeq >= lhs_arity) {
	    ...
	    fprintf (parser_file, "if ((isize[%i] = %s) < 0) %s;\n",
		     next_ii, varstr_tostr (vstr), exit_stmt);
	    /* le < 0 peut etre change en < k, avec k==taille min connue de ce qui reste, sans
	       tenir compte des tailles egales */
#if 0
	    fprintf (parser_file, "isize[%i] = %s;\n",
		     next_ii, varstr_tostr (vstr));
#endif
	    sprintf (string, "isize[%i]", next_ii);
...
static BOOLEAN
put_new_equation (prev_xeq, xeq, k1, bot1, cur1, k2, bot2, cur2)
    int 	prev_xeq, xeq, k1, bot1, cur1, k2, bot2, cur2;
{
  int 	cur3, top3, tnb, Xnb, atom, symb, xeq2;
  int		*eq_ptr = equation [xeq], *eq_ptr2;
  BOOLEAN	ret_val;
...
make_atom ()
{
  int		cur_var, var, next_var, atom, nb, couple; 
...
static BOOLEAN
is_lexicalized (clause)
    int clause;
{
  int lhs_prdct, is_lexicalized_lhs_bot2, is_lexicalized_lhs_top2, cur2, param, cur3, top3;
...
static BOOLEAN
is_cyclic_clause_fun (clause)
    int clause;
{
  int lhs_prdct, is_cyclic_clause_fun_lhs_bot2, lhs_nt, param;
  int rhs, cur, bot, top, prdct, cur2, bot2, top2, nt, pos, symb, cur3, bot3, top3;
  int lsymb, rcur, rprdct, rcur2, rbot2, rnt, rtop2, Ak;
...
static BOOLEAN
fill_param_pos (clause)
    int clause;
{
  /* Collecte les renseignements sur clause */
  int		xeq, xeq2, prev_xeq, pos, pos2, lpos, rpos, mpos;
  int		adr, adr1, adr2, ladr, radr, lmadr, rmadr, top_adr;
  int		bot, cur, top, bot2, cur2, lcur2, rcur2, top2;
  int		bot3, lbot3, rbot3, top3, ltop3, rtop3, cur3, lcur3, rcur3;
  int		arg1, arg2;
  int		symb, prev_symb, lsymb, rsymb, lsymb1, rsymb1, lsymb2, rsymb2;
  int		atom, matom;
  int		lhs_prdct, lhs_nt, prdct, rhs, nt, i, k, param, len, couple;
  int		min, tnb, Xnb, xargs, rhs_prdct_pos, mprdct_pos;
  int		madr, tpos;
  int		lgth;
  int		lA, uA, n, j, cur_wtop;
  int		lparam, rparam; 
  int		pos1, top_adr1, radr1, bot1, top1, top_adr2, radr2, cur1;
  int		item1, param1, item2, param2, inside_etc_nb, Ak, which_arg, rhs_arity, rhs_pos;
  int		xbot, xtop, IJ, I, J;
  int		*eq_ptr, *eq_ptr2;
  ...
  if (is_some_equiv) {
	...
	for (adr2 = lhs_pos2adr [pos2]; adr2 < top_adr2; adr2++) {
	  if (adr2madr [adr2] == madr) {
	    ...
	    if (put_new_equation (0, xeq, 1, bot1, item1, -1, bot2, item2)) {
	      sprintf (string, "ilb[%i]-ilb[%i]", pos2, pos1);
	    }
	    else {
	      sprintf (string, "ilb[%i]-ilb[%i]", pos1, pos2);
	    }
	    ...
	    if (put_new_equation (0, xeq, 1, item1, top1, -1, item2, top2)) {
	      sprintf (string, "iub[%i]-iub[%i]", pos1, pos2);
	    }
	    else {
	      sprintf (string, "iub[%i]-iub[%i]", pos2, pos1);
	    }
  ...
  for (cur = bot; cur < top; cur++) {
	...
	if (SXBA_bit_is_set (pos_is_a_range, pos) &&
	  ...
	  if (XH_list_elem (rcg_parameters, bot3) != 0) {
	    ...
	    sprintf (string, "olb[%i]", pos-lhs_arity);
	  ...
	  if (XH_list_elem (rcg_parameters, top3-1) != 0) {
	    ...
	    if (adr < 0)
	      sxtrap (ME, "fill_param_pos");
#endif
	    sprintf (string, "oub[%i]", pos-lhs_arity);
	  ...
	  if (!SXBA_bit_is_set (pos_is_a_range, pos)) {
		...
		if (!is_in_Tstar (bot3, top3)) {
		  for (cur3 = bot3; cur3 < top3; cur3++) {
		    ...
		    if (symb > 0) {
		      adr = get_lhs_ladr (0, symb);

		      for (i = 0; i <= 1; i++) {
			sprintf (string, "X%sb[%i]", i == 0 ? "l" : "u", symb-1);
			madr = adr2madr [adr+i];
  ...
  if (max_component_nb) {
	...
	if (cur3 == XH_X (rcg_parameters, param)) {
	  ...
	  if (SXBA_bit_is_reset_set (is_madr_an_ob, madr)) {
	    ...
	    sprintf (string, "odddb[%i]", inside_etc_nb);
	    inside_etc_nb++;
		...
		for (adr2 = lhs_pos2adr [pos2]; adr2 < top_adr2; adr2++) {
		  if (adr2madr [adr2] == lmadr) {
			...
			if (bot1 < cur1 || bot2 < cur2) {
			  if (put_new_equation (prev_xeq, xeq, 1, bot1, cur1,
						-1, bot2, cur2)) {
...
static void
gen_StrEq_spcl (prdct_pos, arg_pos, bot3, top3, exit_string, is_pos_call)
    int         prdct_pos, arg_pos, bot3, top3;
    char        *exit_string;
    BOOLEAN     is_pos_call;
{
    int len, cur3, symb, tooth, i;
    ...
    case 1:
        ...
        if (is_pos_call) {
            fprintf (parser_file, "if (!SCAN (oub[%i],%i)) %s;\n",
                     arg_pos, -symb, exit_string);

            if (is_drcg && (tooth = rhs_prdct_pos2tooth [prdct_pos]) > 0) {
                /* Pour definite part eventuel */
                fprintf (parser_file, "rho[%i] = -oub[%i];\n", tooth+1, arg_pos);
            }
        }
        else
            fprintf (parser_file, "if (oub[%i]-olb[%i]==1 && SCAN (oub[%i], %i)) %s;\n",
                     arg_pos, arg_pos, arg_pos, -symb, exit_string);

        break;

    default:
        fprintf (parser_file, "local_val = olb[%i];\n", arg_pos);

        if (is_pos_call)
            fputs ("if (", parser_file);
        else
            fprintf (parser_file, "if (oub[%i]-olb[%i]==%i &&\n", arg_pos, arg_pos, len);
        ...
        do {
            symb = XH_list_elem (rcg_parameters, cur3++);
            len--;
            fprintf (parser_file, "(%sLSCAN (local_val+%i, %i)) %s\n",
                     is_pos_call ? "!" : "=", i, -symb, len ? (is_pos_call ? "||" : "&&") : "");
...
static BOOLEAN
arg_evaluated (nt, is_first_arg, rhs_param_pos, cur2)
    int         nt, rhs_param_pos, cur2;
    BOOLEAN     is_first_arg;
{
    int param, bot3, cur3, top3, symb, adr, lmadr, rmadr;
...
static void
rhs_processing (level, clause, exit_string)
    int         level, clause;
    char        *exit_string;
{
  /* On traite les predicats dont tous les args sont connus */
  int         rhs, bot, cur, top, rhs_param_pos, rhs_prdct_pos, prdct, bot2, cur2, top2, nt, arity;
  int         lmadr, rmadr, pos;
  int         botc, curc, topc, rhs_processing_component_nb, tooth, generator_pos;
  int         param, bot3, top3, l, cur3, symb;
  ...
  if (rhs_prdct_nyp_nb) {
	...
	if (nt < 0 && nt != CUT_ic) {
	  ...
	  if (SXBA_bit_is_set (rhs_prdct_nyp, rhs_prdct_pos)
		...
		fprintf (parser_file, "rho[%i] = MAKE_ij (olb[%i], oub[%i]); /* &True () */\n",
			 rhs_prdct_pos2tooth [rhs_prdct_pos]+1, pos, pos);
		fputs ("#endif\n", parser_file);
		rhs_prdct_nyp_nb--;
		...
		if ((!range1 && !tstar1) || (!range2 && !tstar2)) {
		  ...
		  case STREQ_ic:
		    ...
		    if (!range1) {
		      ...
		      bot3 = XH_X (rcg_parameters, param);
		      top3 = XH_X (rcg_parameters, param+1);
		      fprintf (parser_file, "static int *parg1, rcg_arg1 [] = {%i,", top3-bot3);
		    ...
		    if (!range2) {
		      ...
		      bot3 = XH_X (rcg_parameters, param);
		      top3 = XH_X (rcg_parameters, param+1);
		      fprintf (parser_file, "static int *parg2, rcg_arg2 [] = {%i,", top3-bot3);
		    ...
		    fprintf (parser_file, "if (%s_StrEq2 (&(rho[%i]), ",
		    ...
		    if (tstar1)
		      ...
		      if (range1) {
			fprintf (parser_file, "oub[%i]-olb[%i], glbl_source_olb[%i]+1, ",
				 pos, pos, pos);
		      }
		      else {
		  ...
		  case STRLEN_ic:
		    ...
		    bot3 = XH_X (rcg_parameters, param);
		    top3 = XH_X (rcg_parameters, param+1);
		    fprintf (parser_file, "static int rcg_arg2 [] = {%i,", top3-bot3);
		    ...
		    param = XH_list_elem (rcg_predicates, bot2+2);
		    bot3 = XH_X (rcg_parameters, param);
		    fprintf (parser_file, "if (_StrLen2 (rcg_arg2, Xlb, Xub) == %i) %s;\n", l, exit_string);
		  ...
		  case STREQLEN_ic:
		    ...
		    if (!(range1 || tstar1)) {
		      ...
		      fprintf (parser_file, "static int rcg_arg1 [] = {%i,", top3-bot3);
		    ...
		    if (tstar1) {
		      ...
		      bot3 = XH_X (rcg_parameters, param);
		      top3 = XH_X (rcg_parameters, param+1);
		      fprintf (parser_file, "%i", top3-bot3);
		    }
		    else {
		      if (range1) {
			fprintf (parser_file, "oub[%i]-olb[%i]",
				 pos, pos);
		      }
		      else {
		  ...
		  if (XH_list_elem (rcg_predicates, bot2) == 0) {
...
#if 0
		      fprintf (parser_file, "if (_StrEq (&(rho[%i]), &(olb[%i]), &(oub[%i]))==-1) %s;\n",
			       rhs_prdct_pos2tooth [rhs_prdct_pos]+1, pos, pos, exit_string);
#endif
		      /* Modif le Ven 22 Dec 2000 11:44:55 */
		      fprintf (parser_file, "if (_StrEq (&(olb[%i]), &(oub[%i]))==-1) %s;\n",
			       pos, pos, exit_string);
		    }
			...
			if ((param = XH_list_elem (rcg_predicates, bot2+2))) {
			  ...
			  fprintf (parser_file, "if (oub[%i]-olb[%i]==0) %s;\n",
				   pos+1, pos+1, 
				   exit_string);
		      }
		      else {
#if 0
			fprintf (parser_file, "if (oub[%i]-olb[%i]==oub[%i]-olb[%i] && _StrEq (&(rho[%i]), &(olb[%i]), &(oub[%i]))==1) %s;\n",
				 pos, pos,
				 pos+1, pos+1,
				 rhs_prdct_pos2tooth [rhs_prdct_pos]+1, 
				 pos, pos, exit_string);
#endif
			fprintf (parser_file, "if (oub[%i]-olb[%i]==oub[%i]-olb[%i] && _StrEq (&(olb[%i]), &(oub[%i]))==1) %s;\n",
				 pos, pos,
				 pos+1, pos+1,
				 pos, pos, exit_string);
		    ...
		    case STREQLEN_ic:
		      fprintf (parser_file, "if (oub[%i]-olb[%i]==oub[%i]-olb[%i]) %s;\n",
			       pos, pos,
			       pos+1, pos+1,
			       exit_string);
		      break;
		    ...
		    case STRLEN_ic:
		      ...
		      fprintf (parser_file, "if (oub[%i]-olb[%i]==%i) %s;\n",
			       pos+1, pos+1,
			       l,
			       exit_string);
	  ...
	  if (cur2 == top2) {
		...
		if (arity) {
		  ...
		  if (Lex_file) {
		    ...
		    fprintf (parser_file,
"#if is_earley_guide\n\
local_val = MAKE_ij (olb [%i], oub [%i]);\n\
#if is_large\n\
if ((local_val = X_is_set (&range_1, local_val)) == 0) %s;\n\
#endif /* is_large */\n\
if ((rho [%i] = X_is_set (&rhoA_hd, (local_val<<lognt)+%i)) == 0) %s;\n\
#endif /* is_earley_guide */\n", pos, pos, exit_string, tooth+1, nt, exit_string);
		  }

		  if (is_1_RCG) {
		    fprintf (parser_file,
			     "if ((ret_vals[%i] = _%s(&(rho[%i]), &(olb[%i]), &(oub[%i])\n\
#if is_guided2\n\
, &(tc2rho[%i])\n\
#endif\n\
))==-1) %s;\n",
			     generator_pos, sxstrget (nt2ste [nt]), tooth+1, pos, pos, pos, exit_string);
		  }
		  else {
		    fprintf (parser_file,
			     "if ((ret_vals[%i] = _%s(&(rho[%i]), &(olb[%i]), &(oub[%i])))==-1) %s;\n",
			     generator_pos, sxstrget (nt2ste [nt]), tooth+1, pos, pos, exit_string);
		  }
		}
		else
		  fprintf (parser_file,
			   "if ((ret_vals[%i] = _%s(&(rho[%i])))==-1) %s;\n",
			   generator_pos, sxstrget (nt2ste [nt]), tooth+1, exit_string);
	      }
	      else {
		/* negative call */
...
, &(tc2rho[%i])\n\
			     ...
			     generator_pos, sxstrget (nt2ste [nt]), tooth+1, pos, pos, pos, exit_string, tooth+1, tooth+1);
		  }
		  else {
		    fprintf (parser_file,
			     "if ((ret_vals[%i] = -_%s(&(rho[%i]), &(olb[%i]), &(oub[%i])))==-1) %s;rho[%i] = -rho[%i];\n",
			     generator_pos, sxstrget (nt2ste [nt]), tooth+1, pos, pos, exit_string, tooth+1, tooth+1);
		  }
		}
		else
		  fprintf (parser_file,
			   "if ((ret_vals[%i] = -_%s(&(rho[%i])))==-1) %s;rho[%i] = -rho[%i];\n",
			   generator_pos, sxstrget (nt2ste [nt]), tooth+1, exit_string, tooth+1, tooth+1);
	      }
	    }
	    else {
	      /* On est sur un Cut */
  ...
  if (lhs_arg_tbp == 0 && rhs_prdct_nyp_nb == 0) {
	...
	if (nt < 0) {
	  if (nt == STREQ_ic || nt == STREQLEN_ic || nt == STRLEN_ic) {
	    ...
	    switch (nt) {
	    case STREQ_ic:
	      fprintf (parser_file, "ret_vals[%i] = %s_GStrEq (&(olb[%i]), &(oub[%i]));\n",
		       rhs_prdct_pos2generator_pos [rhs_prdct_pos],
		       is_pos_call ? "" : "-", rhs_param_pos, rhs_param_pos);
	      break;

	    case STREQLEN_ic:
	      fputs ("#if AG\n", parser_file);
	      fprintf (parser_file, "ret_vals[%i] = %s_GStrEqLen (&(olb[%i]), &(oub[%i]));\n",
		       rhs_prdct_pos2generator_pos [rhs_prdct_pos],
		       is_pos_call ? "" : "-", rhs_param_pos, rhs_param_pos);
	      fputs ("#endif\n", parser_file);
	      break;
	    ...
	    case STRLEN_ic:
	      ...
	      fputs ("#if AG\n", parser_file);
	      fprintf (parser_file, "ret_vals[%i] = %s_GStrLen (%i, &(olb[%i]), &(oub[%i]));\n",
		       rhs_prdct_pos2generator_pos [rhs_prdct_pos],
		       is_pos_call ? "" : "-",
		       l,
		       rhs_param_pos, rhs_param_pos);
	      fputs ("#endif\n", parser_file);
    ...
    fprintf (parser_file, "if (!(ret_vals[0] = generator (&G, %i, ret_vals, Indexes, ilb, iub, %s))) %s;\n",
	     clause, max_generator_pos == 0 ? "NULL, NULL" : "olb, oub", exit_string);
    fputs ("#endif\n", parser_file);
...
static void
gen_first_test (clause, rhs_pos, exit_stmt)
    int 	clause, rhs_pos;
    char	*exit_stmt;
{
  ...
  int		symb, rmadr, Ak, param, nt, rhs_prdct_pos, call_kind;
  char	kind;
  ...
  if (!SXBA_bit_is_set (rhs_pos2neg_call_set, rhs_pos)) {
	...
	fprintf (parser_file, "if (lcur_tok = LSOURCE(olb[%i]), %sFIRST_LAST(lcur_tok, first_%i_%i)) %s;\n",
		 rhs_pos-lhs_arity,
		 call_kind == 1 ? "!" : "",
		 clause,
		 rhs_prdct_pos,
	...
	if (first) {
	  if (SXBA_bit_is_set_reset (check_nul_range, rhs_pos) &&
	    ...
	    fprintf (parser_file, "if (olb[%i] != oub[%i]) %s;\n",
		     rhs_pos-lhs_arity,
		     rhs_pos-lhs_arity,
		     exit_stmt);
	    fputs ("#endif\n", parser_file);
	  ...
	  if (SXBA_bit_is_set_reset (check_non_nul_range, rhs_pos) &&
	    ...
	    if (SXBA_bit_is_set (first [Ak], 0)) {
	      fputs ("#if is_first_last\n", parser_file);
	      fprintf (parser_file, "if (oub[%i] <= olb[%i]) %s;\n",
		       rhs_pos-lhs_arity,
		       rhs_pos-lhs_arity,
		       exit_stmt);
	      fputs ("#endif\n", parser_file);
	  ...
	  if (SXBA_bit_is_set_reset (check_olb, rhs_pos)) {
	    ...
	    fputs ("#if is_first_last\n", parser_file);
	    fprintf (parser_file,
		     "if (olb[%i]<oub[%i] && (rcur_tok = SOURCE(oub[%i]), FIRST_LAST(rcur_tok,last_%i))) %s;\n",
		     rhs_pos-lhs_arity,
		     rhs_pos-lhs_arity,
		     rhs_pos-lhs_arity,
		     Ak,
	  ...
	  if (!ff) {
		...
		if (first) {
		  ...
		  if (!is_lcur_tok) {
		    is_lcur_tok = TRUE;
		    is_source_checked = TRUE;
		    sprintf (cur_tok_string, "lcur_tok = LSOURCE(olb[%i]), ",
			     rhs_pos-lhs_arity);
		  }
		  else
		    cur_tok_string [0] = NUL;

		  fprintf (parser_file, "if (%sFIRST_LAST(lcur_tok,first_%i)) %s;\n",
			   cur_tok_string,
			   Ak,
			   exit_stmt);
		...
		if (!fe) {
		  ...
		  if (kind == UnknownSize) {
			...
			fprintf (parser_file, "if (olb[%i]<oub[%i] && (%sFIRST_LAST(lcur_tok,first_%i))) %s;\n",
				 rhs_pos-lhs_arity,
				 rhs_pos-lhs_arity,
				 cur_tok_string,
				 Ak,
...
static void
gen_last_test (clause, rhs_pos, exit_stmt)
    int 	clause, rhs_pos;
    char	*exit_stmt;
{
  ...
  /* Les tests sur les ranges qui ne mettent pas en jeu Ak sont sortis de la boucle */
  int		symb, lmadr, Ak, param, nt, rhs_prdct_pos, call_kind;
  char	kind;
  char	cur_tok_string [32];
  ...
  if (!SXBA_bit_is_set (rhs_pos2neg_call_set, rhs_pos)) {
	...
	fprintf (parser_file, "if (rcur_tok = SOURCE(oub[%i]), %sFIRST_LAST(rcur_tok, last_%i_%i)) %s;\n",
		 rhs_pos-lhs_arity,
		 call_kind == 1 ? "!" : "",
		 clause,
		 rhs_prdct_pos,
	...
	if (first) {
	  ...
	  if (SXBA_bit_is_set_reset (check_oub, rhs_pos)) {
	    ...
	    is_source_checked = TRUE;
	    fprintf (parser_file,
		     "if (olb[%i]<oub[%i] && (lcur_tok = LSOURCE(olb[%i]), FIRST_LAST(lcur_tok,first_%i))) %s;\n",
		     rhs_pos-lhs_arity,
		     rhs_pos-lhs_arity,
		     rhs_pos-lhs_arity,
		     Ak,
	  ...
	  if (!lf) {
		...
		if (first) {
		  ...
		  if (!is_rcur_tok) {
		    is_rcur_tok = TRUE;
		    is_source_checked = TRUE;
		    sprintf (cur_tok_string, "rcur_tok = SOURCE(oub[%i]), ",
			     rhs_pos-lhs_arity);
		  }
		  else
		    cur_tok_string [0] = NUL;

		  fprintf (parser_file, "if (%sFIRST_LAST(rcur_tok,last_%i)) %s;\n",
			   cur_tok_string,
			   Ak,
			   exit_stmt);
		...
		if (!le) {
		  ...
		  if (kind == UnknownSize) {
			...
			fprintf (parser_file, "if (olb[%i]<oub[%i] && (%sFIRST_LAST(rcur_tok,last_%i))) %s;\n",
				 rhs_pos-lhs_arity,
				 rhs_pos-lhs_arity,
				 cur_tok_string,
				 Ak,
...
static void
gen_rfsa_guide (nt, rhs_pos, exit_stmt)
    int 	nt, rhs_pos;
    char	*exit_stmt;
{
  fprintf (parser_file, "if (!fsa_nt2lb (%i, olb[%i])) %s;\n", nt, rhs_pos-lhs_arity, exit_stmt);
}


static void
gen_lfsa_guide (nt, rhs_pos, exit_stmt)
    int 	nt, rhs_pos;
    char	*exit_stmt;
{
  fprintf (parser_file, "if (!fsa_nt2ub (%i, oub[%i])) %s;\n", nt, rhs_pos-lhs_arity, exit_stmt);
}


static void
gen_karg_guide (/* clause, */rhs_pos, exit_stmt)
     int 	/* clause, */rhs_pos;
    char	*exit_stmt;
{
  ...
  int Ak = rhs_pos2Ak [rhs_pos] /* , A = Ak2A (Ak), k = Ak_A2k (Ak, A) */ ;
...
iarg_searched_in_guide++;\n\
  ...
  fprintf (parser_file, "if (%scoupled_prdct (%i, %i)) %s;\n",
	   SXBA_bit_is_set (rhs_pos2neg_call_set, rhs_pos) ? "" :"!",
	   rhs_pos-lhs_arity,
	   Ak2guide_nt [Ak] /* code interne ds le guide du nt A_k */,
	   exit_stmt);
...
static BOOLEAN
is_atom_in_arg (atom, pos)
    int atom, pos;
{
    int		ladr, radr, symb, cur_atom;
...
static BOOLEAN
lhs_arg_processing (clause, pos, exit_stmt, first_call)
    int 	clause, pos;
    char	*exit_stmt;
    BOOLEAN	first_call;
{
  int		adr, ladr, radr, madr, symb, atom, rhs_pos, cur_rhs_pos, Index, rmadr, lmadr, nt;
  VARSTR	lvstr, rvstr;
  char	string [16];
  ...
  if (ladr < radr) {
	...
	if (SXBA_bit_is_set (synchro_set, madr)) {
	  ...
	  if (!SXBA_bit_is_set (is_madr_an_ob, madr)) {
	    ...
	    if ((adr = synchro2adr [madr]) != 0) {
	      ...
	      if (adr > 0) {
				/* C'est une iub */
		sprintf (string, "iub[%i]", adr2pos [adr]);
	      }
	      else {
		adr = -adr;
		sprintf (string, "ilb[%i]", adr2pos [adr]);
	      }
		...
		sxtrap (ME, "lhs_arg_processing");
#endif
	      sprintf (string, "Sindex[%i]", Sindex);
	      Sindex++;
	      fprintf (parser_file, "%s = %s;\n",
	...
	if (symb < 0) {
	  is_source_checked = TRUE;
	  fprintf (parser_file, "if (!LSCAN (%s, %i)) %s;\n", varstr_tostr (lvstr), -symb, exit_stmt);
	  ...
	  if ((Index = adr2Index (pos, ladr)) >= 0) {
	    ...
	    fputs ("#if is_generator\n", parser_file);
	    fprintf (parser_file, "Indexes[%i] = %s;\n",
		     Index, varstr_tostr (lvstr));
	    fputs ("#endif\n", parser_file);
	  }
	...
	if (SXBA_bit_is_reset_set (already_seen_madr_set, madr)) {
	  ...
	  if (ladr < radr) {
	    if (symb < 0) {
	      ...
	      SXBA_1_bit (tmadr_set, madr);
	      is_source_checked = TRUE;
	      fprintf (parser_file, "if (!SCAN (%s, %i)) %s;\n", varstr_tostr (rvstr), -symb, exit_stmt);
	      t_indexes_vstr = varstr_catenate (t_indexes_vstr, "rho[");
    ...
    char	*exit_stmt;
{
  ...
  int		xeq, atom, Xnb, min, max, p, tnb, kind, pos, ladr, radr, symb;
  int		*eq_ptr;
  char	*wstr, string [32];
  ...
  while (!IS_EMPTY (equation_stack)) {
    ...
    if (Xnb == 1) {
      if (is_assign) {
	if (Asize_is_useful) {
	  fprintf (parser_file, "if ((Asize[%i] = %s) < %i) %s;\n",
		   atom-1,
		   wstr,
		   min,
		   exit_stmt);
	  ...
	  if (max >= 0)
	    fprintf (parser_file, "if (Asize[%i] > %i) %s;\n",
		     atom-1,
		     max,
		     exit_stmt);
	}
	...
	fprintf (parser_file, "if (Asize[%i] != %s) %s;\n",
		 atom-1,
		 wstr,
		 exit_stmt);
      }
	...
	if (is_assign) {
	  if (Asize_is_useful) {
	    fprintf (parser_file, "if ((Asize[%i] = (local_val>>1)) < %i) %s;\n",
		     atom-1,
		     min,
		     exit_stmt);
	    ...
	    if (min > 0 || kind == UNKNOWN_k) {
	      fprintf (parser_file, "if ((local_val>>1) < %i) %s;\n",
		       min,
		       exit_stmt);
	  ...
	  fprintf (parser_file, "if (Asize[%i] != (local_val>>1)) %s;\n",
		   atom-1,
		   exit_stmt);
	}
      }
	...
	fprintf (parser_file,
		 "if ((local_val = %s) < 0 || (local_val %% %i)) %s;\n",
		 wstr,
		 Xnb,
		 exit_stmt);
	...
	if (is_assign) {
	  if (Asize_is_useful) {
	    fprintf (parser_file, "if ((Asize[%i] = (local_val/%i)) < %i) %s;\n",
		     atom-1,
		     Xnb,
		     min,
		     exit_stmt);
	    ...
	    if (min > 0 || kind == UNKNOWN_k) {
	      fprintf (parser_file, "if ((local_val/%i) < %i) %s;\n",
		       Xnb,
		       min,
		       exit_stmt);
		...
		fprintf (parser_file, "if ((local_val/%i) < %i) %s;\n",
	      ...
	      if (kind == LEQ0_k)
		fprintf (parser_file, "/* ((local_val/%i) < 0) */ %s;\n",
			 Xnb,
			 exit_stmt);
	    }
	  }
	  ...
	  fprintf (parser_file, "if (Asize[%i] != (local_val/%i)) %s;\n",
		   atom-1,
		   Xnb,
		   exit_stmt);
	}
...
static void
gen_level (level, clause, exit_stmt)
    int		level, clause;
    char	*exit_stmt;
{
  int		pos, xeq, mpos, atom, Xnb, xeq2, ladr, radr, lmadr, rmadr, symb, max;
  int		*eq_ptr, *eq2_ptr;
  char	string [16];
  ...
  if (lhs_arg_tbp > 0) {
    ...
    if (IS_EMPTY (equation_stack)) {
      if (max == -1) {
	if (decrease_order)
	  fprintf (parser_file, "for (Asize[%i] = %s; Asize[%i] >= %i%s; Asize[%i]--) {\n",
		   atom-1, varstr_tostr (wvstr),
		   atom-1, atom2min [atom], clause_has_cut ? " && !cut" : "",
		   atom-1);
	else {
	  fprintf (parser_file, "for (top [%i] = %s, Asize[%i] = %i; Asize[%i] <= top [%i]%s; Asize[%i]++) {\n",
		   atom-1, varstr_tostr (wvstr), atom-1, atom2min [atom],
		   atom-1, atom-1, clause_has_cut ? " && !cut" : "",
		   atom-1);
	}
      }
      else {
	/* Si min==max, len est positionne et on n'est pas ici ... */
	fprintf (parser_file, "if ((min_max = %s) > %i) min_max = %i;\n",
		 varstr_tostr (wvstr),
		 max,
		 max);

	if (decrease_order)
	  fprintf (parser_file, "for (Asize[%i] = min_max; Asize[%i] >= %i%s; Asize[%i]--) {\n",
		   atom-1,
		   atom-1, atom2min [atom], clause_has_cut ? " && !cut" : "",
		   atom-1);
	else {
	  fprintf (parser_file, "for (top [%i] = min_max, Asize[%i] = %i; Asize[%i] <= top [%i]%s; Asize[%i]++) {\n",
		   atom-1, atom-1, atom2min [atom],
		   atom-1, atom-1, clause_has_cut ? " && !cut" : "",
		   atom-1);
	}
      }
...
/* UTILISE' si 1-RCG */
static int		size;
static int		*lhs_arg2pos;
static int		*pos2symb, *pos2lhs_arg, *pos2bound, **posxpos2lgth, *posxpos_stack, *pos2main;
...
static BOOLEAN
are_pos1_pos2_equiv (pos1, pos2)
    int pos1, pos2;
{
    int symb1, symb2, cur_pos1, cur_pos2;
...
left_linear (clause)
    int clause;
{
    int		lhs_prdct, left_linear_lhs_bot2, left_linear_lhs_top2, lhs_arg, pos, cur2, param, cur3, bot3, top3, symb;
    int		ipos, opos;
    int		rhs, bot, top, cur, prdct, bot2, top2, rhs_pos, rhs_param_pos, k, nt;
    int		Ak, len;
    int		*p, *b1, *d1, *t1;
    int		lhs_arg1, lhs_arg2, top1, pos1, pos2, main_pos, main_pos1, main_pos2;
    BOOLEAN	ret_val;
    SXBA	pos_set;
    ...
    int lb;
{
    ...
    sprintf (pstr, "%s%sb[%i]", is_i ? "i" : "o", is_l ? "l" : "u", lb);
}
...
static BOOLEAN
gen_pos1_pos2 (pos1, pos2)
    int pos1, pos2;
{
    int lhs_arg1, bot1, top1, llgth1, rlgth1, llgth2, rlgth2, lhs_arg2, bot2, top2;
	...
	if (((llgth2 >= 0) || (rlgth2 >= 0))) {
	    if (llgth1 >= 0)
		fprintf (parser_file,
			 "if (ilb[%i]+%i!=", lhs_arg1, llgth1);
	    else
		fprintf (parser_file,
			 "if (iub[%i]-%i!=", lhs_arg1, rlgth1);
    ...
    int clause;
{
    ...
    int 	rhs, top, bot, cur, rhs_param_pos, prdct, bot2, nt, rhs_pos, param, bot3, top3, arity, Ak, k, len, xbot, xtop;
    int 	IJ, I, J, rhs_prdct_pos, tooth, generator_pos, top2, cur2;
    int		*p, *b1, *t1, *c1, *c2, *b2, *t2, *d1;
    int		lhs_arg1, pos, top_pos, next_pos, lhs_arg2;
    int		ipos, main_ipos, opos, main_opos;
    int		cur_pos, bot_pos, bot1_tnb, top1_tnb, bot2_tnb, top2_tnb, lb;
...
#if 0
    if (Arhs_arg_nb)
	fprintf (parser_file, "rhs_guide (%i, &local_guide, guide_map_%i, olb, oub);\n",
	...
	if (!gen_guided_bounds_is_left_linear) {
		...
		if (SXBA_bit_is_set (pos_is_a_range, rhs_pos+lhs_arity)) {
			...
			for (c2 = rhs_pos2rhs_pos_1rcg + p [rhs_pos] + 1, c1 = b1+1; c1 < t1; c2++, c1++) {
				...
				if (is_i) {
				    fprintf (parser_file,
					     "if (olb[%i]!=ij2i(range)) continue;\n",
					     rhs_pos);
				}
				else {
				    fprintf (parser_file,
					     "if (oub[%i]!=ij2j(range)) continue;\n",
					     rhs_pos);
				}
			    }
			}
	...
	if (nt < 0 && nt != CUT_ic) {
		...
		if (b1 == t1 || b2 == t2) {
			...
			/* Appel positif */
			fprintf (parser_file,
				 "if (oub[%i]-olb[%i]!=oub[%i]-olb[%i]) continue;\n",
				 rhs_pos, rhs_pos,
				 rhs_pos+1, rhs_pos+1);
...
#if 0
			    fprintf (parser_file,
				     "if (_StrEq (&(rho[%i]), &(olb[%i]), &(oub[%i]))==-1) continue;\n",
				     rhs_prdct_pos2tooth [rhs_prdct_pos]+1, rhs_pos, rhs_pos);
#endif
			    fprintf (parser_file,
				     "if (_StrEq (&(olb[%i]), &(oub[%i]))==-1) continue;\n",
				     rhs_pos, rhs_pos);
			}
		    }
		    else {
...
#if 0
			    fprintf (parser_file,
				     "if (oub[%i]-olb[%i]==oub[%i]-olb[%i] && _StrEq (&(rho[%i]), &(olb[%i]), &(oub[%i]))==1) continue;\n",
				     rhs_pos, rhs_pos,
				     rhs_pos+1, rhs_pos+1,
				     rhs_prdct_pos2tooth [rhs_prdct_pos]+1, 
				     rhs_pos, rhs_pos);
#endif
			    fprintf (parser_file,
				     "if (oub[%i]-olb[%i]==oub[%i]-olb[%i] && _StrEq (&(olb[%i]), &(oub[%i]))==1) continue;\n",
				     rhs_pos, rhs_pos,
				     rhs_pos+1, rhs_pos+1, 
				     rhs_pos, rhs_pos);
			}
	...
	while ((pos = sxba_scan (pos_hd_set, pos)) > 0) {
		...
		do {
			...
			if (is_next_pos2bound) {
			    ...
			    fprintf (parser_file,
				     "if (%s > %s-%i || %s+%i > %s) continue;\n",
				     bot1_str, top2_str, top2_tnb,
				     bot2_str, bot2_tnb, bot1_str);
			}
		    }
		    else {
			if (is_next_pos2bound) {
			    fprintf (parser_file,
				     "if (%s+%i > %s || %s > %s-%i) continue;\n",
				     bot1_str, bot1_tnb, bot2_str,
				     bot2_str, top1_str, top1_tnb);
			}
			else {
			    fprintf (parser_file,
				     "if (%s+%i > %s-%i || %s+%i > %s-%i) continue;\n",
				     bot1_str, bot1_tnb, top2_str, top2_tnb,
				     bot2_str, bot2_tnb, top1_str, top1_tnb);
			}
		    }
	...
	if (nt > 0) {
		...
		if (arity)
		    fprintf (parser_file,
			     "if ((ret_vals[%i] = _%s(&(rho[%i]), &(olb[%i]), &(oub[%i])))==-1) continue;\n",
			     generator_pos, sxstrget (nt2ste [nt]), tooth+1, rhs_pos, rhs_pos);
		else
		    fprintf (parser_file,
			     "if ((ret_vals[%i] = _%s(&(rho[%i])))==-1) continue;\n",
			     generator_pos, sxstrget (nt2ste [nt]), tooth+1);
	    }
	    else {
		/* Le 09/02/04, on ne se moque plus de la valeur de rho.  De plus on le passe en negatif
		   afin de reconnaitre facilement les appels negatifs... */
		...
		if (arity)
		    fprintf (parser_file,
			     "if ((ret_vals[%i] = -_%s(&(rho[%i]), &(olb[%i]), &(oub[%i])))==-1) continue;rho[%i] = -rho[%i];\n",
			     generator_pos, sxstrget (nt2ste [nt]), tooth+1, rhs_pos, rhs_pos, tooth+1, tooth+1);
		else
		    fprintf (parser_file,
			     "if ((ret_vals[%i] = -_%s(&(rho[%i])))==-1) continue;rho[%i] = -rho[%i];\n",
			     generator_pos, sxstrget (nt2ste [nt]), tooth+1, tooth+1, tooth+1);
	    }
...
if (!is_glbl_cycle) {\n\
    ...
    if (lhs_arity) {
      int lhs_prdct, gen_guided_bounds_lhs_bot2, gen_guided_bounds_lhs_top2, lhs_arg_nb, t_nb, gen_guided_bounds_cur2, gen_guided_bounds_param, gen_guided_bounds_bot3, gen_guided_bounds_top3, cur3, pos_1rcg, gen_guided_bounds_bot, gen_guided_bounds_top, gen_guided_bounds_cur, gen_guided_bounds_xtop, x, local_t_nb; 
	...
	if (gen_guided_bounds_param > 0) {
	  ...
	  for (cur3 = gen_guided_bounds_bot3; cur3 < gen_guided_bounds_top3; cur3++) {
		...
		for (gen_guided_bounds_cur = gen_guided_bounds_bot; gen_guided_bounds_cur < gen_guided_bounds_top; gen_guided_bounds_cur++) {
	      ...
	      fprintf (parser_file, "rho[%i] = pnode%i->rho[%i];\n", 2+max_son_nb+t_nb, lhs_arg_nb, ++pos_1rcg);
	      t_nb++;
	      local_t_nb++;
	    }
	  }
    ...
    int clause;
{
  ...
  int		xeq, nb, rhs_pos, pos, param, cur3, nt;
...
gen_local_variables ()
{
    int		A, clause, rhs, cur, bot, top, son_nb, prdct, bot2, nt, t;
    int		*gen_local_variables_clause2son_nb;
...
/* 0 */ \"\",\n", parser_file);
    ...
    fputs ("static int clause2lhs_nt[clmax+1]={0,", parser_file);

    for (clause = 1; clause <= last_clause; clause++) {
	if (clause%16 == 1)
	    fprintf (parser_file, "\n/* %i */ ", clause);

	fprintf (parser_file, "%i, ", clause2lhs_nt (clause));
    }

    fputs ("};\n", parser_file);

    fputs ("static int\tdum_range;\n", parser_file);

    if (has_external) {
	fputs ("int\t\t", parser_file);
	...
	if (has_ext_decl) {
		...
		for (cur = bot; cur < top; cur++) {
			...
			if (!done) {
			    ...
			    fprintf (parser_file, "is_useful_%i = TRUE", clause);
			}
		    }
		}
	    }
	...
	if (has_ext_def) {
		...
		if (SXBA_bit_is_set (is_lhs_nt, A)) {
		    ...
		    fprintf (parser_file, "%s_arity = %i", sxstrget (nt2ste [A]), nt2arity [A]);
		}
	    }
	...
	if (has_ext_def) {
		...
		fputs ("static int\tlarg_nb, *plb, *pub, lower_bound, upper_bound;\n", parser_file);
	    }
	}
#endif /* 0 */
    }
    ...
    fprintf (parser_file, "static int\tsons_0 [%i];\n", max_rhs_prdct + 1);

    gen_local_variables_clause2son_nb = (int*) sxcalloc (last_clause+1, sizeof (int));
	...
	if (has_ext_prdct_call) {
	    ...
	    if (is_first) {
		fputs ("static int", parser_file);
		is_first = FALSE;
	    }
	    else
		fputs (", ", parser_file);

	    fprintf (parser_file, "\n\tsons_%i [%i] = {0, ", clause, son_nb+1);
		...
		if (XH_list_elem (rcg_predicates, bot2) == 0) {
		    ...
		    if (nt > 0 || nt == TRUE_ic) {
			fprintf (parser_file, "%i, ", (nt > 0) ? 0 : nt);
		    }
		}
	    }
    ...
    fputs ("static int gen_local_variables_clause2son_nb [] = {0,", parser_file);

    for (clause = 1; clause <= last_clause; clause++) {
	if ((clause % 8) == 1)
	    fprintf (parser_file, "\n/* %i */ ", clause);

	fprintf (parser_file, "%i, ", gen_local_variables_clause2son_nb [clause]);
    }

    fputs ("\n};\n", parser_file);

    fputs ("static int *clause2son_pid [] = {sons_0,", parser_file);
	...
	if (SXBA_bit_is_set (clause_has_ext_decl, clause))
	    fprintf (parser_file, "sons_%i, ", clause);
	else
...
static void
fill_Lex_t2clause_set (L, t2, c2, with_First_Last, t_nb_lim)
     SXBA **L, **t2, **c2;
     BOOLEAN with_First_Last;
     int t_nb_lim;
{    
  int		clause, prdct, bot2, top2, arity, cur2, param, bot3, cur3, top3, symb;
  int		rhs, top, cur, bot, nt;
  int		t_nb, nt_nb;
...
{
  int     A;
  /* int     *A2max_Aclause, Aclause, max_Aclause; */
  SXBA	  *Lex, *clause2rhs_nt_set;

  fill_Lex_t2clause_set (&Lex, &t2clause_set, &clause2rhs_nt_set, TRUE, lfsa_file ? 0 : 1);
...

  {
    ...
    for (A = 1; A <= max_nt; A++) {
      fprintf (parser_file, "/* %i */ nt2clause_id_set_%i,\n", A, A);
    }
...
#if 0
  {
    ...
    fputs ("static int nt2lex_code[ntmax+1]={0,", parser_file);

    for (A = 1; A <= max_nt; A++) {
      if (A%16 == 1)
	fprintf (parser_file, "\n/* %i */ ", A);

      fprintf (parser_file, "%i, ", lexcode);
    ...
    fputs ("static int lex_code[clmax+ntmax+1];\n", parser_file);
  }
...
#if 0
  {
    ...
    fputs ("static int clause2Aclause[clmax+1]={0,", parser_file);
    
    A2max_Aclause = (int*) sxcalloc (max_nt+1, sizeof (int));
    max_Aclause = 0;
	...
	fprintf (parser_file, "\n/* %i */ ", clause);
      ...
      fprintf (parser_file, "%i, ", Aclause); /* 0 marque une fin de liste */
    ...
    fprintf (parser_file, "#define Gmax_Aclause %i\n", max_Aclause);
	...
	fprintf (parser_file, "%iL, ", A2max_Aclause [A]);
      }
...
gen_shallow ()
{
    int	clause, main_clause, prev_main_clause, i;
    
    fputs ("static int from2var2clause [] = {0,", parser_file);
	...
	fprintf (parser_file, "%i, ", main_clause);
    }

    fputs ("\n};\n", parser_file);
    
    fputs ("static int from2var2clause_disp [] = {0,", parser_file);

    for (clause = 1; clause <= new_clause_nb+1; clause++) {
	if ((clause % 8) == 1)
	    fprintf (parser_file, "\n/* %i */ ", clause);

	fprintf (parser_file, "%i, ", from2var2clause_disp [clause]);
    }

    fputs ("\n};\n", parser_file);
    
    fputs ("static int from2var2clause_list [] = {0,", parser_file);

    for (i = 1; i <= from2var2clause_list_top; i++) {
	if ((i % 8) == 1)
	    fprintf (parser_file, "\n/* %i */ ", i);

	fprintf (parser_file, "%i, ", from2var2clause_list [i]);
    }
...
gen_clause_id ()
{
  ...
  /* ATTENTION, Le 29/5/2002, on fait disparaitre clause2Aclause */
    ...
    int clause, gen_clause_id_clause_id, A;
    ...
    fputs ("static int clause2clause_id [];\n", parser_file);
	...
	fprintf (parser_file, "%iL, ", A2max_clause_id [A]);
    }
...
gen_for_rav2 ()
{
	...
	fputs ("static int clause2rhs_stack_map [] = {0,", parser_file);

	for (clause = 1; clause <= last_clause; clause++) {
	    if ((clause % 8) == 1)
		fprintf (parser_file, "\n/* %i */ ", clause);

	    fprintf (parser_file, "%i, ", clause2rhs_stack_map [clause]);
	}

	fputs ("};\n\n", parser_file);

	fputs ("static int rhs_stack_map [] = {0,", parser_file);
	y = 0;
...
static void
gen_dynam_lex (clause/* , arity */)
    int clause/* , arity */;
{
    int		lhs_prdct, gen_dynam_lex_lhs_bot2, gen_dynam_lex_lhs_top2, pos, cur2, param, bot3, top3, cur3, x, nb;
    BOOLEAN	is_first = TRUE;
...
is_robust_run ||\n\
	...
	for (k = 0; k < arity; k++) {
	  ...
	  fprintf (parser_file, "coupled_clause (%i, %i)", k, clausek2guide_Aclause [clause_k2clausek (clause, k)]);
	}
...
if (", parser_file);
	  ...
	  for (pos = 0, cur2 = gen_dynam_lex_lhs_bot2+2; cur2 < gen_dynam_lex_lhs_top2; pos++, cur2++) {
		...
		fprintf (parser_file, "dynam_lex (ilb[%i], iub[%i], %i, lhst_list+%i)", pos, pos, nb, x);
...
gen_first_last_pp ()
{
    ...
    int		clause, rhs, top, bot, rhs_prdct_pos, cur, prdct, bot2, nt, param, bot3, cur3, top3, symb;
    char	istr [16];
    SXBA	t_set;
	...
	for (rhs_prdct_pos = 1, cur = bot; cur < top; rhs_prdct_pos++, cur++) {
	    ...
	    if (nt == FIRST_ic || nt == LAST_ic) {
		sprintf (istr, "%i_%i", clause, rhs_prdct_pos);
...
gen_first_last ()
{
	...
	if (SXBA_bit_is_set (is_nt_internal, A)) {
	    ...
	    sprintf (istr, "%i", Ak);
	    fputs ("\n/* ", parser_file);
	    fprintf (parser_file, "%s [%i] */", sxstrget (nt2ste [A]), k+1);
...
gen_memo_fun ()
{
    int		A, clause, arity, class, cur, rhs, top, bot, son_nb, prdct, bot2, nt, max;
    BOOLEAN	has_ext;
...
#if is_memop
	fputs ("#if is_memop\n", parser_file);
	fprintf (parser_file, "memoP = sxbm_calloc (l+1, G.memoP_size=%i);\n", gmax+1);
...
memoN = sxbm_resize (memoN, G.memoN_size, G.memoN_size, new_size+1);\n", parser_file);
    fputs ("#if AG\n\
    memoret_val = (int*) sxrealloc (memoret_val, new_size+1, sizeof (int));\n\
#endif\n", parser_file);
    fputs ("\
#endif /* is_memon */\n", parser_file);
...
#if is_memop
    fputs ("#if is_memop\n", parser_file);
    fprintf (parser_file, "memoP = sxbm_calloc (X_top (rhoA_hd)+1, %i);\n", gmax+1);
    fputs ("#endif /* is_memop */\n", parser_file);
#endif /* is_memop */
...
int l;\n", PRGENTNAME(FALSE));
    ...
    if (has_XxYxZ)
	fputs ("static int memo3_assoc [6];\n\n", parser_file);
...
#else /* !is_dag */\n\
        source = (int*) sxalloc (n+2, sizeof (int));\n\
#endif /* !is_dag */\n\
        lsource = source+1;\n\
...
lexicalization (&G);\n\
	...
	while ((A = sxba_scan (ext_decl_set, A)) > 0) {
	    ...
	    arity = nt2arity [A];
	    fprintf (parser_file, "if (%s_arity != %i) {\n", sxstrget (nt2ste [A]), arity);
	    fprintf (parser_file, "is_useful_%i = FALSE;\n", clause);
	    fputs ("#if EBUG || EBUG2\n", parser_file);
	    fprintf (parser_file, "arg_nb_put_error (\"&%s\", %s_arity, %i);\n",
		     sxstrget (nt2ste [A]), sxstrget (nt2ste [A]), arity);
	    fputs ("#endif\n", parser_file);
	    fputs ("}\n", parser_file);
	}
    }
    ...
    fputs ("    fprintf (sxtty, \"The compilation options of the guided parser #%i are incompatible with the current guide.\\n\", G.pid);\n", parser_file);
    fputs ("    exit(1);\n", parser_file);
    fputs ("}\n", parser_file);
...
l+1);\n", parser_file);
    ...
    fputs ("#if AG\n\
memoret_val = (int*) sxalloc (l+1, sizeof (int));\n\
#endif\n", parser_file);
    fputs ("#endif /* is_memon */\n", parser_file);
	...
	fprintf (parser_file, "sons_0 [%i] = ", cur);
	...
	fprintf (parser_file, "sons_%i [0]", clause);
	...
	for (cur = bot; cur < top; cur++) {
		...
		if (nt > 0) {
			...
			fprintf (parser_file, " = sons_%i [%i]", clause, son_nb);
		    else
			has_ext = TRUE;
		}
		else {
	...
	while ((clause = sxba_scan (clause_has_ext_decl, clause)) > 0) {
		...
		if (XH_list_elem (rcg_predicates, bot2) == 0) {
			...
			if (!SXBA_bit_is_set (is_lhs_nt, nt))
			    fprintf (parser_file, "sons_%i [%i] = %s_pid;\n", clause, son_nb, sxstrget (nt2ste [nt]));
		    }
		    else {
			if ((is_drcg && nt == STREQ_ic && is_streq_spcl (bot2+2)) || nt == TRUE_ic) {
			    /* Le rho correspondant est occupe */
...
gen_macros ()
{
    ...
    fprintf (parser_file, "#define tmax\t%i\n", max_t);
    fprintf (parser_file, "#define ntmax\t%i\n", max_nt);
    fprintf (parser_file, "#define clmax\t%i\n", last_clause);
	...
	fprintf (parser_file, "#define lognt\t%i\n", sxlast_bit (new_max_nt));
	fputs ("#else\n", parser_file);
    }
    ...
    fprintf (parser_file, "#define Garity\t%i\n", max_garity);
    fprintf (parser_file, "#define rhs_arg_nb\t%i\n", max_rhs_arg_nb);

    if (is_1_RCG) {
	fprintf (parser_file, "#define gclause_nb\t%i\n", guide_clause);
	fprintf (parser_file, "#define glognt\t%i\n", guide_lognt);
	fprintf (parser_file, "#define gmax_clause_id\t%i\n", guide_max_clause_id);
    }
    else {
...
gen_loops ()
{
	...
	looped_nt = (int*) sxcalloc (max_nt+1, sizeof (int));
	looped_clause = (int*) sxalloc (max_nt+sxba_cardinal (loop_clause_set)+1, sizeof (int));
	looped_clause [0] = 0;
	...
	fputs ("static int looped_clause[] = {0,\n", parser_file);
    
	for (A = 1; A <= max_nt; A++) {
	    xlc = looped_nt [A];

	    if (xlc) {
		fprintf (parser_file, "/* %i */ ", A);
...
gen_instantiated_prdct2struct ()
{
    ...
    fprintf (parser_file, "static int	lower_bounds [%i], upper_bounds [%i];\n", max_garity, max_garity);

    if (profile_nb > 1)
	fputs ("static struct Aij_struct Aij_struct = {(char*)NULL, 0, 0, lower_bounds, upper_bounds};\n", parser_file);
    else
	fprintf (parser_file,
		 "static struct Aij_struct Aij_struct = {(char*)NULL, 0, %i, lower_bounds, upper_bounds};\n", profile);

    fputs ("#define ij2b(ij,l)	lower_bounds [l] = ij2i(ij), upper_bounds [l] = ij2j(ij)\n",
	   parser_file);

    fputs ("\nstatic struct Aij_struct*\n", parser_file);
    fputs ("instantiated_prdct2struct (Aij)\n", parser_file);
    fputs ("int Aij;\n", parser_file);
    fputs ("{\n", parser_file);
    fputs ("int couple, profile, rho, ij, bot, param;\n\n", parser_file);
	...
	fprintf (parser_file, "Aij_struct.arity = (nt2arity+%i) [Aij_struct.A];\n", -(CUT_ic));
	...
	if (has_XH) {
	    fputs ("bot = XH_X (*(G.prange_n), rho);\n", parser_file);
	    fprintf (parser_file, "for (param = 0; param < %i; param++) {\n", profile);
	    fputs ("ij = XH_list_elem (*(G.prange_n), bot+param);\n", parser_file);
	    fputs ("ij2b(ij, param);\n", parser_file);
	    fputs ("}\n", parser_file);
	}
...
{
    int i, x;
    int lhs_prdct, gen_G_lhs_bot2, gen_G_lhs_top2, lhs_cur2, lhs_param, lhs_bot3, lhs_top3, lhs_cur3, symb;
    ...
    clause2lhst_disp = (int*) sxalloc (last_clause+1, sizeof (int));

    fputs ("static int clause2lhst_disp [] = {0,", parser_file);
    x = 1;
    ...
    fputs ("static int lhst_list [] = {0,", parser_file);
    ...
    fputs ("static int clause2lhs [] = {0,", parser_file);
    ...
    fputs ("static int clause2rhs [] = {0,", parser_file);
    ...
    fputs ("static int rhs2prdcts_disp [] = {0,", parser_file);
    ...
    fputs ("static int rhs2prdcts [] = {0,", parser_file);
    ...
    fputs ("static int prdct2args_disp [] = {0,", parser_file);
    ...
    fputs ("static int prdct2args [] = {0,", parser_file);
    ...
    fputs ("static int args2symbs_disp [] = {0,", parser_file);
    ...
    fputs ("static int args2symbs [] = {0,", parser_file);
	...
	fputs ("static int guide_clause2clause [] = {0,", parser_file);
	...
	for (i = 1; i <= guide_clause; i++) {
	    ...
	    fprintf (parser_file, "%i, ", (guide_clause2arg_pos [i]<<logk) + x);
	}

	fputs ("\n};\n", parser_file);

#if 0
	fputs ("static int guide_clause2arg_pos [] = {0,", parser_file);

	for (i = 1; i <= guide_clause; i++) {
	    if (i % 8 == 1)
		fprintf (parser_file, "\n/* %i */ ", i);
	    fprintf (parser_file, "%i, ", guide_clause2arg_pos [i]);
	}

	fputs ("\n};\n", parser_file);

	fputs ("static int guide_clause2clause_id [] = {0,", parser_file);

	for (i = 1; i <= guide_clause; i++) {
	    if (i % 8 == 1)
		fprintf (parser_file, "\n/* %i */ ", i);
	    fprintf (parser_file, "%i, ", guide_clause2clause_id [i]);
	}
    ...
    if (has_identical_clauses) {
	int	clause, clause2identical_top, nb;
	int	*clause2identical_disp = (int*) sxcalloc (last_clause+1, sizeof (int));

	for (clause = 1; clause <= last_clause; clause++)
	    if ((i = clause2identical [clause]) != clause)
		clause2identical_disp [i]++;

	fputs ("static int clause2identical_disp [] = {0,", parser_file);
	...
	for (clause = 1; clause <= last_clause; clause++) {
	    ...
	    fprintf (parser_file, "%i, ", clause2identical_top);
	...
	fprintf (parser_file, "%i\n};\n", clause2identical_top);

	fputs ("static int clause2identical [] = {0,\n", parser_file);
	...
	int	nt, nt2identical_top, nb;
	int	*nt2identical_disp = (int*) sxcalloc (max_nt+1, sizeof (int));

	for (nt = 1; nt <= max_nt; nt++)
	    if ((i = nt2identical [nt]) != nt)
		nt2identical_disp [i]++;

	fputs ("static int nt2identical_disp [] = {0,", parser_file);
	...
	for (nt = 1; nt <= max_nt; nt++) {
	    ...
	    fprintf (parser_file, "%i, ", nt2identical_top);
	...
	fputs ("static int nt2identical [] = {0,\n", parser_file);

	for (nt = 1; nt <= max_nt; nt++) {
	    if (nt2identical_disp [nt+1] > nt2identical_disp [nt]) {
		fprintf (parser_file, "/* %i */ ", nt);
...
0,\n\
    ...
    fputs ("#if is_semantics || is_guided\n", parser_file);
#endif /* 0 */
    fprintf (parser_file, "nt2arity+%i,\n", -(CUT_ic)); /* CUT_ic a le + petit code des predefinis */
#if 0
    fputs ("#else\n", parser_file);
    fputs ("NULL,\n", parser_file);
    fputs ("#endif\n", parser_file);
...
gen_fun_decl ()
{
	...
	if (A > 0) {
		...
		fprintf (parser_file, "_walk_%s ()", sxstrget (nt2ste [A]));
...
void
gen_clause_comment (gen_clause_comment_parser_file, clause)
     FILE *gen_clause_comment_parser_file; 
     int clause;
{
    int rhs, bot, cur, top, ste;
...
static void
nt2cst (nt2cst_A_clause_set)
    SXBA	 nt2cst_A_clause_set;
{
    int		clause, nt2cst_lhs_bot2, nt2cst_lhs_top2, isize_nb, pos, cur2, param, cur3, bot3, top3, var_nb, symb, symb_nb;
    int		Sindex_nb, var, rhs, top, bot, nt2cst_rhs_arg_nb, cur, prdct, bot2, top2, rhs_prdct_nb, etc_nb, nt2cst_lhs_t_nb;
...
static void
gen_function_local_variables (A, gen_function_local_variables_A_clause_nb, A_arity, /* gen_function_local_variables_A_is_loop, */ gen_function_local_variables_A_is_cyclic)
    int 	A, gen_function_local_variables_A_clause_nb, A_arity;
    BOOLEAN	/* gen_function_local_variables_A_is_loop, */ gen_function_local_variables_A_is_cyclic;
{
    int		ts, total_size, k;
	...
	/* Le 04/06/2003 */
        fprintf (parser_file, "SXBA_ELT clause_id_set [1+NBLONGS(%i)] = {%i};\n", k, k);
#if 0
        fprintf (parser_file, "SXBA_ELT clause_id_set [%i] = {%i};\n", NBLONGS (k)+1, k);
#endif
...
int tcrho[%i];\n\
        ...
        if (Arhs_arg_nb) {
	    fputs ("#if is_guided2\n", parser_file);
	    fprintf (parser_file, "int tc2rho[%i];\n", Arhs_arg_nb);
	    fputs ("#endif\n", parser_file);
	}
...
struct spf_node ", parser_file);
		...
		if (k == 0)
		    fprintf (parser_file, "*pnode%i", k);
		else
		    fprintf (parser_file, "*pnodei%i, *pnode%i", k, k);
	    }
...
int range;\n\
    ...
    fprintf (parser_file, "int\t\trho[%i];\n", 2+Arhs_prdct_nb+Alhs_t_nb);
    ...
    if (Arhs_arg_nb) {
        fprintf (parser_file, "olb[%i], oub[%i]", Arhs_arg_nb, Arhs_arg_nb);
    ...
    if (Avar_nb) {
	fprintf (parser_file, "Asize[%i]", Avar_nb);
    ...
    if (ASindex_nb) {
	fprintf (parser_file, "Sindex[%i]", ASindex_nb);
    ...
    if (Aisize_nb) {
	fprintf (parser_file, "isize[%i]", Aisize_nb);
    ...
    if (Aetc_nb) {
	fprintf (parser_file, "odddb[%i]", 2*Aetc_nb);
	...
	fprintf (parser_file, "int\t\ttop[%i];\n", Avar_nb);

    if (Avar_nb && Anon_range) {
	fprintf (parser_file, "int\t\tXlb [%i], Xub [%i];\n", Avar_nb, Avar_nb);
    }

    fputs ("int\t\tret_val, local_ret_val, clause_id;\n", parser_file);
...
gen_sem_act ()
{
    ...
    fputs ("\nstatic BOOLEAN\n", parser_file);
    fprintf (parser_file, "_%s_semact (rho, son_nb, sons, is_first)\n", PRGENTNAME(FALSE));
    fputs ("int *rho, son_nb, sons[];\n", parser_file);
    fputs ("BOOLEAN is_first;\n", parser_file);
    fputs ("{\n", parser_file);
    fputs ("bfun call;\n", parser_file);
    ...
    fputs ("\nstatic int\n", parser_file);
    fprintf (parser_file, "_%s_semact_prologue (G)\n", PRGENTNAME(FALSE));
    fputs ("struct G *G;\n", parser_file);
    fputs ("int local_pid;\n", parser_file);
    fputs ("{\n", parser_file);
    fputs ("#if is_multiple_pass\n", parser_file);
...
output_clause2clause_id ()
{
  ...
  fprintf (parser_file, "static int clause2clause_id [%i] = {0,", last_clause+1);

  for (clause = 1; clause <= last_clause; clause++) {
    if ((clause % 10) == 1)
      fprintf (parser_file, "\n/* %i */ ", clause);

    fprintf (parser_file, "%i, ", clause2clause_id [clause]);
  }
...
gen_parser ()
{
  int 	        A, main_A, A_arity, clause, main_clause, /* clause_nb, */pos, i;
  int		rhs, top, bot, son_nb, cur, prdct, bot2, nt;
  int		top2, Ak, k;
  int		d, degree;
...
int *rho0, *ilb, *iub;\n{\n",
	...
	if (A_clause_nb == 0) {
	  ...
	  if (A_arity) {
	    fprintf (parser_file,
		     "printf (\"(%%s)in_out(%%i): %s (", sxstrget (nt2ste [A]));

	    for (pos = 0; pos < A_arity; pos++) {
	      fputs ("%i..%i", parser_file);
	    ...
	    for (pos = 0; pos < A_arity; pos++) {
	      fprintf (parser_file, "ilb[%i], iub[%i]", pos, pos);
	  ...
	  if (!Lex_file) {
	    ...
	    default:
	      for (i = 0; i < A_arity; i++) {
		fprintf (parser_file, "XH_push(range_n, MAKE_ij(ilb[%i], iub[%i]));\n",
			 i, i);
	      }
	    ...
	    if (A_arity)
	      fprintf (parser_file, "X_set (&rhoA_hd, (local_val<<lognt)+%i, rho0);\n", A);
	    else
	      fprintf (parser_file, "X_set (&rhoA_hd, %i, rho0));\n", A);
	  ...
	  if (A_arity) {
	    fprintf (parser_file,
		     "printf (\"(%%s)in(%%i): %s (", sxstrget (nt2ste [A]));
	  ...
	  if (A_is_external && SXBA_bit_is_set (is_nt_external, 1)) {
	    ...
	    if (A_arity > 1) {
	      fprintf (parser_file, "for (larg_nb = 0; larg_nb < %i; larg_nb++)\n", A_arity);
	    }
...
printf (\"(%%s)out_memo(%%i): %s (", sxstrget (nt2ste [A]));
	  ...
	  if (A_is_cyclic) {
	    ...
	    fprintf (parser_file, "#if EBUG\nprintf (\"(%%s)out_cycle(%%i): %s (", sxstrget (nt2ste [A]));
...
#if EBUG
		  printf ("(%s)out_guide(%i): S1 (%i..%i) FALSE\n", G.name, call_level, ilb[0], iub[0], );
		  call_level--;
#endif /* EBUG */
		  return -1;
		}
...
iprdct_searched_in_guide++;\n\
		...
		fputs ("\n|| ", parser_file);
	      ...
	      fprintf (parser_file, "!coupled_prdct (%i, %i)", k, Ak2guide_nt [Ak] /* code interne ds le guide du nt A_k */);
	    }
	    ...
	    fprintf (parser_file, "#if EBUG\n\
printf (\"(%%s)out_guide(%%i): %s (", sxstrget (nt2ste [A]));
...
iprdct_found_in_guide++;\n\
	  ...
	  if (A_is_cyclic) {
	    ...
	    fprintf (parser_file, "#if EBUG\nprintf (\"(%%s)out_memo_cycle(%%i): %s (", sxstrget (nt2ste [A]));
...
#else /* !is_earley_guide */\n\
clause_id_set = nt2clause_id_set_%i;\n\
#endif /* !is_earley_guide */\n", A);
	    }
	    else {
	      if (is_1_RCG) {
		...
		if (A_arity == 1) {
		  ...
		  for (pos = 0; pos < A_arity; pos++) {
		    if (pos == 0)
		      ...
		      fprintf (parser_file, "sxba_and (clause_id_set, ptcmemoP [tcrho [%i]]);\n", pos);
		  }
		}

		fputs ("#else /* !is_guided4 */\n", parser_file);
		fprintf (parser_file, "clause_id_set = nt2clause_id_set_%i;\n", A);
		fputs ("#endif /* !is_guided4 */\n", parser_file);
	      }
	      else {
		fprintf (parser_file, "clause_id_set = nt2clause_id_set_%i;\n", A);
...
#if 0
	    fprintf (parser_file, "for (xlex_code = nt2lex_code [%i]; (clause_id = lex_code [xlex_code]) != 0; xlex_code++)\n", A);
#endif /* 0 */

	    fputs ("local_ret_val = -1;\n\
switch (clause_id) {\n\
#endif /* is_lex */\n", parser_file);
	  }
	  ...
	  while ((clause = sxba_scan (A_clause_set, clause)) > 0) {
		...
		fprintf (parser_file, "if (fsa_clause2ub (%i, iub[0])", clause);

		if (rfsa_file)
		  fprintf (parser_file, " && fsa_clause2lb (%i, ilb[0])", clause);
		...
		if (A_clause_nb == 1) {
		  ...
		  fputs ("#if is_lex\n", parser_file);
#if 0
		  fprintf (parser_file, "if (lex_code [nt2lex_code [%i]] == 1)\n", A);
#endif /* 0 */
...
#else /* !is_earley_guide */\n\
if (SXBA_bit_is_set (nt2clause_id_set_%i, 1))\n\
#endif /* !is_earley_guide */\n", A);
		  }
		  else {
		    if (is_1_RCG) {
			...
			for (pos = 0; pos < A_arity; pos++) {
			  if (pos == 0)
			    ...
			    fprintf (parser_file, "\n&& SXBA_bit_is_set (ptcmemoP [tcrho [%i]], 1)", pos);
			}
			...
			fputs (")\n", parser_file);
		      ...
		      fputs ("#else /* !is_guided4 */\n", parser_file);
		      fprintf (parser_file, "if (SXBA_bit_is_set (nt2clause_id_set_%i, 1))\n", A);
		      fputs ("#endif /* !is_guided4 */\n", parser_file);
		    }
		    else {
		      fprintf (parser_file, "if (SXBA_bit_is_set (nt2clause_id_set_%i, 1))\n", A);
		...
		fprintf (parser_file, "rho [0] = %i;\n", clause);

		if (SXBA_bit_is_set (ext_decl_clause_set, clause))
		  fprintf (parser_file, "if (is_useful_%i)\n", clause);

		/* Le Ven 19 Jan 2001 13:44:51 */
		if (is_1_RCG) {
		  int rhs_pos, rhs_nt, cur3, bot3, top3, arg_no;
		  ...
		  for (k = 0; k < A_arity; k++) {
		    ...
		    fprintf (parser_file, "%i, G.pid, tcrho [%i], %i)",
			     k, k, clausek2guide_clause [clause_k2clausek (clause, k)]);
		  }
		  ...
		  for (k = 0; k < A_arity; k++) {
		    if (k > 0)
		      fprintf (parser_file, "pnode%i = pnodei%i;\n", k, k);
			...
			for (cur3 = bot3; cur3 < top3; cur3++) {
			  ...
			  if (arg_no == k) {
			    ...
			    if (cur3 == bot3) {
			      fprintf (parser_file,
				       "olb[%i] = ij2i(range), oub[%i] = ij2j(range);\n",
				       rhs_pos, rhs_pos);
			    }
			    else {
			      fprintf (parser_file,
				       "if (olb[%i] != ij2i(range) || oub[%i] != ij2j(range)) continue;\n",
				       rhs_pos, rhs_pos);
			    }
			  }
			}
		  ...
		  for (k = A_arity-1; 0 <= k; k--) {
		    fprintf (parser_file, "} while (pnode%i ? pnode%i = pnode%i->next : NULL);\n", k, k, k);
		  }
		...
		gen_clause_comment (parser_file, clause);
		fprintf (parser_file, "/* %i: Useless clause, identical to #%i (id = %i) */\n",
			 clause, main_clause, clause_id);
#if 0
...
if (ret_val > 0 && is_glbl_cycle) break;\n\
	  ...
	  fprintf (parser_file, "#if EBUG\nprintf (\"(%%s)out(%%i): %s (", sxstrget (nt2ste [A]));
  ...
  if (has_semantics) {
	...
	if ((main_A = nt2identical [A]) != A) {
	  ...
	  if (A_clause_nb) {
	    ...
	    fputs ("int\t*p, clause, clause_id;\n", parser_file);
	    fputs ("BOOLEAN\tret_val, local_ret_val;\n", parser_file);
	    fputs ("struct spf_node\t**prev_icp;\n", parser_file);

	    if (A_is_cyclic) {
	      fputs ("#if is_cyclic\n", parser_file);
	      fputs ("BOOLEAN\tis_cyclic_ip;\n", parser_file);
		...
		if (pos+1 < A_arity)
	      ...
	      for (pos = 0; pos < A_arity; pos++) {
		fprintf (parser_file, ", lower_bounds[%i], upper_bounds[%i]", pos, pos);
	      }
...
memoP [rho0] [0] = %iL;\n\
	    ...
	    if (A_clause_nb == 1/* && is_first_call */)
	      fprintf (parser_file, "SXBA_1_bit (memoP [rho0], %i);\n", clause2clause_id [clause]);
	    else
...
abort ();\n\
  ...
  fputs ("G.pid = global_pid++;\n", parser_file);
  fprintf (parser_file, "G.degree = %i;\n", max_degree);
  fputs ("Gs [G.pid] = &G;\n", parser_file);
...
rcg_gen_parser ()
{
    int	x, nt, arity, pos, *ptr, Ak, clause;
	...
	varstrings [x] = varstr_alloc (32);
      ...
      lhs_pos2adr = (int*) sxalloc (max_garity+1, sizeof (int));
      lhs_pos2ladr = (int*) sxalloc (max_garity+1, sizeof (int));
      lhs_pos2radr = (int*) sxalloc (max_garity+1, sizeof (int));
      lhs_arg_tbp_set = sxba_calloc (max_garity+1);
      useless_lhs_arg = sxba_calloc (max_garity+1);

      for_loops_nb = (int*) sxcalloc (max_garity+1, sizeof (int));

      rhs_pos2static_size_kind = (char*) sxalloc (max_clause_arg_nb+1, sizeof (char));
      rhs_pos2Ak = (int*) sxalloc (max_clause_arg_nb+1, sizeof (int));
      rhs_pos2nt = (int*) sxalloc (max_clause_arg_nb+1, sizeof (int));
      rhs_pos2param = (int*) sxalloc (max_clause_arg_nb+1, sizeof (int));
      rhs_pos2lmadr = (int*) sxalloc (max_clause_arg_nb+1, sizeof (int));
      rhs_pos2rmadr = (int*) sxalloc (max_clause_arg_nb+1, sizeof (int));
      rhs_pos2rhs_mpos = (int*) sxalloc (max_clause_arg_nb+1, sizeof (int));
	...
	check_non_nul_range_done = sxba_calloc (max_clause_arg_nb+1);
      ...
      olbs = (int*) sxalloc (max_clause_arg_nb+1, sizeof (int));
      oubs = (int*) sxalloc (max_clause_arg_nb+1, sizeof (int));

      D = max_arg_size+1;		/* compte large */
      adr2cur = (int*) sxalloc (max_lhs_adr+1, sizeof (int));
      adr2lA = (int*) sxcalloc (max_lhs_adr+1, sizeof (int));
      adr2uA = (int*) sxcalloc (max_lhs_adr+1, sizeof (int));
      adr2pos = (int*) sxalloc (max_lhs_adr+1, sizeof (int));
      adr_stack = (int*) sxalloc (max_lhs_adr+1, sizeof (int)), adr_stack [0] = 0;
      adr2madr = (int*) sxcalloc (max_lhs_adr+1, sizeof (int));
      synchro2adr = (int*) sxcalloc (max_lhs_adr+1, sizeof (int));
      madr2olb = (int*) sxcalloc (max_lhs_adr+1, sizeof (int));
      madr2oub = (int*) sxcalloc (max_lhs_adr+1, sizeof (int));
      ...
      adr2tpos = (int*) sxalloc (max_lhs_adr+1, sizeof (int));

      atom2min = (int*) sxcalloc (max_gvar+1, sizeof (int));
      atom2max = (int*) sxalloc (max_gvar+1, sizeof (int));
      for (x = 0; x <= max_gvar; x++)
	atom2max [x] = -1; /* inconnu */

      gatom2nb = (int*) sxcalloc (max_gvar+1, sizeof (int));
      atom2var_nb = (int*) sxalloc (max_gvar+1, sizeof (int));
      atom2static_size = (int*) sxalloc (max_gvar+1, sizeof (int));
      atom_stack = (int*) sxalloc (max_gvar+1, sizeof (int)), atom_stack [0] = 0;
      Asize_set = sxba_calloc (max_gvar+1);
      XxY_alloc (&VxV, "VxV", 2*(max_gvar) + 1, 1, 1, 1, NULL, NULL);

      ext_decl2clause = (int*) sxcalloc (max_nt+1, sizeof (int));
      ...
      var2atom = (int*) sxcalloc (max_gvar+1, sizeof (int));
      var2nb = (int*) sxcalloc (max_gvar+1, sizeof (int));
      var2lhs_pos = (int*) sxcalloc (max_gvar+1, sizeof (int));

      XH_alloc (&XH_prdct_args, "XH_prdct_args", max_rhs_prdct + 1, 1, max_garity, XH_prdct_args_oflw, NULL);
      XH_prdct_args2mprdct_pos = (int*) sxalloc (XH_size (XH_prdct_args)+1, sizeof (int));

      prdct_pos2mprdct_pos = (int*) sxalloc (max_rhs_prdct+2, sizeof (int));
      mprdct_pos2generated = (int*) sxalloc (max_rhs_prdct+2, sizeof (int));

      for (x = max_rhs_prdct+1; x >= 0; x--) {
	mprdct_pos2generated [x] = -1;
      }
    
      rhs_prdct_pos2tooth = (int*) sxalloc (max_rhs_prdct+2, sizeof (int));
      rhs_prdct_pos2generator_pos = (int*) sxalloc (max_rhs_prdct+2, sizeof (int));
      rhs_prdct_nyp = sxba_calloc (max_rhs_prdct+2);

      component2pos = (int*) sxalloc (max_gcomponent_nb+1, sizeof (int));
      component2bot = (int*) sxalloc (max_gcomponent_nb+1, sizeof (int));
      component2top = (int*) sxalloc (max_gcomponent_nb+1, sizeof (int));
      component2lmadr = (int*) sxalloc (max_gcomponent_nb+1, sizeof (int));
      component2rmadr = (int*) sxalloc (max_gcomponent_nb+1, sizeof (int));

      eq_top = max_clause_arg_nb;
      equation = (int**) sxalloc (eq_top+1, sizeof (int*));
      ptr = eq_list = (int*) sxcalloc ((eq_top+1)*(max_gvar+1)+1, sizeof (int));
      equation2Xnb = (int*) sxalloc (eq_top+1, sizeof (int));
      equation_stack = (int*) sxalloc (eq_top+1, sizeof (int)), equation_stack [0] = 0;
      equation_stack2 = (int*) sxalloc (eq_top+1, sizeof (int)), equation_stack2 [0] = 0;

      equation_already_substituted_set = sxba_calloc (eq_top+1);
      nyp_eq_set = sxba_calloc (eq_top+1);
      equation2vstr_kind = (int*) sxalloc (eq_top+1, sizeof (int));
	...
	lhs_arg2pos = (int*) sxalloc (max_garity+2, sizeof (int));

	size = 1+max_lhs_adr;
	pos2symb = (int*) sxalloc (size, sizeof (int));
	pos2main = (int*) sxcalloc (size, sizeof (int));
	pos2lhs_arg = (int*) sxalloc (size, sizeof (int));
	pos2bound = (int*) sxalloc (size, sizeof (int));
	pos2pos_set = sxbm_calloc (size, size);
	pos_hd_set = sxba_calloc (size);
	posxpos2lgth = (int**) sxalloc (size, sizeof (int*));
	p = (int*) sxalloc (size*size, sizeof (int));
	...
	k_guide_set = sxba_calloc (max_garity+1);
      ...
      clause2clause_id = (int*) sxcalloc (last_clause+1, sizeof (int));

      A2max_clause_id = (int*) sxcalloc (max_nt+1, sizeof (int));
	...
	sxfree (is_last_vide), is_last_vide = NULL;
      ...
      if (sxverbosep) {
	fprintf (sxtty, "(arity=%i, clause_nb=%i, degree=%i for clause #%i, size=%i) done\n",
		 max_garity, last_clause, max_degree, max_degree_clause, gsize); /* Temps d'analyse polynomial, on imprime le degree maximal */
...
for (/* each A_clause */) {
  ...
  case A_clause:
	...
	/* SXBA_bit_is_set (memoP [*rho0], %i) == FALSE */
        (*G.semact.first_pass) ();
      }
      /* ... */
    } while (0);
