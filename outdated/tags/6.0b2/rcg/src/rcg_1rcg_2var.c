...
/************************************************************************/
/* Mar 19 Sep 2000 14:08 (pb):	Ajout de cette rubrique "modifications"	*/
/************************************************************************/
...
#include <string.h>
#include <stdlib.h>
char WHAT_RCG1RCG2VAR[] = "@(#)SYNTAX - $Id: rcg_1rcg_2var.c 642 2007-06-21 08:00:52Z rlacroix $" WHAT_DEBUG;

extern BOOLEAN is_arg_in_Tstar (int param);

/* commun a -lbnf et -lfsa */
static int *nt2lhs_nb, *nt2rhs_nb; /* Cas is_print == FALSE */

static char	lst_name [131];
static FILE	*out_file;

static int	*clause2disp, *new_clause2disp, *new_clauses_body, *nt2clause_disp, *clause_disp2clause;
static int	new_max_rhs_prdct, new_prdct_nb, new_clauses_body_top, new_axiom, init_clause_nb;
static BOOLEAN	to_be_memorized;
static int	*clause2clause2var_form;
static int	*clause2new_clause, *new_clause2rhs_disp, *new_clause2rhs_list, new_clause2rhs_list_top;
...
#define EOC	((int)(((unsigned int)(~0))>>1))
#define EOA	(EOC-1)
#define EOP	(EOA-1)

/* ********************************************************************************** */
static XH_header	local_prdct_calls, args_1rcg;
static int		*call_kinds;
...
static void
out_header (format, file_name, kind_str)
    char	*format, *file_name, *kind_str;
{
    long	date_time;
...
static void
gen_false_clause (prefix, suffix)
    char	*prefix, *suffix;
{
    int t, nt, arity;
...
static int
is_sub_arg (lhs_param, rhs_param)
    int lhs_param, rhs_param;
{
    /* retourne la position de rhs_param ds lhs_param ou -1 */
    int lhs_bot3, lhs_top3, rhs_bot3, rhs_top3, rhs_size, lhs_cur3, lcur3, rcur3, k;
...
compar (p1, p2)
  const int	*p1, *p2;
{
    int	bot1, bot2, top1, top2, cur1, cur2, arg1, arg2;
    int x1 = *p1, x2 = *p2;
...
static void
args_1rcg_oflw (old_size, new_size)
...
one_rcg ()
{
  int 	clause, lhs_prdct, lhs_bot, lhs_top, lhs_cur, lhs_nt, lhs_param, lhs_pos, xbot, xtop, Ak, k;
  int		rhs, rhs_cur, rhs_top, rhs_prdct, rhs_bot2, rhs_nt, rhs_param, rhs_param1, rhs_top2, rhs_pos, rhs_cur2;
  int 	is_pos_call, xcall, x, bot3, top3, new_var, cur3, symb, bot, top, cur, y, len, high, low, IJ, I, J, AI;
  XH_header	XH_1rcg_clauses; /* INUTILE ds la version courante !! */
  SXBA	local_Ak_set, t_set, strlen_set;
  int		*to_be_sorted, *var2new, *local_rhs_param, *call_pos;
  BOOLEAN	rhs_ok, comment_clause, pushed, is_arg1, is_arg2, has_t;
  int		rhs_pos2lhs_pos_top, rhs_pos2disp_top, rhs_arg_pos, rhs_pos_1rcg;
  int		lhs_cur2, lhs_param2, lhs_cur3, lhs_bot3, lhs_top3, list_elem, size, rhs_top3, rhs_cur3;
  int		*guide_A2guide_Aclause, guide_lhs_nt, guide_rhs_nt;
  ...
  XH_header     prod_hd;
  int           lognt, left_prod, right_prod, bot2, top2, xcur;
  BOOLEAN       is_skipped;
  ...
  guide_clause2clause = (int *) sxalloc (last_clause*max_garity+1, sizeof (int));
  guide_clause2clause_id = (int *) sxalloc (last_clause*max_garity+1, sizeof (int));
  guide_clause2clause_id [0] = 0;
  guide_clause2arg_pos = (int *) sxalloc (last_clause*max_garity+1, sizeof (int));
  ...
  local_rhs_param = (int *) sxalloc (max_Ak+1, sizeof (int));
  ...
  call_kinds = (int *) sxalloc (size+1, sizeof (int));
  call_pos = (int *) sxalloc (size+1, sizeof (int));
  to_be_sorted = (int *) sxalloc (size+1, sizeof (int));
  var2new = (int *) sxalloc (max_gvar+1, sizeof (int));

  clause2rhs_pos = (int *) sxalloc (last_clause+2, sizeof (int));
  rhs_pos2disp = (int *) sxalloc (last_clause*size+2, sizeof (int));
  rhs_pos2rhs_nt = (int *) sxalloc (last_clause*size+2, sizeof (int));
  rhs_pos2lhs_pos = (int *) sxalloc (last_clause*size*max_garity+1, sizeof (int));
  rhs_pos2lhs_arg_no = (int *) sxalloc (last_clause*size*max_garity+1, sizeof (int));
  rhs_pos2rhs_pos_1rcg = (int *) sxalloc (last_clause*size*max_garity+1, sizeof (int));
  rhs_pos2lhs_pos_top = rhs_pos2disp_top = 1;
  rhs_pos2disp [rhs_pos2disp_top] = rhs_pos2lhs_pos_top;

  t_set = sxba_calloc (max_t+1);

  Ak2guide_nt = (int *) sxalloc (max_Ak+1, sizeof (int));
  XxY_alloc (&guide_nts, "guide_nts", max_Ak+1, 1, 0, 0, NULL, statistics);
  clausek2guide_Aclause = (int *) sxalloc (max_clausek+1, sizeof (int));
  clausek2guide_clause = (int *) sxalloc (max_clausek+1, sizeof (int));
  guide_A2guide_Aclause = (int *) sxcalloc (max_Ak+1, sizeof (int));
  ...
  for (clause = init_clause_nb; clause <= last_clause; clause++) {
	...
	if (rhs_ok) {
	  ...
	  if (xcall > 0) {
	    ...
	    if (xcall > 1) {
	      ...
	      qsort (to_be_sorted+1, xcall, sizeof (int), (int (*) (const void *, const void *))compar);
	    }
	  }
	...
	if (rhs_ok) {
	  ...
	  if (lrprod_file) {
...
#if 0
	      /* debug */
	      fprintf (stderr, "clause=%i, guide_clause=%i, pos=%i, prod=%i\n", clause, guide_clause, lhs_pos, right_prod);
#endif
	    ...
	    /* debug */
	      ...
	      fprintf (stderr, "clause=%i, guide_clause=%i, pos=%i, SKIPPED\n", clause, guide_clause, lhs_pos);
	    }
#endif
	  }
	  ...
	  out_nt_text (lhs_nt);
#if 0
	  fprintf (out_file, "_%i_%i(", lhs_nt, lhs_pos);
#endif
	  fprintf (out_file, "_%i(", guide_lhs_nt);
	  ...
	  while (xbot < xtop) {
	    ...
	    if (rhs_nt != 0) {
	      ...
	      if (rhs_nt > 0) {
#if 0
		fprintf (out_file, "_%i_%i", rhs_nt, rhs_pos);
#endif
		fprintf (out_file, "_%i", guide_rhs_nt);
	      }
	...
	if (SXBA_bit_is_set (loop_clause_set, clause)) {
	  ...
	  fprintf (out_file, "\n%% %i is identical to clause #%i\n", clause, clause2identical [clause]);
	}
      }
    }
  }
  ...
  if (lrprod_file) {
    int max_prod, prod, left_prod_nb, right_prod_nb, prod_nb, one_rcg_left_prod, one_rcg_right_prod;
    unsigned int v;
    SXBA line;
...
#define IS_SINGLE_PROD X20
    ...
    fprintf (out_file, "#define LEFT_RIGHT_PROD_NB %i\n\n", max_prod = XH_top (prod_hd)-1);
...
Chacune de ces productions peut etre gauche ou droite, la liste est terminee par 0. */\n"
	   , out_file);
    fputs ("\nstatic unsigned int left_right_prod_hd [] = {0,\n", out_file);
	...
	if (prod_nb == 1) {
      ...
      fprintf (out_file, "/* %i */ 0X%X,\n", prod, v);
    }

    fputs ("};\n", out_file);

    fputs ("\nstatic unsigned int left_right_prod_list [] = {0,\n", out_file);
	...
	if (right_prod_nb) {
	  ...
	  while ((one_rcg_right_prod = sxba_scan (line, one_rcg_right_prod)) > 0) {
	    fprintf (out_file, "0X%X, ", one_rcg_right_prod|IS_RIGHT_PROD);
	  }
	}
	...
	if (left_prod_nb) {
	  ...
	  while ((one_rcg_left_prod = sxba_scan (line, one_rcg_left_prod)) > 0) {
	    fprintf (out_file, "0X%X, ", one_rcg_left_prod|IS_LEFT_PROD);
	  }
	}
...
/* ******************************************************************************************************* */
static int	min_degree, cover_size;
static SXBA	t_set, local_t_set, var_set, known_lgth_var_set;
static BOOLEAN	is_t_set, is_local_t_set, has_known_lgth_var, block_extraction, previous_block_extraction;

static int	arg2bot_top_top, arg2bot_top_size, new_var_nb;
static int	*pos2symb, *slice2disp, *slice2new_clause, *var2eq, *var2lgth, *var_stack, *equal_pair_stack, *streqlen_pair_stack;
static int	*var2eq_len, *eq2nb;
static SXBA	eq_set, local_eq_set, best_eq_set, previous_best_eq_set, close_eq_set, cover_set, eq_len_set;

static struct	new_nts {
    int	old_clause, new_clause;
    } *new_nts;
static int	*nt2ident, new_nts_size, new_nts_top, *slice2new_nt;
...
static int
lhs_gen_or_degree (p, arity, is_gen/* , is_main */)
    struct arg2bot_top	*p;
    int arity;
    BOOLEAN /* is_main, */is_gen;
{
    int		degree, arg, pos, top, bot, cur, lpos, rpos, symb, eq, super_var_nb, eq_len;
    BOOLEAN	is_first;
...
fill_local_eq_set (p, arity)
    ...
    int arity;
{
    int	arg, pos, top, lpos, rpos, symb, cur_eq, eq_len, cur;
    ...
    int			clause;
{
    ...
    int 		cur_slice, slice, arity, eq, degree, arg, bot, top;
    int			symb, cur_eq, best_eq, pos, lpos, rhs, cur;
    int			rhs_prdct, rhs_bot2, rhs_nt, rhs_top2, rhs_param, rhs_cur2, rhs_bot3, x, X, Y, rhs_pos; 
    struct arg2bot_top	*p;
    BOOLEAN		is_main, OK, done, is_first, before, after;
	...
	if (!is_main) {
		...
		fprintf (out_file, "clause_%i_%i", clause, slice2new_clause [cur_slice]);
	    }
	}
	...
	if (rhs > 0) {
		...
		while ((symb = sxba_scan_reset (var_set, symb)) > 0) {
			...
			fprintf (out_file, "(%i, ", var2lgth [symb]);
	...
	if (new_var_nb > max_gvar) {
		...
		if (to_be_memorized) {
		    ...
		    fprintf (out_file, "clause_%i_%i(", clause, new_clause_nb);
		}
...
put_in_var2eq_len ()
{
    ...
    int top, X, Y, Xeq_len, Yeq_len, eq_len, old_eq_len, var;
...
static void process_clause_equiv ();
static void print_new_clauses ();
void
two_var_form ()
{
    int		clause, lhs_prdct, lhs_bot2, lhs_nt, lhs_param, symb, symb1, symb2;
    int		rhs, rhs_prdct, rhs_bot2, rhs_nt, rhs_param, Ak, rhs_bot3, rhs_top3, rhs_cur3;
    int		lhs_top2, lhs_arity, lhs_pos, lhs_arg, lhs_cur2, lhs_bot3, lhs_top3, lhs_cur3, rhs_top, rhs_cur;
    int		rhs_top2, k, rhs_cur2, lgth, high, low, I, J, IJ, top, size;
    int		new_clause;
    BOOLEAN	is_pos_call;
    ...
    pos2symb = (int*) sxalloc (1+max_lhs_adr, sizeof (int));
    var2eq = (int*) sxalloc (1+max_gvar, sizeof (int));
    var2eq_len = (int*) sxalloc (1+max_gvar, sizeof (int));
    var2lgth = (int*) sxalloc (1+max_gvar, sizeof (int));
    var_stack = (int*) sxalloc (1+max_gvar, sizeof (int));
    var_stack [0] = 0;
    ...
    eq2nb = (int*) sxalloc (max_gvar+1, sizeof (int));
    ...
    slice2disp = (int*) sxalloc (max_gvar+1, sizeof (int));
    slice2new_clause = (int*) sxalloc (max_gvar+1, sizeof (int));
    equal_pair_stack = (int*) sxalloc (2*max_gvar+1, sizeof (int));
    equal_pair_stack [0] = 0;
    streqlen_pair_stack = (int*) sxalloc (2*max_gvar+1, sizeof (int));
    streqlen_pair_stack [0] = 0;

    clause2clause2var_form = (int*) sxcalloc (last_clause+1, sizeof (int));
    from2var2clause_disp = (int*) sxalloc (last_clause*(max_rhs_prdct+1)+2, sizeof (int));
    from2var2clause_list = (int*) sxalloc (last_clause*(max_rhs_prdct+1)+2, sizeof (int));
    ...
    clause2new_clause = (int*) sxcalloc (last_clause+1, sizeof (int));
    new_clause2rhs_disp  = (int*) sxalloc (last_clause*(max_rhs_prdct+1)+2, sizeof (int));
    size = last_clause*(max_rhs_prdct+max_clause_arg_nb*max_arg_size);
    new_clause2rhs_list = (int*) sxalloc (size+1, sizeof (int));
    new_clause2rhs_disp [new_clause_nb+1] = new_clause2rhs_list_top = 1;

    if (is_factorize) {
	clause2disp = (int*) sxalloc (last_clause+2, sizeof (int));
	new_clause2disp = (int*) sxalloc (last_clause*(max_rhs_prdct+1)+2, sizeof (int));
	size = last_clause*(3/*call_kind+nt+EOP*/*(max_rhs_prdct+1/*lhs_prdct*/)+
			    max_clause_arg_nb*(max_arg_size+1/*EOA*/)
			    +1/*EOC*/);
	new_clauses_body = (int*) sxalloc (size+1, sizeof (int));
	new_nts = (struct new_nts*) sxalloc ((new_nts_size = last_clause)+1, sizeof (struct new_nts));
	slice2new_nt = (int*) sxalloc (max_gvar+1, sizeof (int));
    ...
    from2var2clause = (int*) sxcalloc (new_clause_nb+1, sizeof (int));
...

{
    ...
    int		*pos_list2clause, x;
    XH_header	XH_pos_list;

    XH_alloc (&XH_pos_list, "XH_pos_list", new_clause_nb+1, 1, max_rhs_prdct+1, NULL, statistics);
    pos_list2clause = (int*) sxalloc (last_clause+1, sizeof (int));
    clause2rhs_stack_map = (int*) sxalloc (last_clause+1, sizeof (int));
    clause2rhs_stack_map [0] = 0;
    rhs_stack_map = (int*) sxalloc (last_clause*(max_rhs_prdct+1)+1, sizeof (int));
    rhs_stack_map [0] = 0;
    rhs_stack_map_top = 1;
...
static int		max_new_nt, max_peql;
static int		*nf_clause2lhs, *nf_clause2rhs, *clause2next, *clause2lhs_nt;
static int		*new_clause_rhs_nts, new_clause_rhs_nts_top, *new_clause2rhs_nt_disp;
static int		*nt2clause_nb, *clause2eq, *clause2eq_list, *nt2ident_list, *clause2ident, *clause2ident_list;
...
normal_form ()
{
    ...
    int		clause, new_clause_top, new_clause, bot, cur, top, new_var, symb, symb_list, prdct_call_nb, nf_prdct_pos;
    int		arg_list, lhs_nt, item;
    int		*var2new, *to_be_sorted;
    XH_header	XH_prdct_list, XH_arg_list, XH_symb_list;

    var2new = (int*) sxcalloc (2*max_gvar+1, sizeof (int));
    to_be_sorted = (int *) sxalloc (new_max_rhs_prdct+1, sizeof (int));
	...
	for (new_clause = clause2disp [clause]; new_clause < new_clause_top; new_clause++) {
		...
		if (prdct_call_nb > 1)
		    qsort (to_be_sorted, prdct_call_nb, sizeof (int), (int (*) (const void *, const void *))new_compar);
...
is_pseudo_equiv ()
{
    int		new_clause, nf_clause, prev_clause, l, lhs;
    int		*nf_clause2clause;
    XxY_header	nf_clause_hd;

    nf_clause2clause = (int*) sxalloc (new_clause_nb+1, sizeof (int));
...
#if 0
		fprintf (out_file, "%i et %i sont pseudo-equivalents.\n", nf_clause2clause [nf_clause], new_clause);
#endif
	    }
	    else {
...
static BOOLEAN
is_clause1_eq_clause2 (clause1, clause2)
    int clause1, clause2;
{
    ...
    int bot1, cur1, top1, bot2, cur2, nt1, nt2;
...
static void
put_in_eq (eq, eq_list, it1, it2)
    int *eq, *eq_list, it1, it2;
{
    /* it1 < it2 */
...
static BOOLEAN
is_nt1_eq_nt2 (nt1, nt2)
    int nt1, nt2;
{
    int		bot1, cur1, top1, bot2, cur2, top2, clause1, clause2, nb1, nb2;
    BOOLEAN	ret_val;
...
process_clause_equiv ()
{
    ...
    int		nt, clause, lhs_nt, next, clause1, clause2, nt1, nt2, nb1, top, next1, next2;
    int		*clause_stack;
    BOOLEAN	done, has_eq;

    nf_clause2lhs = (int *) sxalloc (new_clause_nb+1, sizeof (int));
    nf_clause2rhs = (int *) sxalloc (new_clause_nb+1, sizeof (int));

    clause2next = (int *) sxalloc (new_clause_nb+1, sizeof (int));
    clause2eq = (int *) sxalloc (new_clause_nb+1, sizeof (int));
    clause2eq_list = (int *) sxalloc (new_clause_nb+1, sizeof (int));
    clause2ident = (int *) sxalloc (new_clause_nb+1, sizeof (int));
    clause2ident_list = (int *) sxalloc (new_clause_nb+1, sizeof (int));
    ...
    clause2lhs_nt = (int *) sxalloc (new_clause_nb+1, sizeof (int));
    new_clause2rhs_nt_disp = (int *) sxalloc (new_clause_nb+2, sizeof (int));
    new_clause2rhs_nt_disp [1] = 1;

    new_clause_rhs_nts = (int *) sxalloc (new_clause_nb*max_rhs_prdct+1, sizeof (int));
    pseudo_equiv_hd_set = sxba_calloc (new_clause_nb+1);

    max_new_nt = max_nt+new_nts_top;
    nt2ident_list = (int *) sxalloc (max_new_nt+1, sizeof (int));
    nt2ident = (int *) sxalloc (max_new_nt+1, sizeof (int));

    for (nt = 0; nt <= max_new_nt; nt++)
	nt2ident [nt] = nt2ident_list [nt] = nt;

    nt2clause_nb = (int *) sxcalloc (max_new_nt+1, sizeof (int));
	...
	nt2clause_disp = (int *) sxalloc (max_new_nt+3, sizeof (int));
	clause_disp2clause = (int *) sxalloc (new_clause_nb+1, sizeof (int));
	clause_stack = (int*) sxalloc (new_clause_nb+1, sizeof (int));
	clause_stack [0] = 0;
		...
		if ((top = clause_stack [0]) > 0) {
			...
			while (next1 > clause1) {
				...
				if (nt1 == nt2) {
				    ...
				    if (check_set) {
					sxput_error (clause2visited [clause2]->token.source_index,
						     "%sEquivalent with the clause defined at line #%i.",
				...
				if (nb1 <= 0  && (nt2clause_nb [nt2] <= 0)) {
					...
					if (is_nt1_eq_nt2 (nt1, nt2)) {
					    ...
					    if (nt1 > max_nt) {
						fprintf (out_file, "clause_%i_%i",
							 new_nts [nt1-max_nt].old_clause, new_nts [nt1-max_nt].new_clause);
					    }
					    else
						out_nt_text (nt1);

					    fprintf (out_file, "(nt1 = %i) and ", nt1);
					    ...
					    fprintf (out_file, "(nt2 = %i) are equivalent.\n", nt2);
#endif
...
static int
print_new_prdct (is_lhs, cur)
    BOOLEAN	is_lhs;
    int 	cur;
{
    int 	call_kind, nt, symb;
    BOOLEAN 	is_strlen;
    ...
    if (nt > max_nt) {
	nt -= max_nt;
	fprintf (out_file, "clause_%i_%i", new_nts [nt].old_clause, new_nts [nt].new_clause);
...
print_new_clauses ()
{
    int		clause, new_clause_top, new_clause, bot, lhs_nt, top;
    BOOLEAN	is_first;
    int		*new_clause2lnk, *lnk2clause, *lnk2lnk, lnk, lnk_top, clo;

    new_clause2lnk = (int*) sxalloc (new_clause_nb+1, sizeof (int));
    lnk2clause = (int*) sxalloc (new_clause_nb+1, sizeof (int));
    lnk2lnk = (int*) sxalloc (new_clause_nb+1, sizeof (int));
    ...
    int nt1, nt2;
{
    ...
    int		bot1, cur1, top1, bot2, cur2, top2, clause, clause1, clause2;
...
do_reduction ()
{
    int		size, clause, prdct, bot2, top2, call_kind, nt, param, bot3, top3, symb, bot, top, *pnt, new_nt, rhs;
    int		nt1, nt2, clause2, next2;
...
static BOOLEAN is_bnf_in_reverse, fsa_has_reduction;
...
static BOOLEAN has_null_trans, is_deterministic;
static int     tmax, ntmax, last_prod;

static struct bnf {
  int tmax, ntmax, maxnpd, rhs_lgth, nbpro, indpro, last_action;
  int *reduc, *prolon, *numpg, *lispro, *prolis, *npd, *npg, *numpd, *action;
} bnf;
...
static int pda_assoc_list [6] = {1, 0, 1, 1, 0, 0}; /* X, Y, Z, XY, XZ, YZ */
static int ndfsa_assoc_list [6] = {1, 0, 0, 1, 0, 0}; /* X, Y, Z, XY, XZ, YZ */
static int dfsa_assoc_list [6] = {1, 0, 0, 0, 0, 0}; /* X, Y, Z, XY, XZ, YZ */
static int fsa_assoc_list [6] = {1, 0, 0, 0, 0, 0}; /* X, Y, Z, XY, XZ, YZ */
static int pda_state_nb, pda_init_state;
static int ndfsa_init_state, ndfsa_state_nb;
static int *ndfsa_state_stack;
static int dfsa_init_state, dfsa_state_nb, fsa_init_state, fsa_state_nb;
...
static BOOLEAN
fill_lex_set (clause, fill_lex_set_lex_set)
     int clause;
     SXBA fill_lex_set_lex_set;
{
  int prdct, bot2, param, cur3, top3, symb, rhs, cur, top;
...
static void
free_bnf (free_bnf_bnf)
     struct bnf free_bnf_bnf;
{
...
simple1_rcg2cfg ()
{
  ...
  bnf.reduc = (int*) sxalloc (bnf.last_action+2, sizeof (int));
  bnf.prolon = (int*) sxalloc (bnf.last_action+2, sizeof (int));
  bnf.numpg = (int*) sxalloc (bnf.last_action+2, sizeof (int));
  bnf.lispro = (int*) sxalloc (bnf.last_action*(max_arg_size+1)+4, sizeof (int));
  bnf.prolis = (int*) sxalloc (bnf.last_action*(max_arg_size+1)+4, sizeof (int));
  bnf.npd = (int*) sxalloc (bnf.ntmax+2, sizeof (int));
  bnf.npg = (int*) sxalloc (bnf.ntmax+2, sizeof (int));

  bnf.action = (int*) sxcalloc (bnf.last_action+1, sizeof (int));

  nt2lhs_nb = (int*) sxcalloc (bnf.ntmax+1, sizeof (int));
  nt2rhs_nb = (int*) sxcalloc (bnf.ntmax+1, sizeof (int));
  ...
  bnf.numpd = (int*) /* 0:bnf.maxnpd */ sxalloc (bnf.maxnpd + 1, sizeof (int));
...
static void
reduce (s, pi, delta, prod)
    int	s, pi, delta, prod;
{
  int A, q, triple, foo, piq, X;
  ...
  if (s != 0) {
	...
	s = 0;
      ...
      if ((piq = make_ndfsa_state (s, 0, 0, q)) > 0) {
	/* nouveau */
	...
	q = -triple;
      ...
      if ((piq = make_ndfsa_state (0, pi, delta, q)) > 0) {
	/* nouveau */
	S1_push (ndfsa_state_stack, piq);
      }
      else
...
static void
pred (q, pi, delta, l, prod)
    int q, pi, delta, l, prod;
{
  int triple, s;
  ...
  if (l == 0)
	...
	sxtrap (ME, "pred");
#endif
      pred (0, pi, delta-1, l-1, prod);
    }
  }
}

static void
ndfsa_states_oflw (old_size, new_size)
     int old_size, new_size;
{
...
pda2ndfsa ()
{
  int pi, delta, s, triple, X, q, l, piq, foo, prod;
	...
	if (X <= 0) {
	  ...
	  if (X < -bnf.tmax) {
		...
		piq = -piq;
	      ...
	      if (X < 0) {
		/* scan-reduce */
		if ((piq = make_ndfsa_state (0, pi, delta-1, -triple)) > 0) {
		  /* nouveau */
		  S1_push (ndfsa_state_stack, piq);
		}
		else
...
static SXBA	nt_set;
static BOOLEAN	is_t_trans, is_nt_trans;
...
LR0_automaton ()
{
  int	        i, xac1, x, lim, xnt_stack, xt, xnt, item, next_state;
  int           triple, bot;
...
static int *p2kind, *p2c, *c2p, *c2kind;
static BOOLEAN R_is_cyclic, appel_a_fermer;

/* Si c n'est pas deja un cycle, on cree un nouveau cycle de representant p */
static int
create_new_cycle (p, c)
     int p, c;
{
  if (c) return c;
...
is_cyclic (p)
     int p;
{
  int *ppkind, *pqkind, qkind, q, cp, cq;
...
static SXBA*
R_cycle (M, size, nb_de_cycles)
     int size, *nb_de_cycles;
     SXBA *M;
{
  int i, nb;
  int *c2nc;
  ...
  p2kind = (int*) sxcalloc (size, sizeof (int));

  if (appel_a_fermer)
    F = sxbm_calloc (size, size);

  C = sxbm_calloc (size, size);
  c2p = (int*) sxcalloc (size, sizeof (int));
  p2c = (int*) sxcalloc (size, sizeof (int));
  used_p2c = sxba_calloc (size);
  used_c = sxba_calloc (size);
  c2kind = (int*) sxcalloc (size, sizeof (int));
  ...
  if (!appel_a_fermer) {
    if (R_is_cyclic) {
      c2nc = (int*) sxalloc (size, sizeof (int));
...
static void
fill_CRED (size, automaton)
     int size;
     XxYxZ_header automaton;
{
  int p, q, X, triple;
     ...
     XxYxZ_header automaton;
{
  ...
  RED = R_cycle (fill_RED_R, size+1, NULL);
...
static void
dfsa_states_oflw (old_size, new_size)
     int old_size, new_size;
{
...
ndfsa2nfndfsa ()
{
  int             nfstate, state, p, X, nfq, q, triple, foo, nfndfsa_state_nb, nfndfsa_init_state;
  BOOLEAN         is_first;
  int             *nfstate2state, *state2nfstate;
  ...
  nfstate2state = (int*) sxalloc (ndfsa_state_nb+1, sizeof (int));
  state2nfstate = (int*) sxcalloc (ndfsa_state_nb+1, sizeof (int));
...
ndfsa2dfsa ()
{
  ...
  int pi, cur, bot, top, q, triple, X, t, s, prod, pis, foo;
  int *dfsa_state_stack;
...
k_equiv ()
{
  ...
  /* Retourne true si k \neq k-1 */
  int p, triple, X, p2, q, q2, prevp, prevq, new_repr, new_prevq, nextp, nextq;
  BOOLEAN has_changed = FALSE, is_first, p_kequiv;
...
dfsa2min_fsa ()
{
  int p, triple, X, i, pt, dfsa2min_fsa_pred, q, p2, q2, foo, prevp, prevq;
  BOOLEAN p_0equiv;
  int *dfsa_state2t_trans, *dfsa_state2red_trans, *dfsa2fsa;
  SXBA wset, xprod_set;
  ...
  dfsa2eq = (int*) sxalloc (dfsa_state_nb+1, sizeof (int));
  dfsa2eq [0] = 0;

  XH_alloc (&dfsa_trans, "dfsa_trans", dfsa_state_nb+1, 1, (XxYxZ_top (dfsa_hd)/dfsa_state_nb)+1, NULL, statistics);
  dfsa_state2t_trans = (int*) sxalloc (dfsa_state_nb+1, sizeof (int));
  dfsa_state2red_trans = (int*) sxalloc (dfsa_state_nb+1, sizeof (int));
  ...
  pt2p2 =  (int*) sxalloc (bnf.tmax+2, sizeof (int));
  pred2p2 = (int*) sxalloc (last_prod+XH_top(mtrans_hd)+1, sizeof (int));
  ...
  dfsa2fsa = (int*) sxcalloc (dfsa_state_nb+1, sizeof (int));
...
static BOOLEAN
cycle_erasing (state_nb, init_state, final_state_set, automaton, assoc_list)
     int *state_nb, *init_state, *assoc_list;
     SXBA *final_state_set;
     XxYxZ_header *automaton;
{
  /* Il faut calculer le nb de cycles et la taille cumulee des cycles pour connaitre
     le nouveau nb d'etat de ncfsa */
  int     c, p, q, size, p2, p3, ncfsa_init_state, ncfsa_state_nb, foo, triple, r, q2, X, prod, pushed_nb;
  int     cycle_sizes;
  int     *fsa2ncfsa, *fsa2eq, *cycle_erasing_p2c;
  ...
  if (R_nb_de_cycles) {
    ...
    fsa2eq = (int*) sxcalloc (*state_nb+1, sizeof (int));
    cycle_erasing_p2c = (int*) sxalloc (*state_nb+1, sizeof (int));
    cycle_sizes = 0;
    ...
    fsa2ncfsa = (int*) sxcalloc (*state_nb+1, sizeof (int));
...
#if EBUG
    if (sxverbosep) {
      fprintf (sxtty, " %i cycles over %i states ", R_nb_de_cycles, cycle_sizes);
      sxttycol1p = FALSE;
    }
...
reduce_fsa ()
{
  int p, X, q, sq, triple, foo, prod, trans_nb, cur, bot, top, pushed_nb, size = 0;
  int last_xprod = last_prod+XH_top(mtrans_hd);
  int *q2trans_nb;
  SXBA state_set, state_set2, xprod_set, *mtrans2xprod_set, *S_RED = NULL, lower_set, upper_set;
  XxYxZ_header    ncfsa_hd;
  BOOLEAN is_multiple, must_shrink = FALSE, is_final;
  ...
  q2trans_nb = (int*) sxalloc (fsa_state_nb+1, sizeof (int));
  ...
  if (must_shrink) {
    ...
    int ncfsa_state_nb, ncfsa_init_state, ncp, ncq;
    ...
    p2ncp = (int*) sxcalloc (fsa_state_nb+1, sizeof (int));
    ncp2p = (int*) sxalloc (fsa_state_nb+1, sizeof (int));
...
fsa2rcg ()
{
  int clause, prdct, bot2, nt, param, p, triple, foo, r, t, q, red, symb, cur, bot, top;
  SXBA fsa_state_set, line;
  int *state_stack;
  BOOLEAN is_first, has_xtrans, has_null, has_UBs, T0;
  ...
  for (clause = 1; clause <= last_clause; clause++) {
	...
	&& (!useless_clause_set || !SXBA_bit_is_set (useless_clause_set, clause))) {
      ...
      fprintf (out_file, "%% %i: ", clause);
      out_clause (clause);
      fsa_clause++;
      fprintf (out_file, "UB%i() --> . %% %i\n", clause, fsa_clause);
  ...
  while (!IS_EMPTY (state_stack)) {
	...
	if (t < 0) {
	  ...
	  if (t > last_clause && process_single_transition) {
		...
		fprintf (out_file, "q%i(T0 X) --> &StrLen(0, T0) UB%i(T0) ", p, clause);
	    ...
	    if (has_xtrans || has_null) {
	      ...
	      if (has_xtrans) {
		fprintf (out_file, "q%i(T0 X) --> &StrLen(0, T0) ", p);

		for (cur = bot; cur < top; cur++) {
		  clause = XH_list_elem (mtrans_hd, cur);
    
		  if (clause > 0 && !SXBA_bit_is_set (clause_has_lex, clause)) {
		    fprintf (out_file, "UB%i(T0) ", clause);
		  }
		}
	      }
	      else {
		fprintf (out_file, "q%i(X) --> ", p);
	      }
...
#include "sxmatrix2vector.h"
  ...
  int  i, p, X, q, triple, top, *ip, *intarea, fsa_red_top, prod, final_state, prev_final_state, fsa_shift_nb;
  char string [16];
  ...
  fsa_shift.matrix = (int**) sxalloc (fsa_state_nb+1, sizeof (int*));
  ip = intarea = (int*) sxalloc ((fsa_state_nb+1)*(tmax+1), sizeof (int));
  ...
  fsa_red_disp = (int*) sxalloc (fsa_state_nb+2, sizeof (int)), fsa_red_disp [0] = 0;
  ...
  fprintf (out_file, "#define is_lex %i\n", (!is_tagger || is_keep_lex) ? 1 : 0);
#endif
  fprintf (out_file, "#define is_shallow %i\n", is_shallow ? 1 : 0);
  fprintf (out_file, "\n#define fsa_state_nb %i\n", fsa_state_nb);
  fprintf (out_file, "#define last_prod %i\n", last_prod);
  fprintf (out_file, "#define ntmax %i\n", max_nt);
  fprintf (out_file, "/* Pour le debug */\n\
static int final_state = %i, prev_final_state = %i;\n", final_state, prev_final_state);
  ...
  if (fsa_red_top == 1) {
    fputs ("static int *fsa_red_disp;\n", out_file);
    fputs ("static struct trans *fsa_red_trans;\n", out_file);
  }
  else {
    fprintf (out_file, "\nstatic int fsa_red_disp [%i] = {0,", fsa_state_nb+2);

    for (i = 1; i <= fsa_state_nb+1; i++) {
      if ((i%10)==1)
	fprintf (out_file, "\n/* %i */ ", i);

      fprintf (out_file, "%i, ", fsa_red_disp [i]);
    }

    fputs ("\n};\n", out_file);

    fprintf (out_file, "\nstatic struct trans fsa_red_trans [%i] = {{0, 0},", fsa_red_top);

    for (i = 1; i < fsa_red_top; i++) {
      if ((i%10)==1)
	fprintf (out_file, "\n/* %i */ ", i);

      fprintf (out_file, "{%i, %i}, ", fsa_red_trans [i].X, fsa_red_trans [i].q);
    }
  ...
  if ((top = XH_top (mtrans_hd)) > 1) {
    fprintf (out_file, "\nstatic int mtrans_disp [%i] = {0,", top+1);

    for (i = 1; i <= top; i++) {
      if (i % 10 == 1)
	fprintf (out_file, "\n/* %i */ ", i);

      fprintf (out_file, "%i, ", XH_X (mtrans_hd, i));
    }

    fputs ("\n};\n", out_file);

    fprintf (out_file, "static int mtrans_list [%i] = {0,", XH_list_top (mtrans_hd));
    ...
    fputs ("\nstatic int *mtrans_disp, *mtrans_list;\n", out_file);
...
fsa ()
{
...
#if EBUG
  if (sxverbosep) {
    fprintf (sxtty, "%i/%i, ", bnf.nbpro, bnf.indpro);
    sxttycol1p = TRUE;
  }
#endif
  ...
  /* fsa_clause2clause = (int*) sxcalloc (last_clause+2, sizeof (int)); */
...
#if EBUG
  if (sxverbosep) {
    fprintf (sxtty, "%i/%i, NDFSA = ", pda_state_nb, XxYxZ_top (pda_hd));
    sxttycol1p = TRUE;
  }
#endif
...
#if EBUG
  if (sxverbosep) {
    fprintf (sxtty, "%i/%i", ndfsa_state_nb, XxYxZ_top (ndfsa_hd));
    sxttycol1p = TRUE;
  }
#endif
...
#if EBUG
    if (sxverbosep) {
      fprintf (sxtty, " -- cf[%i/%i]", ndfsa_state_nb, XxYxZ_top (ndfsa_hd));
      sxttycol1p = TRUE;
    }
#endif
  }
  ...
  if (has_null_trans && used_nfndfsa) {
    ...
    NULL_RED = R_cycle (fsa_R, ndfsa_state_nb+1, NULL);
...
#if EBUG
    if (sxverbosep) {
      fprintf (sxtty, " -- nf[%i/%i]", ndfsa_state_nb, XxYxZ_top (ndfsa_hd));
      sxttycol1p = TRUE;
    }
#endif
  }
...
#if EBUG
  if (sxverbosep) {
    fprintf (sxtty, "%i/%i, FSA = ", dfsa_state_nb, XxYxZ_top (dfsa_hd));
    sxttycol1p = TRUE;
  }
#endif
...
#if EBUG
  if (sxverbosep) {
    fprintf (sxtty, "%i/%i", fsa_state_nb, XxYxZ_top (fsa_hd));
    sxttycol1p = TRUE;
  }
#endif
...
#if EBUG
    if (sxverbosep) {
      fprintf (sxtty, "[%i/%i] ", last_index, last_red);
      sxttycol1p = TRUE;
    }
#endif
  }
...
lrfsa ()
{
  if (max_garity != 1 || !is_left_linear || !is_right_linear || is_combinatorial_grammar ||
      is_top_down_erasing|| is_bottom_up_erasing) {
    if (sxverbosep) {
  ...
  if (is_shallow) {
	...
	if (symb == EOF || isspace (symb)) {
	  if (varstr_length (prdct_name)) {
	    ...
	    if (ste != ERROR_STE && (nt = ste2nt [ste]) != 0) {
	  ...
	  prdct_name = varstr_catchar (prdct_name, symb);
	}
      } while (symb != EOF);
      ...
      if (unknown_nt_nb)
	fprintf (sxtty, "WARNING: the \"shallow\" file %s contains %i unknown predicate names.\n",
		 shallow_file_name, unknown_nt_nb);
    }
  }
...
static BOOLEAN has_eps_rules, has_eps_trans, eps_rules_are_null;

static int *eps_trans_stack;
static SXBA *EPS_RED;
...
fill_bvide ()
{
  ...
  int	rule, nt, i, top, rhs_nt;
  int *null_stack, *rhs_size;

  null_stack = (int*) sxalloc (ntmax + 1, sizeof (int)), null_stack [0] = 0;
  rhs_size = (int*) sxalloc (bnf.nbpro + 1, sizeof (int));
...
static int efnbpro, efindpro, efsize1, efsize2;
static int *efstack, *efprolon, *eflispro;
     ...
     int item;
{
  ...
  if (X == 0) {
	...
	efbnf.prolon = (int *) sxrealloc (efbnf.prolon, (efsize1 *= 2) + 1, sizeof (int));
	...
	efbnf.lispro = (int *) sxrealloc (efbnf.lispro, (efsize2 *= 2) + 1, sizeof (int));
...
cfg2efcfg ()
{
  int prod, efprod, top, *p, *q, *pbot, symb, efxnumpd, efxnumpg, i, nt;
  struct bnf efbnf;
  int *efnt2lhs_nb, *efnt2rhs_nb;

  efsize1 = 2*bnf.nbpro;
  efsize2 = 2*bnf.indpro;

  efbnf.prolon = (int*) sxalloc (efsize1+1, sizeof (int));
  efbnf.lispro = (int*) sxalloc (efsize2+1, sizeof (int));

  efstack  = (int*) sxalloc (bnf.rhs_lgth+1, sizeof (int)), efstack [0] = 0;
  ...
  if (efbnf.nbpro+1 >= efsize1)
    efbnf.prolon = (int *) sxrealloc (efbnf.prolon, (efsize1 += 1) + 1, sizeof (int));

  if (efbnf.indpro+1 >= efsize2)
    efbnf.lispro = (int *) sxrealloc (efbnf.lispro, (efsize2 += 1) + 1, sizeof (int));

  efbnf.prolon [efbnf.nbpro+1] = efbnf.indpro+1;
  
  efbnf.reduc = (int*) sxalloc (efbnf.nbpro+2, sizeof (int));
  efbnf.numpg = (int*) sxalloc (efbnf.nbpro+2, sizeof (int));
  efbnf.prolis = (int*) sxalloc (efbnf.indpro+2, sizeof (int));
  efbnf.npd = (int*) sxalloc (bnf.ntmax+2, sizeof (int));
  efbnf.npg = (int*) sxalloc (bnf.ntmax+2, sizeof (int));

  efbnf.action = (int*) sxcalloc (efbnf.nbpro+2, sizeof (int));

  efnt2lhs_nb = (int*) sxcalloc (bnf.ntmax+1, sizeof (int));
  efnt2rhs_nb = (int*) sxcalloc (bnf.ntmax+1, sizeof (int));
  ...
  efbnf.numpd = (int*) sxalloc (efbnf.maxnpd + 1, sizeof (int));
...
rcg2bnf (is_print)
     BOOLEAN is_print;
{
  int clause, lhs_prdct, bot2, lhs_nt, lhs_param, rhs, top, bot, cur, rhs_prdct, rhs_nt, rhs_param, l;
  int symb, top3, bot3, cur3, prod_no, len, lognt;
  int *var2nt, *var2strlen;
  XH_header prod_hd;
  ...
  var2nt = (int*) sxalloc (max_gvar+1, sizeof (int));
  var2strlen = (int*) sxalloc (max_gvar+1, sizeof (int));
...
rcg2Lex_file ()
{
  int clause, lhs_prdct, bot2, lhs_nt, lhs_param, rhs_nt_nb, rhs, top, bot, cur, rhs_prdct, rhs_nt, rhs_param, rhs_param2;
  int symb, top3, bot3, cur3, rhs_nt1, prod_no, pos, x, y, len, lognt;
  int *var2nt, *var2strlen, *var2pos;
  int *prod2rhs_stack_map, *rcg2Lex_file_rhs_stack_map, rcg2Lex_file_rhs_stack_map_top;
  XH_header prod_hd;
  int litem, lgth, nb, t, nbpro, indpro, mclause;
  int *clause2prod, *prod2clause, *prolon;
  int *clause2streq, *streq_list;
  ...
  BOOLEAN has_lex, has_identical_Lex, is_map_needed, has_strlen, has_streq;

  int *clause2idem_disp, *clause2idem, *idem2next, itop;
  ...
  var2pos = (int*) sxalloc (max_gvar+1, sizeof (int));
  ...
  clause2prod = (int*) sxcalloc (last_clause+1, sizeof (int));
  prod2clause = (int*) sxcalloc (last_clause+1, sizeof (int));
  prolon = (int*) sxalloc (last_clause+2, sizeof (int));
  prolon [0] = 0;
  prolon [nbpro = 1] = indpro = 4;
  has_Lex = sxba_calloc (last_clause+1);
  rcg2Lex_file_clause2lex_set = sxbm_calloc (last_clause+1, max_t+2);

  clause2idem_disp = (int*) sxcalloc (last_clause+2, sizeof (int));
  clause2idem = (int*) sxalloc (last_clause+1, sizeof (int));
  idem2next = (int*) sxalloc (last_clause+1, sizeof (int));

  rcg2Lex_file_loop_clause_set = sxba_calloc (last_clause+1);

  prod2rhs_stack_map = (int*) sxalloc (last_clause+1, sizeof (int));
  rcg2Lex_file_rhs_stack_map = (int*) sxalloc ((max_gvar+1)*last_clause+1, sizeof (int));
  rcg2Lex_file_rhs_stack_map_top = 1;

  clause2streq = (int*) sxcalloc (last_clause+1, sizeof (int));
  ...
  fprintf (out_file, "#define rcg_itemmax %i\n", indpro-1);
  ...
  fprintf (out_file, "#define has_strlen %i\n", has_strlen ? 1 : 0);
  fprintf (out_file, "#define has_streq %i\n", S1_is_empty (streq_list) ? 0 : 1);

  fprintf (out_file, "static int prod2clause [%i] = {0,", nbpro);

  for (prod_no = 1; prod_no < nbpro; prod_no++) {
    if (prod_no % 10 == 1)
      fprintf (out_file, "\n/* %i */ ", prod_no);

    fprintf (out_file, "%i, ", prod2clause [prod_no]);
  }

  fputs ("};\n", out_file);
  
  fprintf (out_file, "\nstatic int clause2identical_disp [%i] = {1,\n", last_clause+2);
  ...
  fprintf (out_file, "/* dummy %i */ %i,\n", clause, top);
  fputs ("};\n", out_file);
  
  fprintf (out_file, "\nstatic int clause2identical [%i] = {0,\n", top);
  ...
  for (clause = 1; clause <= last_clause; clause++) {
    ...
    if ((cur = clause2idem_disp [clause])) {
      ...
      do {
	fprintf (out_file, "%i, ", clause2idem [cur]);
	bot++;
      } while ((cur = idem2next [cur]) > 0);
  ...
  for (clause = 1; clause <= last_clause; clause++) {
	...
	if (!has_identical_Lex) {
	  ...
	  BOOLEAN	must_check_identical_prod = TRUE;
  ...
  fprintf (out_file, "#define last_clause\t%i\n\n", last_clause);
  ...
  fputs ("static int prod2rhs_stack_map [] = {0,", out_file);

  for (prod_no = 1; prod_no < nbpro; prod_no++) {
    if (prod_no % 10 == 1)
      fprintf (out_file, "\n/* %i */ ", prod_no);

    fprintf (out_file, "%i, ", prod2rhs_stack_map [prod_no]);
  }

  fputs ("\n};\n\n", out_file);

  fputs ("static int rcg2Lex_file_rhs_stack_map [] = {0,", out_file);
  ...
  if (!S1_is_empty (streq_list)) {
    fputs ("static int clause2streq [] = {0,", out_file);

    for (clause = 1; clause <= last_clause;clause++) {
      if (clause % 10 == 1)
	fprintf (out_file, "\n/* %i */ ", clause);

      fprintf (out_file, "%i, ", clause2streq [clause]);
    }

    fputs ("\n};\n\n", out_file);

    fputs ("static int streq_list [] = {0,", out_file);
...
tag2tig ()
{
  int clause, lhs_prdct, lhs_bot2, lhs_nt, lhs_arity, lhs_top2, lhs_arg, lhs_cur2, lhs_param, lhs_bot3, lhs_top3, lhs_cur3, symb;
  int rhs, rhs_top, rhs_bot, rhs_cur, rhs_prdct, rhs_bot2, rhs_nt, rhs_arity, left_symb, right_symb;
  int  strong_left_A_nb, strong_right_A_nb, A, left_clause_nb, right_clause_nb, left_var, right_var, modified_clause_nb; 
  BOOLEAN is_strong_left_clause, is_strong_right_clause, is_strong_left, is_strong_right, done, is_first, has_changed; 
...
#if 0
      fprintf (sxtty, "left_tig = %i, right_tig = %i ",
	  ...
	  while (rhs_cur < rhs_top) {
		...
		if (SXBA_bit_is_set (lvar_set, left_symb) && SXBA_bit_is_set (lvar_set, right_symb)) {
		  ...
		  if (SXBA_bit_is_set (lvar_set, left_symb)) {
		    ...
		    print_nt (rhs_nt);
		    fprintf (out_file, "(X%i) ", left_symb-1);
		  }
		  else {
		    if (SXBA_bit_is_set (lvar_set, right_symb)) {
		      /* on genere A(right_symb); */
		      print_nt (rhs_nt);
		      fprintf (out_file, "(X%i) ", right_symb-1);
		    }
		    /* else Ca disparait */
		  }
		}
		...
		if (SXBA_bit_is_set (lvar_set, left_symb) && SXBA_bit_is_set (lvar_set, right_symb)) {
		  ...
		  if (SXBA_bit_is_set (lvar_set, left_symb)) {
		    ...
		    fprintf (out_file, "&%s(0,X%i) ", sxttext(sxtab_ptr, strlen_t), left_symb-1);
		  }
		  else {
		    if (SXBA_bit_is_set (lvar_set, right_symb)) {
		      /* on genere &StrLen(0, right_symb); */
		      fprintf (out_file, "&%s(0,X%i) ", sxttext(sxtab_ptr, strlen_t), right_symb-1);
		    }
		  }
		}
...
tag2tig ()
{
  int modified_clause_nb, A, arity, clause, lhs_prdct, lhs_bot2, lhs_top2, lhs_cur2, lhs_nt, k, lhs_param, lhs_bot3, lhs_top3, lhs_cur3;
  int rhs_param, rhs_top, rhs_bot, rhs_cur, rhs_bot3, rhs_top3, rhs_cur3, symb, rhs_prdct, rhs_bot2, rhs_top2, rhs_cur2, rhs_nt, left_symb, right_symb, rhs; 
  BOOLEAN is_vanished, has_changed, is_first_arg, is_first_symb; 
  SXBA lvar_set;
...
tig_form ()
{
  ...
  if (sxverbosep) {
    if (modified_clause_nb)
      fprintf (sxtty, "succes[%i/%i]\n", modified_clause_nb, last_clause);
    else
      fputs ("failure\n", sxtty);
