...
/************************************************************************/
/* Mer 21 Jun 2000 16:58 (pb):	Utilisation de is_1_RCG			*/
/************************************************************************/
/* Lun  5 Jun 2000 13:27 (pb):	Construction de nt2IeqJ			*/
/************************************************************************/
/* Mer 12 Avr 2000 10:08 (pb):	Construction de Ak2min et Ak2len	*/
/************************************************************************/
/* Mar 21 Jul 1998 17:51 (pb):	Ajout des external_action		*/
/************************************************************************/
/* Lun 27 Avr 1998 16:32 (pb):	Ajout des clause2da pour les DRCG	*/
/************************************************************************/
/* Lun 30 Mar 1998 13:32 (pb):	Ajout des PP &first et &last		*/
/************************************************************************/
/* Ven  6 Fev 1998 13:39 (pb):	Clause : lhs --> rhs . (1 seule rhs)	*/
/************************************************************************/
/* Jeu  5 Fev 1998 11:34 (pb):	Ajout des ".." en RHS			*/
/************************************************************************/
/* Jeu 18 Sep 1997 17:51 (pb):	Ajout de cette rubrique "modifications"	*/
/************************************************************************/
...
#include <string.h>
#include <stdlib.h>
char WHAT_RCGSMP[] = "@(#)SYNTAX - $Id: rcg_smp.c 625 2007-06-12 08:35:56Z syntax $" WHAT_DEBUG;


int         clause2lhs_nt (int clause);
extern void gen_clause_comment (FILE *parser_file, int clause);
extern void lrbnf (), tig_form (), rcg_gen_parser (), gen_bnf (), one_rcg (), two_var_form (), robust_form (), lrfsa (), rcg2bnf ();
...
static BOOLEAN		is_rhs, has_non_range_arg /*, is_empty_arg */, must_be_checked, is_false_clause;
static int		max_ste, cur_var, lhs_var, lhs_prdct, cur_prdct;
static int		max_rhs_arity = 1; /* prudence */
static int		etc_nb, log_max_prdct_nb;
static int		cur_param, cur_arity, rhs_arity, lhs_max_ovar_nb, ovar_nb;
static int		lhs_adr;
static int		rhs_arg_nb, lhs_arg_nb;
static int		cur_rhs, cur_clause, cur_nt, cur_t, ste;
static int		*clause2rhsnb;
static int		prev_var, prev_symb;
static BOOLEAN		is_first_pass;
static SXBA		lhs_var_ste_set, rhs_var_ste_set, ste_set;
static SXBA             lhs_pos_set, lhs_pos_hd_set;
static int              *lhs_pos2ste, max_lhs_pos, max_lhs_pos_nb;
static int		*ste_stack;
static XxY_header	var_couples;

static SXBA		next_rhs_set;
static int		*next_var2cur;

static SXBA		multiple_var_set, var_set, param_set;
static int		*state_stack, *equal_stack, *multiple_var2eq;
...
#if 0
static int		*U_stack, *NV_stack, *V_stack, *param_stack;
...
static int		**var_stacks_disp, *var_stacks;
#endif /* 0 */
...
static int		*first_Ak_stack, *last_Ak_stack, *Ak_stack;
static SXBA		first_Ak_set, last_Ak_set;
static XxY_header	first_rel, last_rel;
static XH_header	first_Ak_list, last_Ak_list;

static int		tooth_pos, clause;

static XxY_header	memel;
static int              *memel2val;

static void
memel_oflw (old_size, new_size)
int old_size, new_size;
{
    memel2val = (int*) sxrealloc (memel2val, new_size+1, sizeof (int));
}
    ...
    int		*val;
{
    ...
    *val = (int) strtol (str, &str2, 10);

    return (*str2 == NUL);
}

static BOOLEAN
is_monotone (is_monotone_ste)
    int is_monotone_ste;
{
    int		couple;
    BOOLEAN	ret_val;
...
{
  /* On verifie que les var de la LHS sont monotones et on affecte les codes dans tous les cas */
  int		check_monotonicity_ste, ste1, couple, lhs_pos;
  BOOLEAN	ret_val = TRUE;
...
rcg_pi ()
{
    ...
    case ARGUMENT_S_n :		/* VISITED->name = STRING_S_n */
	prev_var = 0;
	...
	case 2 :		/* VISITED->name = PREDICATE_S_n */
	    ...
	    if (is_first_pass) {
		is_top_down_erasing = is_bottom_up_erasing = FALSE;
		is_proper_grammar = is_simple_grammar = is_left_linear = is_right_linear = TRUE;
	...
	case 3 :		/* VISITED->name = {VOID_n, external_action_n} */
	    if (!is_first_pass) {
		if (VISITED->name == VOID_n)
		    clause2action [cur_clause] = EMPTY_STE;
		else {
		    has_sem_act = TRUE;
		    clause2action [cur_clause] = VISITED->token.string_table_entry;
		}
	    }
	    break;
	...
	is_first_pass = TRUE;
	clause = VISITED->position;
	is_false_clause = FALSE;
	break;

    case NOT_n :		/* VISITED->name = {PREDICATE_n} */
	break;

    case PREDICATE_n :
	switch (VISITED->position) {
	case 1 :		/* VISITED->name = {VOID_n, external_predicate_n, internal_predicate_n} */
    ...
    case PREDICATE_S_n :	/* VISITED->name = {NOT_n, PREDICATE_n} */
	if (!is_first_pass) {
	    if (VISITED->name == NOT_n) {
		int lahead = VISITED->son->token.lahead;
    ...
    case STRING_S_n :		/* VISITED->name = {ETC_n, terminal_n, variable_n} */
	break;
...
static int
is_a_sub_param (param, bot3, top3)
    int	param, bot3, top3;
{
  int		cur3, arg_lgth, lhs_bot3, lhs_top3, lhs_cur3, i;
...
static BOOLEAN
is_a_sub_arg (is_a_sub_arg_lhs_prdct, bot3, top3)
    int	is_a_sub_arg_lhs_prdct, bot3, top3;
{
  /* On regarde si l'argument courant est une sous-chaine de l'un des args de la LHS */
  /* Il doit contenir au moins une variable */
  int		lhs_bot2, lhs_top2, lhs_cur2, param;
...
BOOLEAN
rhs_arg2sub_arg (rhs_arg2sub_arg_lhs_prdct, arg)
    int	rhs_arg2sub_arg_lhs_prdct, arg;
{
    /* Si l'arg est de la forme ...u...v...w..., on decoupe en u, v et w */
    int bot3, cur3, top3, symb;
...
fill_clause2attr_disp (bot3, cur3, rhs_pos)
     int bot3, cur3, rhs_pos;
{
  int  fill_clause2attr_disp_lhs_prdct, lhs_bot2, lhs_top2, lhs_cur2, lhs_pos;
...
set_clause_attrs ()
{
  int set_clause_attrs_lhs_prdct, lhs_bot2, lhs_nt, lhs_arity, rhs, top, bot, cur, rhs_pos, prdct, bot2, cur2, top2;
  int nt, k, param, bot3, top3, cur3, symb;
...
rcg_pd ()
{
	...
	if (!is_first_pass) {
		...
		if (XH_list_elem (rcg_predicates, bot+1) < 0) {
		    ...
		    switch (VISITED->father->son->token.lahead) {
		    case streq_t:
			if (VISITED->degree != 2) {
			    sxput_error (VISITED->token.source_index,
				...
				if (is_drcg && is_pos_call) {
					...
					if (XH_X (rcg_parameters, arg2+1) == bot3+1) {
					    ...
					    if (da_top+2 > da_size)
						clause2da = (int*) sxrealloc (clause2da, (da_size *=2) + 1,
									      sizeof (int));
	...
	if (is_first_pass) {
	    ...
	    sxat_snv (INHERITED, VISITED->son);
	}
	else {
	    if (is_drcg)
	...
	if (!is_first_pass) {
		...
		if (VISITED->degree > max_rhs_prdct)
		    max_rhs_prdct = VISITED->degree;
	    }
	...
	if (!is_first_pass) {
	    ...
	    if (is_rhs) {
	      if (!is_false_clause) { 
		if (VISITED->father->father->son->name != VOID_n) {
		  /* On n'est pas ds un predicat predefini */
		  if (VISITED->degree > max_arg_size)
		    max_arg_size = VISITED->degree;
		}
	      }

	      lahead = VISITED->father->father->son->token.lahead;

	      if (VISITED->position > 1 || (lahead != strlen_t && lahead != first_t && lahead != last_t && lahead != lex_t)) {
		/* On verifie que l'arg courant n'est pas un element de T* et est
		   une sous-chaine d'un arg de la LHS. */
		/* Modif du 2/9/98 */
		if (is_in_Tstar (XH_X (rcg_parameters, cur_param), XH_X (rcg_parameters, cur_param+1))) {
		  /* Seul le 1er arg de StrEq ou (a la rigueur) de StrEqLen peut etre ds T^* */
		  if (VISITED->position == 1 && (lahead == streq_t || lahead == streqlen_t)) {
		  }
		  else
		    sxput_error (VISITED->token.source_index,
		  ...
		  if (!rhs_arg2sub_arg (lhs_prdct, cur_param)) {
		    ...
		    if (lahead == streq_t || lahead == streqlen_t || lahead == strlen_t) {
		      if (lahead == streq_t || VISITED->father->father->father->name == NOT_n)
		...
		if (lahead == first_t || lahead == last_t || lahead == lex_t) {
		  ...
		  for (p = VISITED->son; p != NULL; p = p->brother) {
		    if (p->name == variable_n)
		      sxput_error (p->token.source_index,
	      ...
	      if (!is_false_clause && VISITED->degree > max_arg_size)
		max_arg_size = VISITED->degree;
	
	      lhs_adr += VISITED->degree+1;
	    }
    ...
    case VOID_n :
	if (VISITED->father->name == PREDICATE_n) {
	    if (is_first_pass) {
		if (VISITED->token.lahead == false_t)
		    is_false_clause = TRUE;
	    }
	    else {
		switch (VISITED->token.lahead) {
		case streq_t:
	...
	if (!is_first_pass) {
	    ...
	    cur_nt = ste2nt [ste];
	    arg_nb = VISITED->brother->degree;

	    if (!is_rhs && arg_nb == 1 && VISITED->brother->son->degree == 0)
		/* cas de A() en lhs */
		arg_nb = 0;
		...
		if (VISITED->name == internal_predicate_n)
		    SXBA_1_bit (is_nt_internal, cur_nt);
		else
		    SXBA_1_bit (is_nt_external, cur_nt);

		if (cur_nt == 1 && VISITED->name == internal_predicate_n) {
		...
		if (cur_arity >= SXBITS_PER_LONG)
		    sxput_error (VISITED->token.source_index,
				 "%sImplementation restriction: the maximum arity is limited to #%i.",
				 sxtab_ptr->err_titles [2],
				 SXBITS_PER_LONG-1);
#endif
	...
	if (!is_first_pass) {
		...
		sxput_error (VISITED->token.source_index,
	      ...
	      int t = VISITED->father->father->father->son->token.lahead;

	      if (VISITED->position == 1 &&
		  VISITED->father->degree == 1 &&
		  VISITED->father->position == 1 &&
		  VISITED->father->father->father->name == PREDICATE_n &&
		  (t == strlen_t || t == streqlen_t)) {
		BOOLEAN is_int = str2int (sxstrget (ste), &cur_t);
	...
	if (is_first_pass) {
	    ...
	    if (is_rhs) {
		lahead = VISITED->father->father->father->son->token.lahead;
		...
		if (lahead == internal_user_predicate_t || lahead == external_user_predicate_t) {
			...
			is_proper_grammar = is_simple_grammar = is_right_linear = FALSE;
		    }
		}

		if (!SXBA_bit_is_set (lhs_var_ste_set, ste)) {
		    /* Variable de la RHS non deja definie en LHS */
		    is_bottom_up_erasing = TRUE;
		    is_simple_grammar = FALSE;
		    sxput_error (VISITED->token.source_index,
		...
		is_proper_grammar = is_simple_grammar = is_left_linear = FALSE;
	      }
...
static int		*nt2clause_disp, *nt2clause;
...
{
  int false_clause_propagate_clause, A, A_clause, bot, top;
  BOOLEAN done;
  SXBA A_clause_set;
...
#if 0
	      fprintf (sxtty, "Induced false clause #%i\n", false_clause_propagate_clause);
#endif
	    }
...
static int
equiv_prdct (cur_x, next_x, equiv_prdct_prev_var, equiv_prdct_lhs_var)
    int cur_x, next_x, equiv_prdct_prev_var, equiv_prdct_lhs_var;
{
    int		equiv_prdct_cur_prdct, cur_prdct_bot, cur_op, equiv_prdct_cur_nt, next_prdct, next_prdct_bot, next_op, next_nt;
    int		arity, next_var, z, var;
    int		equiv_prdct_cur_param, next_param, cur_param_bot, next_param_bot, param_lgth, l, cur_symb, next_symb;
...
static BOOLEAN
is_equiv_prdct (cur_x, next_clause_bot, delta, rhs_lgth, is_equiv_prdct_lhs_var, is_equiv_prdct_prev_var, is_equiv_prdct_max_var, n)
    int cur_x, next_clause_bot, delta, rhs_lgth, is_equiv_prdct_lhs_var, is_equiv_prdct_prev_var, is_equiv_prdct_max_var, n;
{
    int		next_x, next_var, next_delta, y;
...
static BOOLEAN
equiv_rhs (equiv_rhs_cur_clause, next_clause)
    int equiv_rhs_cur_clause, next_clause;
{
    int		cur_clause_rhs, cur_clause_bot, next_clause_rhs, next_clause_bot, rhs_lgth, equiv_rhs_lhs_var, equiv_rhs_max_var;
    int		delta, y;
...
is_a_set ()
{
    ...
    int		next_clause, next_rhs, rhs_lgth, cur_bot, cur_x, is_a_set_cur_prdct, next_prdct;
    BOOLEAN	is_cur_bottom_up_erasing, is_next_bottom_up_erasing;
    int		*ne_couples, *e_couples;
    SXBA	tbp;
	...
	int		min_prdct, prdct, prdct_nb;
	XH_header	rhs_nf;
	int		*clause2rhs_nf;

	XH_alloc (&rhs_nf, "rhs_nf", sxba_cardinal (tbp) + 1, 1, ml, NULL, NULL);
	clause2rhs_nf = (int*) sxalloc (last_clause+1, sizeof (int));
	...
	do {
	    ...
	    if (clause2rhs_nf [cur_clause] == clause2rhs_nf [next_clause]) {
	      if (check_set) {
		sxput_error (clause2visited [cur_clause]->token.source_index,
...
static void
extract_params (prdct)
    int prdct;
{
    int prdct_top, x, param, bot, top, y, symb;
...
static BOOLEAN
is_instantiable_clause ()
{
    /* On ne considere que les parametres ayant une variable de multiple_var_set */
    int		param, bot, top, y, symb, equiv_class, i, j, k, l, symbi, symbj;
...
is_instantiable ()
{
    ...
    int is_istantiable_cur_clause, is_istantiable_lhs_prdct, rhs, top, x, var, i;
	...
	if (clause2identical [is_istantiable_cur_clause] == is_istantiable_cur_clause && !SXBA_bit_is_set (false_clause_set, is_istantiable_cur_clause)) {
		...
		sxput_error (clause2visited [is_istantiable_cur_clause]->token.source_index,
...
Xref ()
{
  int		Xref_clause, nt, nt_nb, rhs, top, x, y, prdct;
  ...
  XxY_foreach (rcg_clauses, Xref_clause) {
	...
	sxput_error (clause2visited [Xref_clause]->token.source_index,
...
has_loops ()
{
  ...
  int has_loops_clause, rhs, has_loops_lhs_prdct, rhs_prdct, x, top, nt;
  int	cur2, bot2, top2, param,cur3, bot3, top3;
  BOOLEAN	has_lhs_prdct;
...
is_productive ()
{
  ...
  int		is_productive_clause, is_productive_lhs_prdct, nt, nt_nb, rhs, rhs_prdct, x, top;
  SXBA	        ntline;
  ...
  while ((is_productive_clause = sxba_0_lrscan (productive_clause_set, is_productive_clause)) > 0) {
    if (clause2identical [is_productive_clause] == is_productive_clause && SXBA_bit_is_reset_set (false_clause_set, is_productive_clause)) {
      ...
      if (check_instantiable_clause)
	sxput_error (clause2visited [is_productive_clause]->token.source_index,
...
is_accessible ()
{
	...
	if (check_instantiable_clause)
	  sxput_error (clause2visited [is_accessible_clause]->token.source_index,
...
negative_language ()
{
    /* On a des vrais predicats negatifs en RHS.  On verifie que le langage de A ne depend pas
       de !A */
    int init_nt, nt, negative_language_clause, rhs_nt, rhs, pos, x, y, top, prdct;
...
static int
clause2rhsnt_nb (clause, nt)
     int clause, nt;
{
  int rhs, top, cur, nb = 0;
    ...
    SXBA bvect;
{
  ...
  int		clause, rhsnb, pos;
  SXBA	        line;
  BOOLEAN	is_first_occur, has_changed;
  SXBA          empty_vector;
    ...
    int clause;
{
  ...
  */
  int		lhs_prdct, bot, top, nt, pos, param, bot2, top2, symb, uparnb;
  int		x, y;
  BOOLEAN	is_undef;
...
static void walk_rhs_prdct_bvect ();

static void
walk_rhs_prdct (clause, cur, top)
    int clause, cur, top;
{
    /* Parcourt de la rhs de clause.  Le predicat courant est en cur */
    int		U_set_nb, Vtop, NVtop0, NVtop, xparam, param, bot3, cur3, top3, Vnb, NVnb, Unb, var;
    int		prdct, bot2, top2, nt, arity;
    int		n, max, v, i, l;
    BOOLEAN	is_set;
    SXBA	U_line;
...
static void
walk_rhs_prdct_bvect (clause, cur, top, bot2, top2, bvect, predef_bvect)
    int 	clause, cur, top, bot2, top2, predef_bvect;
    SXBA        bvect;
{
  ...
  int		Vtop, NVtop, paramtop, pos, cur2, param, bot3, cur3, top3, symb, var; 
  BOOLEAN	is_arg_empty, is_arg_non_empty, has_t;
  /* Le 19/12/03 */
...
empty_construction ()
{
  ...
  while ((clause = sxba_0_lrscan (productive_eclause_set, clause)) > 0) {
	...
	if (check_instantiable_clause)
	  sxput_error (clause2visited [clause]->token.source_index,
		       "%sNon productive clause.",
		       sxtab_ptr->err_titles [2]);
      }
    }
...
static int
walk_empty_memel (param, val)
     int param, val;
{
  int pair, local_pair, val2;
...
/* val > 1, on exploite */
static BOOLEAN
process_memel_result (process_memel_result_clause, param, rhs_pos, val)
     int process_memel_result_clause, param, rhs_pos, val;
{
  int     size;
  ...
  if (val == 3) {
    ...
    if (SXBA_bit_is_reset_set (false_clause_set, process_memel_result_clause)) {
      ...
      if (check_instantiable_clause)
	sxput_error (clause2visited [process_memel_result_clause]->token.source_index,
		     "%sNon instantiable clause.",
		     sxtab_ptr->err_titles [2]);
...
empty_construction ()
{
  int     empty_construction_clause, bot, cur, top, empty_construction_lhs_prdct, lhs_pos, param, param1, param2, bot2, cur2, top2;
  int     rhs, prdct, nt, pair, rhs_pos, cur_rhs_pos, pos, val; 
  SXBA    clause_empty_vector, nt_empty_vector, line;
  BOOLEAN is_init_phase, has_changed, is_first;
	  ...
	  if (nt > 0) {
		...
		if (nt == STRLEN_ic) {
		  ...
		  if (param1 == 0) {
		    ...
		    if (val > 1 && process_memel_result (empty_construction_clause, cur_rhs_pos+1, val))
		      has_changed = TRUE;
		  }
		}
		else {
...
/* ESSAI */
static void
first_last_complete (first_last_complete_clause)
    int first_last_complete_clause;
{
  int   first_last_complete_lhs_prdct, lhs_bot, lhs_nt, ntpos, lhs_pos, lhs_cur, lhs_top, lhs_param, lhs_bot2, symb, lhs_top2, lhs_param_size; 
  int   rhs, rhs_top, rhs_bot, rhs_cur, rhs_prdct, rhs_bot2, rhs_top2, rhs_nt, Ak, pos, rhs_cur2, rhs_pos, rhs_param;
  int   rhs_bot3, rhs_top3, rhs_cur3, elem, xx, Bh, elem2; 
...
first_last_construction ()
{
  ...
  int 	first_last_construction_clause, A, k, Ak, Bh, Cl, elem, arity, bot, cur, top, pos;
  ...
  */
  ...
  for (A = 1; A <= max_nt; A++) {
	...
	if (sxba_is_empty (first [Ak]) || sxba_is_empty (last [Ak])) {
	  if (check_instantiable_clause)
	    sxtmsg (sxsrcmngr.source_coord.file_name,
		    "%sThe %i%s parameter of predicate <%s> is non instantiable.",
		    sxtab_ptr->err_titles [2],
		    k+1,
    ...
    int clause;
{
    /* La substitution des variables de la partie droite est contenue dans V_set et NV_set.
       ...
       V_set contient l'ensemble des variables de la RHS qui derivent dans epsilon.
       */
    int		lhs_prdct, bot, top, nt, pos, param, bot2, top2, symb, x, y, z, t;
    int		ntpos, *X_stack, Ak, param3, bot3, top3, x3, symb3;
    int		xx, Bh, elem, elem2;
...
#if 0
static BOOLEAN first_last_arg_walk ();

static BOOLEAN
first_last_rhs_walk (clause, cur, top)
    int clause, cur, top;
{
    /* Parcourt de la rhs de clause.  Le predicat courant est en cur */
    int		U_set_nb, Vtop, NVtop0, NVtop, xparam, param, bot3, cur3, top3, Vnb, NVnb, Unb, var;
...
static BOOLEAN
first_last_arg_walk (clause, cur, top, bot2, top2, bvect, predef_bvect)
    int 	clause, cur, top, bot2, top2, predef_bvect;
    SXBA        bvect;
{
  /* On parcourt les arguments du predicat en position cur de clause dont le vecteur caracteristique
  ...
  /* Si bvect != NULL, c'est une chaine de bits  */
  int		Vtop, NVtop, paramtop, pos, cur2, param, bot3, cur3, top3, symb, var; 
  BOOLEAN	is_arg_empty, is_arg_non_empty, has_t, ret_val;
  /* Le 19/12/03 */
  /* Si is_arg_empty ou is_arg_non_empty sont positionnes, on est su^r que c'est vide ou que c'est non vide
     sinon doute!! */
    ...
    int clause;
{
    ...
    int lhs_prdct, bot, top, cur, param, bot2, cur2, top2, symb;
    ...
    int clause;
{
    ...
    int		rhs, pos, bot, cur, top, bot2, cur2, top2, bot3, cur3, top3, prdct, nt, Ak, param, symb;
    int		*X_stack;
...
{
    /* Il faudrait etre + fin */
    fprintf (listing, "%s", sxstrget (t2ste [t]));
}
...
int clause2lhs_nt (int clause2lhs_nt_clause)
{
    int clause2lhs_nt_lhs_prdct;
...
first_last_construction ()
{
  ...
  int 	clause, rhs, A, k, Ak, Bh, Cl, elem, arity, bot, cur, top, pos;
...
static int		*clause2iargs, *infix_args;
...
static int		*A_stack, *param_stack, *memel_SS_stack, *clause2clausek;
static int		infix_args_top, infix_args_size, iargs2arg_disp_top, iargs2arg_disp_size, arg_memo_top, arg_memo_size,
                        max_iarg_rel;
...
static BOOLEAN
set_iargs2arg_disp (set_iargs2arg_disp_clause, param, iarg)
    int set_iargs2arg_disp_clause, param, *iarg;
{
...
static void
fill_infix_args (fill_infix_args_clause, lhs_cur3, lhs_top3, rhs)
    int fill_infix_args_clause, lhs_cur3, lhs_top3, rhs;
{
    /* On met ds infix_args les iargs qui correspondend a la reconnaissance de la position lhs_cur3 */
    int		symb, cur, top, bot, prdct, bot2, top2, nt, cur2, k, param, bot3, top3, lcur3, cur3, x;
    BOOLEAN	is_lgth_1 = FALSE;
	...
	if (infix_args_top >= infix_args_size)
	    infix_args = (int*) sxrealloc (infix_args, (infix_args_size *= 2)+1, sizeof (int));
    }
...
static int
walk_memel (iarg_bot, iarg, with_Ak)
    int		iarg_bot, iarg;
    BOOLEAN	with_Ak;
{
    int pair, iarg1, iarg2, iarg_rel;
...
static void
update_memel (update_memel_clause, rhs)
    int update_memel_clause, rhs;
{
  ...
  int		top, cur, bot, prdct, bot2, nt, arity, cur2, cur3, top2, arg1, A1, A2, iarg1, arg2, iarg2, pair, base;
  int         iarg1_lgth, iarg2_lgth;
  BOOLEAN	is_old;
...
#if EBUG_PRDCT
		    if (nt == EBUG_PRDCT) {
		      fprintf (sxtty, "%s-clause(%i) [%i == %i]***%s\n",
			       sxstrget (nt2ste [nt]),
			       update_memel_clause,
			       Ak_A2k (A1,nt),
			       Ak_A2k (A2,nt),
...
static void
canonical_form (canonical_form_clause, iarg_struct, can_struct)
    int			canonical_form_clause;
    struct iarg_struct	*iarg_struct;
    struct can_struct	*can_struct;
{
    int base, loc_iarg, iarg;
...
static BOOLEAN
fill_iarg2_struct (fill_iarg2struct_clause, bot, top)
    int			fill_iarg2struct_clause;
    struct arg_memo	*bot, *top;
{
	...
	if (iarg <= 0) {
	    if ((unsigned)iarg != TOP_DOWN_ERASED) {
...
static BOOLEAN
fill_iarg1_struct (fill_iarg1struct_clause, bot, top, iarg2)
    int			fill_iarg1struct_clause, iarg2;
    struct arg_memo	*bot, *top;
{
    int arg2, iarg, x, arg, next;
    int	*bot2, *top2;
...
static BOOLEAN
is_iarg_equality (is_iarg_equality_clause, iarg1, iarg2)
    int is_iarg_equality_clause, iarg1, iarg2;
{
    int arg1;
    struct arg_memo	*pbot, *ptop;
...
static struct Ak_memo
eval_arg (eval_arg_clause, bot, top)
    int			eval_arg_clause;
    struct arg_memo	*bot, *top;
{
  /* La valeur retournee est le max des min!! */
  int			Ak, next, iarg, min, lgth, x; 
  int			*top2, *bot2;
  ...
  while (bot2 < top2) {
    ...
    if (iarg <= 0)
      ...
      int arg = iargs2arg_disp [iarg].arg;
      next = XH_X (rcg_parameters, arg+1)-XH_X (rcg_parameters, arg);
    }
...
{
  /* La valeur retournee est le max des min!! */
  int			max, Ak, next, iarg, min, local_min, local_max;
  int			x /*, len, local_len */; 
  int			*top2, *bot2;
...
static void
clear_arg_memo (clear_arg_memo_clause)
    int	clear_arg_memo_clause;
{
...
{
  int max_nt2clause, nt2clause_construction_clause, rhs, bot, top, cur, prdct, bot2, nt, nt2clause_construction_lhs_prdct;

  lhsnt2clause = sxbm_calloc (max_nt+1, last_clause+1);
  nt2clause_disp = (int*) sxcalloc (max_nt+2, sizeof (int));
  ...
  nt2clause = (int*) sxalloc (max_nt2clause+1, sizeof (int));
...
Ak2min_construction ()
{
  int			x, y, max_pos, Ak2min_construction_clause, lhs_nt, Ak2min_construction_lhs_prdct, lhs_bot2, lhs_cur2, lhs_top2, lhs_arity, param, rhs, top, bot, cur;
  int			prdct, bot2, cur2, top2, nt, iarg1, iarg2, pair, k, Ak, bot3, cur3, top3, iarg;
  int			min, max, lhs_pos, A1, A2, son1, son2, arg1, arg2, A1_min, arity, Ak2min_construction_max_clausek, clausek;
  int			local_min, local_max, xbot, nt2IeqJ_top, nt2IeqJ_size;
  int			*ip;
  BOOLEAN		is_initial, noteq_has_changed = FALSE, has_min_changed;
  struct arg_memo	*pbot, *ptop;
...
#if 0
  Ak2min = (int*) sxalloc (max_Ak+1, sizeof (int));
  Ak2max = (int*) sxalloc (max_Ak+1, sizeof (int));
  Ak2len = (int*) sxalloc (max_Ak+1, sizeof (int));
#else
  ...
  StrLen1 = max_Ak+1;
  Ak2min = (int*) sxalloc (StrLen1+1, sizeof (int));
  Ak2max = (int*) sxalloc (StrLen1+1, sizeof (int));
  Ak2len = (int*) sxalloc (StrLen1+1, sizeof (int));
  ...
  clause2clausek = (int*) sxalloc (last_clause+2, sizeof (int));
  ...
  clause2iargs = (int*) sxalloc (last_clause+2, sizeof (int));
  ...
  infix_args = (int*) sxalloc ((infix_args_size = arg_memo_size)+1, sizeof (int));

  A_stack = (int*) sxalloc (max_nt+1, sizeof (int));
  A_stack [0] = 0;
  A_set = sxba_calloc (max_nt+1);

  iarg_rel_set = sxba_calloc (max_iarg_rel+1);
  Ak_stack = (int*) sxalloc (max_Ak+1, sizeof (int));
  Ak_stack [0] = 0;
  ...
  can1_struct.nb = (int*) sxalloc (max_iarg_rel+1, sizeof (int));
  can2_struct.set = sxba_calloc (max_iarg_rel+1);
  can2_struct.nb = (int*) sxalloc (max_iarg_rel+1, sizeof (int));
  ...
  nt2IeqJ_disp = (int*) sxalloc (max_nt+2, sizeof (int));
  nt2IeqJ = (int*) sxalloc ((nt2IeqJ_size = max_nt+1)+1, sizeof (int));
  ...
  for (nt = 1; nt <= max_nt; nt++) {
    if (SXBA_bit_is_set (is_lhs_nt, nt) && nt2identical [nt] == nt) {
      fprintf (sxtty, "%s(%i) [", sxstrget (nt2ste [nt]), nt);
      lhs_arity = nt2arity [nt];
      Ak = A_k2Ak (nt, 0);

      for (x = 1; x <= lhs_arity; x++) {
	fprintf (sxtty, "(<%i..%i>, %i)", Ak2min [Ak], Ak2max [Ak], Ak2len [Ak]);
	Ak++;
...
rcg_lo ()
{
  ...
  if (!IS_ERROR) {
    int nt, arity, bot, top, IJ, I, J, x, Ak;
	...
	if (arity >= 2) {
	  ...
	  while (bot < top) {
	    ...
	    fprintf (listing, "%s(%i) [%i == %i]\n", sxstrget (nt2ste [nt]), nt, I+1, J+1);
    ...
    for (nt = 1; nt <= max_nt; nt++) {
      if (SXBA_bit_is_set (is_lhs_nt, nt) && nt2identical [nt] == nt) {
	fprintf (listing, "%s(%i) [", sxstrget (nt2ste [nt]), nt);
	arity = nt2arity [nt];
	Ak = A_k2Ak (nt, 0);

	for (x = 1; x <= arity; x++) {
	  fprintf (listing, "(<%i..%i>, %i)", Ak2min [Ak], Ak2max [Ak], Ak2len [Ak]);
	  Ak++;
...
smppass ()
{
    ...
    lhs_pos2ste = (int*) sxalloc (max_lhs_pos_nb + 1, sizeof (int));
    lhs_var_ste_set = sxba_calloc (max_ste + 1);
    rhs_var_ste_set = sxba_calloc (max_ste + 1);
    ste_set = sxba_calloc (max_ste + 1);
    ste_stack = (int*) sxalloc (max_ste + 1, sizeof (int)), ste_stack [0] = 0;
    ...
    nt2arity = (int*) sxalloc (max_ste + 1, sizeof (int));
    ...
    var2ste = (int*) sxalloc (max_ste + 1, sizeof (int));

    clause2lhs_var = (int *) sxalloc (rule_nb + 1, sizeof (int));
    clause2max_var = (int *) sxalloc (rule_nb + 1, sizeof (int));
    clause2action = (int *) sxalloc (rule_nb + 1, sizeof (int));
    ...
    new_false_clause_stack = (int*) sxalloc (rule_nb + 1, sizeof (int));

    has_an_identical_clause_set = sxba_calloc (rule_nb+1);
    clause2identical = (int *) sxalloc (rule_nb+1, sizeof (int));
	...
	clause2da_disp = (int*) sxalloc (rule_nb + 2, sizeof (int));
	clause2da_disp [1] = da_top = 0;
	clause2da = (int*) sxalloc ((da_size = rule_nb + 2) + 1, sizeof (int));
    }
    ...
    if (!IS_ERROR) {
      int size, nt, smppass_clause;

      nt2clause_construction ();

      next_rhs_set = sxba_calloc (max_rhs_prdct+1);
      next_var2cur = (int *) sxcalloc (max_gvar+1, sizeof (int));
      ...
      memel2val = (int*) sxalloc (XxY_size (memel)+1, sizeof (int));
      nt2empty_vector = sxbm_calloc (max_nt+1, max_garity+1);
      clause2empty_vector = sxbm_calloc (last_clause+1, max_garity+1);
      clause_stack = (int*) sxalloc (last_clause+1, sizeof (int)), clause_stack [0] = 0;
      clause_set = sxba_calloc (last_clause+1);
      nt_stack =  (int*) sxalloc (max_nt+1, sizeof (int)), nt_stack [0] = 0;
	...
	if (has_an_identical_nt_set) {
	  ...
	  nt2identical = (int *) sxalloc (rule_nb+1, sizeof (int));
	}
	...
	multiple_var2eq = (int*) sxcalloc (max_gvar+1, sizeof (int));

	size = max_clause_arg_nb*(max_arg_size+1);
	state_stack = (int*) sxalloc (size+1, sizeof (int)), state_stack [0] = 0;
	...
	clause2rhsnb = (int*) sxalloc (last_clause+1, sizeof (int));
	...
	for (nt = 1; nt <= max_nt; nt++) {
	  if (nt2arity [nt] == -1) {
	    ...
	    if (!SXBA_bit_is_set (is_nt_external, nt)) {
	      ...
	      sxput_error (clause2visited [smppass_clause]->token.source_index,
	...
	if (!IS_ERROR && is_first_last) {
	  ...
	  if (lhs_var_set) {
	    ...
	    U_stack = (int*) sxalloc (max_gvar+1, sizeof (int));
	    NV_stack = (int*) sxalloc (max_gvar+1, sizeof (int));
	    V_stack = (int*) sxalloc (max_gvar+1, sizeof (int));
	    param_stack = (int*) sxalloc (max_rhs_arity+1, sizeof (int));
	  }
	...
	if (!IS_ERROR) {
	  int smppass_nt, Ak, arity, k, smppass_clause_2, main_clause;
	  ...
	  if (A2A0 == NULL) {
	    A2A0 = (int*) sxalloc (max_nt+1, sizeof (int));
	    A02A = (int*) sxalloc (max_Ak+1, sizeof (int));
	  }
	  ...
	  if (clause2clause0 == NULL) {
	    ...
	    clause2clause0 = (int*) sxalloc (last_clause+1, sizeof (int));
	  }
	  ...
	  if (is_first_last && max_t) {
	    ...
	    Ak_stack = (int*) sxalloc (max_Ak+1, sizeof (int)), Ak_stack [0] = 0;

	    first_Ak_set = sxba_calloc (max_Ak+1);
	    first_Ak_stack = (int*) sxalloc (max_Ak+1, sizeof (int)), first_Ak_stack [0] = 0;
	    last_Ak_set = sxba_calloc (max_Ak+1);
	    last_Ak_stack = (int*) sxalloc (max_Ak+1, sizeof (int)), last_Ak_stack [0] = 0;

#if 0
	    rhs_var_set = sxba_calloc (max_gvar+1);
	    var_stacks_disp = (int**) sxalloc (max_gvar+1, sizeof (int*));
	    p = var_stacks = (int*) sxalloc (max_gvar*(2*max_rhs_arity+1)+1, sizeof (int));
    ...
    if (!IS_ERROR) {
      param_stack = (int*) sxalloc (max_rhs_arity+1, sizeof (int)), param_stack [0] = 0;
