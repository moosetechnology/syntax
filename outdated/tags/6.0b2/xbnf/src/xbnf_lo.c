...
/************************************************************************/
/* 06-04-89 17:40 (phd):	Change un char en int pour Lichnewsky	*/
/************************************************************************/
...
#include "varstr.h"
#include "put_edit.h"
char WHAT_XBNFLO[] = "@(#)SYNTAX - $Id: xbnf_lo.c 634 2007-06-18 13:11:02Z rlacroix $" WHAT_DEBUG;
...
static VOID	print_tables ()
{
    ...
    int		l, x, prev_rule_no, rule_no;
    int		context_table_slice_size, max_slice_no, slice_no, bi, bs, maxsize;
    int		*sort_symbols;
...
/* on calcule la taille du plus grand nom de terminal */
		...
		{
		    register char	**ren /* bi:bs *//* maxsize */ ;
			...
			sprintf (line, "%-*s | ", print_tables_M_t, varstr_tostr (cat_t_string (varstr_raz (vstr), -sort_symbols [i])))
			     ;
			k = print_tables_M_t + 1;
...
static VOID	bnf_output_error (bnf_output_error_errno)
    register int	bnf_output_error_errno;
{
...
static VOID	bnf_header ()
{
    ...
    {
	register int	c;
	register int	line_no, rule_no, next_rule_line_no;
	int	bnf_header_errno, i, n;
	...
	while ((c = getc (sxsrcmngr.infile)) != EOF) {
	    ...
	    if (errline_no == (unsigned)line_no) {
		static char	STARS [] = "\n\n******";
		...
		} while (errline_no == (unsigned)line_no);
