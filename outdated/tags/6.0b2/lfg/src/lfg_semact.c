...
#include "sxunix.h"
#include "earley.h"
char WHAT_LFGSEMACT[] = "@(#)SYNTAX - $Id: lfg_semact.c 529 2007-05-09 14:26:33Z rlacroix $" WHAT_DEBUG;

#if 0
/* A voir + tard... */
#include "lfg.h"
#endif /* 0 */
BOOLEAN is_print_f_structure, is_unfold_parse_forest, is_forest_filtering, is_sum_priority;
int     sentence_id, rank [6];
FILE    *xml_file;
...
static int               maxxt; /* nb des terminaux instancies Tij, y compris ceux couvrant les nt de la rcvr
				   (maxxt >= -spf.outputG.maxt) */
static unsigned long int tree_id;
static int               Xpq_top;
...
static int               *fs_id_stack;
static int               rhs_pos2fs [MAXRHS+1];
static int               rhs_pos2digit [MAXRHS+1];
#if 0
static int               shared_f_struct2wfs_id [MAX_SHARED_F_STRUCTURE_ID+1];
static SXBA_ELT          shared_f_struct_id_set [NBLONGS (MAX_SHARED_F_STRUCTURE_ID+1)+1] = {MAX_SHARED_F_STRUCTURE_ID+1};
static SXBA              cyclic_wfs_id_set;
static int               *wfs_id2simulated_son_nb, *simulated_wfs_id_stack, *cyclic_wfs_id2simulated_fs_id;
#endif /* 0 */
...
static int               *Xpq2disp, *fs_id_dstack;
static SXBA              Xpq_set; /* Cas unfold pour un clear apres chaque arbre */

/* Pour traiter le $$ courant */
static int               wfs_id, wfs_page_nb;
static int               *wfs_id2son_nb, *wfs_id_stack;
...
static struct wfs_id_pred_item {
  int wfs_id, *field_ptr, next;
} *wfs_id_pred_list;

static int    shared_struct_nb; /* local a chaque $$ */
...
static int *post_stack, *post_display; /* DSTACKs */
...
static int               Aij; /* La lhs de la prod courante ($$) */
...
static BOOLEAN    is_consistent; /* !is_consistent <=> toutes les f_struct sont incoherentes */
static BOOLEAN    is_rcvr_done; /* TRUE <=> Aucune f_structure a la racine, on a recupere ce qu'on a pu ... */
static SXBA       head_set;
static int        *head_stack;
static int        ranking_kind;

/* Pour la passe "is_forest_filtering" */
static int     *Pij2disp, *fs_id_Pij_dstack, Pij_cur_bot;
static int     doldol;
static SXBA    unconsistent_heads_set;
static BOOLEAN is_locally_unconsistent;

/* Pour dag_td_walk */
static SXBA    valid_heads_set;

static BOOLEAN is_relaxed_run; /* positionne si le 1er passage a echoue'.  On en refait un sans la verif de la coherence */

static int              *Tij2tok_no;
...
wfs_area_allocate ()
{
  ...
  wfs_id2son_nb = (int*) sxalloc (AREA_SIZE, sizeof (int)), wfs_id2son_nb [0] = 0;
  wfs_id_stack = (int*) sxalloc (AREA_SIZE+1, sizeof (int)), wfs_id_stack [0] = 0;
  
  wfs_id_set = sxba_calloc (AREA_SIZE);
#if 0
  cyclic_wfs_id_set = sxba_calloc (AREA_SIZE);
  cyclic_wfs_id2simulated_fs_id = (int*) sxalloc (AREA_SIZE, sizeof (int));
  wfs_id2simulated_son_nb = (int*) sxalloc (AREA_SIZE, sizeof (int));
  simulated_wfs_id_stack = (int*) sxalloc (AREA_SIZE+1, sizeof (int)), simulated_wfs_id_stack [0] = 0;
#endif /* 0 */

  wfs_id2equiv = (int*) sxalloc (AREA_SIZE, sizeof (int));
...
wfs_area_oflw ()
{
  ...
  wfs_id2son_nb = (int*) sxrealloc (wfs_id2son_nb, l, sizeof (int));
  wfs_id_stack = (int*) sxrealloc (wfs_id_stack, l+1, sizeof (int));
  wfs_id_set = sxba_resize (wfs_id_set, l);
#if 0
  cyclic_wfs_id_set = sxba_resize (cyclic_wfs_id_set, l);
  cyclic_wfs_id2simulated_fs_id = (int*) sxrealloc (cyclic_wfs_id2simulated_fs_id, l+1, sizeof (int));
  wfs_id2simulated_son_nb = (int*) sxrealloc (wfs_id2simulated_son_nb, l, sizeof (int));
  simulated_wfs_id_stack = (int*) sxrealloc (simulated_wfs_id_stack, l+1, sizeof (int));
#endif /* 0 */
...
merge (id1, id2)
     int id1, id2;
{
  int                     last1, first2, *field_ptr, *field_ptr2;
...
allocate_all ()
{
  ...
  if (is_forest_filtering || xml_file) {
    Pij2disp = (int*) sxcalloc (spf.outputG.maxprod+1, sizeof (int));
    DALLOC_STACK (fs_id_Pij_dstack, 5*spf.outputG.maxprod+1);
  } 

  Tij2tok_no = (int*) sxcalloc (maxxt+1, sizeof (int));
}
...
struct store_wfs {
  int                     wfs_id, wfs_page_nb, XH_fs_hd_size;
  SXBA                    **wfs_field_set;
...
put_in_store_wfs ()
{
  ...
  if ((store_wfs_ptr = store_wfs_hd.wfs [i]) == NULL) {
    ...
    store_wfs_ptr->wfs_id2son_nb = (int*) sxalloc (size, sizeof (int));
...
set_wfs_from_store_wfs (i)
     int i;
{
  int              cur_wfs_id, page_nb, delta_nb, id, fs_id, field_id, pos;
  int              *attr_ptr, *store_attr_ptr;
  SXBA             field_set;
  struct store_wfs *store_wfs_ptr;
...
coherence (Xpq)
     int     Xpq;
{
  int                    cur_wfs_id, page_nb, delta_nb, i, sous_cat_id, cur2, top2, cur3, bot3, top3, field_id, nb;
  int                    *attr_ptr0;
  SXBA                   field_set;
  BOOLEAN                is_root, is_optional, ret_val;
  int                    *dpv_ptr, *cur_val_ptr;
  int                    val, fs_id, top, bot, cur, ufs_id;
...
fill_fs ()
{
  int             val, x, y, page_nb, delta_nb, field_id, fs_id, pair, bot, top, id, simulated_fs_id, son_nb;
  SXBA            field_set;
  BOOLEAN         done, is_shared;
...
get_priority (fs_id)
     int fs_id;
{
  int x, max_priority, priority, prio, local_priority, bot, top, val, field_id, static_field_kind, bot2, cur2, top2;
...
print_f_structure (Xpq, fs_id, is_unconsistent)
     int     Xpq, fs_id;
     BOOLEAN is_unconsistent;
{
  int             bot, top, bot2, cur2, top2, bot3, cur3, top3, max_priority, priority;
  int             val, field_id, field_kind, sous_cat_id, atom_id, local_atom_id, x, Tpq, T, p, q, id;
  int             *local2atom_id;
  BOOLEAN         is_optional, is_first, is_first2, is_first3, is_main_struct;
  ...
  if (fs_id) {
	...
	if (field_id == PRED_ID) {
	  int *dpv_ptr = &(XH_list_elem (dynam_pred_vals, XH_X (dynam_pred_vals, val)));
...
eval (local_ptr, local_kind)
     int **local_ptr, *local_kind;
{
  int    field_id, kind, i, ref, Xpq, left_kind, val, head, oper, string_id, local_atom_id, atom_id, static_field_kind;
...
{
  int prev_top, x, y, head, left_kind, i, field_id, kind, new_top;
  int *ptr;
...
re_eval (input)
     int input;
{
  int     oper, left_ret_val, right_ret_val, string_id, prev_top, x, ref, kind, field_id, atom_id, local_atom_id, val, new_top;
...
static BOOLEAN is_constrained_unification;
static BOOLEAN unify ();
...
unbounded_structure_unification (left_val_ptr, right_val_ptr, right_dol)
     int *left_val_ptr, *right_val_ptr, right_dol;
{
  int left_id, right_id, bot, top, cur, prev_top;
...
structure_unification (left_wfs_id, right_wfs_id, right_dol, checking)
     int     left_wfs_id, right_wfs_id, right_dol;
     BOOLEAN checking;
{
  int     field_id, field_kind, bot, cur, top, static_field_kind;
  int     left_page_nb, left_delta_nb, *left_attr_ptr0, *left_attr_ptr;
  int     right_page_nb, right_delta_nb, *right_attr_ptr0, *right_attr_ptr;
  int     right_id, new_wfs_id, bot2, top2, page_nb;
  int     *ptr;
  SXBA    left_field_set, right_field_set;
  BOOLEAN ret_val, has_right_field, is_shared, is_new_wfs_id;
...
unify_in (left_val_ptr, left_kind, right_val_ptr, right_kind)
     int     *left_val_ptr, left_kind, *right_val_ptr, right_kind;
{
  int top, left_id, right_id;
...
unify (left_val_ptr, left_kind, right_val_ptr, right_kind, checking)
     ...
     BOOLEAN checking;
{
  int             right_dol, main_head, val, left_id, right_id, new_wfs_id, top;
...
call_unify (operator, left_val_ptr, left_kind, right_val_ptr, right_kind, checking)
     int     operator, *left_val_ptr, left_kind, *right_val_ptr, right_kind;
     BOOLEAN checking;
{
  int              dum_kind, x, id;
  int              *dum_val_ptr; 
...
evaluate (xcode)
     int xcode;
{
  int              left_val, right_val, left_kind, right_kind, eval_result, operator, lhs_code_size;
  ...
  BOOLEAN          ret_val, is_left_re, is_right_re, is_operator_possible, is_skip, done;
...
tree_walk (prod)
     int prod;
{
  int        rule, equation_block, xcode, struct_nb, rhs_pos, pos, q, d, Xpq, init_prod, head, x, fs_id, mod, cur_bot;
  int        *ptr, *base_ptr, *bot_ptr;
  BOOLEAN    ret_val;
  struct lhs *plhs;
  struct rhs *p;
...
{
  int             bot2, cur2, top2, bot3, cur3, top3;
  int             field_id, field_kind, sous_cat_id, atom_id, local_atom_id;
  int             *local2atom_id;
  BOOLEAN         is_optional, is_first, is_first2, is_first3;
  struct attr_val *ptr;
  struct pred_val *ptr2;
  unsigned char   static_field_kind;
...
tfs_instantiation (struct_id, lexeme_id, priority, Tpq)
     int  struct_id, lexeme_id, priority, Tpq;
{
  int             dl_id, field_id, field_kind, field_val, fs_id, index;
  struct attr_val *ptr;
...
call_tfs_instantiation (x, Tpq)
     int  x, Tpq;
{
  int                        struct_id, lexeme_id, priority, dl_id, fs_id, index, lex_id;
     ...
     /* char *terminal_string; */
{
  ...
  int                        lexeme_id, struct_id, priority, head, fs_id;
...
set_terminal_leaves ()
{
  int  Tpq, T, p, q, trans, tok_no, if_id;
  char *terminal_string;
...
process_priority ()
{
  int     head_nb, prev_Xpq, x, max_prio, head, Xpq, y, fs_id, prev_y, prio;
  int     *new_head_stack;
  SXBA    new_head_set;

  new_head_stack = (int*) sxalloc (XxY_top (heads)+1, sizeof (int)), new_head_stack [0] = 0;
...
get_args_weight (fs_id)
     int fs_id;
{
  int x, weight, bot, top, val, field_id, static_field_kind, bot2, cur2, top2;
...
ranking_by_args ()
{
   ...
   */
  int     head_nb, prev_Xpq, x, max_weight, head, Xpq, y, fs_id, prev_y, weight;
  int     *new_head_stack;
  SXBA    new_head_set;
...
get_args_vicinity (fs_id, nb, dist)
     int fs_id, *nb, *dist;
{
  int bot, top, val, field_id, static_field_kind, bot2, cur2, top2;
...
ranking_by_vicinity ()
{
  ...
  */
  int     head_nb, prev_Xpq, x, min_vicinity, head, Xpq, y, fs_id, prev_y, vicinity, nb, dist;
  int     *new_head_stack;
  SXBA    new_head_set;
...
get_args_embedding (fs_id)
     int fs_id;
{
  int x, embedding, max_embedding, bot, top, val, field_id, static_field_kind, bot2, cur2, top2;
...
ranking_by_embedding ()
{
   ...
   */
  int     head_nb, prev_Xpq, x, max_embedding, head, Xpq, y, fs_id, prev_y, embedding;
  int     *new_head_stack;
  SXBA    new_head_set;
...
static int special_atomic_field_id;

static int
get_special_atomic_field (fs_id)
     int fs_id;
{
  int bot, cur, top, val, field_id, local_atom_id, *local2atom_id;
...
{
  int     head_nb, prev_Xpq, x, max_prio, head, Xpq, y, fs_id, prev_y, prio, id;
  int     *new_head_stack;
  SXBA    new_head_set;
  char    *str;
...
dag_td_walk (Aij_or_prod)
     int Aij_or_prod;
{
  int        prod, i, x, rule, fs_id, head, Xpq, pos, d;
  int        *local_head_stack, *base_ptr;
  BOOLEAN    is_valid, is_root;
  struct lhs *plhs;
  struct rhs *p;
...
{
  int  Tpq, T, p, q, trans, tok_no, t_nb;
  char *terminal_string;
...
{
  int             fs_id, bot, top, bot2, cur2, top2, bot3, cur3, top3, max_priority, priority;
  int             val, field_id, field_kind, sous_cat_id, atom_id, local_atom_id, x, i, Tpq, id;
  int             *local2atom_id;
  BOOLEAN         is_optional, is_first2;
  struct pred_val *ptr2;
  unsigned char   static_field_kind;
...
static int           subj_id, cat_id, obj_id, aij_id, vcomp_id, scomp_id, acomp_id, pcas_id;
static int           obj_ids [12]; /* initialises avec les valeurs suivantes */
static int           a_obj_id, de_obj_id, par_obj_id, devant_obj_id, dans_obj_id, sur_obj_id, en_obj_id, contre_obj_id, apres_obj_id,
                     avec_obj_id, vers_obj_id, pour_obj_id;
static int           vcomp_ids [3]; /* initialises avec les valeurs suivantes */
static int           a_vcomp_id, de_vcomp_id, par_vcomp_id;
static int           scomp_ids [3]; /* initialises avec les valeurs suivantes */
static int           scomp_id, a_scomp_id, de_scomp_id;
static int           acomp_ids [3]; /* initialises avec les valeurs suivantes */
static int           acomp_id, de_acomp_id, pour_acomp_id;
static int           adjunct_id;

static int           atom_v, atom_adv, atom_nc, atom_adj, atom_prep;
static int           Ei_lgth;
...
{
  int     head_nb, prev_Xpq, x, head, Xpq;
  int     *new_head_stack;
  SXBA    new_head_set;
...
easy_td_walk (Aij)
     int Aij;
{
  int Xpq, A, triple, trans, prod, item, Tpq, t, i, j, i_rcvr, j_rcvr;
...
easy_Sujet_Verbe ()
{
  int  fs_id, sujet_val, atom_id, href_verbe, aij_val, val, cur, top, Pij, Aij, A, item, Xpq, href_sujet, x; 
  int  *dpv_ptr, *val_ptr;
  char *str;
...
easy_td_Auxiliaire_Verbe (Aij)
     int Aij;
{
  int prod, item, Xpq, href_aux, href2_aux, href3_aux, href_v;
  char *str;
...
{  
  int  fs_id, objet_val, atom_id, x, aij_val, href_verbe, href_objet, object_aij_val; 
  int  *dpv_ptr, *val_ptr;
  char *str;
...
{  
  int  fs_id, objet_val, atom_id, x, aij_val, href_verbe, href_objet, i, cur, top, adjunct_val, object_aij_val, adjunct_aij_val; 
  int  *dpv_ptr, *val_ptr;
  char *str;
...
{
  int  fs_id, objet_val, atom_id, x, aij_val, href_verbe, href_objet, i, cur, top, adjunct_val, object_aij_val; 
  int  *dpv_ptr, *val_ptr;
  char *str;
...
easy_Complementeur ()
{
  int x, fs_id, i, objet_val, aij_val, object_aij_val, href1, href2;
  int *val_ptr;
...
{
  int  fs_id, objet_val, atom_id, x, aij_val, href1, href2, href3, i, cur, top, subj_val, val, object_aij_val; 
  int  *val_ptr;
...
easy_constituent ()
{
  ...
  easy_constituent_stack = (int*) sxalloc (final_mlstn+1, sizeof (int)), easy_constituent_stack [0] = 0;
  easy_Tij_stack = (int*) sxalloc (final_mlstn+1, sizeof (int)), easy_Tij_stack [0] = 0;
...
run \"bnf\" and \"csynt_lc\" on the CF skeleton generated by the SXLFG constructor executed with the \"-bnf\" option.\n", sxstderr);
  ...
  if (spf.outputG.start_symbol == 0) {
	...
	head_stack = (int*) sxalloc (XxY_top (heads)+1, sizeof (int)), head_stack [0] = 0;
