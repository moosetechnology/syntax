...
/* DO travaille sur where [1]
   ACTION travaille sur where [0] (apres swap eventuel)
   UNDO travaille sur where [1]
*/
    ...
    struct sxtables	*arg;
{
	...
	sxliglocals.stacks = (int **) sxcalloc ((unsigned long int) (sxindex_size (sxliglocals.tank) + 1),
						sizeof (int*));
	...
	*/
	...
	/* Reaffectation en sens inverse du DO */
	...
	if (act_no >= 10000)
	  {
	    act_no -= 10000;
	    stack_id = sxliglocals.DO_stack [STACKtop ()];
