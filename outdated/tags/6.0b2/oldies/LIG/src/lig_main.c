...
/* includes et variable pour la manipulation de la date pour lig_modif_time */
#include <sys/types.h>
#include <sys/stat.h>
#include <sys/timeb.h>


char	by_mess [] = "the SYNTAX grammar processor LIG";
    ...
    register char	*pathname;
{
	...
	if ((infile = fopen (mktemp (strcpy (pathname, "/tmp/ligXXXXXX")), "w")) == NULL) {
...
run:
    ...
    sxstr_mngr (BEGIN);
    (*(lig_tables.analyzers.parser)) (BEGIN);
    syntax (OPEN, &lig_tables);
    ...
    syntax (CLOSE, &lig_tables);
    (*(lig_tables.analyzers.parser)) (END);
    sxstr_mngr (END);
