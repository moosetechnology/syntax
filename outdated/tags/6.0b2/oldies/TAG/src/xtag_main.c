...
run:
    ...
    sxstr_mngr (BEGIN);
    (*(xtag_tables.analyzers.parser)) (BEGIN);
    syntax (OPEN, &xtag_tables);
	...
	do {
	    ...
	    if (argnum < argc)
	      ...
	      sxatc (CLOSE, &xtag_tables);

	    sxerrmngr.nbmess [2] += severity;
	} while (argnum < argc);
    }

    syntax (CLOSE, &xtag_tables);
    (*(xtag_tables.analyzers.parser)) (END);
