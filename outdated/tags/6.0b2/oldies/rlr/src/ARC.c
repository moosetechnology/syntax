...
static BOOLEAN	is_new_pss (pss, state, index, next_state, new_pss)
...
static BOOLEAN is_new_knoted_pss (pss, pred, index, next_state, pss1)
...
static BOOLEAN	is_same_pss (xkas, pss_sets)
    int		xkas;
    SXBA	*pss_sets;
{
...
static BOOLEAN	check_lc (max_min_lgth)
    int	max_min_lgth;
{
...
static BOOLEAN	install_arc_state (ARC, kas, xkas, new_arc)
    struct ARC_struct	*ARC;
    struct kas	*kas;
    int		xkas, *new_arc;
{
    ...
    int		(*F_get_conflict_kind) ();
{
    ...
    BOOLEAN		give_up, is_new_ARC_state, local_conflict_has_been_solved;
