;;; Modified Wed Aug 13 12:17:12 1986		;;;
;;; by Philippe Deschamp (PhD) of INRIA		;;;
;;; to clean up code of some bad actions.	;;;
;;; By the way, I figured everything out...	;;;
...
(defun
    ...
    (aton s ln l tracking done rebuild-summary
	  ...
	  (if rebuild-summary
		  ...
		  (pop-to-buffer "Aton Search")
		  (setq needs-checkpointing 0)	; no CKP files needed + PhD
		  (setq read-only 0)		; PhD
		  (local-bind-to-key "exit-emacs" "\^C"); PhD
		  (erase-buffer)
	      )
	  )
	  (save-excursion
	      ...
	      (pop-to-buffer "Aton Search")		; show user the results
	      (if rebuild-summary
		  (previous-line)
		  (next-line)
	      )
