...
#include "XH.h"
#include "XxY.h"
char WHAT_CHECKBNF[] = "@(#)SYNTAX - $Id: check_bnf.c 588 2007-05-25 12:32:11Z rlacroix $" WHAT_DEBUG;
...
static BOOLEAN  is_help, is_debug1, is_debug2, is_stdout, is_lexicon;
static BOOLEAN  is_cycle_free, is_lr_recursive_free, is_full_eps_rule_free, is_eps_rule_free;
...
static void
out_header (format, out_header_language_name, kind_str)
    char	*format, *out_header_language_name, *kind_str;
{
    long	date_time;
    
    date_time = time (0);

    printf (format, ctime (&date_time), out_header_language_name, kind_str);
}
     ...
     int prod, A;
{
  ...
  printf ("%i: %s = ", prod, get_nt_string (A));
...
static void
print_xnt (prefix, suffix, xnt)
     char *prefix, *suffix, xnt;
{
  int  X, Y, Z;
  char *p, *ptop;
...
static void
print_prod (prod, pleft, pright, pdot)
     int prod, *pleft, *pright, *pdot;
{
  int A, X;
...
complete_prod_hd ()
{
  int A, prod, bot_item, top_item, X, item, ntXprod, top_prod;
  BOOLEAN is_nullable;

  if (item2prod == NULL) {
    item2prod = (int*) sxalloc (XH_list_size (prod_hd)+1, sizeof (int));
    NULLABLE = sxba_calloc (XH_list_size (prod_hd)+1);
    XxY_alloc (&lhsntXprod_hd, "lhsntXprod_hd", 2*NBPRO+1, 1, 1, 0, NULL, NULL);
    XxY_alloc (&xnt_hd, "xnt_hd", 2*NTMAX+1, 1, 0, 0, NULL, NULL);
  }
  else {
    item2prod = (int*) sxrealloc (item2prod, XH_list_size (prod_hd)+1, sizeof (int));
...
fill_prod_hd ()
{
  int     A, bot, top, prod, item, X, prod_no;
  BOOLEAN has_changed;

  if (is_debug2)
    fputs ("\n*Initialisation\n", stdout);

  XH_alloc (&prod_hd, "prod_hd", 2*NBPRO+1, 1, (INDPRO/NBPRO)+1, NULL, NULL);
  ...
  for (A = 1; A <= NTMAX; A++) {
	...
	if (XH_set (&prod_hd, &prod_no)) {
	  ...
	  if (is_debug2) {
	    ...
	    print_prod (prod_no, &(REDUC (prod_hd, prod_no)), &(REDUC (prod_hd, prod_no+1)), NULL);
	  }
	}
	else {
...
eps_rule_elimination (empty_set)
     SXBA empty_set;
{
  int       A, x, prod, bot_item, item, top_item, X, nb, val, pos, prev_top, prod_no;
  ...
  XH_alloc (&new_prod_hd, "prod_hd", 2*XH_top (prod_hd)+1, 1, (INDPRO/NBPRO)+1, NULL, NULL);
  ...
  for (A = 1; A <= XNTMAX; A++) {
	...
	if (!SXBA_bit_is_set (empty_set, A)) {
	  ...
	  if (is_debug2) {
	    fputs ("*Copy ", stdout);
	    print_prod (prod, &(REDUC (prod_hd, prod)), &(REDUC (prod_hd, prod+1)), NULL);
	  }
	  ...
	  if (XH_set (&new_prod_hd, &prod_no)) {
	    ...
	    if (is_debug2) {
	      fputs ("*Into duplicate of ", stdout);
	      print_prod (prod_no, &(REDUC (new_prod_hd, prod_no)), &(REDUC (new_prod_hd, prod_no+1)), NULL);
	    }
	  }
	  else {
	    if (is_debug2) {
...
static BOOLEAN            is_cyclic;

static void
cycle_elimination (repr_set, cyclic_sets)
     SXBA repr_set, *cyclic_sets;
{
  int           A, B, prod, item, X, mark, prod_no, x, bot_item, top_item, eqA; 
  int           *nt2equiv, *empty_stack;
  SXBA          line, empty_set;
  XH_header     new_prod_hd;

  nt2equiv = (int*) sxcalloc (NTMAX+1, sizeof (int));
  ...
  if (!is_epsilon_free) {
    ...
    empty_stack = (int*) sxalloc (NTMAX+1, sizeof (int));
    empty_stack [0] = 0;
  ...
  if (SXBA_bit_is_set (empty_set, 1)) {
    ...
    */
    ...
    printf ("<new_%s = %s ;\n", get_nt_string (1)+1, get_nt_string (1));
    printf ("<new_%s = ;\n", get_nt_string (1)+1);
  }

  XH_alloc (&new_prod_hd, "prod_hd", 2*XH_top (prod_hd)+1, 1, XH_list_top (prod_hd)/XH_top (prod_hd)+1, NULL, NULL);
...
cycle_check ()
{
  int                prod, A, item, X, B, x, bot_item, top_item; 
  SXBA               line;
...
static void
left_recursion_elimination (lr_nt_set/*, rr_nt_set */)
     SXBA lr_nt_set/*, rr_nt_set */;
{
  int A, x, prod, bot_item, top_item, item, X, prod_no, A2prod, Aprime, Aprod, B, Bprod, B2prod, repr, minusA, plusA;
  int Abot_item, Atop_item, Aitem, dum, leftmostA; 
  ...
  while ((A = sxba_scan (lr_nt_set, A)) > 0) {
	...
	if (is_debug2) {
	  fputs ("*Into ", stdout);
	  print_prod (prod_no, &(REDUC (lr_prod_hd, prod_no)), &(REDUC (lr_prod_hd, prod_no+1)), NULL);
	}
	...
	if (is_debug2) {
	  printf ("*Into a duplicate of prod #%i:  ", prod_no);
	  print_prod (prod_no, &(REDUC (lr_prod_hd, prod_no)), &(REDUC (lr_prod_hd, prod_no+1)), NULL);
	}
      }
    }
  ...
  while ((A = sxba_1_rlscan (lr_nt_set, A)) > 0) {
	...
	if (is_debug2) {
	  fputs ("*Change ", stdout);
	  print_prod (Aprod, &(REDUC (lr_prod_hd, Aprod)), &(REDUC (lr_prod_hd, Aprod+1)), NULL);
	}
	...
	XxY_Xforeach (lrA2prod_hd, B, B2prod) {
	  ...
	  if (bot_item < top_item
	    ...
	    if (plusA != 0) {
	      ...
	      if (is_debug2) {
		fputs ("*Expand ", stdout);
		print_prod (Bprod, &(REDUC (lr_prod_hd, Bprod)), &(REDUC (lr_prod_hd, Bprod+1)), NULL);
	      }
...
lr_recursive_check ()
{
  int                prod, A, X, nb, B, item, x, bot_item, top_item, Aprod, prod_no, prev_xnt, Aprime; 
  ...
  if (is_ambiguous) {
    ...
    if (is_debug1) {
      printf ("\n* This grammar is ambiguous, %i nonterminal symbol%s both left and right recursive:\n* ",
	      nb, nb == 1 ? " is" : "s are");
    ...
    tbp = (int*) sxalloc (XNTMAX+1, sizeof (int));
    tbp [0] = 0;
    ...
    if (is_lr_recursive_free) {
      ...
      XH_alloc (&lr_prod_hd, "lr_prod_hd", XH_top (prod_hd)+1, 1, XH_list_top (prod_hd)/XH_top (prod_hd)+1, NULL, NULL);
      XxY_alloc (&lrA2prod_hd, "lrA2prod_hd", XH_top (prod_hd)+1, 1, 1, 0, NULL, NULL);
      ilr_nt_set = sxba_calloc (XNTMAX+1);
      non_lr_nt_set = sxba_calloc (XNTMAX+1);
      Aprimes = (int*) sxcalloc (NTMAX+1, sizeof (int));
	...
	left_recursion_elimination (LR [A]/*, RR [A] */);
      ...
      XH_alloc (&new_prod_hd, "prod_hd", XH_top (prod_hd)+XH_top (lr_prod_hd)+1, 1, 
		XH_list_top (lr_prod_hd)/XH_top (lr_prod_hd)+1, NULL, NULL);
	...
	if (SXBA_bit_is_set (LR [0], A)) {
	  XxY_Xforeach (lrA2prod_hd, A, Aprod) {
	    ...
	    if (is_debug2) {
	      fputs ("*From ", stdout);
	      print_prod (prod, &(REDUC (lr_prod_hd, prod)), &(REDUC (lr_prod_hd, prod+1)), NULL);
	    }
    ...
    char	*name;
{
  ...
  if (is_lexicon) {
	...
	reached_stack = (int*) sxalloc (XNTMAX+1, sizeof (int));
	reached_stack [0] = 0;
	...
	for (A = 1; A <= XNTMAX; A++) {
	  if (SXBA_bit_is_set (reached_set, A)) {
	    foreach_NPG (A, x) {
	      prod = NUMPG (x); /* prod est une A-prod */
	      print_prod (0, &(REDUC (prod_hd, prod)), &(REDUC (prod_hd, prod+1)), NULL);
	    }
	  }
	}
