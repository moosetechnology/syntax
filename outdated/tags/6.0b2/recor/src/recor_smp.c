...
char WHAT_RECORSMP[] = "@(#)SYNTAX - $Id: recor_smp.c 580 2007-05-24 12:45:54Z rlacroix $" WHAT_DEBUG;
...
static int	maxlregle, xstring_mess, nbcart, lstring_mess, maxparam, maxglobal_mess, xparam, nmax, xregle;
...
static VOID	set_code (no, ic, designator)
    SXBA	no;
    int		ic;
    struct sxsource_coord	designator;
{
...
static VOID	process (no, node_ptr, bi, bs)
    SXBA	no;
    struct recor_node	*node_ptr;
    int		*bi, *bs;
{
...
static VOID	set_classes (node_ptr, no)
    struct recor_node	*node_ptr;
    SXBA	no;
{
    /* Remplit "no" avec les classes de caracteres reperes par ate */
    register struct recor_node	*son;
    struct sxsource_coord	designator;
    register int	ic;
    int		bi, bs, binf, bsup;
...
{
    register struct recor_node	*visited = VISITED;
    struct recor_node	*node_ptr;
...
*/
	...
	if (!visited->not_is_first_visit) {
	    ...
	    nmax = 0;
	    maxglobal_mess = 5;
	    lregle = R_tables.P_lregle = (nbcart == 0) ? NULL : (int**) sxalloc (nbcart + 1, sizeof (int*));

	    for (i = 1; i <= nbcart; i++)
		R_tables.P_lregle [i] = (int*) sxcalloc (maxlregle + 1, sizeof (int));

	    R_tables.P_right_ctxt_head = (nbcart == 0) ? NULL : (int*) sxalloc (nbcart + 1, sizeof (int));
	...
	if (!visited->not_is_first_visit) {
	    ...
	    nmax = 0;
	    maxglobal_mess = 5;
	    lregle = R_tables.S_lregle = (nbcart == 0) ? NULL : (int**) sxalloc (nbcart + 1, sizeof (int*));

	    for (i = 1; i <= nbcart; i++)
		R_tables.S_lregle [i] = (int*) sxcalloc (maxlregle + 1, sizeof (int));
...
/*   A T T R I B U T E S    E V A L U A T I O N   */
	...
	for (x = 1; x <= R_tables.R_tbl_size.P_nbcart; x++) {
	    ...
	    while (--l > 0) {
		register int	i = R_tables.P_lregle [x] [l];
...
VOID	recor_smp (what, sxtables_ptr)
    int		what;
    struct sxtables	*sxtables_ptr;
{
    switch (what) {
