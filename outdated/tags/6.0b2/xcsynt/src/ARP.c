...
char WHAT_XCSYNTARP[] = "@(#)SYNTAX - $Id: ARP.c 625 2007-06-12 08:35:56Z syntax $" WHAT_DEBUG;

#include "SS.h"
#include "RLR.h"

extern BOOLEAN	is_old_clone ();
extern BOOLEAN	foreach_LA ();
extern VOID	ARP_conflict_messages ();
extern BOOLEAN	bh_ARC_one_LA ();
extern BOOLEAN	bs_ARC_one_LA ();

static SXBA	ARP_items_set;
static BOOLEAN	is_ARP_state_ambig, is_ARP_opened;
...
static BOOLEAN	ARP_is_final_state (ARP_state)
    int		ARP_state;
{
...
static int	ARP_install_items_set ()
{
   /* ARP_items_set contient un ensemble d'items, on les stocke dans ARP_hd */
    ...
    register int	item = 0;
    register BOOLEAN	is_final = FALSE;
    int		result;
...
static VOID	ARP_init (XARC, XARC_state)
    ...
    int		XARC_state;
{
    register int	x, y, ARP_init_fork, xk;
...
static BOOLEAN	ARP_is_ambiguous (XARC, XARC_state, ARP_state)
    struct ARC_struct	*XARC;
    int		ARP_state, XARC_state;
{
    ...
    register int	item, pss, wst;
    register int	*bot, *top, *bot1, *top1;
    int		type, wsb, n_0;
    BOOLEAN	is_first_time, is_ambig;
    SXBA	or_set;
...
{
  register int	XARC_state, ARP_state;
...
BOOLEAN PARTIAL_FSA_ARP_construction (XARC, bfsa1, bfsa2)
    struct ARC_struct	*XARC;
    int		bfsa1, bfsa2;
{
  register int	ARP_state;
