...
/*				a l'index t_error			*/
/************************************************************************/
/* Jul 5 1996 14:34 (pb):	Modifs de sxtime (ds OPTIM) et header	*/
/************************************************************************/
/* 29-05-95 18:15 (pb):		Bug ds la taille max de t_items si nd	*/
/************************************************************************/
...
#include "D_tables.h"
#include "bstr.h"
char WHAT_XCSYNTLALR1[] = "@(#)SYNTAX - $Id: LALR1.c 625 2007-06-12 08:35:56Z syntax $" WHAT_DEBUG;


extern BOOLEAN	prio_read (), prio_free ();
extern BOOLEAN	ambiguity (), is_not_nullable ();
...
extern BOOLEAN	ante ();
extern BOOLEAN	ARC_construction ();
/* extern VOID	sxtime (); */
...
static char	star_71 [] = "***********************************************************************";
static BOOLEAN	is_lalr1, is_ARCs, is_conflict, is_ARC_init_called, ff_needed,
...
static BOOLEAN	is_users_conflict, is_systems_conflict;
...

static VOID	oflw_Q0xT_to_conflict_action (old_size, new_size)
...
/* A METTRE DS LR(0) */
static BOOLEAN is_item_in_state (item, state)
    int item, state;
{
    /* Retourne TRUE ssi item est un element de state. */
    register int	tnt, n, item_n, is_item_in_state_StNt;
    int			lim;
...
static VOID	lookback (state, item, lookback_StNt)
    int		state, item, lookback_StNt;
{
...
VOID compute_Next_states_set ()
{
    register int	xac1, nt, compute_Next_states_set_StNt;
    register struct Q0	*aQ0;
    int			lim;
...
static VOID	compute_includes (xac1)
    int		xac1;
{
    register int	compute_includes_StNt, lim;
...
static VOID	set_chosen (drk, at, rn, p)
    int		drk, at, rn;
    struct priority	p;
{
...
static BOOLEAN erase_StNt_StNt1 (St, item)
    int St, item;
{
    /* Appelee depuis ante */
...
static VOID	fill_items_set (fill_items_set_nt_trans_set, fill_items_set_nt_trans_stack, nt_trans)
    ...
    int		nt_trans;
{
    ...
    register int	*bot, *top;
    int			item, xpnt2, fill_items_set_StNt;
    BOOLEAN		is_a_tree = FALSE;
...
static VOID	fill_look_ahead_set (fill_look_ahead_set_items_set, la_set)
    register int	*fill_look_ahead_set_items_set;
...
static VOID	set_conflict_action (xac1)
    int xac1;
{
    ...
    register int	t, set_conflict_action_StNt, xla, red_no;
    register struct Q0	*aQ0;
    int			lim, min_red_no, indice;
    BOOLEAN		to_be_processed;
...
static int equiv_rule_processing (equiv_rule_processing_StNt, lim, base, set)
    int		equiv_rule_processing_StNt, lim, base;
    SXBA	set;
{
...
static BOOLEAN	compute_lalr1_conflicts (xac1)
    int		xac1;
{
    /* Recherche des conflits (eventuels) en LALR(1) */
    register int	r1, compute_lalr1_conflicts_StNt, xla, t;
    int			lim, base_xla, base_StNt, cur_rule_no;
    register SXBA	la_set_r1;
    register struct Q0	*aQ0;
    BOOLEAN	compute_lalr1_conflicts_is_conflict = FALSE;
...
static struct bstr	*bstr_cat_T_set (bstr, bstr_cat_T_set_t_set)
    register struct bstr	*bstr;
    register SXBA	bstr_cat_T_set_t_set;
{
    register int nb, t;
    BOOLEAN is_af;
...
static BOOLEAN state_to_conflict_set (xac1, is_u, is_s, mask, state_to_conflict_set_t_set)
    int 		xac1, mask;
    BOOLEAN		is_u, is_s;
    register SXBA	state_to_conflict_set_t_set;
{
    ...
    register BOOLEAN	not_empty = FALSE;
...
VOID print_conclusions (xac1, print_conclusions_t_set)
    int xac1;
    SXBA print_conclusions_t_set;
{
...
static VOID print_minimal_message (xac1, print_minimal_message_t_set, gk)
    int		xac1;
    SXBA	print_minimal_message_t_set;
    char	*gk;
{
    register int	print_minimal_message_StNt, lim;
    register SXBA	la_set;
    register struct Q0	*aQ0 = Q0 + xac1;
...
static VOID print_minimal_messages ()
{
    register int	xac1 = 0;
...
static VOID set_conflict_kind (germe, set_conflict_kind_t_set, failed_kind, is_ALL)
    int			germe, failed_kind;
    register SXBA	set_conflict_kind_t_set;
    BOOLEAN		is_ALL;
{
...
VOID solved_by_ARC (germe, arc_no, solved_by_ARC_t_set)
    int germe, arc_no;
    SXBA	solved_by_ARC_t_set;
{
    /* Les conflits de solved_by_ARC_t_set sur l'etat germe ont ete resolus par l'ARC arc_no. */
    register int t = 0;
    register struct chosen	*pchosen;
...
BOOLEAN	should_print_conflict (conflict_kind)
    int conflict_kind;
{
...
static BOOLEAN	call_ARC (is_u, is_s, processed_kind)
    BOOLEAN	is_u, is_s;
    int		processed_kind;
{
    register int	xac1 = 0;
...
/* t_set est non vide, on construit l'ARC */
		...
		if (xac2 > oxac2) {
		    ...
		    register int	call_ARC_StNt, call_ARC_cur_red_no, xla;
		    int			lim;
...

    ...
    if (sxverbosep)
	sxtime (ACTION, processable_state_nb > 0 ? "failed" : "succeeded");
...

	...
	if (ambig_root_set != NULL) {
		...
		register int	xac1, LALR1_StNt, lim, x;
		register struct Q0	*aQ0;
		BOOLEAN		LALR1_should_print_conflict;
	...
	if (sxverbosep)
	    sxtime (ACTION, is_ambiguous ? "failed" : "succeeded");
	...
	if (!is_epsilon_free) {
		...
		processable_state_nb > 0) {
		register int	xac1;
		register int		lim, LALR1_StNt_2;
		register struct Q0	*aQ0;
		SXBA			StNt_set2;
		BOOLEAN			LALR1_should_print_conflict_2;
	    ...
	    if (sxverbosep)
		sxtime (ACTION, is_not_LRpi ? (kind == NOT_LRPI_ ? "failed" : "failed (ambiguous)") : "succeeded");
	}
    }
...

    ...
    if (processable_state_nb > 0) {
	BOOLEAN is_u, is_s, old_is_lr_constructor;
	...
	if (is_systems_conflict) {
		...
		if (sxverbosep)
		    sxtime (ACTION, is_conflict > 0 ? "failed" : "succeeded");
	    }
...
/* Floyd_Evans des transitions non-terminales */
    ...
    {
	register int	xac1, LALR1_StNt_4, x, lim;
...

	{
	    ...
	    BOOLEAN	is_xt, should_call_RLR_cg;
	    register struct Q0	*aQ0;
		...
		if ((lim = aQ0->red_trans_nb) > 0) {
		    register int		LALR1_StNt_6, t, indice;
		    int				red_no, dum;
