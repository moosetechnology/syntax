...
/************************************************************************/
/* 19-12-94 13:15 (pb):		si "is_left_corner" les transitions	*/
/*				non-terminales sont differentes		*/
/************************************************************************/
/* 16-12-94 17:34 (pb):		On ne genere pas de shift_reduce	*/
/*				si "is_left_corner"			*/
/************************************************************************/
/* 19-08-94 14:46 (pb):		Les "codes" des sequences non		*/
/*				deterministes sont superieures a 20000	*/
/*				(et non plus a 10000)			*/
/************************************************************************/
/* 08-08-94 15:35 (pb):		Correction de bugs sur les predicats	*/
/*				et le non-determinisme			*/
/************************************************************************/
...
char WHAT_XCSYNTOPTIM[] = "@(#)SYNTAX - $Id: OPTIM.c 613 2007-06-07 13:54:29Z rlacroix $" WHAT_DEBUG;
...
int
put_in_fe (fe, put_in_fe_hashs, put_in_fe_lnks, afe)
    struct floyd_evans	*fe, *afe;
    int			*put_in_fe_lnks;
    int			*put_in_fe_hashs;
{
    int			hash, x, y;
...
static int gen_nd_seq (hd, links, gen_nd_seq_t_items, fe)
    int			hd, *links;
...
static int gen_prdct_seq (hd, links, gen_prdct_seq_t_items, fe, is_t)
    int			hd, *links;
    struct P_item	*gen_prdct_seq_t_items;
    struct floyd_evans	*fe;
    BOOLEAN		is_t; /* Cas des t_tables */
{
...
static SXVOID compute_action_set (xac1, compute_action_set_t_items, action_set)
    int			xac1;
...
/* Le floyd_evans des transitions terminales, non-terminales et des predicats est regenere
   dans une structure unique tnt */
	...
	if (!is_left_corner) {
	    ...
	    int			OPTIM_action_2, xlink, old_xlink, old_xlink_lim;
	    struct P_item	*old_aitem, *old_nt_items = nt_items;
	    SXBA		nt_set;

	    int			*OPTIM_link, *xnt_to_link;

	    nt_items = (struct P_item*) sxalloc (xnt_items + 1, sizeof (struct P_item));
	    OPTIM_link = (int*) sxalloc (xnt_items + 1, sizeof (int));
	    xnt_to_link = (int*) sxcalloc (bnf_ag.WS_TBL_SIZE.xntmax + 1, sizeof (int));
	    nt_set = sxba_calloc (NTMAX + 1);
...
/* Attention le test de "EOF" doit etre conserve sur l'instruction "Halt" */
	...
	register struct state		*astate;
	register struct floyd_evans	*afe;
	int				l, action, xlink, old_xlink, old_xlink_lim;
	int				*reduces, *OPTIM_link_2, *xt_to_link;
	BOOLEAN				is_force_error;

	t_items = (struct P_item*) sxalloc (xt_items + 1, sizeof (struct P_item));
	OPTIM_link_2 = (int*) sxalloc (xt_items + 1, sizeof (int));
	...
	for (i = 1; i < xe1; i++) {
		...
		if (xt_state_set != NULL && SXBA_bit_is_set (xt_state_set, i)) {
			...
			if (aitem->action != 0) {
				...
				/* Tete de lien sur les predicats. */
				register int OPTIM_x = xt_to_link [t], y;
...
/*   L I N E A R I S A T I O N   D E S   M A T R I C E S   */
	...
	int	*state_to_state, *witness, xw, vector_used_nb;
	SXBA	bases_set;
...

	{
	    ...
	    algoV (FALSE, xe1, bnf_ag.WS_TBL_SIZE.xtmax, xt_items, t_states, t_items,
		   t_bases, &vector, &bases_set, &MIN, &MAX, &delta, &vector_used_nb);
	    ...
	    if (sxverbosep) {
	        fprintf (sxtty, "(density=%i%%) ", 100*vector_used_nb/t_vector_size);
		sxtime (ACTION, "done");
		sxttycol1p = TRUE;
	    }
	}
   ...
   elevee. */ 

	{
	    ...
	    algoV (TRUE, xac2, bnf_ag.WS_TBL_SIZE.xntmax, xnt_items, nt_states, nt_items,
		   nt_bases, &vector, &bases_set, &MIN, &MAX, &delta, &vector_used_nb);
	...
	if (sxverbosep) {
	    fprintf (sxtty, "(density=%i%%) ", 100*vector_used_nb/(MAX - MIN + 1));
	    sxtime (ACTION, "done");
	    sxttycol1p = TRUE;
	}
    }
...
/* Permet, a l'analyse (par dichotomie), de retrouver un predicat a partir
   du couple (red_no, position). */
	    int	item, OPTIM_lim, v, red_no, pos, prdct;
