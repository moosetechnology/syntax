...
#include "release.h"
#include "bstr.h"
char WHAT_XCSYNTLEFTCORNER[] = "@(#)SYNTAX - $Id: left_corner.c 599 2007-05-29 13:06:45Z rlacroix $" WHAT_DEBUG;
...
static int	option_get_kind (option_get_kind_arg)
    register char	*option_get_kind_arg;
{
    register char	**opt;
...
char *get_constructor_name (string, is_lalr, is_rlr, h, k)
    char	*string;
    BOOLEAN	is_lalr, is_rlr;
    int		h, k;
{
    sprintf (string, "%sLC (%d)", k > 0 ? "LA" : "", k);
