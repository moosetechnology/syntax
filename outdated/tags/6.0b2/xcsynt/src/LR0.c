...
/************************************************************************/
/* 20030313 09:13 (phd):	(undefvarfetch) sur Alpha		*/
/************************************************************************/
...
char WHAT_XCSYNTLR0[] = "@(#)SYNTAX - $Id: LR0.c 580 2007-05-24 12:45:54Z rlacroix $" WHAT_DEBUG;


static int	limitem, limt, limnt, limxnt;
    ...
    int		old_size, new_size;
{
    ...
    Q0xV_to_Q0xXNT = (int*) sxrealloc (Q0xV_to_Q0xXNT, new_size + 1, sizeof (int));
}



static VOID	mettre (item)
    register int	item;
{
    register int	x, tnt;
    register int	*q;
...
VOID	LR0 ()
{
	...
	Q0xV_to_Q0xXNT = (int*) sxalloc (XxY_size (Q0xV_hd) + 1, sizeof (int));
	XxY_alloc (&Q0xQ0_hd, "Q0xQ0", INDPROt4, 2, 0, 1, NULL, statistics_file);
    }

    items_list = (int*) sxalloc (limitem, sizeof (int));
    tsymb1 = (int*) sxalloc (limxnt - limt, sizeof (int)) - limt;
