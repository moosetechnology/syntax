...
/************************************************************************/
/* 02-12-94 16:41 (pb):		Ajout de cette rubrique "modifications"	*/
/************************************************************************/
...
char WHAT_XCSYNTLC0[] = "@(#)SYNTAX - $Id: LC0.c 625 2007-06-12 08:35:56Z syntax $" WHAT_DEBUG;
...
static int		*tbp_stack;
static SXBA		tbp_set;


static int	limitem, limt, limnt, limxnt, limpro, INDPRO;

static int	Mt_items, Mnt_items;
static BOOLEAN	ff_needed;



static VOID
ntXnt_oflw (old_size, new_size)
...
static VOID
stateXlhs_oflw (old_size, new_size)
    int	old_size, new_size;
{
    ...
    int state, item;
{
    ...
    BOOLEAN	ret_val;
    int		tnt, xprod, trans, next_state, dum;
...
print_state (state)
    int	state;
{
    int		next_state, tnt, trans, la, xprod, empty, item;
    char	s1 [12], *string;
    BOOLEAN	is_non_kernel;
...
static void
gen_fe (state, red_no, xtnt)
    int state, red_no, xtnt;
{
    int			next_state, tnt, xprod, reduce, next, pspl;
    struct P_item	*aitem;
    ...
    int kind, t, *act;
{
	...
	if (nd_hd == NULL) {
	    ...
	    nd_hd = (int*) sxcalloc (size, sizeof (int));
...
ntfe ()
{
    int			state, next_state, tnt, xnt, act, red_no, x, trans, xprod;
    struct state	*astate;
    int			*nter;
    BOOLEAN		is_xnt, is_nd_list;
    SXBA		xnt_set;
    ...
    nt_lnks = (int*) sxalloc (limitem + 1, sizeof (int));

    nter = (int*) sxcalloc (limxnt, sizeof (int));
...
redfe ()
{
    int			xprod, item, trans, state, para_state, next_state, t, empty, nt, la, xt,
                        prev_item;
...
tfe ()
{
    int			state, tnt, xt, t, red_no, act, ntla, empty, next_state, trans,
    ...
    t_lnks = (int*) sxalloc (size, sizeof (int));
    t_lnks [0] = 0;
    ...
    ter2red = (int*) sxcalloc (limt, sizeof (int));
...
LC0 ()
{
    int		xprod, B, A, nt, state, tnt, next_state, trans, lim, i, item, lhs, la, ntla,
                empty, prev_state, n, x, z;
    int		prev, next, depend;
    SXBA	work_set, prev_set, next_set;
    SXBA	line;
    BOOLEAN	is_epsilon, look_ahead_needed;
    ...
    state_stack = (int*) sxalloc (INDPRO, sizeof (int));
...
/* On traite les productions vides. */
	...
	while (!SS_is_empty (tbp_stack)) {
		...
		sxba_copy (work_set, prev_set);
		sxba_minus (work_set, next_set);

		if (!sxba_is_empty (work_set)) {
		    /* non inclus */
...
/* On verifie que toutes les reductions sont generees.
   Le traitement des conflits ayant pu en supprimer. */
	...
	t_state_equiv_class = (int*) sxalloc (limitem + 1, sizeof (int));
	nt_state_equiv_class = (int*) sxalloc (limitem + 1, sizeof (int));
