...
#include "sxunix.h"
char WHAT_TDEFPPTDEFMAIN[] = "@(#)SYNTAX - $Id: pptdef_main.c 580 2007-05-24 12:45:54Z rlacroix $" WHAT_DEBUG;
...
static int	option_kind [] = {UNKNOWN_ARG, VERBOSE, VERBOSE, -VERBOSE, -VERBOSE, LINE_LENGTH, LINE_LENGTH, TABS, -
     TABS, EVERY};
...
static SXVOID	pptdef_run (pathname)
...
int main (int argc, char *argv[])
{
  ...
  for (argnum = 1; argnum < argc; argnum++) {
	...
	fprintf (sxstderr, "%s: a numeric argument must follow \"-ll\", %d value is retained.\n", ME,
    ...
    case EVERY:
      if (++argnum >= argc || !sxnumarg (argv [argnum], &val))
	fprintf (sxstderr, "%s: a numeric argument must follow \"-every\", %d value is retained.\n", ME,
