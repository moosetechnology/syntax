...
#include "sxunix.h"
#include "B_tables.h"
char WHAT_TABC[] = "@(#)SYNTAX - $Id: tabc.c 580 2007-05-24 12:45:54Z rlacroix $" WHAT_DEBUG;
...
/* on multiplie par deux leur taille et on continue */
    ...
    register int	i, old_size, new_size;

    fprintf (sxtty, "%s: Tables of size %d overflowed: reallocation with size %d.\n", ME, tbl_lgth, new_size = tbl_lgth *
	 2);
    old_size = tbl_lgth;
    tbl_lgth = new_size;
    attr_to_ste = (int*) sxrealloc (attr_to_ste - NBTERM, new_size + NBTERM, sizeof (int)) + NBTERM;
    defini = (int*) sxrealloc (defini, new_size + 1, sizeof (int));
    type_attr = (int*) sxrealloc (type_attr, new_size + 1, sizeof (int));

    for (i = old_size + 1; i <= new_size; i++) {
	type_attr [i] = 0;
    }

    attr_nt = (char**) sxrealloc (attr_nt, new_size + 1, sizeof (int));
...
static VOID	put_ste_to (ste, code, nature)
    register int	ste;
    short	code, nature;
{
    register struct ste_elem	*p1, *p2;
    register int	x;
...
static short	get_ste_to (ste, nature)
    int		ste;
    short	nature;
{
    register struct ste_elem	*ptr;
    register int	x;
...
int	posprod (prodnum, tntno, quote)
    int		prodnum /* numero d'une production */
		       , tntno /* indice d'un nt ou d'un t */
			      , quote;
/* nombre de quotes */

{
    register int	i, quote_bis, code, lgth;
...
static VOID	print_ident (attrg, attrd, posd)
    int		attrg, attrd, posd;
{
    register int	pg;
    register char	*nt1, *nt2, *quotes_ptr;
...
VOID	creervide (attrg, attrd, posd)
...
VOID	creermilieu (attrg, attrd, posd)
    int		attrg, attrd, posd;
{
...
VOID	tilt (attrno, ntno, posp)
    int		attrno, ntno, posp;
{
    /* pour crier quand il y a une erreur */

    register int	i, nbq = 0, s;
...
/* longueur variable */
    ...
    defini = (int*) sxalloc (tbl_lgth + 1, sizeof (int));
    type_attr = (int*) sxcalloc (tbl_lgth + 1, sizeof (int));
