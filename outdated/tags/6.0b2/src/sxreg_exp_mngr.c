...
char WHAT_SXREG_EXP_MNGR[] = "@(#)SYNTAX - $Id: sxreg_exp_mngr.c 527 2007-05-09 14:02:47Z rlacroix $" WHAT_DEBUG;
...
static int	node_no, item_no;

static VARSTR	vstr;
static int	master_reg_exp_no;

static void
reg_exp_pcn_oflw (old_size, new_size)
    int	old_size, new_size;
{
    if (SXWORD_is_static (SXDS->sc_names))
    {
	int *old = SXDS->sc_names_to_char_sets;

	SXDS->sc_names_to_char_sets = (int*) sxalloc (new_size,
						      sizeof (int));
	memcpy (SXDS->sc_names_to_char_sets, old, sizeof (int) * old_size);
    }
    else
	SXDS->sc_names_to_char_sets = (int*) sxrealloc (SXDS->sc_names_to_char_sets,
							new_size,
							sizeof (int));
}
...
reg_exp_pcn_alloc ()
{
    int 		number = sizeof (predefs) / sizeof (predefs [0]);
    int			c, i;
    unsigned int	ste;
    SXBA		set;
    char		*string;
    ...
    sxword_alloc (&SXDS->sc_names, "simple_class_names",
		  (unsigned int) 2*number,
		  1, 16, sxcont_malloc, sxcont_alloc, sxcont_realloc, NULL, sxcont_free, reg_exp_pcn_oflw, NULL);

    SXDS->sc_names_to_char_sets = (int*) sxalloc (SXWORD_size (SXDS->sc_names),
						  sizeof (int));
...
static void
terminals_oflw (old_size, new_size)
    int	old_size, new_size;
{
...
static void
abbrev_names_oflw (old_size, new_size)
    int	old_size, new_size;
{
...
static void
reg_exp_names_oflw (old_size, new_size)
    int	old_size, new_size;
{
...
sxre_alloc ()
{
    ...
    SXDS->context_sets = sxbm_calloc ((int) SXWORD_size (SXDS->reg_exp_names),
				      (int) SXWORD_size (SXDS->reg_exp_names));
...
void
sxre_doit (infile, file_name_or_source_string)
    FILE	*infile;
...
static void
set_new_item (kind, set_new_item_item_no, val, param, next, is_erased)
    int		kind, set_new_item_item_no, val, param;
    SXBA	next;
    BOOLEAN	is_erased;
{
    int new_item_no, next_item, couple;
...
static void
insert_abbrev (abbrev_no, insert_abbrev_item_no, next, is_erased)
    int		abbrev_no, insert_abbrev_item_no;
    SXBA	next;
    BOOLEAN	is_erased;
{
    int			init_item, final_item, new_item, delta, item, next_item,
                        couple, new_couple;
...
static void
abbrev_to_first (abbrev_no, abbrev_to_first_item_no, first)
    int		abbrev_no, abbrev_to_first_item_no;
    SXBA	first;
{
    int		init_item, delta, new_item, couple;
...
static	BOOLEAN
process_action (visited, action, param)
    struct reg_exp_node	*visited;
    int			*action, *param;
{
    SXBA	char_set;
...
static	BOOLEAN
process_predicate (visited, action, param)
    struct reg_exp_node	*visited;
    int			*action, *param;
{
    *param = 0;
...
static	BOOLEAN
third_pass (visited, next, is_del)
    struct reg_exp_node	*visited;
    SXBA		next;
    BOOLEAN		is_del;
{
	...
	set_new_item (KACTION, visited->item_no, action, param, next, is_del);
	break;
	...
	set_new_item (KCLASS, visited->item_no, SXDS->char_sets_top, 0, next, is_del);
	break;
	...
	set_new_item (KCLASS, visited->item_no, SXDS->char_sets_top, 0,
		      next, is_del);
	break;
	...
	set_new_item (KPREDICATE, visited->item_no, action, param, next, is_del);
	break;
	...
	else
	{
	    ste = sxword_retrieve (&SXDS->sc_names, name);
	    /* ste est l'identifiant d'une classe simple */
	    set_new_item (KCLASS, visited->item_no, SXDS->sc_names_to_char_sets [ste], 0,
			  next, is_del);
	}
...
/* Visite recursive des CLASSES */
	...
	if (sxword_2retrieve (&SXDS->abbrev_names, name, (unsigned long) lgth) != ERROR_STE)
	    /* C'est une abbrev */
	    return error ("illegal usage of an abbreviation name", son_1), FALSE;

	/* On peut redefinir une classe simple */
	ste = sxword_2save (&SXDS->sc_names, name, (unsigned long) lgth);
	SXDS->sc_names_to_char_sets [ste] = SXDS->char_sets_top;
...
static BOOLEAN
regular_expression (visited, is_action)
    struct reg_exp_node	*visited;
    BOOLEAN		*is_action;
{
    ...
    {
      int prev_line_nb = SXDS->tfirst_line_nb;
...
/* Visite recursive des ABBREVIATIONS */
	...
	if (sxword_2retrieve (&SXDS->sc_names, name, (unsigned long) lgth) != ERROR_STE)
	...
	SXDS->current_reg_exp_or_abbrev_no = -sxword_2save (&SXDS->abbrev_names, name, (unsigned long) lgth);
	SXDS->abbrev_to_attr [-SXDS->current_reg_exp_or_abbrev_no].init_item =
	    SXDS->item_top;
...
static BOOLEAN
set_priority_kind (visited, current)
    struct reg_exp_node	*visited;
    char		current;
{
...
process_context:
	...
	ste = sxword_2retrieve (&SXDS->reg_exp_names, name, (unsigned long) lgth);
	...
	if (ste == ERROR_STE)
	{
	    ...
	    ste = sxword_2save (&SXDS->reg_exp_names, name, (unsigned long) lgth);
	...
	ste = sxword_2save (&SXDS->terminals, name, (unsigned long) lgth);
        {
...

{
    ...
    struct reg_exp_to_attr	*p;
    char		*name;
    int			lgth, init_item, couple, new_couple, cur_reg_exp;
    BOOLEAN		is_action;
	...
	SXDS->current_reg_exp_or_abbrev_no =
	    sxword_2retrieve (&SXDS->reg_exp_names, name, (unsigned long) lgth);
	...
	if (SXDS->current_reg_exp_or_abbrev_no == ERROR_STE)
	{
	    ...
	    SXDS->current_reg_exp_or_abbrev_no =
		sxword_2save (&SXDS->reg_exp_names, name, (unsigned long) lgth);
	...
	set_new_item (KREDUCE, item_no, 0, 0, NULL, FALSE);
...
int
sxre_smp (sxre_smp_what, sxtables_ptr)
    int				sxre_smp_what;
    struct reg_exp_tables	*sxtables_ptr;
{
    struct reg_exp_node	*son;
    int			old_counters_size, prev_ste;
    BOOLEAN             b_ret;
...
int
sxre_string_to_re (string, t_code, is_a_generic_terminal)
    char	*string;
    int		t_code;
    BOOLEAN	is_a_generic_terminal;
{
    ...
    /* La forme normale des noms d'ER est capitalisee, les noms en "bnf" sont
       inchanges. */
    int 			string_lgth, string_ste, ste, re_no, couple, old_size;
    char			*pc;
    struct reg_exp_to_attr	*preattr;
    struct item_to_attr		*pitattr;
    ...
    string_ste = sxword_2save (&SXDS->terminals, string, (unsigned long) string_lgth);
	...
	SXDS->token_no_to_terminal =
	    (int*) sxalloc (SXDS->token_no_to_terminal_size, sizeof (int));
	...
	if ((old_size = SXDS->token_no_to_terminal_size) <= t_code)
	{
	    ...
	    {
		int	*old = SXDS->token_no_to_terminal;

		SXDS->token_no_to_terminal =
		    (int*) sxalloc (SXDS->token_no_to_terminal_size, sizeof (int));

		memcpy (SXDS->token_no_to_terminal, old, sizeof (int) * old_size);
		SXDS->is_token_no_to_terminal_static = FALSE;
	    }
	    else
		SXDS->token_no_to_terminal =
		    (int*) sxrealloc (SXDS->token_no_to_terminal,
    ...
    if ((ste = sxword_2retrieve (&SXDS->reg_exp_names, SXDS->ws, (unsigned long) string_lgth + 2)) != ERROR_STE)
    {
	...
	*pc = sxtoupper (*pc)	/* passage en majuscules */ ;
    ...
    if ((ste = sxword_2retrieve (&SXDS->reg_exp_names, SXDS->ws + 1, (unsigned long) string_lgth)) != ERROR_STE)
    {
	preattr = SXDS->reg_exp_to_attr + ste;
	preattr->token_no = t_code;
	...
	ForeachConfig (pconfigs, pconfig)
	{
		...
		ste = sxword_2save (&SXDS->terminals,
				    ...
				    (unsigned long) (SXDS->dslv [pconfig->current.lvno].ts_lgth));
    ...
    re_no = sxword_2save (&SXDS->reg_exp_names, string, (unsigned long) string_lgth);
...
#define WRITE(p,l)	((bytes=(l))>0&&(write(file_descr, p, (size_t) bytes) == bytes))
  ...
  return (sxword_write (&(SXDS->sc_names), file_descr)
	  && WRITE (SXDS->sc_names_to_char_sets, sizeof (int) * (SXWORD_top (SXDS->sc_names)))

	  && WRITE (&SXDS->char_sets_top, sizeof (int))
	  && WRITE (&SXDS->char_sets_size, sizeof (int))
	  ...
	  && WRITE (&SXDS->current_reg_exp_or_abbrev_no, sizeof (int))
	  && WRITE (&SXDS->current_reduce_id, sizeof (int))
	  && WRITE (&SXDS->delta, sizeof (int))
	  && sxbm_write (file_descr, SXDS->context_sets, (int) SXWORD_top (SXDS->reg_exp_names))	  

	  && WRITE (&SXDS->item_size, sizeof (int))
	  && WRITE (&SXDS->item_top, sizeof (int))
	  ...
	  && WRITE (&SXDS->token_no_to_terminal_size, sizeof (int))
	  && (SXDS->token_no_to_terminal_size > 0
	      ? WRITE (SXDS->token_no_to_terminal, sizeof (int) * SXDS->token_no_to_terminal_size)
	      : TRUE)

	  && WRITE (&SXDS->counters_size, sizeof (int))
	  && WRITE (&SXDS->eof_code, sizeof (int))
	  && WRITE (&SXDS->eof_reg_exp_name, sizeof (int))
...
#define READ(p,l)	((bytes=(l))>0&&(read (file_descr, p, (size_t)bytes) == bytes)) 
  ...
  BOOLEAN b_ret = (sxword_read (&(SXDS->sc_names), file_descr, "simple_class_names",
		   ...
		   && READ (SXDS->sc_names_to_char_sets =
			    (int*) sxalloc (SXWORD_size (SXDS->sc_names), sizeof (int)),
			    sizeof (int) * (SXWORD_top (SXDS->sc_names)))

		   && READ (&SXDS->char_sets_top, sizeof (int))
		   && READ (&SXDS->char_sets_size, sizeof (int))
		   ...
		   && READ (&SXDS->current_reg_exp_or_abbrev_no, sizeof (int))
		   && READ (&SXDS->current_reduce_id, sizeof (int))
		   && READ (&SXDS->delta, sizeof (int))
		   && sxbm_read (file_descr,
				 (SXBA*)(SXDS->context_sets = sxbm_calloc ((int)SXWORD_size (SXDS->reg_exp_names),
									   (int)SXWORD_size (SXDS->reg_exp_names))),
				 (int)SXWORD_top (SXDS->reg_exp_names))	  
		   ...
		   && READ (&SXDS->item_size, sizeof (int))
		   && READ (&SXDS->item_top, sizeof (int))
		   ...
		   && READ (&SXDS->token_no_to_terminal_size, sizeof (int)) );
  if ( b_ret ) {
    if ( SXDS->token_no_to_terminal_size > 0 )
      b_ret = READ (SXDS->token_no_to_terminal =
		    (int*) sxalloc (SXDS->token_no_to_terminal_size, sizeof (int)),
		    sizeof (int) * SXDS->token_no_to_terminal_size) ;
    else {
      SXDS->token_no_to_terminal =NULL;
    }
  }
  if ( b_ret ) {
    SXDS->ws_lgth = 0;
    SXDS->ws = NULL;
    b_ret = READ (&SXDS->counters_size, sizeof (int))
         && READ (&SXDS->eof_code, sizeof (int))
         && READ (&SXDS->eof_reg_exp_name, sizeof (int))
...
static void
re_out_tab_int (file, name1, name2, tab, size, top)
    FILE	*file;
    char	*name1, *name2;
    int		*tab, size, top;
{
...
static void
re_out_tab_struct (file, struct_name, name1, name2, print, size, top)
    FILE	*file;
    char	*struct_name, *name1, *name2;
    void	(*print) ();
    int		size, top;
{
    int i;

    fprintf (file, "\n\nstatic %s %s%s [%i] = {", struct_name, name1, name2, size + 1);
...
static void
re_print_abbrev_to_attr (file, re_print_abbrev_to_attr_index)
    FILE	*file;
    int		re_print_abbrev_to_attr_index;
{
    struct abbrev_to_attr	*p = SXDS->abbrev_to_attr + re_print_abbrev_to_attr_index;

    fprintf (file, "{%i, %i, %i}, ", p->init_item, p->final_item, p->is_empty);
}


static void
re_print_reg_exp_to_attr (file, re_print_reg_exp_to_attr_index)
    FILE	*file;
    int		re_print_reg_exp_to_attr_index;
{
    struct reg_exp_to_attr	*p = SXDS->reg_exp_to_attr + re_print_reg_exp_to_attr_index;

    fprintf (file, "{%i, %i, %i, %i, %i, %i, %i}, ", p->init_item, p->post_action, p->token_no, p->reduce_id, p->context_kind, p->priority_kind, p->kw_kind);
}

static void
re_print_item_to_attr (file, re_print_item_to_attr_index)
    FILE	*file;
    int		re_print_item_to_attr_index;
{
    struct item_to_attr	*p = SXDS->item_to_attr + re_print_item_to_attr_index;

    fprintf (file, "{%i, %i, %i, %i, %i}, ", p->reg_exp_no, p->val, p->param, p->is_erased, p->kind);
}

static void
re_print_terminal_to_attr (file, re_print_terminal_to_attr_index)
    FILE	*file;
    int		re_print_terminal_to_attr_index;
{
    struct terminal_to_attr	*p = SXDS->terminal_to_attr + re_print_terminal_to_attr_index;

    fprintf (file, "{%i, %i, %i, %i}, ", p->token_no, p->next_syno, p->is_kw, p->is_generic);
}
...
static unsigned int sc_names_total_lgth, abbrev_names_total_lgth, reg_exp_names_total_lgth, terminals_total_lgth;
    ...
    char	*name;
{
    ...
    re_out_tab_struct (file, "struct reg_exp_to_attr", name, "_reg_exp_to_attr",
		    ...
		    SXWORD_top (SXDS->reg_exp_names));
    sxbm_to_c (file, SXDS->context_sets, (int) SXWORD_size (SXDS->reg_exp_names),
    ...
    XxY_array_to_c (&SXDS->nfsa, file, re_cat ("_nfsa"), TRUE);
    ...
    char	*name;
{
    ...
    fprintf (file, ",\n");
    fprintf (file, "%s_sc_names_to_char_sets /* int *sc_names_to_char_sets */,\n", name);
    fprintf (file, "%s_char_sets /* SXBA *char_sets */,\n", name);
    fprintf (file, "%i /* int char_sets_top */,\n", SXDS->char_sets_top);
    fprintf (file, "%i /* int char_sets_size */,\n", SXDS->char_sets_size);
    ...
    fprintf (file, "%i /* int current_reg_exp_or_abbrev_no */,\n",
	     SXDS->current_reg_exp_or_abbrev_no);
    fprintf (file, "%i /* int current_reduce_id */,\n", SXDS->current_reduce_id);
    fprintf (file, "%i /* int delta */,\n", SXDS->delta);
    fprintf (file, "%s_context_sets /* SXBA *context_sets */,\n", name);
    fprintf (file, "NULL /* SXBA *tfirst */,\n");
    fprintf (file, "0 /* int tfirst_line_nb */,\n");
    fprintf (file, "0 /* int tfirst_col_nb */,\n");
    fprintf (file, "%s_item_to_attr /* struct item_to_attr *item_to_attr */,\n", name);
    fprintf (file, "%i /* int item_size */,\n", SXDS->item_size);
    fprintf (file, "%i /* int item_top */,\n", SXDS->item_top);
    XxY_header_to_c (&SXDS->nfsa, file, re_cat ("_nfsa"), TRUE);
    fprintf (file, ",\n");
    ...
    if (SXDS->token_no_to_terminal_size == 0)
	fprintf (file, "NULL /* int *token_no_to_terminal */,\n");
    else
	fprintf (file, "%s_token_no_to_terminal /* int *token_no_to_terminal */,\n", name);

    fprintf (file, "%i /* int token_no_to_terminal_size */,\n", SXDS->token_no_to_terminal_size);
    fprintf (file, "NULL /* char *ws */,\n");
    fprintf (file, "0 /* int ws_lgth */,\n");
    fprintf (file, "%s_context_sets_line_0 /* SXBA ctxt_set */,\n", name);
    fprintf (file, "%s_context_sets_line_1 /* SXBA filter */,\n", name);
    fprintf (file, "%i /* int counters_size */,\n", SXDS->counters_size);
    fprintf (file, "%i /* int eof_code */,\n", SXDS->eof_code);
    fprintf (file, "%i /* int eof_reg_exp_name */,\n", SXDS->eof_reg_exp_name);    
