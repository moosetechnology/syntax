...
/************************************************************************/
/* 20041012 14:48 (phd):	Un petit "cast" pour Solaris		*/
/************************************************************************/
/* 24-08-04 13:45 (pb):		Ajout de cette rubrique "modifications"	*/
/************************************************************************/
...
char WHAT_SXSPELL_MNGR[] = "@(#)SYNTAX - $Id: sxspell_mngr.c 527 2007-05-09 14:02:47Z rlacroix $" WHAT_DEBUG;

static struct FSA          *cur_FSA;
static struct spell_result *cur_spell_result;
static struct dico         *cur_dico;

/* Structures de travail */
static int                 *spell_result_id_stack;
static XH_header           cur_multiple_word_ids;
static struct weight       *spell_result_weight_stack;
static int                 cross_id_size, *cross_id;
static struct weight       *cross_weight;
static struct weight       *ij_weights; 
static int                 sorted_size, *sorted, *ij_total_weights, *ij_ids;
static int                 cur_spell_result_stack_bot;
...
static int                 *fsa_state2local_path, local_path_nb;
static int                 *fsa_state2lequiv, *fsa_state2requiv, fsa_state2equiv_size;
...
static int                 underflow_nb;

static BOOLEAN             FSA_HAS_EPS_TRANS /* ssi sxspell_suppress_all a ete appele */;
static int                 MEMO_BASE /* Nb d'etats initiaux+1 */;
static BOOLEAN             WORD_HAS_SPACES /* Le mot a corrige a-t-il des espaces */;

static int                 *memo_hd;
static int                 *memo_id_stack, *memo_weight_limit;
static struct weight       *memo_weight_stack;
static X_header            key_hd;
static int                 *key2index;
static int                 KEY_SHIFT;
#define MAKE_KEY(n,p,q)    (((((n)<<KEY_SHIFT)+(p))<<KEY_SHIFT)+(q))

static void
key_hd_oflw (old_size, new_size)
     int old_size, new_size;
{
  key2index = (int*) sxrealloc (key2index, new_size+1, sizeof (int));
}

/* Petit package de gestion des char sets et des char weights */
/* Taille d'un SXBA pour stocker 256 bits (en nb de SXBA_ELT) */
#define CHAR_SET_SIZE        (NBLONGS (256)+1)
...
static unsigned int	   base, tooth, is_suffix, suffix_mask, base_shift, class_mask, stop_mask, *comb_vector;
...
static void
words_oflw (old_size, new_size)
     int old_size, new_size;
{
    cur_spell_result->codes = (int *) sxrealloc (cur_spell_result->codes, new_size+1, sizeof (int));
}

void
sxspell_init (dico, FSA, kind, max_weight, cmp_weight, seek_sub_words, sub_word_weight)
     struct dico         *dico;
     struct FSA          *FSA;
     unsigned char       kind;
     int                 max_weight, cmp_weight, sub_word_weight;
     BOOLEAN             seek_sub_words;
{

  FSA->dico = dico;

  if (dico->partiel)
    sxtrap (ME, "Rebuild a not \"partial\" dictionary");

  FSA->spell_result.kind = (int)kind; /* ALL, BEST ou NBEST */
  ...
  FSA->spell_result.seek_sub_words = (int)seek_sub_words;
     ...
     struct FSA          *FSA;
{
  ...
  if (sorted) {
    ...
    sxfree (ij_ids), ij_ids;
  }
}

/* On represente un mot sous forme de FSA */
void
sxspell_init_fsa (mot, lgth, FSA)
     unsigned char *mot;
     int           lgth;
     struct FSA    *FSA;
{
  ...
  if (FSA->orig_char_set == NULL) {
    ...
    XxY_alloc (&(FSA->Q_hd), mot, 10*lgth+1, 1, 1 /* Xforeach */, 1 /* Yforeach */, area_oflw, NULL);
    ...
    if (BASIZE (FSA->final_set) <= (unsigned)FSA->Q_nb)
      FSA->final_set = sxba_resize (FSA->final_set, FSA->Q_nb+1);

    XxY_clear (&(FSA->Q_hd));
    FSA->Q_hd.name = (char *) mot;  
  ...
  if (fsa_state2lequiv == NULL) {
    ...
    fsa_state2lequiv = (int*) sxalloc (fsa_state2equiv_size+1, sizeof (int));
    fsa_state2requiv = (int*) sxalloc (fsa_state2equiv_size+1, sizeof (int));
    fsa_state2local_path = (int*) sxalloc (fsa_state2equiv_size+1, sizeof (int));
  }
  else {
    if (fsa_state2equiv_size < 5*lgth) {
      fsa_state2equiv_size = 5*lgth;
      fsa_state2lequiv = (int*) sxrealloc (fsa_state2lequiv, fsa_state2equiv_size+1, sizeof (int));
      fsa_state2requiv = (int*) sxrealloc (fsa_state2requiv, fsa_state2equiv_size+1, sizeof (int));
      fsa_state2local_path = (int*) sxrealloc (fsa_state2local_path, fsa_state2equiv_size+1, sizeof (int));
    }
  }
...
#define unicode ((int) (unsigned char) '\201')
...
get_new_compose_weight (old_weight, oper_weight, oper_nb)
     struct weight   old_weight, oper_weight;
     int             oper_nb;
{
...
static struct weight
get_new_min_weight (old_weight, new_weight)
     struct weight old_weight, new_weight;
{
...
void
sxspell_no_repeat (FSA, p, nr_weight)
     struct FSA    *FSA;
     int           p, nr_weight;
{
  int           pair, pair2, car, n, couple;
  struct weight cur_weight, weight, *new_char_weight;
  struct trans  trans, new_trans;
  SXBA          new_char_set; 
  BOOLEAN       is_an_old_pair;
  ...
  for (pair = 1; pair <= FSA->Q_init-p; pair = pair2) {
    car = (int) FSA->word [pair-1];

    n = 1;

    for (pair2 = pair+1; pair2 < FSA->Q_init; pair2++) {
      if (car != (int) FSA->word [pair2-1])
	break;
	...
	sxba_empty (new_char_set);
      ...
      weight = get_new_compose_weight (trans.char_weight [car], cur_weight, 1);
...
*/
void
sxspell_replace (FSA, R, R_size, replace_weight)
     struct FSA    *FSA;
     unsigned char *R [];
     int           R_size;
     int           replace_weight;
...
void
sxspell_replace_n_p (FSA, R, R_size, replace_weight)
     struct FSA    *FSA;
     unsigned char *R [];
     int           R_size;
     int           replace_weight;
{
  int           n, p, i, j, top_j, k, car, pair, new_j, prefix_lgth, suffix_lgth, min_n_p, true_n;
  SXBA          new_char_set, char_set;
  unsigned char true_car, true_rcar, *line, *left_line, *right_line, *replace_line, *mot, *r_line;
  BOOLEAN       is_an_old_pair, is_first, is_last, is_ps_done;
  ...
  for (i = 0; i < R_size; i += 2) {
	...
	if (car != unicode) {
      ...
      if (n > 0) {
	while ((int) (true_car = *line) == unicode)
	  line++;
	...
	for (;;) {
	  if ((!is_first && !is_last) || (is_first && j == 0) || (is_last && j == top_j)) {
		...
		while ((int)(true_car = *++left_line) == unicode);
		...
		if (!is_ps_done) {
		  ...
		  if (p > 0) {
		    ...
		    do {
		      while ((int)(true_car = *left_line++) == unicode);
		      while ((int)(true_rcar = *r_line++) == unicode);
	  ...
	  */
	  if (is_first || is_last) {
		...
		local_path_nb++;
	      ...
	      while ((car = (int) *replace_line++) == unicode);
...
/* Attention, les remplacements elementaires deja effectues sont pris en compte (l'ordre des appels
   ...
   Ex si un 'e' a deja ete remplace' par 'é' et si le mot est "te", la suppression du 't' conduira aux mots "e" et "é" */

/* Tous les caracteres du mot peuvent etre supprimes */
/* on met des transitions epsilon (ca peut etre tres cher a l'execution !!) */
void
sxspell_suppress_all (FSA, sup_weight)
     struct FSA    *FSA;
     int           sup_weight;
{
  int           pair, top_pair;
...
void
sxspell_suppress_i (FSA, sup_weight, i)
     struct FSA    *FSA;
     int           sup_weight;
     int           i;
{
  int           pair, pair1, car, top_state, n, p;
  ...
  for (pair = 1; pair <= top_state; pair++) {
    ...
    while ((car = sxba_scan (char_set, car)) >= 0) {
      weight = get_new_compose_weight (char_weight [car], cur_weight, 1);
	...
	weight = get_new_compose_weight (char_weight [car], cur_weight, 2);
...
void
sxspell_swap (FSA, swp_weight)
     struct FSA    *FSA;
     int           swp_weight;
{
  int           p, pair, car, top_state;
  struct weight cur_weight, *new_char_weight, *char_weight, weight;
  SXBA          char_set, new_char_set;
  struct trans  trans, trans1;
  ...
  for (p = 1; p <= top_state; p++) {
    ...
    while ((car = sxba_scan (char_set, car)) >= 0) {
      weight = new_char_weight [car] = get_new_compose_weight (char_weight [car], cur_weight, 1);
    }
...
static void
store_sub_word (id, weight, is_an_old_id)
     int           id;
     struct weight weight;
     BOOLEAN       is_an_old_id;
{
  int           new_total_weight, old_total_weight, prev_total_weight, cur_total_weight, total_weighti;
  int           id_nb, weight_nb, nb, i, j, insert_point, store_id;
  int           *id_stack;
  struct weight *weight_stack, store_weight;
  BOOLEAN       is_single, unique_insert;
  ...
  if (cur_spell_result->kind) {
    /* NBEST */
    weight_nb = ((unsigned int)id_nb) >> WEIGHT_NB_SHIFT;
    id_nb &= ID_NB_AND;
  }
  ...
  if (is_an_old_id) {
	...
	is_single = !(((i > 1) && (get_total_weight (weight_stack [i-1]) == total_weighti))
		      || ((i < id_nb) && (get_total_weight (weight_stack [i+1]) == total_weighti)));
  ...
  if (cur_spell_result->kind && weight_nb == cur_spell_result->kind && unique_insert) {
    ...
    do {
      id_nb--;
      nb++;
    } while (get_total_weight (weight_stack [id_nb]) == old_total_weight);
...
void store_word (int code)
{
  int  id, rsize;
  unsigned int top;
  int  cur_total_weight;
  ...
  id = sxword_2save (&(cur_spell_result->words), varstr_tostr (lvstr), (unsigned long)varstr_length (lvstr));
  ...
  store_sub_word (id, total_weight, SXWORD_top (cur_spell_result->words) <= top);
}


static struct weight
get_weight (weight1, weight2)
     struct weight weight1, weight2;
{
    struct weight weight;
     ...
     int p, q, code;
{
  ...
  if (is_suffix) {
	...
	if (get_total_weight (total_weight) <= cur_spell_result->weight_limit)
...
{
  int           cur_char, pair, r;
  unsigned int  new_tooth;
  struct weight *char_weight;
  SXBA          char_set;
  struct trans  trans;
  unsigned char source_char;
  BOOLEAN       p_or_q_in_word;
  ...
  if (FSA_walk_done (p, q)) {
    ...
    if ((new_tooth & class_mask) == 0 && (new_tooth & stop_mask)) {
      ...
      store_word (((int) new_tooth) >> base_shift);
    }
...
static void
dico_walk (cur_char, cur_weight, p, q)
     ...
     int           p, q;
{
  struct weight old_total_weight;
  int           cur_total_weight;
  unsigned int	old_tooth, old_is_suffix, new_tooth, old_base;
  unsigned char	class;
  BOOLEAN       FSA_walk_complete, stop_mask_hit;
  ...
  if ((class = char2class [cur_char])) {
    new_tooth = comb_vector [base+class];

    if ((new_tooth & class_mask) == class) {
      old_total_weight = total_weight;
      total_weight = get_weight (total_weight, cur_weight);
      cur_total_weight = get_total_weight (total_weight);
      ...
      if (cur_total_weight <= cur_spell_result->weight_limit) {
	if (is_suffix)
	  varstr_catchar (rvstr, cur_char);
	else
	  varstr_catchar (lvstr, cur_char);
...
merge_spell_result_stack (top, x, y)
    int top, x, y;
{
  int           top_size, x_size, y_size, size, i, top_nb, nb, greatest_weight; 
  int           ij_total_weight, top_total_weight, min_total_weight, x_id, y_id, xy_top, cur_bot, cur_top, cur_id; 
  int           *int_ptr;
  struct weight *x_weights, *y_weights, *top_weights, ij_weight, weighti, weightj, top_weight;
  BOOLEAN       done, from_top;
  int           sorted_top, sorted_nb, merge_spell_result_stack_base, j, ij, top_id, cross_size, old_size, new_size, xij, min_cross_weight;
  int           *x_ids, *y_ids, *top_ids;
  ...
  if (x_size == 0 || y_size == 0 ||
      (min_cross_weight = get_total_weight (get_weight (get_weight (spell_result_weight_stack [x+1],
								    spell_result_weight_stack [y+1]),
							cur_spell_result->sub_word_weight))) > cur_spell_result->weight_limit) {
    /* Pas de croisement */
  ...
  if (top_size > 0 && cur_spell_result->kind > 0 && (((unsigned int)spell_result_id_stack [top]) >> WEIGHT_NB_SHIFT) == (unsigned int)(cur_spell_result->kind)) {
    /* Y'a deja des resultats pour top ... */
    /* ... et BEST ou NBEST ...*/
    /* ... et plein */
    if (get_total_weight (spell_result_weight_stack [top+top_size]) < min_cross_weight) {
  ...
  if (cross_id == NULL) {
    cross_id = (int*) sxalloc ((cross_id_size = 128+size)+1, sizeof (int));
    cross_weight = (struct weight*) sxalloc (cross_id_size+1, sizeof (struct weight));
  }
  else {
    if (size > cross_id_size) {
      cross_id = (int*) sxrealloc (cross_id, (cross_id_size += size)+1, sizeof (int));
  ...
  if (sorted == NULL) {
    ...
    sorted = (int*) sxalloc (sorted_size, sizeof (int));
    ij_weights = (struct weight*) sxalloc (sorted_size, sizeof (struct weight));
    ij_total_weights = (int*) sxalloc (sorted_size, sizeof (int));
    ij_ids = (int*) sxalloc (sorted_size, sizeof (int));
  }
  else {
    if ((i = x_size*y_size) > sorted_size) {
      sorted_size = i;
      sorted = (int*) sxrealloc (sorted, sorted_size, sizeof (int));
      ij_weights = (struct weight*) sxrealloc (ij_weights, sorted_size, sizeof (struct weight));
      ij_total_weights = (int*) sxrealloc (ij_total_weights, sorted_size, sizeof (int));
      ij_ids = (int*) sxrealloc (ij_ids, sorted_size, sizeof (int));
    }
  }
  ...
  for (i = 0; i < x_size; i++) {
	...
	break;
      ...
      ij_total_weights [sorted_top] = ij_total_weight;
  ...
  if (sorted_top > 1)
    qsort (sorted, (size_t)sorted_top, sizeof (int), (int (*) (const void *, const void *)) merge_compare);
  ...
  while (!done) {
	...
	if (from_top) {
	  ...
	  if (x_id > 0) {
	    XH_push (cur_multiple_word_ids, x_id);
	  }
	  else {
	    cur_top = XH_X (cur_multiple_word_ids, -x_id+1);
	    cur_bot = XH_X (cur_multiple_word_ids, -x_id);

	    while (cur_bot < cur_top) {
	      cur_id = XH_list_elem (cur_multiple_word_ids, cur_bot++);
	      XH_push (cur_multiple_word_ids, cur_id);
	    }
	  }

	  if (y_id > 0) {
	    XH_push (cur_multiple_word_ids, y_id);
	  }
	  else {
	    cur_top = XH_X (cur_multiple_word_ids, -y_id+1);
	    cur_bot = XH_X (cur_multiple_word_ids, -y_id);
	  ...
	  if (XH_set (&cur_multiple_word_ids, &xy_top)) {
     ...
     int x;
{
  ...
  weight_nb = ((unsigned int)memo_id_stack [x]) >> WEIGHT_NB_SHIFT;
...
fill_spell_result_stacks (top, x)
     int top, x;
{
  int           i, nb, old_size, new_size, weight_nb, top_total_weight, prev_top_total_weight;
  int           *mid_stack, *id_stack;
  ...
  weight_nb = ((unsigned int)mid_stack [0]) >> WEIGHT_NB_SHIFT; /* Si cur_spell_result->kind==1 => == 0 */
...
static int
fill_memo_stacks (top)
     int top;
{
  int           i, x, nb, old_size, new_size;
  int           *mid_stack, *id_stack;
  ...
  if ((new_size = DSIZE (memo_id_stack)) > old_size) {
    memo_weight_limit = (int*) sxrealloc (memo_weight_limit, new_size+1, sizeof (int));
...
static int
memo_cut_in_subwords (p, q, cut_nb)
     int p, q, cut_nb;
{
  int           old_size, new_size, top, i, j, x, y, k, key_id, costly_cut_nb, space_nb;
  int           local_old_weight_limit, x_cut_total_weight, total_cut_weight;
  BOOLEAN       is_a_subword;
  ...
  if (cut_nb == 0) {
	...
	/* On essaie en premier un decoupage sur les blancs */
	i = p+1;

	while (cur_FSA->word [i-1] != ' ')
	  i++;

	x = memo_cut_in_subwords (p, i, 0);
	...
	cur_spell_result->weight_limit = local_old_weight_limit - total_cut_weight;
	x = memo_cut_in_subwords (p, i, 0);
	...
	if (x) {
	  ...
	  if (WORD_HAS_SPACES && (cur_FSA->word [i-1] == ' ')) {
		...
		if (costly_cut_nb > 0)
	      ...
	      if (total_cut_weight <= local_old_weight_limit) {
		/* ... on a suffisamment de provisions pour tenter ce decoupage ... */
		/* ... au cout max */
		cur_spell_result->weight_limit = local_old_weight_limit - total_cut_weight;
...
call_cut ()
{
  int           max_cut_nb, cut_nb, space_nb, costly_cut_nb;
  ...
  for (cut_nb = 1; cut_nb <= max_cut_nb; cut_nb++) {
	...
	if (get_total_weight (get_new_compose_weight (NUL_WEIGHT, cur_spell_result->sub_word_weight, costly_cut_nb)) > cur_spell_result->weight_limit)
...
sxspell_do_it (FSA)
     struct FSA          *FSA;
{
  int           x, cur_total_weight, prev_total_weight, nb, store_nbest, nmemb;
  int           *id_stack;
  ...
  if (FSA->spell_result.words.table == NULL) {
    sxword_alloc (&(FSA->spell_result.words), "FSA->spell_result.words", 63, 1, 16,
		  sxcont_malloc, sxcont_alloc, sxcont_realloc, NULL, sxcont_free, words_oflw, NULL);
    FSA->spell_result.codes = (int *) sxalloc (SXWORD_size (FSA->spell_result.words)+1, sizeof (int));
    XH_alloc (&(FSA->spell_result.multiple_word_ids), "multiple_word_ids", 256, 1, 4, NULL, NULL);
  }
  else {
  ...
  cur_multiple_word_ids = cur_spell_result->multiple_word_ids;
  ...
  if (cur_spell_result->seek_sub_words) {
    ...
    WORD_HAS_SPACES = SXBA_bit_is_set (FSA->orig_char_set, ((int)' '));

    if (memo_hd == NULL) {
      memo_hd = (int*) sxcalloc ((unsigned long int)size, sizeof (int));
      memo_hd [0] = size;

      DALLOC_STACK (memo_id_stack, ((FSA->spell_result.kind == 0) ? 5 : FSA->spell_result.kind)*size);
      memo_weight_limit = (int*) sxalloc (DSIZE (memo_id_stack)+1, sizeof (int));
      ...
      key2index = (int*) sxalloc (X_size (key_hd)+1, sizeof (int));
    }
    else {
      int_ptr = memo_hd;
      top = memo_hd [0];
	...
	*++int_ptr = 0;
      ...
      if (size > memo_hd [0]) {
	memo_hd = (int*) sxrecalloc (memo_hd, memo_hd [0], size, sizeof (int));
	memo_hd [0] = size;
      }
...
void
sxspell_model (kind, max_weight, is_sub_word, sub_word_weight, mot, dico)
     unsigned char kind;
     int           max_weight;
     BOOLEAN       is_sub_word;
     int           sub_word_weight;
     ...
     struct dico   *dico;
{
  ...
  sxspell_add (&main_FSA, strlen (mot)+1, "cdefhpstxz", add_weight);
  /* et une hache muette au début... */
  ...
  /* remplacements */
  // ATTENTION: Commenté par BS à des fins d'efficacité (05/05)
  //  for (x = strlen (mot); x >= 1; x--)
