...
/************************************************************************/
/* Ver 06 sep 2006 14:30:	Ajout de cette rubrique "modifications"	*/
/************************************************************************/


#include "sxunix.h"

char WHAT_CYKPARSER[] = "@(#)SYNTAX - $Id: CYK_parser.c 535 2007-05-10 13:35:37Z rlacroix $" WHAT_DEBUG;
...
static int          *working_nt_stack;
static SXBA         *CYK_ij2t_set, *CYK_Tij;
static int          **CYK_ij, *CYK_ij_area;


#if is_parser
/* Structures et variables utilisees par l'analyseur (pas le reconnaisseur) CYK */
static int          CYK_item_hd_foreach [6] =  {0, 0, 0, 1, 0, 0} /* XYforeach */,
                    CYK_reduce_item_hd_foreach [6] =  {0, 0, 0, 0, 1, 0} /* XZforeach */;
static int          spf_prod_nb;
static int          *CYK_npd, *nt2reduce_item_set_id, *CYK_numpd;
static int          *CYK_rhs_stack; /* DSTACK */
static XxYxZ_header CYK_item_hd;
static XxYxZ_header CYK_reduce_item_hd;


#if is_epsilon
static void spf_empty_nt_in_spf (int A, int i);

static void
put_empty_prod_in_spf (int prod, int i)
{
  int     bot, top, lgth, Aii, X, Xii;
  BOOLEAN is_new_Xii;
...
static void
spf_empty_nt_in_spf (int A, int i)
{
  int bot, top, prod, item;
...
static void
spf_td_extraction (int i, int j, int item)
{
  int     ij, reduce_item, X, prod, triple, triple2, k, kj, Aij; 
  BOOLEAN is_new_Aij;
...
#if LOG
static void
print_xitem (int item, int i, int j, int k, char *prefix, char *suffix)
{
  int top_item, bot_item, X, prod;
...
static void
print_item (int item, char *prefix, char *suffix)
{
  int top_item, bot_item, X, prod;
...
{
  int     item, next_item, t, it, next_it, X;
  int     ret_val;
#if is_parser
  int     CYK_item, dummy, ij;
#endif /* is_parser */
...
{
  int     item, next_item, prod, B, old_B, it, next_it, X;
  int     ret_val;
  SXBA    local_working_item_set;
#if is_parser
  int     CYK_item, dummy, x, id, reduce_item, CYK_reduce_item, ij;
#endif /* is_parser */
#if LLOG
  int     bot, top;
  BOOLEAN is_new;
...
{
#if is_dag
  int mlstn, dag_state, trans, Y, next_mlstn, t;
...
#if is_lex
extern BOOLEAN lexicalizer2basic_item_set (BOOLEAN is_mlstn2lex_la_tset, BOOLEAN is_mlstn2la_tset);
...
sxearley_parse_it ()
{
  int     size, t_size, i, j, k, ii, jj, nn, ij, ik, kj, max_span, max_i, span, CYK_ij_t_top, nt, CYK_ij_top, val;
  SXBA    Tij, Tik, Tkj, T11, T22, Tnn, Tii, Tjj, swap, new_reduce_Tij_1, new_reduce_Tij_2;
  BOOLEAN is_empty, is_in_L, R1, R2;
...
#  endif /* !is_lex */
...
#if is_parser
      CYK_npd = (int*) sxalloc (MAXNT+1, sizeof (int));
      CYK_numpd = (int*) sxalloc (MAXITEM+1, sizeof (int));
      nt2reduce_item_set_id = (int*) sxalloc (MAXNT+1, sizeof (int));
	...
	CYK_npd [nt] = NPD (nt+1)-1;
      ...
      {
	int *bot_area_ptr, **bot_ptr, **top_ptr;

	bot_area_ptr = CYK_ij_area = (int*) sxcalloc (size, sizeof (int));
	bot_ptr = CYK_ij = (int**) sxalloc (final_mlstn+1, sizeof (int*));
