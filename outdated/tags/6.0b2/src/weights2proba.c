...
/************************************************************************/
/* Mar 17 2006 15:10:		Ajout de cette rubrique "modifications"	*/
/************************************************************************/
...
char WHAT_WEIGHTS2PROBA[] = "@(#)SYNTAX - $Id: weights2proba.c 527 2007-05-09 14:02:47Z rlacroix $" WHAT_DEBUG;
...
static int max_signature_depth;/* defaut 0 => unbounded */
...
static int          *eprod_father2nb, *eprod_son2nb, *full_ctxtXeprod2nb, *eprod_ctxtXeprod2nb, *prod_ctxtXeprod2nb, *A_ctxtXeprod2nb;
static int          *prod2nb, *A2nb, *B2nb, *prod_father2nb, *prod_son2nb, *full_ctxt2nb;
static int          *ff_id2nb, *full_ctxtXlex_id2nb, *eprod_ctxtXlex_id2nb, *prod_ctxtXlex_id2nb, *A_ctxtXlex_id2nb, *lex_id2nb;
static int          *B2sign_nb, *ctxtXsign2nb;

static void
prodXval2eprod_oflw (old_size, new_size)
     int old_size, new_size;
{
  eprod_father2nb = (int*) sxrecalloc (eprod_father2nb, old_size+1, new_size+1, sizeof (int));
  eprod_son2nb = (int*) sxrecalloc (eprod_son2nb, old_size+1, new_size+1, sizeof (int));
}
...
full_ctxt_oflw (old_size, new_size)
     int old_size, new_size;
{
  full_ctxt2nb = (int*) sxrecalloc (full_ctxt2nb, old_size+1, new_size+1, sizeof (int));
}
...
full_ctxtXeprod_oflw (old_size, new_size)
     int old_size, new_size;
{
  full_ctxtXeprod2nb = (int*) sxrecalloc (full_ctxtXeprod2nb, old_size+1, new_size+1, sizeof (int));
}
...
eprod_ctxtXeprod_oflw (old_size, new_size)
     int old_size, new_size;
{
  eprod_ctxtXeprod2nb = (int*) sxrecalloc (eprod_ctxtXeprod2nb, old_size+1, new_size+1, sizeof (int));
}
...
prod_ctxtXeprod_oflw (old_size, new_size)
     int old_size, new_size;
{
  prod_ctxtXeprod2nb = (int*) sxrecalloc (prod_ctxtXeprod2nb, old_size+1, new_size+1, sizeof (int));
}
...
A_ctxtXeprod_oflw (old_size, new_size)
     int old_size, new_size;
{
  A_ctxtXeprod2nb = (int*) sxrecalloc (A_ctxtXeprod2nb, old_size+1, new_size+1, sizeof (int));
}
...
ff_id_oflw (old_size, new_size)
     int old_size, new_size;
{
  ff_id2nb = (int*) sxrecalloc (ff_id2nb, old_size+1, new_size+1, sizeof (int));
}
...
full_ctxtXlex_id_oflw (old_size, new_size)
     int old_size, new_size;
{
  full_ctxtXlex_id2nb = (int*) sxrecalloc (full_ctxtXlex_id2nb, old_size+1, new_size+1, sizeof (int));
}
...
eprod_ctxtXlex_id_oflw (old_size, new_size)
     int old_size, new_size;
{
  eprod_ctxtXlex_id2nb = (int*) sxrecalloc (eprod_ctxtXlex_id2nb, old_size+1, new_size+1, sizeof (int));
}
...
prod_ctxtXlex_id_oflw (old_size, new_size)
     int old_size, new_size;
{
  prod_ctxtXlex_id2nb = (int*) sxrecalloc (prod_ctxtXlex_id2nb, old_size+1, new_size+1, sizeof (int));
}
...
A_ctxtXlex_id_oflw (old_size, new_size)
     int old_size, new_size;
{
  A_ctxtXlex_id2nb = (int*) sxrecalloc (A_ctxtXlex_id2nb, old_size+1, new_size+1, sizeof (int));
}
...
ff_idXt2lex_id_oflw (old_size, new_size)
     int old_size, new_size;
{
  lex_id2nb = (int*) sxrecalloc (lex_id2nb, old_size+1, new_size+1, sizeof (int));
}
...
ctxtXsign_oflw (old_size, new_size)
     int old_size, new_size;
{
  ctxtXsign2nb =  (int*) sxrecalloc (ctxtXsign2nb, old_size+1, new_size+1, sizeof (int));
}
...
process_weights ()
{
  int          A, B, nb, total_nb, sentence_nb, Bnb, prod, val, single, couple, lex_id, top, eprod_father, eprod_son, ff_id, t;
  int          prod_father, val_father, prod_son, val_son, pos, full_ctxt, fs_signature_size, max_fs_signature_size, full_couple, x, elem, signature;
  ...
  int          empty_foreach [6] = {0, 0, 0, 0, 0, 0};
  int          LINE_LGTH = 256;
  int          cur_level_nb, prev_level_nb, depth;
  ...
  */
  ...
  if (fgets (number_str, LINE_LGTH, stdin)) {
    ...
    eprod_father2nb = (int *) sxcalloc (XxY_size (prodXval2eprod_hd)+1, sizeof (int));
    eprod_son2nb = (int *) sxcalloc (XxY_size (prodXval2eprod_hd)+1, sizeof (int));
    A2nb = (int*) sxcalloc (ntmax+1, sizeof (int));
    B2nb = (int*) sxcalloc (ntmax+1, sizeof (int));
    B2sign_nb = (int*) sxcalloc (ntmax+1, sizeof (int));
    prod_father2nb = (int *) sxcalloc (prodmax+1, sizeof (int));
    prod_son2nb = (int *) sxcalloc (prodmax+1, sizeof (int));

    XxY_alloc (&full_ctxt_hd, "full_ctxt_hd", prodmax+ntmax+1, 1, 0, 0, full_ctxt_oflw, NULL);
    full_ctxt2nb = (int *) sxcalloc (XxY_size (full_ctxt_hd)+1, sizeof (int));

    XxY_alloc (&full_ctxtXeprod_hd, "full_ctxtXeprod_hd", prodmax+ntmax+1, 1, 0, 0, full_ctxtXeprod_oflw, NULL);
    full_ctxtXeprod2nb = (int*) sxcalloc (XxY_size (full_ctxtXeprod_hd)+1, sizeof (int));

    XxY_alloc (&eprod_ctxtXeprod_hd, "eprod_ctxtXeprod_hd", prodmax+ntmax+1, 1, 0, 0, eprod_ctxtXeprod_oflw, NULL);
    eprod_ctxtXeprod2nb = (int*) sxcalloc (XxY_size (eprod_ctxtXeprod_hd)+1, sizeof (int));

    XxY_alloc (&prod_ctxtXeprod_hd, "prod_ctxtXeprod_hd", prodmax+ntmax+1, 1, 0, 0, prod_ctxtXeprod_oflw, NULL);
    prod_ctxtXeprod2nb = (int*) sxcalloc (XxY_size (prod_ctxtXeprod_hd)+1, sizeof (int));

    XxY_alloc (&A_ctxtXeprod_hd, "A_ctxtXeprod_hd", prodmax+ntmax+1, 1, 0, 0, A_ctxtXeprod_oflw, NULL);
    A_ctxtXeprod2nb = (int*) sxcalloc (XxY_size (A_ctxtXeprod_hd)+1, sizeof (int));

    X_alloc (&ff_id_hd, "ff_id_hd", prodmax+ntmax+1, 1, ff_id_oflw, NULL);
    ff_id2nb = (int*) sxcalloc (X_size (ff_id_hd)+1, sizeof (int));

    XxY_alloc (&full_ctxtXlex_id_hd, "full_ctxtXlex_id_hd", prodmax+ntmax+1, 1, 0, 0, full_ctxtXlex_id_oflw, NULL);
    full_ctxtXlex_id2nb = (int*) sxcalloc (XxY_size (full_ctxtXlex_id_hd)+1, sizeof (int));

    XxY_alloc (&eprod_ctxtXlex_id_hd, "eprod_ctxtXlex_id_hd", prodmax+ntmax+1, 1, 0, 0, eprod_ctxtXlex_id_oflw, NULL);
    eprod_ctxtXlex_id2nb = (int*) sxcalloc (XxY_size (eprod_ctxtXlex_id_hd)+1, sizeof (int));

    XxY_alloc (&prod_ctxtXlex_id_hd, "prod_ctxtXlex_id_hd", prodmax+ntmax+1, 1, 0, 0, prod_ctxtXlex_id_oflw, NULL);
    prod_ctxtXlex_id2nb = (int*) sxcalloc (XxY_size (prod_ctxtXlex_id_hd)+1, sizeof (int));

    XxY_alloc (&A_ctxtXlex_id_hd, "A_ctxtXlex_id_hd", prodmax+ntmax+1, 1, 0, 0, A_ctxtXlex_id_oflw, NULL);
    A_ctxtXlex_id2nb = (int*) sxcalloc (XxY_size (A_ctxtXlex_id_hd)+1, sizeof (int));

    XxY_alloc (&ff_idXt2lex_id_hd, "ff_idXt2lex_id_hd", prodmax+ntmax+1, 1, 0, 0, ff_idXt2lex_id_oflw, NULL);
    lex_id2nb = (int*) sxcalloc (XxY_size (ff_idXt2lex_id_hd)+1, sizeof (int));
	...
	if (number_str [LINE_LGTH-2] != '\n') {
	  ...
	  str = number_str+LINE_LGTH-1; /* pointe sur le NUL */
  ...
  */
  ...
  fputs ("int logprobs_nt_nb = NT_NB;\n", stdout);
