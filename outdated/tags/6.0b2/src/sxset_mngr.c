...
/************************************************************************/
/* 29-09-94 11:53 (pb):		Ajout de cette rubrique "modifications"	*/
/************************************************************************/


#include        "sxcommon.h"

char WHAT_SXSET_MNGR[] = "@(#)SYNTAX - $Id: sxset_mngr.c 527 2007-05-09 14:02:47Z rlacroix $" WHAT_DEBUG;
...
void
sxset_alloc (header, name, universe_size, sets_nb, average_elems_nb_per_set,
		     sets_oflw, elems_oflw, stat_file)
    sxset_header	*header;
    char		*name;
    unsigned long	universe_size, sets_nb, average_elems_nb_per_set;
    void		(*sets_oflw) (), (*elems_oflw) ();
    FILE		*stat_file;
{
    /* Cette fonction "cree" un adresseur extensible d'ensembles de unsigned long int */

    header->name = name;
    header->sparse = (unsigned long*) sxalloc (header->sparse_size = universe_size,
					       sizeof (unsigned long));
    header->sets = (struct sxset_sets*) sxalloc (header->sets_size = sets_nb,
					     sizeof (struct sxset_sets));
    header->elems =
	(unsigned long*) sxalloc (header->elems_size = sets_nb * average_elems_nb_per_set,
				  sizeof (unsigned long));
...
void
sxset_stat (header, stat_file)
    sxset_header	*header;
    FILE		*stat_file;
{
    unsigned int	x, total_size, total_top, sets_top = 0;
	...
	+ sizeof (unsigned long) * header->elems_size;
    total_top = sizeof (sxset_header) + sizeof (struct sxset_sets) * (header->sets_top - sets_top)
	+ sizeof (unsigned long) * header->elems_top;

    fprintf (stat_file, "name = %s, Total_size (byte) = %d (%d%%)\n",
		 header->name, total_size, (100*total_top)/total_size);
}
...
void
sxset_local_pack (header)
    sxset_header	*header;
{
    /* Pack the current active set */
    unsigned long	x, n, elem, *to, *from, *bot;
    ...
    sxset_header	*header;
{
    ...
    unsigned long	x, *new_elems, *bot, *new_bot, *top, *new_top;

    if (header->erased_set_nb > 0 || header->erased_elem_nb > 0) {
	new_bot = new_top = 
	    new_elems = (unsigned long *) sxalloc (header->elems_size, sizeof (unsigned long));
...
BOOLEAN
sxset_member (header, set_id, elem)
    sxset_header	*header;
    ...
    sxset_header	*header;
{
	...
	header->elems = (unsigned long*) sxrealloc (header->elems,
						    header->elems_size *= 2,
						    sizeof (unsigned long));
    ...
    unsigned long	elem;
{
	...
	header->sparse = (unsigned long*) sxrealloc (header->sparse,
						     header->sparse_size,
						     sizeof (unsigned long));
	not_a_member = TRUE;
    }
...
unsigned long
sxset_not (header, set_id)
    sxset_header	*header;
    unsigned long	set_id;
{
    /* header->current_set_id = ~set_id */
    unsigned long	x, elem;
