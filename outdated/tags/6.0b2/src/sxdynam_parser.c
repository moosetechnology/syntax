...
/************************************************************************/
/* 20030505 13:05 (phd):	Modifs pour calmer le compilo SGI	*/
/************************************************************************/
...
char WHAT_SXDYNAM_PARSER[] = "@(#)SYNTAX - $Id: sxdynam_parser.c 527 2007-05-09 14:02:47Z rlacroix $" WHAT_DEBUG;
    ...
    int		old_size, new_size;
{
  ...
  if (XxY_is_static (SXDG.P))
    {
      memcpy (new_ptr = sxalloc (new_size + 1, sizeof (int)),
	      SXDG.rule_to_rhs_lgth,
	      sizeof (int) * (old_size + 1));
      SXDG.rule_to_rhs_lgth = (int*) new_ptr;
	
      memcpy (new_ptr = sxalloc (new_size + 1, sizeof (int)),
	      SXDG.rule_to_first_item,
	      sizeof (int) * (old_size + 1));
      SXDG.rule_to_first_item = (int*) new_ptr;
	
      memcpy (new_ptr = sxalloc (new_size + 1, sizeof (int)),
	      SXDG.rule_to_action,
	      sizeof (int) * (old_size + 1));
      SXDG.rule_to_action = (int*) new_ptr;
    ...
    {
      SXDG.rule_to_rhs_lgth = (int*) sxrealloc (SXDG.rule_to_rhs_lgth, new_size + 1, sizeof (int));
      SXDG.rule_to_first_item = (int*) sxrealloc (SXDG.rule_to_first_item, new_size + 1, sizeof (int));
      SXDG.rule_to_action = (int*) sxrealloc (SXDG.rule_to_action, new_size + 1, sizeof (int));
  ...
  if (XxY_is_static (SXDG.items))
    {
      memcpy (new_ptr = sxalloc (new_size + 1, sizeof (int)),
	       ...
	       sizeof (int) * (old_size + 1));
      SXDG.item_to_next_item = (int*) new_ptr;
    ...
    {
      SXDG.item_to_next_item = (int*) sxrealloc (SXDG.item_to_next_item,
						 new_size + 1,
						 sizeof (int));
   ...
   if (sxlist_is_static (SXDG.states))
    {
      memcpy (new_ptr = sxalloc (new_size + 1, sizeof (int)),
	       ...
	       sizeof (int) * (old_size + 1));
      SXDG.state_to_action = (int*) new_ptr;

      memcpy (new_ptr = sxalloc (new_size + 1, sizeof (int)),
	       SXDG.state_to_in_symbol,
	       sizeof (int) * (old_size + 1));
      SXDG.state_to_in_symbol = (int*) new_ptr;

      memcpy (new_ptr = sxalloc (new_size + 1, sizeof (int)),
	       SXDG.state_to_used_lnk,
	       sizeof (int) * (old_size + 1));
      SXDG.state_to_used_lnk = (int*) new_ptr;
    ...
    {
      SXDG.state_to_action = (int*) sxrealloc (SXDG.state_to_action, new_size + 1, sizeof (int));
      SXDG.state_to_in_symbol = (int*) sxrealloc (SXDG.state_to_in_symbol,
						  new_size + 1, sizeof (int));
      SXDG.state_to_used_lnk = (int*) sxrealloc (SXDG.state_to_used_lnk,
						 new_size + 1, sizeof (int));
    ...
    int		old_size, new_size;
{
  ...
  if (SXDG.t_priorities != NULL)
  {
	...
	memcpy (new_ptr = sxalloc (new_size + 1, sizeof (struct priority)),
		 SXDG.t_priorities,
		 sizeof (struct priority) * (old_size + 1));
	SXDG.t_priorities = (struct priority*) new_ptr;
      }
    else
      {						       
...
static  SXVOID
transitions_oflw (old_size, new_size)
    int		old_size, new_size;
{
  if (XxY_is_static (SXDG.transitions))
    {
      SXDG.trans_to_next_state = (int*) sxalloc (new_size + 1, sizeof (int));
      SXDG.trans_has_prdct = sxba_calloc (new_size + 1);
    }
  else
    {
      SXDG.trans_to_next_state = (int*) sxrealloc (SXDG.trans_to_next_state,
						   new_size + 1, sizeof (int));
...
static  SXVOID
parse_stacks_oflw (old_size, new_size)
    int		old_size, new_size;
{
...
update_automaton (rule)
    int rule;
{
    int lhs, transition, state, couple;
...
int
sxdp_rule_is_set (rhs_lgth, symbols)
    int rhs_lgth, *symbols;
{
    int rhs, i;
...
BOOLEAN
sxdp_rule_is_activated (int rule)
{
...
BOOLEAN
sxdp_rule_set (rhs_lgth, symbols, rule, action_nb, prio, assoc)
    int rhs_lgth, *symbols, *rule, action_nb, prio, assoc;
{
    /* retourne TRUE ssi la regle est nouvelle. */
    int	rhs, i, item, next_item;
    BOOLEAN	is_new;
...
SXVOID
sxdp_rule_get_attr (rule, action_nb, prio, assoc)
    int rule, *action_nb, *prio, *assoc;
{
...
SXVOID
sxdp_rule_set_attr (rule, action_nb, prio, assoc)
    int rule, action_nb, prio, assoc;
{
    SXDG.rule_to_action [rule] = action_nb;
...
GC_states ()
{
    ...
    int ps, state, next_state, transition, itoc, used_hd;
...
static BOOLEAN
new_pstack (ops, state, ptok_no, nps)
    int ops, state, ptok_no, *nps;
{
#if EBUG
    if (state <= 0)
...
BOOLEAN
sxdp_add_new_state (transition, result)
    int transition, *result;
{
    /* Construction de l'etat atteint par transition */
    /* Retourne TRUE ssi l'etat *result est nouveau */
    int next_state, nucleus, item_occur, item, inter_trans, symbol, couple, prdct_no, x, scrmbl;
    BOOLEAN has_prdct, is_new_state;
...
static BOOLEAN
check_viable_prefix (xps, parse_stack)
    int xps, *parse_stack;
{
    int x, prev_state, state, next_state, xsymbol, symbol, transition, old_pstack, pstack;
    BOOLEAN stack_has_changed;
...
static SXVOID
reactivate_path (xps, pstack)
    int xps, pstack;
{
    /* On est sur que les etats de pstack existent, on reactive les liens
       entre 2 etats consecutifs. */
    int prev_state, state, inter_state, xsymbol, symbol, transition, couple;
...
static SXVOID
push_next_state (state, symbol)
    int state, symbol;
{
    /* state et symbol sont negatifs */
    int trans, next_state;
...
get_next_states (transition)
    int transition;
{
    int 	prdct_true_symbol, prdct_no, state, trans, symbol;
    BOOLEAN	is_dynam_prdct_already_processed;
	...
	XxY_Xforeach (SXDG.transitions, state, trans)
	{
		...
		if (prdct_no < -1)
		{
			...
			if (sxplocals.mode.with_parsprdct)
			    ...
			    (*sxplocals.SXP_tables.parsact) (PREDICATE, prdct_no);
		    }
		}
		else
		    if (sxplocals.mode.with_parsprdct
...
static int
check_new_pstack (pstack_orig, pstack, state, ptok_no)
    int pstack_orig, pstack, state, ptok_no;
{
    int		conf, pstack1, son_pstack;
    ...
    int rule, rhs_lgth;
{
	...
	(*sxplocals.SXP_tables.parsact) (DO, -action);
    }
    else
	action = 0;
    ...
    int rule, rhs_lgth, action;
{
    ...
    (*sxplocals.SXP_tables.parsact) (UNDO, -action);
}


static BOOLEAN
check_reduce (pstack_orig, la_lgth, rule)
    int pstack_orig, rule, la_lgth;
{
    int		lhs, prdct_no, rhs_lgth, i, transition, state, pstack, nps, nc, rule_or_trans, action;
    ...
    action = parsact_DO (rule, rhs_lgth); /* Si <0, appel de DO */
	...
	if ((nps = check_new_pstack (pstack_orig, pstack, state, sxplocals.ptok_no)) != 0)
	{
		...
		is_checked = (*sxplocals.SXP_tables.parsact) (PREDICATE, prdct_no);
	    else
		is_checked = TRUE;
...
static int
check_t_trans (pstack, la_lgth, transition)
    int pstack, la_lgth, transition;
{
    ...
    int		state, valid_trans_nb, pstack1, lahead, rule_or_trans, nc;
    struct pstack_to_attr	*pconf;
    BOOLEAN	last_t_trans, is_checked;
...
static BOOLEAN
call_solve_conflicts (pstack_orig, pstack, la_lgth)
    int pstack_orig, pstack, *la_lgth;
{
    /* On ajuste la parse-stack pour la faire passer a pstack
       et on appelle solve_conflicts sans verification. */
    int rule_or_trans, pstack1, rhs_lgth, rule, action;
    BOOLEAN	is_solved;
 /* int state = XxYxZ_Y (SXDG.parse_stacks, pstack); */
...
static BOOLEAN
solve_conflicts (parse_stack, la_lgth, rule_or_trans, pstack1)
    int parse_stack, *la_lgth, *rule_or_trans, *pstack1;
{
    ...
    int		rule, prio, prio_reduce, xt, prio_shift, lahead, sup_lahead, valid_action_nb, pstack, state, item_occur, reduce_transition, shift_transition, shift_pstack, reduce_pstack, assoc, choosed_pstack, choosed_rule;
    int		*pconf;
...
static int
reduce (rule, pstack_orig, pstack1)
    int rule, pstack_orig, pstack1;
{
    /* Si pstack1 == 0, on doit la calculer */
    int		action, rhs_lgth, conf, pstack, lhs, prdct_no, state, transition;
    BOOLEAN	is_semact, is_checked;
	...
	if (sxplocals.mode.with_parsact)
	    (*sxplocals.SXP_tables.parsact) (ACTION, -action);
...
shift ()
{
    int rule_or_trans, pstack, state, la_lgth;
    int next_state, xt, trans;
	...
	else
	{
	    ...
	    if (next_state != state)
		/* L'inegalite peut etre due a l'absence de /DO/UNDO/ ds les parsact :
...
sxdp_parse_it ()
{
    ...
    new_pstack (0, SXDG.init_state, sxplocals.ptok_no, &SXDG.parse_stack);
...
BOOLEAN
sxdp_new_xt (t, prdct, xt, prio, assoc)
    int t, prdct, *xt, prio, assoc;
{
    BOOLEAN is_new;
...
#define WRITE(p,l)	((bytes=(l))>0&&(write(file_descr, p, (size_t)bytes) == bytes))
  ...
  return (sxlist_write (&(SXDG.states), file_descr)
	  ...
	  && WRITE (SXDG.rule_to_rhs_lgth, sizeof (int) * (XxY_top (SXDG.P) + 1))
	  && WRITE (SXDG.rule_to_first_item, sizeof (int) * (XxY_top (SXDG.P) + 1))
	  && WRITE (SXDG.rule_to_action, sizeof (int) * (XxY_top (SXDG.P) + 1))
	  && WRITE (SXDG.state_to_action, sizeof (int) * (sxlist_size (SXDG.states) + 1))
	  && WRITE (SXDG.state_to_in_symbol, sizeof (int) * (sxlist_size (SXDG.states) + 1))
	  && WRITE (SXDG.state_to_used_lnk, sizeof (int) * (sxlist_size (SXDG.states) + 1))
	  && WRITE (SXDG.item_to_next_item, sizeof (int) * (XxY_top (SXDG.items) + 1))
	  && WRITE (SXDG.trans_to_next_state, sizeof (int) * (XxY_top (SXDG.transitions) + 1))

	  && WRITE (&(SXDG.init_state), sizeof (int))
	  && WRITE (&(SXDG.init_transition), sizeof (int))
	  && WRITE (&(SXDG.super_start_symbol), sizeof (int))
	  && WRITE (&(SXDG.start_symbol), sizeof (int))
	  && WRITE (&(SXDG.super_rule), sizeof (int))
	  && WRITE (&(SXDG.eof_code), sizeof (int))

	  && WRITE (&is_prio, sizeof (BOOLEAN))
...
#define READ(p,l)	((bytes=(l))>0&&(read (file_descr, p, (size_t) bytes) == bytes))
  ...
  return (sxlist_read (&(SXDG.states), file_descr, "states", states_oflw, NULL, NULL)

	  && XxY_read (&SXDG.P, file_descr, "P", P_oflw, NULL)
	  && XxY_read (&SXDG.rhs, file_descr, "rhs", NULL, NULL)
	  ...
	  && XxY_read (&SXDG.item_occurs, file_descr, "item_occurs", NULL, NULL)
	  && XxY_read (&SXDG.transitionxnext_state, file_descr, "transitionxnext_state", NULL, NULL)
	  && XxY_read (&SXDG.xt, file_descr, "xt", xt_oflw, NULL)
	  && XxY_read (&SXDG.xnt, file_descr, "xnt", NULL, NULL)
	  ...
	  && READ (SXDG.rule_to_rhs_lgth = (int*) sxalloc (XxY_size (SXDG.P) + 1, sizeof (int)),
		   sizeof (int) * (XxY_top (SXDG.P) + 1))
	  && READ (SXDG.rule_to_first_item = (int*) sxalloc (XxY_size (SXDG.P) + 1, sizeof (int)),
		   sizeof (int) * (XxY_top (SXDG.P) + 1))
	  && READ (SXDG.rule_to_action = (int*) sxalloc (XxY_size (SXDG.P) + 1, sizeof (int)),
		   sizeof (int) * (XxY_top (SXDG.P) + 1))
	  && READ (SXDG.state_to_action = (int*) sxalloc (sxlist_size (SXDG.states) + 1, sizeof (int)),
		   sizeof (int) * (sxlist_size (SXDG.states) + 1))
	  && READ (SXDG.state_to_in_symbol = (int*) sxalloc (sxlist_size (SXDG.states) + 1,
							     sizeof (int)),
		   sizeof (int) * (sxlist_size (SXDG.states) + 1))
	  && READ (SXDG.state_to_used_lnk = (int*) sxalloc (sxlist_size (SXDG.states) + 1, sizeof (int)),
		   sizeof (int) * (sxlist_size (SXDG.states) + 1))
	  && READ (SXDG.item_to_next_item = (int*) sxalloc (XxY_size (SXDG.items) + 1, sizeof (int)),
		   sizeof (int) * (XxY_top (SXDG.items) + 1))
	  && READ (SXDG.trans_to_next_state = (int*) sxalloc (XxY_size (SXDG.transitions) + 1,
							      sizeof (int)),
		   sizeof (int) * (XxY_top (SXDG.transitions) + 1))

	  && READ (&(SXDG.init_state), sizeof (int))
	  && READ (&(SXDG.init_transition), sizeof (int))
	  && READ (&(SXDG.super_start_symbol), sizeof (int))
	  && READ (&(SXDG.start_symbol), sizeof (int))
	  && READ (&(SXDG.super_rule), sizeof (int))
	  && READ (&(SXDG.eof_code), sizeof (int))

	  && READ (&is_prio, sizeof (BOOLEAN))
...
sxdp_alloc (rule_nb)
     int rule_nb;
{
    static int	parse_stacks_foreach [] = {0, 0, 0, 0, 0, 0};
    
    if (SXDG.rule_to_rhs_lgth == NULL)
    {
	/* Allocation des structures pour la grammaire. */
	XxY_alloc (&SXDG.P, "P", rule_nb, 1, 1, 0, P_oflw, NULL);
	SXDG.rule_to_rhs_lgth = (int*) sxalloc (XxY_size (SXDG.P) + 1, sizeof (int));
	SXDG.rule_to_first_item = (int*) sxalloc (XxY_size (SXDG.P) + 1, sizeof (int));
	SXDG.rule_to_action = (int*) sxalloc (XxY_size (SXDG.P) + 1, sizeof (int));
	SXDG.active_rule_set = sxba_calloc (XxY_size (SXDG.P) + 1);
	
	XxY_alloc (&SXDG.rhs, "rhs", rule_nb*2, 1, 0, 0, NULL, NULL);

	XxY_alloc (&SXDG.items, "items", rule_nb*3, 1, 0, 1, items_oflw, NULL);
	SXDG.item_to_next_item = (int*) sxalloc (XxY_size (SXDG.items) + 1, sizeof (int));
	SXDG.item_set = sxba_calloc (XxY_size (SXDG.items) + 1);
	XxY_alloc (&SXDG.xt, "xt", rule_nb / 3, 1, 0, 0, xt_oflw, NULL);
	XxY_alloc (&SXDG.xnt, "xnt", rule_nb / 2, 1, 0, 0, NULL, NULL);
    }

    if (SXDG.state_to_action == NULL)
    {
	/* Allocation des structures pour l'automate. */
	sxlist_alloc (&SXDG.states, "states", rule_nb * 2, 1, 20, states_oflw, NULL, NULL);
	SXDG.state_to_action = (int*) sxalloc (sxlist_size (SXDG.states) + 1, sizeof (int));
	SXDG.state_to_in_symbol = (int*) sxalloc (sxlist_size (SXDG.states) + 1, sizeof (int));
	SXDG.state_to_used_lnk = (int*) sxalloc (sxlist_size (SXDG.states) + 1, sizeof (int));
...
sxdp_G0 ()
{
    extern char		*language_name; /* Doit etre positionne' par le
					   "main ()" de l'utilisateur. */
    if (*language_name != NUL)
    {
	...
	{
	    sxlist_reuse (&SXDG.states, "states", states_oflw, NULL, NULL);

	    XxY_reuse (&SXDG.P, "P", P_oflw, NULL);
	    XxY_reuse (&SXDG.rhs, "rhs", NULL, NULL);
	    XxY_reuse (&SXDG.items, "items", items_oflw, NULL);
	    XxY_reuse (&SXDG.transitions, "transitions", transitions_oflw, NULL);
	    XxY_reuse (&SXDG.item_occurs, "item_occurs", NULL, NULL);
	    XxY_reuse (&SXDG.transitionxnext_state, "transitionxnext_state", NULL, NULL);
	    XxY_reuse (&SXDG.xt, "xt", xt_oflw, NULL);
	    XxY_reuse (&SXDG.xnt, "xnt", NULL, NULL);
	}
    }
    ...
    struct sxtables	*arg;
{
	...
	if (!sxplocals.mode.is_silent)
	    sxput_error (SXGET_TOKEN (sxplocals.ptok_no).source_index, "%ssyntax error", sxplocals.sxtables->err_titles [2]);
