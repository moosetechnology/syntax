...
char WHAT_SXATC[] = "@(#)SYNTAX - $Id: sxatc.c 566 2007-05-23 11:20:09Z syntax $" WHAT_DEBUG;
    ...
    sxindex_header	*index_hd;
{
    ...
    while (--i >= k)
	sxatcvar.atc_lv.u.New.nodes [i] = (NODE *) sxcalloc ((unsigned long int)j, (unsigned long int)sxatcvar.atc_lv.node_size);
}
    ...
    /* si sxatc_what=ACTION, arg est un entier entre 0 et 6 ;  si sxatc_what != ACTION, arg est un pointeur vers des tables. */
{
    ...
    case ACTION:
	 ...
	 et 6, on veut que le compilo C ne hurle pas sur un problème de typage, d'où le(s?) cast(s?) */
	action ((int) (long) arg);
