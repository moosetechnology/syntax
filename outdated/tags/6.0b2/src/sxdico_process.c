...
char WHAT_SXDICO_PROCESS[] = "@(#)SYNTAX - $Id: sxdico_process.c 654 2007-06-26 10:42:18Z rlacroix $" WHAT_DEBUG;
...
static int 	state_top, teeth_nb, pw_nb, ps_nb; 
...
static int 	as_size, class_nb, max_lgth;

static int	**automaton, *vector;

static int	*state2trans_nb, *sorted_by_state, start_state;


static BOOLEAN	state_by_lgth (i, j)
    int		i, j;
{
...
static BOOLEAN	less (i, j)
    int		i, j;
{
    int x, y, x_min, x_max, y_min, y_max, x_char, y_char;
    ...
    int	state;
{
	...
	if (!sxba_bit_is_set (comb.disp_set, start)) {
		...
		if ((unsigned)start+last_class > dico.max)
		    dico.max = start+last_class;
    
		if (x > max_x) {
		  max_x = x;

		  if (max_x + last_class + 1 >= comb.size) {
		...
		if (mot2.print_on_sxtty) fprintf (sxtty, "\010\010\010\010\010\010%-6i", ++ps_nb);
		return start;
	    }
	}
    }
...
get_next_state ()
{
	...
	vector = (int*) sxrealloc (vector, as_size*(class_nb+1), sizeof (int));
	automaton = (int**) sxrealloc (automaton, as_size, sizeof (int*));
	automaton [0] = NULL;
...
gen (bi, bf)
    int		bi, bf;
{
    int			cur_state, next_state, x, y, kw, kw2, min, max, min2, max2;
	...
	if ((mot2.process_kind == PARTIEL) || (min > max)) {
	    ++pw_nb;
#if !EBUG
	    if (mot2.print_on_sxtty) fprintf (sxtty, "\010\010\010\010\010\010%-6i", pw_nb);
#endif
	    return kw;
	}
    }
...
make_dico ()
{
    ...
    if (mot2.nb) {
	if (dico.max) {
	    dico.comb_vector = (unsigned int*) sxalloc (dico.max+1, sizeof (unsigned int));
	...
	if (dico.max) {
	    for (i = 0; (unsigned int)i <= dico.max; i++) {
...
struct dico *output;
{
    ...
    sorted = (int*) sxalloc (mot2.nb + 1, sizeof (int));
    ...
    if (mot2.print_on_sxtty)
      fprintf (sxtty, "\nProcessing of %i words (%i characters, %i classes)...       ",
	       mot2.nb, mot2.total_lgth, class_nb);

    sxba_empty (char_set1);
    
    sorted_by_char = (int*) sxalloc (mot2.nb + 1, sizeof (int));
    char_set2 = sxba_calloc (256);

    as_size = (2*mot2.nb)+1;
    automaton_signature = sxbm_calloc (as_size, class_nb+1);
    automaton = (int**) sxalloc (as_size, sizeof (int*));
    automaton [0] = NULL;
    vector = (int*) sxalloc (as_size*(class_nb+1), sizeof (int));
    ...
    start_state = gen (1, mot2.nb);

    if (mot2.print_on_sxtty) fprintf (sxtty, " done\n(state_nb = %i, last_class = %i, teeth_nb = %i)\n",
				      state_top, last_class, teeth_nb);
    ...
    if (mot2.print_on_sxtty) fprintf (sxtty, "Automaton packing of %i states (%i cells)...       ",
				      state_top, state_top*last_class);

    state2trans_nb = (int*) sxalloc (state_top+1, sizeof (int));
    state2trans_nb [0] = 0;

    for (i = 1; i <= state_top; i++)
	state2trans_nb [i] = sxba_cardinal (automaton_signature [i]);

    sorted_by_state = (int*) sxalloc (state_top+1, sizeof (int));
    ...
    comb.disp = (int*) sxalloc (state_top+1, sizeof (int));
    ...
    if (mot2.print_on_sxtty) {
	fprintf (sxtty, " in %i cells\n", dico.max);
	fprintf (sxtty, "Dictionary size = %lu bytes for %i words of %i characters\n",
		 (long)(256+(dico.max+1)*(sizeof (int))), mot2.nb, mot2.total_lgth);
    }
	...
	if (sxdico_get (&dico, &kw, &kwl) != mot2.code [i]) {
	    unsigned char uchar = kw [kwl];
...
void
sxdico2c (sxdico2c_dico, file, dico_name, is_static)
     struct dico	*sxdico2c_dico;
     FILE		*file;
     char               *dico_name;
     BOOLEAN            is_static;
{
  ...
  fprintf (file, "\nstatic unsigned int %scomb_vector [%i] = {", dico_name == NULL ? "" : dico_name, sxdico2c_dico->max+1);

  for (i = 0; (unsigned int)i <= sxdico2c_dico->max; i++) {
    if ((i % 10) == 9)
      fputs ("\n", file);

    fprintf (file, "%i, ", sxdico2c_dico->comb_vector [i]);
  }
  ...
  fprintf (file, "%i /* max */, %i /* init_base */, %i /* class_mask */, %i /* stop_mask */, %i /* is_suffix_mask */, %i /* base_shift */, %i /* partiel */,\n",
	  sxdico2c_dico->max, sxdico2c_dico->init_base, sxdico2c_dico->class_mask, sxdico2c_dico->stop_mask, sxdico2c_dico->is_suffix_mask, sxdico2c_dico->base_shift, sxdico2c_dico->partiel);
