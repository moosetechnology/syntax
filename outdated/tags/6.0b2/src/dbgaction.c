...
char WHAT_DGBCATION[] = "@(#)SYNTAX - $Id: dbgaction.c 527 2007-05-09 14:02:47Z rlacroix $" WHAT_DEBUG;
...
{
    printf ("token string = \"%.*s\", state = %d,\n", sxsvar.lv.ts_lgth, sxsvar.lv_s.token_string, sxsvar.lv.current_state_no);
    printf ("terminal token = {lahead = %d, entry =%d,\n", sxsvar.lv.terminal_token.lahead, sxsvar.lv.terminal_token.string_table_entry);
    printf ("\tsource_index = \"%s\" [%lu, %u],\n", sxsvar.lv.terminal_token.source_index.file_name, sxsvar.lv.terminal_token.source_index.line, sxsvar.lv.terminal_token.source_index.column);
