...
/************************************************************************/
/* 20030507 18:07 (phd):	Modifs pour calmer le compilo SGI	*/
/************************************************************************/
...
/*				(et non plus a 10000)			*/
/************************************************************************/
/* 11-06-93 16:25 (pb):		Analyse non deterministe sur un tag de	*/
/*				tokens					*/
/************************************************************************/
...
char WHAT_SXND2PARSER[] = "@(#)SYNTAX - $Id: sxnd2parser.c 527 2007-05-09 14:02:47Z rlacroix $" WHAT_DEBUG;
...
static int	*rhs_stack, *symbol_stack, *symb_to_rule_hd, *symb_to_rule_list;
...
VOID sxnd2parser_GC ()
{
    ...
    int				i, x, father, son, xarea, index;
...
BOOLEAN	set2_next_trans (abase, check, action, Max, next_action_kind)
    struct SXP_bases	*abase;
    int			*check, *action, Max;
    int			*next_action_kind;
{
...
static int	ARC_traversal (ref, latok_no)
    int	ref, latok_no;
{
    int next, first_t_state, t_state;
	...
	if (ref < -sxplocals.SXP_tables.Mred && ref >= -sxplocals.SXP_tables.Mprdct) {
		...
		while (aprdct->prdct >= 0 /* User's predicate */  &&
		       (!(*sxplocals.SXP_tables.parsact) (PREDICATE, aprdct->
...
static BOOLEAN new_symbol (p, k, i, j, symbol)
    int p, k, i, j, *symbol;
{
    int		kind, range;
    ...
    int			lhs, rhs;
{
	...
	if (sxplocals.mode.with_parsact) {
	    ...
	    (*sxplocals.SXP_tables.parsact) (ACTION, parse_stack.semact [reduce]);
#endif
	}
	else {
...
static int build_a_rule (parser, index, rhs)
    int parser, index, rhs;
{
    int symb;
...
int mreds_push (int parser, int ref)
{
    int			x, i;
    int			*ai;
...
int seek_parser (son, state, ref, symbol)
    int		son, state, ref, symbol;
{
    /* Si ref est negatif, il s'agit d'une transition non-terminale */
    int				x, father, local_link, index, son_state;
    struct  parsers_attr	*pattr;
    BOOLEAN			is_an_old_father, is_active_son;
...
SXVOID sxndsubparse_a_token (parser, ref, symbol, do_shift_reduce)
    ...
    BOOLEAN	do_shift_reduce;
{
    int			state, lgth, rhs, t_trans, next_action_kind, dum_parser;
    int			*pref;
    struct SXP_bases	*abase;
    ...
    BOOLEAN	is_in_set;
{
    /* We want to perform goto (son, A) where A is the lhs of the
       reduced production. */
    int				ref, state, next_son, symb, lhs; 
...
SXVOID reducer ()
{
    /* new_top est non nul et designe la tete de la liste qui parcourt les
       active parsers. */
    int		p, ref, lgth, x, bot, cur_top, xsr;
	...
	int symbol;
{
	...
	parse_stack.grammar = (int*) sxrealloc (parse_stack.grammar,
    ...
    int tnt;
{
    ...
    int item, old_top, x, rhs, rule_no;
...
{
    int		lhs_symbol, rule, item, tnt, lim, symbol, pos, rhs, nb;
    int		*aelem;

    parse_stack.rule_size = XxY_top (parse_stack.sf_rule) + 1;

    symb_to_rule_hd =
	(int*) sxcalloc (XxY_top (parse_stack.symbols) + 1, sizeof (int));

    symb_to_rule_list =
	(int*) sxalloc (parse_stack.rule_size + 1, sizeof (int));
    ...
    parse_stack.grammar =
	(int*) sxalloc ((parse_stack.grammar_size = 4 * parse_stack.rule_size) + 1,
			sizeof (int));
    ...
    symbol_stack = (int*) sxcalloc (XxY_top (parse_stack.symbols) + 1, sizeof (int));
    symbol_stack [0] = -1;
    ...
    parse_stack.positions = (int*) sxalloc (parse_stack.grammar_top + 1,
					    sizeof (int));
...
static VOID sxndparse_it ()
{
    ...
    } while (!parse_stack.halt_hit && (*sxplocals.SXP_tables.recovery) (ACTION));
}
    ...
    int index, succ_kind, new_kind;
{
    /* Si tous les successeurs de chaque predecesseur complet de index
       ...
       mis (|=) a new_kind */
    int				x, z, prev_ms_nb, next_ms_nb, mtsa, sa, tok_no;
...
{
    int				x, y, z, prev_ms_nb, next_ms_nb, parser;
    struct sxmilstn_elem 	*prev_ms_ptr, *next_ms_ptr;
    BOOLEAN			is_wait;
...
{
#if EBUG
    sxtime (ACTION, "Scanning, Non-Deterministic Parsing & Parse Forest Sharing done in");
    ...
    sxtime (ACTION, "Shared Parse Forest Printing done in");
#endif
...
#if EBUG
	sxtime (ACTION, "Unused Sub-Tree Erasing done in");
	output_grammar ();
	sxtime (ACTION, "Shared Parse Forest Printing done in");
#endif
    }
...
#if EBUG
	    sxtime (ACTION, "Action & Predicate Phase done in");
	    fputs ("\n\n*\tAfter \"Action & Predicate\" phase.\n\n", stdout);
	    output_grammar ();
	    sxtime (ACTION, "Parse Tree Printing done in");
#endif
...
#if EBUG
	    sxtime (ACTION, "Desambiguation Phase done in");
    ...
    struct sxtables	*arg;
{
    ...
    static int	shift_reduce_foreach [] = {0, 0, 0, 0, 0, 0};
	...
	parse_stack.semact = (int*) sxalloc (arg->SXP_tables.P_xnbpro + 1, sizeof (int));
...
*/
	...
	parse_stack.for_reducer.refs = (int*) sxcalloc (i + 1, sizeof (int));
	parse_stack.for_reducer.refs [0] = -1;
	parse_stack.for_reducer.triples_hd = (int*) sxalloc (arg->SXP_tables.Mred + 1,
							    sizeof (int));
	parse_stack.for_reducer.shift_reduce_symb = (int*) sxcalloc (arg->SXP_tables.Mred + 1,
							 sizeof (int));
...
BOOLEAN sxnd22parser (what_to_do, arg)
    int		what_to_do;
    struct sxtables	*arg;
{
    int i, p, link;
...
BOOLEAN sxnd2parse_in_la (ep_la, Ttok_no, Htok_no, mode_ptr)
    ...
    struct sxparse_mode		*mode_ptr;
{
    ...
    int			i, current_token, current_lahead, current_level, p, link;
    BOOLEAN		ret_val;
