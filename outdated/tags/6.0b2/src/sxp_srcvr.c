...
#ifndef VARIANT_32
char WHAT_SXP_SRECOVERY[] = "@(#)SYNTAX - $Id: sxp_srcvr.c 617 2007-06-11 08:24:34Z rlacroix $" WHAT_DEBUG;
#else
char WHAT_SXP_SRECOVERY[] = "@(#)SYNTAX - $Id: sxp_srcvr.c 617 2007-06-11 08:24:34Z rlacroix $ SXP_SRCVR_32" WHAT_DEBUG;
#endif
#define Commun 1
#define Propre 2
#define Defaut 3
...
static BOOLEAN	REF_is_a_right_ctxt (head, tail, pxs, pstate, ref, look_ahead, keep_stack)
    int		head, tail, *pxs;
...

static VOID set_vt_set ();

static VOID new_ref (stack, xstd, ref, test)
    SXP_SHORT	*stack;
    int		xstd;
    SXP_SHORT	ref, test;
{
    struct SXP_reductions	*ared;
    SXP_SHORT			j = xstd, prev_state, nt, state;
...
static BOOLEAN	recovery ()
{
    ...
    int					head, tail, k, vt_set_card, xps, next_action_kind, xs2;
...
/* L'erreur sera marquee (listing) a l'endroit de la detection */
	...
	|| is_a_right_ctxt (0, 0, &xs2, &state, FALSE)) 
...

BOOLEAN		sxpsrecovery (what_to_do, at_state, latok_no)
