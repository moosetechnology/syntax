...
char WHAT_SXLISTOUT[] = "@(#)SYNTAX - $Id: sxlistout.c 527 2007-05-09 14:02:47Z rlacroix $" WHAT_DEBUG;
    ...
    FILE	*LISTING;
{
    ...
    listing = LISTING;
    err_no = 0, errline_no = ~((unsigned int) 0);
	...
	if (errline_no == (unsigned int)line_no) {
	    static char		STARS [] = "\n\n******";
		...
		errline_no = (++err_no < sxerrmngr.err_kept && sxerrmngr.err_infos [err_no].file_ste == this_file_ste ?
		     sxerrmngr.err_infos [err_no].line : ~((unsigned int)0));
	    } while (errline_no == (unsigned int)line_no);
