...
char WHAT_FERMER[] = "@(#)SYNTAX - $Id: fermer.c 527 2007-05-09 14:02:47Z rlacroix $" WHAT_DEBUG;
 ...
 */
{
  ...
  register int  lhs_size = BASIZE (lhs_bits_array), rhs_size = BASIZE (rhs_bits_array);
  register int	slices_number = NBLONGS (lhs_size);
  ...
  sxbassert ((*(rhs_bits_array+NBLONGS (rhs_size)) & ((~((SXBA_ELT)0)) << 1 << MOD (rhs_size - 1))) == 0, "or");
...
static int
FIRST_DIFFERENCE (bits_array_1, bits_array_2)
 ...
 */
{
  register int	    size1 = BASIZE (bits_array_1), size2 = BASIZE (bits_array_2);
  register int	    slices_number;
  register SXBA	    start_1 = bits_array_1;
  register SXBA_ELT oper2;

  slices_number = NBLONGS (size1);
  ...
  sxbassert ((*(bits_array_2+NBLONGS (size2)) & ((~((SXBA_ELT)0)) << 1 << MOD (size2 - 1))) == 0, "first_difference");
...
SXVOID	fermer2 (SXBA *R, int taille)
{
  int     A, B;
  int     **tilde_stacks, *area_ptr, *tilde_stack, *tilde_stack_top, *tbp_stack, *glbl_ptr;
  SXBA    tbp_set, line_A;

  tbp_set = sxba_calloc (taille);
  tbp_stack = (int*) sxalloc (taille, sizeof (int)), tbp_stack [0] = 0;

  /* Contiendra la relation transposee de R sous-forme de piles : tilde_stacks [B] = [A_1, A_2, ..., A_p] <=> (A_i, B) \in R */
  tilde_stacks = (int**) sxcalloc ((unsigned long int)taille, sizeof (int*)); /* calloc pour tester la non initialisation */
  glbl_ptr = area_ptr = (int*) sxalloc (taille*taille, sizeof (int));
