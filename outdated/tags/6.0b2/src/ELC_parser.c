...
char WHAT_ELC_PARSER[] = "@(#)SYNTAX - $Id: ELC_parser.c 527 2007-05-09 14:02:47Z rlacroix $" WHAT_DEBUG;
...
#define RAZ(t)		*t=0
#define ALLOC(t,s)      t = (int*) sxalloc (s+2, sizeof (int))+1, t [-1] = s;
...
static int          max_node_set_size /* Estimation de la taille des ensembles node2node_set pour la tranche j */;
static BOOLEAN      membership;

#if EBUG2
static int  node_hd_calls;
#endif
...
static int          *stack1, *stack2, *cur_scan_stack, *scan_stack;
...
p_dispatch (j, q, node_jq, node_ip)
     int j, q, node_jq, node_ip;
{
  int item, Y, prod, t, i, p, p1, q1, base, A, r, bot, top, k, node_kr, node_jr; 
...
sxelc_parse_it ()
{
  int j, q, node_00, node_01, node_jq, node_ip, *stack;
  SXBA node_ip_set;
#if EBUG3
  char str [12];
#endif /* EBUG3 */
...
static int	    null_assoc [6];
static XxYxZ_header chart_hd;

#if EBUG2
static int    memo_hit, *i2chart_nb;
#endif
...
e_dispatch (i, p, chart)
     int i, p, chart;
{
  int X, item, cur_chart, base, q, bot, top, prod, t, A, j, prodji, r, bot_chart;
...
#if EBUG2
  i2chart_nb = (int*) sxcalloc (n+2, sizeof (int));
#endif
...
{
  SXBA	lhs_bits_ptr, rhs_bits_ptr;
  int         rhs_size = BASIZE (rhs_bits_array);
  int	        slices_number;
...
p_dispatch (j, Q, node_jQ, node_ip, node_ip_set)
     ...
     SXBA node_ip_set;
{
  int item, Y, prod, t, i, p, p1, q, q1, base, A, r, bot, top, k, node_kr, node_jr; 
  BOOLEAN new, ntk_trans;
...
sxelc_parse_it ()
{
  int j, q, bot, top, node_00, node_01, node_jq, *stack;
  SXBA node_jq_set;
