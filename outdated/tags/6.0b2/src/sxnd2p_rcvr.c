...
/************************************************************************/
/* 19-08-94 14:36 (pb):		Les "codes" des sequences non		*/
...
char WHAT_SXND2P_RECOVERY[] = "@(#)SYNTAX - $Id: sxnd2p_rcvr.c 527 2007-05-09 14:02:47Z rlacroix $" WHAT_DEBUG;

extern int			NDP_access ();
extern int			seek_parser ();
extern SXVOID			set2_first_trans ();
extern BOOLEAN			set2_next_trans ();
...
static BOOLEAN validate_model (model_no, pos)
    int		model_no, pos;
{
    int 	*model = sxplocals.SXP_tables.P_lregle [model_no];
    int		bot, parser, index;
...
static BOOLEAN try_a_model (pos, parser_seed, model_no)
    int		pos, parser_seed, model_no;
{
    int 		*model = sxplocals.SXP_tables.P_lregle [model_no];
    int 		lmod = model [0];
    int			head, tail, index, parser, seed, prev_parser;
...
static BOOLEAN dont_delete (model_no)
    int model_no;
{
...
static int is_a_spelling_model (model_no)
    int model_no;
{
    int 	*model = sxplocals.SXP_tables.P_lregle [model_no];
    ...
    int son, fathers_state;
{
    ...
    int			size, link, father, next_action_kind, state;
    int			check, ref;
...
{
    int				z, model_no, pos, index, prev_for_scanner, parser_seed;
...
static BOOLEAN 	global_recovery (parser_seed)
    int parser_seed;
{
    int	head, tail, x, top, p;
...
static int nd2_search_validation_context (parser_seed, mstn_nb, n, lim)
    int parser_seed, mstn_nb, n, lim;
{
    struct sxmilstn_elem 	*mstn_ptr;
    int				x, ret_val;
    ...
    if (n == lim) {
	return (int) global_recovery (parser_seed);
    }
...
static BOOLEAN nd2_search_key_terminal (parser_seed, mstn_ptr)
    int				parser_seed;
    struct sxmilstn_elem 	*mstn_ptr;
{
    /* Recherche d'un milestone qui est atteint par un terminal-cle */
    int	z, mstn_nb, lahead, i, ret_val, tok_no;
...
static SXVOID modify_dag ()
{
    ...
    int			model_no, bot, first_ms, prev_ms, last_ms, x, l, i, cur_tok, cur_ms, tde;
    ...
    int		what_to_do;
{
	...
	if (sxplocals.rcvr.vt_set == NULL) {
	    int 	size, model_no, x, index, prev_index, elem;
	    int 	*model;
		...
		int	index, model_no, best_l, mn, l, best_model_no;
		int	*m, *cur_model;
