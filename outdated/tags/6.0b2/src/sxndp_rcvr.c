...
/************************************************************************/
/* 30-05-95 16:43 (pb):		GC est appele avec un param bidon	*/
/************************************************************************/
/* 24-04-95 15:31 (pb):		"reduce" est dans "parsers"		*/
/************************************************************************/
/* 01-02-95 17:01 (pb):		sxndsubparse_a_token a 1 arg de plus	*/
/************************************************************************/
/* 19-08-94 14:37 (pb):		Les "codes" des sequences non		*/
/*				deterministes sont superieures a 20000	*/
/*				(et non plus a 10000)			*/
/************************************************************************/
/* 06-07-93 11:25 (pb):		sxndpsrecovery devient un point d'entree*/
/************************************************************************/
/* 25-03-93 10:32 (pb):		Ajout de cette rubrique "modifications"	*/
/************************************************************************/

#include        "sxcommon.h"

#ifndef VARIANT_32
char WHAT_SXNDP_RECOVERY[] = "@(#)SYNTAX - $Id: sxndp_rcvr.c 617 2007-06-11 08:24:34Z rlacroix $" WHAT_DEBUG;
#else
char WHAT_SXNDP_RECOVERY[] = "@(#)SYNTAX - $Id: sxndp_rcvr.c 617 2007-06-11 08:24:34Z rlacroix $ SXNDP_RCVR_32" WHAT_DEBUG;
#endif
#if EBUG
...
extern int			NDP_access ();
extern int			new_parser ();
extern SXVOID			set_first_trans ();
extern BOOLEAN			set_next_trans ();
extern BOOLEAN			sxndsubparse_a_token ();
extern SXVOID			reducer ();
extern struct sxmilstn_elem	*milestone_new ();
extern int                      mreds_push (int parser, int ref, int tok_no, BOOLEAN is_new_triple, BOOLEAN is_shift_reduce, int symbol);
extern SXVOID                   sxndparser_GC (int current_parser);


/* E R R O R   R E C O V E R Y */

static SXVOID compute_a_trans (parser_seed, head)
    int parser_seed, head;
{
    ...
    *parse_stack.for_scanner.next_hd = 0;
    ...
    sxndsubparse_a_token (parser_seed, scan_ref, 0 /* symbol */, NULL);
...
static SXVOID nd2_modify_dag ()
{
    /* On a trouve une correction (dans rcvr.best), on modifie le dag
       des tokens en consequence. */
    int			model_no, pos, first_ms, prev_ms, last_ms, x, cur_tok, cur_ms, tde, elem;
	...
	if (elem < 0) {
	    ...
	    tok.string_table_entry = ERROR_STE /* error value */ ;
...
static SXVOID put_in_repair_list (repair_tok_no,
				 error_tok_no,
				 model_no,
				 kind,
				 msg_params)
    int 	repair_tok_no, error_tok_no, model_no, kind;
    char	*msg_params [5];
{
    int 		i;
    struct repair	*pelem;
...
SXVOID output_repair_string (output_repair_string_index)
    int output_repair_string_index;
{
    int			i;
    ...
    sxput_error (SXGET_TOKEN (pelem->error_tok_no).source_index,
		 sxplocals.SXP_tables.SXP_local_mess [pelem->model_no].P_string,
		 sxplocals.sxtables->err_titles [pelem->kind],
		 msg_params [0],
		 msg_params [1],
		 msg_params [2],
		 msg_params [3],
...
static int set_token_dag (to_be_cloned, is_copy, ptok, tok_no, ms1, ms2)
    BOOLEAN		to_be_cloned, is_copy;
    struct sxtoken	*ptok;
    int			tok_no, ms1, ms2;
{
...
static SXVOID one_correction_message ()
{
    /* OK ca a marche */
    char				*msg_params [5];
    int				        *regle;
    ...
    int		n, l, x, y, x1, y1, im, ll, j, i, ate, carac_tok_no;
    int		*mm;
    BOOLEAN	do_put_error, is_copy, is_carac;
    SXBA	back_bone;
	...
	for (x1 = x1; x1 <= x; x1++) {
		...
		sxplocals.rcvr.com [y] = sxrealloc (sxplocals.rcvr.com [y],
						    strlen (sxplocals.rcvr.com [y])
						    + strlen (tok->comment) + 1,
						    sizeof (char));
	...
	if (parse_stack.rcvr.all_corrections) {
	    ...
	    *tok = SXGET_TOKEN (sxplocals.rcvr.TOK_i);
		...
		put_in_repair_list (carac_tok_no,
				    ...
				    parse_stack.rcvr.is_warning ? 1 : 2,
				    msg_params);
	}
	else {
    ...
    if (do_put_error)
	sxput_error (parse_stack.rcvr.source_index,
		     local_mess->P_string,
...
static BOOLEAN correction_already_found (/* model_no, */model, lim)
     int	/* model_no, */lim;
    int 	*model;
{
    ...
    int		hd, tail, l, elem, correction_already_found_index;
...
static BOOLEAN validate_submodel (model_no, model, parser, pos, bot, a_la_rigueur)
    int		model_no, parser, pos, bot;
    int 	*model;
    BOOLEAN	a_la_rigueur;
{
    /* Retourne vrai ssi un modele est trouve'. */
    int		lim, l, x, seed, elem, t;
    BOOLEAN	local_a_la_rigueur;
...
static BOOLEAN validate_model (parser, model_no, pos)
    int		parser, model_no, pos;
{
    /* Si parser < 0 (-model_no), on a execute' l'instruction halt sur fin de fichier. */
    int 	*model = sxplocals.SXP_tables.P_lregle [model_no];
    int		bot;
...
static BOOLEAN try_a_model (pos, parser_seed, model_no, check_first_trans, tok_no_to_be_checked)
    int		pos, parser_seed, model_no, tok_no_to_be_checked;
    BOOLEAN	check_first_trans;
{
    int 		*model = sxplocals.SXP_tables.P_lregle [model_no];
    int 		lmod = model [0];
    int			head, tail, try_a_model_index, parser, seed, prev_parser;
    BOOLEAN		ret_val, is_halt_hit;
    ...
    /* Ex: modele 0 S 2 marche, ou 2 est eof (=> parse_stack.halt_hit == TRUE) mais
       modele invalide' (pas de faute d'orthographe sur un mot-cle'). */
    /* parse_stack.halt_hit = FALSE; deja fait */
    return ret_val;
}


static int dont_delete (model_no)
    int model_no;
{
    ...
    int 	*model = sxplocals.SXP_tables.P_lregle [model_no];
    int		lmod = (int) model [0];
    int		n = (int) model [lmod];
    int		i, x, lahead;
...
static BOOLEAN check_for_spelling (model_no)
    int model_no;
{
    /* Retourne TRUE ssi ce n'est pas un modele de correction de faute d'orthographe
    ...
    int 	*model = sxplocals.SXP_tables.P_lregle [model_no];
    int 	lim = sxplocals.SXP_tables.P_right_ctxt_head [model_no];
    int 	val;
    int		x;
...
static SXVOID compute_vt_set (vt_set, parser)
    SXBA	vt_set;
    int		parser;
{
    int xmscan, scan_ref;
...
{
    int		z, i, model_no, parser, parser_seed;
    BOOLEAN	is_a_0_model;
   ...
   le scanner est non-deterministe. */
	...
	if ((i = dont_delete (model_no)) >= 0) {
		...
		if (check_for_spelling (model_no)) {
			...
			if (parser_seed > 0 && try_a_model (is_a_0_model ? 1 : 0,
							    parser_seed,
							    model_no,
							    FALSE,
							    0))
    ...
    int		ref, tnt;
{
	...
	if (state != fathers_state /* chemin trouve par le pere */
	    ...
	    mreds_push (father, 0 /* ref */, parse_stack.current_token, FALSE, FALSE, 0);
#if 0
    ...
    for (;;) {
	set_next_shift_state (son, fathers_state, (int) aprdct->action, tnt);
...
static SXVOID process_sons (father, grand_fathers_state)
    int		father, grand_fathers_state;
{
...
static BOOLEAN is_a_right_ctxt (head, tail)
    int head, tail;
{
    ...
    int current, parser, xmscan, scan_ref;
...
{
    int				z, dd, model_no, parser_seed, p2_ms;
    BOOLEAN			is_local_correction;
	...
	if (check_for_spelling (model_no)) {
		...
		if ((dd = dont_delete (model_no)) >= 0) {
		    ...
		    if ((model_no < parse_stack.rcvr.best.model_no
			 || !parse_stack.rcvr.dont_delete))
			is_local_correction =
			    try_a_model (1, prev_ms_ptr->next_for_scanner, model_no,
					 TRUE, parse_stack.rcvr.TOK [0]);
		}
	    }
	    else {
		XxYxZ_Zforeach (sxndtkn.dag, prev_ms_ptr->my_index, z) {
			...
			if ((model_no < parse_stack.rcvr.best.model_no
			    ...
			    if (parser_seed > 0
				&& try_a_model (0, parser_seed, model_no,
						TRUE, XxYxZ_Y (sxndtkn.dag, z))) {
...
static VOID nd2_unfold (prev_tok_no, mstn, n, lim)
    int prev_tok_no, mstn, n, lim;
{
    int 			xyz, tok_no;
...
static BOOLEAN 	global_recovery ()
{
    int	tail, x, top, p, current_milestone;
...
static int nd2_search_validation_context (orig_mstn_ptr, mstn_nb, n, lim)
    struct sxmilstn_elem 	*orig_mstn_ptr;
    int				mstn_nb, n, lim;
{
    
    struct sxmilstn_elem 	*mstn_ptr;
    int				xyz, orig_tok_no, ret_val, parser;
	...
	*parse_stack.for_scanner.next_hd = 0;
	...
	XxYxZ_Zforeach (sxndtkn.dag, orig_mstn_ptr->my_index, xyz) {
		...
		if (XxYxZ_Y (parse_stack.parsers, parser) == orig_tok_no)
		    process_sons (parser, 0 /* father's state : son is top state */);
	    }
	}

	return (int) global_recovery ();
    }
...
static int nd2_search_key_terminal ()
{
    /* Recherche d'un milestone dont est issu un terminal-cle */
    int				xyz, next_mstn_nb, mstn_nb, lahead, i, ret_val, tok_no;
...
static BOOLEAN	less_equal (z1, z2)
    int z1, z2;
{
...
static SXVOID nd2_local_correction ()
{
    ...
    int 			prev_ms_nb, cur_ms_nb, z, preds_nb, parser;
    struct sxmilstn_elem	*prev_ms_ptr;
	...
	if (sxmilstn_access (sxndtkn.milestones, prev_ms_nb).kind & MS_WAIT) {
		...
		parse_stack.rcvr.preds = (int*) sxrealloc (parse_stack.rcvr.preds,
							   (parse_stack.rcvr.preds_size *= 2) + 1,
							   sizeof (int));
		parse_stack.rcvr.to_be_sorted = (int*) sxrealloc (parse_stack.rcvr.to_be_sorted,
	...
	if ((sxplocals.mode.kind & SXWITH_CORRECTION)
	    ...
	    nd2_unfold (parse_stack.rcvr.TOK [0], cur_ms_nb, 1, sxplocals.SXP_tables.P_nmax);
...
static SXVOID sort_milestones ()
{
    ...
    int				parser, tde, my_milestone, node, prev_node;
...
static BOOLEAN	ndp_recovery ()
{
    char				*msg_params [5];
    struct SXP_local_mess		*local_mess;
    struct sxtoken			*tok;
    
    BOOLEAN			with_semact = sxplocals.mode.with_semact;
    BOOLEAN			ret_val = TRUE;
    int				mode = sxplocals.mode.mode;
    int				i, j, ate;
    int				vt_set_card;
	...
	if (parse_stack.rcvr.best.model_no == sxplocals.SXP_tables.P_nbcart + 1) {
	    ...
	    toks.string_table_entry = ERROR_STE /* error value */ ;
	    toks.comment = NULL;
		...
		for (;;) {
			...
			*parse_stack.for_scanner.next_hd = 0;
			...
			for (i = *parse_stack.for_scanner.current_hd;
			    ...
			    process_sons (i, 0 /* father's state : son is top state */);
			}
		...
		while (i-- > 0) {
		    sxplocals.rcvr.TOK_i = parse_stack.rcvr.TOK [i];
		    tok = &(SXGET_TOKEN (sxplocals.rcvr.TOK_i));
		    msg_params [i] = (ate = tok->string_table_entry) == EMPTY_STE
			|| ate == ERROR_STE
			    ? ttext (sxplocals.sxtables, tok->lahead)
			    : sxstrget (ate);
		}
...
BOOLEAN		sxndprecovery (what_to_do)
    int		what_to_do;
{
    static int	rcvr_shifts_foreach [] = {0, 0, 0, 0, 0, 0};
    
    switch (what_to_do) {
    case OPEN:
	parse_stack.rcvr.all_corrections = FALSE;
	...
	parse_stack.rcvr.with_parsprdct = TRUE;
	...
	if (sxplocals.rcvr.vt_set == NULL) {
	    ...
	    sxplocals.rcvr.Mvector = sxplocals.SXP_tables.vector + sxplocals.SXP_tables.Mxvec;
	    parse_stack.rcvr.preds = (int*) sxalloc ((parse_stack.rcvr.preds_size = 10) + 1,
						     sizeof (int));
	    parse_stack.rcvr.to_be_sorted = (int*) sxalloc (parse_stack.rcvr.preds_size + 1,
							    sizeof (int));
	    ...
	    parse_stack.rcvr.TOK = (int*) sxalloc (x + 1, sizeof (int));
	    parse_stack.rcvr.MS = (int*) sxalloc (x + 2, sizeof (int));
	    parse_stack.rcvr.best.TOK = (int*) sxalloc (x + 1, sizeof (int));
	    parse_stack.rcvr.best.MS = (int*) sxalloc (x + 2, sizeof (int));
	    
	    parse_stack.rcvr.index_0X = 0;

	    if ((sxplocals.mode.kind & SXWITH_CORRECTION) && sxplocals.SXP_tables.P_nbcart > 0) {
		int	sxndprecovery_index, model_no, best_l, mn, l, best_model_no;
		int	*m, *cur_model;
		...
		parse_stack.rcvr.best.correction =
		    (int*) sxalloc (sxplocals.SXP_tables.P_sizofpts + 1, sizeof (int));
		parse_stack.rcvr.prefixes = (int**) sxalloc (sxplocals.SXP_tables.P_nbcart + 1,
							     sizeof (int*));
		...
		for (model_no = 1; model_no <= sxplocals.SXP_tables.P_nbcart; model_no++) {
		    ...
		    parse_stack.rcvr.prefixes [model_no] = (int*)
			sxalloc (cur_model [0] + 1, sizeof (int));
...
static SXVOID models_dag ()
{
    ...
    */
    int 	*model, lim;
    int		elem, model_no, node, next_node, pos, xy, xsm, x, z, models_dag_index, node_set_size;
    int		*aux, *mm, *sorted_milestones;
    SXBA	to_be_processed, already_processed, back_bone, node_set;
    XxY_header	models_dag_var;

    static int tokens_dag_foreach [] = {1, 0, 1, 0, 0, 0};
...
/* On est dans le cas d'un scanner deterministe, on alloue "sxndtkn.dag" qui contiendra
   l'ensemble des "corrections". */
    ...
    XxYxZ_alloc (&sxndtkn.dag, "tokens_dag", 31, 1, tokens_dag_foreach, NULL, stdout_or_NULL); 
    XxY_alloc (&models_dag_var, "models_dag", 31, 1, 1, 1, NULL, NULL);
    XxY_alloc (&parse_stack.rcvr.correction_strings,
	       "correction_strings", 50, 1, 0, 0, NULL, NULL);
    ...
    aux = (int*) sxcalloc ((unsigned long int)sxplocals.SXP_tables.P_nmax + 1, sizeof (int));
    ...
    parse_stack.rcvr.mm = (int**) sxalloc (sxplocals.SXP_tables.P_nbcart + 1,
					   sizeof (int*));

    for (model_no = 1; model_no <= sxplocals.SXP_tables.P_nbcart; model_no++) {
	model = sxplocals.SXP_tables.P_lregle [model_no];
	mm = parse_stack.rcvr.mm [model_no] = (int*) sxalloc (sxplocals.SXP_tables.P_sizofpts + 1,
						  sizeof (int));
    ...
    sorted_milestones = (int*) sxalloc (parse_stack.rcvr.milstn_nb + 1,
							 sizeof (int));
...
BOOLEAN		sxndpallcorrections (what_to_do)
    int		what_to_do;
{
    int i;
	...
	parse_stack.rcvr.all_corrections = TRUE;
	return TRUE;
...
BOOLEAN		sxndpsrecovery (what_to_do)
    int		what_to_do;
{
    int		kind = sxplocals.mode.kind;
    BOOLEAN	ret_val;
