...
char WHAT_LFG_FILTERS_MNGR[] = "@(#)SYNTAX - $Id: lfg_filters_mngr.c 661 2007-06-27 12:21:59Z sagot $" WHAT_DEBUG;
...
BOOLEAN     structure_sem_pass_arg;
BOOLEAN     chunker_sem_pass_arg;
BOOLEAN     proper_sem_pass_arg;
...
static BOOLEAN call_sem_pass (call_sem_pass_for_semact)
...
static BOOLEAN call_sem_final (call_sem_final_for_semact)
...
timeout_mngr ()
{
  ...
  if (cur_spec != '\1') { /* si on timeout dans Earley, aucune sémantique catastrophe possible */
    ...
    while ((cur_spec = *map_spec++) && (cur_spec != '/'));
...
lfg_filter_mngr_sem_pass ()
{
  ...
  BOOLEAN    do_sthg [16], else_passed [16];
  char       msg [30];
  SXBA       sxba_tmp;
