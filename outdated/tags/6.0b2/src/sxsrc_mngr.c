...
/************************************************************************/
/* 20030311 17:09 (phd) :	Muselage du compilateur sur ce coup-là	*/
/************************************************************************/
...
char WHAT_SXSRC_MNGR[] = "@(#)SYNTAX - $Id: sxsrc_mngr.c 527 2007-05-09 14:02:47Z rlacroix $" WHAT_DEBUG;


#if	BUG

static SXVOID	srcdebug (format, arg)
    char	*format, *arg;
{
    register unsigned int	srcdebug_index;
    ...
    fprintf (sxstdout, "\tbuffer = 0x%lx [%d]", (long)sxsrcmngr.buffer, sxsrcmngr.buflength);
...
VOID	sxsrc_mngr (sxsrc_mngr_what, infile, name_or_string)
    int		sxsrc_mngr_what;
    FILE	*infile;
    char	*name_or_string;
{
    ...
    sprintf (header, "sxsrc_mngr (%d) %%ss with ", sxsrc_mngr_what);
    srcdebug (header, "begin");
