...
/*			    executions multiples dans le meme processus	*/
/************************************************************************/
...
char WHAT_SINGLE[] = "@(#)SYNTAX - $Id: single.c 605 2007-05-31 13:18:23Z rlacroix $" WHAT_DEBUG;

static int	xnt_trans, nt_trans_size, max_lhs;
static struct nt_trans {
	   int	lnk, check, action;
       }	*nt_trans;
static struct nt_etat {
	   int	lnk, lgth;
       }	*nt_etats;
static int	xnt_size, Delta, xntstr;
static int	*xnt_lnk;
static int	xnt_hash [HASH_SIZE];




static int	set_an_xnt (nt, prdct)
    int		nt, prdct;
{
	...
	xnt_lnk = (int*) sxrealloc (xnt_lnk, xnt_size, sizeof (int));
	bnf_ag.ADR = (int*) sxrealloc (bnf_ag.ADR - bnf_ag.WS_TBL_SIZE.xtmax - 1, bnf_ag.WS_TBL_SIZE.xtmax + bnf_ag.
	     WS_TBL_SIZE.ntmax + 3 + xnt_size, sizeof (int)) + bnf_ag.WS_TBL_SIZE.xtmax + 1;
    }
    ...
    if (xntstr + strlen (s) + 1 + (is_new_xnt ? 0 : get_nt_length (nt)) > (unsigned)bnf_ag.WS_TBL_SIZE.nt_string_length) {
    ...
    if (is_new_xnt)
	sprintf (bnf_ag.NT_STRING + xntstr, ">%d<", nt);
    else
	sprintf (bnf_ag.NT_STRING + xntstr, "%s &%d", get_nt_string (nt), prdct);
    ...
    int		nt, prdct;
{
	...
	xnt_lnk = (int*) sxcalloc (xnt_size, sizeof (int));
	bnf_ag.ADR = (int*) sxrealloc (bnf_ag.ADR - bnf_ag.WS_TBL_SIZE.xtmax - 1, bnf_ag.WS_TBL_SIZE.xtmax + bnf_ag.
	     WS_TBL_SIZE.ntmax + 3 + xnt_size, sizeof (int)) + bnf_ag.WS_TBL_SIZE.xtmax + 1;
...
static int	next (s, item)
    int		s, item;
{
    /* retourne goto* (s, item) */

    register int	symb, x, y, lim;

    if ((symb = bnf_ag.WS_INDPRO [item].lispro) == 0)
	return s;
	...
	for (x = S1 [x].ptnt; x < lim; x++) {
	    ...
	    if (bnf_ag.WS_INDPRO [tant [S1 [y].ptant]].lispro == symb)
		return next (S1 [y].tsucnt, item + 1);
	}
    }
    else {
	x = S1 [s].shftt;
	...
	for (x = S1 [x].ptt; x < lim; x++) {
	    ...
	    if (bnf_ag.WS_INDPRO [tat [S1 [y].ptat]].lispro == symb)
		return next (S1 [y].tsuct, item + 1);
	}
    }
...
static SXBA	pred (set, temp, lgth)
    SXBA	set, temp;
    int		lgth;
{
    ...
    for (s = sxba_scan (temp, 0); s != -1; s = sxba_scan (temp, s)) {
	for (x = S1 [s].pprd; x != 0; x = lstprd [x].f2) {
	    sxba_1_bit (set, lstprd [x].f1);
	}
    }
...
static void create_new_trans (aetat, A, prdct, action)
...
static int	complete_transition (state_set, noyau, temp_set, xnt_set1, xnt_set2, A, prdct, C, xAs, xCs)
    SXBA	state_set, noyau, temp_set, *xnt_set1, *xnt_set2;
    int		A, prdct, C;
    struct P_prdct	*xAs, *xCs;
{
    ...
    register int	i, j, k, xa, xc;
    register struct nt_etat	*aetat;
    register struct nt_trans	*atrans;
...
/* On regarde si A convient quand meme */
    ...
    for (i = sxba_scan (state_set, 0); i != -1; i = sxba_scan (state_set, i)) {
	...
	for (i = sxba_scan (state_set, 0); i != -1; i = sxba_scan (state_set, i)) {
	    ...
	    for (nt = sxba_scan (*xnt_set2, 0); nt != -1; nt = sxba_scan (*xnt_set2, nt)) {
		xa = 0;
	...
	for (Aprime = sxba_scan (*xnt_set1, 0); Aprime != -1; Aprime = sxba_scan (*xnt_set1, Aprime)) {
	    if (XNT_TO_PRDCT_CODE (Aprime) == -1)
		break;
	}

	if (Aprime == -1) {
	    Aprime = NTMAX = get_next_xnt (0, -1);
	    sxfree (*xnt_set1);
	    sxfree (*xnt_set2);
...
static int	nt_repr (ntpd, pd_to_pg)
    register int	ntpd, *pd_to_pg;
{
...

SXVOID
total_spe (total_spe_single_prod)
    SXBA	total_spe_single_prod;
{
...
/* On remplit head_tail et head_xtail
	...
	for (i = 1; i <= bnf_ag.WS_TBL_SIZE.xnbpro; i++) {
		...
		sxba_1_bit (head_xtail [j = bnf_ag.WS_NBPRO [i].reduc], k = bnf_ag.WS_INDPRO [k].lispro);
		sxba_1_bit (head_tail [j], XNT_TO_NT_CODE (k));
		sxba_1_bit (heads, j);
		sxba_1_bit (tails, k);
	    }
	}
	...
	while (!sxba_is_empty (heads)) {
	    ...
	    for (i = sxba_scan (heads, 0); i != -1; i = sxba_scan (heads, i)) {
		if (sxba_bit_is_set (origs, i) /* origine sans predecesseur */ ) {
		    line = head_xtail [i];
		    sxba_0_bit (heads, i);

		    for (j = sxba_scan (line, 0); j != -1; j = sxba_scan (line, j)) {
	...
	for (i = 1; i < xe1; i++) {
		...
		for (lim = (aitem = t_items + astatei->start) + astatei->lgth; aitem < lim; aitem++) {
		    ...
		    if (afe->codop == Reduce && bnf_ag.WS_NBPRO [afe->reduce].bprosimpl && !sxba_bit_is_set (already_found, afe->
			 reduce)) {
			sxba_1_bit (already_found, afe->reduce);
	...
	sp_to_t_state = (int*) sxcalloc (sp_t_occur_no + 1, sizeof (int));
...

	...
	int	lhs, rhs, xrhs, lim1, lim2, t, n, red, a, lhsprime, old_action, new_action;
	int	*la_to_lhs;
	struct P_prdct	*xAs, *xCs;
	register struct state	*astate;
	register struct P_item	*aitem, *lim;
	register struct floyd_evans	*afe;
	SXBA	pred_set, pred1_set, pred2_set, and_set, temp_set, la_red_set, xnt_set1, xnt_set2;

	pred_set = sxba_calloc (xe1 + 1);
	pred1_set = sxba_calloc (xe1 + 1);
	pred2_set = sxba_calloc (xe1 + 1);
	and_set = sxba_calloc (xe1 + 1);
	...
	la_to_lhs = (int*) sxcalloc (bnf_ag.WS_TBL_SIZE.xtmax + 1, sizeof (int));
	...
	for (i = sorted_single_prod [0].lnk; i != 0; i = sorted_single_prod [i].lnk) {
		...
		sxba_empty (pred_set);
		sxba_1_bit (pred_set, t = sp_to_t_state [j]);
		/* t est un etat ou la reduction no i apparait */
		pred_set = pred (pred_set, temp_set, 1);
...
/* On remplit la_to_lhs */
		...
		for (l = bnf_ag.WS_NTMAX [rhs].npg; l < lim2; l++) {
		    ...
		    for (s = sxba_scan (pred1_set, 0); s != -1; s = sxba_scan (pred1_set, s)) {
			/* goto (s, xrhs) = t */ 
			n = next (s, bnf_ag.WS_NBPRO [red].prolon);
...
/* Tous les etats de and_set sont traites simultanement (s en fait partie) */

			sxba_and (sxba_copy (and_set, pred1_set), pred2_set);
			sxba_minus (pred1_set, and_set);
...
/* On exploite la_red_set */
			...
			for (a = sxba_scan (la_red_set, 0); a != -1; a = sxba_scan (la_red_set, a)) {
			    sxba_0_bit (la_red_set, a);
			    lhs = la_to_lhs [a];
			    ...
			    lhsprime = complete_transition (pred2_set, and_set, temp_set, &xnt_set1, &xnt_set2, lhs,
...

...

	{
		...
		if ((astate = t_states + i)->lgth > 0) {
		    ...
		    for (j = sxba_scan (lhs_set, 0); j != -1; j = sxba_scan (lhs_set, j)) {
			sxba_0_bit (lhs_set, j);
			...
			for (k = sxba_scan (pred_set, 0); k != -1; k = sxba_scan (pred_set, k)) {
			    sxba_0_bit (pred_set, k);
...

	{
	    /* Suppression */
	    register int	i;
	    register struct nt_trans	*patrans, *natrans;
	    register struct nt_etat	*aetat;
		...
		if ((aetat = nt_etats + i)->lgth > 0) {
		    ...
		    for (natrans = nt_trans + aetat->lnk, patrans = NULL; natrans != nt_trans; natrans = nt_trans +
			 natrans->lnk) {
			if (!sxba_bit_is_set (nt_set, XNT_TO_NT_CODE (natrans->check))) {
			    aetat->lgth--;
			    nt_etats [0].lgth--;
...

	{
	    ...
	    for (i = sxba_scan (sp_states_set, 0); i != -1; i = sxba_scan (sp_states_set, i)) {
		astate = t_states + i;
	...
	register struct P_item	*aitem, *lim;
	register struct state	*astate;
	register struct P_item	*old_t_items = t_items;
...

SXVOID
partial_spe (partial_spe_single_prod)
    SXBA	partial_spe_single_prod;
{
  /*  ELIMINATION PARTIELLE DES PRODUCTIONS SIMPLES */

  int		*pd_set, *pd_to_pg, *pg_to_act, pg;
  struct P_item	**pd_to_aitem;
  struct state	*astatei;
  register struct P_item	*aitem, *bi, *bs;
  register struct floyd_evans		*afe;
  ...
  pd_to_pg = (int*) sxcalloc (bnf_ag.WS_TBL_SIZE.ntmax + 1, sizeof (int));
  pg_to_act = (int*) sxcalloc (bnf_ag.WS_TBL_SIZE.ntmax + 1, sizeof (int));
  xpd = 0;
  ...
  for (i = 1; i < xac2; i++) {
	...
	else if (!sxba_bit_is_set (nt_set, aitem->check) && (afe = nt_fe + aitem->action)->codop == Reduce && bnf_ag.
		 WS_NBPRO [afe->reduce].bprosimpl) {
