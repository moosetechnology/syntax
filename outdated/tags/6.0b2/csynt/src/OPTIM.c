...
char WHAT_OPTIM[] = "@(#)SYNTAX - $Id: OPTIM.c 607 2007-06-04 09:35:31Z syntax $" WHAT_DEBUG;
...
static int	put_in_reductions (put_in_reductions_hashs, put_in_reductions_lnks, afe)
    register struct floyd_evans		*afe;
    int		*put_in_reductions_hashs, *put_in_reductions_lnks;
{
...
static BOOLEAN	is_0_indistinguable (afei, afej)
...
static BOOLEAN	is_k_indistinguable (afei, afej)
    register struct floyd_evans		*afei, *afej;
{
...
static BOOLEAN	is_stable (equiv_class, states, items, fe, partitions, eq_lnks, partition_no)
    register int	*equiv_class;
    struct state	*states;
    struct P_item	*items;
    struct floyd_evans	*fe;
...
static SXVOID	insert_xt_lnk (xt, xt_to_act)
    register int	xt;
    register struct xt_to_act	*xt_to_act;
{
...
/* Le floyd_evans des transitions terminales, non-terminales et des predicats est regenere
   dans une structure unique tnt */
	...
	tnt_lnks = (int*) sxalloc (i, sizeof (int));
	tnt_fe_to_new = (int*) sxcalloc (i, sizeof (int));
	reductions = (struct P_reduction*) sxalloc (i, sizeof (struct P_reduction));
	lnks = (int*) sxalloc (i, sizeof (int));
...
/*   L I N E A R I S A T I O N   D E S   M A T R I C E S   */
...

	{
	    ...
	    algoV (FALSE, xe1, bnf_ag.WS_TBL_SIZE.xtmax, xt_items, t_states, t_items,
		   t_bases, &vector, &bases_set, &MIN, &MAX, &delta);
...
/* On prend comme defaut l'adresse la plus populaire et en cas d'egalite la plus
   elevee. */ 

	{
	    ...
	    algoV (TRUE, xac2, bnf_ag.WS_TBL_SIZE.xntmax, xnt_items, nt_states, nt_items,
		   nt_bases, &vector, &bases_set, &MIN, &MAX, &delta);
...
/* Permet, a l'analyse (par dichotomie), de retrouver un predicat a partir
   du couple (red_no, position). */
	...
	int	*old_t_state_to_new, *old_nt_state_to_new;
	SXBA	state_set;
...
/* On remplit new_t_state_to_old, new_nt_state_to_old, old_t_state_to_new et old_nt_state_to_new*/
	...
	new_t_state_to_old = (int*) sxcalloc (xe1 + 1, sizeof (int));
	new_nt_state_to_old = (int*) sxcalloc (xac2 + 1, sizeof (int));
	old_t_state_to_new = (int*) sxcalloc (xe1 + 1, sizeof (int));
	old_nt_state_to_new = (int*) sxcalloc (xac2 + 1, sizeof (int));
