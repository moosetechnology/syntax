...
#include "D_tables.h"
char WHAT_LALR1[] = "@(#)SYNTAX - $Id: LALR1.c 628 2007-06-13 11:05:36Z rlacroix $" WHAT_DEBUG;

extern BOOLEAN	prio_read (), prio_free ();
extern VARSTR	prod_to_str ();
extern VARSTR	path ();
extern BOOLEAN	check_lr1 ();
extern BOOLEAN	check_ambig ();
extern BOOLEAN	open_listing ();
extern VARSTR	cat_t_string ();
extern char	*laset ();
static struct priority	*t_priorities /* 1:xtmax */ ;
static struct priority	*r_priorities /* 1:xnbpro */ ;
static struct H1 {
	   int	hshr, hshat, hsht, hshant, hshnt;
       }	*H1 /* -31:lgt (1) */ ;
static struct S3 {
	   int	pkod, kod;
       }	*S3 /* 0:lgt (3) */ ;
static int	*tr /* 1:lgt (4) */ ;
static int	*ptrans /* 1:lgt (7) */ ;
static char	star_71 [] = "***********************************************************************";
static BOOLEAN	is_lalr1, is_lr1, is_ambiguous;
static int	limitem, limt, limnt, limpro;
static int	lgt [13];
static int	usr, ssr, urr, srr;
static int	xtr2, xlstprd, xptr;
static int	xlas, xplxi, Mt_items, Mnt_items;
static int	*item /* 0:indpro */ ;
static int	xptr2;
static int	*tsymb1 /* limt:xntmax */ ;
static int	*tsymb2 /* limt:xntmax */ ;
static int	xpter, xpconf, xkod;
static int	plulong;
static int	*pter /* 1:xtmax */ ;
static int	*pconf /* 1:xtmax */ ;
static int	*ter /* -xtmax:-1 */ ;
static int	*conf /* -xtmax:-1 */ ;
static int	*plxi /* 1:xtr2 */ ;
...
static struct {
	   int	conflict_kind /* SHIFT_REDUCE, REDUCE_REDUCE */ ;
	   int	desambig_rule_kind /* USER, SYSTEM */ ;
	   int	action_taken
...
static int	max_red_per_state, cur_red_no, base_xlas;
/* static SXBA	*FIRST; *NV* */
static int	xor_hash_size;
static int	*xor_hash;
static int	**Titems_set, *items_set;
...
static void
oflw_STATExNT2tnt (old_size, new_size)
    int		old_size, new_size;
{
    STATExNT2tnt = (int*) sxrealloc (STATExNT2tnt, new_size + 1, sizeof (int));
}



#define maj(x,n)	if (++*(x) > lgt [n]) sature (n); else ((void)0)
#define majc1(x)	if (++*(x) > lgt [1]) {sature (1); aS1xac1=S1+xac1;} else ((void)0)

static SXVOID	sature (sature_nbt)
    register int	sature_nbt;
{
	...
	tr = (int*) sxalloc (lgt [4] + 1, sizeof (int));
	includes = (struct include*) sxalloc (lgt [5] + 1, sizeof (struct include));
	lstprd = (struct lstprd*) sxalloc (lgt [6] + 1, sizeof (struct lstprd));
	ptrans = (int*) sxalloc (lgt [7] + 1, sizeof (int));
	tt = (int*) sxalloc (lgt [8] + 1, sizeof (int));
	tat = (int*) sxalloc (lgt [9] + 1, sizeof (int));
	tant = (int*) sxalloc (lgt [10] + 1, sizeof (int));
	tnt = (int*) sxalloc (lgt [11] + 1, sizeof (int));
	tnt_next = (int*) sxalloc (lgt [11] + 1, sizeof (int));
	nucl = (int*) sxalloc (lgt [12] + 1, sizeof (int));
	break;
    ...
    case 4:
	tr = (int*) sxrealloc (tr, new_size + 1, sizeof (int));
	break;
    ...
    case 7:
	ptrans = (int*) sxrealloc (ptrans, new_size + 1, sizeof (int));
	break;

    case 8:
	tt = (int*) sxrealloc (tt, new_size + 1, sizeof (int));
	break;

    case 9:
	tat = (int*) sxrealloc (tat, new_size + 1, sizeof (int));
	break;

    case 10:
	tant = (int*) sxrealloc (tant, new_size + 1, sizeof (int));
	break;

    case 11:
	tnt = (int*) sxrealloc (tnt, new_size + 1, sizeof (int));
	tnt_next = (int*) sxrealloc (tnt_next, new_size + 1, sizeof (int));
	break;

    case 12:
	nucl = (int*) sxrealloc (nucl, new_size + 1, sizeof (int));
	break;
    default:
#if EBUG
    ...
    int state, xtnt;
{
    ...
    int		xnt = bnf_ag.WS_INDPRO [tant [S1 [tnt [xtnt]].ptant]].lispro;
    int 	nt = XNT_TO_NT_CODE (xnt), couple;
    int		next = (XxY_set (&STATExNT_hd, state, nt, &couple)) ? STATExNT2tnt [couple] : 0;
...
static BOOLEAN	is_old_StNt (state, nt, StNt)
    register int	state;
    int		nt;
    register int	*StNt;
{
    ...
    register struct xor_StNt	*aStNt;
...
static VOID	put_r_in_includes (X1, X2)
    int		X1;
    register int	X2;
{
...
#if 0
int	state_nt_to_items (state, nt)
    int		state, nt;
{
    ...
    register int	bottom, top, xtnt, mid_nt;
...
static VOID	lookback (state, item, StNt)
    int		state, item, StNt;
{
...
static VOID	lookback (state, lookback_item, StNt)
    int		state, lookback_item, StNt;
{
    /* Cette fonction recursive calcule la relation binaire includes entre des couples
       (StNt, StNt1) ou chaque StNt est un couple (etat, nt)
       StNt = (s, nt) includes StNt1 = (s1, nt1) ou
       "nt -> . alpha" et "nt1 -> delta . nt gamma" sont dans s
       s = goto*(s1, delta), et gamma =>* epsilon */
    int				x, nt, xnt, StNt1, couple, xtant, lim, lookback_xptant;
    ...
    int		xac1;
{
    ...
    if ((y = S1 [y].ptr) < lim) {
	int	ntbidon = bnf_ag.WS_TBL_SIZE.xntmax;
	register int	*orig = plxi + xplxi;

	for (; y < lim; y++, orig++, xplxi++) {
	    register int	compute_includes_item = tr [y];
...
static int	isint (t, kind, xpt, hsh)
    int		*t, xpt, hsh;
    register int	kind;
{
    int		m1, taille, taille1, m;
    register int	i, x, y;
...
static SXVOID	sornt (x)
    register int	x;
{
    int		z, suc;
    register int	lim, lim1, y;
...
static SXVOID	sort (x)
    register int	x;
{
    int		lim, lim1;
    register int	z, suc, y;
    char	s1 [12];
...
static SXVOID	set_chosen (drk, at, rn, p)
    int		drk, at, rn;
    struct priority	p;
{
...
BOOLEAN		t_in_deriv (t, t_in_deriv_item)
    register int	t, t_in_deriv_item;
{
    /* Teste si le terminal "t" appartient a S avec
       S = FIRST (beta) et t_in_deriv_item == A->alpha . beta */

    register int	t_in_deriv_tnt;
...
static VOID	cutback (St1, cutback_item, StNt, q, q1, to_be_cut)
    int		St1, cutback_item, StNt, q, q1;
    BOOLEAN	to_be_cut;
{
    register int	x, St;
    int		StNt1;
    ...
    int		St, q, q1;
{
    ...
    register int	StNt, cut_xptant, xtant, lim, cut_item;
...
static int	state_t_to_items (state, xt)
    int		state, xt;
{
    /* Recherche dans l'etat "state" la liste des items dont le marqueur LR repere
       le terminal "xt" */
    ...
    register int	bottom, top, xtt, mid_xt;
...
VOID	fill_items_set (fill_items_set_nt_trans_set, fill_items_set_nt_trans_stack, nt_trans)
    register int	*fill_items_set_nt_trans_set, *fill_items_set_nt_trans_stack;
    int		nt_trans;
{
    ...
    register int	xpnt1, xpnt2, x, xtant;
    int		X2, fill_items_set_xptant, lim, fill_items_set_item;
...
static int
equiv_rule_processing (StNt, lim, equiv_rule_processing_xlas, equiv_rule_processing_cur_red_no)
    int		StNt, lim, equiv_rule_processing_xlas, equiv_rule_processing_cur_red_no;
{
...
static SXVOID	fill_look_ahead_set (fill_lookahead_set_items_set, la_set)
    register int	*fill_lookahead_set_items_set;
    register SXBA	la_set;
{
    ...
    register int	x, fill_lookahead_set_tnt, fill_lookahead_set_item;
    ...
    char	*language_name;
{
    ...
    {
	register int	NBPRO, INDPRO, INDPROt3, INDPROt2;
	int	INDPROd2, INDPROt6, NBPROd2;
    ...
    STATExNT2tnt = (int*) sxalloc (XxY_size (STATExNT_hd) + 1, sizeof (int));
...
/* Construction de l'automate LR (0) */
	...
	item = (int*) sxalloc (limitem, sizeof (int));
	tsymb1 = (int*) sxalloc (bnf_ag.WS_TBL_SIZE.xntmax - limt + 1, sizeof (int)) - limt;
	/* limt est negatif (-xtmax-1) */
	tsymb2 = (int*) sxalloc (bnf_ag.WS_TBL_SIZE.xntmax - limt + 1, sizeof (int)) - limt;
...
/* limt est negatif (-xtmax-1) */
	...
	{
	    register int	xac1, lim;

	    for (xac1 = 1; xac1 < xac2; xac1++) {
		register struct S1	*aS1xac1;
...

		{
		    ...
		    if ((aS1xac1->pred = isint (tr, 1, k, hsh)) != k) {
			xtr2 = xtr1;
			xptr = k;
		    }
		    else {
		...
		{
		    register int	m1, m2;
		    int		hsh, hsh1, xtt1, xtat1, k, xtant1, xtnt1, x;
		    ...
		    if ((aS1xac1->shftt = isint (tt, 3, k, hsh1)) != k) {
			xtt2 = xtt1;
			xptt = k;
		    }
...
filsuc:				S1 [k].tsucnt = suc;
		    ...
		    if ((x = aS1xac1->shftnt = isint (tnt, 5, k, hsh1)) != k) {
			xtnt2 = xtnt1;
			xptnt = k;
		    }
...

    ...
    plxi = (int*) sxalloc (reduce_item_no + 1, sizeof (int));
	...
	/* *NV* */
	{
	    ...
	    register SXBA	*DEB_s, *BA_s, bidon, line;
...
/* Calcul de la relation includes */
	...
	nt_trans_set = (int*) sxcalloc (xxor_StNt + 1, sizeof (int));
	nt_trans_set [0] = -1;
	nt_trans_stack = (int*) sxalloc (xxor_StNt + 1, sizeof (int));
...

	if (is_lr1_constructor) {
	    ...
	    int	        xac1;
	    int		old_xac2, old_reduce_item_no;
	    int	        x, xt1, xtr1, base_xtr1, lim, lim_xtr1, t, l, l2, LALR1_xtr2;
	    SXBA	la_set_r1;
	    int		r1;
...
/* On regarde s'il y a un conflit Shift/Reduce, si oui => non LR(1) */
		...
		if (xac2 > old_xac2) {
		    plxi = (int*) sxrealloc (plxi, reduce_item_no + 1, sizeof (int));
...
/* Mise a jour de la relation includes */
		    ...
		    xor_hash_size = xac2 + bnf_ag.WS_TBL_SIZE.ntmax + max_red_per_state + 1;
		    xor_hash = (int*) sxrealloc (xor_hash, xor_hash_size, sizeof (int));

		    {
			register int	*p, *LALR1_lim;
		    ...
		    nt_trans_set = (int*) sxrealloc (nt_trans_set, xxor_StNt + 1, sizeof (int));

		    {
			register int	*p, *LALR1_lim_2;
		    ...
		    nt_trans_set [0] = -1;
		    nt_trans_stack = (int*) sxrealloc (nt_trans_stack, xxor_StNt + 1, sizeof (int));
		    /* reallocations */
		    path_reallocation (bnf_ag.FIRST);
		}
	    }
...

	...
	for (xac1 = 1; xac1 < xac2; xac1++) {
	    register struct S1	*aS1xac1 = S1 + xac1;
...
#define garde_fou (Mt_items+2*xac2)
	...
	nt_fe = (struct floyd_evans*) sxalloc (xptant + xe1 /* pour all_single */  + 1 /* error */ + 1,
					       sizeof (struct floyd_evans));
	nt_lnks = (int*) sxalloc (xptant + xe1 + 1 + 1, sizeof (int));
	tant_fe = (int*) sxcalloc (xptant + 1, sizeof (int));
...
/* Floyd_Evans des transitions terminales */
	...
	pter = (int*) sxalloc (-limt, sizeof (int));
	pconf = (int*) sxalloc (-limt, sizeof (int));
	ter = (int*) sxcalloc (-limt, sizeof (int)) - limt;
	conf = (int*) sxcalloc (-limt, sizeof (int)) - limt;
...

	{
		...
		/* Version 3.9 */
		{
			...
			for (; xtr1 < lim; xtr1++) {
			    ...
			    register SXBA	la_set = look_ahead_sets [xlas];
...
*/
		...
		if (xpconf > 0) {
			...
			conf [t] = 0;
			set_chosen (USER, REDUCE, 0, UNDEF_PRIO);

			if (chosen.conflict_kind == SHIFT_REDUCE) {
			    current_prio = t_priorities [-t];

			    if (current_prio.assoc == UNDEF_ASSOC)
				set_chosen (SYSTEM, SHIFT, 0, UNDEF_PRIO);
			    else
				set_chosen (USER, SHIFT, 0, current_prio);
			}
...

			{
				...
				if (current_prio.assoc == UNDEF_ASSOC || chosen.desambig_rule_kind == SYSTEM) {
					...
					set_chosen (SYSTEM, SHIFT, 0, current_prio);
				}
				else {
					...
					set_chosen (USER, REDUCE, reduce, current_prio);
				      ...
				      if (chosen.action_taken == SHIFT) {
						...
						if (current_prio.assoc == LEFT_ASSOC)
						    ...
						    /* nonassoc */
						    set_chosen (USER, ERROR_ACTION, 0, current_prio);
					    }
					    else
						/* REDUCE, ERROR_ACTION,
					       SYSTEM_ACTION */
						set_chosen (USER, SYSTEM_ACTION, 0, current_prio);
				      }
				}
   ...
   Elaboration des messages d'erreur */
			...
			if (is_listing_opened && ((chosen.desambig_rule_kind == SYSTEM && is_list_system_conflicts) ||
				...
				if (!is_message && xac1 != 1) {
				    is_message = TRUE;
				    put_edit_ff ();
				    fprintf (listing, "SAMPLE PATH from state 1 to %d\n\n", xac1);
			    ...
			    fprintf (listing, "    N O T    %sL R (1)\n", chosen.conflict_kind == SHIFT_REDUCE ? "" :
				 "L A ");
			    fprintf (listing, "%s-Reduce conflict in state %d on %s between:\n", chosen.conflict_kind ==
				 SHIFT_REDUCE ? "Shift" : "Reduce", xac1, varstr_tostr (cat_t_string (varstr_raz (vstr),
				 t)));
			}

			{
			    int		LALR1_xptat;
			    int		bxac1 = S1 [S1 [xac1].pred].ptr;

			    LALR1_xptat = tt [-ter [t]];

			    {
				register int	y, LALR1_item;
...
/* L'action reduce si elle existe sera generee en fin d'etat */
					...
					register int	*LALR1_items_set = Titems_set [cur_red_no];
...
/* On regarde si la grammaire est ambigue (une condition suffisante) */
				...
				register int	y, LALR1_item_2;
				BOOLEAN		is_local_ambiguity = FALSE;

				y = 0;

				while ((y = S3 [y].pkod) != 0) {
				    register int	*LALR1_items_set_2 = Titems_set [cur_red_no = S3 [y].kod - bxac1];
...
/* On regarde si la grammaire est LR(1) et si non on dit pourquoi */
			...
			if (is_listing_opened && ((chosen.desambig_rule_kind == SYSTEM && is_list_system_conflicts) ||
				...
				if (chosen.action_taken == SHIFT)
				    ...
				    fprintf (listing, "reduction number %d.\n", chosen.reduce_no);
			    }
			}
		    }
		}
...
/* S'il y a au moins un terminal etendu dans le coup, on ne fait rien; on laisse
   l'optimiseur s'en occuper */
		...
		{
		    int	n, xtr1, lim, cur_nbt, red_no, x;
...
/* Recapitulatif au terminal des conflits */
	...
	if (!is_lalr1) {
	    ...
	    sprintf (results, "   %-4d   | %-4d |  %-4d  | %-4d |  %-4d\n", usr + ssr + urr + srr, usr, ssr, urr, srr);
	...
	if (is_automaton && open_listing (1, language_name, ME, ".la.l")) {
	    register VARSTR	varstring;
	    register int	xac1, x, z;
	    int		lim;
		...
		if (xac1 != 1) {
			...
			put_edit_apnb (lstprd [S1 [xac1].pprd].f1);
			put_edit_nl (1);
		    }
		    else {
			register int	j;
