...
char WHAT_SXLIST_MNGR[] = "@(#)SYNTAX - $Id: sxlist_mngr.c 981 2008-02-05 13:13:21Z rlacroix $" WHAT_DEBUG;
...
BOOLEAN
sxlist_write (sxlist_header *header, filedesc_t file_descr)
{
    return
	     ...
	     FILE *stat_file)
{
	...
	&& READ (header->hd_to_list = (SXINT*) sxalloc (X_size (header->hd) + 1, sizeof (SXINT)),
		 sizeof (SXINT) * (X_top (header->hd) + 1))
	&& sxindex_read (&(header->list_hd), file_descr, NULL)
	...
	&& sxindex_read (&(header->elem_hd), file_descr, NULL)
	&& READ (header->buckets =
...
print_list_to_attr (SXINT x)
{
	...
	fprintf (_file, "{%ld, %ld, %ld, %ld}, ",
		 (long)_p_hd->list_to_attr [x].next_list,    
		 (long)_p_hd->list_to_attr [x].scrmbl,    
		 (long)_p_hd->list_to_attr [x].list_lgth,    
		 (long)_p_hd->list_to_attr [x].first_elem);    
}
...
print_buckets (SXINT x)
{
	...
	fprintf (_file, "{%ld, %ld}, ",
	 (long)_p_hd->buckets [x].next,    
	 (long)_p_hd->buckets [x].elem);    
}
...
sxlist_out_tab_int (FILE *file, char *name, SXINT *tab, SXINT size, SXINT top)
{
    ...
    fprintf (file, "\n\nstatic SXINT %s [%ld] = {", name, (long)size + 1);
...
void
sxlist_to_c (sxlist_header *header, FILE *file, char *name, BOOLEAN is_static)
{
