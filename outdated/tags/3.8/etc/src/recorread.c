...
static char	what [] = "@(#)recor_read.c\t- SYNTAX [unix] - 31 Mai 1988";
...
#define READ(f,p,l)		\
	if (bytes = (l), (read (f, p, bytes) != bytes))		\
		goto read_error; else
#define BA_READ(f, b)	\
	if (!sxba_read (f,b))					\
		goto read_error


BOOLEAN		recor_read (R_tables, langname)
    register struct R_tables_s	*R_tables;
    char	*langname;
{
    register int	bytes;
    register int	i;
    register int	/* file descriptor */ F_rt;
    int		rt_version;
    char	ent_name [128];
...
/* allocation et lecture des tables de recor */
    ...
    if ((F_rt = open (strcat (strcpy (ent_name, langname), ".rt"), 0)) < 0) {
	fprintf (sxstderr, "%s: cannot open (read) ", ME);
	goto file_error;
    }

    READ (F_rt, &(rt_version), sizeof (int));
    ...
    R_tables->S_lregle = R_tables->R_tbl_size.S_nbcart == 0 ? NULL : (int**) sxalloc (R_tables->R_tbl_size.S_nbcart + 1, sizeof (int*));

    for (i = 1; i <= R_tables->R_tbl_size.S_nbcart; i++)
	READ (F_rt, R_tables->S_lregle [i] = (int*) sxalloc (R_tables->R_tbl_size.S_maxlregle + 1, sizeof (int)), (
	     R_tables->R_tbl_size.S_maxlregle + 1) * sizeof (int));
    ...
    for (i = 1; i <= R_tables->R_tbl_size.S_nbcart; i++)
	READ (F_rt, &(R_tables->SXS_local_mess [i].param_no), sizeof (int));
    ...
    R_tables->P_lregle = R_tables->R_tbl_size.P_nbcart == 0 ? NULL : (int**) sxalloc (R_tables->R_tbl_size.P_nbcart + 1, sizeof (int*));

    for (i = 1; i <= R_tables->R_tbl_size.P_nbcart; i++)
	READ (F_rt, R_tables->P_lregle [i] = (int*) sxalloc (R_tables->R_tbl_size.P_maxlregle + 1, sizeof (int)), (
	     R_tables->R_tbl_size.P_maxlregle + 1) * sizeof (int));

    R_tables->P_right_ctxt_head = R_tables->R_tbl_size.P_nbcart == 0 ? NULL : (int*) sxalloc (R_tables->R_tbl_size.P_nbcart + 1, sizeof (int));

    if (R_tables->R_tbl_size.P_nbcart != 0)
	READ (F_rt, R_tables->P_right_ctxt_head, (R_tables->R_tbl_size.P_nbcart + 1) * sizeof (int));
	...
	READ (F_rt, &(R_tables->SXP_local_mess [i].param_no), sizeof (int));
    ...
    if (R_tables->R_tbl_size.E_labstract != 0)
        READ (F_rt, R_tables->E_abstract = sxalloc (R_tables->R_tbl_size.E_labstract, sizeof (char)), R_tables->R_tbl_size.
	 E_labstract * sizeof (char));
    else
