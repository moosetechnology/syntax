...
static char	what [] = "@(#)recor_write.c\t- SYNTAX [unix] - 31 Mai 1988";
...
BOOLEAN		recor_write (R_tables, langname)
...
/* sortie des tables de recor */
    ...
    if ((F_rt = creat (strcat (strcpy (ent_name, langname), ".rt"), 0666)) < 0) {
    ...
    WRITE (F_rt, &(rt_version), sizeof (int));
	...
	WRITE (F_rt, R_tables->S_lregle [i], (R_tables->R_tbl_size.S_maxlregle + 1) * sizeof (int));

    for (i = 1; i <= R_tables->R_tbl_size.S_nbcart; i++)
	WRITE (F_rt, &(R_tables->SXS_local_mess [i].param_no), sizeof (int));
	...
	WRITE (F_rt, R_tables->P_lregle [i], (R_tables->R_tbl_size.P_maxlregle + 1) * sizeof (int));

    if (R_tables->R_tbl_size.P_nbcart != 0)
	WRITE (F_rt, R_tables->P_right_ctxt_head, (R_tables->R_tbl_size.P_nbcart + 1) * sizeof (int));

    for (i = 1; i <= R_tables->R_tbl_size.P_nbcart + 1; i++)
	WRITE (F_rt, &(R_tables->SXP_local_mess [i].param_no), sizeof (int));
