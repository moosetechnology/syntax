...
#ifndef lint
#ifdef	SIMPLE
char	what_sxerr_mngr [] = "@(#)sxerrmngr.s.c\t- SYNTAX [unix] - 16 Aout 1990";
#else
char	what_sxerr_mngr [] = "@(#)sxerrmngr.c\t- SYNTAX [unix] - 16 Aout 1990";
#endif
#endif

#include <stdio.h>  /* pour fileno() */
#ifndef fileno
   /* fileno() n'est pas ANSI mais disponible sur la plupart des OS */
...
extern SXVOID sort_by_tree (int *, int, int, BOOLEAN (*) ());
...
VOID	sxput_error (source_index, format, arg1, arg2, arg3, arg4, arg5, arg6)
...
VOID	sxhmsg (file_name, format, arg1, arg2, arg3, arg4, arg5, arg6)
    char	*file_name;
    char	*format;
    char	*arg1, *arg2, *arg3, *arg4, *arg5, *arg6;
{
...
VOID	sxtmsg (file_name, format, arg1, arg2, arg3, arg4, arg5, arg6)
    char	*file_name;
    char	*format;
    char	*arg1, *arg2, *arg3, *arg4, *arg5, *arg6;
{
...
static BOOLEAN	find_line (infile, line_no)
    register FILE	*infile;
    register unsigned long	line_no;
{
...
static BOOLEAN	compute_lines (infile, source_index)
    struct sxsource_coord	source_index;
    register FILE	*infile;
{
#define blanks_len	(sizeof (blanks) - 1) /* MUST be >= TAB_INTERVAL */
    static char		blanks [] = "               ";
    register unsigned int	char_count /* number of characters read */ ;
    register int	line_len /* size of output line */ ;
    register int	mark_pos /* whitespace length before the marker */ ;
	...
	if (++char_count == source_index.column) {
	    mark_pos = line_len;
	}
	...
	default:
		...
		fputs (str = sxc_to_str [ch], sxerrmngr.scratch_file);
		line_len += strlen (str);
	    }
...
static BOOLEAN	output_marker ()
{
    ...
    /* patch by Marc.Herbert@inria.fr (projet VASY) Feb 10 2000
       It is necessary to insert the following "fseek" call, because it is
       forbidden to alternate reads and writes on a file without doing
       a system call such as fseek() in between. This problem is well 
       documented in the "fopen" man page of Linux RedHat 6.1. On Solaris
       2.5 and 2.6 (and perhaps later), this omission caused a bug in the
       display of error messages (some erroneous source lines in the program
       compiled by the Syntax-generated compiler were not displayed); this
       problem was caused by the fact that the sxerrmngr.scratch_file kept
       a EOF status even if data were written at the end of the file) */
...
static BOOLEAN	find_source (source_index)
    struct sxsource_coord	source_index;
{
    register FILE	*infile;
    register BOOLEAN	code;
    register long	here;


/* First try opening a new file */

    if (source_index.file_name [0] != NUL && (infile = fopen (source_index.file_name, "r")) != NULL) {
...
static VOID	keep_message (source_index, severity)
    struct sxsource_coord	source_index;
    int		severity;
{
...
static BOOLEAN	errlessp (i1, i2)
    register int	i1, i2;
{
    register struct sxerr_info	*r1, *r2;
...
VOID	sxerrsort ()
{
    register int	*sorted /* temporary array */ ;
    ...
    sorted = (int*) sxalloc (sxerrmngr.err_kept, sizeof (int));
 ...
 */
	...
	register int	hole, next;
	register int	index /* looks at each message in turn */ ;

	for (index = sxerrmngr.err_kept - 1; index >= 0; index--) {
	    if (sorted [index] == index) {
		/* Already in its final place */
		continue;
	    }

	    temp = sxerrmngr.err_infos [hole = index] /* Create a hole */ ;
