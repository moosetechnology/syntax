...
/************************************************************************/
/* 21-01-94 14:45 (pb):		Caractere change de "char" en "int"	*/
/************************************************************************/
/* 21-10-93 13:00 (phd&pb):	Adaptation a OSF			*/
/************************************************************************/
/* 15-04-92 13:40 (pb):		Etiquette ccnewlextok			*/
/************************************************************************/
/* 30-03-92 11:40 (pb):		Utilisation de sxsvar.sxlv.mode		*/
/************************************************************************/
...
#ifndef lint
#ifdef	BUG
char	what_sxscanner [] = "@(#)sxscanner.d.c\t- SYNTAX [unix] - Ven 21 Jan 1994 14:45:24";
#else
char	what_sxscanner [] = "@(#)sxscanner.c\t- SYNTAX [unix] - Ven 21 Jan 1994 14:45:24";
#endif
#endif
...
static int	ind_incl, ind_incl_use;
...
VOID	sxscan_it ()
{
    register SXSTMI	stmt;
    register int	current_state;
    register unsigned char	current_class;
...
SWITCH:
    ...
    case SameState:
	{
	    register int	seed_class;
	    register SXSTMI	*ctmline = sxsvar.SXS_tables.S_transition_matrix [current_state];
	    register SXSTMI	seed_stmt = stmt;
...
read_la_transition:
	...
	if (action_or_prdct_code->is_predicate) {
	    /* transition vers un groupe de predicats */
	    register BOOLEAN	is_prdct_satisfied = FALSE;

	    action_or_prdct_code--;

	    do {
		if ((++action_or_prdct_code)->is_system && sxsvar.sxlv.mode.with_system_prdct) {
		...
		switch (action_or_prdct_code->action_or_prdct_no) {
		    register char	sxchar;

		case LowerToUpper:
		    /* Lower_To_Upper (last char) */

		    if (isascii (sxchar = sxsvar.sxlv_s.token_string [sxsvar.sxlv.ts_lgth - 1]) && islower (sxchar))
			sxsvar.sxlv_s.token_string [sxsvar.sxlv.ts_lgth - 1] = sxchar + 'A' - 'a';
		...
		case UpperToLower:
		    ...
		    if (isascii (sxchar = sxsvar.sxlv_s.token_string [sxsvar.sxlv.ts_lgth - 1]) && isupper (sxchar))
			sxsvar.sxlv_s.token_string [sxsvar.sxlv.ts_lgth - 1] = sxchar - ('A' - 'a');
		...
		case UpperCase:
			...
			for (i = 0; i < sxsvar.sxlv.ts_lgth; i++) {
			    if (isascii (sxchar = sxsvar.sxlv_s.token_string [i]) && islower (sxchar))
				sxsvar.sxlv_s.token_string [i] = sxchar + ('A' - 'a');
			}
		...
		case LowerCase:
			...
			for (i = 0; i < sxsvar.sxlv.ts_lgth; i++) {
			    if (isascii (sxchar = sxsvar.sxlv_s.token_string [i]) && isupper (sxchar))
				sxsvar.sxlv_s.token_string [i] = sxchar - ('A' - 'a');
			}
...
# ifdef BUG
    ...
    printf ("\n\t\t\ttoken = (%d, \"%s\")\n", sxsvar.sxlv.terminal_token.lahead, sxttext (sxsvar.sxtables, sxsvar.sxlv.
	 terminal_token.lahead));
    printf ("\t\t\tstring = \"%s\"\n", sxsvar.sxlv.terminal_token.string_table_entry == EMPTY_STE ? "" : sxstrget (sxsvar.
	 lv.terminal_token.string_table_entry));
    printf ("\t\t\tsource_index = (%s, %d, %d)\n", sxsvar.sxlv.terminal_token.source_index.file_name, sxsvar.sxlv.
	 terminal_token.source_index.line, sxsvar.sxlv.terminal_token.source_index.column);
