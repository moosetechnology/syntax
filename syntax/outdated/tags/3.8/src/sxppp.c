...
#ifndef lint
#ifdef	BUG
char	what_sxppp [] = "@(#)sxppp.d.c\t- SYNTAX [unix] - Ven 22 Nov 1996 17:07:36";
#else
char	what_sxppp [] = "@(#)sxppp.c	\t- SYNTAX [unix] - Ven 22 Nov 1996 17:07:36";
#endif
#endif
...
static char	verbose_format [] = "\b\b\b\b\b\b%6d";
...
#include <stdarg.h>	/* pour definir ppabort() */
...
/***
   ...
   Dans /usr/include/ctype.h sur MacOS 10.2, isascii est defini par :
...
static struct saved_info {
	   ...
	   int	marge /*			current margin value		*/ ;
	   int	marge_erreur /*			current error margin value	*/ ;
       }	saved_info;
static int	save_stack_index;
static int	column /* Where shall the next character lay */ ;
static int	current_column /* Where we are going to write */ ;
...
static struct string_info {
	   unsigned int		ste /*		string table entry	*/ ;
	   long		lgth /*			string length		*/ ;
	   short	width /*		first line length	*/ ;
	   short	goes_to_column /*	last line length	*/ ;
	   short	lahead /*		how it was scanned	*/ ;
       }	string_info;



static VOID	string_modify (string, strl, its_case, must_darken)
    char	*string;
    long	strl;
    CASE	its_case;
    BOOLEAN	must_darken;
	...
	for (; (ch = *chp) != NUL; chp++) {
		...
		if (isalpha (ch)) {
			...
			*chp = toupper (ch);
		    ...
		    while (isascii (ch = *++chp) && isalpha (ch)) {
			if (isupper (ch)) {
			    *chp = tolower (ch);
			}
		    }
...
static VOID	string_manager (ste, lahead)
    unsigned int	ste;
    short	lahead;
{
#ifdef	BUG_STRINGS
    {
	printf ("string_manager (%d, %d):\t", ste, lahead);
    }
#endif
...
#ifdef	BUG_STRINGS
	{
	    printf ("Generic \"%s\" of length %d =>\t", chp, chl);
	}
#endif
	{
	    register char	*nlp;
	...
	{
	    register CASE	its_case;
	    register BOOLEAN	must_darken;
...
static VOID	kw_manager (lahead)
    short	lahead;
{
#ifdef	BUG_STRINGS
    {
	printf ("kw_manager (%d):\t", lahead);
    }
#endif
...
#ifdef	BUG_STRINGS
	{
	    printf ("Non-Generic \"%s\" of length %d =>\t", chp, chl);
	}
#endif
...

	{
	    ...
	    its_case = (sxppvariables.terminal_case != NULL ? sxppvariables.terminal_case [lahead] : (sxkeywordp (
		 sxppvariables.sxtables, lahead) ? sxppvariables.kw_case : NO_SPECIAL_CASE));
	    must_darken = (sxppvariables.terminal_dark != NULL ? sxppvariables.terminal_dark [lahead] : (sxkeywordp (
		 sxppvariables.sxtables, lahead) && sxppvariables.kw_dark));
...
static VOID	call_put_error ()
{
	...
	register struct SXPP_schema	*co;
	register int	rd;
	...
	sxput_error (source_index, "%sError on margin in directive %d of production %d.", severity, co - sxppvariables.
	     PP_tables [rd] + 1, rd);
    }
    else
...
static VOID	go_to_column ()
{
    register int	nb_spaces;
    register int	nb_tabs;
	...
	if (column > current_column) {
	    if ((nb_tabs = (sxppvariables.no_tabs ? 0 : (column / sxppvariables.tabs_interval - (current_column /
		 sxppvariables.tabs_interval)))) > 0) {
		sxppvariables.char_count += nb_tabs;

		do
		    putc (HORIZONTAL_TAB, stdout);
		while (--nb_tabs > 0);
...
static VOID	impr_commentaire (ch_ptr)
    char	*ch_ptr;
{
    int		lg, longueur_premiere_ligne /* 0 si lg */ ;
    int		nb_NEW_LINEs, nb_NEW_LINEs_en_fin, depart;
    int		sv_column;
...
/* copy: */
    {
	register char	*chaine;
	register long	longueur;

	sxppvariables.char_count += longueur = string_info.lgth;
	chaine = sxstrget (string_info.ste);
...
static VOID	sxppnp (node)
    register struct sxnode_pp	*node;
{
    if (node != NULL) {
	printf ("%o (%d):\t%o\n\t", node, node->terminal_token.lahead, node->next);
	...
	if (node->name > 0) {
	    printf ("Production %d", node->name);
	}
	else if (node->name == 0) {
	    printf ("ERROR NODE");
	}
	else {
	    printf ("%s (%s)", sxstrget (node->terminal_token.string_table_entry), sxttext (sxppvariables.sxtables, -node
		 ->name));
	}
...
static VOID	paragrapheur (root_ptr)
    struct sxnode_pp	*root_ptr;
{
    ...
    static char		consistency_msg [] = "Your tables may be inconsistant...";
...
decode_directive:
	...
	printf ("directive #%d of production %d:\t", saved_info.CO - sxppvariables.PP_tables [saved_info.noeud->name] + 1
	     , saved_info.noeud->name);
    ...
    default:
	ppabort ("%s: illegal directive: %d\n", ME, directive.PP_codop);
	...
	if (++save_stack_index == sxppvariables.save_stack_length)
	    sxppvariables.save_stack = (struct saved_info*) sxrealloc (sxppvariables.save_stack, sxppvariables.
		 save_stack_length += 100, sizeof (struct saved_info));
    ...
    struct sxtables	*sxtables;
{
	...
	sxhmsg (sxsrcmngr.source_coord.file_name, "%s%s: incorrect \"max_margin\". Value taken: %d.", sxtables->
	     err_titles [0], ME, sxppvariables.max_margin = (sxppvariables.line_length * 2) / 3);
	...
	sxhmsg (sxsrcmngr.source_coord.file_name, "%s%s: incorrect tabulation interval. Value taken: %d.", sxtables->
	     err_titles [0], ME, sxppvariables.tabs_interval = (DFLT_TABS_INTERVAL > sxppvariables.max_margin ?
	     sxppvariables.max_margin : DFLT_TABS_INTERVAL));

    sxppvariables.tmax = sxtables->SXS_tables.S_termax;
    sxppvariables.terminal_string_info = (struct string_info*) sxcalloc ((sxppvariables.tsi_top = 100) + sxppvariables.
	 tmax, sizeof (struct string_info)) + sxppvariables.tmax;
    sxppvariables.save_stack = (struct saved_info*) sxalloc (sxppvariables.save_stack_length = 100, sizeof (struct
	 saved_info));

    {
	register int	key;
	register struct string_info	*tsi;
    ...
    struct sxtables	*sxtables;
{
	...
	{
	    register int	old_top = sxppvariables.tsi_top;

	    if ((sxppvariables.tsi_top = sxstrmngr.top) > old_top) {
		sxppvariables.terminal_string_info = (struct string_info*) sxrealloc (sxppvariables.terminal_string_info
		     - sxppvariables.tmax, sxppvariables.tsi_top + sxppvariables.tmax, sizeof (struct string_info)) +
		     sxppvariables.tmax;
	    }
	}
	...
	if (sxverbosep) {
	    fprintf (sxtty, "char count:%6d", 0);
	}

	{
	    FILE	save_stdout; /* hack to attempt IO efficiency */
