...
#include "lecl_pcn.h"
char WHAT_LECLSMP[] = "@(#)SYNTAX - $Id: lecl_smp.c 581 2007-05-25 08:17:56Z syntax $" WHAT_DEBUG;
...
extern VOID	lecl_css ();
extern VOID	lecl_st ();
extern VOID	lecl_fsa_construction ();
extern VOID	lecl_code_gen ();
extern VOID	gen_check_kw ();
extern VOID	lecl_tbls_out ();
extern VOID	lecl_list_out ();
extern VOID	lecl_ag_free ();
...
/* LECTURE DES TABLES DE BNF */
	...
	ate_to_t_top = ate_to_token_no_top = ((int) sxstrmngr.top) + 2 * termax + synonym_no + predef_class_name_no + 1
	     /* Include eventuel */ ;
	...
	ate_to_token_no = (int*) sxcalloc ((int) ate_to_token_no_top + 1, sizeof (int));
	t_to_ate = (int*) sxalloc ((int) termax + 1, sizeof (int));
	ate_to_t = (int*) sxalloc ((int) ate_to_t_top + 1, sizeof (int));
	abbrev = (struct abbrev_item*) sxalloc ((int) abbrev_size, sizeof (struct abbrev_item));
	abbrev += abbrev_size;
	abbrev_or_class_to_ate = (int*) sxalloc ((int) abbrev_size + (int) class_size + 1, sizeof (int));
	abbrev_or_class_to_ate += abbrev_size;
	action_or_prdct_to_ate = (int*) sxalloc ((int) action_size + (int) predicate_size + 1, sizeof (int));
	action_or_prdct_to_ate += action_size;
	prdct_to_ers = (int*) sxalloc ((int) predicate_size + 1, sizeof (int));
	class_to_char_set = sxbm_calloc (class_size + 1, char_max + 1);
	is_class_used_in_re = sxba_calloc (class_size + 1);
	not_declared = (int*) sxalloc ((int) (class_size + 1), sizeof (int));
	ers_disp = (struct ers_disp_item*) sxcalloc ((int) token_size + 1, sizeof (struct ers_disp_item));
	RE_to_T = sxbm_calloc (token_size + 1, termax + 1);
	keywords = (struct keywords_item*) sxalloc ((int) token_size + 1, sizeof (struct keywords_item));
	exclude = (struct exclude_item*) sxalloc ((int) exclude_size + 1, sizeof (struct exclude_item));
	keyword = (int*) sxalloc ((int) keyword_size + 1, sizeof (int));
	is_a_keyword = sxba_calloc (termax + 1);
	t_is_defined_by_a_token = sxba_calloc (termax + 1);
	classe = (int*) sxalloc ((int) target_collate_length, sizeof (int));
	simple_classe = (int*) sxalloc ((int) char_max + 1, sizeof (int));
	target_code = (int*) sxalloc ((int) target_collate_length, sizeof (int));
	target_collate = (char*) sxalloc ((int) target_collate_length, sizeof (char));
...

VOID	lecl_smp (what, sxtables_ptr)
    int		what;
    struct sxtables	*sxtables_ptr;
{
    switch (what) {
