...
/*                                                                  */
/*                                                                  */
/*  This program has been translated from lecl_fsa_construction.pl1 */
/*  on Wednesday the second of April, 1986, at 10:29:35,            */
...
/************************************************************************/
/* 20030424 09:38 (phd):	Correction d'une faute d'orthographe	*/
/************************************************************************/
/* Mar 24 Dec 1996 13:42 (pb):	Ajout de clear_sc_prdct_to_next ()	*/
/************************************************************************/
/* Lun 25 Nov 1996 11:11 (pb):	Adaptation a LINUX			*/
/************************************************************************/
/* 22-10-93 10:00 (pb):		Adaptation a OSF			*/
/************************************************************************/
/* 18-05-93 11:55 (pb):		Traitement des actions dans le cas du	*/
/*				 non-determinisme			*/
/************************************************************************/
...
#include "varstr.h"
#include "lecl_ag.h"
char WHAT_LECLFSA[] = "@(#)SYNTAX - $Id: lecl_fsa.c 642 2007-06-21 08:00:52Z rlacroix $" WHAT_DEBUG;

extern int	lecl_diagnoses_one_la ();
extern int	lecl_diagnoses_unbounded ();
extern int	lecl_diagnoses_mixte ();
extern int	lecl_diagnoses_immediate ();
extern int	lecl_diagnoses_unbounded_output ();
extern int	lecl_sature ();
/* extern int	sort_by_tree (); 1. c'est un SXVOID et 2. il est maintenant dans sxu2.h */
extern VARSTR	lecl_gen_cc ();
static int	ctxt_mngr__put ();
struct sc_prdct_to_next_item {
    int		state_no, lnk, *bwrd_ptr;
    BOOLEAN	is_del, is_scan;
};
static char	*u_head_ptr, *m_head_ptr;
static int	FSA_HSH_TBL [127];
static int	*fsa_hsh_tbl;
...
static struct lr_marker		*lr_marker_ptr, lrm, *current_state, *store
				/* 1:items_no */
									   ;
static int	action_no, hsh, i, j, k, ner, next_state, seed_state, shift_state,
...
static BOOLEAN	bdel, scan, is_first_reduce, is_the_same_trans, is_first,
                process_predicates, has_predicate;
static char	current_state_kind, next_state_kind /* bit (6) */ ;
static int	*to_be_sorted, *indexes, *sorted_actions, *new_ctxt /* 1:items_no */ ;
...
static int	**sc_prdct_to_next; /* 1:smax, 1:xprdct_to_ate */
static struct sc_prdct_to_next_item	*next_state_no_list;
static int	next_state_no_list_top, next_state_no_list_size;
...
#define ONELA_CONFLICT 1
#define UNBOUNDED_CONFLICT 2

#define UNDEF_KIND 0
#define FIRST_REDUCE 1
#define SHIFT_KIND 2
#define ACTION_KIND 3
#define NEXT_REDUCE 0

#define PURE_SHIFT 1
#define PURE_ACTION 2
#define DISGUISED_SHIFT 3
#define DISGUISED_ACTION 4
#define REDUCE_CONFLICT 5
#define UNKNOWN 6

static int	T [6] [6] = { /* bidon */ {0, 0, 0, 0, 0, 0},
...

static void	maj (maj_x, n)
    register int	*maj_x, n;
{
    (*maj_x)++;
...
static void	put_item_in_substate (put_item_in_substate_index, put_item_in_substate_origine, context_no, kind)
    int		put_item_in_substate_index, put_item_in_substate_origine, context_no;
    char	kind /* bit (2) */ ;
{
    register struct lr_marker	*p;
...
static void	put (put_lrm)
    struct lr_marker	*put_lrm;
{
    maj (&xstate, 1);
...
static int	get_red_no (get_red_no_x)
    register int	get_red_no_x;
{
...
static void	filsubstate (filsubstate_lrm)
    register struct lr_marker	*filsubstate_lrm;
{
    /* Le marqueur LR est en fin d'ER; cette procedure remplit substate avec les
       debutants des suivants valides de cette ER. */
    register SXBA	/* max_re */ filsubstate_ctxt_set;
    register int	filsubstate_i, filsubstate_j;
...

static void	subsuiv (is_kept, is_next, subsuiv_is_first_reduce)
    BOOLEAN	is_kept, is_next, subsuiv_is_first_reduce;
{
    register int	l, t, subsuiv_origine, xs1, subsuiv_ner;
    struct lr_marker	subsuiv_lrm;
    register SXBA	/* max_re_lgth */ subsuiv_follow;
...

static BOOLEAN	eq_lr_marker (si, sj)
    register struct lr_marker	*si, *sj;
{
...
static BOOLEAN	is_state_already_defined (is_state_already_defined_seed_state, else_prdct, is_del, is_scan, xfsa, is_state_already_defined_hsh)
    int		is_state_already_defined_seed_state, else_prdct, *xfsa, is_state_already_defined_hsh;
    BOOLEAN	is_del, is_scan;
{
   ...
   - non	xfsa=xfsa2
*/
    register int	is_state_already_defined_i, m, is_state_already_defined_x, y, taille;
    register struct lr_marker	*si, *sj;
...

static BOOLEAN	less (less_i, less_j)
    int		less_i, less_j;
{
    register struct lr_marker	*si, *sj;
...
static	void build_state (xt, build_state_hsh, state_kind, build_state_ner, is_la)
    register int	*build_state_hsh;
    int		*xt, *build_state_ner;
    BOOLEAN	is_la;
    register char	*state_kind;
{
    ...
    register int		build_state_i, xl, class, bsup;
    int				build_state_x, mner;
    register struct lr_marker	*item1_ptr, *item2_ptr;
...

static	void put_in_substate (put_in_substate_not_yet_processed, d)
    SXBA	put_in_substate_not_yet_processed;
    int		d;
{
    maj (&xsubstate, 1);
...
static	void put_in_state (put_in_state_not_yet_processed, d)
    SXBA	put_in_state_not_yet_processed;
    int		d;
{
    put (item + xitem);
...
static	void put_new_trans (ref, state_no, del, put_new_trans_scan)
    int		ref, state_no;
    BOOLEAN	del, put_new_trans_scan;
{
    register struct fsa_trans_item	*fsa_trans_item_ptr;
...
static void	set_new_state (seed_state_no, shift_state_no, else_prdct_no, set_new_state_bdel, set_new_state_scan, state_kind)
    int		seed_state_no, shift_state_no, else_prdct_no;
    BOOLEAN	set_new_state_bdel, set_new_state_scan;
    char	state_kind;
{
    register struct fsa_item	*fsa_item_ptr;
...
static	void predicates_processing (predicates_processing_origine, predicates_processing_items_no, is_in_la)
    int		predicates_processing_origine, predicates_processing_items_no;
    BOOLEAN	is_in_la;
{
    BOOLEAN	predicates_processing_is_first, is_prdct_true_used, is_first_trans, is_the_same, is_first_pass, predicates_processing_bdel, else_del, predicates_processing_scan, else_scan
	 , is_trans_to_prdct, is_conflict;
    register int	d, predicates_processing_x, prd, prdct;
    int		predicates_processing_predicate_no, predicates_processing_k, predicates_processing_hsh, predicates_processing_next_state, predicates_processing_ner, predicates_processing_else_state;
...
/* on a des predicats differents non disjoints, il y a conflit si leur 
   next_state sont differents */
			...
			sxtmsg (sxsrcmngr.source_coord.file_name, "%sThe following predicates are not disjoint :\n\r%s%s"
			     , err_titles [1] /* warning */ , varstr_tostr (names), sxstrget (action_or_prdct_to_ate [-
			     ers [item [xitem].index - 1].prolis]));
		    }
...

static int	ctxt_mngr__put (ctxt_mngr__put_ctxt_set)
    SXBA	ctxt_mngr__put_ctxt_set;
{
    register int	ctxt_mngr__put_i, ctxt_mngr__put_old_size;
    SXBA	*old_ptr;
...
static BOOLEAN	less_action (less_action_i, less_action_j)
    int		less_action_i, less_action_j;
{
    return ers [state [less_action_i].index].lispro < ers [state [less_action_j].index].lispro;
}

static void clear_sc_prdct_to_next ()
{
...
static void set_sc_prdct_to_next (set_sc_prdct_to_next_k, set_sc_prdct_to_next_x)
    int set_sc_prdct_to_next_k, set_sc_prdct_to_next_x;
{
    int					set_sc_prdct_to_next_i;
    struct sc_prdct_to_next_item	*p;
    int					*bwrd_ptr = &(sc_prdct_to_next [set_sc_prdct_to_next_k] [set_sc_prdct_to_next_x]);
...
static	void build_action_states (build_action_states_items_no)
    int		build_action_states_items_no;
{
    int				build_action_states_x, y, build_action_states_action_no, build_action_states_hsh;
    register struct lr_marker	*build_action_states_lrm;
	...
	if (!is_state_already_defined (0, 0, FALSE, FALSE, &next_state, build_action_states_hsh))
	    set_new_state (0, 0, 0, FALSE, FALSE, next_state_kind);

	if (is_non_deterministic_automaton)
	    set_sc_prdct_to_next (1, prdct_true_code) /* Any */;
	else
...
static int	get_action_state (state_no, act_no)
    int		state_no, act_no;
{
    register int	get_action_state_i;
    register int	lim = fsa [state_no + 1].transition_ref;
...

 static BOOLEAN	get_del (get_del_sc_set)
    SXBA get_del_sc_set;
{
   ...
   l'analyse lexico. */
    register struct ers_item	*ers_item_ptr;
    register int	get_del_i, get_del_x;
    register BOOLEAN	all_0 = TRUE, all_1 = TRUE;
    int	n;
    BOOLEAN is_sysRE = FALSE;
	...
	if (ers_item_ptr->liserindx.line > 0) {
	    ...
	    sxput_error (ers_item_ptr->liserindx,
		 "%sErase inconsistency #%d upon %s in this class which occurs in %s%s.", err_titles [1],
		 erase_inconsistency, sc_name->first, er_name, suffixe);
	}
    }
...
static BOOLEAN	is_first_reduction (state_no)
    int		state_no;
{
    register int	is_first_reduction_x;
    register int	lim = fsa [state_no + 1].item_ref;
...

static void	maj_ctxt (maj_ctxt_x, set, sra_set)
    int		maj_ctxt_x;
    SXBA	set, sra_set;
{
...

static int	get_type (get_type_x, kind)
    int		get_type_x;
    char	kind /* bit (2) */ ;
{
    register int	class;
...
static void set_conflict_mode_prio (sra_set, set_conflict_mode_prio_conflict_mode, user_priority)
    SXBA	sra_set;
    int		*set_conflict_mode_prio_conflict_mode;
...
static BOOLEAN	is_conflict (sra_set, is_conflict_conflict_mode, user_priority)
    SXBA	sra_set;
    int		*is_conflict_conflict_mode;
    char	*user_priority /* bit (3) */ ;
{
...
static void apply_user_prio (sra_set, apply_user_prio_prio)
    SXBA	sra_set;
    char	apply_user_prio_prio /* bit (3) */ ;
{
    int		apply_user_prio_x, apply_user_prio_ner;
...
static	BOOLEAN conflict_mngr (sra_set, conflict_mngr_conflict_mode, conflict_mngr_prio)
    SXBA		sra_set;
    int			conflict_mngr_conflict_mode;
    register char	conflict_mngr_prio /* bit (3) */ ;
{
    int		conflict_mngr_x, conflict_mngr_j;
    int		conflict_mngr_ner, choice, choice_priority, typ, class, orig, current_conflict_mode;
    BOOLEAN	is_shift, is_action, is_reduce;
    char	kind /* bit (2) */ ;
...
static	BOOLEAN conflict_in_comments (sra_set)
    SXBA	sra_set;
{
    register int	sra_set_card, x1, x2;
    char	priority /* bit (3) */ ;
	...
	if (!sxba_is_empty (ctxt_set))
		...
		sxba_not (ctxt_set), SXBA_0_bit (ctxt_set, 0);
	...
	register int	conflict_in_comments_x, conflict_in_comments_ner;
...
static	BOOLEAN unbounded_conflict (sra_set, unbounded_conflict_prio)
    SXBA	sra_set;
    char	unbounded_conflict_prio /* bit (3) */ ;
{
    SXBA	/* xcurrent_state */ set;
    register int	unbounded_conflict_x, xindexes, unbounded_conflict_i, ind;
    BOOLEAN		has_conflict;
...
static	void maj_trans ()
{
    register BOOLEAN	found;
    register int	maj_trans_x, cc_ref;
...
static BOOLEAN	is_act_red (w_set, is_act_red_sc_set, is_act_red_next_state)
    SXBA	w_set, is_act_red_sc_set;
    int		is_act_red_next_state;
{
    ...
    register int	is_act_red_xitem, is_act_red_x, is_act_red_index, lim, is_act_red_k;
...
static void complete_trans ()
{
    BOOLEAN	is_sc_set_empty = FALSE;
...
static int create_nd_state (p)
    struct sc_prdct_to_next_item	*p;
{
    /* traitement d'une liste d'actions non-deterministes. */
    int				create_nd_state_x, create_nd_state_hsh, create_nd_state_i;
    ...
    if (!is_state_already_defined (0, 0, bdel,
				   scan, &next_state, create_nd_state_hsh))
	set_new_state (0, 0, 0, bdel,
...
VOID	lecl_fsa_construction ()
{
    ...
    sc_prdct_to_next = (int**) sxalloc (smax + 1, sizeof (int*));

    {
	int	lecl_fsa_construction_i;

	sc_prdct_to_next [0] = NULL;

	for (lecl_fsa_construction_i = 1; lecl_fsa_construction_i <= smax; lecl_fsa_construction_i++)
	    sc_prdct_to_next [lecl_fsa_construction_i] = (int*) sxcalloc ((int) (xprdct_to_ate + 1), sizeof (int));
...
/* creation de l'etat initial */
	...
	if (j != ers_disp [i + 1].prolon)
		...
		put_item_in_substate (j, 0, i, UNDEF_KIND);
    }
...
/* on boucle tant qu'il y a un etat non traite */
	...
	if ((current_state_kind & ACT) && !(current_state_kind & MIXTE)) {
	    ...
	    if (!is_state_already_defined (0, 0, FALSE, FALSE, &next_state, hsh))
		set_new_state (0, 0, 0, FALSE, FALSE, next_state_kind);
...
/* Si &False => xstate = 0 */
	    ...
	    if (!is_state_already_defined (0, 0, bdel, scan, &next_state, hsh))
		set_new_state (0, 0, 0, bdel, scan, next_state_kind);
	    ...
	    /* Il n'y a pas de conflit impliquant un predicat:
	       ...
	       "&Else" */
	    predicates_processing (origine, items_no, current_state_kind & LA);
		...
		put_new_trans (++i,
			       item [xitem].index,
			       item [xitem].attributes.kind == 1, /* del */
			       item [xitem].attributes.ctxt_no == 1 /* scan */);
	    }
	    ...
	    sxba_fill (sc1_set), SXBA_0_bit (sc1_set, 0), SXBA_0_bit (sc1_set, 1);
		...
		if (x <= xstate) {
			...
			if (current_state_kind & SHIFT) {
				...
				if (xstate > 0) {
				    ...
				    if (!is_state_already_defined (0, 0, FALSE, FALSE,
								   &next_state, hsh))
					set_new_state (0, 0, 0, FALSE, FALSE, next_state_kind);
			...
			sxba_fill (sc_set), SXBA_0_bit (sc_set, 0), SXBA_0_bit (sc_set, 1); /* Any+Eof */
			maj_trans ();
			fsa [xfsa1 + 1].transition_ref = xfsa_t;
			continue;
		    }
...
/* Il est impossible de modifier current_state pendant la resolution des
   conflits, chaque item pouvant etre "partage" entre plusieurs sous-ensembles */
		...
		if (is_nd_trans) {
		    ...
		    BOOLEAN	doit, lecl_fsa_construction_is_first;
		    int		orig, class, lecl_fsa_construction_ner, cur_sc, cur_prdct;
			...
			xstate = 0;
			subsuiv (is_non_deterministic_automaton
			...
			if (!(next_state_kind & MIXTE)) {
			    ...
			    if (!is_state_already_defined (seed_state, 0, bdel,
							   scan, &next_state, hsh))
				set_new_state (seed_state, shift_state, 0, bdel,
					       scan, next_state_kind);
			}
		    }
...
/* On est en optimize. Les predicats ne sont pas testes s'ils conduisent
   tous a des comportements identiques. */
			...
			if (!is_state_already_defined (seed_state, else_state, bdel,
						       scan, &next_state, hsh))
			    set_new_state (seed_state, 0, else_state, bdel,
					   scan, next_state_kind);
