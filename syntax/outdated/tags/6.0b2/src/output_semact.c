...
char WHAT_OUTPUT_SEMACT[] = "@(#)SYNTAX - $Id: output_semact.c 661 2007-06-27 12:21:59Z sagot $" WHAT_DEBUG;
...
static BOOLEAN has_VP3;
static int     *easy_upper_bound;
static BOOLEAN xml_close_tag;
...
easy_Sujet_Verbe ()
{
  ...
  int     *dpv_ptr, *val_ptr;
  char    *str;
  BOOLEAN has_mode_field, has_v_form_field, is_MOD_N;
...
{
  int             fs_id, bot, top, bot2, cur2, top2, bot3, cur3, top3, max_priority, priority, orig_field_val;
  ...
  while (++x <= fs_id_stack [0]) {
    ...
    while (bot < top) {
      ...
      field_id = val & FIELD_AND;
      static_field_kind = field_id2kind [field_id];
      field_kind = (val >> MAX_FIELD_ID_SHIFT) & KIND_AND;
	...
	if (static_field_kind & ATOM_KIND) {
	  ...
	  if ((static_field_kind & UNBOUNDED_KIND) && (static_field_kind & Aij_KIND)) {
		...
		if (id > spf.outputG.maxxnt) { 
		  ...
		  fprintf (xml_file, "idref=\"R_%i_%i\"", id, spf.outputG.lhs [id].init_prod);
		}
		else
		  fprintf (xml_file, "idref=\"N%i\"", id /* Aij */);
	      }
	    }
	    else
	      fputs ("error_val=\"\"", xml_file);
	  }
	  else
#endif /* ATOM_Aij || ATOM_Pij */
	    {
#ifndef ESSAI
	      if (static_field_kind & UNBOUNDED_KIND) {
		fprintf (xml_file, "atomic_val=\"%s\"", atom_id2string [val]);
	      }
	      else {
		...
		while ((val >>= 1) != 0) {
		  ...
		  if (val & 1) {
		    ...
		    fprintf (xml_file, "atomic_val=\"%s\"", atom_id2string [atom_id]);
		  }
		}
	      }
#else /* ESSAI */
		...
		while ((local_atom_id = sxba_scan_reset (left_local_atom_id_set, local_atom_id)) > 0) {
		  atom_id = X_X (atom_id2local_atom_id_hd, local_atom_id);
		  fprintf (xml_file, "/>\n      <value atomic_val=\"%s\"", atom_id2string [atom_id]);
		}
	  ...
	  if (static_field_kind == STRUCT_KIND) {
	    if (val == 0) {
	      fputs ("emtpy_val=\"\"", xml_file);
	    }
	    else {
	      if (SXBA_bit_is_reset_set (fs_id_set, val))
		PUSH (fs_id_stack, val);
		
	      fprintf (xml_file, "idref=\"F%i\"", val);
	    }
	  }
	  else {
