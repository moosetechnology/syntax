...
char WHAT_SXNDTOKEN_MNGR[] = "@(#)SYNTAX - $Id: sxndtoken_mngr.c 527 2007-05-09 14:02:47Z rlacroix $" WHAT_DEBUG;
...
VOID sxmilstn_oflw (index_header_ptr, old_line_nb, old_size)
    sxindex_header 	*index_header_ptr;
    int			old_line_nb, old_size;
{
    int new_line_nb = sxindex_line_nb (*index_header_ptr);
    int new_column_nb = sxindex_column_nb (*index_header_ptr);
    int new_size = sxindex_size (*index_header_ptr);
...
VOID sxmilstn_alloc (header, size)
    sxmilstn_header	*header;
    int			size;
{
    int i, l;
...
struct sxmilstn_elem	*milestone_new (prev_milstn_ptr, my_milestone, kind)
    struct sxmilstn_elem	*prev_milstn_ptr;
    int 			kind, my_milestone;
{
    int		new_milstn = sxmilstn_seek (sxndtkn.milestones);
...
VOID	sxndtkn_put (set)
    SXBA	set;
{
    int			tde, old_milestone, x = -1;
...
VOID	sxndtkn_mngr (sxndtkn_mngr_what, size)
    int		sxndtkn_mngr_what, size;
{
    int 		tde, prev_mlstn;
	...
	sxmilstn_alloc (&sxndtkn.milestones, 50);
	break;
	...
	parse_stack.milstn_tail = 
	    sxndtkn.milstn_head = milestone_new (NULL,
						 sxndtkn.last_milestone_no = 0,
						 MS_WAIT | MS_COMPLETE | MS_BEGIN);
	...
	sxndtkn.milstn_current = sxndtkn.milstn_head =
	    milestone_new (sxndtkn.milstn_head, ++sxndtkn.last_milestone_no, MS_BEGIN);
