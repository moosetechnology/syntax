...
char WHAT_SXBA_2OP[] = "@(#)SYNTAX - $Id: sxba_2op.c 528 2007-05-09 14:13:40Z rlacroix $" WHAT_DEBUG;
...
BOOLEAN
sxba_2op (SXBA lhs, int op1, SXBA rhs1, int op2, SXBA rhs2)
{
  SXBA	   lhs_bits_ptr, rhs_bits_ptr1, rhs_bits_ptr2;
  int	   slices_number = NBLONGS (BASIZE (rhs2));
...
*/
BOOLEAN
sxba_incr (SXBA lhs, SXBA rhs, int incr)
{
  SXBA	                source_bits_ptr, object_bits_ptr;
  int			delta = DIV (incr); /* decalage en mots */
  int                   size = BASIZE (lhs);
  int	                slices_number = NBLONGS (size);
  int			left_shift = MOD (incr);
  int			right_shift = SXBITS_PER_LONG - left_shift;
...
*/
BOOLEAN
sxba_decr (SXBA lhs, SXBA rhs, int decr)
{
  SXBA	                source_bits_ptr, object_bits_ptr;
  int			delta = DIV (decr); /* decalage en mots */
  int                   size = BASIZE (lhs);
  int	                slices_number = NBLONGS (size);
  int			right_shift = MOD (decr);
