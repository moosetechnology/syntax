...
char WHAT_SXMEM_MNGR[] = "@(#)SYNTAX - $Id: sxmem_mngr.c 530 2007-05-10 08:05:39Z rlacroix $" WHAT_DEBUG;
...
# else
#  ifdef __alpha
unsigned long __sbrk_override;
unsigned long __taso_mode;
...
void	*sxcont_malloc (unsigned long int size)
...
# if BUG
    fprintf (sxstdout, "mallocating %lu:", size);
# endif
...
void	*sxcont_alloc (unsigned long int n, unsigned long int size)
...
# if BUG
    fprintf (sxstdout, "allocating %lu (%lu):", n, size);
# endif
...
void	*sxcont_realloc (void *table, unsigned long int size)
...
# if BUG
    fprintf (sxstdout, "\nreallocating %lX (%lu):", (unsigned long int) table, size);
# endif
...
void	*sxcont_recalloc (void *table, unsigned long int old_size, unsigned long int new_size)
...
# if BUG
    fprintf (sxstdout, "\nrecallocating %lX (%lu):", (unsigned long int) table, new_size);
# endif
...
# if	BUG
	...
	fprintf (sxstdout, "freeing %lX:", (unsigned long int) zone);
	free (zone);
