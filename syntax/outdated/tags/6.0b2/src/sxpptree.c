...
char WHAT_SXPPTREE[] = "@(#)SYNTAX - $Id: sxpptree.c 623 2007-06-11 14:18:24Z rlacroix $" WHAT_DEBUG;
...
static VOID	rateau ()
{
	...
	if (node->pos.middle >= start_pos) {
	  ...
	  /* prepare_arrows: 
		(pas de goto sur ce label, ce qui provoque un warning du compilo icc d'intel) */
	  {
	    register long	spaces_nb = node->pos.middle - pos;
    ...
    long	switches;
{
	...
	{
	    register unsigned	strlength = args.page_width + 1;
