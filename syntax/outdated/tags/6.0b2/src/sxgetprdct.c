...
char WHAT_SXGETPRDCT[] = "@(#)SYNTAX - $Id: sxgetprdct.c 527 2007-05-09 14:02:47Z rlacroix $" WHAT_DEBUG;

int		sxgetprdct (red_no, pos)
    int				red_no, pos;
/*
...
*/
{
    int				low, high /* bornes inf et sup */, mid;
	...
	while (low < high) {
	    ...
	    if (red_no <= (int) sxplocals.SXP_tables.prdct_list [mid].red_no)
		high = mid;
	    else
		low = mid + 1;
	}
	...
	while (red_no == ((int) ap->red_no) && ((int) ap->pos) <= pos) {
	    if (((int) ap->pos) == pos)
		return (int) ap->prdct;
