...
/* Le couple dag_scanner/read_a_dag simule un scanner non-deterministe pour un analyseur Earley.
   ...
   Techniquement, read_a_dag est la passe semantique (parcour d'un arbre abstrait) associe a un scanner/parser
   ...
   ds la librairie SYNTAX en $sx/incl car il doit etre recompiler par l'utilisateur qui doit l'instancier
   en lui fournissant un certain nombre de donnees additionnelles (dictionnaire par exemple).
   C'est le main associe a sag_scanner qui se charge du src_mngr qui lit le dag d'entree.
...
char WHAT_DAG_SCANNER[] = "@(#)SYNTAX - $Id: dag_scanner.c 648 2007-06-21 13:55:12Z sagot $" WHAT_DEBUG;
...
static int ciu_name_id, lciu_name_id, digits_name_id;
static int wstr_lgth;
     ...
     int  lgth;
{
  ...
  BOOLEAN has_changed, is_capitalized_initial;
...
void distribute_comment (t)
     int t;
{
  int    pos, pos2, tok_no, tok_no2, triple, triple2;
...
static int toks_buf_size, toks_buf_top; /* init : toks_buf_size = last_tok_no+2; toks_buf_top = last_tok_no+1; */
...
int
put_repair_tok (int t, char *t_str, char *comment_str, struct sxsource_coord *psource_index)
{
  int                   lgth, id;
...
void make_new_dag (f)
     void (*f)();
{
  int          delta_new_dag, i, j, k, Tij, Tpq, tok_no, new_triple, left_j, right_j, new_j, nb; 
  int          *i2new_lb;
  BOOLEAN      is_loop;
  XxYxZ_header new_dag_hd;

  XxYxZ_alloc (new_dag_hd, "new_dag_hd", XxYxZ_top (dag_hd)+1, 1, dag_doreach, 0, statistics);
  i2new_lb = (int*) sxalloc (final_pos+1, sizeof (int));
...
fill_dag_infos ()
{
  int cur_trans, last_trans, lpos, rpos, tok_no, lgth, cur_lgth, id, top, cur, tok, ste;
	...
	if (cur_str = strstr (str, "__")) {
	  ...
	  if (id) {
...
#else /* ?keywords_h */
	      /* ATTENTION : ce "-1" permet de compenser le décalage dû au fait que t_names_string.chars commence par NUL
	    ...
	    if (tok == 0)
	      ...
	      /* On force la forme flechie */
	      ptok->string_table_entry = sxstr2save (str, lgth);
	    
#ifndef UNFOLD  
#if is_sdag 
	      SXBA_1_bit (glbl_source_i, tok);
#else /* !is_sdag */
	      glbl_source [tok_no] = tok;
#endif /* !is_sdag */
#endif /* !UNFOLD */
...
store_dag (lpos_set, tok_no, ptok, rpos_set)
     ...
     struct sxtoken *ptok;
{
  int            triple, lpos, rpos;
  static BOOLEAN not_first_call;
     ...
     struct sxtoken    *ptok;
{
  ...
  if (SXGET_TOKEN (sxplocals.atok_no).lahead == get_udag_eof ()) { // si on est sur le dernier token
...
after_last_trans ()
{
  ...
  */
  ...
  dag_state2mlstn = (int*) sxalloc (final_pos+1, sizeof (int));
  mlstn2dag_state = (int*) sxalloc (final_pos+1, sizeof (int));
...
#else /* !is_sdag */
  glbl_source_area = glbl_source = (int*) sxalloc (2*(last_tok_no+2+max_rcvr_tok_no), sizeof (int));
  glbl_source [0] = eof;
  glbl_source [last_tok_no+1] = eof;
#endif /* !is_sdag */
...
#ifndef UNFOLD
    after_last_trans(); // partie qui nécessite des trucs de sxparser_earley -> la fonction est définie là dedans
#endif /* UNFOLD */
    fill_dag_infos(); // partie faisable ici -> la fonction est définie est ici
    break;
  case CLOSE:
    if (tables != NULL) {
      if (0 /* A FAIRE : identifier le cas non-deterministe */)
    ...
    if (XxYxZ_is_allocated (dag_hd)) {
      XxYxZ_free (&dag_hd);
      if (dag2comment) sxfree (dag2comment), dag2comment = NULL;
...
fill_Tij2tok_no (int Tpq, int maxTpq)
{
  ...
  XxYxZ_Xforeach (dag_hd, p, trans) {
	...
	spf.outputG.Tij2tok_no [Tpq] = tok_no; /* Il faut utiliser toks_buf [] */
...
{
  int  mlstn, next_mlstn, dag_state, trans, Y, t;
