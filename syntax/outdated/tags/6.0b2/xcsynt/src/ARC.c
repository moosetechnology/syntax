...
/************************************************************************/
/* 05-08-94 14:17 (pb):		Modif ds compute_back pour les trans	*/
/*				sur les XNT a partir du LHS		*/
/************************************************************************/
...
char WHAT_XCSYNTARC[] = "@(#)SYNTAX - $Id: ARC.c 642 2007-06-21 08:00:52Z rlacroix $" WHAT_DEBUG;

#include "SS.h"
#include "RLR.h"


extern BOOLEAN		erase_quads ();
...
static BOOLEAN		is_cycle_on_reads_completed;
...
static BOOLEAN		IS_SHIFT_REDUCE, THERE_IS_AN_XPROD;
static SXBA		CONFLICT_T_SET;



static int	RLR_sature ();
...
BOOLEAN check_level_conformity (item, level)
    int item, level;
{
...
BOOLEAN check_bot_state_conformity (state, level)
    int state, level;
{
    /* Appele depuis earley.
...
static BOOLEAN	is_new_pss (pss, state, is_new_pss_index, next_state, new_pss)
    int		pss, state, is_new_pss_index, next_state, *new_pss;
{
    ...
    BOOLEAN	is_knot = FALSE;

    if (pss_kind == FSA_) {
	register int	bot;
	int	new_couple, start;
	BOOLEAN		new_couple_pushed = FALSE;
...
static BOOLEAN is_new_knoted_pss (pss, pred, is_new_knoted_pss_index, next_state, pss1)
    int	pss, pred, is_new_knoted_pss_index, next_state, *pss1;
{
...
static VOID	compute_back (ppss, state, compute_back_index, item, pss)
    int		ppss, state, compute_back_index, item, pss;
{
	...
	if (ppss > 0) {
	    ...
	    case -2 : /* UNBOUNDED_ avec (peut etre) des knots */
	        {
		    register int	StNt;
		    register int	*as = PSS_BOT (ppss) + compute_back_index;
...
/* Quand les StNt seront des Q0xV, on pourra faire mieux. */
	...
	register int *as;
	int	StNt, pss1, next_state, nt, new_item;
	BOOLEAN brand_new_pss;
...
static VOID	call_compute_back (ppss, state, call_compute_back_index, item, pss)
    int		ppss, state, call_compute_back_index, item, pss;
{
    if (ibfsa_needed) {
...
BOOLEAN is_pss_include (bot1, top1, bot2, top2)
...
static int ARC_store (arc_no, t_set, ARC_store_QxTxQ_hd)
    int		arc_no;
    SXBA	t_set;
    register XxYxZ_header	*ARC_store_QxTxQ_hd;
{
    ...
    int		xac1_init, new_state_nb, plulong;
{
    ...
    register struct state	*astate;
    BOOLEAN			is_germe_to_gr_act_completed = TRUE;
...
static BOOLEAN ARC_max_la (state, lgth)
    int state, lgth;
{
    ...
    register BOOLEAN is_loop = FALSE;
...
static BOOLEAN	BS_couple (X, Y, n)
    int	*X, *Y, n;
{
...
static VARSTR	couples_to_re (bot, top, entree, sortie, couples_to_re_vstr)
    register int	*bot, *top, entree, sortie;
    VARSTR		couples_to_re_vstr;
{
...
static BOOLEAN	is_same_pss (xkas, is_same_pss_pss_sets)
    int		xkas;
    SXBA	*is_same_pss_pss_sets;
{
...
BOOLEAN	make_inter_pss (pss1, pss2)
    int pss1, pss2;
{
...
static BOOLEAN is_cycle_on_fsa (pss)
    int	pss;
{
...
static int	is_inter_pss (xkas, is_inter_pss_pss_sets, is_all, is_cycle_needed)
    int		xkas;
    SXBA	*is_inter_pss_pss_sets;
    BOOLEAN	is_all, is_cycle_needed;
{
    ...
    BOOLEAN		found, empty = TRUE, is_couple_without_cycle = FALSE, is_cycle_on_pss1;
...
BOOLEAN ARC_walk_backward (ARC, qtq, bh, bs)
    struct ARC_struct	*ARC;
    int		qtq;
    BOOLEAN	(*bh)(), (*bs)();
{
...
static BOOLEAN ARC_walk_forward (ARC, qtq, fh, fs)
    struct ARC_struct	*ARC;
    int			qtq;
    BOOLEAN		(*fh)(), (*fs)();
{
...
static BOOLEAN fh_propagate_cycle (ARC, qtq)
    struct ARC_struct	*ARC;
    int			qtq;
{
...
static BOOLEAN fh_propagate_lgth (ARC, qtq)
    struct ARC_struct	*ARC;
    int			qtq;
{
...
static int ARC_UNBOUNDED_get_conflict_kind (ARC, ARC_state, is_new_arc_state, t)
    struct ARC_struct	*ARC;
    int			ARC_state, t;
    BOOLEAN		is_new_arc_state;
{
	...
	register int	ARC_UNBOUNDED_get_conflict_kind_lgth = attr->lgth;
...
static BOOLEAN	make_fknot (e, s, nb, result)
    int		e, s, nb, *result;
{
...
static VARSTR	fknot_to_vstr (fknot_to_vstr_vstr, entree, sortie, nb)
    VARSTR	fknot_to_vstr_vstr;
    ...
    struct ARC_struct	*ARC;
{
    ...
    int			lim1, lim2, lim3, xpts, type, next_ARC_state;
...
static BOOLEAN	check_lc (check_lc_max_min_lgth)
    register int	check_lc_max_min_lgth;
{
...
VOID	fill_kas (ARC, ARC_state, fill_kas_kas, fill_kas_pss_sets, xkas, conflict_kind)
    struct ARC_struct	*ARC;
...
VOID	ARC_init (print_conflicts, kind, ARC_init_max_red_per_state, arcs_size)
    BOOLEAN	print_conflicts;
    int		kind, ARC_init_max_red_per_state, arcs_size;
{
...
static BOOLEAN	install_arc_state (ARC, install_arc_state_kas, xkas, new_arc)
    struct ARC_struct	*ARC;
    ...
    int		ARC_state;
{
	...
	int			t, lim1, pss, lim3, xpts, botas, type;
...
static BOOLEAN ARC_trans (ARC, ARC_state, t, next_ARC_state)
    struct ARC_struct	*ARC;
    int		ARC_state, t, *next_ARC_state;
{
    ...
    struct kas				*akas;
    int					indice, type, pss;
    BOOLEAN				is_old_aelem, is_first, is_brand_new_state;
...
static BOOLEAN ARC_conflicting_node_processing (ARC, qtq)
    struct ARC_struct	*ARC;
    int			qtq;
{
    ...
    int			xkas;
    BOOLEAN		local_conflict_has_been_solved = FALSE;
...
static BOOLEAN fh_remote_conflict (ARC, qtq)
    struct ARC_struct	*ARC;
    int			qtq;
{
...
static BOOLEAN bh_not_pure_tree (ARC, qtq)
    struct ARC_struct	*ARC;
    int			qtq;
{
    register struct ARC_ATTR	*attr = ARC->attr + XxYxZ_X (ARC->QxTxQ_hd, qtq);
    ...
    int		(*F_get_conflict_kind) ();
{
    ...
    register struct ARC_ATTR	*attr;
    int			next_ARC_state, qtq, ck, x_tree_orig, type_nb, new_lgth;
    BOOLEAN		give_up, is_new_ARC_state;
...
BOOLEAN ARC_construction (failed_kind, xac1, conflict_t_set, is_shift_reduce, StNt
			  , red_no)
    int		xac1, StNt, red_no, *failed_kind;
    SXBA	conflict_t_set;
    BOOLEAN	is_shift_reduce;
{
