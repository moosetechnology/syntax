...
/************************************************************************/
/* 05-08-94 14:57 (pb):		Utilisation de Q0xV_to_Q0xXNT pour 	*/
/*				retrouver les transitions [x]nt a partir*/
/*				d'une LHS				*/
/************************************************************************/
...
char WHAT_XCSYNTEARLEY[] = "@(#)SYNTAX - $Id: earley.c 625 2007-06-12 08:35:56Z syntax $" WHAT_DEBUG;
...
extern BOOLEAN	check_real_conformity ();
extern BOOLEAN	check_level_conformity ();
extern BOOLEAN	make_a_quad ();
...
static int		*EISL_disp, EISL_stack_top;
...
static int		next_t, zi, unused, list_elem_nb, LA_length,
    ETAT_CHARNIERE, ITEM_CHARNIERE, BS2_TOP;

static int		*items_stack, *up_stack, *left_stack;
static SXBA		right_set;

static XH_header	*stack_hd;
static int		LA_init, LA_cur, LA_final;

static int		*parse_stack;

static int		*EISL_check_stack;
static BOOLEAN		make_a_quad_has_been_called;
...
static VOID EISL_stack_sature ()
{
    ...
    EISL_check_stack = (int*) sxrealloc (EISL_check_stack, new_size + 1, sizeof (int));

    while (new_size > old_size)
	EISL_check_stack [new_size--] = 0;
}

BOOLEAN ttrue (item, LA_ref, is_prefixe)
    int		item, LA_ref;
    BOOLEAN	is_prefixe;
{
    ...
    int l;
{
	...
	EISL_check_stack = (int*) sxcalloc (xac2 + 1, sizeof (int));
	EISL_check_stack [0] = -1;
	EISL_disp = (int*) sxalloc (l + 3, sizeof (int));
	EISL_disp [0] = l;
	...
	if (EISL_disp [0] < l) {
	    EISL_disp = (int*) sxrealloc (EISL_disp, l + 3, sizeof (int));
	    EISL_disp [0] = l;
	}
...
static BOOLEAN check_item (item)
    int item;
{
...
static BOOLEAN new_EI (state, item)
    int state, item;
{
...
static BOOLEAN prev_state (state, item)
    int		state, item;
{
...
BOOLEAN earley (item, q, LA, LA_lgth, hd, BS2)
