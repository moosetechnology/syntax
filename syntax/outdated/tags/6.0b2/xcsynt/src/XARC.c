...
char WHAT_XCSYNTXARC[] = "@(#)SYNTAX - $Id: XARC.c 643 2007-06-21 09:35:02Z rlacroix $" WHAT_DEBUG;
...
extern BOOLEAN	ARP_simple_ambiguity ();
extern BOOLEAN	PARTIAL_FSA_ARP_construction ();
extern int	TOTAL_FSA_ARP_construction ();
extern int	UNBOUNDED_ARP_construction ();
extern VOID	put_new_clone ();
extern BOOLEAN	is_old_clone ();
extern BOOLEAN	make_inter_pss ();
extern char	*conflict_kind_to_string ();
extern VARSTR	conflict_paths_to_re ();
extern BOOLEAN	bh_mark ();
extern BOOLEAN	bh_unmark ();
...
static BOOLEAN check_loop_on_back (X1)
    int		X1;
{
...
static BOOLEAN call_check_loop_on_back ()
{
    register int ipss = 0;
...
static VARSTR make_ibfsa_to_ate (make_ibfsa_to_ate_vstr, ibfsa, X, Y)
    VARSTR	make_ibfsa_to_ate_vstr;
    int		ibfsa, X, Y;
{
    ...
    int	bfsa_init, pssxt;
{
    ...
    int			y1, bfsa, X1, ibfsa, bot, top, init, local_bfsa, final, orig, local_bfsa_init, xtnd_local_bfsa;
...
static VOID	reached_states (reached_set, reached_states_couples_set, normal)
    SXBA	reached_set, reached_states_couples_set;
    BOOLEAN	normal;
{
    ...
    register BOOLEAN	is_stable = FALSE;
...
BOOLEAN	XARC_state_scan (XARC, qtq, F)
    ...
    BOOLEAN		(*F)();
{
    ...
    int			lim, top1, top2, XARC_state_scan_fork1, fork2, pss1, pss2, type1, type2;
...
static BOOLEAN	FSA_case (XARC, qtq, FSA_case_fork1, fork2, type1, type2)
    struct ARC_struct	*XARC;
    int			qtq, FSA_case_fork1, fork2, type1, type2;
{
...
static BOOLEAN	fork_equality (XARC, qtq, fork_equality_fork1, fork2, type1, type2)
    struct ARC_struct	*XARC;
    int			qtq, fork_equality_fork1, fork2, type1, type2;
{
    ...
    struct ARC_struct	*XARC;
{
    ...
    int			XARC_state, next_XARC_state, ate, XARC_print_fork;
	...
	for (lim1 = XH_X (XARC->Q_hd, XARC_state + 1), z = XH_X (XARC->Q_hd, XARC_state); z < lim1
		...
		put_edit_apnnl ("=(");
		sprintf (string, "<%d>", XxY_X (forks, XARC_print_fork));
		put_edit_apnnl (string);
		put_edit_apnnl (", ");
		sprintf (string, "%d", XxY_Y (forks, XARC_print_fork));
		put_edit_apnnl (string);
		put_edit_apnnl (")");
...
VOID	n_to_forks (XARC, XARC_state, n, type)
    struct ARC_struct	*XARC;
    int		XARC_state, n, type;
{
    ...
    int		pss, n_to_forks_fork, top, lim;
    ...
    struct ARC_struct	*XARC;
{
    ...
    int					pss_init, XARC_init_state_building_fork, bfsa_init, init_fork_list, xred_no,
...
static BOOLEAN	XARC_trans (XARC, XARC_state, t, next_XARC_state)
    struct ARC_struct	*XARC;
