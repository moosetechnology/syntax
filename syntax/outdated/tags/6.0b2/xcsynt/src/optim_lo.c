...
char WHAT_XCSYNTOPTIMLO[] = "@(#)SYNTAX - $Id: optim_lo.c 613 2007-06-07 13:54:29Z rlacroix $" WHAT_DEBUG;
...
VARSTR	cat_t_string (cat_t_string_vstr, code)
    VARSTR	cat_t_string_vstr;
    int		code;
{
    ...
    char	*c1, *s2;
{
	...
	l = ((j > 0) ? (unsigned)get_nt_length (j) : strlen (t_string = xt_to_str (j))) + 1;
	c += l;
...
VARSTR	prod_to_str (prod_to_str_vstr, prod)
    VARSTR	prod_to_str_vstr;
    int		prod;
{
...
BOOLEAN		open_listing (kind, open_listing_language_name, ME, component_name)
    int		kind;
    char	*open_listing_language_name, *ME, *component_name;
{
    struct bstr	*bstr;
#include <time.h>
...
VOID	optim_lo (optim_lo_language_name, ME)
    char	*optim_lo_language_name, *ME;
{
    static char		hyph60 [] = "-------------------------------------------------------------";
    static char		dot46 [] = ".............................................";
    register int	i, x, y, l;
    register struct P_item	*avector;
    ...
    open_listing (2, optim_lo_language_name, ME, ".op.l");
    stats.nt.c.cols = 0;
    ...
    *nt_to_ad = 0 /* nombre de branchements directs */ ;
	...
	if (is_long_listing) {
	    ...
	    for (i = xe1 - 1; i > 0; i--) {
		register struct P_base	*optim_lo_abase = t_bases + i;
	...
	if (is_long_listing) {
	    ...
	    for (i = xac2 - 1; i > 0; i--) {
		register struct P_base	*optim_lo_abase_2 = nt_bases + i;
	...
	if (xpredicates > 0) {
	    register struct P_prdct	*optim_lo_aitem, *optim_lo_lim;
