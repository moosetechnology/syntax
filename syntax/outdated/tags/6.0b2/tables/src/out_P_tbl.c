...
/************************************************************************/
/* 20030523 10:41 (phd):	Ajout de l'élément -1 de reductions	*/
...
#include "tables.h"
#include "out.h"
char WHAT_TABLESOUTPTBL[] = "@(#)SYNTAX - $Id: out_P_tbl.c 640 2007-06-19 15:45:47Z rlacroix $" WHAT_DEBUG;
...
static VOID	out_prdcts ()
{
    register int	i, prdct;
    ...
    static VOID	out_germe_to_gr_act ()
{
    register int	i;
...
static VOID	out_P_right_ctxt_head ()
{
...
static VOID	out_P_local_mess ()
{
	...
	for (j = 1; j <= PL [i].param_no; j++) {
	    printf ("%d,", PL [i].string_ref [j].param);
	}
    ...
    out_end_struct ();
    out_struct ("SXP_local_mess", "{0,NULL,0},");

    for (param = 0, i = 1; i <= RC.P_nbcart + 1; param += PL [i++].param_no) {
	printf ("{%d,\"", PL [i].param_no);

	for (j = 0; j <= PL [i].param_no; j++) {
	    fputs ("%s", stdout);

	    if ((lg = PL [i].string_ref [j].length) != 0) {
		printf ("%.*s", lg, R.P_string_mess + PL [i].string_ref [j].index);
	    }
	}

	printf ("\",&P_param_ref[%d]},\n", param);
    }
...
static VOID	out_P_global_mess ()
{
    register int	i, j, lg;
    register char	*m, *prefixe, *motif;
    /* chaines */
...
out_P_tables ()
{
    ...
    out_ext_BOOLEAN (RC.P_nbcart == 0
		     ? (PC.nd_degree >= 0 ? "sxndpsrecovery" : "sxpsrecovery")
		     : (PC.nd_degree >= 0 ? "sxndprecovery" : "sxprecovery"));
}
