...
#include "out.h"
#include "sxba.h"
char WHAT_TABLESC[] = "@(#)SYNTAX - $Id: tables_c.c 612 2007-06-06 10:27:32Z rlacroix $" WHAT_DEBUG;
...
extern BOOLEAN	recor_read () /* (struct R_tables_s *recor_ag_ptr, char *langname) */ ;
extern VOID	recor_free () /* (struct R_tables_s *recor_ag_ptr) */ ;
extern BOOLEAN	paradis_read () /* (struct PP_ag_item *paradis_ag_ptr, char *langname) */ ;
/* extern VOID	paradis_free () already declared in etc/incl/PP_tables.h (struct PP_ag_item *paradis_ag_ptr) ; */ 
extern BOOLEAN	semat_read () /* (struct T_ag_item *semat_ag_ptr, char *langname) */ ;
extern VOID	semat_free () /* (struct T_ag_item *semat_ag_ptr) */ ;
extern BOOLEAN	init_tables () /* (char *name) */ ;
extern VOID	out_P_parser ();
extern int    	max_P_tables ();
extern int    	max_S_tables ();
extern VOID     sxdp_free ();
extern BOOLEAN	out_lig_tables ();
extern VOID     out_T_tables ();
extern VOID     out_PP_tables ();
extern VOID     out_P_tables ();
extern BOOLEAN  out_S_tables ();
...
static VOID	out_name_date (nom, lib)
    char	*nom;
    char	*lib;
{
    long	lt;
...
static VOID	out_sxtables ()
{
    out_ext_int (SC.S_is_non_deterministic ? "sxndscanner" : "sxscanner");
    out_ext_int (PC.nd_degree >= 0 ? "sxndparser" : "sxparser");
    ...
    printf ("%lu, /* magic */\n", SXMAGIC_NUMBER);
    printf ("{sx%sscanner,sx%sparser}, ", SC.S_is_non_deterministic ? "nd" : "", PC.nd_degree >= 0 ? "nd" : "");
    printf ("{%d, %d, %d, ", SC.S_last_char_code < 255 ? SC.S_last_char_code : 255, SC.S_termax, SC.S_constructor_kind);
    printf ("%d, %d, ", RC.S_nbcart, RC.S_nmax);
    printf ("%d, %d, %d, ", SC.S_last_simple_class_no, SC.S_counters_size, SC.S_last_state_no);
    ...
    puts (SC.S_is_user_action_or_prdct == 0 ? P0 : "SCANACT,");
    printf ("sx%ss%srecovery,\n", SC.S_is_non_deterministic ? "nd" : "", RC.S_nbcart == 0 ? "s" : "");
    puts (SC.S_check_kw_lgth == 0 ? P0 : "check_keyword,");
    puts ("},");
    printf ("{%d, %d, %d, %d, ", -PC.mMrednt, -PC.mMred, -PC.mMprdct, PC.Mfe);
    printf ("%d, %d, %d, %d, ", PC.M0ref, PC.Mref, PC.deltavec + 1, PC.deltavec + PC.Mvec);
    printf ("%d, %d, %d, %d, ", PC.tmax, PC.ntmax, PC.nbpro, PC.xnbpro);
    printf ("%d, %d, %d, ", PC.init_state, PC.final_state, PC.Mprdct_list);
    printf ("%d, %d, %d, %d, %d, ", RC.P_nbcart, RC.P_nmax, RC.P_maxlregle, RC.P_validation_length, RC.P_followers_number);
    printf ("%d, %d, %d,\n", RC.P_sizeofpts, RC.P_max_prio_X, RC.P_min_prio_0);
    ...
    puts (PPR != NULL || PC.xnbpro > PC.nbpro /* P_is_user_action_or_predicate */  ? "PARSACT," : P0);
