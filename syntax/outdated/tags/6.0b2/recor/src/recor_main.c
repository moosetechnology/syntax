...
/************************************************************************/
/* 20020318 14:30 (phd):	"sxtmpfile" dans "recor_run"		*/
/************************************************************************/
/* Lun 25 Nov 1996 10:15(pb):	Adaptation a LINUX			*/
/************************************************************************/
/* 21-01-94 14:24 (pb):		Caractere change de "char" en "int"	*/
/************************************************************************/
...
/************************************************************************/
/* 28-10-86 11:03 (pb):	Ajout de cette rubrique "modifications"	   	*/
/************************************************************************/

#include "sxunix.h"
char WHAT_RECORMAIN[] = "@(#)SYNTAX - $Id: recor_main.c 529 2007-05-09 14:26:33Z rlacroix $" WHAT_DEBUG;
...
static char	*option_tbl [] = {"", "v", "verbose", "nv", "noverbose", "sc", "source", "nsc", "nosource", "ln",
     "language_name",};
static int	option_kind [] = {UNKNOWN_ARG, VERBOSE, VERBOSE, -VERBOSE, -VERBOSE, SOURCE, SOURCE, -SOURCE, -SOURCE,
...

static	SXVOID recor_run (pathname)
...
int main (int argc, char *argv[])
{
 ...
 run:
  ...
  sxstr_mngr (BEGIN);
  (*(recor_tables.analyzers.parser)) (BEGIN) /* Allocation des variables globales du parser */ ;
  syntax (OPEN, &recor_tables) /* Initialisation de SYNTAX */ ;
  ...
  if (options_set & OPTION (LANGUAGE_NAME)) {
    ...
    do {
      ...
      recor_run (argv [argnum++]);
      sxerrmngr.nbmess [2] += severity;
    } while (argnum < argc);
  }

  syntax (CLOSE, &recor_tables);
  (*(recor_tables.analyzers.parser)) (END);
  sxstr_mngr (END);
