...
char WHAT_XPRDCT_MNGR[] = "@(#)SYNTAX - $Id: xprdct_mngr.c 605 2007-05-31 13:18:23Z rlacroix $" WHAT_DEBUG;
...
static int	xprdct_hash [HASH_SIZE];
...
static int	install_prdct_seq (install_prdct_seq_predicates, install_prdct_seq_prdcts, hashs, lnks, install_prdct_seq_xpredicates, hash_code)
    struct state	*install_prdct_seq_predicates;
    struct P_prdct	*install_prdct_seq_prdcts;
    int		*hashs, *lnks, hash_code;
    register int	install_prdct_seq_xpredicates;
{
...
BOOLEAN		prdct_equal (xprdct, prdct1, prdct2)
...
void build_branch (xprdct, action, father)
    int		xprdct, action, father;
{
