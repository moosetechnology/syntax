...
/************************************************************************/
/* Jeu 23 Avr 1998 10:19 (pb):	Traitement des &StrEq("a", X)		*/
/************************************************************************/
/* Ven  6 Fev 1998 13:39 (pb):	Ajout de cette rubrique "modifications"	*/
/************************************************************************/
...
#include <ctype.h>
#include <string.h>
char WHAT_RCGGENBNF[] = "@(#)SYNTAX - $Id: rcg_gen_bnf.c 600 2007-05-29 14:31:15Z rlacroix $" WHAT_DEBUG;
...
static int		*arg_stack, *lhs_param_stack, *rhs_pos2Ak, *rhs_pos2bot3, *rhs_pos2top3;
static SXBA		rhs_pos_set, spcl_pos_set;
static BOOLEAN		Tstar_is_needed;
...
gen_bnf_rule ()
{
    ...
    int		x, symb, Ak, A, k, top, gen_bnf_rule_rule_nb;
	...
	for (x = 1; x <= top; x++) {
	    ...
	    if (symb < 0)
		/* terminal */
		fprintf (bnf_file, "\"%s\" ", sxstrget (t2ste [-symb]));
	    else {
		if (symb < predef_nt_nb) {
			...
			k = Ak_A2k (Ak, A);
			fprintf (bnf_file, "<%s[%i]> ", sxstrget (nt2ste [A]), k+1);
		    }
		    else {
...
static void
substitute (lhs_bot3, lhs_top3)
    int 	lhs_bot3, lhs_top3;
{
    int		lgth, rhs_pos, rhs_bot3, rhs_cur3, rhs_top3, lhs_cur3, symb, x;
    BOOLEAN	has_a_substitution;
...
pre_substitute (lhs_bot3, lhs_top3, is_spcl)
    ...
    BOOLEAN	is_spcl;
{
    int		lhs_lgth, rhs_lgth, rhs_pos, rhs_bot3, rhs_cur3, rhs_top3, lhs_cur3, symb;
    BOOLEAN	has_a_substitution;
	...
	substitute (1, S1_top (lhs_param_stack)+1);
    }
    else {
	if (is_spcl) {
	    has_a_substitution = FALSE;
...
process_clause (clause)
    int clause;
{
    int 	lhs_prdct, lhs_bot2, lhs_cur2, lhs_top2, lhs_arity, lhs_param, lhs_bot3, lhs_top3, lhs_arg, lhs_nt;
    int		rhs_cur, rhs_top, rhs_bot, rhs_prdct, rhs_bot2, rhs_cur2, rhs_nt, rhs_top2, k, rhs_param, rhs_bot3,
                rhs_cur3, rhs_top3, rhs_pos, rhs, lsymb, rsymb;
    int		max_rhs_pos, Ak;
    BOOLEAN	has_component, is_spcl;
	...
	if (rhs_nt > 0) {
		...
		if (rhs_param == 0 ||
		    is_in_Tstar (XH_X (rcg_parameters, rhs_param), XH_X (rcg_parameters, rhs_param+1))) {
		    is_spcl = TRUE;
...
gen_bnf ()
{
    ...
    if (is_CF_parser) {
	/* Allocations */
	arg_stack = (int*) sxalloc (max_clause_arg_nb+1, sizeof (int)), arg_stack [0] = 0;
	S1_alloc (lhs_param_stack, max_arg_size+1);
	rhs_pos2Ak = (int*) sxalloc (max_clause_arg_nb+1, sizeof (int));
	rhs_pos2bot3 = (int*) sxalloc (max_clause_arg_nb+1, sizeof (int));
	rhs_pos2top3 = (int*) sxalloc (max_clause_arg_nb+1, sizeof (int));
