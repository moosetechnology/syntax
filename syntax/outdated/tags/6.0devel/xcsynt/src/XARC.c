...
char WHAT_XCSYNTXARC[] = "@(#)SYNTAX - $Id: XARC.c 2227 2018-09-26 16:22:59Z garavel $" WHAT_DEBUG;

#include "SS.h"
#include "RLR.h"


extern SXVOID	print_bfsa (SXINT bfsa, SXINT ate);
extern SXVOID	print_pss (SXINT pss);
extern SXBOOLEAN	PARTIAL_FSA_ARP_construction (struct ARC_struct *XARC, SXINT fsa1, SXINT bfsa2);
extern SXINT	TOTAL_FSA_ARP_construction (struct ARC_struct *XARC, SXINT qtq);
extern SXINT	UNBOUNDED_ARP_construction (struct ARC_struct *XARC,
					    SXINT	qtq,
					    SXBOOLEAN message_needed);
extern SXBOOLEAN	make_inter_pss (SXINT pss1, SXINT pss2);
extern char	*conflict_kind_to_string (VARSTR result, SXINT conflict_kind, SXINT h_val, SXINT k_val);
extern SXBOOLEAN	bh_mark (struct ARC_struct *ARC, SXINT qtq);
extern SXBOOLEAN	bh_unmark (struct ARC_struct *ARC, SXINT qtq);


static SXBA		*X_plus;
static SXINT		X_plus_size;

static SXBA		fork_work_set, final_fork_set;
static SXINT		fork_work_set_m, fork_work_set_M, final_fork_set_m,
                        final_fork_set_M;
...
struct ipss_to_attr_global {
    SXINT repr, lnk, in, occur_nb;
};
struct ipss_to_attr_local {
    SXINT bfsa, local_bfsa;
};
struct ipss_to_attr {
...

static SXVOID XARC_UNBOUNDED_Q_oflw (SXINT old_size, SXINT new_size)
{
...
static SXVOID XARC_FSA_Q_oflw (SXINT old_size, SXINT new_size)
{
    sxuse(old_size); /* pour faire taire gcc -Wunused */
    ARCs [XARC_FSA].attr = (struct ARC_ATTR*) sxrealloc (ARCs [XARC_FSA].attr, new_size + 1, sizeof (struct ARC_ATTR));
}

static SXVOID	oflw_XQ0xXQ0 (SXINT old_size, SXINT new_size)
{
    sxuse(old_size); /* pour faire taire gcc -Wunused */
...
static SXVOID	oflw_forks (SXINT old_size, SXINT new_size)
{
    sxuse(old_size); /* pour faire taire gcc -Wunused */
...
static SXVOID	oflw_FORKxT (SXINT old_size, SXINT new_size)
{
    sxuse(old_size); /* pour faire taire gcc -Wunused */
    FORKxT_to_fks = (SXINT*) sxrealloc (FORKxT_to_fks, new_size + 1, sizeof (SXINT))
	 ;
}



static SXVOID	oflw_bfsa (SXINT old_size, SXINT new_size)
{
    SXINT i;

    bfsa_to_ate = (SXINT*) sxrealloc (bfsa_to_ate, i = new_size + 1, sizeof (SXINT));
...
static SXVOID	oflw_pss_to_seq (SXINT old_size, SXINT new_size)
{
    sxuse(old_size); /* pour faire taire gcc -Wunused */
...
static SXVOID	oflw_back (SXINT old_size, SXINT new_size)
{
    sxuse(old_size); /* pour faire taire gcc -Wunused */
...
static SXVOID	XARC_allocate (void)
{
...
#define to	4
	...
	Q0_to_occur_nb = (SXINT*) sxalloc (xac2, sizeof (SXINT));
	...
	FORKxT_to_fks = (SXINT*) sxalloc (XxY_size (FORKxT_hd) + 1, sizeof (SXINT));
	lgt [11] = 20;
	
	lpss = SS_alloc (xac2);
	
	if (is_automaton && is_listing_opened) {
	    bfsa_to_ate = (SXINT*) sxcalloc (XH_size (bfsa_hd) + 1, sizeof (SXINT));
	}
...
static SXVOID	fks_to_fork_set (SXINT xfks, SXBA set, SXINT *m, SXINT *M)
{
...
static SXVOID put_in_fork_work_set (SXINT bfsa, SXINT pss)
{
    SXINT	put_in_fork_work_set_fork;

    if (SXBA_bit_is_set (CUR_PSS_SET, pss)) {
	XxY_set (&forks, bfsa, pss, &put_in_fork_work_set_fork);
	
	if (SXBA_bit_is_reset_set (fork_work_set, put_in_fork_work_set_fork)) {
	    fork_work_set_m = min (fork_work_set_m, put_in_fork_work_set_fork);
	    fork_work_set_M = max (fork_work_set_M, put_in_fork_work_set_fork);
	}
    }
}

static SXVOID LRpi_Next (SXINT bfsa, SXINT pss)
{
    /* On memorise les resultats de l'application de Next sur un pss. */
    SXINT	x, top_state, n;
    SXINT			lim, ptn;

    sxinitialise(n); /* pour faire taire gcc -Wuninitialized */
...
static SXVOID couples_set_to_XH (XH_header *header)
{
    SXINT couple = 0;
...
static SXVOID XH_to_couples_set (SXINT *bot, SXINT *top)
{
    SXINT couple;
...
static SXVOID back_to_couples_set (SXINT ibfsa, SXINT X, SXINT Y)
{
...
static SXVOID	process_knot (SXBA ipss_set)
{
  ...
  SXINT			repr;
    ...
    repr = sxba_scan (ipss_set, 0);
    ipss_to_attr [repr].global.repr = new_ipss = ipss = repr;
...
static SXBOOLEAN check_loop_on_back (SXINT X1)
{
...
static SXBOOLEAN call_check_loop_on_back (void)
{
    SXINT ipss = 0;
...
static SXVOID down_occur_nb (SXINT X, SXINT nb);
static SXVOID up_occur_nb (SXINT Y, SXINT nb)
{
    SXINT X, i;
...
static SXVOID compute_occur_nb (SXINT R, SXINT N)
{
...
static VARSTR make_ibfsa_to_ate (VARSTR make_ibfsa_to_ate_vstr, 
				 SXINT ibfsa, 
				 SXINT X, 
				 SXINT Y)
{
    SXINT	ate, prev_lgth, nb;
    SXINT	*bot, *top;
    char		s [8];

    if ((nb = ipss_to_attr [X].global.occur_nb) != 0) {
	sprintf (s, "%ld:", (long)nb);
	varstr_catenate (make_ibfsa_to_ate_vstr, s);
    }

    if ((ate = ibfsa_to_ate [ibfsa]) == 0) {
	prev_lgth = varstr_length (make_ibfsa_to_ate_vstr);
	bot = IBFSA_BOT (ibfsa);
	top = IBFSA_TOP (ibfsa);
	...
	ibfsa_to_ate [ibfsa] = sxstrsave (varstr_tostr (make_ibfsa_to_ate_vstr) + prev_lgth);
    }
    else
	make_ibfsa_to_ate_vstr = varstr_catenate (make_ibfsa_to_ate_vstr, sxstrget (ate));
    ...
    return make_ibfsa_to_ate_vstr;
}

static SXINT make_ubfsa_to_ate (void)
{
    ...
    SXINT	top, back_elem, in_nb, ibfsa;
    SXINT			bot, x, X, bfsa;
	...
	sprintf (s, "<%ld>", (long)ipss_to_attr [1].local.bfsa);
	varstr_catenate (vstr, s);
    }
...
static SXINT make_bfsa_to_ate (SXINT bfsa_init, 
			       SXINT local_bfsa, 
			       SXINT xtnd_local_bfsa, 
			       SXINT bfsa, 
			       SXINT *ret)
{
    ...
    while (--top >= bot) {
	couple = *top;
	sprintf (s, "%ld-%ld", (long)XxY_Y (Q0xQ0_hd, couple), (long)XxY_X (Q0xQ0_hd, couple));
	varstr_catenate (vstr, s);
    ...
    *ret = sxstrsave (varstr_tostr (vstr));
    ...
    sprintf (s, "%ld <%ld> %ld", (long)*bot, (long)xtnd_local_bfsa, (long)*(bot + 2)); /* e <i> s */
    return sxstrsave (s);
}
...
static SXVOID fork_collecting (SXINT bfsa_init, SXINT pssxt)
{
    ...
    SXINT	x, x1, y, i;
    SXINT			y1, bfsa, X1, ibfsa, bot, top, init, local_bfsa, final, orig, local_bfsa_init, xtnd_local_bfsa;
...
static SXINT	set_to_fks (SXBA set, SXINT low, SXINT high)
{
    /* Fabrique un "ts" a partir de set */
    SXINT	set_to_fks_fork;
    SXINT			result;

    if (high == 0)
	sxtrap (ME, "set_to_fks");

    for (set_to_fks_fork = low; set_to_fks_fork != -1; set_to_fks_fork = sxba_scan (set, set_to_fks_fork)) {
	if (Q0 [TopState (XxY_Y (forks, set_to_fks_fork))].t_trans_nb != 0)
	    XH_push (fks_hd, set_to_fks_fork);

	if (set_to_fks_fork == high)
	    break;
    }
...
static SXVOID	reached_states (SXBA reached_set, SXBA reached_states_couples_set, SXBOOLEAN normal)
{
    /* reached_states_couples_set contient un ensemble d'element de Q0xQ0.
       ...
       de reached_states_couples_set, soit dans le sens normal (transitions de
       l'automate LR(0), soit dans le sens retrograde. */
    SXINT	couple, X, Y;
    SXBOOLEAN	is_stable = SXFALSE;

    while (!is_stable) {
	is_stable = SXTRUE;
	couple = 0;

	while ((couple = sxba_scan (reached_states_couples_set, couple)) > 0) {
		...
		SXBA_0_bit (reached_states_couples_set, couple);
    ...
    sxba_empty (reached_states_couples_set);
}



SXVOID make_inter (SXINT *bot1, 
		 SXINT *top1, 
		 SXINT *bot2, 
		 SXINT *top2, 
		 SXBOOLEAN normal, 
...
static SXVOID back_to_forest (void)
{
    SXINT		x, y, i;
    struct back	*aback;
    SXBA		set, loop_set;
    SXBOOLEAN			is_a_forest;
    SXINT				x1, x2, x1bx2, N, l;
    SXINT				R;
...
SXBOOLEAN	XARC_state_scan (struct ARC_struct *XARC, 
			 SXINT qtq, 
			 SXBOOLEAN (*F)(struct ARC_struct*, SXINT, SXINT, SXINT, SXINT, SXINT))
{
    /* Cette procedure parcourt les forks de XARC_state et pour chaque couple
       XARC_state_scan_fork1, fork2 dont les pss sont compatibles avec pss_sets, appelle
       F (XARC_state_scan_fork1, fork2, type1, type2).
       Si F retourne SXTRUE, la visite est interrompue. */
    SXINT	y2, y1, x2, x1;
    SXBA	pss_set1, pss_set2;
    SXINT			lim, top1, top2, XARC_state_scan_fork1, fork2, pss1, pss2, type1, type2;
    SXINT			xk1, xk2, XARC_state;
	...
	for (top1 = XH_X (fks_hd, y1 + 1), y1 = XH_X (fks_hd, y1); y1 < top1; y1++) {
	    pss1 = XxY_Y (forks, XARC_state_scan_fork1 = XH_list_elem (fks_hd, y1));
		...
		for (xk2 = xk1 + 1, x2 = x1 + 1; x2 <= lim; x2++, xk2++) {
			...
			if (SXBA_bit_is_set (pss_set2, pss2)) {
			    if (F (XARC, qtq, XARC_state_scan_fork1, fork2, type1, type2))
				return SXTRUE;
			}
		    }
		}
...
static SXBOOLEAN	FSA_case (struct ARC_struct *XARC,
			  SXINT	qtq, 
			  SXINT	FSA_case_fork1, 
			  SXINT	fork2, 
			  SXINT	type1, 
			  SXINT	type2)
{
    ...
    sxuse(type1); /* pour faire taire gcc -Wunused */
    sxuse(type2); /* pour faire taire gcc -Wunused */
    sxuse(qtq); /* pour faire taire gcc -Wunused */
    return ((pss1 = XxY_Y (forks, FSA_case_fork1)) == (pss2 = XxY_Y (forks, fork2)) ||
	make_inter_pss (pss1, pss2)) &&
	    PARTIAL_FSA_ARP_construction (XARC, XxY_X (forks, FSA_case_fork1), XxY_X (forks, fork2));
}

static SXBOOLEAN	fork_equality (struct ARC_struct *XARC,
			       SXINT qtq, 
			       SXINT fork_equality_fork1, 
			       SXINT fork2, 
			       SXINT type1, 
			       SXINT type2)
{
...
SXINT	is_LRpi (struct ARC_struct *XARC, SXINT qtq)
{
...
SXVOID	XARC_print (struct ARC_struct *XARC)
{
    /* Conflit sur germe, on imprime l'XARC correspondant a arc_no */
    SXINT	x, bfsa, y, z;
    SXINT			XARC_state, next_XARC_state, ate, XARC_print_fork;
    char		c_name [24], string [12];

    put_edit_nl (4);
    get_constructor_name (c_name, SXFALSE, (SXBOOLEAN) (XARC->pss_kind != UNBOUNDED_), XARC->h_value, XARC->k_value);
	...
	for (lim1 = XH_X (XARC->Q_hd, XARC_state + 1), z = XH_X (XARC->Q_hd, XARC_state); z < lim1
		...
		XARC_print_fork = XH_list_elem (fks_hd, y);
		put_edit_apnb (XARC_print_fork);
		put_edit_apnnl ("=(");
		sprintf (string, "<%ld>", (long)XxY_X (forks, XARC_print_fork));
		put_edit_apnnl (string);
		put_edit_apnnl (", ");
		sprintf (string, "%ld", (long)XxY_Y (forks, XARC_print_fork));
		put_edit_apnnl (string);
		put_edit_apnnl (")");
...
SXVOID	n_to_forks (struct ARC_struct *XARC,
		    SXINT XARC_state, 
		    SXINT n, 
		    SXINT type)
{
    /* n est l'indice dans bfsa_hd d'un bfsa. Retourne dans ws les forks
       ...
       a XARC_state, pss_sets tel que n_to_forks_fork = (bfsa, pss) et n designe bfsa. */
    SXINT	x, y, bfsa, xk;
    SXBA	pss_set;
    SXINT		pss, n_to_forks_fork, top, lim;

    for (xk = 0, lim = XH_X (XARC->Q_hd, XARC_state + 1), x = XH_X (XARC->Q_hd, XARC_state); x < lim; x++, xk++) {
	if (XH_list_elem (XARC->Q_hd, ++x) == type) {
	    y = XH_list_elem (XARC->Q_hd, x - 1);
	    pss_set = pss_sets [xk];

	    for (top = XH_X (fks_hd, y + 1), y = XH_X (fks_hd, y); y < top; y++) {
		pss = XxY_Y (forks, n_to_forks_fork = XH_list_elem (fks_hd, y));

		if (SXBA_bit_is_set (pss_set, pss)) {
		    if (n >= XH_X (bfsa_hd, bfsa = XxY_X (forks, n_to_forks_fork)) && n < XH_X (bfsa_hd, bfsa + 1))
			SS_push (ws, n_to_forks_fork);
		}
	    }
...
static SXVOID XARC_init_state_building (struct ARC_struct *XARC)
{
    
    /* Calcul de l'etat initial dans les cas d'un XARC (FSA_ ou UNBOUNDED_). */
    SXINT			cur_red;
    struct init_state_attr	*pinit;
    SXINT					pss_init, XARC_init_state_building_fork, bfsa_init, init_fork_list, xred_no,
	...
	XxY_set (&forks, bfsa_init, pss_init, &XARC_init_state_building_fork);
	XH_push (fks_hd, XARC_init_state_building_fork);
	XH_set (&fks_hd, &fks);
...
static SXBOOLEAN	XARC_trans (struct ARC_struct *XARC,
			    SXINT XARC_state, 
			    SXINT t, 
			    SXINT *next_XARC_state)
{
    ...
    SXINT	xfks, xpis, pss, t_trans_nb;
    SXINT			xpfks, bfsa, next_state, type, indice, lim1, lim2, indice1,
	                fks, xk, xkas, new_pss, ipss, XARC_trans_fork;

    sxinitialise(fks); /* pour faire taire gcc -Wuninitialized */
    sxinitialise(bfsa); /* pour faire taire gcc -Wuninitialized */
	...
	for (lim2 = XH_X (fks_hd, xpfks + 1), xfks = XH_X (fks_hd, xpfks); xfks
	    ...
	    XARC_trans_fork = XH_list_elem (fks_hd, xfks);
	    pss = XxY_Y (forks, XARC_trans_fork);

	    if (XxY_is_set (&Q0xV_hd, TopState (pss), -t)) {
		/* sur t */
		if ((XxY_is_set (&FORKxT_hd, XARC_trans_fork, t) == 0) && t != -bnf_ag.WS_TBL_SIZE.tmax) {
	...
	for (lim2 = XH_X (fks_hd, xpfks + 1), xfks = XH_X (fks_hd, xpfks); xfks
		...
		if (XxY_set (&FORKxT_hd, XARC_trans_fork, t, &indice1)) {
			...
			bfsa = XxY_X (forks, XARC_trans_fork);
			...
			if (t != -bnf_ag.WS_TBL_SIZE.tmax) {
				...
				XxY_set (&forks, bfsa, new_pss, &XARC_trans_fork);
			    else
				XARC_trans_fork = 0;
			}
...
/* else on garde le fork precedent. */
			...
			if (XARC_trans_fork != 0) {
			    SXBA_1_bit (fork_work_set, XARC_trans_fork);
			    fork_work_set_m = min (fork_work_set_m, XARC_trans_fork);
			    fork_work_set_M = max (fork_work_set_M, XARC_trans_fork);
			}
		    }
...
static SXINT XARC_get_conflict_kind (struct ARC_struct *XARC, 
				     SXINT XARC_state, 
				     SXBOOLEAN is_new_arc_state, 
				     SXINT t)
{
    SXINT		ck;
    struct ARC_ATTR	*attr = XARC->attr + XARC_state;
...
SXINT call_XARC_building (struct ARC_struct *ARC, SXINT qtq)
{
...
SXVOID	XARC_resize (SXINT old_xac2, SXINT new_non_lalr1_state_nb)
{
    ...
    sxuse(new_non_lalr1_state_nb); /* pour faire taire gcc -Wunused */
    if (xac2 > old_xac2) {
	couples_set = sxba_resize (couples_set, Q0xQ0_top + XxY_size (XQ0xXQ0_hd) + 1);
	LR0_states_set = LR0_sets [5];
	Q0_to_occur_nb = (SXINT*) sxrealloc (Q0_to_occur_nb, xac2, sizeof (SXINT));
    }
}
