...
char WHAT_XCSYNTLC0[] = "@(#)SYNTAX - $Id: LC0.c 2227 2018-09-26 16:22:59Z garavel $" WHAT_DEBUG;
...
static SXBOOLEAN
walk_forward (SXINT state, SXINT item, SXBOOLEAN (*f)(SXINT, SXINT))
{
    /* item : A -> alpha . X1 ... Xn
...
print_state (SXINT state)
{
    ...
    if (tnt > 0) {
	is_non_kernel = SXFALSE;

	XxY_Xforeach (LALC1, state, trans) {
	    next_state = XxY_Y (LALC1, trans);

	    if (next_state != state + 1) {
		sprintf (s1, " ---> %ld", (long)next_state);
		soritem (next_state - 1, 1, 71, "*", s1);
		is_non_kernel = SXTRUE;
	    }
	}
...
LC0 (void)
{
    ...
    SXBA	line;
    SXBOOLEAN	is_epsilon, look_ahead_needed;
...
/* On verifie que toutes les reductions sont generees.
   Le traitement des conflits ayant pu en supprimer. */
    ...
    is_non_deterministic_automaton = SXTRUE;
