...
#include "sxversion.h"
#include "sxunix.h"
char WHAT_LEXICALIZER_MNGR[] = "@(#)SYNTAX - $Id: lexicalizer_mngr.c 2227 2018-09-26 16:22:59Z garavel $" WHAT_DEBUG;
...
static SXBA*
bmm (SXBA *P, SXBA *Q, SXBA *R, SXINT l /*, SXINT m, SXINT n */)
{
  SXINT	i, j;
  SXBA	P_line, Q_line;
...
static void
idag_t_stack_recode (SXINT *old_t2new_t)
{
  SXINT   pq, y, lgth, new_t, max_pq, old_t;
  ...
  max_pq = idag.last_pq + 2; /* +2 pour les deux transitions spéciales sur EOF, à savoir 0..1 et final_state..final_state+1 */
...
#    if MEASURES || LOG
  printf ("%sThe multiple anchor occurrence post test erases " SXILD " productions.\n",
#      if MEASURES
	  "MEASURES: "
#      else /* LOG */
	  ""
...
#    if MEASURES || LOG
  printf ("%sThe multiple anchor sequence test erases " SXILD " productions.\n",
#      if MEASURES
	  "MEASURES: "
#      else /* LOG */
	  ""
...
static void
left_neighbour (SXINT A, SXBA t_set, SXBA nt_set)
{
  SXINT bot, item, X;
...
static void
right_neighbour (SXINT A, SXBA t_set, SXBA nt_set)
{
  SXINT bot, item, X;
...
#endif /* is_epsilon */
	  {
		...
		if ((B > 0) ? sxba_2op (w2_t_set, SXBA_OP_COPY, Asuf_set, SXBA_OP_AND, nt2first_set [B]) : SXBA_bit_is_set (Asuf_set, -B)) {
		  ...
		  for (i = prev_item+1; i < item; i++) {
			...
			if ((B > 0) ? sxba_2op (NULL, SXBA_OP_IS, nt2look_ahead_set [Xj], SXBA_OP_AND, w2_t_set) : SXBA_bit_is_set (nt2look_ahead_set [Xj], -B)) {
...
#if MEASURES || LOG
    printf ("%sThe check_rhs_sequences procedure in inside mode erases " SXILD " productions.\n",
#if MEASURES
	    "MEASURES: ",
#else /* LOG */
	    "",
...
#if MEASURES || LOG
    printf ("%sThe check_rhs_sequences procedure in left-corner neighbours mode erases " SXILD " productions.\n",
#if MEASURES
	    "MEASURES: ",
#else /* LOG */
	    "",
...
#if MEASURES || LOG
    printf ("%sThe check_rhs_sequences procedure in right-corner neighbours mode erases " SXILD " productions.\n",
#if MEASURES
	    "MEASURES: ",
#else /* LOG */
	    "",
...
{
  SXINT             item, nb, X, prod, prev_prod, A, bot, top, bot_item, cur_item, unlexicalized_prod_nb;
  SXINT             new_prod_nb = 0;
  ...
  /* prod2rhs_nt_nb doit etre initialisé a 0 */
...
#if LLOG
	fputs ("This production generates a terminal or empty string:", stdout);
	output_prod (prod);
#endif /* LLOG */
  ...
  while (!IS_EMPTY (working_nt_stack)) {
    ...
    bot = spf.inputG.npd [X]; //offset dans spf.inputG.numpd du début de la pile des items ayant X en partie droite
    ...
    while ((item = spf.inputG.numpd [bot])) {
      ...
      if (prod == prev_prod) {
	/* Ceci est pour éviter le cas suivant: supposons que X soit A, et qu'on ait la prod A -> Y X X.
	   Supposons également que Y soit inutilisable au vu du source. Alors sans le test ci-dessus, 
	   on passerait une première fois (pour l'item A -> Y • X X), on aurait nb == 0, on calculerait
	   donc le prod2rhs_nt_nb [prod], et il vaudrait 1 (puisque X ne compte pas:  on a fait
	   SXBA_1_bit (tgen_nt_set, X). On passerait une seconde fois (pour l'item A -> Y X • X), on aurait
	   là un nb non nul, égal à 1, et on tomberait dans le nb--: on aurait l'impression de devoir faire 
	   nb-- au titre du deuxième X, qui est bel et bien "connu comme bon", alors qu'en vrai ce nb--
	   reviendrait à traiter Y comme bon de façon erronée */
	bot++;
	continue;
      }
      
      if (prod > 0) {
	/* On est sûr que prod n'est pas la super-re`gle */
	A = spf.inputG.lhs [prod];
	...
	if (!SXBA_bit_is_set (spf.inputG.BVIDE, A))
...
#endif /* is_epsilon */
	  {
		...
		if (nb == 0) {
		  /* prod jamais vue (et non lexicalisee, sinon ça aurait été initialisé à > 0 ou à -1) */
...
#if LLLOG
		  printf ("  Newly studied production has a rhs_nt_nb value of " SXILD ": ", nb);
		  output_prod (prod);
#endif /* LLLOG */
		}
		else {
...
#if MEASURES || LOG
  printf ("%sFirst Reduced Grammar = (|N| = " SXILD ", |T| = " SXILD ", |P| = " SXILD "(" SXILD "), |G| = " SXILD ")\n",
#if MEASURES
	  "MEASURES: "
#else /* LOG */
	  ""
...
cycle_check ()
{
  ...
  if (is_cycle) {
    sxtmsg (sxsvar.sxlv.terminal_token.source_index.file_name,
	    "The lexical filtering results in a reduced cyclic sub-grammar.",
...
#if LLOG
	 fputs ("This production generates a non empty terminal string (2):", stdout);
#ifdef MAKE_INSIDEG
	 prod = spf.insideG.prod2init_prod [prod];
		...
		if (!SXBA_bit_is_set (nrg_working_nt_set, A)) {
		  ...
		  if (nb == 0) {
		    if (SXBA_bit_is_reset_set (nrg_working_nt_set, A))
		      PUSH (nrg_working_nt_stack, A);
#if LLOG
		    fputs ("This production generates a non empty terminal string (3):", stdout);
#  ifdef MAKE_INSIDEG
		    old_prod = spf.insideG.prod2init_prod [prod];
#  else /* ifndef MAKE_INSIDEG */
		    old_prod = prod;
...
#if MEASURES || LOG
  printf ("%sNext Reduced Grammar = (|N| = " SXILD ", |T| = " SXILD ", |P| = " SXILD "(" SXILD "), |G| = " SXILD ")\n",
#if MEASURES
	  "MEASURES: "
#else /* LOG */
	  ""
...
output_G (char *header, SXBA basic_item_set, SXBA basic_prod_set)
{
  ...
  printf ("%s%s = (|N| = " SXILD ", |T| = " SXILD ", |P| = " SXILD ", |G| = " SXILD ")\n",
#if MEASURES
	  "MEASURES: "
#else /* LOG */
	  ""
	      ...
	      )
{
...
#if LLOG
  {
    ...
    printf ("Pass LR [?.." SXILD "] recognizes production ", mlstn);
    output_prod (old_prod);
  }
#endif /* LLOG */
		 ...
		 )
{
...
#if LLOG
  {
    ...
    printf ("Pass RL [" SXILD "..?] recognizes production ", mlstn);
    output_prod (old_prod);
  }
#endif /* LLOG */
...
static void
make_insideG (SXINT *old_prod2new_prod)
{
...
#if MEASURES || LOG
  printf ("%sinsideG = (|N| = " SXILD ", |T| = " SXILD ", |P| = " SXILD "(" SXILD "), |G| = " SXILD ")\n",
#if MEASURES
	  "MEASURES: "
#else /* LOG */
	  ""
...
#if MEASURES || LOG
  printf ("%sspf.insideG = (|N| = " SXILD ", |T| = " SXILD ", |P| = " SXILD "(" SXILD "), |G| = " SXILD ")\n",
#if MEASURES
	  "MEASURES: "
#else /* LOG */
	  ""
...
#if MEASURES || LOG
    if (nb > 0)
      printf ("%sThe multiple anchor occurrence pre test erases " SXILD " productions.\n",
#if MEASURES
	      "MEASURES: "
#else /* LOG */
	      ""
...
#ifndef HUGE_CFG
static void
copy_inputG_in_insideG (void)
{
  SXINT i, pos_tmax;

  spf.insideG.maxt = tmax;
  spf.insideG.maxnt = ntmax;
  spf.insideG.maxitem = itemmax;
  spf.insideG.maxprod = prodmax;
  spf.insideG.maxrhs = rhs_lgth;
  spf.insideG.maxrhsnt = rhs_maxnt;
  spf.insideG.is_eps = is_epsilon;
  spf.insideG.right_recursive_set = is_right_recursive ? (SXBA)1 /* truc non-NULL du bon type: ne sera utilisé (if ever) que sous la forme d'un test regardant si c'est NULL ou pas, i.e., si on est right_recursive ou pas */ : NULL;
  spf.insideG.is_cycle = is_cyclic;

  spf.insideG.lispro = spf.inputG.lispro;
  spf.insideG.prolis = spf.inputG.prolis;
  spf.insideG.prolon = spf.inputG.prolon;
  spf.insideG.lhs = spf.inputG.lhs;
  spf.insideG.npg = spf.inputG.npg;
  spf.insideG.numpg = spf.inputG.numpg;
  spf.insideG.npd = spf.inputG.npd;
  spf.insideG.numpd = spf.inputG.numpd;
  spf.insideG.tpd = NULL;
  spf.insideG.tnumpd = NULL;
  spf.insideG.prod2init_prod = (SXINT*) sxalloc (prodmax+1, sizeof(SXINT));
  spf.insideG.nt2init_nt = (SXINT*) sxalloc (ntmax+1, sizeof(SXINT));
  for (i = 0; i <= prodmax; i++)
    spf.insideG.prod2init_prod [i] = i;
  for (i = 0; i <= ntmax; i++)
    spf.insideG.nt2init_nt [i] = i;
  pos_tmax = -tmax;
  spf.insideG.t2init_t = (SXINT*) sxalloc (pos_tmax+1, sizeof(SXINT));
  for (i = 0; i <= pos_tmax; i++)
    spf.insideG.t2init_t [i] = i;
  
  spf.insideG.rhs_nt2where = rhs_nt2where;
  spf.insideG.lhs_nt2where = lhs_nt2where;
  
  spf.insideG.bvide = spf.inputG.BVIDE;
  spf.insideG.titem_set = titem_set;
  spf.insideG.rc_titem_set = rc_titem_set;
  spf.insideG.empty_prod_item_set = 
#if is_epsilon
    empty_prod_item_set;
#else /* is_epsilon */
    NULL;
#endif /* is_epsilon */
  spf.insideG.left_corner = left_corner;
  spf.insideG.nt2item_set = nt2item_set;
  spf.insideG.right_corner = NULL;
  spf.insideG.nt2rc_item_set = (SXBA*)nt2rc_item_set;
  spf.insideG.valid_prefixes = (SXBA*)valid_prefixes;
  spf.insideG.valid_suffixes = (SXBA*)valid_suffixes;
  spf.insideG.t2prod_item_set = (SXBA*)t2prod_item_set;
  spf.insideG.t2item_set = t2item_set;
  spf.insideG.nt2min_gen_lgth = nt2min_gen_lgth;
}
#endif /* HUGE_CFG */
...
t_set2basic_item_set (void)
{
  ...
  printf ("%sinputG = (|N| = " SXILD ", |T| = " SXILD ", |P| = " SXILD "(" SXILD "), |G| = " SXILD ")\n",
#if MEASURES
	  "MEASURES: "
#else /* LOG */
	  ""
#endif /* LOG */
	  , (SXINT)inputG_MAXNT
	  , (SXINT)inputG_SXEOF-1
	  , (SXINT)inputG_MAXPROD
	  , unlexicalized_prod_nb
	  , (SXINT)inputG_MAXITEM);
#endif /* MEASURES || LOG */
...
#ifdef multiple_t_item_set_is_not_empty
...
#endif /* multiple_t_item_set_is_not_empty */
#endif /* ifndef HUGE_CFG */
...
#if is_set_automaton
    if (IS_DYNAMIC_SET_AUTOMATON && is_axiom_kept) {
      if ((is_axiom_kept = dynamic_set_automaton ())) {
	/* dynamic_set_automaton a marche, retourne-t-il automatiquement une grammaire reduite ?? */
	/* Non Ex :
	   S -> X \alpha X
	   X -> \eps
...
#  ifdef HUGE_CFG
    if (!spf.insideG.is_allocated) {
      ...
      */
      sxtmsg (sxsvar.sxlv.terminal_token.source_index.file_name,
	      "No reduced sub-grammar can be constructed from the input text.",
	      sxplocals.sxtables->err_titles [2]);
#if 0
      SXEXIT (2);
#endif /* 0 */
...
#  ifdef MAKE_INSIDEG
  if (ret_val && is_smallest_insideG && (IS_CHECK_RHS_SEQUENCES != 0 || IS_DYNAMIC_SET_AUTOMATON) && !sxba_is_full (basic_item_set)) {
    /* Ici tout a marche' et une insideG est remplie */
    /* ... et basic_item_set est restreint  */
    /* && il est demandé de l'incorporer dans un nouvel insideG */
    make_smallest_insideG ();
  }
#  endif /* MAKE_INSIDEG */
#endif /* !MAKE_PROPER */
  ...
  /* Ds le cas MAKE_INSIDEG source_set et glbl_source ont ete recodés */
  if (
#ifdef HUGE_CFG
      ret_val &&
#endif /* HUGE_CFG */
     ...
     SXBA *lb2prod_item_set;
{
  ...
  if (lrprod_filtered_nb)
    fprintf (sxstderr, "lrprod_filtered_nb = " SXILD " ", lrprod_filtered_nb);
...
#if EBUG2
    sprintf (str, "\tRFSA[t=" SXILD "/c=" SXILD "/ic=" SXILD "] (%s)",
	     tnb,
	     fsaG->fsa_valid_prod_set ? sxba_cardinal (fsaG->fsa_valid_prod_set) : 0,
	     fsaG->instantiated_prod_nb,
	     ret_val ? "SXTRUE" : "SXFALSE");
#else
    sprintf (str, "\tRFSA[t=" SXILD "] (%s)",
	     tnb,
	     ret_val ? "SXTRUE" : "SXFALSE");
#endif
...
static void
generate (SXINT min_proto, SXINT lgth)
{
...
#    if MEASURES || LOG
  printf ("%sThe check_generate test validates " SXILD " productions.\n",
#      if MEASURES
	  "MEASURES: "
#      else /* LOG */
	  ""
