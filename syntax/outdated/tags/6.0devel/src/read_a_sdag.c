...
#include <stdarg.h>
#include <setjmp.h>
char WHAT_READASDAG[] = "@(#)SYNTAX - $Id: read_a_sdag.c 2291 2020-03-10 15:30:10Z garavel $" WHAT_DEBUG;
  ...
  case SXACTION:
	...
	<TERMINAL>	= %form				; 1
      */
      ...
      if (prev_ste > ste_set_size)
	/* prudence sur ce qui est fait a l'exterieur */
	ste_set = sxba_resize (ste_set, (ste_set_size *= 2) + 1);
...
sxparser_sdag_tcut  (SXINT what_to_do, struct sxtables *arg )
{
  ...
  SXINT   (*semact_saved) (SXINT what, ...) = (SXINT(*)(SXINT, ...)) NULL;
  ...
  case SXACTION:
	...
	ste_set = sxba_calloc ((ste_set_size = SXSTRtop()) + 1);
  ...
  case SXINIT:
    ...
    if (sdag_parser)
      (*sdag_parser) (what_to_do /* était SXINIT, mais on comprend pas pourquoi on ferait SXINIT quand on vient depuis SXCLOSE */, arg);
