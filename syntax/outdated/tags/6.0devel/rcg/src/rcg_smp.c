...
#include <string.h>
#include <stdlib.h>
char WHAT_RCGSMP[] = "@(#)SYNTAX - $Id: rcg_smp.c 2227 2018-09-26 16:22:59Z garavel $" WHAT_DEBUG;
...
rcg_pd (void)
{
	...
	if (SXVISITED->father->name == PREDICATE_n) {
		...
		if (SXVISITED->token.lahead == false_t)
	      ...
	      SXINT tokenste = SXVISITED->token.string_table_entry;
	      SXINT nt = 0;
		...
		sxtrap (ME, "rcg_pd");
	      ...
	      XH_push (rcg_predicates, nt);
		
	      if (ste2nt [tokenste] == 0) {
		ste2nt [tokenste] = nt;
		nt2ste [nt] = tokenste;
	      }
	    }
	...
	if (!is_first_pass) {
	  ...
	  if (*str == '"' && sxstrlen (ste) == 1) {
		...
		if (SXVISITED->father->degree == 1 &&
		  ...
		  /* ... 1er arg de &StrLen */
		  if (cur_t > max_len_in_strlen)
		    max_len_in_strlen = cur_t;
...
Xref (void)
{
  ...
  undefined_nt_set = sxba_calloc (max_ste+1);
  ...
  if ((nt = sxba_scan (undefined_nt_set, 0)) > 0) {
    ...
    sxtmsg (sxsrcmngr.source_coord.file_name,
	    "The following predicate%s undefined:\n{%s}\n",
	    sxtab_ptr->err_titles [2],
	    nt_nb > 1 ? "s are" : " is",
	    varstr_tostr (vstr));
...
is_productive (void)
{
  ...
  if ((nt = sxba_0_lrscan (nt_set, 0)) > 0) {
    ...
    sxtmsg (sxsrcmngr.source_coord.file_name,
	    "The following predicate%s non productive:\n{%s}\n",
	    sxtab_ptr->err_titles [2],
	    nt_nb > 1 ? "s are" : " is",
	    varstr_tostr (vstr));
  ...
  */
  ...
  for (A = 1; A <= max_nt; A++) {
	...
	if (!SXBA_bit_is_set (counter_Ak_set, Ak) && (sxba_is_empty (first [Ak]) || sxba_is_empty (last [Ak]))) {
	  if (check_instantiable_clause)
	    sxtmsg (sxsrcmngr.source_coord.file_name,
		    "The %ld%s parameter of predicate <%s> is non instantiable.",
		    sxtab_ptr->err_titles [2]+1,
		    k+1,
		    k==0?"st":(k==1?"nd":(k==2?"rd":"th")),
		    sxstrget (nt2ste [A]));
...
static SXINT		infix_args_top, infix_args_size, iargs2arg_disp_top, iargs2arg_disp_size, arg_memo_top, arg_memo_size,
                        max_iarg_rel;
#define TOP_DOWN_ERASED	(-SXINT_MAX)
...
smppass (void)
{
    ...
    nt2ste += (-CUT_ic); /* comme ça nt2ste [nt] sera une zone allouée valide que nt soit > 0 (prédicat standard) ou négatif (entre CUT_ic et -1 compris, prédicat prédéfini) */
