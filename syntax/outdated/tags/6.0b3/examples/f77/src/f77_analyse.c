...
#include "f77_td.h"
#include <ctype.h>
char WHAT_F77ANALYSE[] = "@(#)SYNTAX - $Id: f77_analyse.c 1175 2008-03-10 12:00:24Z rlacroix $" WHAT_DEBUG;

/* Il faudrait INTERDIRE l'insertion d'un EOL par le rattrapage d'erreur
   du scanner. Utiliser sxs_srcvr? */


extern BOOLEAN		is_ansi, is_pretty_printer; /* Options d'appel */
...
#ifdef EBUG
static VOID print_toks_buf (void)
{
    SXINT i;
    struct sxtoken	*pt;
...
static BOOLEAN fill_buf (void)
{
    SHORT c;
    SXUINT x;
    BOOLEAN	ligne_vide = FALSE, is_an_initial_line, is_header_empty;
    struct sxsource_coord source_index;
...
static VOID get_next_source_item (void)
{
...
static VOID do_stack_mngr (char *str, struct sxsource_coord *source_index)
{
    SXINT	val;
...
static BOOLEAN check_balanced (SXINT *tok_no)
{
...
static BOOLEAN check_for_a_lhs (SXINT tok_no)
{
...
static SXVOID check_id_lgth (struct sxtoken *tt)
{
	...
	if (Tlen == NULL)
	    ...
	    BOOLEAN	*low = Tlen + Tlen_size, *high;
...
static BOOLEAN check_FUNCTION (char *str, SXINT strlgth, SXINT l, SXINT Mtok_no)
{
...
static VOID process_initial_id (SXINT Mtok_no, BOOLEAN after_if_log)
{
...
static BOOLEAN check_stmt_sequence (SXINT lahead, SXINT tok_no, SXINT newla)
{
...
static BOOLEAN check_block_data_subprogram (SXINT lahead, SXINT tok_no)
{
