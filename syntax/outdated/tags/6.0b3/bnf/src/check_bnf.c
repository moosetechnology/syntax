...
#include "XH.h"
#include "XxY.h"
char WHAT_CHECKBNF[] = "@(#)SYNTAX - $Id: check_bnf.c 1209 2008-03-12 15:58:48Z rlacroix $" WHAT_DEBUG;
...
print_old_prod (SXINT prod, SXINT A)
{
  ...
  printf ("%ld: %s = ", (long)prod, get_nt_string (A));
...
left_recursion_elimination (SXBA lr_nt_set /*, rr_nt_set */)
{
  ...
  while ((A = sxba_scan (lr_nt_set, A)) > 0) {
	...
	if (is_debug2) {
	  printf ("*Into a duplicate of prod #%ld:  ", (long)prod_no);
	  print_prod (prod_no, &(REDUC (lr_prod_hd, prod_no)), &(REDUC (lr_prod_hd, prod_no+1)), (SXINT*)NULL);
	}
      }
    }
