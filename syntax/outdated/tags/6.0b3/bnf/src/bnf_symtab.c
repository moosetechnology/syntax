...
#include "bnf_vars.h"
#include "varstr.h"
char WHAT_BNFSYMTAB[] = "@(#)SYNTAX - $Id: bnf_symtab.c 1102 2008-02-25 15:21:58Z rlacroix $" WHAT_DEBUG;

static SXINT	st_get_nt_code (SXINT ste, BOOLEAN has_prdct);
static SXINT	st_get_t_code (struct sxsource_coord source_index, SXINT ste, BOOLEAN is_gene, BOOLEAN has_prdct);
static SXINT	st_get_xnt_code (SXINT ste, SXINT xste, SXINT pste);
static SXINT	st_get_xt_code (struct sxsource_coord source_index, SXINT ste, SXINT xste, SXINT pste, BOOLEAN is_gene);
...
static VOID	set_xsymbols (NODE *visited)
{
...

static VOID	pass_indpro_size (NODE *visited)
{
    SXINT ste;
/*
D E R I V E D
...
Check for a missing semi-colon.", sxtab_ptr->err_titles [1], x_rule);
	...
	if (VS.string_table_entry == (SXUINT)EOF_ste) {
...
VOID	symbol_table_processing (NODE *adam)
{
    SXINT		st_top;
