...
struct attribute {
    ...
    NODE	*declarator; 	/* DECLARATOR_n ou DECLARATOR_FUNCTION_n */
};
...
static BOOLEAN		has_declarator, is_in_type_name;
...
static void dc_pi () {
	...
	type_specifier = t_to_type (type_specifier, VISITED->token.lahead, &(VISITED->token.source_index));
	break;
	...
	case 1 :/* VISITED->name = {POINTER_n, VOID_n} */
		...
		declarator_n_ptr = VISITED->father;
	...
	type_specifier = t_to_type (type_specifier, VISITED->token.lahead, &(VISITED->father->token.source_index));
	break;
...
static void dc_pd () {
    ...
    case IDENTIFIER_n :
	if (VISITED->father->name == OPERATOR_PTR_n &&
	    VISITED->position == 2) {
	    /* name_id -> id */
	...
	else if (VISITED->father->name == OPERATOR_DOT_n &&
		 VISITED->position == 2) {
	    /* name_id . id */
	...
	else if (VISITED->father->name == TAG_NAME_n) {
	    int x, kind;
		...
		if (name_id == 0) {
		    name_id = process_tag_name (VISITED->token.string_table_entry,
						type_specifier & BASIC_TYPE_RANGE_,
						0, /* declaration */
						&(LEFT->token.source_index));
		}
	...
	else if (VISITED->father->name == SPECIFIER_NAMED_STRUCT_n) {
	    name_id = process_tag_name (VISITED->token.string_table_entry,
					type_specifier & BASIC_TYPE_RANGE_,
					new_struct_level + 1,
					&(LEFT->token.source_index));
	...
	if (operand_type.name_or_type != 0) {
	    ...
	    if (operand_type.name_or_type == 0)
		sxput_error (LAST_ELEM->token.source_index,
			     "%sIllegal indexing.",
			     sxplocals.sxtables->err_titles [2]);
	}
