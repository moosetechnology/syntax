...
#ifndef lint
#define WHAT	"@(#)L_act.c\t- SYNTAX [unix] -  Mar 15 Mar 1994 11:11:03"
static struct what {
  struct what	*whatp;
...
static char LANGUAGE_NAME [] = "L";
char *language_name = &(LANGUAGE_NAME [0]);
...
/*
  ...
  Difficultes :
  - Provoque des problemes avec DO/UNDO/ACTION, puisque maintenant il peut y avoir
    ...
    BOOLEAN delete;
{
    ...
    /* Si on est dans UNDO, les regles ne doivent pas etre detruites mais
    ...
    BOOLEAN delete;
{
    ...
    /* Si on est en "UNDO", on conserve les entrees des tables de symboles
       qui ont pu etre construites au cours du "DO" afin de retrouver les
       memes valeurs sur "ACTION" */
    ...
    int		what, act_no;
{
    ...
    case OPEN:
        ...
        semact = sxplocals.SXP_tables.semact;
	sxplocals.SXP_tables.semact = L_semact;
	...
	sxsymbol_table_put (&symbol_table, EMPTY_STE, 0, &name);
	...
	sxsymbol_table_put (&type_table, EMPTY_STE, 0, &name);
...
*/
	...
	case 13: /* <SUBTYPE_NAME+> = <SUBTYPE_NAME+> , %id @13 ;
		...
		if (prev_xname == 0)
		{
		    ...
		    /* On code de facon qu'un "ACTION" qui suit un "DO" genere
		    ...
		    /* Il faut assurer qu'un bloc visite' en DO puis en ACTION aura
		       le meme current_block_nb. */
		    /* Ces couples seront "recuperes" en sortie de bloc sur "ACTION" */
			...
			/* On "recuperera" ces noms en sortie de bloc "ACTION" */
	...
	case 14: /* <STMT>	= <LABEL> @14 <STMT> ; */
	    ...
	    rule = sem_stack [STACKnewtop () - 1].rule;
	    sxdp_rule_deactivate (rule);
...
/* @20 et @21 doivent-ils etre mis dans DO/UNDO ?? */
	case 20: /* <STMT> = with <RECORDTYPE_LHS> do @20 <STMT> ";" @21 ; */
		...
		rule = sem_stack [STACKnewtop () - 2].rule;
	...
	/* Sur la sous-chaine : "... { goto ..." on se trouve dans l'etat s apres transition
	   ...
	   goto ...
	   Meme chose sur : "... { A : ..."
	   On est dans le cas DO/UNDO. En fait comme c'est la seule action possible, on pourrait
	   se contenter de faire DO (sans UNDO) et de ne rien faire sur ACTION (@1).
    ...
    int		what, act_no;
{
	...
	sem_stack [STACKnewtop ()].rule = sxpglobals.reduce;
