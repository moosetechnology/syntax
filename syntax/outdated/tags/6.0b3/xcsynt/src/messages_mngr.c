...
char WHAT_XCSYNTMESSAGESMNGR[] = "@(#)SYNTAX - $Id: messages_mngr.c 1128 2008-03-04 11:49:22Z rlacroix $" WHAT_DEBUG;
...
extern BOOLEAN	is_t_in_first (SXINT t, SXINT item);
extern BOOLEAN	ttrue (SXINT item, SXINT LA_ref, BOOLEAN is_prefixe);
extern BOOLEAN	earley (SXINT item, 
			SXINT q, 
			SXINT LA, 
			SXINT LA_lgth, 
			XH_header *hd, 
			SXINT BS2);
extern BOOLEAN	is_pss_include (SXINT *bot1, SXINT *top1, SXINT *bot2, SXINT *top2);
extern SXBA	fill_pss_set (struct ARC_struct *ARC, 
			      ...
			      SXINT type, 
			      SXBA set);
extern BOOLEAN	bh_ARC_one_LA (struct ARC_struct *ARC, SXINT qtq);
extern BOOLEAN	bs_ARC_one_LA (struct ARC_struct *ARC, SXINT qtq);
...

static struct bstr	*bstr_cat_Vs (struct bstr *bstr, SXINT Vs, BOOLEAN is_bot_up)
{
...
#if 0
static VOID compute_items_np (SXINT cur_index, SXINT top_index, SXINT write_index, BOOLEAN is_shift)
{
...
static BOOLEAN is_conflict_prod_reached (SXINT **bot_items)
{
...
static BOOLEAN ecrire (BOOLEAN should_write)
{
    /* BOT_items et TOP_items sont des pointeurs dans stack_hd dans une liste d'items. */
    SXINT	x, item, tnt, nb;
    SXINT			red_no, *ptr;
...
static VOID	print_a_quad (BOOLEAN is_eof, SXINT Vs, SXINT items/* , Ts */ /*, delta_items */)
{
...
BOOLEAN erase_quads (SXINT ARC_kind, SXINT qtq)
{
...
BOOLEAN make_a_quad (SXINT etat_charniere, SXINT item_charniere, SXINT *items_stack)
{
...
static BOOLEAN call_earley (SXINT call_earley_fork)
{
...
static BOOLEAN conformity_parse_stack_pss (SXINT *parse_stack, SXINT pss)
{
...
static BOOLEAN conformity_LALR (SXINT *parse_stack)
{
...
BOOLEAN check_real_conformity (SXINT *parse_stack)
{
...
static BOOLEAN	ARP_knot_xpath (SXINT entree, SXINT sortie, SXINT *kbot, SXINT *ktop)
{
...
static BOOLEAN one_ambig_LA (struct ARC_struct *XARC,
...
static BOOLEAN is_in (SXINT path1, SXINT path2)
{
...
static BOOLEAN fork_couple_not_processed (SXINT fork_couple_not_processed_fork1, SXINT fork2)
{
