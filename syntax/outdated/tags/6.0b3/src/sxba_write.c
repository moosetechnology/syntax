...
char WHAT_SXBA_WRITE[] = "@(#)SYNTAX - $Id: sxba_write.c 815 2007-10-15 11:38:13Z rlacroix $" WHAT_DEBUG;

BOOLEAN
sxba_write (filedesc_t file, SXBA bits_array)
/*
...
{
    SXBA_INDEX	bytes_number = sxba_cast (NBLONGS (BASIZE (bits_array)) * sizeof (SXBA_ELT));
...
/* Sinon, produire une ft qui fera a la fois l'allocation dynamique et les initialisations
   ...
   {
      static SXINT elems_%name [] = {e1, ..., en};
...
void
sxbm2c (FILE *file_descr, SXBA *bits_matrix, SXBA_INDEX line_nb, char *name1, char *name2, BOOLEAN is_static, VARSTR vstr)
{
