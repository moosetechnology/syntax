...
char WHAT_XXY_MNGR[] = "@(#)SYNTAX - $Id: XxY_mngr.c 1009 2008-02-12 15:18:48Z rlacroix $" WHAT_DEBUG;
...
BOOLEAN	XxY_set (XxY_header *header, SXINT X, SXINT Y, SXINT *ref)
{
    /* Si l'element X-Y de la matrice creuse header existe, cette fonction
...
BOOLEAN	XxY_set_and_close (XxY_header *header, SXINT X, SXINT Y, SXINT *ref)
{
...
void	XxY_to_SXBM (XxY_header *header, BOOLEAN (*get_couple) (SXINT*, SXINT*, SXINT), SXINT couple_nb, X_header *index_hd, SXBA **M)
{
...
BOOLEAN XxY_write (XxY_header *header, filedesc_t F_XxY /* file descriptor */)
{
    SXINT		bytes, kind;
    X_header	*X_hd;
    SXINT		*lnk_hd, *lnk;
...
BOOLEAN XxY_read (XxY_header *header, filedesc_t F_XxY /* file descriptor */, char *name, void (*user_oflw) (SXINT, SXINT), FILE *stat_file)
{
    SXINT		bytes, kind;
    X_header	*X_hd;
    SXINT		*lnk_hd, *lnk;
...
void XxY_array_to_c (XxY_header *header, FILE *F_XxY /* named output stream */, char *name)
{
	...
	fprintf (F_XxY, "{%ld, %ld}, ", (long)header->display [i].X, (long)header->display [i].Y);
    } 
	...
	if ((X_hd = header->X_hd [kind]) != NULL) {
	    ...
	    sprintf (sub_name + i, "_X_hd_%ld", (long)kind);
		...
		fprintf (F_XxY, "%ld, ", (long)header->lnk_hd [kind] [i]);
	    } 
		...
		fprintf (F_XxY, "%ld, ", (long)header->lnk [kind] [i]);
	    } 
...
void XxY_to_c (XxY_header *header, FILE *F_XxY /* named output stream */, char *name, BOOLEAN is_static)
{
