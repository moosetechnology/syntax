...
#ifndef VARIANT_32
char WHAT_SXNDP_RECOVERY[] = "@(#)SYNTAX - $Id: sxndp_rcvr.c 1026 2008-02-14 09:36:03Z rlacroix $" WHAT_DEBUG;
#else
char WHAT_SXNDP_RECOVERY[] = "@(#)SYNTAX - $Id: sxndp_rcvr.c 1026 2008-02-14 09:36:03Z rlacroix $ SXNDP_RCVR_32" WHAT_DEBUG;
#endif
#if EBUG
...
extern BOOLEAN			set_next_trans (struct SXP_bases *abase, 
						SXINT *check, 
						SXINT *action, 
						SXINT Max, 
						SXINT *next_action_kind);
extern BOOLEAN			sxndsubparse_a_token (SXINT parser, SXINT ref, SXINT symbol, SXINT (*f) (SXINT, SXINT, SXINT) );
...
extern SXINT                      mreds_push (SXINT parser, SXINT ref, SXINT tok_no, BOOLEAN is_new_triple, BOOLEAN is_shift_reduce, SXINT symbol);
...
static BOOLEAN	morgan (char *c1, char *c2)
...
static SXINT set_token_dag (BOOLEAN to_be_cloned, 
			    BOOLEAN is_copy,
			    struct sxtoken *ptok,
			    SXINT  tok_no, 
			    SXINT ms1, 
...
static BOOLEAN correction_already_found (SXINT *model, SXINT lim)
{
...
static BOOLEAN validate_submodel (SXINT model_no, SXINT *model, SXINT parser, SXINT pos, SXINT bot, BOOLEAN a_la_rigueur)
{
...
static BOOLEAN validate_model (SXINT parser, SXINT model_no, SXINT pos)
{
...
static BOOLEAN try_a_model (SXINT pos, SXINT parser_seed, SXINT model_no, BOOLEAN check_first_trans, SXINT tok_no_to_be_checked)
{
    SXINT 		*model = sxplocals.SXP_tables.P_lregle [model_no];
    SXINT 		lmod = model [0];
...
static BOOLEAN check_for_spelling (SXINT model_no)
{
...
static BOOLEAN is_a_right_ctxt (SXINT head, SXINT tail)
{
...
static VOID nd2_try_a_correction (void)
{
...
static VOID nd2_unfold (SXINT prev_tok_no, SXINT mstn, SXINT n, SXINT lim)
{
    SXINT 			xyz, tok_no;
...
static BOOLEAN 	global_recovery (void)
{
...
static BOOLEAN	less_equal (SXINT z1, SXINT z2)
{
...
static BOOLEAN	ndp_recovery (void)
{
    char				*msg_params [5];
    struct SXP_local_mess		*local_mess;
    struct sxtoken			*tok;
...
BOOLEAN		sxndprecovery (SXINT what_to_do)
{
...
BOOLEAN		sxndpallcorrections (SXINT what_to_do)
{
    SXINT i;
...
BOOLEAN		sxndpsrecovery (SXINT what_to_do)
{
