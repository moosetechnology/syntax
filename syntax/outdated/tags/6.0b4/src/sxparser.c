...
#include "sxunix.h"
#ifndef VARIANT_32
char WHAT_SXPARSER[] = "@(#)SYNTAX - $Id: sxparser.c 1032 2008-02-14 13:20:22Z rlacroix $" WHAT_DEBUG;
#else
char WHAT_SXPARSER[] = "@(#)SYNTAX - $Id: sxparser.c 1032 2008-02-14 13:20:22Z rlacroix $ SXPARSER_32" WHAT_DEBUG;
#endif
...

static BOOLEAN	sxparse_it (void)
{
    SXP_SHORT			ref;
    SXINT                       lahead;
    SXP_SHORT                   state;
...
/* Swap de s1 et s2 : premiere etiquette apres scan
   Evaluation de s1 : premier test explicite */
	...
	if ((state = ref - sxplocals.SXP_tables.Mprdct) > 0 /* Shift */ ) {
		...
		sxplocals.state = (sxplocals.ptok_no == sxplocals.atok_no) ? labs (s2->state) : s1->state;
...

BOOLEAN sxparser (SXINT	what_to_do, struct sxtables *arg)
{
    switch (what_to_do) {
    case BEGIN:
...
BOOLEAN sxparse_in_la (SXINT ep_la, SXINT Ttok_no, SXINT *Htok_no, struct sxparse_mode *mode_ptr)
{
