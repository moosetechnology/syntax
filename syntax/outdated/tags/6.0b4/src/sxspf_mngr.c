...
char WHAT_SXSPF_MNGR[] = "@(#)SYNTAX - $Id: sxspf_mngr.c 1226 2008-03-20 11:02:05Z sagot $" WHAT_DEBUG;
...
duplicate_subDAG (/* source */ SXINT source_init_state, SXINT source_final_state,
		  /* cible de la copie, on récupère les valeurs */ SXINT *init_state, SXINT *final_state)
{
...
spf_second_yield_bu_walk (SXINT Pij)
{
  ...
  while ((cur_Xpq = spf.outputG.rhs [item++].lispro) != 0
	 && (cur_Xpq > 0 && !SXBA_bit_is_set (special_Aij_set, cur_Xpq) && Aij2subDAG_init_state [cur_Xpq] == 0 /* déjà traité mais associé à aucun sous-DAG: yield vide */));
  ...
  if (cur_Xpq != 0) {
    ...
    while ((next_Xpq = spf.outputG.rhs [item++].lispro) != 0) {
      ...
      if (next_Xpq < 0 || SXBA_bit_is_set (special_Aij_set, next_Xpq) || Aij2subDAG_init_state [next_Xpq] != 0 /* == 0 veut dire déjà traité mais associé à aucun sous-DAG: yield vide */) {
	...
	if (cur_Xpq > 0 && !SXBA_bit_is_set (special_Aij_set, cur_Xpq)) {
	  ...
	  if (++Aij2visited_rhs_occ_nb [cur_Xpq] < Aij2rhs_occ_nb [cur_Xpq]) {
	    ...
	    duplicate_subDAG (/* source */ Aij2subDAG_init_state [cur_Xpq], Aij2subDAG_final_state [cur_Xpq],
			      /* cible de la copie, on récupère les valeurs */ &cur_Xpq_subDAG_init_state, &cur_Xpq_subDAG_final_state);
	  }
	}
	else {
	  /* T ou NT spécial */
    ...
    if (cur_Xpq > 0 && !SXBA_bit_is_set (special_Aij_set, cur_Xpq)) {
      ...
      if (Aij2subDAG_init_state [cur_Xpq] != 0 /* == 0 veut dire déjà traité mais associé à aucun sous-DAG: yield vide */ &&
    ...
    if ((global_first_state = Aij2subDAG_init_state [Aij]) == 0) { // première visite d'un Pij ayant ce Aij en lhs
