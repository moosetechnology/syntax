...
#ifndef VARIANT_32
char WHAT_SXNDPARSER[] = "@(#)SYNTAX - $Id: sxndparser.c 1214 2008-03-13 12:49:51Z rlacroix $" WHAT_DEBUG;
#else
char WHAT_SXNDPARSER[] = "@(#)SYNTAX - $Id: sxndparser.c 1214 2008-03-13 12:49:51Z rlacroix $ SXNDPARSER_32" WHAT_DEBUG;
#endif
#if EBUG
...
extern SXVOID	output_repair_list (void);
extern SXVOID	output_repair_string (SXINT output_repair_string_index);
...
static BOOLEAN	set_next_item (struct SXP_item *vector, SXINT *check, SXINT *action, SXINT Max)
{
...
BOOLEAN	set_next_trans (struct SXP_bases *abase, SXINT *check, SXINT *action, SXINT Max, SXINT *next_action_kind)
{
...
static SXINT build_a_rule (SXINT parser, SXINT build_a_rule_index, SXINT rhs)
{
    SXINT symb;
    SXBOOLEAN bool;
    bool = (new_symbol ((SXINT)parse_stack.ared->reduce,
			build_a_rule_index,
...
SXINT mreds_push (SXINT parser, SXINT ref, SXINT tok_no, BOOLEAN is_new_triple, BOOLEAN is_shift_reduce, SXINT symbol)
...
static BOOLEAN call_predicate (SXINT current_token, SXINT prdct)
{
    BOOLEAN 	ret_value;
...
static BOOLEAN do_reductions (SXINT son, BOOLEAN is_in_set)
{
...
static BOOLEAN do_pops (SXINT p, SXINT l)
{
...
static SXVOID seek_paths (SXINT p, SXINT p_trans, BOOLEAN is_in_set, SXINT l);
...
/* Amelioration eventuelle :
   ...
   - Ne lancer un do_limited_pops que si un parser actif existe ds la descendance
     de l'origine.
*/
static BOOLEAN do_limited_pops (SXINT p, SXINT l)
{
...
static SXVOID parse_forest (void)
{
	...
	else
	{
		...
		if (parser > 0)
		    seek_paths (parser, main_rhs, parser_set != NULL, (SXINT)parse_stack.ared->lgth);
	    }
	}
    }
...
static void
single_reduce (SXINT xtriple, BOOLEAN is_limited)
{
	...
	do {
	    ...
	    *parse_stack.for_scanner.next_hd = 0;
		...
		if (scan_ref < 0) {
			...
			sxndsubparse_a_token (parser, XxY_Y (parse_stack.mscans, xmscan), tok_no, (SXINT(*)())NULL);
		    }

		    scan_ref = -scan_ref;
		}

		sxndsubparse_a_token (parser, scan_ref, tok_no, (SXINT(*)())NULL);
	    }
		...
		*parse_stack.for_scanner.next_hd = 0;
		...
		XxYxZ_Xforeach (sxndtkn.dag, cur_node, fxyz) {
			...
			for (parser = parse_stack.rcvr.milstn_elems [prev_node].next_for_scanner;
				...
				sxndsubparse_a_token (parser, scan_ref, -prev_token /* symbol */, (SXINT(*)())NULL);
			    }
			}
		    }
		}
...
static BOOLEAN dag_set (SXINT dag_set_index, SXINT succ_kind, SXINT new_kind)
{
...
static BOOLEAN sxnd2_error_detected (void)
{
...
static BOOLEAN sxnddesambig_it (void)
{
    extern SXINT sxvoid (void);
	...
	if (sxplocals.mode.global_errors_nb == 0) {
		...
		if (parse_stack.start_symbol != 0
		    ...
		    && (void(*)(void))sxplocals.SXP_tables.desambig != (void(*)(void))sxvoid) {
	...
	if (parse_stack.start_symbol > 0
	    && (void*)sxplocals.SXP_tables.semact != (void*)sxvoid)
...
BOOLEAN sxndparser (SXINT what_to_do, struct sxtables *arg)
{
    SXINT i;
...
/* prepare new local variables */
	...
	create_ref (new_parser ((SXINT)0 /* son */,
				(SXINT)sxplocals.SXP_tables.init_state,
				(SXINT)0 /* symb */ ),
...
BOOLEAN sxnd2parser (SXINT what_to_do, struct sxtables *arg)
{
    switch (what_to_do) {
    case BEGIN:
	sxndparser ((SXINT)BEGIN, arg);
	break;

    case OPEN:
	sxndparser ((SXINT)OPEN, arg);
	sxndtkn_mngr (OPEN, 2);
	break;

    case INIT:
	sxndparser ((SXINT)INIT, arg);
	...
	*parse_stack.for_scanner.next_hd = 0;
    ...
    case FINAL:
	sxndparser ((SXINT)FINAL, arg);
	sxndtkn_mngr (FINAL, 0);
	break;

    case CLOSE:
	sxndparser ((SXINT)CLOSE, arg);
	sxndtkn_mngr (CLOSE, 0);
	break;

    case END:
	sxndparser ((SXINT)END, arg);
	break;
...
BOOLEAN sxndparse_in_la (SXINT ep_la, SXINT Ttok_no, SXINT *Htok_no, struct sxparse_mode *mode_ptr)
{
	...
	*parse_stack.for_scanner.next_hd = 0;
	...
	for (parser = *parse_stack.for_scanner.current_hd;
	    ...
	    sxndsubparse_a_token (parser, scan_ref, (SXINT)0 /* symbol */, (SXINT(*)())NULL);
	}
