...
/************************************************************************/
/* 23-01-92 08:50 (pb):		"SCRMBL" devient un "SXUINT"	*/
/************************************************************************/
...
char WHAT_XH_MNGR[] = "@(#)SYNTAX - $Id: XH_mngr.c 935 2008-01-17 10:20:42Z rlacroix $" WHAT_DEBUG;
...
BOOLEAN	XH_oflw (XH_header *header)
{
    SXINT old_size;
...
#define max(x,y)	((x)>=(y) ? (x) : (y))
	...
	fprintf (stat_file, "name = %s, hash_size = %ld (%ld%%), size = %ld (%ld%%), minl = %ld, maxl = %ld, meanl = %ld, list_size = %ld (%ld%%), minll = %ld, maxll = %ld, meanll = %ld, Total_size (byte) = %ld (%ld%%)\n",
		 header->name, (long)hash_size, (long)(100*n)/hash_size, (long)size, (long)(100*top)/size, (long)minl, (long)maxl, (long)meanl, (long)list_size, (long)(100*list_top)/list_size, (long)minll, (long)maxll, (long)list_top/top, (long)total_size, (long)(100*total_top)/total_size);
    }
    else
	fprintf (stat_file, "name = %s, hash_size = %ld, size = %ld, list_size = %ld is empty.\n", header->name, (long)hash_size, (long)size, (long)list_size);
}
...
BOOLEAN	XH_set (XH_header *header, SXINT *result)
{
    /* Recherche si la liste
           header->list[header->display[header->display->lnk].X .. *(header->list)]
       existe par ailleurs */
    /* Retourne l'indice dans display de la [nouvelle] liste */
    SXINT		x, z, hash_value = header->scrmbl % header->hash_size;
...
/* Nouvelle liste */
	...
	for (hash_value = z - 1; hash_value >= 0; hash_value--) {
	    *(high = (low = hash_display + hash_value) + z) = 0;
		...
		do {
		    if (((aelem = display + x)->scrmbl) % header->hash_size == (SXUINT)hash_value) {
			if (!is_low) {
			    *low &= XH_80;
			    *low += x;
			    is_low = TRUE;
...
void XH2c (XH_header *header, FILE *F_XH /* named output stream */, char *name, BOOLEAN is_static)
{
  ...
  fprintf (F_XH, "static SXINT %s_hash_display [%ld] = {", name, (long)hash_size);
  for (x = 0; x < hash_size; x++) {
    if (x%10 == 0) fprintf (F_XH, "\n/* %ld */ ", (long)x);
    fprintf (F_XH, "%ld, ", (long)header->hash_display [x]);
  }
  fprintf (F_XH, "\n} /* End %s_hash_display */;\n", name);
  ...
  for (x = 1; x < top; x++) {
    fprintf (F_XH, "/* %ld */ {%ld, %ld, %ld},\n", (long)x, (long)header->display [x].lnk, header->display [x].scrmbl, header->display [x].X); 
  }
  fprintf (F_XH, "/* top==%ld */ {0, 0, %ld},\n", (long)top, (long)header->display [top].X); 
  ...
  fprintf (F_XH, "static SXINT %s_list [%ld] = {\n", name, (long)list_size+1);
  fprintf (F_XH, "/* list_top */ %ld,", (long)list_top);
  for (x = 1; x < top; x++) {
    y = header->display [x].X;
    z = header->display [x+1].X;
    fprintf (F_XH, "\n/* %ld */ ", (long)x);
    while (y < z) {
      fprintf (F_XH, "%ld, ", (long)header->list [y]);
      y++;
    } 
  }
  fprintf (F_XH, "\n/* list_top==%ld */ 0,\n", (long)list_top);
