...
char WHAT_OPTIM[] = "@(#)SYNTAX - $Id: OPTIM.c 3179 2023-05-06 08:32:16Z garavel $" WHAT_DEBUG;
...
extern SXVOID	parser_write (struct parser_ag_item *parser_ag_ptr, char *langname), 
...
static SXBOOLEAN	row_le (SXINT i, SXINT j)
{
...
static SXBOOLEAN	row_eq (SXINT i, SXINT j)
{
    SXINT	li, vali, valj;
    struct row		*arowi = rows + i, *arowj = rows + j;
    struct P_item	*ai, *aj;
...
static SXBOOLEAN	is_0_indistinguable (struct floyd_evans *afei, struct floyd_evans *afej)
{
...
static SXBOOLEAN	is_k_indistinguable (struct floyd_evans *afei, struct floyd_evans *afej)
{
...
static SXVOID	zero_indistinguable (SXINT state_no,
				     SXINT *equiv_class,
				     struct state *states,
				     struct P_item *items,
...
static SXBOOLEAN	is_stable (SXINT *equiv_class,
			   struct state	*states,
			   struct P_item *items,
			   struct floyd_evans *fe,
			   SXINT *partitions,
...
/*   L I N E A R I S A T I O N   D E S   M A T R I C E S   */
...

	{
	    ...
	    delta = 0;
	    /* Le 25/02/2003 
	    algoV (SXFALSE, xe1, -bnf_ag.WS_TBL_SIZE.tmax, xt_items, t_states, t_items,
	    t_bases, &vector, &bases_set, &MIN, &MAX, &delta); 
	    pour le cas ou on manipule des XT (prudence), est change par : */
	    algoV (SXFALSE, xe1, bnf_ag.WS_TBL_SIZE.xtmax, xt_items, t_states, (struct item*) t_items,
...
/* On prend comme defaut l'adresse la plus populaire et en cas d'egalite la plus
   elevee. */ 

	{
	    ...
	    algoV (SXTRUE, xac2, bnf_ag.WS_TBL_SIZE.xntmax, xnt_items, nt_states, (struct item*) nt_items,
...
*/
	...
	XxY_alloc (&ref_to_ee, "ref_to_ee", 2 * xac2, 4, 0, 0, (sxoflw0_t) NULL, NULL);
