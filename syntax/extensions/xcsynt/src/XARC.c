...
char WHAT_XCSYNTXARC[] = "@(#)SYNTAX - $Id: XARC.c 3179 2023-05-06 08:32:16Z garavel $" WHAT_DEBUG;
...
static SXINT make_ubfsa_to_ate (void)
{
	...
	sprintf (s, "<%ld>", (SXINT) ipss_to_attr [1].local.bfsa);
	varstr_catenate (vstr, s);
    }
			       ...
			       SXINT *ret)
{
    ...
    while (--top >= bot) {
	couple = *top;
	sprintf (s, "%ld-%ld", (SXINT) XxY_Y (Q0xQ0_hd, couple), (SXINT) XxY_X (Q0xQ0_hd, couple));
	varstr_catenate (vstr, s);
    ...
    *ret = sxstrsave (varstr_tostr (vstr));
    ...
    sprintf (s, "%ld <%ld> %ld", (SXINT) *bot, (SXINT) xtnd_local_bfsa, (SXINT) *(bot + 2)); /* e <i> s */
    return sxstrsave (s);
}
...
SXVOID	XARC_print (struct ARC_struct *XARC)
{
	...
	for (lim1 = XH_X (XARC->Q_hd, XARC_state + 1), z = XH_X (XARC->Q_hd, XARC_state); z < lim1
		...
		put_edit_apnnl ("=(");
		sprintf (string, "<%ld>", (SXINT) XxY_X (forks, XARC_print_fork));
		put_edit_apnnl (string);
		put_edit_apnnl (", ");
		sprintf (string, "%ld", (SXINT) XxY_Y (forks, XARC_print_fork));
		put_edit_apnnl (string);
		put_edit_apnnl (")");
