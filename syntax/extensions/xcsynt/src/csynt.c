...
char WHAT_XCSYNTCSYNT[] = "@(#)SYNTAX - $Id: csynt.c 3234 2023-05-15 16:52:27Z garavel $" WHAT_DEBUG;
...
int main (int argc, char *argv[])
{
  ...
  for (argnum = 1; argnum < argc; argnum++) {
    ...
    case UC:
      is_unique_conflict_per_state = SXTRUE, csynt_options_set |= OPTION (UC);
      break;

    case -UC:
      is_unique_conflict_per_state = SXFALSE, csynt_options_set &= noOPTION (UC);
      break;

    case UD:
      is_unique_derivation_per_conflict = SXTRUE, csynt_options_set |= OPTION (UD);
      break;

    case -UD:
      is_unique_derivation_per_conflict = SXFALSE, csynt_options_set &= noOPTION (UD);
      break;
    ...
    case SA:
      is_super_arc = SXTRUE, csynt_options_set |= OPTION (SA);
      break;

    case -SA:
      is_super_arc = SXFALSE, csynt_options_set &= noOPTION (SA);
      break;
    ...
    case DETERMINISTIC:
      is_non_deterministic_automaton = SXFALSE, csynt_options_set |= OPTION (DETERMINISTIC);
      break;

    case -DETERMINISTIC:
      is_non_deterministic_automaton = SXTRUE, csynt_options_set &= noOPTION (DETERMINISTIC);
      break;

    case AMBIGUITY_CHECK:
      is_ambiguity_check = SXTRUE, csynt_options_set |= OPTION (AMBIGUITY_CHECK);
      break;

    case -AMBIGUITY_CHECK:
      is_ambiguity_check = SXFALSE, csynt_options_set &= noOPTION (AMBIGUITY_CHECK);
      break;
