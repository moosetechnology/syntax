    ...
    SXBA	lhs_bits_array, op1_bits_array, op2_bits_array;
{
    ...
    SXINT	slices_number = NBLONGS (BASIZE (op2_bits_array));
    BOOLEAN		ret_val = FALSE;
    ...
    SXBA	lhs_bits_array, op1_bits_array, op2_bits_array;
{
    ...
    SXBA	lhs_bits_ptr, op1_bits_ptr, op2_bits_ptr;
    SXINT			delta = DIV (shift);
    SXINT	slices_number = NBLONGS (BASIZE (op2_bits_array)) - delta;
    SXINT			right_shift = MOD (shift);
    ...
    SXINT p;
{
    ...
    if (is_FSA_loop) {
	if ((pXq = XxY_is_set (&FSA_pXq_hd, p, p)) > 0) {
	    /* boucle sur p */
	    q = p;
...
#if is_rcvr
		sxba_empty (loop_non_kernel_item_sets);
		T2_non_kernel_item_set = loop_non_kernel_item_sets;
#endif
	    ...
	    } while (complete (p, q) /* T2 est non vide */ && T2_has_changed); 
	}
    }
	...
	if (is_FSA_loop && p == q || q > FSA_final_state)
	    continue;
     ...
     SXINT j, init_prod_order;
{
    /* un ensemble de productions mutuellement recursives droites vient d'etre detecte'
       ...
       connue. */
    SXINT 		prod_order, min_prod_order, prod, A, right_item, i, item, B, k, nbt;
    SXINT			*i_prod_stack;
    struct PT2_item	*PT2j;
