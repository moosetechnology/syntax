...
SXINT			rcvr_n; /* repere l'index ds global_source du eof du (vrai) source */

SXUINT                  nt_calls, clause_calls, memo_prdct_hit, memo_clause_hit;
SXUINT		        iarg_searched_in_guide, iarg_found_in_guide, iprdct_searched_in_guide, iprdct_found_in_guide,
                        iclause_searched_in_guide, iclause_found_in_guide;

SXINT			global_pid;
SXINT			n, logn, ANDj;
SXINT			min_max, local_val;
SXINT			call_level; /* #if EBUG niveau d'appel des predicats */
...
SXUINT           instantiated_clause_nb, dynam_lex_call_nb, dynam_lex_false_call_nb;
SXUINT           total_clause_nb, total_clause1_nb, total_clause2_nb;
...
extern SXINT		sxivoid (), *StrConcat (BOOLEAN is_first_arg, SXINT *arg, SXINT *Xlb, SXINT *Xub);
extern void		sxtime (), sxvoid ();
extern void		tree_nb (struct G *G), store_elem_tree (struct G *G), parse_forest (struct G *G), build_coupled_guide (struct G *G);
extern void		fill_buf (), arg_nb_put_error (char *prdct_name, SXINT def_nb, SXINT decl_nb);
