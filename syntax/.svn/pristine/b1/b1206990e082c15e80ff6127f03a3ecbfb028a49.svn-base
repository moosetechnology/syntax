...
#ifndef lint
#define WHAT	"@(#)sxindparser.c\t- SYNTAX [unix] - 1 Septembre 1992"
static struct what {
  struct what	*whatp;
    ...
    BOOLEAN     parse_time_building;
{
  ...
  if (ltd_parse_stack.current_token == sxplocals.SXP_tables.P_tmax && !parse_time_building) return;
    ...
    int			son, state, symbol;
{
  ...
  if (state > lr0_automaton_state_nb) 
  ...
  else 
    {
      ...
      if (ref > 0 && (red_nb = lim = aQ0->red_trans_nb) > 0) 
	{
	  /* conflit shit-reduce */
