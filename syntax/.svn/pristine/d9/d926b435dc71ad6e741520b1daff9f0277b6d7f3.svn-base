...
/*   I N C L U D E S   */
#define NODE struct amlgm_cmpd_node
#include "varstr.h"
#include "sxunix.h"
...
static NODE                    **lhs_id2first_visited;
...
amlgm_cmpd_pi (void)
{
  ...
  */
  ...
  case AMLGM_CMPD_ROOT_n :/* VISITED->name = RULE_S_n */
    break;

  case LHS_S_n :/* VISITED->name = {CHAINE_n, IDENT_n} */
    break;

  case RHS_S_n :/* VISITED->name = {CHAINE_n, IDENT_n} */
    break;

  case RULE_n :
    switch (VISITED->position) {
    case 1 :/* VISITED->name = LHS_S_n */
      break;

    case 2 :/* VISITED->name = RHS_S_n */
      break;
...
amlgm_cmpd_pd (void)
{
  ...
  */
  ...
  case LHS_S_n :
    for (son = VISITED->son; son != 0; son = son->brother) {
    ...
    if (!XH_set (&lhs_hd, &VISITED->id)) {
      /* On retarde l'appel car on va mettre comme id un index qui repere une pile des rhs */
      VISITED->next_visited = NULL;
    }
    else {
      VISITED->next_visited = lhs_id2first_visited [VISITED->id];
    }
      
    /* liste des LHS_S_n qui ont meme partie gauche */
    lhs_id2first_visited [VISITED->id] = VISITED;
  ...
  case RULE_n :
    if (XxY_set (&rule_hd, VISITED->son->id, VISITED->son->brother->id, &VISITED->id)) {
      sxput_error (VISITED->token.source_index,
		   "%sMultiple rules are ignored.",
		   sxtab_ptr->err_titles [2]);
      VISITED->id = 0; /* Multiple */
    }
...
smppass (void)
{
  ...
  lhs_id2first_visited = (NODE **) sxalloc (rule_nb+1, sizeof (NODE *));
  ...
  sxdfa2comb_vector (&lhs_acc_name_sxdfa, 0 /* comb_kind */, 10000 /* comb_vector_threshold */, &lhs_acc_name_comb);
...
%sSXINT rhs_list [%ld] = {\n", is_static ? "static " : "", (long)(top+list_top+1));
  ...
  for (x = 1; x < top; x++) {
    ...
    while (y < z) {
      printf ("%ld, ", (long)rhs_list_hd.list [y]);
      y++;
    } 
  }
...
%sSXINT rhs_str_list_display [%ld] = {\n", is_static ? "static " : "", (long)top+1);
  printf ("/* top */ %ld,\n", (long)top);

  for (x = 1; x <= top; x++) {
    printf ("/* %ld */ %ld,\n", (long)x, (long) rhs_hd.display [x].X); 
  } 

  fputs ("} /* End rhs_str_list_display */;\n", stdout);


  printf ("\n\n%schar *rhs_str_list [%ld] = {NULL,", is_static ? "static " : "", (long)list_top+1);

  for (x = 1; x < top; x++) {
    y = rhs_hd.display [x].X;
    z = rhs_hd.display [x+1].X;
    printf ("\n/* [%ld..%ld] */ ", (long)x, (long)(z-1));
    while (y < z) {
      id = rhs_hd.list [y];
      ste = rhs_id2ste [id];
...
%sSXINT rhs_id_list [%ld] = {0,", is_static ? "static " : "", (long)list_top+1);
