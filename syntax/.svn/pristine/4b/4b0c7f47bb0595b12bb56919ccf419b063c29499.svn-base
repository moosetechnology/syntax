...
extern SXBOOLEAN input_dag_transition_names_are_ffs (void); /* rend SXTRUE si on passe par un dico de ff, SXFALSE si on parse directement des terminaux */
...
extern void   fill_idag_pqt2logprob     (void);
extern void   fill_states_in_all_paths_set     (void);
...
#endif /* 0 */
extern void   fill_more_idag_infos    (SXBOOLEAN is_qstack_needed, SXBOOLEAN is_pstack_needed, SXBOOLEAN is_inside_t_needed, SXBOOLEAN is_range2dag_kind_needed);
...
#define p_q2rho(p,q)                  (idag.p_q2range [p][q])
#define range2p(r)                    (idag.range2p [r])
#define rho2q(r)                      (idag.range2q [r])
#define rho2p(r)                      (idag.range2p [r])
#define range2q(r)                    (idag.range2q [r])
#define range2dag_kind(r)             (idag.range2dag_kind [r])
#define p_q2dag_kind(p,q)             (idag.range2dag_kind [p_q2range(p,q)])
...
#define pqt2logprob(pqt)              (idag.pqt2logprob [pqt])
#define pq_t2logprob(pq,t)              (idag.pqt2logprob [pq_t2pqt (pq, t)])
#define p_q_t2logprob(p,q,t)            (idag.pqt2logprob [p_q_t2pqt (p, q, t)])
...
struct idag {
  ...
  /* Non renseigne' en standard, se construit en appelant fill_more_idag_infos () avec SXTRUE en quatrième argument*/
    char *range2dag_kind /* permet connaissant un range [p..q] de retrouver le type du sous-DAG contenant toutes les transitions appartenant à au moins un chemin allant de p à q (STRING_KIND, SDAG_KIND ou DAG_KIND) */
    ;
  ...
  ***pt2q_disp /* *(pt2q_disp [p]+t) est une stack de q */
  ...
  source_set
    ...
    , *pq2t_set /* chaque ligne du SXBM est un bitarray qui donne les terminaux (convertis en inside-terminaux si pertinent) sur lesquels on peut faire une transition (directe) de p à q à partir du pq correspondant ; les transitions 0->1 et final_state->final_state+1 sur EOF sont incluses */
    , *p2t_set /* p2t_set [p] est un SXBA des t tels qu'il existe au moins un q où on peut aller par une transition de la forme p-t->q */
    , *path /* path [i] = {j | i ->x j, x \in T^+} */
    , *range2inside_t_set /* ensemble des inside_t qui se trouvent entre p et q */
    , states_in_all_paths_set /* ensemble des états par lesquels tous les chemins passent */
    ;
  ...
  SXLOGPROB
  *pqt2logprob /* proba max de l'ensemble des transitions p --t--> q */
  ;
};
...
/* peut s'additionner à l'un des précédents */
#define IS_CONSECUTIVE_SEQUENCE 16
