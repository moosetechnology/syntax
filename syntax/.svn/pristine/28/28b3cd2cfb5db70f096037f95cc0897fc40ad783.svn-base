...
static int	read_a_line (source, line, line_size, line_length)
    register FILE	*source;
    register char	*line;
    int			line_size, *line_length;
{
    register int	c, ll = 0;
...
static int	is_header (line, node_name)
    register char	*line, *node_name;
{
    /* recherche de :
       si is_c
...
static int	is_define (line, node_name)
    register char	*line, *node_name;
{
    /* recherche de :
       si is_c
...
static char	*next_define (t_lines_ptr, xline, node)
    register struct t_lines	*t_lines_ptr;
    register int	*xline;
    register char	*node;
{
...
static char	*skip (t_lines_ptr, xline, node)
    register struct t_lines	*t_lines_ptr;
    register int	*xline;
    register char	*node;
{
...
static char	*read_write (t_lines_ptr, xline, objet, node)
    register struct t_lines	*t_lines_ptr;
    register int	*xline;
    register FILE	*objet;
    register char	*node;
...
static VOID	read_until (source, source_name, line, line_size, string)
    register FILE	*source;
    register char	*source_name, *line, *string;
...
static VOID	copy_until (source, source_name, line, line_size, string)
    register FILE	*source;
    register char	*source_name, *line, *string;
    register int	line_size;
{
...
static int	copy_until2 (source, source_name, line, line_size, string1, string2)
    register FILE	*source;
    char	*source_name;
    register char	*line, *string1, *string2;
    int		line_size;
{
    register int	c;
    int			line_lgth;
...
static SXVOID	read_lines_until (source, source_name, string, t_lines_ptr)
    register FILE	*source;
    char	*source_name;
...
static SXVOID	free_lines (t_lines_ptr)
    register struct t_lines	*t_lines_ptr;
{
    register int	xl;
...
static VOID	process (kind, frame_ptr, cur_old_pass_ptr, is_kept)
    register int	kind;
    register struct t_lines	*frame_ptr, *cur_old_pass_ptr;
    register SXBA	is_kept;
{
    int		xlcop, xlf;
...
/* On sort en premier les nouveaux noeuds */
	...
	if (!sxba_bit_is_set (is_kept, sxstrretrieve (fn))) {
