...
char WHAT_STRUCTURE_SEMACT[] = "@(#)SYNTAX - $Id$" WHAT_DEBUG;
  ...
  */
  ...
  while ((cur_Pij = spf.outputG.rhs [hook].lispro) != 0) {
    if (cur_Pij > 0) { /* il ne s'agit pas d'une branche elaguee */
      ...
      fprintf(stdout," %e\n",Pij2weight[cur_Pij]);
#endif
      if (Pij2weight[cur_Pij] < localmax-0.00001 /* si cette production n'est pas optimale... */
...
structure_sem_pass ()
{
  ...
  specific_contribution = prefer_local_COO;
  spf_topological_walk (spf.outputG.start_symbol, NULL, select_optimal_trees);
  
  specific_contribution = prefer_ADV_MOD;
  spf_topological_walk (spf.outputG.start_symbol, NULL, select_optimal_trees);
  
  specific_contribution = prefer_Easy_NVA;
  spf_topological_walk (spf.outputG.start_symbol, NULL, select_optimal_trees);
