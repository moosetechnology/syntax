...
/************************************************************************/
/* Mer 22 Jan 1997 10:06(PB)	On "type" les references		*/
/************************************************************************/
/* Lun 30 Dec 1996 14:13(PB)	Ajout de cette rubrique "modifications"	*/
/************************************************************************/
...
typedef struct {
    ...
    char	*name;
    int		used_size, prev_used_size, total_size; /* #if statistics */
} bag_header;
...
static struct tree2attr {
    int		prod, Aij, next, node_list, var_hd;
    SXBA	tree_set;
#if DCG_is_cyclic==1
    int		cyclic_tree_id;
#endif
} *tree2attr;
static int		glob_tree_top, tree_size;
...
static struct var_list {
    int		next, sub_tree, val;
} *var_list;
static int		var_list_top, var_list_size;
static int		*var_hd, var_hd_top, var_hd_size;

static int		*Aij2tree;
static int		sons_stack [SXDCGrhs_maxnt+1];
static int		*Frhs_stack;
...
#if 0
static int		to_be_cloned [SXDCGrhs_maxnt+1];
#endif
...
static int		*clone_stack, *clone2node;
static int		clone_stack_top, clone_stack_size, clone_root;
static int		com_sub_tree, com_tree_set_nb, tree_clone_base;
static int		clone2node_top, clone2node_size;
static SXBA		com_tree_set, sub_tree1_set, sub_tree2_set;

#if SXDCGis_dynam_constant==1
#if SXDCGmax_atom!=0
static int atom2dynam_atom [SXDCGmax_atom+1];
#endif
#endif
...
#if SXDCGhas_prolog==1
{
    ...
    int			prod, gvar_val, ppp_item, init_ppp_item, next_ppp_item;
    SXBA		ppp_item_set;
    int			*ppp_prolon;
    ...
    int sub_tree_var, x, sub_tree_term, t;
{
    ...
    struct tree2attr	*sub_tree_ptr;
#if SXDCGhas_prolog==1
    int			prod, gvar_val, item, next_item, pos;
    SXBA		item_set;
#endif
    ...
    int sub_tree1, t1, sub_tree2, t2;
{
    ...
    int 	t1_kind, t2_kind, t1_val, t2_val, t1_ref, t2_ref;
    ...
    int size;
{
    ...
    Aij2tree = (int*) sxcalloc (size+1, sizeof (int));
    ...
    clone_stack = (int*) sxalloc ((clone_stack_size = SXDCGrhs_maxnt+10) + 1, sizeof (int));
    clone2node = (int*) sxalloc ((clone2node_size = SXDCGrhs_maxnt+10) + 1, sizeof (int));
...
clone_node (node)
    int 	node;
{
    int 		clone_node, son_nb, nb, son_node, var_nb, var, next, pos_node;
    int			*son_ptr, *var_hd_ptr, *clone_var_hd_ptr;
    ...
    int node;
{
	...
	do {
		...
		clone_stack = (int*) sxrealloc (clone_stack, (clone_stack_size *= 2) + 1, sizeof (int));
...
common_sub_tree (node)
    int node;
{
    int			son_nb, son, common_node, com_node;
    int			*son_ptr;
    struct tree2attr	*node_ptr, *next_node_ptr;
    ...
    int node;
{
    ...
    int			son_nb, son, n, marked_son, clone_son;
    int			*son_ptr;
    struct tree2attr	*node_ptr, *next_node_ptr;
    ...
    int prod, nbnt;
{
    ...
    int 		sub_tree, sub_prod, param_nb, j, t1, t2, Aij, var_nb, failed_pos, indx, nb;
	...
	for (failed_pos = 1; failed_pos <= nbnt; failed_pos++) {
		...
		for (pos = 1; pos < failed_pos;pos++) {
			...
			if ((tree2attr [com_sub_tree+1].var_hd-tree2attr [com_sub_tree].var_hd) > 0) {
				...
				clone2node = (int*) sxrealloc (clone2node, (clone2node_size *= 2) + 1, sizeof (int));
...
#if SXDCGhas_prolog==1
{
	...
	if ((dc = SXDCGprod2dc [prod]) > 0) {
	    ...
	    switch (dc) {
	    case INTEGER_CONSTANT:
		str = ste == EMPTY_STE ? inputG.tstring [token->lahead] : sxstrget (ste);
		val = atoi (str);
    ...
    int prod, nbnt;
{
	...
	if (failed_pos > nbnt) {
	    /* l'unification du reste a marche et il y a des contraintes a verifier */
	    BOOLEAN 	done;
	    int		x, /* sub_tree_id, */ sub_prod, cur_root, cur_pos, cur_item;
	    int		*param_ptr;
...
#if SXDCGhas_prolog==1
    {
	int 			la3, var_nb, old_glob_tree_top, old_var_list_top;
	int			*var_ptr;
...
#if DCG_is_cyclic==1
    return is_cyclic_prod ? 0 : (int)is_unified;
#else
    return (int)is_unified;
#endif
}
...
print_rule (tree)
    int tree;
{
    int		prod, Aij, A, param_nb, pos, j, k, item, node, X, Blk, ste, ppp;
    int		*param_ptr, *node_ptr;
    BOOLEAN	is_first;
	...
	if (X > 0) {
	    ...
	    printf ("[%s]", ste == EMPTY_STE ? inputG.tstring [-X] : sxstrget (ste));
	    k++;
	}
...
static struct {
    int		rule_top, rule_size, item_top, item_size;
...
pf_rule_oflw ()
{
    pf.prolon = (int*) sxrealloc (pf.prolon, (pf.rule_size *=2)+1, sizeof (int));
}
...
{
    pf.item_size *=2;
    pf.lispro = (int*) sxrealloc (pf.lispro, pf.item_size+1, sizeof (int));
    pf.next_item = (int*) sxrealloc (pf.next_item, pf.item_size+1, sizeof (int));
}
    ...
    int size;
{
    /* size == pAij_top */
    pf.rule_size = size;
    pf.prolon = (int*) sxalloc (pf.rule_size+1, sizeof (int));

    pf.item_size = ((inputG.maxitem/inputG.maxprod)+2)*pf.rule_size;
    pf.lispro = (int*) sxalloc (pf.item_size+1, sizeof (int));
    pf.next_item = (int*) sxalloc (pf.item_size+1, sizeof (int));
...
erase_prod (Bkl)
    int Bkl;
{
    int it, pf_prod, it_top, it_cur, Cmn, new_it, prod;
...
herbrand_post_semantics (S0n)
    int S0n;
{
    int pf_prod, item_top, prod, item, Aij, next_item;
