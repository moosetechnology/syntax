...
static SXVOID ltd_do_reductions (parse_time_building, complete_item_father, son, is_in_set)
    ...
    BOOLEAN	parse_time_building, is_in_set;
{
  ...
  do 
	    ...
	    ) < 0) 
	{
	 ...
	 If exists q in active_parsers with state(q)==state then */
      ...
      if ((link = ltd_add_active_parser (parse_time_building, family, son, next, symb)) > 0) 
	{
...
static BOOLEAN ltd_shifter (parse_time_building)
    BOOLEAN     parse_time_building;
{
  int		p, ref, n;
    ...
    int	shift_ref, parser;
{
  ...
  while ((x = ltd_parse_stack.active_states [x]) > 0)
    {
      ...
      PUSH (ltd_parse_stack.tmp_active_states, x);
      ltd_parse_stack.tmp_active_parsers [x] = ltd_parse_stack.active_parsers [x];
    }
  ...
  while ((x = ltd_parse_stack.for_shifter [x]) > 0)
    {
      POP  (ltd_parse_stack.for_shifter, x);
      PUSH (ltd_parse_stack.tmp_for_shifter, x);
    }
  ...
  if (shift_ref > 0)
    {
      ...
      sxba_1_bit (ltd_parse_stack.families [parser], 0);
    }
  ...
  x = 0;
  while ((x = ltd_parse_stack.tmp_active_states [x]) > 0)
    {
      POP  (ltd_parse_stack.tmp_active_states, x);
      PUSH (ltd_parse_stack.active_states, x);
      ltd_parse_stack.active_parsers [x] = ltd_parse_stack.tmp_active_parsers [x];
    }
  ...
  x = 0;
  while ((x = ltd_parse_stack.tmp_for_shifter [x]) > 0)
    {
      POP  (ltd_parse_stack.tmp_for_shifter, x);
      PUSH (ltd_parse_stack.for_shifter, x);
    }
