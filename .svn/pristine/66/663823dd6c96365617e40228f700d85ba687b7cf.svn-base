...
static char	what [] = "@(#)fsa_to_re.c\t- SYNTAX [unix] - Ven 22 Nov 1996 17:25:41";
...
static struct node	*create_list (p_s, m, M, op)
...
static	gather_op_sons (p_s, xp, p, op)
    struct node		**p_s;
    int		*xp, op;
    struct node		*p;
{
...
static SXVOID process_substr (p_s, xstar, xconc, M)
    register struct node	**p_s;
    int		xstar, xconc, M;
{
	...
	if (xconc < xstar || d + y <= M) {
		...
		for (i = y; i >= 1; i--, xconc--) {
			...
			tete_pop (2);
			process_substr (p_s, xconc, xconc + 1, M);
			return;
		    }
		}
	    }
    ...
    struct node		*p1, *p2;
{
    ...
    int		p1_son_name, p2_son_name, xp1, xp2;

    if ((p1_son = p1->body.brother.left) == (p2_son = p2->body.brother.left))
	return (TRUE);

    if ((p2_son_name = p2_son->name) != or)
	return (FALSE);

    if ((p1_son_name = p1_son->name) != or)
...
static struct node	*create_or_list (p_s, m, M)
    struct node		**p_s;
    int		m, M;
{
    register int	x;
	...
	to_be_sorted = (int*) sxalloc ((size_of_to_be_sorted = M) + 1, sizeof (int));
    else if (M > size_of_to_be_sorted)
	to_be_sorted = (int*) sxrealloc (to_be_sorted, (size_of_to_be_sorted = M) + 1, sizeof (int));
    
    for (x = 1; x <= M; x++) {
	to_be_sorted [x] = x;
    }

    sort_by_tree (to_be_sorted, m, M, less);
...
/* On canonicalise */
...

	{
		...
		if (is_a_member (l_ptr, r_ptr) /* {U} {U|V} => {U|V} */ )
			...
			q_s = tete_open (cn);
			x = 0;
			gather_op_sons (q_s, &x, rl_ptr, concat);
			p_s [xp] = NULL;
			p_s [xp + 1] = create_op (star, canon (or, l_ptr->body.brother.left, create_list (q_s, 1, cn - 1,
			     concat)), NULL);
			tete_pop (1);
		    }
		}
...
static	gen_a_loop (pred, next, loop)
    int		pred, next, loop;
{
...
static int	get_current_node (to_be_processed)
    ...
    struct node		*or_ptr;
{
...

	{
	    ...
	    p_s = (struct node***) sxcalloc ((int) (or_no + 1), sizeof (struct node**));
	    p_s [0] = NULL;
    ...
    struct node		*node_ptr;
{
    ...
    case leaf:
	if (*(node_ptr->body.string_ptr) == NUL)
	    return (NULL);
	else
	    return (node_ptr);
	...
	if (node_ptr->body.brother.left == NULL && node_ptr->body.brother.right == NULL)
	    return (NULL);
	else
	    return (node_ptr);
...
/* Les etats de l'automate sont dans [0..size]. */
 /* Automate sous forme de vecteur de bits et sa fermeture transitive */
	...
	re_ptrs_base = p = (struct node**) sxcalloc (SIZE * SIZE, sizeof (struct node*));
    ...
    in = (int*) sxcalloc (SIZE, sizeof (int));
    out = (int*) sxcalloc (SIZE, sizeof (int));
    ints = NULL;
    sets = NULL;
