...
static VOID	fill_items_set (nt_trans_set, nt_trans_stack, nt_trans)
...
static VOID	fill_look_ahead_set (items_set, la_set)
...
static BOOLEAN state_to_conflict_set (xac1, is_u, is_s, mask, t_set)
    int 		xac1, mask;
    BOOLEAN		is_u, is_s;
    register SXBA	t_set;
{
...
VOID print_conclusions (xac1, t_set)
    int xac1;
    SXBA t_set;
{
...
static VOID print_minimal_message (xac1, t_set, gk)
    int		xac1;
    SXBA	t_set;
    char	*gk;
{
...
static VOID set_conflict_kind (germe, t_set, failed_kind, is_ALL)
...
VOID solved_by_ARC (germe, arc_no, t_set)
    int germe, arc_no;
    SXBA	t_set;
{
...

    ...
    if (sxverbosep)
	sxtime (FINAL, processable_state_nb > 0 ? "failed" : "succeeded");
...

	...
	if (sxverbosep)
	    sxtime (FINAL, is_ambiguous ? "failed" : "succeeded");
	...
	if (!is_epsilon_free) {
	    ...
	    if (sxverbosep)
		sxtime (FINAL, is_not_LRpi ? (kind == NOT_LRPI_ ? "failed" : "failed (ambiguous)") : "succeeded");
	}
    }
...

	...
	if (is_systems_conflict) {
		...
		if (sxverbosep)
		    sxtime (FINAL, is_conflict > 0 ? "failed" : "succeeded");
	    }
